---
title: "Walkthrough: Binding Content Controls to Custom XML Parts"
ms.custom: na
ms.date: "10/01/2016"
...
1.  Create a Word document project with the name **EmployeeControls**. Create a new document for the solution. For more information, see [How to: Create Office Projects in Visual Studio](../VS_officedev/how-to--create-office-projects-in-visual-studio.md).  
  
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] opens the new Word document in the designer and adds the **EmployeeControls** project to **Solution Explorer**.  
...
1.  In the Word document that is hosted in the [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] designer, on the Ribbon, choose the **Insert** tab.  
...
6.  In the **Controls** group, choose the **Text** button ![PlainTextContentControl](../VS_officedev/media/plaintextcontrol.gif "PlainTextControl") to add a \<xref:Microsoft.Office.Tools.Word.PlainTextContentControl> to the first cell.  
  
7.  In the second column of the table, choose the second row (next to **Hire Date**).  
  
8.  In the **Controls** group, choose the **Date Picker** button ![DatePickerContentControl](../VS_officedev/media/datepicker.gif "DatePicker") to add a \<xref:Microsoft.Office.Tools.Word.DatePickerContentControl> to the second cell.  
  
9. In the second column of the table, choose the third row (next to **Title**).  
  
10. In the **Controls** group, choose the **Drop-Down List** button ![DropDownListContentControl](../VS_officedev/media/dropdownlist.gif "DropDownList") to add a \<xref:Microsoft.Office.Tools.Word.DropDownListContentControl> to the last cell.  
...
#### To attach the XML schema to the document ([!INCLUDE[Word_15_short](../VS_officedev/includes/word_15_short_md.md)])  
...
2.  Add the following declarations to the `ThisDocument` class. This code declares several objects that you will use to add a custom XML part to the document.  
     ...
     [!code[Trin_ContentControlXmlPartWalkthrough#1](../VS_officedev/codesnippet/CSharp/walkthrough--binding-content-controls-to-custom-xml-parts_1.cs)]
[!code[Trin_ContentControlXmlPartWalkthrough#1](../VS_officedev/codesnippet/VisualBasic/walkthrough--binding-content-controls-to-custom-xml-parts_1.vb)]  
...
3.  Add the following method to the `ThisDocument` class. This method gets the contents of the XML data file that is embedded as a resource in the assembly, and returns the contents as an XML string.  
     ...
     [!code[Trin_ContentControlXmlPartWalkthrough#3](../VS_officedev/codesnippet/CSharp/walkthrough--binding-content-controls-to-custom-xml-parts_2.cs)]
[!code[Trin_ContentControlXmlPartWalkthrough#3](../VS_officedev/codesnippet/VisualBasic/walkthrough--binding-content-controls-to-custom-xml-parts_2.vb)]  
...
4.  Add the following method to the `ThisDocument` class. The `AddCustomXmlPart` method creates a new custom XML part that contains an XML string that is passed to the method.  
     ...
     To ensure that the custom XML part is only created once, the method creates the custom XML part only if a custom XML part with a matching GUID does not already exist in the document. The first time this method is called, it saves the value of the \<xref:Microsoft.Office.Core._CustomXMLPart.Id*> property to the `employeeXMLPartID` string. The value of the `employeeXMLPartID` string is persisted in the document because it was declared by using the \<xref:Microsoft.VisualStudio.Tools.Applications.Runtime.CachedAttribute> attribute.  
  
     [!code[Trin_ContentControlXmlPartWalkthrough#4](../VS_officedev/codesnippet/CSharp/walkthrough--binding-content-controls-to-custom-xml-parts_3.cs)]
[!code[Trin_ContentControlXmlPartWalkthrough#4](../VS_officedev/codesnippet/VisualBasic/walkthrough--binding-content-controls-to-custom-xml-parts_3.vb)]  
...
1.  Add the following method to the `ThisDocument` class. This method binds each content control to an element in the custom XML part and sets the date display format of the \<xref:Microsoft.Office.Tools.Word.DatePickerContentControl>.  
     ...
     [!code[Trin_ContentControlXmlPartWalkthrough#5](../VS_officedev/codesnippet/CSharp/walkthrough--binding-content-controls-to-custom-xml-parts_4.cs)]
[!code[Trin_ContentControlXmlPartWalkthrough#5](../VS_officedev/codesnippet/VisualBasic/walkthrough--binding-content-controls-to-custom-xml-parts_4.vb)]  
...
1.  Add the following code to the `ThisDocument_Startup` method of the `ThisDocument` class. This code gets the XML string from the **employees.xml** file, adds the XML string to a new custom XML part in the document, and binds the content controls to elements in the custom XML part.  
     ...
     [!code[Trin_ContentControlXmlPartWalkthrough#2](../VS_officedev/codesnippet/CSharp/walkthrough--binding-content-controls-to-custom-xml-parts_5.cs)]
[!code[Trin_ContentControlXmlPartWalkthrough#2](../VS_officedev/codesnippet/VisualBasic/walkthrough--binding-content-controls-to-custom-xml-parts_5.vb)]  
...
-   Use all the available content controls to create a template. For more information, see [Walkthrough: Creating a Template By Using Content Controls](../VS_officedev/walkthrough--creating-a-template-by-using-content-controls.md).  
...
-   Use content controls to protect parts of a document. For more information, see [How to: Protect Parts of Documents by Using Content Controls](../VS_officedev/how-to--protect-parts-of-documents-by-using-content-controls.md).  
