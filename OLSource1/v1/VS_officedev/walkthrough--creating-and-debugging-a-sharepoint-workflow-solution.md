---
title: "Walkthrough: Creating and Debugging a SharePoint Workflow Solution"
ms.custom: na
ms.date: "10/01/2016"
...
6.  In the **Specify the site and security level for debugging** page, choose the **Deploy as a farm solution** option button, and then choose the **Finish** button to accept the trust level and default site.  
     ...
     This step sets the trust level for the solution as farm solution, the only available option for workflow projects. For more information, see [Sandboxed Solution Considerations](../VS_officedev/sandboxed-solution-considerations.md).  
...
## Testing the SharePoint Workflow Template  
 When you start the debugger, [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] deploys the workflow template to the SharePoint server and associates the workflow with the **Shared Documents** list. To test the workflow, start an instance of the workflow from a document in the **Shared Documents** list.  
...
6.  In [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)], verify that the debugger stops at the breakpoint next to the `onWorkflowActivated` method.  
