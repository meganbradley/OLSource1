---
title: "Improving the Performance of a VSTO Add-in"
ms.custom: na
ms.date: "10/01/2016"
...
1.  In the registry, set the `LoadBehavior` entry of the *Root*\Software\Microsoft\Office\\*ApplicationName*\Addins\\*Add-in ID* key to **0x10**.  
     ...
     For more information, see [Registry Entries for VSTO Add-ins](../VS_officedev/registry-entries-for-vsto-add-ins.md).  
...
-   Validating the digital signatures of the deployment manifests.  
 ...
 For more information, see [Deploying an Office Solution by Using Windows Installer](../VS_officedev/deploying-an-office-solution-by-using-windows-installer.md).  
  
##  <a name="Bypass"></a> Bypass Ribbon Reflection  
 If you build a solution by using [!INCLUDE[vs_dev11_long](../VS_officedev/includes/vs_dev11_long_md.md)], ensure that your users have installed the most recent version of the Visual Studio 2010 Tools for Office Runtime when you deploy the solution. Older versions of that runtime reflected into solution assemblies to locate ribbon customizations. This process can cause the VSTO Add-in to load more slowly.  
 ...
 [!code[Trin_Ribbon_Choose_Ribbon#1](../VS_officedev/codesnippet/VisualBasic/improving-the-performance-of-a-vsto-add-in_1.vb)]
[!code[Trin_Ribbon_Choose_Ribbon#1](../VS_officedev/codesnippet/CSharp/improving-the-performance-of-a-vsto-add-in_1.cs)]  
  
##  <a name="Perform"></a> Perform Expensive Operations in a Separate Execution Thread  
 Consider performing time-consuming tasks (such as long running tasks, database connections, or other sorts of network calls) in a separate thread. For more information, see [Threading Support in Office](../VS_officedev/threading-support-in-office.md).  
