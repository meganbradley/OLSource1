---
title: "Walkthrough: Adding Controls to a Document at Run Time in a VSTO Add-In"
ms.custom: na
ms.date: "10/01/2016"
 ...
 **Applies to:** The information in this topic applies to VSTO Add-in projects for Word 2010. For more information, see [Features Available by Office Application and Project Type](../VS_officedev/features-available-by-office-application-and-project-type.md).  
...
1.  Create an VSTO Add-in project for Word with the name **WordDynamicControls**. For more information, see [How to: Create Office Projects in Visual Studio](../VS_officedev/how-to--create-office-projects-in-visual-studio.md).  
...
2.  Add the following code to the `ThisAddIn` class. This code declares \<xref:Microsoft.Office.Tools.Word.Controls.Button> and \<xref:Microsoft.Office.Tools.Word.RichTextContentControl> objects that represent the controls that will be added to the document.  
     ...
     [!code[Trin_WordAddInDynamicControlsWalkthrough#1](../VS_officedev/codesnippet/VisualBasic/walkthrough--adding-controls-to-a-document-at-run-time-in-a-vsto-add-in_1.vb)]
[!code[Trin_WordAddInDynamicControlsWalkthrough#1](../VS_officedev/codesnippet/CSharp/walkthrough--adding-controls-to-a-document-at-run-time-in-a-vsto-add-in_1.cs)]  
...
3.  Add the following method to the `ThisAddIn` class. When the user clicks the **Add Button** check box on the Ribbon, this method adds a \<xref:Microsoft.Office.Tools.Word.Controls.Button> to the current selection on the document if the check box is selected, or removes the \<xref:Microsoft.Office.Tools.Word.Controls.Button> if the check box is cleared.  
     ...
     [!code[Trin_WordAddInDynamicControlsWalkthrough#2](../VS_officedev/codesnippet/VisualBasic/walkthrough--adding-controls-to-a-document-at-run-time-in-a-vsto-add-in_2.vb)]
[!code[Trin_WordAddInDynamicControlsWalkthrough#2](../VS_officedev/codesnippet/CSharp/walkthrough--adding-controls-to-a-document-at-run-time-in-a-vsto-add-in_2.cs)]  
...
4.  Add the following method to the `ThisAddIn` class. When the user clicks the **Add Rich Text Control** check box on the Ribbon, this method adds a \<xref:Microsoft.Office.Tools.Word.RichTextContentControl> to the current selection on the document if the check box is selected, or removes the \<xref:Microsoft.Office.Tools.Word.RichTextContentControl> if the check box is cleared.  
     ...
     [!code[Trin_WordAddInDynamicControlsWalkthrough#3](../VS_officedev/codesnippet/VisualBasic/walkthrough--adding-controls-to-a-document-at-run-time-in-a-vsto-add-in_3.vb)]
[!code[Trin_WordAddInDynamicControlsWalkthrough#3](../VS_officedev/codesnippet/CSharp/walkthrough--adding-controls-to-a-document-at-run-time-in-a-vsto-add-in_3.cs)]  
...
1.  In the ThisAddIn.cs or ThisAddIn.vb code file, add the following method to the `ThisAddIn` class. This method is an event handler for the \<xref:Microsoft.Office.Interop.Word.ApplicationEvents4_Event.DocumentBeforeSave> event. If the saved document has a \<xref:Microsoft.Office.Tools.Word.Document> host item that is associated with it, the event handler gets the host item and removes the \<xref:Microsoft.Office.Tools.Word.Controls.Button> control, if it exists.  
     ...
     [!code[Trin_WordAddInDynamicControlsWalkthrough#4](../VS_officedev/codesnippet/VisualBasic/walkthrough--adding-controls-to-a-document-at-run-time-in-a-vsto-add-in_4.vb)]
[!code[Trin_WordAddInDynamicControlsWalkthrough#4](../VS_officedev/codesnippet/CSharp/walkthrough--adding-controls-to-a-document-at-run-time-in-a-vsto-add-in_4.cs)]  
...
2.  In C#, add the following code to the `ThisAddIn_Startup` event handler. This code is required in C# to connect the `Application_DocumentBeforeSave` event handler with the \<xref:Microsoft.Office.Interop.Word.ApplicationEvents4_Event.DocumentBeforeSave> event.  
     ...
     [!code[Trin_WordAddInDynamicControlsWalkthrough#5](../VS_officedev/codesnippet/CSharp/walkthrough--adding-controls-to-a-document-at-run-time-in-a-vsto-add-in_5.cs)]  
...
1.  In the MyRibbon.cs or MyRibbon.vb code file, replace the generated `addButtonCheckBox_Click` and `addRichTextCheckBox_Click` event handlers with the following code. This code redefines these event handlers to call the `ToggleButtonOnDocument` and `ToggleRichTextControlOnDocument` methods that you added to the `ThisAddIn` class earlier in this walkthrough.  
     ...
     [!code[Trin_WordAddInDynamicControlsWalkthrough#6](../VS_officedev/codesnippet/VisualBasic/walkthrough--adding-controls-to-a-document-at-run-time-in-a-vsto-add-in_6.vb)]
[!code[Trin_WordAddInDynamicControlsWalkthrough#6](../VS_officedev/codesnippet/CSharp/walkthrough--adding-controls-to-a-document-at-run-time-in-a-vsto-add-in_6.cs)]  
...
-   For a sample that demonstrates how to add many other types of controls to a document at run time and recreate the controls when the document is reopened, see the Word Add-In Dynamic Controls Sample at [Office Development Samples and Walkthroughs](../VS_officedev/office-development-samples-and-walkthroughs.md).  
  
-   For a walkthrough that demonstrates how to add controls to a worksheet by using an VSTO Add-in for Excel, see [Walkthrough: Adding Controls to a Worksheet at Run Time in VSTO add-in Project](../VS_officedev/walkthrough--adding-controls-to-a-worksheet-at-run-time-in-vsto-add-in-project.md).  
