---
title: "Updating Form Regions in Outlook Projects that You Migrate to the .NET Framework 4 or the .NET Framework 4.5"
ms.custom: na
ms.date: "10/01/2016"
...
# Updating Form Regions in Outlook Projects that You Migrate to the .NET Framework 4 or the .NET Framework 4.5
  If the target framework of an Outlook VSTO Add-in project with a form region is changed to the [!INCLUDE[net_v40_short](../VS_officedev/includes/net_v40_short_md.md)] or later, you must make some changes to the generated form region code and to any code that instantiates certain form region classes at run time.  
  
## Updating the Generated Form Region Code  
 If the target framework of the project is changed to the [!INCLUDE[net_v40_short](../VS_officedev/includes/net_v40_short_md.md)] or later, you must change the generated form region code. The changes you make are different for form regions that you designed in Visual Studio and form regions that you imported from Outlook. For more information about the differences between these types of form regions, see [Creating Outlook Form Regions](../VS_officedev/creating-outlook-form-regions.md).  
...
3.  Modify the constructor of the form region class as shown in the following code examples.  
    ...
    ```  
     ...
     The following code example shows the constructor of a form region class in a project that targets the [!INCLUDE[net_v40_short](../VS_officedev/includes/net_v40_short_md.md)].  
...
4.  Modify the signature of the `InitializeManifest` method as shown below. Make sure that you do not modify the code in the method; this code represents form region settings that you applied in the designer. In Visual C# projects, you must expand the region that is named `Form Region Designer generated code` to see this method.  
    ...
    ```  
     ...
     The following code example shows the signature `InitializeManifest` method in a project that targets the [!INCLUDE[net_v40_short](../VS_officedev/includes/net_v40_short_md.md)].  
...
3.  Modify the constructor of the form region class as shown in the following code examples.  
    ...
    ```  
     ...
     The following code example shows the signature of the constructor of a form region class in a project that targets the [!INCLUDE[net_v40_short](../VS_officedev/includes/net_v40_short_md.md)].  
...
4.  For each line of code in the `InitializeControls` method that initializes a control in the form region class, modify the code as shown below.  
    ...
    ```  
     ...
     The following code example shows how to initialize a control in a project that targets the [!INCLUDE[net_v40_short](../VS_officedev/includes/net_v40_short_md.md)]. In this code, the \<xref:Microsoft.Office.Tools.Outlook.ImportedFormRegionBase.GetFormRegionControl*> method does not have a type parameter. You must cast the return value to the type of the control that you are initializing.  
...
## Instantiating Form Region Classes  
 You must modify any code that dynamically instantiates certain form region classes. In projects that target the .NET Framework 3.5, you can instantiate form region classes such as Microsoft.Office.Tools.Outlook.FormRegionManifest directly. In projects that target the [!INCLUDE[net_v40_short](../VS_officedev/includes/net_v40_short_md.md)] or later, these classes are interfaces that you cannot instantiate directly.  
  
 If the target framework of your project is changed to the [!INCLUDE[net_v40_short](../VS_officedev/includes/net_v40_short_md.md)] or later, you must instantiate the interfaces by using methods that are provided by the Globals.Factory property. For more information about the Globals.Factory property, see [Global Access to Objects in Office Projects](../VS_officedev/global-access-to-objects-in-office-projects.md).  
  
 The following table lists the form region types and the method to use to instantiate the types in projects that target the [!INCLUDE[net_v40_short](../VS_officedev/includes/net_v40_short_md.md)] or later.  
  
|Type|Factory method to use|  
|----------|---------------------------|  
|\<xref:Microsoft.Office.Tools.Outlook.FormRegionCustomAction>|\<xref:Microsoft.Office.Tools.Outlook.Factory.CreateFormRegionCustomAction*>|  
|\<xref:Microsoft.Office.Tools.Outlook.FormRegionInitializingEventArgs>|\<xref:Microsoft.Office.Tools.Outlook.Factory.CreateFormRegionInitializingEventArgs*>|  
|\<xref:Microsoft.Office.Tools.Outlook.FormRegionManifest>|\<xref:Microsoft.Office.Tools.Outlook.Factory.CreateFormRegionManifest*>|  
