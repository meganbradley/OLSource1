...
> [!NOTE]  
>  For an alternate version of this walkthrough that uses the client object model for SharePoint instead of its server object model, see [Walkthrough: Calling into the SharePoint Client Object Model in a Server Explorer Extension](../VS_officedev/walkthrough--calling-into-the-sharepoint-client-object-model-in-a-server-explorer-extension.md).  
...
5.  Choose the **VSIX Project** template, name the project **WebPartNode**, and then choose the **OK** button.  
     ...
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] adds the **WebPartNode** project to **Solution Explorer**.  
...
4.  In the list of project templates, choose **Class Library**, name the project **WebPartNodeExtension**, and then choose the **OK** button.  
     ...
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] adds the **WebPartNodeExtension** project to the solution and opens the default Class1 code file.  
...
5.  In the list of project templates, choose **Class Library**, name the project **WebPartCommands**, and then choose the **OK** button.  
     ...
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] adds the **WebPartCommands** project to the solution and opens the default Class1 code file.  
...
7.  In the **Default namespace** box (C#) or **Root namespace** box ([!INCLUDE[vbprvb](../VS_officedev/includes/vbprvb_md.md)]), enter **ServerExplorer.SharePointConnections.WebPartNode**.  
...
4.  Choose the arrow next to the **Add** button, and then choose **Add As Link** in the menu that appears.  
     ...
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] adds the code files to the WebPartCommands project as links. As a result, the code files are located in the WebPartNodeExtension project, but the code in the files are also compiled in the WebPartCommands project.  
...
9. In the **Default namespace** box (C#) or **Root namespace** box ([!INCLUDE[vbprvb](../VS_officedev/includes/vbprvb_md.md)]), enter **ServerExplorer.SharePointConnections.WebPartNode**.  
...
3.  Choose the **Resources** tab, and then choose the **This project does not contain a default resources file. Click here to create one** link.  
     ...
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] creates a resource file and opens it in the designer.  
...
1.  In the WebPartNodeExtension project, open the SiteNodeExtension code file, and then paste the following code into it.  
    ...
    >  After you add this code, the project will have some compile errors, but they'll go away when you add code in later steps.  
     ...
     [!code[SPExtensibility.SPExplorer.WebPartNodeWithCommands#1](../VS_officedev/codesnippet/CSharp/walkthrough--extending-server-explorer-to-display-web-parts_1.cs)]
[!code[SPExtensibility.SPExplorer.WebPartNodeWithCommands#1](../VS_officedev/codesnippet/VisualBasic/walkthrough--extending-server-explorer-to-display-web-parts_1.vb)]  
...
1.  In the WebPartNodeExtension project, open the WebPartNodeTypeProvder code file, and then paste the following code into it.  
     ...
     [!code[SPExtensibility.SPExplorer.WebPartNodeWithCommands#2](../VS_officedev/codesnippet/VisualBasic/walkthrough--extending-server-explorer-to-display-web-parts_2.vb)]
[!code[SPExtensibility.SPExplorer.WebPartNodeWithCommands#2](../VS_officedev/codesnippet/CSharp/walkthrough--extending-server-explorer-to-display-web-parts_2.cs)]  
...
1.  In the WebPartNodeExtension project, open the WebPartNodeInfo code file, and then paste the following code into it.  
     ...
     [!code[SPExtensibility.SPExplorer.WebPartNodeWithCommands#3](../VS_officedev/codesnippet/VisualBasic/walkthrough--extending-server-explorer-to-display-web-parts_3.vb)]
[!code[SPExtensibility.SPExplorer.WebPartNodeWithCommands#3](../VS_officedev/codesnippet/CSharp/walkthrough--extending-server-explorer-to-display-web-parts_3.cs)]  
...
1.  In the WebPartNodeExtension project, open the WebPartCommandIds code file, and then paste the following code into it.  
     ...
     [!code[SPExtensibility.SPExplorer.WebPartNodeWithCommands#4](../VS_officedev/codesnippet/CSharp/walkthrough--extending-server-explorer-to-display-web-parts_4.cs)]
[!code[SPExtensibility.SPExplorer.WebPartNodeWithCommands#4](../VS_officedev/codesnippet/VisualBasic/walkthrough--extending-server-explorer-to-display-web-parts_4.vb)]  
...
1.  In the WebPartCommands project, open the WebPartCommands code file, and then paste the following code into it.  
     ...
     [!code[SPExtensibility.SPExplorer.WebPartNodeWithCommands#6](../VS_officedev/codesnippet/CSharp/walkthrough--extending-server-explorer-to-display-web-parts_5.cs)]
[!code[SPExtensibility.SPExplorer.WebPartNodeWithCommands#6](../VS_officedev/codesnippet/VisualBasic/walkthrough--extending-server-explorer-to-display-web-parts_5.vb)]  
...
1.  In **Solution Explorer**, under the WebPartNode project, open the **source.extension.vsixmanifest** file in the manifest editor.  
     ...
     The source.extension.vsixmanifest file is the basis for the extension.vsixmanifest file that all VSIX packages require. For more information about this file, see [VSIX Extension Schema 1.0 Reference](assetId:///76e410ec-b1fb-4652-ac98-4a4c52e09a2b).  
...
10. In the **Type** box, enter **SharePoint.Commands.v4**.  
    ...
    > [!NOTE]  
    >  This element specifies a custom extension that you want to include in the Visual Studio extension. For more information, see [Asset Element (VSX Schema)](assetId:///9fcfc098-edc7-484b-9d4c-acd17829d737).  
...
## Testing the Extension  
 You're now ready to test the new **Web Part Gallery** node in **Server Explorer**. First, start debugging the extension in an experimental instance of Visual Studio. Then, use the new **Web Parts** node in the experimental instance of [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)].  
  
#### To start debugging the extension  
  
1.  Restart [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] with administrative credentials, and then open the WebPartNode solution.  
...
1.  In the experimental instance of [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)], on the menu bar, choose **View**, **Server Explorer**.  
...
4.  Verify that the code in the other instance of [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] stops on the breakpoint that you set earlier in the `NodeChildrenRequested` method, and then choose F5 to continue to debug the project.  
...
9. In the instance of [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] that you're debugging, verify that details about the Web Part appear in the **Properties** window.  
...
1.  In the experimental instance of [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)], on the menu bar, choose **Tools**, **Extensions and Updates**.  
...
## See Also  
 ...
 [Creating an Icon or Other Image &#40;Image Editor for Icons&#41;](../Topic/Creating%20an%20Icon%20or%20Other%20Image%20(Image%20Editor%20for%20Icons).md)  
