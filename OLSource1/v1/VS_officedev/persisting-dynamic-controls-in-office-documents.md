---
title: "Persisting Dynamic Controls in Office Documents"
ms.custom: na
ms.date: "08/09/2016"
...
# Persisting Dynamic Controls in Office Documents
 ...
 Controls that you add to documents at run time are called *dynamic controls*. For more information about dynamic controls, see [Adding Controls to Office Documents at Run Time](../VS_officedev/adding-controls-to-office-documents-at-run-time.md).  
...
## Persisting Windows Forms Controls in Documents  
 When a document is saved and then closed, the [!INCLUDE[vsto_runtime](../VS_officedev/includes/vsto_runtime_md.md)] automatically removes all dynamically created Windows Forms controls from the document. However, the behavior is different for document-level and VSTO Add-in projects.  
...
#### Removing ActiveX Wrappers When the Document Is Opened  
 ...
 [!code[Trin_WordAddInDynamicControls#11](../VS_officedev/codesnippet/VisualBasic/persisting-dynamic-controls-in-office-documents_2.vb)]
[!code[Trin_WordAddInDynamicControls#11](../VS_officedev/codesnippet/CSharp/persisting-dynamic-controls-in-office-documents_2.cs)]  
  
 Although the GetVstoObject method is used primarily to generate a new host item at run time, this method also clears all ActiveX wrappers from the document the first time it is called for a specific document. For more information about how to use the GetVstoObject method, see [Extending Word Documents and Excel Workbooks in VSTO Add-ins at Run Time](../VS_officedev/extending-word-documents-and-excel-workbooks-in-vsto-add-ins-at-run-time.md).  
...
#### Removing the Dynamic Controls Before the Document Is Closed  
 ...
 [!code[Trin_WordAddInDynamicControls#10](../VS_officedev/codesnippet/VisualBasic/persisting-dynamic-controls-in-office-documents_3.vb)]
[!code[Trin_WordAddInDynamicControls#10](../VS_officedev/codesnippet/CSharp/persisting-dynamic-controls-in-office-documents_3.cs)]  
