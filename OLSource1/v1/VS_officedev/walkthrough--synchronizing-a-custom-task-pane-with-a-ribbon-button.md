---
title: "Walkthrough: Synchronizing a Custom Task Pane with a Ribbon Button"
ms.custom: na
ms.date: "10/01/2016"
...
-   Microsoft Excel or Microsoft [!INCLUDE[Excel_15_short](../VS_officedev/includes/excel_15_short_md.md)].  
...
1.  Create an Excel Add-in project with the name **SynchronizeTaskPaneAndRibbon**, using the Excel Add-in project template. For more information, see [How to: Create Office Projects in Visual Studio](../VS_officedev/how-to--create-office-projects-in-visual-studio.md).  
  
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] opens the **ThisAddIn.cs** or **ThisAddIn.vb** code file and adds the **SynchronizeTaskPaneAndRibbon** project to **Solution Explorer**.  
  
## Adding a Toggle Button to the Ribbon  
 One of the Office application design guidelines is that users should always have control of the Office application UI. To enable users to control the custom task pane, you can add a Ribbon toggle button that shows and hides the task pane. To create a toggle button, add a **Ribbon (Visual Designer)** item to the project. The designer helps you add and position controls, set control properties, and handle control events. For more information, see [Ribbon Designer](../VS_officedev/ribbon-designer.md).  
...
3.  Add the following code to the `ThisAddIn` class. This code declares an instance of `TaskPaneControl` as a member of `ThisAddIn`.  
     ...
     [!code[Trin_TaskPaneRibbonSynchronize#1](../VS_officedev/codesnippet/CSharp/walkthrough--synchronizing-a-custom-task-pane-with-a-ribbon-button_1.cs)]
[!code[Trin_TaskPaneRibbonSynchronize#1](../VS_officedev/codesnippet/VisualBasic/walkthrough--synchronizing-a-custom-task-pane-with-a-ribbon-button_1.vb)]  
  
4.  Replace the `ThisAddIn_Startup` event handler with the following code. This code adds the `TaskPaneControl` object to the `CustomTaskPanes` field, but it does not display the custom task pane (by default, the \<xref:Microsoft.Office.Tools.CustomTaskPane.Visible*> property of the \<xref:Microsoft.Office.Tools.CustomTaskPane> class is **false**). The Visual C# code also attaches an event handler to the \<xref:Microsoft.Office.Tools.CustomTaskPane.VisibleChanged> event.  
  
     [!code[Trin_TaskPaneRibbonSynchronize#2](../VS_officedev/codesnippet/CSharp/walkthrough--synchronizing-a-custom-task-pane-with-a-ribbon-button_2.cs)]
[!code[Trin_TaskPaneRibbonSynchronize#2](../VS_officedev/codesnippet/VisualBasic/walkthrough--synchronizing-a-custom-task-pane-with-a-ribbon-button_2.vb)]  
...
5.  Add the following method to the `ThisAddIn` class. This method handles the \<xref:Microsoft.Office.Tools.CustomTaskPane.VisibleChanged> event. When the user closes the task pane by clicking the **Close** button (X), this method updates the state of the toggle button on the Ribbon.  
     ...
     [!code[Trin_TaskPaneRibbonSynchronize#3](../VS_officedev/codesnippet/CSharp/walkthrough--synchronizing-a-custom-task-pane-with-a-ribbon-button_3.cs)]
[!code[Trin_TaskPaneRibbonSynchronize#3](../VS_officedev/codesnippet/VisualBasic/walkthrough--synchronizing-a-custom-task-pane-with-a-ribbon-button_3.vb)]  
...
6.  Add the following property to the `ThisAddIn` class. This property exposes the private `myCustomTaskPane1` object to other classes. Later in this walkthrough, you will add code to the `MyRibbon` class that uses this property.  
     ...
     [!code[Trin_TaskPaneRibbonSynchronize#4](../VS_officedev/codesnippet/CSharp/walkthrough--synchronizing-a-custom-task-pane-with-a-ribbon-button_4.cs)]
[!code[Trin_TaskPaneRibbonSynchronize#4](../VS_officedev/codesnippet/VisualBasic/walkthrough--synchronizing-a-custom-task-pane-with-a-ribbon-button_4.vb)]  
...
2.  Replace the `toggleButton1_Click` event handler with the following code. When the user clicks the toggle button, this code displays or hides the custom task pane, depending on whether the toggle button is pressed or not pressed.  
     ...
     [!code[Trin_TaskPaneRibbonSynchronize#5](../VS_officedev/codesnippet/VisualBasic/walkthrough--synchronizing-a-custom-task-pane-with-a-ribbon-button_5.vb)]
[!code[Trin_TaskPaneRibbonSynchronize#5](../VS_officedev/codesnippet/CSharp/walkthrough--synchronizing-a-custom-task-pane-with-a-ribbon-button_5.cs)]  
...
-   Automate an application from a custom task pane. For more information, see [Walkthrough: Automating an Application from a Custom Task Pane](../VS_officedev/walkthrough--automating-an-application-from-a-custom-task-pane.md).  
