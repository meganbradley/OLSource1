...
##  <a name="EditDeleteControl"></a> Protecting a Content Control  
 ...
 You can also protect content controls that you add to a document at run time by using a VSTO Add-in project. For more information, see [How to: Add Content Controls to Word Documents](../VS_officedev/how-to--add-content-controls-to-word-documents.md).  
  
#### To protect a content control at design time  
  
1.  In the document that is hosted in the [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] designer, select the content control that you want to protect.  
...
1.  Set the `LockContents` property of the content control to **true** to prevent users from editing the control, and set the `LockContentControl` property to **true** to prevent users from deleting the control.  
     ...
     The following code example demonstrates using the \<xref:Microsoft.Office.Tools.Word.RichTextContentControl.LockContents*> and \<xref:Microsoft.Office.Tools.Word.RichTextContentControl.LockContentControl*> properties of two different \<xref:Microsoft.Office.Tools.Word.RichTextContentControl> objects in a document-level project. To run this code, add the code to the `ThisDocument` class in your project, and call the `AddProtectedContentControls` method from the `ThisDocument_Startup` event handler.  
  
     [!code[Trin_ContentControlHowToProtect#2](../VS_officedev/codesnippet/CSharp/how-to--protect-parts-of-documents-by-using-content-controls_1.cs)]
[!code[Trin_ContentControlHowToProtect#2](../VS_officedev/codesnippet/VisualBasic/how-to--protect-parts-of-documents-by-using-content-controls_1.vb)]  
  
     The following code example demonstrates using the \<xref:Microsoft.Office.Tools.Word.RichTextContentControl.LockContents*> and \<xref:Microsoft.Office.Tools.Word.RichTextContentControl.LockContentControl*> properties of two different \<xref:Microsoft.Office.Tools.Word.RichTextContentControl> objects in a VSTO Add-in project. To run this code, add the code to the `ThisAddIn` class in your project, and call the `AddProtectedContentControls` method from the `ThisAddIn_Startup` event handler.  
  
     [!code[Trin_WordAddInDynamicControls#14](../VS_officedev/codesnippet/VisualBasic/how-to--protect-parts-of-documents-by-using-content-controls_2.vb)]
[!code[Trin_WordAddInDynamicControls#14](../VS_officedev/codesnippet/CSharp/how-to--protect-parts-of-documents-by-using-content-controls_2.cs)]  
...
1.  In the document that is hosted in the [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] designer, select the area that you want to protect.  
...
1.  Programmatically select the area that you want to protect, and then call the \<xref:Microsoft.Office.Tools.Word.ControlCollection.AddGroupContentControl*> method to create a \<xref:Microsoft.Office.Tools.Word.GroupContentControl>.  
     ...
     [!code[Trin_ContentControlHowToProtect#1](../VS_officedev/codesnippet/CSharp/how-to--protect-parts-of-documents-by-using-content-controls_3.cs)]
[!code[Trin_ContentControlHowToProtect#1](../VS_officedev/codesnippet/VisualBasic/how-to--protect-parts-of-documents-by-using-content-controls_3.vb)]  
     ...
     [!code[Trin_WordAddInDynamicControls#15](../VS_officedev/codesnippet/VisualBasic/how-to--protect-parts-of-documents-by-using-content-controls_4.vb)]
[!code[Trin_WordAddInDynamicControls#15](../VS_officedev/codesnippet/CSharp/how-to--protect-parts-of-documents-by-using-content-controls_4.cs)]  
