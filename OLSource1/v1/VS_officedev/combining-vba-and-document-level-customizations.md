---
title: "Combining VBA and Document-Level Customizations"
ms.custom: na
ms.date: "10/01/2016"
...
-   For Word, call the \<xref:Microsoft.Office.Interop.Word._Application.Run*>method of the \<xref:Microsoft.Office.Interop.Word.Application> class.  
  
-   For Excel, call the \<xref:Microsoft.Office.Interop.Excel._Application.Run*> method of the \<xref:Microsoft.Office.Interop.Excel.Application> class.  
...
> [!NOTE]  
>  For information about using the global `missing` variable in place of optional parameters in Visual C#, see [Writing Code in Office Solutions](../VS_officedev/writing-code-in-office-solutions.md).  
...
-   You want to make services that you develop in a document-level customization available to end users who can access the services by writing VBA code in the document.  
 ...
 The Office development tools in Visual Studio provide a similar feature for VSTO Add-ins. If you are developing a VSTO Add-in, you can call code in your VSTO Add-in from other Microsoft Office solutions. For more information, see [Calling Code in VSTO Add-ins from Other Office Solutions](../VS_officedev/calling-code-in-vsto-add-ins-from-other-office-solutions.md).  
...
-   You can expose members in any public class in a Visual C# project, or members in a non-host item class in a [!INCLUDE[vbprvb](../VS_officedev/includes/vbprvb_md.md)] project, to VBA. This option provides you with more freedom to choose which classes you expose to VBA, but it also requires more manual steps.  
 ...
 For detailed instructions, see [How to: Expose Code to VBA in a Visual Basic Project](../VS_officedev/how-to--expose-code-to-vba-in-a-visual-basic-project.md) and [How to: Expose Code to VBA in a Visual C&#35; Project](../VS_officedev/how-to--expose-code-to-vba-in-a-visual-csharp-project.md).  
  
 The **EnableVbaCallers** and **ReferenceAssemblyFromVbaProject** properties are available only in the **Properties** window at design time; they cannot be used at run time. To view the properties, open the designer for a host item in [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)]. For more information about the specific tasks that Visual Studio performs when you set these properties, see [Tasks Performed by the Host Item Properties](#PropertyTasks).  
...
-   For [!INCLUDE[vbprvb](../VS_officedev/includes/vbprvb_md.md)] projects in which you expose members of a host item class by using the **EnableVbaCallers** property, Visual Studio also adds a property named `CallVSTOAssembly` to the `ThisDocument`, `ThisWorkbook`, `Sheet1`, `Sheet2`, or `Sheet3` module in the VBA project.  
...
##  <a name="Guidelines"></a> Guidelines for Adding VBA Code to the Document  
 ...
 If you want to save the VBA code that you add to the document while debugging or running the solution, copy the VBA code into the document in the project folder. For more information about the build process, see [Building Office Solutions](../VS_officedev/building-office-solutions.md).  
...
2.  It adds a reference to the following type libraries in the VBA project in the document:  
    ...
    -   The Microsoft Visual Studio Tools for Office Execution Engine 9.0 Type Library. This type library is included in the [!INCLUDE[vsto_runtime](../VS_officedev/includes/vsto_runtime_md.md)].  
