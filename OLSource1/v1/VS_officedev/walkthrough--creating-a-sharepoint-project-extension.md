...
# Walkthrough: Creating a SharePoint Project Extension
  This walkthrough illustrates how to create an extension for SharePoint projects. You can use a project extension to respond to project-level events such as when a project is added, deleted, or renamed. You can also add custom properties or respond when a property value changes. Unlike project item extensions, project extensions cannot be associated with a particular SharePoint project type. When you create a project extension, the extension loads when any kind of SharePoint project is opened in [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)].  
  
 In this walkthrough, you will create a custom Boolean property that is added to any SharePoint project created in [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)]. When set to **True**, the new property adds, or maps, an Images resource folder to your project. When set to **False**, the Images folder is removed, if it exists. For more information, see [How to: Add and Remove Mapped Folders](../VS_officedev/how-to--add-and-remove-mapped-folders.md).  
  
 This walkthrough demonstrates the following tasks:  
  
-   Creating a [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] extension for SharePoint projects that does the following:  
    ...
    -   Uses the [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] automation object model (DTE) to delete a mapped folder from the project.  
  
-   Building a [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] Extension (VSIX) package to deploy the project property's extension assembly.  
...
-   Supported editions of [!INCLUDE[TLA#tla_win](../VS_officedev/includes/tlasharptla_win_md.md)], SharePoint and [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)]. For more information, see [Requirements for Developing SharePoint Solutions](../VS_officedev/requirements-for-developing-sharepoint-solutions.md).  
  
-   The [!INCLUDE[vssdk_current_long](../VS_officedev/includes/vssdk_current_long_md.md)]. This walkthrough uses the **VSIX Project** template in the [!INCLUDE[TLA2#tla_sdk](../VS_officedev/includes/tla2sharptla_sdk_md.md)] to create a VSIX package to deploy the project property extension. For more information, see [Extending the SharePoint Tools in Visual Studio](../VS_officedev/extending-the-sharepoint-tools-in-visual-studio.md).  
...
1.  In **Solution Explorer**, open the shortcut menu for the solution node, choose **Add**, and then choose **New Project**.  
    ...
    > [!NOTE]  
    >  In [!INCLUDE[vbprvb](../VS_officedev/includes/vbprvb_md.md)] projects, the solution node appears in **Solution Explorer** only if the **Always show solution** check box is selected in the [NIB: General, Projects and Solutions, Options Dialog Box](assetId:///8f8e37e8-b28d-4b13-bfeb-ea4d3312aeca).  
...
4.  In the **Name** box, enter **ProjectExtension**, and then choose the **OK** button.  
     ...
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] adds the **ProjectExtension** project to the solution and opens the default Class1 code file.  
...
## Testing the Project Property  
 You're now ready to test the custom project property. It's easiest to debug and test the new project property extension in an experimental instance of [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)]. This instance of [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] is created when you run a VSIX or other extensibility project. After you debug the project, you can install the extension on your system and then continue to debug and test it in a regular instance of [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)].  
  
#### To debug and test the extension in an experimental instance of Visual Studio  
  
1.  Restart [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] with administrative credentials, and then open the ProjectExtensionPackage solution.  
  
2.  Start a debug build of your project either by choosing the **F5** key or, on the menu bar, choosing **Debug**, **Start Debugging**.  
  
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] installs the extension to %UserProfile%\AppData\Local\Microsoft\VisualStudio\11.0Exp\Extensions\Contoso\Custom Project Property\1.0 and starts an experimental instance of [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)].  
  
3.  In the experimental instance of [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)], create a SharePoint project for a farm solution, and use the default values for the other values in the wizard.  
  
    1.  On the menu bar, choose **File**, **New**, **Project**.  
  
    2.  At the top of the **New Project** dialog box, choose **.NET Framework 3.5** in the list of versions of the .NET Framework.  
  
         SharePoint tool extensions require features in this version of the [!INCLUDE[dnprdnshort](../VS_officedev/includes/dnprdnshort_md.md)].  
...
7.  Close the experimental instance of [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)].  
