---
title: "Programming Document-Level Customizations"
ms.custom: na
ms.date: "10/01/2016"
...
-   Customize the user interface (UI) of the application.  
 ...
 For general information about document-level customizations and other types of solutions you can create by using the Office development tools in Visual Studio, see [Office Solutions Development Overview &#40;VSTO&#41;](../VS_officedev/office-solutions-development-overview--vsto-.md).  
...
-   `Sheet` *n*: derives from \<xref:Microsoft.Office.Tools.Excel.WorksheetBase>.  
 ...
 These base classes redirect all calls to their members to internal implementations of the corresponding host item interfaces in the [!INCLUDE[vsto_runtime](../VS_officedev/includes/vsto_runtime_md.md)]. For example, if you call the \<xref:Microsoft.Office.Tools.Word.DocumentBase.Protect*> method of the `ThisDocument` class, the \<xref:Microsoft.Office.Tools.Word.DocumentBase> class redirects this call to the internal implementation of the \<xref:Microsoft.Office.Tools.Word.Document> interface in the [!INCLUDE[vsto_runtime](../VS_officedev/includes/vsto_runtime_md.md)].  
...
```  
 ...
 Because the `ThisDocument` class obtains most of its members from the \<xref:Microsoft.Office.Tools.Word.Document> host item, the `Save` method that is called in this code is really the \<xref:Microsoft.Office.Tools.Word.Document.Save*> method of the \<xref:Microsoft.Office.Tools.Word.Document> host item. This method corresponds to the \<xref:Microsoft.Office.Interop.Word._Document.Save*> method of the \<xref:Microsoft.Office.Interop.Word.Document> object in the Word object model.  
  
 For more information about using the object models of Word and Excel, see [Word Object Model Overview](../VS_officedev/word-object-model-overview.md) and [Excel Object Model Overview](../VS_officedev/excel-object-model-overview.md).  
  
 For more information about the `Globals` object, see [Global Access to Objects in Office Projects](../VS_officedev/global-access-to-objects-in-office-projects.md).  
...
## Adding Controls to Documents  
 ...
 For more information, see [Host Items and Host Controls Overview](../VS_officedev/host-items-and-host-controls-overview.md) and [Windows Forms Controls on Office Documents Overview](../VS_officedev/windows-forms-controls-on-office-documents-overview.md).  
...
## Combining VBA and Document-Level Customizations  
 ...
 For more information, see [Combining VBA and Document-Level Customizations](../VS_officedev/combining-vba-and-document-level-customizations.md).  
...
## Managing Documents on a Server  
 ...
 For more information, see [Managing Documents on a Server by Using the ServerDocument Class](../VS_officedev/managing-documents-on-a-server-by-using-the-serverdocument-class.md).  
...
-   Add host controls or Windows Forms controls to the document surface.  
     ...
     For more information, see [Automating Word by Using Extended Objects](../VS_officedev/automating-word-by-using-extended-objects.md), [Automating Excel by Using Extended Objects](../VS_officedev/automating-excel-by-using-extended-objects.md), and [Windows Forms Controls on Office Documents Overview](../VS_officedev/windows-forms-controls-on-office-documents-overview.md).  
  
-   Add an actions pane to the document.  
  
     For more information, see [Actions Pane Overview](../VS_officedev/actions-pane-overview.md).  
  
-   Add custom tabs to the ribbon.  
  
     For more information, see [Ribbon Overview](../VS_officedev/ribbon-overview.md).  
  
-   Add custom groups to a built-in tab on the ribbon.  
  
     For more information, see [How to: Customize a Built-in Tab](../VS_officedev/how-to--customize-a-built-in-tab.md).  
  
 For more information about customizing the UI of Microsoft Office applications, see [Office UI Customization](../VS_officedev/office-ui-customization.md).  
...
## Getting Extended Objects from Native Office Objects in Document-Level Customizations  
 ...
 When you have a native Office object, you can test whether that object has been extended into a *host item* or *host control* in a document-level customization. Host items and host controls are types provided by the [!INCLUDE[vsto_runtime](../VS_officedev/includes/vsto_runtime_md.md)] that add functionality to objects that exist natively in the Word or Excel object models (called *native Office objects*). Collectively, host items and host controls are also called *extended objects*. For more information about host items and host controls, see [Host Items and Host Controls Overview](../VS_officedev/host-items-and-host-controls-overview.md).  
