...
## Prerequisites  
 To perform this walkthrough, you must first create the CustomActionProjectItem solution by completing [Walkthrough: Creating a Custom Action Project Item with an Item Template, Part 1](../VS_officedev/walkthrough--creating-a-custom-action-project-item-with-an-item-template--part-1.md).  
...
5.  Choose the **WPF User Control Library** project template, name the project **ItemTemplateWizard**, and then choose the **OK** button.  
     ...
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] adds the **ItemTemplateWizard** project to the solution.  
...
2.  In the **ItemTemplateWizard** project, replace the code in the Strings code file with the following code.  
     ...
     [!code[SPExtensibility.ProjectItem.CustomAction#6](../VS_officedev/codesnippet/CSharp/walkthrough--creating-a-custom-action-project-item-with-an-item-template--part-2_1.cs)]
[!code[SPExtensibility.ProjectItem.CustomAction#6](../VS_officedev/codesnippet/VisualBasic/walkthrough--creating-a-custom-action-project-item-with-an-item-template--part-2_1.vb)]  
...
2.  In the XAML view, replace the current XAML with the following XAML. The XAML defines a UI that includes a heading, controls for specifying the behavior of the custom action, and navigation buttons at the bottom of the window.  
    ...
    >  Your project will have some compile errors after you add this code. These errors will go away when you add code in later steps.  
     ...
     [!code[SPExtensibility.ProjectItem.CustomAction#9](../VS_officedev/codesnippet/Xaml/walkthrough--creating-a-custom-action-project-item-with-an-item-template--part-2_2.xaml)]  
  
    > [!NOTE]  
    >  The window that's created in this XAML is derived from the \<xref:Microsoft.VisualStudio.PlatformUI.DialogWindow> base class. When you add a custom WPF dialog box to Visual Studio, we recommend that you derive your dialog box from this class to have consistent styling with other dialog boxes in Visual Studio and to avoid issues that might otherwise occur with modal dialog boxes. For more information, see [Creating and Managing Modal Dialog Boxes](../Topic/Creating%20and%20Managing%20Modal%20Dialog%20Boxes.md).  
...
4.  In the code-behind file for the WizardWindow.xaml file, replace the current code with the following code.  
     ...
     [!code[SPExtensibility.ProjectItem.CustomAction#7](../VS_officedev/codesnippet/VisualBasic/walkthrough--creating-a-custom-action-project-item-with-an-item-template--part-2_3.vb)]
[!code[SPExtensibility.ProjectItem.CustomAction#7](../VS_officedev/codesnippet/CSharp/walkthrough--creating-a-custom-action-project-item-with-an-item-template--part-2_3.cs)]  
...
1.  In the **ItemTemplateWizard** project, open the **CustomActionWizard** code file, and then replace the current code in this file with the following code:  
     ...
     [!code[SPExtensibility.ProjectItem.CustomAction#8](../VS_officedev/codesnippet/CSharp/walkthrough--creating-a-custom-action-project-item-with-an-item-template--part-2_4.cs)]
[!code[SPExtensibility.ProjectItem.CustomAction#8](../VS_officedev/codesnippet/VisualBasic/walkthrough--creating-a-custom-action-project-item-with-an-item-template--part-2_4.vb)]  
...
## Adding Replaceable Parameters to the Elements.xml File in the Item Template  
 ...
 A replaceable parameter is a token that starts and ends with the dollar sign ($) character. In addition to defining your own replaceable parameters, you can use built-in parameters that the SharePoint project system defines and initializes. For more information, see [Replaceable Parameters](../VS_officedev/replaceable-parameters.md).  
