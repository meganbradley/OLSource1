...
-   Creating two custom SharePoint commands that the extension assembly calls. SharePoint commands are methods that can be called by extension assemblies to use APIs in the server object model for SharePoint. For more information, see [Calling into the SharePoint Object Models](../VS_officedev/calling-into-the-sharepoint-object-models.md).  
...
-   The Visual Studio SDK. This walkthrough uses the **VSIX Project** template in the SDK to create a VSIX package to deploy the extension. For more information, see [Extending the SharePoint Tools in Visual Studio](../VS_officedev/extending-the-sharepoint-tools-in-visual-studio.md).  
...
5.  Choose the **VSIX Project** template, name the project **UpgradeDeploymentStep**, and then choose the **OK** button.  
     ...
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] adds the **UpgradeDeploymentStep** project to **Solution Explorer**.  
...
4.  Choose the **Class Library** project template, name the project **DeploymentStepExtension**, and then choose the **OK** button.  
     ...
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] adds the **DeploymentStepExtension** project to the solution and opens the default Class1 code file.  
...
1.  In **Solution Explorer**, open the shortcut menu for the UpgradeDeploymentStep solution node, choose **Add**, and then choose **New Project**.  
    ...
    > [!NOTE]  
    >  In Visual Basic projects, the solution node only appears in **Solution Explorer** when the **Always show solution** check box is selected in the [NIB: General, Projects and Solutions, Options Dialog Box](assetId:///8f8e37e8-b28d-4b13-bfeb-ea4d3312aeca).  
...
1.  In the **DeploymentStepExtension** project, open the UpgradeStep code file, and then paste the following code into it.  
    ...
    >  After you add this code, the project will have some compile errors, but they'll go away when you add code in later steps.  
     ...
     [!code[SPExtensibility.ProjectExtension.UpgradeDeploymentStep#1](../VS_officedev/codesnippet/CSharp/walkthrough--creating-a-custom-deployment-step-for-sharepoint-projects_1.cs)]
[!code[SPExtensibility.ProjectExtension.UpgradeDeploymentStep#1](../VS_officedev/codesnippet/VisualBasic/walkthrough--creating-a-custom-deployment-step-for-sharepoint-projects_1.vb)]  
...
2.  In the **DeploymentStepExtension** project, open the DeploymentConfigurationExtension code file, and then paste the following code into it.  
     ...
     [!code[SPExtensibility.ProjectExtension.UpgradeDeploymentStep#2](../VS_officedev/codesnippet/CSharp/walkthrough--creating-a-custom-deployment-step-for-sharepoint-projects_2.cs)]
[!code[SPExtensibility.ProjectExtension.UpgradeDeploymentStep#2](../VS_officedev/codesnippet/VisualBasic/walkthrough--creating-a-custom-deployment-step-for-sharepoint-projects_2.vb)]  
...
1.  In the **SharePointCommands** project, open the Commands code file, and then paste the following code into it.  
     ...
     [!code[SPExtensibility.ProjectExtension.UpgradeDeploymentStep#4](../VS_officedev/codesnippet/CSharp/walkthrough--creating-a-custom-deployment-step-for-sharepoint-projects_3.cs)]
[!code[SPExtensibility.ProjectExtension.UpgradeDeploymentStep#4](../VS_officedev/codesnippet/VisualBasic/walkthrough--creating-a-custom-deployment-step-for-sharepoint-projects_3.vb)]  
...
1.  In **Solution Explorer**, under the **UpgradeDeploymentStep** project, open the shortcut menu for the **source.extension.vsixmanifest** file, and then choose **Open**.  
     ...
     Visual Studio opens the file in the manifest editor. The source.extension.vsixmanifest file is the basis for the extension.vsixmanifest file that all VSIX packages require. For more information about this file, see [VSIX Extension Schema 1.0 Reference](assetId:///76e410ec-b1fb-4652-ac98-4a4c52e09a2b).  
...
3.  At the top of the dialog box, make sure that **.NET Framework 3.5** appears in the list of versions of the .NET Framework.  
     ...
     Projects for [!INCLUDE[wss_14_long](../VS_officedev/includes/wss_14_long_md.md)] and [!INCLUDE[moss_14_long](../VS_officedev/includes/moss_14_long_md.md)] require this version of the .NET Framework.  
...
7.  Choose the **Finish** button.  
     ...
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] creates the EmployeesListDefinition project.  
...
11. On the **Choose List Settings** page, verify the following settings, and then choose the **Finish** button:  
    ...
    3.  **Default (Blank)** is chosen in the **Create a customizable list based on:** list.  
     ...
     [!INCLUDE[vsprvs](../VS_officedev/includes/vsprvs_md.md)] creates the Employees List item with a Title column and a single empty instance and opens the List Designer.  
