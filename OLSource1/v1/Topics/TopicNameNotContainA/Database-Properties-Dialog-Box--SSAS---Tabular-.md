...
|**DirectQueryMode**|Specifies whether the database uses only in\-memory storage \(no disk storage\), only disk\-based storage, or some combination of the two. Valid values are InMemory, DirectQuery, InMemoryWithDirectQuery \(mostly memory\-based with some paging to disk\), or DirectQueryWithInMemory \(mostly disk\-based with some in\-memory storage\). For more information, see [DirectQuery for Tabular 1100 or 1103 models &#40;SSAS Tabular&#41;](../Topic/DirectQuery%20for%20Tabular%201100%20or%201103%20models%20\(SSAS%20Tabular\).md).|  
