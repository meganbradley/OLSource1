...
|Property|Data type|Description|  
|--------------|---------------|-----------------|  
|ExpectBOM|Boolean|A value that specifies whether the Import Column transformation expects a byte\-order mark \(BOM\). A BOM is only expected if the data has the DT\_NTEXT data type.|  
