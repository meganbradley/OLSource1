...
##  <a name="DatabaseSnapshot"></a> Database Snapshots  
 [!INCLUDE[ssNoVersion](../../Token\Other/ssNoVersion_md.md)] does not support [database snapshots](../Topic/Database%20Snapshots%20\(SQL%20Server\).md) for FILESTREAM filegroups. If a FILESTREAM filegroup is included in a CREATE DATABASE ON clause, the statement will fail and an error will be raised.  
...
##  <a name="LogShipping"></a> Log Shipping  
 [Log shipping](../Topic/About%20Log%20Shipping%20\(SQL%20Server\).md) supports FILESTREAM. Both the primary and secondary servers must be running [!INCLUDE[ssKatmai](../../Token\Other/ssKatmai_md.md)], or a later version, and have FILESTREAM enabled.  
