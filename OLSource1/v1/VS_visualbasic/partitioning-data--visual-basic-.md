...
Partitioning in LINQ refers to the operation of dividing an input sequence into two sections, without rearranging the elements, and then returning one of the sections.  
 ...
 ![LINQ Partitioning Operations](../VS_csharp/media/linq_partition.png "LINQ_Partition")  
...
### Skip  
 The following code example uses the `Skip` clause in [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] to skip over the first four strings in an array of strings before returning the remaining strings in the array.  
  
 [!code[CsLINQPartitioning#1](../VS_csharp/codesnippet/VisualBasic/partitioning-data--visual-basic-_1.vb)]  
  
### SkipWhile  
 The following code example uses the `Skip While` clause in [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] to skip over the strings in an array while the first letter of the string is "a". The remaining strings in the array are returned.  
  
 [!code[CsLINQPartitioning#2](../VS_csharp/codesnippet/VisualBasic/partitioning-data--visual-basic-_2.vb)]  
  
### Take  
 The following code example uses the `Take` clause in [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] to return the first two strings in an array of strings.  
  
 [!code[CsLINQPartitioning#3](../VS_csharp/codesnippet/VisualBasic/partitioning-data--visual-basic-_3.vb)]  
  
### TakeWhile  
 The following code example uses the `Take While` clause in [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] to return strings from an array while the length of the string is five or less.  
  
 [!code[CsLINQPartitioning#4](../VS_csharp/codesnippet/VisualBasic/partitioning-data--visual-basic-_4.vb)]  
