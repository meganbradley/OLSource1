...
In [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)], two variable references are considered identical if their pointers are the same, that is, if both variables point to the same class instance in memory. For example, in a Windows Forms application, you might want to make a comparison to determine whether the current instance (`Me`) is the same as a particular instance, such as `Form2`.  
  
 [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] provides two operators to compare pointers. The [Is Operator (Visual Basic)](../VS_csharp/is-operator--visual-basic-.md) returns `True` if the objects are identical, and the [IsNot Operator](../VS_csharp/isnot-operator--visual-basic-.md) returns `True` if they are not.  
...
## Example  
 ...
 [!code[VbVbalrKeywords#14](../VS_csharp/codesnippet/VisualBasic/how-to--determine-whether-two-objects-are-identical--visual-basic-_1.vb)]  
