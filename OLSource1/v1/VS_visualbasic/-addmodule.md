...
## Remarks  
 ...
 If you specify (implicitly or explicitly) any[/target (Visual Basic)](../VS_visualbasic/-target--visual-basic-.md) option other than `/target:module` with `/addmodule`, the files you pass to `/addmodule` become part of the project's assembly. An assembly is required to run an output file that has one or more files added with `/addmodule`.  
  
 Use [/reference (Visual Basic)](../VS_visualbasic/-reference--visual-basic-.md) to import metadata from a file that contains an assembly.  
...
## Example  
 ...
 [!code[VbVbalrCompiler#47](../VS_visualbasic/codesnippet/VisualBasic/-addmodule_1.vb)]  
  
 The following code imports the module's types.  
  
 [!code[VbVbalrCompiler#48](../VS_visualbasic/codesnippet/VisualBasic/-addmodule_2.vb)]  
