---
title: "Miscellaneous Operators (Visual Basic)"
ms.custom: na
ms.date: "10/03/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-visual-basic"
ms.tgt_pltfrm: na
ms.topic: "article"
dev_langs: 
  - "VB"
helpviewer_keywords: 
  - "operators [Visual Basic]"
  - "operators [Visual Basic], miscellaneous"
ms.assetid: 2423b3c5-fc3f-479c-bcd2-2c6ebe92814f
caps.latest.revision: 17
ms.author: "shoag"
manager: "wpickett"
translation.priority.ht: 
  - "cs-cz"
  - "de-de"
  - "es-es"
  - "fr-fr"
  - "it-it"
  - "ja-jp"
  - "ko-kr"
  - "pl-pl"
  - "pt-br"
  - "ru-ru"
  - "tr-tr"
  - "zh-cn"
  - "zh-tw"
---
# Miscellaneous Operators (Visual Basic)
The following are miscellaneous operators defined in [!INCLUDE[vbprvb](../VS_visualbasic/includes/vbprvb_md.md)].  
  
 [AddressOf Operator](../VS_visualbasic/addressof-operator--visual-basic-.md)  
  
 [Await Operator](../VS_visualbasic/await-operator--visual-basic-.md)  
  
 [GetType Operator](../VS_visualbasic/gettype-operator--visual-basic-.md)  
  
 [Function Expression](../VS_visualbasic/function-expression--visual-basic-.md)  
  
 [If Operator](../VS_visualbasic/if-operator--visual-basic-.md)  
  
 [TypeOf Operator](../VS_visualbasic/typeof-operator--visual-basic-.md)  
  
## See Also  
 [Operators Listed by Functionality](../VS_visualbasic/operators-listed-by-functionality--visual-basic-.md)