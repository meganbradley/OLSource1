...
A *procedure* is a block of [!INCLUDE[vbprvb](../VS_visualbasic/includes/vbprvb_md.md)] statements enclosed by a declaration statement (`Function`, `Sub`, `Operator`, `Get`, `Set`) and a matching `End` declaration. All executable statements in [!INCLUDE[vbprvb](../VS_visualbasic/includes/vbprvb_md.md)] must be within some procedure.  
...
## Returning from a Procedure  
 A procedure returns control to the calling code when it has finished running. To do this, it can use a [Return Statement](../VS_visualbasic/return-statement--visual-basic-.md), the appropriate [Exit Statement](../VS_visualbasic/exit-statement--visual-basic-.md) statement for the procedure, or the procedure's [End \<keyword> Statement](../VS_visualbasic/end--keyword--statement--visual-basic-.md) statement. Control then passes to the calling code following the point of the procedure call.  
...
## Types of Procedures  
 [!INCLUDE[vbprvb](../VS_visualbasic/includes/vbprvb_md.md)] uses several types of procedures:  
  
-   [Sub Procedures](../VS_visualbasic/sub-procedures--visual-basic-.md) perform actions but do not return a value to the calling code.  
...
-   [Function Procedures](../VS_visualbasic/function-procedures--visual-basic-.md) return a value to the calling code. They can perform other actions before returning.  
  
-   [Property Procedures](../VS_visualbasic/property-procedures--visual-basic-.md) return and assign values of properties on objects or modules.  
  
-   [Operator Procedures](../VS_visualbasic/operator-procedures--visual-basic-.md) define the behavior of a standard operator when one or both of the operands is a newly-defined class or structure.  
  
-   [Generic Procedures in Visual Basic](../VS_visualbasic/generic-procedures-in-visual-basic.md) define one or more *type parameters* in addition to their normal parameters, so the calling code can pass specific data types each time it makes a call.  
