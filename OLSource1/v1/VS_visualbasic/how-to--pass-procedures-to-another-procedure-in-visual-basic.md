...
This example shows how to use delegates to pass a procedure to another procedure.  
 ...
 A delegate is a type that you can use like any other type in [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)]. The `AddressOf` operator returns a delegate object when applied to a procedure name.  
...
1.  Create a delegate named `MathOperator`.  
     ...
     [!code[VbVbalrDelegates#1](../VS_csharp/codesnippet/VisualBasic/how-to--pass-procedures-to-another-procedure-in-visual-basic_1.vb)]  
...
2.  Create a procedure named `AddNumbers` with parameters and return value that match those of `MathOperator`, so that the signatures match.  
     ...
     [!code[VbVbalrDelegates#2](../VS_csharp/codesnippet/VisualBasic/how-to--pass-procedures-to-another-procedure-in-visual-basic_2.vb)]  
  
3.  Create a procedure named `SubtractNumbers` with a signature that matches `MathOperator`.  
  
     [!code[VbVbalrDelegates#3](../VS_csharp/codesnippet/VisualBasic/how-to--pass-procedures-to-another-procedure-in-visual-basic_3.vb)]  
...
4.  Create a procedure named `DelegateTest` that takes a delegate as a parameter.  
     ...
     [!code[VbVbalrDelegates#4](../VS_csharp/codesnippet/VisualBasic/how-to--pass-procedures-to-another-procedure-in-visual-basic_4.vb)]  
...
5.  Create a procedure named `Test` that calls `DelegateTest` once with the delegate for `AddNumbers` as a parameter, and again with the delegate for `SubtractNumbers` as a parameter.  
     ...
     [!code[VbVbalrDelegates#5](../VS_csharp/codesnippet/VisualBasic/how-to--pass-procedures-to-another-procedure-in-visual-basic_5.vb)]  
