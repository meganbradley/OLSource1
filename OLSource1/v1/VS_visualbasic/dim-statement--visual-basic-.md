...
-   `Static`  
     ...
     Optional. See [Static](../VS_visualbasic/static--visual-basic-.md).  
  
-   `ReadOnly`  
  
     Optional. See [ReadOnly](../VS_visualbasic/readonly--visual-basic-.md).  
  
-   `WithEvents`  
  
     Optional. Specifies that these are object variables that refer to instances of a class that can raise events. See [WithEvents](../VS_visualbasic/withevents--visual-basic-.md).  
...
-   `variablelist`  
    ...
    |-|-|  
    |Part|Description|  
    |`variablename`|Required. Name of the variable. See [Declared Element Names](../VS_visualbasic/declared-element-names--visual-basic-.md).|  
...
```  
 ...
 You can declare a variable in a procedure, block, class, structure, or module. You cannot declare a variable in a source file, namespace, or interface. For more information, see [Declaration Contexts and Default Access Levels](../VS_visualbasic/declaration-contexts-and-default-access-levels--visual-basic-.md).  
 ...
 The following access modifiers are used to declare variables outside a procedure: `Public`, `Protected`, `Friend`, `Protected Friend`, and `Private`. For more information, see [Access Levels in Visual Basic](../VS_visualbasic/access-levels-in-visual-basic.md).  
...
```  
 ...
 If `Option Explicit` is on (the default), the compiler requires a declaration for every variable you use. For more information, see [Option Explicit Statement](../VS_visualbasic/option-explicit-statement--visual-basic-.md).  
...
```  
 ...
 Type inference applies at the procedure level. It does not apply outside a procedure in a class, structure, module, or interface. For more information about type inference, see [Option Infer Statement](../VS_visualbasic/option-infer-statement.md) and [Local Type Inference](../VS_visualbasic/local-type-inference--visual-basic-.md).  
  
 For information about what happens when a data type or initializer is not specified, see [Default Data Types and Values](../VS_visualbasic/dim-statement--visual-basic-.md#default) later in this topic.  
...
```  
 ...
 For more information about object initializers, see [How to: Declare an Object by Using an Object Initializer](../VS_visualbasic/how-to--declare-an-object-by-using-an-object-initializer--visual-basic-.md), [Object Initializers: Named and Anonymous Types](../VS_visualbasic/object-initializers--named-and-anonymous-types--visual-basic-.md), and [Anonymous Types](../VS_visualbasic/anonymous-types--visual-basic-.md).  
...
## Arrays  
 You can declare a variable to hold an *array*, which can hold multiple values. To specify that a variable holds an array, follow its `variablename` immediately with parentheses. For more information about arrays, see [Arrays](../VS_visualbasic/arrays-in-visual-basic.md).  
...
```  
 ...
 For more information about array literals, see [Arrays](../VS_visualbasic/arrays-in-visual-basic.md).  
...
|No|No|`Dim qty`|If [Option Strict](../VS_visualbasic/option-strict-statement.md) is off (the default), the variable is set to `Nothing`.<br /><br /> If `Option Strict` is on, a compile-time error occurs.|  
|No|Yes|`Dim qty = 5`|If [Option Infer](../VS_visualbasic/option-infer-statement.md) is on (the default), the variable takes the data type of the initializer. See [Local Type Inference](../VS_visualbasic/local-type-inference--visual-basic-.md).<br /><br /> If `Option Infer` is off and `Option Strict` is off, the variable takes the data type of `Object`.<br /><br /> If `Option Infer` is off and `Option Strict` is on, a compile-time error occurs.|  
...
|Yes|Yes|`Dim qty  As Integer = 5`|If the data type of the initializer is not convertible to the specified data type, a compile-time error occurs.|  
 ...
 If you specify a data type but do not specify an initializer, [!INCLUDE[vbprvb](../VS_visualbasic/includes/vbprvb_md.md)] initializes the variable to the default value for its data type. The following table shows the default initialization values.  
...
## Attributes and Modifiers  
 ...
 You can specify `WithEvents` only on member variables, not on local variables inside a procedure. If you specify `WithEvents`, the data type of the variable must be a specific class type, not `Object`. You cannot declare an array with `WithEvents`. For more information about events, see [Events](../VS_visualbasic/events--visual-basic-.md).  
...
## Releasing Managed Resources  
 ...
 The `Using` statement automates the process of acquiring a resource, executing a set of statements, and then disposing of the resource. However, the resource must implement the \<xref:System.IDisposable> interface. For more information, see [Using Statement](../VS_visualbasic/using-statement--visual-basic-.md).  
  
## Example  
 The following example declares variables by using the `Dim` statement with various options.  
  
 [!code[VbVbalrStatements#141](../VS_visualbasic/codesnippet/VisualBasic/dim-statement--visual-basic-_1.vb)]  
...
## Example  
 ...
 [!code[VbVbalrStatements#142](../VS_visualbasic/codesnippet/VisualBasic/dim-statement--visual-basic-_2.vb)]  
...
## Example  
 ...
 [!code[VbVbalrStatements#144](../VS_visualbasic/codesnippet/VisualBasic/dim-statement--visual-basic-_3.vb)]  
  
 [!code[VbVbalrStatements#145](../VS_visualbasic/codesnippet/VisualBasic/dim-statement--visual-basic-_4.vb)]  
...
## See Also  
 ...
 [Object Initializers: Named and Anonymous Types](../VS_visualbasic/object-initializers--named-and-anonymous-types--visual-basic-.md)   
 [How to: Declare an Object by Using an Object Initializer](../VS_visualbasic/how-to--declare-an-object-by-using-an-object-initializer--visual-basic-.md)   
