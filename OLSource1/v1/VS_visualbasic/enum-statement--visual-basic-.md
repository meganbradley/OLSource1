...
-   `attributelist`  
     ...
     Optional. List of attributes that apply to this enumeration. You must enclose the [attribute list](../VS_visualbasic/attribute-list--visual-basic-.md) in angle brackets ("`<`" and "`>`").  
...
-   `Shadows`  
     ...
     Optional. Specifies that this enumeration redeclares and hides an identically named programming element, or set of overloaded elements, in a base class. You can specify [Shadows](../VS_visualbasic/shadows--visual-basic-.md) only on the enumeration itself, not on any of its members.  
  
-   `enumerationname`  
  
     Required. Name of the enumeration. For information on valid names, see [Declared Element Names](../VS_visualbasic/declared-element-names--visual-basic-.md).  
...
-   Ensures forward compatibility. If you use enumerations, your code is less likely to fail if in the future someone changes the values corresponding to the member names.  
 ...
 Member enumerations can be accessed from anywhere within their class, structure, module, or interface. Code outside a class, structure, or module must qualify a member enumeration's name with the name of that class, structure, or module. You can avoid the need to use fully qualified names by adding an [Imports](../VS_visualbasic/imports-statement--.net-namespace-and-type-.md) statement to the source file.  
 ...
 The *declaration context* for an enumeration must be a source file, namespace, class, structure, module, or interface, and cannot be a procedure. For more information, see [Declaration Contexts and Default Access Levels](../VS_visualbasic/declaration-contexts-and-default-access-levels--visual-basic-.md).  
...
## Example  
 ...
 [!code[VbEnumsTask#41](../VS_visualbasic/codesnippet/VisualBasic/enum-statement--visual-basic-_1.vb)]  
  
## Example  
 The method in the following example is outside the `Egg` class. Therefore, `EggSizeEnum` is fully qualified as `Egg.EggSizeEnum`.  
  
 [!code[VbEnumsTask#42](../VS_visualbasic/codesnippet/VisualBasic/enum-statement--visual-basic-_2.vb)]  
...
## Example  
 ...
 [!code[VbEnumsTask#30](../VS_visualbasic/codesnippet/VisualBasic/enum-statement--visual-basic-_3.vb)]  
  
## Example  
 The following example shows values that include both positive and negative numbers.  
  
 [!code[VbEnumsTask#31](../VS_visualbasic/codesnippet/VisualBasic/enum-statement--visual-basic-_4.vb)]  
  
## Example  
 In the following example, an `As` clause is used to specify the `datatype` of an enumeration.  
  
 [!code[VbEnumsTask#6](../VS_visualbasic/codesnippet/VisualBasic/enum-statement--visual-basic-_5.vb)]  
...
## Example  
 ...
 [!code[VbEnumsTask#61](../VS_visualbasic/codesnippet/VisualBasic/enum-statement--visual-basic-_6.vb)]  
...
## Example  
 ...
 [!code[VbEnumsTask#51](../VS_visualbasic/codesnippet/VisualBasic/enum-statement--visual-basic-_7.vb)]  
