...
The [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] compiler calls the Assembly Linker (Al.exe, also known as Alink) to generate an assembly with a manifest, with the linker reporting an error in the emission stage of creating the assembly.  
...
6.  In [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)], add a .NET Assembly reference to the file you just created.  
