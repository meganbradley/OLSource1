...
The [!INCLUDE[vbprvb](../VS_visualbasic/includes/vbprvb_md.md)] compiler performs a process called `binding` when an object is assigned to an object variable. An object is *early bound* when it is assigned to a variable declared to be of a specific object type. Early bound objects allow the compiler to allocate memory and perform other optimizations before an application executes. For example, the following code fragment declares a variable to be of type \<xref:System.IO.FileStream>:  
  
 [!code[VbVbalrOOP#90](../VS_visualbasic/codesnippet/VisualBasic/early-and-late-binding--visual-basic-_1.vb)]  
