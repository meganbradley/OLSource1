...
This is the second of two topics that demonstrate how to work with events. The first topic, [Walkthrough: Declaring and Raising Events](../VS_visualbasic/walkthrough--declaring-and-raising-events--visual-basic-.md), shows how to declare and raise events. This section uses the form and class from that walkthrough to show how to handle events when they take place.  
  
 The `Widget` class example uses traditional event-handling statements. [!INCLUDE[vbprvb](../VS_visualbasic/includes/vbprvb_md.md)] provides other techniques for working with events. As an exercise, you can modify this example to use the `AddHandler` and `Handles` statements.  
  
### To handle the PercentDone event of the Widget class  
  
1.  Place the following code in `Form1`:  
  
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#4](../VS_visualbasic/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_1.vb)]  
...
3.  Add the following code to the `mWidget_PercentDone` event handler:  
     ...
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#5](../VS_visualbasic/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_2.vb)]  
...
4.  Add the following code for the `Button2_Click` event handler:  
     ...
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#6](../VS_visualbasic/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_3.vb)]  
...
3.  Add the following code for the `Form1_Load` event procedure to create the `Widget`:  
     ...
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#7](../VS_visualbasic/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_4.vb)]  
  
 When this code executes, [!INCLUDE[vbprvb](../VS_visualbasic/includes/vbprvb_md.md)] creates a `Widget` object and connects its events to the event procedures associated with `mWidget`. From that point on, whenever the `Widget` raises its `PercentDone` event, the `mWidget_PercentDone` event procedure is executed.  
  
#### To call the LongTask method  
  
-   Add the following code to the `Button1_Click` event handler:  
  
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#8](../VS_visualbasic/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_5.vb)]  
...
-   Add the following line of code to the `Button1_Click` procedure, immediately preceding the line that reads `mWidget.LongTask(12.2, 0.33)`:  
     ...
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#9](../VS_visualbasic/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_6.vb)]  
...
## See Also  
 [Walkthrough: Declaring and Raising Events](../VS_visualbasic/walkthrough--declaring-and-raising-events--visual-basic-.md)   
