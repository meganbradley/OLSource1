...
5.  Add this element to that `<sharedListeners>` section:  
    ...
    > [!NOTE]
    >  Typically, an application writes only errors to the event log. For information on filtering log output, see [Walkthrough: Filtering My.Application.Log Output](../VS_visualbasic/walkthrough--filtering-my.application.log-output--visual-basic-.md).  
  
### To write event information to the event log  
  
-   Use the `My.Application.Log.WriteEntry` or `My.Application.Log.WriteException` method to write information to the event log. For more information, see [How to: Write Log Messages](../VS_visualbasic/how-to--write-log-messages--visual-basic-.md) and [How to: Log Exceptions](../VS_visualbasic/how-to--log-exceptions-in-visual-basic.md).  
