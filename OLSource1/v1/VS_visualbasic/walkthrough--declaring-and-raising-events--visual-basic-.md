...
This walkthrough demonstrates how to declare and raise events for a class named `Widget`. After you complete the steps, you might want to read the companion topic, [Walkthrough: Handling Events](../VS_visualbasic/walkthrough--handling-events--visual-basic-.md), which shows how to use events from `Widget` objects to provide status information in an application.  
...
1.  Open a new [!INCLUDE[vbprvb](../VS_visualbasic/includes/vbprvb_md.md)] Windows Application project and create a form named `Form1`.  
...
-   Use the `Event` keyword to declare an event in the `Widget` class. Note that an event can have `ByVal` and `ByRef` arguments, as `Widget`'s `PercentDone` event demonstrates:  
     ...
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#1](../VS_visualbasic/codesnippet/VisualBasic/walkthrough--declaring-and-raising-events--visual-basic-_1.vb)]  
...
1.  To simplify access to the `Timer` property used by this class, add an `Imports` statement to the top of the declarations section of your class module, above the `Class Widget` statement.  
     ...
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#2](../VS_visualbasic/codesnippet/VisualBasic/walkthrough--declaring-and-raising-events--visual-basic-_2.vb)]  
  
2.  Add the following code to the `Widget` class:  
  
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#3](../VS_visualbasic/codesnippet/VisualBasic/walkthrough--declaring-and-raising-events--visual-basic-_3.vb)]  
 ...
 Now that the `Widget` class can raise events, you can move to the next walkthrough. [Walkthrough: Handling Events](../VS_visualbasic/walkthrough--handling-events--visual-basic-.md) demonstrates how to use `WithEvents` to associate an event handler with the `PercentDone` event.  
  
## See Also  
 \<xref:Microsoft.VisualBasic.DateAndTime.Timer*>   
 \<xref:Microsoft.VisualBasic.DateAndTime.Now*>   
 [Walkthrough: Handling Events](../VS_visualbasic/walkthrough--handling-events--visual-basic-.md)   
