...
Variables used in [!INCLUDE[vbteclinqext](../VS_visualbasic/includes/vbteclinqext_md.md)] query operations are strongly typed and must be compatible with each other. Strong typing is used in the data source, in the query itself, and in the query execution. The following illustration identifies terms used to describe a [!INCLUDE[vbteclinq](../VS_visualbasic/includes/vbteclinq_md.md)] query. For more information about the parts of a query, see [Basic Query Operations (Visual Basic)](../VS_visualbasic/basic-query-operations--visual-basic-.md).  
  
 ![Pseudocode query with elements highlighted.](../VS_visualbasic/media/sjltyperels.png "SJLtypeRels")  
Parts of a LINQ query  
 ...
 [!INCLUDE[vbprvb](../VS_visualbasic/includes/vbprvb_md.md)] makes strong typing convenient by implementing local type inference, also known as *implicit typing*. That feature is used in the previous example, and you will see it used throughout the [!INCLUDE[vbteclinq](../VS_visualbasic/includes/vbteclinq_md.md)] samples and documentation. In Visual Basic, local type inference is accomplished simply by using a `Dim` statement without an `As` clause. In the following example, `city` is strongly typed as a string.  
  
 [!code[VbLINQTypeRels#1](../VS_visualbasic/codesnippet/VisualBasic/type-relationships-in-query-operations--visual-basic-_1.vb)]  
  
> [!NOTE]
>  Local type inference works only when `Option Infer` is set to `On`. For more information, see [Option Infer Statement](../VS_visualbasic/option-infer-statement.md).  
  
 However, even if you use local type inference in a query, the same type relationships are present among the variables in the data source, the query variable, and the query execution loop. It is useful to have a basic understanding of these type relationships when you are writing [!INCLUDE[vbteclinq](../VS_visualbasic/includes/vbteclinq_md.md)] queries, or working with the samples and code examples in the documentation.  
  
 You may need to specify an explicit type for a range variable that does not match the type returned from the data source. You can specify the type of the range variable by using an `As` clause. However, this results in an error if the conversion is a [narrowing conversion](../VS_visualbasic/widening-and-narrowing-conversions--visual-basic-.md) and `Option Strict` is set to `On`. Therefore, we recommend that you perform the conversion on the values retrieved from the data source. You can convert the values from the data source to the explicit range variable type by using the \<xref:System.Linq.Enumerable.Cast*> method. You can also cast the values selected in the `Select` clause to an explicit type that is different from the type of the range variable. These points are illustrated in the following code.  
  
 [!code[VbLINQTypeRels#4](../VS_visualbasic/codesnippet/VisualBasic/type-relationships-in-query-operations--visual-basic-_2.vb)]  
  
## Queries That Return Entire Elements of the Source Data  
 The following example shows a [!INCLUDE[vbteclinq](../VS_visualbasic/includes/vbteclinq_md.md)] query operation that returns a sequence of elements selected from the source data. The source, `names`, contains an array of strings, and the query output is a sequence containing strings that start with the letter M.  
  
 [!code[VbLINQTypeRels#2](../VS_visualbasic/codesnippet/VisualBasic/type-relationships-in-query-operations--visual-basic-_3.vb)]  
 ...
 [!code[VbLINQTypeRels#3](../VS_visualbasic/codesnippet/VisualBasic/type-relationships-in-query-operations--visual-basic-_4.vb)]  
