...
-   Use the line-continuation character, which is an underscore (`_`), at the point at which you want the line to break. The underscore must be immediately preceded by a space and immediately followed by a line terminator (carriage return).  
    ...
    > [!NOTE]
    >  In some cases, if you omit the line-continuation character, the Visual Basic compiler will implicitly continue the statement on the next line of code. For a list of syntax elements for which you can omit the line-continuation character, see "Implicit Line Continuation" in [Statements](../VS_visualbasic/statements-in-visual-basic.md).  
     ...
     [!code[VbVbcnConventions#20](../VS_visualbasic/codesnippet/VisualBasic/how-to--break-and-combine-statements-in-code--visual-basic-_1.vb)]  
 ...
 Although placing each statement on a separate line is the recommended method, [!INCLUDE[vbprvb](../VS_visualbasic/includes/vbprvb_md.md)] also allows you to place multiple statements on the same line.  
  
### To place multiple statements on the same line  
  
-   Separate the statements with a colon (`:`), as in the following example.  
  
     [!code[VbVbcnConventions#10](../VS_visualbasic/codesnippet/VisualBasic/how-to--break-and-combine-statements-in-code--visual-basic-_2.vb)]  
