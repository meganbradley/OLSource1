...
When you call a procedure, each argument you supply corresponds to one of the parameters defined in the procedure. In some cases, the procedure code can change the value underlying an argument in the calling code. In other cases, the procedure can change only its local copy of an argument.  
 ...
 When you call the procedure, [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] makes a local copy of every argument that is passed [ByVal](../VS_csharp/byval--visual-basic-.md). For each argument passed [ByRef](../VS_csharp/byref--visual-basic-.md), [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] gives the procedure code a direct reference to the programming element underlying the argument in the calling code.  
...
1.  In the procedure declaration, specify [ByRef](../VS_csharp/byref--visual-basic-.md) for the parameter corresponding to the argument.  
...
1.  In the procedure declaration, specify [ByVal](../VS_csharp/byval--visual-basic-.md) for the parameter corresponding to the argument.  
  
     -or-  
  
     In the calling code, enclose the argument in parentheses in the argument list. This forces [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] to pass the argument by value, even if the corresponding parameter specifies `ByRef`.  
...
## Example  
 ...
 [!code[VbVbcnProcedures#35](../VS_csharp/codesnippet/VisualBasic/how-to--change-the-value-of-a-procedure-argument--visual-basic-_1.vb)]  
  
 [!code[VbVbcnProcedures#36](../VS_csharp/codesnippet/VisualBasic/how-to--change-the-value-of-a-procedure-argument--visual-basic-_2.vb)]  
  
 [!code[VbVbcnProcedures#37](../VS_csharp/codesnippet/VisualBasic/how-to--change-the-value-of-a-procedure-argument--visual-basic-_3.vb)]  
 ...
 You can prevent the procedure from modifying the variable itself in the calling code. See [How to: Protect a Procedure Argument Against Value Changes](../VS_csharp/how-to--protect-a-procedure-argument-against-value-changes--visual-basic-.md).  
  
## Compiling the Code  
 When you pass a variable by reference, you must use the `ByRef` keyword to specify this mechanism.  
  
 The default in [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] is to pass arguments by value. However, it is good programming practice to include either the [ByVal](../VS_csharp/byval--visual-basic-.md) or [ByRef](../VS_csharp/byref--visual-basic-.md) keyword with every declared parameter. This makes your code easier to read.  
...
## See Also  
 ...
 [Differences Between Passing an Argument By Value and By Reference](../VS_csharp/differences-between-passing-an-argument-by-value-and-by-reference--visual-basic-.md)   
