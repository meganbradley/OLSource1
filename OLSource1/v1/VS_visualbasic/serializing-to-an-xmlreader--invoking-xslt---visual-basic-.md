...
When you use the \<xref:System.Xml?displayProperty=fullName> interoperability capabilities of [!INCLUDE[sqltecxlinq](../VS_visualbasic/includes/sqltecxlinq_md.md)], you can use \<xref:System.Xml.Linq.XNode.CreateReader*> to create an \<xref:System.Xml.XmlReader>. The module that reads from this \<xref:System.Xml.XmlReader> reads the nodes from the XML tree and processes them accordingly.  
