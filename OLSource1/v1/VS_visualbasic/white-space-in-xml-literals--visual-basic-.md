...
The [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] compiler incorporates only the significant white space characters from an XML literal when it creates a [!INCLUDE[sqltecxlinq](../VS_csharp/includes/sqltecxlinq_md.md)] object. The insignificant white space characters are not incorporated.  
...
-   When they are in an embedded expression for an element's text content.  
 ...
 Otherwise, the compiler treats white space characters as insignificant and does not include then in the [!INCLUDE[sqltecxlinq](../VS_csharp/includes/sqltecxlinq_md.md)] object for the literal.  
...
> [!NOTE]
>  If the `xml:space` attribute appears in an XML element literal, the [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] compiler includes the attribute in the <xref:System.Xml.Linq.XElement*> object, but adding this attribute does not change how the compiler treats white space.  
...
## Examples  
 ...
 [!code[VbXMLSamples#29](../VS_csharp/codesnippet/VisualBasic/white-space-in-xml-literals--visual-basic-_1.vb)]  
