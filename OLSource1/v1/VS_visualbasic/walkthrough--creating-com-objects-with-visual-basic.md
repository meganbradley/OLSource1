...
When creating new applications or components, it is best to create .NET Framework assemblies. However, [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] also makes it easy to expose a .NET Framework component to COM. This enables you to provide new components for earlier application suites that require COM components. This walkthrough demonstrates how to use [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] to expose [!INCLUDE[dnprdnshort](../VS_csharp/includes/dnprdnshort_md.md)] objects as COM objects, both with and without the COM class template.  
...
> [!NOTE]
>  Although you can also expose a class created in [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] as a COM object for unmanaged code to use, it is not a true COM object and cannot be used by [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)]. For more information, see [COM Interoperability in .NET Framework Applications](../VS_csharp/com-interoperability-in-.net-framework-applications--visual-basic-.md).  
...
4.  Select **COM Class** from the **Templates** list, and then click **Add**. [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] adds a new class and configures the new project for COM interop.  
  
5.  Add code such as properties, methods, and events to the COM class.  
  
6.  Select **Build ClassLibrary1** from the **Build** menu. [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] builds the assembly and registers the COM object with the operating system.  
...
3.  Add the following constants to `ComClass1`. They will store the Globally Unique Identifier (GUID) constants that the COM objects are required to have.  
     ...
     [!code[VbVbalrInterop#2](../VS_csharp/codesnippet/VisualBasic/walkthrough--creating-com-objects-with-visual-basic_1.vb)]  
...
5.  Replace the empty string for the `ClassId` with the GUID, removing the leading and trailing braces. For example, if the GUID provided by Guidgen is `"{2C8B0AEE-02C9-486e-B809-C780A11530FE}"` then your code should appear as follows.  
     ...
     [!code[VbVbalrInterop#3](../VS_csharp/codesnippet/VisualBasic/walkthrough--creating-com-objects-with-visual-basic_2.vb)]  
  
6.  Repeat the previous steps for the `InterfaceId` and `EventsId` constants, as in the following example.  
  
     [!code[VbVbalrInterop#4](../VS_csharp/codesnippet/VisualBasic/walkthrough--creating-com-objects-with-visual-basic_3.vb)]  
...
7.  Add the `ComClass` attribute to `ComClass1`, specifying the GUIDs for the Class ID, Interface ID, and Events ID as in the following example:  
     ...
     [!code[VbVbalrInterop#5](../VS_csharp/codesnippet/VisualBasic/walkthrough--creating-com-objects-with-visual-basic_4.vb)]  
...
8.  COM classes must have a parameterless `Public Sub New()` constructor, or the class will not register correctly. Add a parameterless constructor to the class:  
     ...
     [!code[VbVbalrInterop#6](../VS_csharp/codesnippet/VisualBasic/walkthrough--creating-com-objects-with-visual-basic_5.vb)]  
  
9. Add properties, methods, and events to the class, ending it with an `End Class` statement. Select **Build Solution** from the **Build** menu. [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] builds the assembly and registers the COM object with the operating system.  
  
    > [!NOTE]
    >  The COM objects you generate with [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] cannot be used by other [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] applications because they are not true COM objects. Attempts to add references to such COM objects will raise an error. For details, see [COM Interoperability in .NET Framework Applications](../VS_csharp/com-interoperability-in-.net-framework-applications--visual-basic-.md).  
...
## See Also  
 ...
 [#Region Directive](../VS_csharp/sharpregion-directive.md)   
 [COM Interoperability in .NET Framework Applications](../VS_csharp/com-interoperability-in-.net-framework-applications--visual-basic-.md)   
