...
The *scope* of a declared element is the set of all code that can refer to it without qualifying its name or making it available through an [Imports Statement (.NET Namespace and Type)](../VS_visualbasic/imports-statement--.net-namespace-and-type-.md). An element can have scope at one of the following levels:  
...
-   The access level you declare for the element  
 ...
 Use care when you define variables with the same name but different scope, because doing so can lead to unexpected results. For more information, see [References to Declared Elements](../VS_visualbasic/references-to-declared-elements--visual-basic-.md).  
...
### Procedure Scope  
 An element declared within a procedure is not available outside that procedure. Only the procedure that contains the declaration can use it. Variables at this level are also known as *local variables*. You declare them with the [Dim Statement](../VS_visualbasic/dim-statement--visual-basic-.md), with or without the [Static](../VS_visualbasic/static--visual-basic-.md) keyword.  
...
> [!NOTE]
>  All local elements, even if they are `Static` variables, are private to the procedure in which they appear. You cannot declare any element using the [Public](../VS_visualbasic/public--visual-basic-.md) keyword within a procedure.  
...
### Module Scope  
 ...
 Elements for which you declare [Private](../VS_visualbasic/private--visual-basic-.md) access level are available to every procedure in that module, but not to any code in a different module. The `Dim` statement at module level defaults to `Private` if you do not use any access level keywords. However, you can make the scope and access level more obvious by using the `Private` keyword in the `Dim` statement.  
...
### Namespace Scope  
 If you declare an element at module level using the [Friend](../VS_visualbasic/friend--visual-basic-.md) or [Public](../VS_visualbasic/public--visual-basic-.md) keyword, it becomes available to all procedures throughout the namespace in which the element is declared. With the following alteration to the preceding example, the string variable `strMsg` can be referred to by code anywhere in the namespace of its declaration.  
...
```  
 ...
 If your project does not contain any [Namespace Statement](../VS_visualbasic/namespace-statement.md)s, everything in the project is in the same namespace. In this case, namespace scope can be thought of as project scope. `Public` elements in a module, class, or structure are also available to any project that references their project.  
...
-   **Memory Consumption.** Local variables consume memory only while their procedure is running. Their memory is released when the procedure returns to the calling code. By contrast, [Shared](../VS_visualbasic/shared--visual-basic-.md) and [Static](../VS_visualbasic/static--visual-basic-.md) variables consume memory resources until your application stops running, so use them only when necessary. *Instance variables* consume memory while their instance continues to exist, which makes them less efficient than local variables, but potentially more efficient than `Shared` or `Static` variables.  
  
### Minimizing Scope  
 In general, when declaring any variable or constant, it is good programming practice to make the scope as narrow as possible (block scope is the narrowest). This helps conserve memory and minimizes the chances of your code erroneously referring to the wrong variable. Similarly, you should declare a variable to be [Static](../VS_visualbasic/static--visual-basic-.md) only when it is necessary to preserve its value between procedure calls.  
  
## See Also  
 [Declared Element Characteristics](../VS_visualbasic/declared-element-characteristics--visual-basic-.md)   
 [How to: Control the Scope of a Variable](../VS_visualbasic/how-to--control-the-scope-of-a-variable--visual-basic-.md)   
