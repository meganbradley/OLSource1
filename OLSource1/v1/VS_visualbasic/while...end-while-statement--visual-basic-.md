...
## Remarks  
 Use a `While...End While` structure when you want to repeat a set of statements an indefinite number of times, as long as a condition remains `True`. If you want more flexibility with where you test the condition or what result you test it for, you might prefer the [Do...Loop Statement](../VS_visualbasic/do...loop-statement--visual-basic-.md). If you want to repeat the statements a set number of times, the [For...Next Statement](../VS_visualbasic/for...next-statement--visual-basic-.md) is usually a better choice.  
  
> [!NOTE]
>  The `While` keyword is also used in the [Do...Loop Statement](../VS_visualbasic/do...loop-statement--visual-basic-.md), the [Skip While Clause](../VS_visualbasic/skip-while-clause--visual-basic-.md) and the [Take While Clause](../VS_visualbasic/take-while-clause--visual-basic-.md).  
 ...
 The `condition` usually results from a comparison of two values, but it can be any expression that evaluates to a [Boolean Data Type](../VS_visualbasic/boolean-data-type--visual-basic-.md) value (`True` or `False`). This expression can include a value of another data type, such as a numeric type, that has been converted to `Boolean`.  
  
 You can nest `While` loops by placing one loop within another. You can also nest different kinds of control structures within one another. For more information, see [Nested Control Structures](../VS_visualbasic/nested-control-structures--visual-basic-.md).  
  
## Exit While  
 The [Exit While](../VS_visualbasic/exit-statement--visual-basic-.md) statement can provide another way to exit a `While` loop. `Exit While` immediately transfers control to the statement that follows the `End While` statement.  
 ...
 The `Continue While` statement immediately transfers control to the next iteration of the loop. For more information, see [Continue Statement](../VS_visualbasic/continue-statement--visual-basic-.md).  
  
## Example  
 In the following example, the statements in the loop continue to run until the `index` variable is greater than 10.  
  
 [!code[VbVbalrStatements#171](../VS_visualbasic/codesnippet/VisualBasic/while...end-while-statement--visual-basic-_1.vb)]  
  
## Example  
 The following example illustrates the use of the `Continue While` and `Exit While` statements.  
  
 [!code[VbVbalrStatements#172](../VS_visualbasic/codesnippet/VisualBasic/while...end-while-statement--visual-basic-_2.vb)]  
...
## Example  
 ...
 [!code[VbVbalrStatements#173](../VS_visualbasic/codesnippet/VisualBasic/while...end-while-statement--visual-basic-_3.vb)]  
...
## See Also  
 ...
 [Continue Statement](../VS_visualbasic/continue-statement--visual-basic-.md)