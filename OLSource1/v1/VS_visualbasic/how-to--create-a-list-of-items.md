---
title: "How to: Create a List of Items"
ms.custom: na
ms.date: "10/03/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-visual-basic"
ms.tgt_pltfrm: na
ms.topic: "article"
dev_langs: 
  - "VB"
helpviewer_keywords: 
  - "list [LINQ in Visual Basic]"
  - "objects [Visual Basic], list of items"
ms.assetid: fe941aba-6340-455c-8b1f-ffd9c3eb1ac5
caps.latest.revision: 29
ms.author: "shoag"
manager: "wpickett"
translation.priority.ht: 
  - "de-de"
  - "es-es"
  - "fr-fr"
  - "it-it"
  - "ja-jp"
  - "ko-kr"
  - "ru-ru"
  - "zh-cn"
  - "zh-tw"
translation.priority.mt: 
  - "cs-cz"
  - "pl-pl"
  - "pt-br"
  - "tr-tr"
---
# How to: Create a List of Items
The code in this topic defines a `Student` class and creates a list of instances of the class. The list is designed to support the topic [Walkthrough: Writing Queries in Visual Basic](../VS_visualbasic/walkthrough--writing-queries-in-visual-basic.md). It also can be used for any application that requires a list of objects. The code defines the items in the list of students by using object initializers.  
  
## Example  
 If you are working on the walkthrough, you can use this code for the Module1.vb file of the project that is created there. Just replace the lines marked with **** in the `Main` method with the queries and query executions that are provided in the walkthrough.  
  
 [!code[VbLINQHowToCreateList#1](../VS_visualbasic/codesnippet/VisualBasic/how-to--create-a-list-of-items_1.vb)]  
  
## See Also  
 [Walkthrough: Writing Queries in Visual Basic](../VS_visualbasic/walkthrough--writing-queries-in-visual-basic.md)   
 [Getting Started with LINQ in Visual Basic](../VS_visualbasic/getting-started-with-linq-in-visual-basic.md)   
 [Object Initializers: Named and Anonymous Types](../VS_visualbasic/object-initializers--named-and-anonymous-types--visual-basic-.md)   
 [Introduction to LINQ in Visual Basic](../VS_visualbasic/introduction-to-linq-in-visual-basic.md)   
 [LINQ](../VS_visualbasic/linq-in-visual-basic.md)   
 [Queries](../VS_visualbasic/queries--visual-basic-.md)