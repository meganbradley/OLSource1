...
|`name`|Required. Name of this interface. See [Declared Element Names](../VS_visualbasic/declared-element-names--visual-basic-.md).|  
|`Of`|Optional. Specifies that this is a generic interface.|  
|`typelist`|Required if you use the [Of](../VS_visualbasic/of-clause--visual-basic-.md) keyword. List of type parameters for this interface. Optionally, each type parameter can be declared variant by using `In` and `Out` generic modifiers. See [Type List](../VS_visualbasic/type-list--visual-basic-.md).|  
|`Inherits`|Optional. Indicates that this interface inherits the attributes and members of another interface or interfaces. See [Inherits Statement](../VS_visualbasic/inherits-statement.md).|  
...
## Remarks  
 ...
 A class or structure implements the interface by supplying code for every member defined by the interface. Finally, when the application creates an instance from that class or structure, an object exists and runs in memory. For more information, see [Objects and Classes](../VS_visualbasic/objects-and-classes-in-visual-basic.md) and [Interfaces](../VS_visualbasic/interfaces--visual-basic-.md).  
  
 You can use `Interface` only at namespace or module level. This means the *declaration context* for an interface must be a source file, namespace, class, structure, module, or interface, and cannot be a procedure or block. For more information, see [Declaration Contexts and Default Access Levels](../VS_visualbasic/declaration-contexts-and-default-access-levels--visual-basic-.md).  
  
 Interfaces default to [Friend](../VS_visualbasic/friend--visual-basic-.md) access. You can adjust their access levels with the access modifiers. For more information, see [Access Levels in Visual Basic](../VS_visualbasic/access-levels-in-visual-basic.md).  
...
-   **Member Modifiers.** You cannot use any access modifiers when defining module members, nor can you specify [Shared](../VS_visualbasic/shared--visual-basic-.md) or any procedure modifier except [Overloads](../VS_visualbasic/overloads--visual-basic-.md). You can declare any member with [Shadows](../VS_visualbasic/shadows--visual-basic-.md), and you can use [Default](../VS_visualbasic/default--visual-basic-.md) when defining a property, as well as [ReadOnly](../VS_visualbasic/readonly--visual-basic-.md) or [WriteOnly](../VS_visualbasic/writeonly--visual-basic-.md).  
  
-   **Inheritance.** If the interface uses the [Inherits Statement](../VS_visualbasic/inherits-statement.md), you can specify one or more base interfaces. You can inherit from two interfaces even if they each define a member with the same name. If you do so, the implementing code must use name qualification to specify which member it is implementing.  
...
-   **Implementation.** When a class uses the [Implements](../VS_visualbasic/implements-clause--visual-basic-.md) statement to implement this interface, it must implement every member defined within the interface. Furthermore, each signature in the implementing code must exactly match the corresponding signature defined in this interface. However, the name of the member in the implementing code does not have to match the member name as defined in the interface.  
  
     When a class is implementing a procedure, it cannot designate the procedure as `Shared`.  
  
-   **Default Property.** An interface can specify at most one property as its *default property*, which can be referenced without using the property name. You specify such a property by declaring it with the [Default](../VS_visualbasic/default--visual-basic-.md) modifier.  
  
     Notice that this means that an interface can define a default property only if it inherits none.  
  
## Behavior  
  
-   **Access Level.** All interface members implicitly have [Public](../VS_visualbasic/public--visual-basic-.md) access. You cannot use any access modifier when defining a member. However, a class implementing the interface can declare an access level for each implemented member.  
     ...
     [!code[VbVbalrStatements#39](../VS_visualbasic/codesnippet/VisualBasic/interface-statement--visual-basic-_1.vb)]  
...
## Example  
 ...
 [!code[VbVbalrStatements#40](../VS_visualbasic/codesnippet/VisualBasic/interface-statement--visual-basic-_2.vb)]  
