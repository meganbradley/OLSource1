...
-   The *colorizer* implements the <xref:Microsoft.VisualStudio.TextManager.Interop.IVsColorizer*> interface.  
 ...
 ![Language Service Model graphic](../VS_csharp/media/vslanguageservicemodel.gif "vsLanguageServiceModel")  
Basic language service model  
  
 The document window hosts the *document view* of the editor, in this case the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] core editor. The document view and the text buffer are owned by the editor. These objects work with [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] through a specialized document window called a *code window*. The code window is contained in an <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowFrame*> object that is created and controlled by the IDE.  
...
|Code window|<xref:Microsoft.VisualStudio.TextManager.Interop.VsCodeWindow*>|A document window that contains one or more text views. When [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] is in multiple-document interface (MDI) mode, the code window is an MDI child.|  
