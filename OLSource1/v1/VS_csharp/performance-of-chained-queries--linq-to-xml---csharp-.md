...
-   In `query2`, both the `where` clause and the `select` clause have the same performance profile as in `query1`.  
 ...
 For more information on iterators, see [yield (C# Reference)](../VS_csharp/yield--csharp-reference-.md).  
  
 For a more detailed tutorial on chaining queries together, see [Chaining Queries Together](../VS_csharp/tutorial--chaining-queries-together.md).  
