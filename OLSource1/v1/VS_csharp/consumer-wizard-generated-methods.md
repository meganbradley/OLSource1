...
```  
 ...
 The following example shows how you can call `OpenAll` and `CloseAll` when you execute the same command repeatedly. Compare the code example in [CCommand::Close](../VS_csharp/ccommand--close.md), which shows a variation that calls **Close** and `ReleaseCommand` instead of `CloseAll`.  
...
## Remarks  
 The wizard defines the methods `OpenDataSource` and `CloseDataSource`; `OpenDataSource` calls [CDataSource::OpenFromInitializationString](../VS_csharp/cdatasource--openfrominitializationstring.md).  
