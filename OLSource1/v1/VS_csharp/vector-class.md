...
## Remarks  
 Vectors allow constant time insertions and deletions at the end of the sequence. Inserting or deleting elements in the middle of a vector requires linear time. The performance of the [deque Class](../VS_csharp/deque-class.md) container is superior with respect to insertions and deletions at the beginning and end of a sequence. The [list Class](../VS_csharp/list-class.md) container is superior with respect to insertions and deletions at any location within a sequence.  
 ...
 The [vector<bool\> Class](../VS_csharp/vector-bool--class.md) is a full specialization of the template class vector for elements of type bool with an allocator for the underlying type used by the specialization.  
  
 The [vector<bool\> reference Class](../VS_csharp/vector-bool--class.md#vector_lt_bool_gt___reference_class) is a nested class whose objects are able to provide references to elements (single bits) within a vector<bool\> object.  
...
### Remarks  
 ...
 When compiling with _SECURE_SCL 1, a runtime error will occur if you attempt to access an element in an empty vector.  See [Checked Iterators](../VS_csharp/checked-iterators.md) for more information.  
...
### Return Value  
 A const reverse random-access iterator addressing the first element in a reversed [vector](../VS_csharp/vector-class.md) or addressing what had been the last element in the unreversed `vector`.  
...
### Return Value  
 A const reverse random-access iterator that addresses the location succeeding the last element in a reversed [vector](../VS_csharp/vector-class.md) (the location that had preceded the first element in the unreversed `vector`).  
...
### Return Value  
 A pointer to the first element in the [vector](../VS_csharp/vector-class.md) or to the location succeeding an empty `vector`.  
...
|`_Where`|The position in the [vector](../VS_csharp/vector-class.md) where the first element is inserted.|  
...
### Remarks  
 Any insertion operation can be expensive, see [vector Class](../VS_csharp/vector-class.md) for a discussion of `vector` performance.  
...
### Remarks  
 ...
 When compiling with _SECURE_SCL 1 (controlled with [_ITERATOR_DEBUG_LEVEL](../VS_csharp/_iterator_debug_level.md)), a runtime error will occur if you attempt to access an element outside the bounds of the vector.  See [Checked Iterators](../VS_csharp/checked-iterators.md) for more information.  
...
|-|-|  
|Parameter|Description|  
|`_Right`|The [vector](../VS_csharp/vector-class.md) being copied into the `vector`.|  
