...
-   Using the C++ **new** and **delete** operators.  
 ...
 For more information about how to use `CMemoryState` and other diagnostics, see [Debugging MFC Applications](../VS_csharp/mfc-debugging-techniques.md).  
...
### Example  
  [!code[NVC_MFC_Utilities#18](../VS_csharp/codesnippet/CPP/cmemorystate-structure_1.cpp)]  
...
### Remarks  
 The report, which is printed on the [afxDump](../VS_csharp/afxdump--cdumpcontext-in-mfc-.md) device, shows the following:  
...
-   total memory currently used by the program (in bytes)  
 ...
 Free blocks are the number of blocks whose deallocation was delayed if `afxMemDF` was set to **delayFreeMemDF**. For more information, see [afxMemDF](../VS_csharp/afxmemdf.md), in the "MFC Macros and Globals" section. See [Types of Blocks on the Debug Heap](assetId:///db2e7f62-0679-4b39-a23f-26f2c2f407c5) for more information on these block types.  
  
### Example  
  The following code should be placed in                                 *projname*App.cpp. Define the following global variables:  
  
 [!code[NVC_MFC_Utilities#40](../VS_csharp/codesnippet/CPP/cmemorystate-structure_2.cpp)]  
  
 In the `InitInstance` function, add the line:  
  
 [!code[NVC_MFC_Utilities#41](../VS_csharp/codesnippet/CPP/cmemorystate-structure_3.cpp)]  
  
 Add a handler for the `ExitInstance` function and use the following code:  
  
 [!code[NVC_MFC_Utilities#42](../VS_csharp/codesnippet/CPP/cmemorystate-structure_4.cpp)]  
