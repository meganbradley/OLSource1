...
Convert a time value and correct for the local time zone. More secure versions of these functions are available; see [_localtime32_s, _localtime64_s](../VS_csharp/localtime_s--_localtime32_s--_localtime64_s.md).  
...
## Remarks  
 The `localtime` function converts a time stored as a [time_t](../VS_csharp/standard-types.md) value and stores the result in a structure of type `tm`. The `long` value `timer` represents the seconds elapsed since midnight (00:00:00), January 1, 1970, UTC. This value is usually obtained from the `time` function.  
...
>  The target environment should try to determine whether daylight saving time is in effect.  
 ...
 These functions validate their parameters. If `timer` is a null pointer, or if the timer value is negative, these functions invoke an invalid parameter handler, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, the functions return `NULL` and set `errno` to `EINVAL`.  
