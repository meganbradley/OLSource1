...
## Remarks  
 ...
 C# 2.0 added the ability to embed an array of fixed size in a [struct](../VS_csharp/struct--csharp-reference-.md) when it is used in an [unsafe](../VS_csharp/unsafe--csharp-reference-.md) code block.  
 ...
 [!code[csProgGuidePointers#19](../VS_csharp/codesnippet/CSharp/fixed-size-buffers--csharp-programming-guide-_1.cs)]  
 ...
 [!code[csProgGuidePointers#20](../VS_csharp/codesnippet/CSharp/fixed-size-buffers--csharp-programming-guide-_2.cs)]  
  
 The size of the 128 element `char` array is 256 bytes. Fixed size [char](../VS_csharp/char--csharp-reference-.md) buffers always take two bytes per character, regardless of the encoding. This is true even when char buffers are marshaled to API methods or structs with `CharSet = CharSet.Auto` or `CharSet = CharSet.Ansi`. For more information, see <xref:System.Runtime.InteropServices.CharSet*>.  
  
 Another common fixed-size array is the [bool](../VS_csharp/bool--csharp-reference-.md) array. The elements in a `bool` array are always one byte in size. `bool` arrays are not appropriate for creating bit arrays or buffers.  
  
> [!NOTE]
>  Except for memory created by using [stackalloc](../VS_csharp/stackalloc--csharp-reference-.md), the C# compiler and the common language runtime (CLR) do not perform any security buffer overrun checks. As with all unsafe code, use caution.  
...
## See Also  
 ...
 [Unsafe Code (C#)](../VS_csharp/unsafe-code-and-pointers--csharp-programming-guide-.md)   
