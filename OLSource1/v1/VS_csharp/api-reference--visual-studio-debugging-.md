...
> [!NOTE]
>  When a [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] debugging method returns `S_OK`, it is assumed that all out parameter pointers are valid, that is, no validation is conducted on out parameter pointers when `S_OK` is returned.  
  
> [!NOTE]
>  Invalid or `NULL` [out] parameters may cause the IDE to crash.  
  
## See Also  
 [Interfaces](../VS_csharp/interfaces--visual-studio-debugging-.md)   
 [Enumerations](../VS_csharp/enumerations--visual-studio-debugging-.md)   
