...
-   [DECLARE_AGGREGATABLE](../VS_csharp/declare_aggregatable.md) Declares that your object can be aggregated.  
 ...
 [!code[NVC_ATL_COM#2](../VS_csharp/codesnippet/CPP/ccomcoclass-class_1.h)]  
...
### Return Value  
 A standard `HRESULT` value. See [CoCreateInstance](http://msdn.microsoft.com/library/windows/desktop/ms686615) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)] for a description of possible return values.  
...
### Remarks  
 ...
 The ATL class implementing the required COM object (that is, the class used as the first template parameter to [CComCoClass](../VS_csharp/ccomcoclass-class.md)) must be in the same project as the calling code. The creation of the COM object is carried out by the class factory registered for this ATL class.  
  
 These functions are useful for creating objects that you have prevented from being externally creatable by using the [OBJECT_ENTRY_NON_CREATEABLE_EX_AUTO](../VS_csharp/object_entry_non_createable_ex_auto.md) macro. They are also useful in situations where you want to avoid the COM API for reasons of efficiency.  
  
 Note that the interface `Q` must have an IID associated with it that can be retrieved using the [__uuidof](../VS_csharp/__uuidof-operator.md) operator.  
...
### Example  
 ...
 [!code[NVC_ATL_COM#11](../VS_csharp/codesnippet/CPP/ccomcoclass-class_2.cpp)]  
...
### Parameters  
 ...
 `hInst`  
 [in] The handle to the resource. By default, this parameter is **_AtlModule::GetResourceInstance**, where **_AtlModule** is the global instance of [CAtlModule](../VS_csharp/catlmodule-class.md).  
...
### Remarks  
 The default implementation returns **NULL**. You can override this method with the [DECLARE_OBJECT_DESCRIPTION](../VS_csharp/declare_object_description.md) macro. For example:  
  
 [!code[NVC_ATL_COM#12](../VS_csharp/codesnippet/CPP/ccomcoclass-class_3.h)]  
 ...
 For more information about the ATL Project Wizard, see the article [Creating an ATL Project](../VS_csharp/creating-an-atl-project.md).  
