...
---
# CComModule Class
As of ATL 7.0, `CComModule` is deprecated: see [ATL Module Classes](../VS_csharp/atl-module-classes.md) for more details.  
...
> [!NOTE]
>  This class is deprecated, and the ATL code generation wizards now use the [CAtlAutoThreadModule](../VS_csharp/catlautothreadmodule-class.md) and [CAtlModule](../VS_csharp/catlmodule-class.md) derived classes. See [ATL Module Classes](../VS_csharp/atl-module-classes.md) for more information. The information that follows is for use with applications created with older releases of ATL. `CComModule` is still part of ATL for backwards capability.  
...
-   Performing lifetime management of class objects.  
 ...
 When you run the ATL COM AppWizard, the wizard automatically generates `_Module`, a global instance of `CComModule` or a class derived from it. For more information about the ATL Project Wizard, see the article [Creating an ATL Project](../VS_csharp/creating-an-atl-project.md).  
  
 In addition to `CComModule`, ATL provides [CComAutoThreadModule](../VS_csharp/ccomautothreadmodule-class.md), which implements an apartment-model module for EXEs and Windows services. Derive your module from `CComAutoThreadModule` when you want to create objects in multiple apartments.  
...
##  <a name="ccommodule__getclassobject"></a>  CComModule::GetClassObject  
 As of ATL 7.0, `CComModule` is obsolete: see [ATL Module Classes](../VS_csharp/atl-module-classes.md) for more details.  
...
### Parameters  
 `dwClsContext`  
 [in] Specifies the context in which the class object is to be run. Possible values are **CLSCTX_INPROC_SERVER**,                                 **CLSCTX_INPROC_HANDLER**, or **CLSCTX_LOCAL_SERVER**. For a description of these values, see [CLSCTX](http://msdn.microsoft.com/library/windows/desktop/ms693716) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
  
 `dwFlags`  
 [in] Determines the connection types to the class object. Possible values are **REGCLS_SINGLEUSE**,                                 **REGCLS_MULTIPLEUSE**, or **REGCLS_MULTI_SEPARATE**. For a description of these values, see [REGCLS](http://msdn.microsoft.com/library/windows/desktop/ms679697) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
...
### Remarks  
 ...
 See [OBJECT_ENTRY_AUTO](../VS_csharp/object_entry_auto.md) for information on how to add an entry to the object map.  
...
### Remarks  
 ...
 By specifying the [DECLARE_REGISTRY](../VS_csharp/declare_registry.md) macro, `UpdateRegistryClass` will be invoked automatically when your object map is processed.  
...
### Remarks  
 ...
 By specifying the [DECLARE_REGISTRY_RESOURCE](../VS_csharp/declare_registry_resource.md) or [DECLARE_REGISTRY_RESOURCEID](../VS_csharp/declare_registry_resourceid.md) macro, `UpdateRegistryFromResourceD` will be invoked automatically when your object map is processed.  
...
> [!NOTE]
>  To substitute replacement values at run time, do not specify the [DECLARE_REGISTRY_RESOURCE](../VS_csharp/declare_registry_resource.md) or [DECLARE_REGISTRY_RESOURCEID](../VS_csharp/declare_registry_resourceid.md) macro. Instead, create an array of **_ATL_REGMAP_ENTRIES** structures, where each entry contains a variable placeholder paired with a value to replace the placeholder at run time. Then call `UpdateRegistryFromResourceS`, passing the array for the `pMapEntries` parameter. This adds all the replacement values in the **_ATL_REGMAP_ENTRIES** structures to the Registrar's replacement map.  
