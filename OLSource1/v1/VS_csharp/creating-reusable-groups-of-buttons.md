...
1.  Create a VSIX project named `ReusableButtons`. For more information, see [Creating a VSPackage with a Menu Command](../VS_csharp/creating-an-extension-with-a-menu-command.md).  
...
1.  Create an entry in the `CommandPlacements` section. Set the GUID and ID of the `CommandPlacement` element to those of your group, and set the parent GUID and ID to those of the target location.  
    ...
    ```  
     ...
     A command group can be included on more than one menu. The parent menu can be one that you created, one that is supplied by [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] (as described in ShellCmdDef.vsct or SharedCmdDef.vsct), or one that is defined in another VSPackage. The number of parenting layers is unlimited as long as the parent menu is eventually connected to [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] or to a shortcut menu that is displayed by a VSPackage.  
