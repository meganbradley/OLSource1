...
## Remarks  
 ...
 To communicate with an HTTP server, you must first create an instance of [CInternetSession](../VS_csharp/cinternetsession-class.md), and then create a [CHttpConnection](#_mfc_chttpconnection) object. You never create a `CHttpConnection` object directly; rather, call [CInternetSession::GetHttpConnection](../VS_csharp/cinternetsession-class.md#cinternetsession__gethttpconnection), which creates the `CHttpConnection` object and returns a pointer to it.  
  
 To learn more about how `CHttpConnection` works with the other MFC Internet classes, see the article [Internet Programming with WinInet](../VS_csharp/win32-internet-extensions--wininet-.md). For more information about connecting to servers using the other two supported Internet protocols, gopher and FTP, see the classes [CGopherConnection](../VS_csharp/cgopherconnection-class.md) and [CFtpConnection](../VS_csharp/cftpconnection-class.md).  
...
### Remarks  
 You never create a `CHttpConnection` directly. Rather, you create an object by calling [CInternetSession::GetHttpConnection](../VS_csharp/cinternetsession-class.md#cinternetsession__gethttpconnection).  
...
### Return Value  
 A pointer to the [CHttpFile](../VS_csharp/chttpfile-class.md) object requested.  
...
|`INTERNET_FLAG_NO_AUTO_REDIRECT`|Used only with HTTP, specifies that redirections should not be automatically handled in [CHttpFile::SendRequest](../VS_csharp/chttpfile-class.md#chttpfile__sendrequest).|  
  
 Override the `dwContext` default to set the context identifier to a value of your choosing. The context identifier is associated with this specific operation of the `CHttpConnection` object created by its [CInternetSession](../VS_csharp/cinternetsession-class.md) object. The value is returned to [CInternetSession::OnStatusCallback](../VS_csharp/cinternetsession-class.md#cinternetsession__onstatuscallback) to provide status on the operation with which it is identified. See the article [Internet First Steps: WinInet](../VS_csharp/wininet-basics.md) for more information about the context identifier.  
