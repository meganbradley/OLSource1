...
> [!NOTE]
>  If you are working with the Data Access Objects (DAO) classes rather than the Open Database Connectivity (ODBC) classes, use class [CDaoDatabase](../VS_csharp/cdaodatabase-class.md) instead. For more information, see the article [Overview: Database Programming](../VS_csharp/data-access-programming--mfc-atl-.md).  
 ...
 For more information about `CDatabase`, see the articles [Data Source (ODBC)](../VS_csharp/data-source--odbc-.md) and [Overview: Database Programming](../VS_csharp/data-access-programming--mfc-atl-.md).  
...
> [!CAUTION]
>  Depending on your ODBC driver, opening a recordset before calling **BeginTrans** may cause problems when calling **Rollback**. You should check the specific driver you are using. For example, when using the Microsoft Access driver included in the Microsoft ODBC Desktop Driver Pack 3.0, you must account for the Jet database engine's requirement that you should not begin a transaction on any database that has an open cursor. In the MFC database classes, an open cursor means an open `CRecordset` object. For more information, see [Technical Note 68](../VS_csharp/tn068--performing-transactions-with-the-microsoft-access-7-odbc-driver.md).  
  
 **BeginTrans** may also lock data records on the server, depending on the requested concurrency and the capabilities of the data source. For information about locking data, see the article [Recordset: Locking Records (ODBC)](../VS_csharp/recordset--locking-records--odbc-.md).  
  
 User-defined transactions are explained in the article [Transaction (ODBC)](../VS_csharp/transaction--odbc-.md).  
...
>  Calling **BeginTrans** again without calling **CommitTrans** or **Rollback** is an error.  
 ...
 For more information about transactions, see the article [Transaction (ODBC)](../VS_csharp/transaction--odbc-.md).  
...
### Remarks  
 For information about transactions, see the article [Transaction (ODBC)](../VS_csharp/transaction--odbc-.md).  
...
### Example  
 ...
 [!code[NVC_MFCDatabase#9](../VS_csharp/codesnippet/CPP/cdatabase-class_1.h)]  
  
 [!code[NVC_MFCDatabase#10](../VS_csharp/codesnippet/CPP/cdatabase-class_2.cpp)]  
...
### Example  
 [!code[NVC_MFCDatabase#12](../VS_csharp/codesnippet/CPP/cdatabase-class_3.cpp)]  
...
### Parameters  
 `lpszSQL`  
 Pointer to a null-terminated string containing a valid SQL command to execute. You can pass a [CString](../VS_csharp/cstringt-class.md).  
...
### Example  
 [!code[NVC_MFCDatabase#13](../VS_csharp/codesnippet/CPP/cdatabase-class_4.cpp)]  
...
|`SQL_BP_OTHER_HSTMT`|Bookmarks associated with one recordset object are valid on a second recordset.|  
 ...
 For more information about this return value, see the ODBC API function **SQLGetInfo** in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)]. For more information about bookmarks, see the article [Recordset: Bookmarks and Absolute Positions (ODBC)](../VS_csharp/recordset--bookmarks-and-absolute-positions--odbc-.md).  
...
### Return Value  
 A `const`[CString](../VS_csharp/cstringt-class.md) containing the connection string if `OpenEx` or `Open` has been called; otherwise, an empty string.  
...
|`SQL_CB_PRESERVE`|Proceed normally with `CRecordset` operations.|  
 ...
 For more information about this return value, see the ODBC API function **SQLGetInfo** in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)]. For more information about transactions, see the article [Transaction (ODBC)](../VS_csharp/transaction--odbc-.md).  
...
### Return Value  
 A [CString](../VS_csharp/cstringt-class.md) containing the database name if successful; otherwise, an empty `CString`.  
...
### Example  
 [!code[NVC_MFCDatabase#15](../VS_csharp/codesnippet/CPP/cdatabase-class_5.cpp)]  
...
>  Calling the [OpenEx](#cdatabase__openex) member function is the preferred way to connect to a data source and initialize your database object.  
 ...
 You can also use the connection string for multiple levels of login authorization (each for a different `CDatabase` object) or to convey other data source-specific information. For more information about connection strings, see Chapter 5 in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
...
### Example  
 [!code[NVC_MFCDatabase#14](../VS_csharp/codesnippet/CPP/cdatabase-class_6.cpp)]  
...
### Example  
 [!code[NVC_MFCDatabase#11](../VS_csharp/codesnippet/CPP/cdatabase-class_7.cpp)]  
...
### Remarks  
 ...
 After a rollback, the record that was current before the rollback remains current. For details about the state of the recordset and the data source after a rollback, see the article [Transaction (ODBC)](../VS_csharp/transaction--odbc-.md).  
