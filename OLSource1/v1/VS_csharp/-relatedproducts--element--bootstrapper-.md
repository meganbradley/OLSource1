...
|---------------|-----------------|  
|`Code`|The code name of the included product, as specified by the `ProductCode` attribute of the `Product` element. For more information, see [<Product\> Element (ClickOnce Bootstrapper)](../VS_csharp/-product--element--bootstrapper-.md).|  
...
## Example  
 The following code example specifies that the Microsoft Installer is installed with the [!INCLUDE[dnprdnshort](../VS_csharp/includes/dnprdnshort_md.md)], and therefore will not need a separate installation.  
