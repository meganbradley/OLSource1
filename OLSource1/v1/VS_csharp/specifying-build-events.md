...
You can use build events to specify commands that run before the build starts, before the link process, or after the build finishes.  
 ...
 For general information on build events, see [Understanding Custom Build Steps and Build Events](../VS_csharp/understanding-custom-build-steps-and-build-events.md).  
  
### To specify a build event  
  
1.  In **Solution Explorer**, select the project for which you want to specify the build event.  
  
2.  Open the project's **Property Pages** dialog box. For more information, see [Modifying Project Settings](../VS_csharp/working-with-project-properties.md).  
