...
This walkthrough demonstrates how to use [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] to build a [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)] project at a command prompt. You will learn how to create the C++ source files and an XML-based project file for a [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)] console application. After building the project, you will learn how to customize the build process.  
  
 This walkthrough illustrates the following tasks:  
  
-   Creating the C++ source files for your project.  
  
-   Creating the XML [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] project file.  
  
-   Using [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] to build your project.  
  
-   Using [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] to customize your project.  
  
## Prerequisites  
 You need the following to complete this walkthrough:  
  
-   [!INCLUDE[vs_dev12](../VS_csharp/includes/vs_dev12_md.md)]  
  
-   A general understanding of the [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] system.  
  
## Creating the C++ Source Files  
 In this walkthrough you will create a project that has a source file and a header file. The source file main.cpp contains the main function for the console application. The header file main.h contains code to include the iostream header file. You can create these C++ files by using [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] or a text editor.  
...
## Creating the XML MSBuild Project File  
 An [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] project file is an XML file that contains a project root element (<Project\>). In the following example project, the <Project\> element contains seven child elements:  
  
-   Three item group tags (<ItemGroup\>) that specify project configuration and platform, source file name, and header file name.  
  
-   Three import tags (<Import\>) that specify the location of Microsoft [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)] settings.  
...
```  
 ...
 [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] creates a directory for the output files, and then compiles and links your project to generate the Myproject.exe program. After the build process finishes, use the following command to run the application:  
...
## Customizing Your Project  
 [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] enables you to execute predefined build targets, apply user-defined properties, and use custom tools, events, and build steps. This section illustrates the following tasks:  
  
-   Using [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] with build targets.  
  
-   Using [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] with build properties.  
  
-   Using [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] with the 64-bit compiler and tools.  
  
-   Using [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] with different toolsets.  
  
-   Adding [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] customizations.  
...
### Using MSBuild with the 64-bit Compiler and Tools  
 If you have installed [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)] on 64-bit Windows, by default, the 64-bit x64 native and cross tools are installed. You can configure [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] to use the 64-bit compiler and tools to build your application by setting the `PreferredToolArchitecture` property. This property does not affect the project configuration or platform properties. By default, the 32-bit version of the tools is used. To specify the 64-bit version of the compiler and tools, add the following property group element to the Myproject.vcxproj project file after the `Microsoft.Cpp.default.props` <Import /\> element:  
...
### Using MSBuild with a Different Toolset  
 If you have the toolsets and libraries for other versions of [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)] installed, [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] can build applications for either the current [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)] version or for the other installed versions. For example, if you have installed [!INCLUDE[cpp_dev11_long](../VS_csharp/includes/cpp_dev11_long_md.md)], to specify the [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)] 11.0 toolset for Windows XP, add the following property group element to the Myproject.vcxproj project file after the Microsoft.Cpp.props `<Import />` element:  
...
```  
 ...
 To rebuild your project with the [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)] 11.0 Windows XP toolset, type either of the following commands:  
...
### Adding MSBuild Customizations  
 [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] provides various ways to customize your build process. The following topics show how to add custom build steps, tools, and events to your [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] project:  
  
-   [How to: Add a Custom Build Step to an MSBuild Project](../VS_csharp/how-to--add-a-custom-build-step-to-msbuild-projects.md)  
  
-   [How to: Add a Custom Build Tool to an MSBuild Project](../VS_csharp/how-to--add-custom-build-tools-to-msbuild-projects.md)  
  
-   [How to: Add a Custom Build Event to an MSBuild Project](../VS_csharp/how-to--use-build-events-in-msbuild-projects.md)