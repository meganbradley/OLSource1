...
## Remarks  
 Pass the `IID` for the [IEEVisualizerServiceProvider](../VS_csharp/ieevisualizerserviceprovider.md) interface (`IID_IEEVisualizerServiceProvider`) to see if the Type Visualizer service is available. If so, the expression evaluator can obtain the [IEEVisualizerService](../VS_csharp/ieevisualizerservice.md) interface to support type visualizers. See [Visualizing and Viewing Data](../VS_csharp/visualizing-and-viewing-data.md) for details.  
