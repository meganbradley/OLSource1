...
This article describes how to add the following levels of functionality to your [CObject](../VS_csharp/cobject-class.md)-derived class:  
...
-   [Serialization support](#_core_to_add_serialization_support)  
 ...
 For a general description of `CObject` functionality, see the article [Deriving a Class from CObject](../VS_csharp/deriving-a-class-from-cobject.md).  
  
#### To add run-time class information  
  
1.  Derive your class from `CObject`, as described in the [Deriving a Class from CObject](../VS_csharp/deriving-a-class-from-cobject.md) article.  
  
2.  Use the `DECLARE_DYNAMIC` macro in your class declaration, as shown here:  
  
     [!code[NVC_MFCCObjectSample#2](../VS_csharp/codesnippet/CPP/specifying-levels-of-functionality_1.h)]  
...
3.  Use the `IMPLEMENT_DYNAMIC` macro in the implementation file (.CPP) of your class. This macro takes as arguments the name of the class and its base class, as follows:  
     ...
     [!code[NVC_MFCCObjectSample#3](../VS_csharp/codesnippet/CPP/specifying-levels-of-functionality_2.cpp)]  
...
>  A "polymorphic pointer" points to an object of a class (call it A) or to an object of any class derived from A (say, B). To serialize through a polymorphic pointer, the framework must determine the run-time class of the object it is serializing (B), since it might be an object of any class derived from some base class (A).  
 ...
 For more details on how to enable serialization when you derive your class from `CObject`, see the articles [Files in MFC](../VS_csharp/files-in-mfc.md) and [Serialization](../VS_csharp/serialization-in-mfc.md).  
  
## See Also  
 [Deriving a Class from CObject](../VS_csharp/deriving-a-class-from-cobject.md)