...
#### Parameters  
 ...
 If the specified drive is not available, or the kind of drive (for example, removable, fixed, CD-ROM, RAM disk, or network drive) cannot be determined, the invalid-parameter handler, which is described in [Parameter Validation](../VS_csharp/parameter-validation.md), is invoked.  
 ...
 If `maxlen` is not greater than zero, the invalid-parameter handler, which is described in [Parameter Validation](../VS_csharp/parameter-validation.md), is invoked.  
...
## Return Value  
 ...
 If `buffer` is specified as `NULL` and there is insufficient memory to allocate `maxlen` characters, an error occurs and `errno` is set to `ENOMEM`. If the length of the path,  which includes the terminating null character, exceeds `maxlen`, an error occurs and `errno` is set to `ERANGE`. For more information about these error codes, see [errno, _doserrno, _sys_errlist, and _sys_nerr](../VS_csharp/errno--_doserrno--_sys_errlist--and-_sys_nerr.md).  
...
## Remarks  
 ...
 The version of this function that has the `_nolock` suffix behaves identically to this function except that it is not thread-safe and is not protected from interference by other threads. For more information, see [_getdcwd_nolock, _wgetdcwd_nolock](../VS_csharp/_getdcwd_nolock--_wgetdcwd_nolock.md).  
  
 When `_DEBUG` and `_CRTDBG_MAP_ALLOC` are defined, calls to `_getdcwd` and `_wgetdcwd` are replaced by calls to `_getdcwd_dbg` and `_wgetdcwd_dbg` so that you can debug memory allocations. For more information, see[_getdcwd_dbg, _wgetdcwd_dbg](../VS_csharp/_getdcwd_dbg--_wgetdcwd_dbg.md).  
...
## Example  
 See the example in [_getdrive](../VS_csharp/_getdrive.md).  
