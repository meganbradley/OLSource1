...
'main' : mixing _m64 and floating point expressions may result in incorrect code  
 ...
 A function uses [__m64](../VS_csharp/__m64.md) and **float**/**double** types. Because the MMX and floating-point registers share the same physical register space (cannot be used simultaneously), using `__m64` and **float**/**double** types in the same function can result in data corruption, possibly causing an exception.  
