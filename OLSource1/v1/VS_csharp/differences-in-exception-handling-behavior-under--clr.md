...
[Basic Concepts in Using Managed Exceptions](../VS_csharp/basic-concepts-in-using-managed-exceptions.md) discusses exception handling in managed applications. In this topic, differences from the standard behavior of exception handling and some restrictions are discussed in detail. For more information, see [The _set_se_translator Function](../VS_csharp/_set_se_translator.md).  
  
##  <a name="vcconjumpingoutofafinallyblock"></a> Jumping Out of a Finally Block  
 In native C/C++ code, jumping out of a __**finally** block using structured exception handling (SEH) is allowed although it produces a warning.  Under [/clr](../VS_csharp/-clr--common-language-runtime-compilation-.md), jumping out of a **finally** block causes an error:  
...
##  <a name="vcconraisingexceptionswithinanexceptionfilter"></a> Raising Exceptions Within an Exception Filter  
 When an exception is raised during the processing of an [exception filter](../VS_csharp/writing-an-exception-filter.md) within managed code, the exception is caught and treated as if the filter returns 0.  
...
##  <a name="vcconexceptionfiltersandexception_continue_execution"></a> Exception Filters and EXCEPTION_CONTINUE_EXECUTION  
 If a filter returns `EXCEPTION_CONTINUE_EXECUTION` in a managed application, it is treated as if the filter returned `EXCEPTION_CONTINUE_SEARCH`. For more information on these constants, see [try-except Statement](../VS_csharp/try-except-statement.md).  
...
## See Also  
 [Exception Handling](../VS_csharp/exception-handling---c---component-extensions-.md)   
