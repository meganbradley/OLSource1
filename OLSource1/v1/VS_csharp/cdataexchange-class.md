...
## Remarks  
 ...
 Use this class if you are writing data exchange routines for custom data types or controls, or if you are writing your own data validation routines. For more information on writing your own DDX and DDV routines, see [Technical Note 26](../VS_csharp/tn026--ddx-and-ddv-routines.md). For an overview of DDX and DDV, see [Dialog Data Exchange and Validation](../VS_csharp/dialog-data-exchange-and-validation.md) and [Dialog Boxes](../VS_csharp/dialog-boxes.md).  
...
### Parameters  
 *pDlgWnd*  
 A pointer to the parent window that contains the control. Usually this is a [CDialog](../VS_csharp/cdialog-class.md)-derived object.  
...
### Remarks  
 Construct a `CDataExchange` object yourself to store extra information in the data exchange object to pass to your window's [CWnd::DoDataExchange](../VS_csharp/cwnd-class.md#cwnd__dodataexchange) member function.  
  
### Example  
  [!code[NVC_MFCControlLadenDialog#70](../VS_csharp/codesnippet/CPP/cdataexchange-class_1.cpp)]  
...
### Remarks  
 **Fail** restores the focus and selection to the control whose validation failed (if there is a control to restore). **Fail** then throws an exception of type [CUserException](../VS_csharp/cuserexception-class.md) to stop the validation process. The exception causes a message box explaining the error to be displayed. After DDV validation fails, the user can reenter data in the offending control.  
  
 Implementors of custom DDV routines can call **Fail** from their routines when a validation fails.  
  
 For more information on writing your own DDX and DDV routines, see [Technical Note 26](../VS_csharp/tn026--ddx-and-ddv-routines.md). For an overview of DDX and DDV, see [Dialog Data Exchange and Validation](../VS_csharp/dialog-data-exchange-and-validation.md) and [Dialog Box Topics](../VS_csharp/dialog-boxes.md).  
...
##  <a name="cdataexchange__m_pdlgwnd"></a>  CDataExchange::m_pDlgWnd  
 Contains a pointer to the [CWnd](../VS_csharp/cwnd-class.md) object for which dialog data exchange (DDX) or validation (DDV) is taking place.  
...
### Remarks  
 This object is usually a [CDialog](../VS_csharp/cdialog-class.md) object. Implementors of custom DDX or DDV routines can use this pointer to obtain access to the dialog window that contains the controls they are operating on.  
...
## See Also  
 ...
 [CWnd::DoDataExchange](../VS_csharp/cwnd-class.md#cwnd__dodataexchange)   
 [CWnd::UpdateData](../VS_csharp/cwnd-class.md#cwnd__updatedata)