...
---
# max_fixed_size Class
Describes a [max class](../VS_csharp/-allocators-.md) object that limits a [freelist](../VS_csharp/freelist-class.md) object to a fixed maximum length.  
...
### Remarks  
 Decrements the stored value `_Nblocks`. The `released` member function of the current [max class](../VS_csharp/-allocators-.md) is called by `cache_freelist::allocate` whenever it removes a memory block from the free list.  
