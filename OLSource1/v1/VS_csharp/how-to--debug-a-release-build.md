...
1.  Open the **Property Pages** dialog box for the project. For details, see [Working with Project Properties](../VS_csharp/working-with-project-properties.md).  
  
2.  Click the **C/C++** node. Set **Debug Information Format** to [C7 compatible (/Z7)](../VS_csharp/-z7---zi---zi--debug-information-format-.md) or **Program Database (/Zi)**.  
  
3.  Expand **Linker** and click the **General** node. Set **Enable Incremental Linking** to [No (/INCREMENTAL:NO)](../VS_csharp/-incremental--link-incrementally-.md).  
  
4.  Select the **Debugging** node. Set **Generate Debug Info** to [Yes (/DEBUG)](../VS_csharp/-debug--generate-debug-info-.md).  
  
5.  Select the **Optimization** node. Set **References** to [/OPT:REF](../VS_csharp/-opt--optimizations-.md) and **Enable COMDAT Folding** to [/OPT:ICF](../VS_csharp/-opt--optimizations-.md).  
...
6.  You can now debug your release build application. To find a problem, step through the code (or use Just-In-Time debugging) until you find where the failure occurs, and then determine the incorrect parameters or code.  
     ...
     You can use [/RTC](../VS_csharp/-rtc--run-time-error-checks-.md) to try to expose such bugs in your debug builds.  
  
     For more information, see [Optimizing Your Code](../VS_csharp/optimizing-your-code.md).  
