...
## Remarks  
 User-defined types can overload the `/` operator (see [operator](../VS_csharp/operator--csharp-reference-2.md)). An overload of the `/` operator implicitly overloads the [/= operator](../VS_csharp/-=-operator--csharp-reference-1.md).  
  
 When you divide two integers, the result is always an integer. For example, the result of 7 / 3 is 2. To determine the remainder of 7 / 3, use the remainder operator ([%](../VS_csharp/--operator--csharp-reference-.md)). To obtain a quotient as a rational number or fraction, give the dividend or divisor type `float` or type `double`. You can assign the type implicitly if you express the dividend or divisor as a decimal by putting a digit to the right side of the decimal point, as the following example shows.  
  
## Example  
 [!code[csRefOperators#42](../VS_csharp/codesnippet/CSharp/--operator--csharp-reference-1_1.cs)]  
