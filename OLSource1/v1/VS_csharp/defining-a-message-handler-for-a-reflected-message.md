...
Once you have created a new MFC control class, you can define message handlers for it. Reflected message handlers allow your control class to handle its own messages before the message is received by the parent. You can use the MFC [CWnd::SendMessage](../Topic/CWnd::SendMessage.md) function to send messages from your control to a parent window.  
 ...
 With this functionality you could, for example, create a list box that will redraw itself rather than relying on the parent window to do so (owner drawn). For more information on reflected messages, see [Handling Reflected Messages](../VS_csharp/handling-reflected-messages.md).  
  
 To create an [ActiveX control](../VS_csharp/activex-controls-on-the-internet.md) with the same functionality, you must create a project for the ActiveX control.  
...
3.  In the [Properties window](../VS_csharp/properties-window.md), the control class name appears in the **Class Name** list.  
...
## See Also  
 [Mapping Messages to Functions](../VS_csharp/mapping-messages-to-functions.md)   
