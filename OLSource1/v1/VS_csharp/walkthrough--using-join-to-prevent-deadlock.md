...
##  <a name="problem"></a> The Dining Philosophers Problem  
 ...
 ![The Dining Philosophers Problem](../VS_csharp/media/dining_philosophersproblem.png "Dining_PhilosophersProblem")  
...
##  <a name="deadlock"></a> A Naïve Implementation  
 The following example shows a naïve implementation of the dining philosophers problem. The `philosopher` class, which derives from [concurrency::agent](../VS_csharp/agent-class.md), enables each philosopher to act independently. The example uses a shared array of [concurrency::critical_section](../VS_csharp/critical_section-class.md) objects to give each `philosopher` object exclusive access to a pair of chopsticks.  
...
### Code  
 [!code[concrt-philosophers-deadlock#1](../VS_csharp/codesnippet/CPP/walkthrough--using-join-to-prevent-deadlock_1.cpp)]  
...
##  <a name="solution"></a> Using join to Prevent Deadlock  
 ...
 To relate this example to the earlier one, the `philosopher` class replaces each `critical_section` object by using a [concurrency::unbounded_buffer](../VS_csharp/unbounded_buffer-class.md) object and a `join` object. The `join` object serves as an arbiter that provides the chopsticks to the philosopher.  
 ...
 For more information about greedy and non-greedy joins, and the differences between the various message buffer types, see [Asynchronous Message Blocks](../VS_csharp/asynchronous-message-blocks.md).  
  
#### To prevent deadlock in this example  
  
1.  Remove the following code from the example.  
  
     [!code[concrt-philosophers-deadlock#2](../VS_csharp/codesnippet/CPP/walkthrough--using-join-to-prevent-deadlock_2.cpp)]  
  
2.  Change the type of the `_left` and `_right` data members of the `philosopher` class to `unbounded_buffer`.  
  
     [!code[concrt-philosophers-join#2](../VS_csharp/codesnippet/CPP/walkthrough--using-join-to-prevent-deadlock_3.cpp)]  
  
3.  Modify the `philosopher` constructor to take `unbounded_buffer` objects as its parameters.  
  
     [!code[concrt-philosophers-join#3](../VS_csharp/codesnippet/CPP/walkthrough--using-join-to-prevent-deadlock_4.cpp)]  
...
4.  Modify the `pickup_chopsticks` method to use a non-greedy `join` object to receive messages from the message buffers for both chopsticks.  
     ...
     [!code[concrt-philosophers-join#4](../VS_csharp/codesnippet/CPP/walkthrough--using-join-to-prevent-deadlock_5.cpp)]  
...
5.  Modify the `putdown_chopsticks` method to release access to the chopsticks by sending a message to the message buffers for both chopsticks.  
     ...
     [!code[concrt-philosophers-join#5](../VS_csharp/codesnippet/CPP/walkthrough--using-join-to-prevent-deadlock_6.cpp)]  
...
6.  Modify the `run` method to hold the results of the `pickup_chopsticks` method and to pass those results to the `putdown_chopsticks` method.  
     ...
     [!code[concrt-philosophers-join#6](../VS_csharp/codesnippet/CPP/walkthrough--using-join-to-prevent-deadlock_7.cpp)]  
...
7.  Modify the declaration of the `chopsticks` variable in the `wmain` function to be an array of `unbounded_buffer` objects that each hold one message.  
     ...
     [!code[concrt-philosophers-join#7](../VS_csharp/codesnippet/CPP/walkthrough--using-join-to-prevent-deadlock_8.cpp)]  
...
### Code  
 [!code[concrt-philosophers-join#1](../VS_csharp/codesnippet/CPP/walkthrough--using-join-to-prevent-deadlock_9.cpp)]  
