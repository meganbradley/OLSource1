...
To use a custom directive processor in [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] on any computer, you must register it by one of the methods described in this topic.  
...
-   [VSPackage](../VS_csharp/vspackages.md). If you are defining a VSPackage that contains other features in addition to the directive processor, there is a convenient method of registering the directive processor.  
  
-   Set a registry key. In this method, you add a registry entry for the directive processor.  
  
 You need to use one of these methods only if you want to transform your text template in [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] or [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)]. If you use a custom host in your own application, your custom host is responsible for finding the directive processors for each directive.  
...
1.  Create a VSIX project in [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)].  
...
3.  Double-click the .vsix file. The [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] Extension Installer appears.  
  
4.  Restart [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)]. You will now be able to run text templates that contain directives that refer to the custom directive processor. Each directive is of this form:  
  
     `<#@ CustomDirective Processor="CustomDirectiveProcessorName" parameter1="value1" … #>`  
  
#### To uninstall or temporarily disable the custom directive processor  
  
1.  In the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] **Tools** menu, click **Extension Manager**.  
...
>  This attribute is placed on the package class, not the directive processor class.  
 ...
 For more information, see [VSPackages](../VS_csharp/vspackages.md).  
...
2.  In regedit, navigate to  
     ...
     **HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\VisualStudio\\\*.0\TextTemplating\DirectiveProcessors**  
     ...
     If you want to install the directive processor in the experimental version of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)], insert "Exp" after "11.0".  
...
## See Also  
 [Creating Custom Directive Processors](../VS_csharp/creating-custom-t4-text-template-directive-processors.md)