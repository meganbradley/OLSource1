...
-   [Deletion Events](#rules) – Use store events to propagate updates outside the store, for example to other [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] documents.  
...
> [!NOTE]
>  To add program code to your DSL definition, create a separate code file in the **Dsl** project and write partial definitions to augment the classes in the Generated Code folder. For more information, see [Writing Code](../VS_csharp/writing-code-to-customise-a-domain-specific-language.md).  
...
3.  OnDeleting and OnDeleted are called when the user invokes Undo after a creating an element, and when an earlier deletion is repeated in Redo. Use `this.Store.InUndoRedoOrRollback` to avoid updating store elements in these cases. For more information, see [Transactions](../VS_csharp/how-to--use-transactions-to-update-the-model.md).  
...
1.  **Deleting** and **Delete** rules are triggered only in a transaction, and not in an Undo or Redo. You can set them to be queued to execute at the end of the transaction in which the deletion is performed. Deleting rules are always executed before any Deleted rules that are in the queue.  
     ...
     For more information, see [Rules](../VS_csharp/rules-propagate-changes-within-the-model.md).  
  
2.  **Deleted** store event is invoked at the end of a transaction, and is called after an undo or redo. It can therefore be used to propagate deletions to objects outside the store such as files, database entries or other objects in [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)].  
  
     For more information, see [Events](../VS_csharp/event-handlers-propagate-changes-outside-the-model.md).  
...
##  <a name="unmerge"></a> UnMerge  
 ...
 For more information, see [Customizing Element Creation and Movement](../VS_csharp/customizing-element-creation-and-movement.md).  
  
## See Also  
 [Customizing Copy](../VS_csharp/customizing-copy-behavior.md)   
 [Customizing Element Creation and Movement](../VS_csharp/customizing-element-creation-and-movement.md)   
 [Writing Code](../VS_csharp/writing-code-to-customise-a-domain-specific-language.md)