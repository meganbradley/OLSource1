...
## Remarks  
 ...
 `_mbsnbicmp` recognizes multibyte-character sequences according to the [multibyte code page](../VS_csharp/code-pages.md) currently in use. It is not affected by the current locale setting.  
  
 If either `string1` or `string2` is a null pointer, `_mbsnbicmp` invokes the invalid parameter handler as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, the function returns `_NLSCMPERROR` and sets `errno` to `EINVAL`.  
...
## Example  
 See the example for [_mbsnbcmp, _mbsnbcmp_l](../VS_csharp/_mbsnbcmp--_mbsnbcmp_l.md).  
