...
## Multiple Versions of Visual C++  
 ...
 If you get an unresolved linker error, run [dumpbin /linkermember](../VS_csharp/-linkermember.md):1 on the delayimp.lib that you expect to contain the helper function to see which helper function is defined instead. The helper function could also be defined in an object file; run [dumpbin /symbols](../VS_csharp/-symbols.md) and look for `delayLoadHelper(2)`.  
...
## Load All Imports for a Delay-Loaded DLL  
 The linker can load all imports from a DLL that you specified to be delay loaded. See [Loading All Imports for a Delay-Loaded DLL](../VS_csharp/loading-all-imports-for-a-delay-loaded-dll.md) for more information.  
