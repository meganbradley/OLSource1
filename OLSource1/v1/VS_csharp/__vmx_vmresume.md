...
## Remarks  
 An application can perform a VM-enter operation by using either the [__vmx_vmlaunch](../VS_csharp/__vmx_vmlaunch.md) or `__vmx_vmresume` function. The `__vmx_vmlaunch` function can be used only with a VMCS whose launch state is `Clear`, and the `__vmx_vmresume` function can be used only with a VMCS whose launch state is `Launched`. Consequently, use the [__vmx_vmclear](../VS_csharp/__vmx_vmclear.md) function to set the launch state of a VMCS to `Clear`, and then use the `__vmx_vmlaunch` function for your first VM-enter operation and the `__vmx_vmresume` function for subsequent VM-enter operations.  
...
|---------------|------------------|  
|`__vmx_vmresume`|[!INCLUDE[vcprx64](../VS_csharp/includes/vcprx64_md.md)]|  
