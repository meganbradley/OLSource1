 ...
 **Microsoft Specific**  
 ...
 If the Windows error reporting handler is not invoked, then `abort` calls [_exit](../VS_csharp/exit--_exit--_exit.md) to terminate the process with exit code 3 and returns control to the parent process or the operating system. `_exit` does not flush stream buffers or do `atexit`/`_onexit` processing.  
...
## See Also  
 [Using abort](../VS_csharp/using-abort.md)   
 [abort Function](../VS_csharp/abort-function--c-.md)   
 ...
 [_set_abort_behavior](../VS_csharp/_set_abort_behavior.md)