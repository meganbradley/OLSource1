...
## Specifying Inputs  
 Items represent the inputs for a build. For more information on items, see [MSBuild Items](../VS_csharp/msbuild-items.md).  
  
 To include files for a build, they must be included in an item list in the [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] project file. Multiple files can be added to item lists by either including the files individually or using wildcards to include many files at once.  
...
## Specifying Inputs with Wildcards  
 You can also use wildcards to recursively include all files or only specific files from subdirectories as inputs for a build. For more information about wildcards, see [MSBuild Items](../VS_csharp/msbuild-items.md)  
...
> [!NOTE]
>  You must use wildcards with items to specifiy the inputs for a build; you cannot specify the inputs using the `Sources` attribute in [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] tasks such as [Csc](../VS_csharp/csc-task.md) or [Vbc](../VS_csharp/vbc-task.md). The following example is not valid in a project file:  
>   
>  `<CSC Sources="*.cs">...</CSC>`  
...
## See Also  
 [How To: Build all Files in a Directory Except One](../VS_csharp/how-to--exclude-files-from-the-build.md)   
