...
The Parallel Patterns Library (PPL) provides algorithms that concurrently perform work on collections of data. These algorithms resemble those provided by the Standard Template Library (STL).  
 ...
 The parallel algorithms are composed from existing functionality in the Concurrency Runtime. For example, the [concurrency::parallel_for](../VS_csharp/parallel_for-function.md) algorithm uses a [concurrency::structured_task_group](../VS_csharp/structured_task_group-class.md) object to perform the parallel loop iterations. The `parallel_for` algorithm partitions work in an optimal way given the available number of computing resources.  
...
##  <a name="parallel_for"></a> The parallel_for Algorithm  
 The [concurrency::parallel_for](../VS_csharp/parallel_for-function.md) algorithm repeatedly performs the same task in parallel. Each of these tasks is parameterized by an iteration value. This algorithm is useful when you have a loop body that does not share resources among iterations of that loop.  
...
-   The loop iteration must be forward. The `parallel_for` algorithm throws an exception of type [std::invalid_argument](../VS_csharp/invalid_argument-class.md) if the `_Step` parameter is less than 1.  
...
-   The exception-handling mechanism for the `parallel_for` algorithm differs from that of a `for` loop. If multiple exceptions occur simultaneously in a parallel loop body, the runtime propagates only one of the exceptions to the thread that called `parallel_for`. In addition, when one loop iteration throws an exception, the runtime does not immediately stop the overall loop. Instead, the loop is placed in the cancelled state and the runtime discards any tasks that have not yet started. For more information about exception-handling and parallel algorithms, see [Exception Handling in the Concurrency Runtime](../VS_csharp/exception-handling-in-the-concurrency-runtime.md).  
 ...
 Although the `parallel_for` algorithm does not support arbitrary termination conditions, you can use cancellation to stop all tasks. For more information about cancellation, see [Cancellation in the PPL](../VS_csharp/cancellation-in-the-ppl.md).  
...
### Example  
 ...
 [!code[concrt-parallel-for-structure#1](../VS_csharp/codesnippet/CPP/parallel-algorithms_1.cpp)]  
 ...
 **1 2 4 3 5** Because the `parallel_for` algorithm acts on each item in parallel, the order in which the values are printed to the console will vary.  
 ...
 For a complete example that uses the `parallel_for` algorithm, see [How to: Write a parallel_for Loop](../VS_csharp/how-to--write-a-parallel_for-loop.md).  
  
 [[Top](#top)]  
  
##  <a name="parallel_for_each"></a> The parallel_for_each Algorithm  
 The [concurrency::parallel_for_each](../VS_csharp/parallel_for_each-function.md) algorithm performs tasks on an iterative container, such as those provided by the STL, in parallel. It uses the same partitioning logic that the `parallel_for` algorithm uses.  
...
### Example  
 The following example shows the basic structure of the `parallel_for_each` algorithm. This example prints to the console each value in a [std::array](../VS_csharp/array-class--stl-.md) object in parallel.  
  
 [!code[concrt-parallel-for-each-structure#1](../VS_csharp/codesnippet/CPP/parallel-algorithms_2.cpp)]  
 ...
 **4 5 1 2 3** Because the `parallel_for_each` algorithm acts on each item in parallel, the order in which the values are printed to the console will vary.  
 ...
 For a complete example that uses the `parallel_for_each` algorithm, see [How to: Write a parallel_for_each Loop](../VS_csharp/how-to--write-a-parallel_for_each-loop.md).  
  
 [[Top](#top)]  
  
##  <a name="parallel_invoke"></a> The parallel_invoke Algorithm  
 The [concurrency::parallel_invoke](../VS_csharp/parallel_invoke-function.md) algorithm executes a set of tasks in parallel. It does not return until each task finishes. This algorithm is useful when you have several independent tasks that you want to execute at the same time.  
 ...
 Like other parallel algorithms, `parallel_invoke` does not execute the tasks in a specific order. The topic [Task Parallelism (Concurrency Runtime)](../VS_csharp/task-parallelism--concurrency-runtime-.md) explains how the `parallel_invoke` algorithm relates to tasks and task groups.  
...
-   Perform 3-D ray tracing, where each iteration refers to one pixel that must be rendered.  
 ...
 The following example shows the basic structure that is used to call the `parallel_transform` algorithm. This example negates each element of a [std::vector](../VS_csharp/vector-class.md) object in two ways. The first way uses a lambda expression. The second way uses [std::negate](../VS_csharp/negate-struct.md), which derives from [std::unary_function](../VS_csharp/unary_function-struct.md).  
  
 [!code[concrt-basic-parallel-transform#1](../VS_csharp/codesnippet/CPP/parallel-algorithms_4.cpp)]  
...
-   Combine a list of elements, such as strings, into one element.  
 ...
 [!code[concrt-basic-parallel-reduce#1](../VS_csharp/codesnippet/CPP/parallel-algorithms_6.cpp)]  
  
 In many cases, you can think of `parallel_reduce` as shorthand for the use of the `parallel_for_each` algorithm together with the [concurrency::combinable](../VS_csharp/combinable-class.md) class.  
...
###  <a name="map_reduce_example"></a> Example: Performing Map and Reduce in Parallel  
 ...
 [!code[concrt-parallel-map-reduce-sum-of-primes#1](../VS_csharp/codesnippet/CPP/parallel-algorithms_7.cpp)]  
  
 For another example that performs a map and reduce operation in parallel, see [How to: Perform Map and Reduce Operations in Parallel](../VS_csharp/how-to--perform-map-and-reduce-operations-in-parallel.md).  
...
##  <a name="partitions"></a> Partitioning Work  
 ...
 [concurrency::affinity_partitioner](../VS_csharp/affinity_partitioner-class.md)  
 ...
 [concurrency::auto_partitioner](../VS_csharp/auto_partitioner-class.md)  
 ...
 [concurrency::simple_partitioner](../VS_csharp/simple_partitioner-class.md)  
 ...
 [concurrency::static_partitioner](../VS_csharp/static_partitioner-class.md)  
...
>  The `parallel_for_each` and `parallel_transform` algorithms support only containers that use random access iterators (such as [std::vector](../VS_csharp/vector-class.md)) for the static, simple, and affinity partitioners. The use of containers that use bidirectional and forward iterators produces a compile-time error. The default partitioner, `auto_partitioner`, supports all three of these iterator types.  
 ...
 [!code[concrt-static-partitioner#1](../VS_csharp/codesnippet/CPP/parallel-algorithms_8.cpp)]  
 ...
 [!code[concrt-affinity-partitioner#1](../VS_csharp/codesnippet/CPP/parallel-algorithms_9.cpp)]  
...
##  <a name="parallel_sorting"></a> Parallel Sorting  
 The PPL provides three sorting algorithms: [concurrency::parallel_sort](../VS_csharp/parallel_sort-function.md), [concurrency::parallel_buffered_sort](../VS_csharp/parallel_buffered_sort-function.md), and [concurrency::parallel_radixsort](../VS_csharp/parallel_radixsort-function.md). These sorting algorithms are useful when you have a data set that can benefit from being sorted in parallel. In particular, sorting in parallel is useful when you have a large dataset or when you use a computationally-expensive compare operation to sort your data. Each of these algorithms sorts elements in place.  
...
|`parallel_radixsort`|Integer key-based sort that requires O(N) space.|Hash-based|Stable|Requires additional O(N) space|O(N/P)|Random|  
 ...
 ![Comparison of the sorting algorithms](../VS_csharp/media/concrt_parallel_sorting.png "concrt_parallel_sorting")  
  
 These parallel sorting algorithms follow the rules of cancellation and exception handling. For more information about cancellation and exception handling in the Concurrency Runtime, see [Canceling Parallel Algorithms](../VS_csharp/cancellation-in-the-ppl.md#algorithms) and [Exception Handling in the Concurrency Runtime](../VS_csharp/exception-handling-in-the-concurrency-runtime.md).  
  
> [!TIP]
>  These parallel sorting algorithms support move semantics. You can define a move assignment operator to enable swap operations to occur more efficiently. For more information about move semantics and the move assignment operator, see [Rvalue Reference Declarator: &&](../VS_csharp/rvalue-reference-declarator----.md), and [How to: Write a Move Constructor](../VS_csharp/move-constructors-and-move-assignment-operators--c---.md). If you do not provide a move assignment operator or swap function, the sorting algorithms use the copy constructor.  
  
 The following basic example shows how to use `parallel_sort` to sort a `vector` of `int` values. By default, `parallel_sort` uses [std::less](../VS_csharp/less-struct.md) to compare values.  
  
 [!code[concrt-basic-parallel-sort#1](../VS_csharp/codesnippet/CPP/parallel-algorithms_10.cpp)]  
  
 This example shows how to provide a custom compare function. It uses the [std::complex::real](../Topic/complex::real.md) method to sort [std::complex<double\>](../VS_csharp/complex-class.md) values in ascending order.  
  
 [!code[concrt-basic-parallel-sort#2](../VS_csharp/codesnippet/CPP/parallel-algorithms_11.cpp)]  
 ...
 [!code[concrt-parallel-sort-points#1](../VS_csharp/codesnippet/CPP/parallel-algorithms_12.cpp)]  
 ...
 This example uses a lambda expression as the hash function. You can also use one of the built-in implementations of the [std::hash](../VS_csharp/hash-class.md) class or define your own specialization. You can also use a custom hash function object, as shown in this example:  
  
 [!code[concrt-parallel-sort-points#2](../VS_csharp/codesnippet/CPP/parallel-algorithms_13.cpp)]  
  
 [!code[concrt-parallel-sort-points#3](../VS_csharp/codesnippet/CPP/parallel-algorithms_14.cpp)]  
  
 The hash function must return an integral type ([std::is_integral::value](../VS_csharp/is_integral-class.md) must be `true`). This integral type must be convertible to type `size_t`.  
...
>  Implementing a good hash function requires that you know the dataset range and how each element in the dataset is transformed to a corresponding unsigned value. Because the hash operation works on unsigned values, consider a different sorting strategy if unsigned hash values cannot be produced.  
 ...
 [!code[concrt-choosing-parallel-sort#1](../VS_csharp/codesnippet/CPP/parallel-algorithms_15.cpp)]  
...
|Title|Description|  
|-----------|-----------------|  
|[How to: Write a parallel_for Loop](../VS_csharp/how-to--write-a-parallel_for-loop.md)|Shows how to use the `parallel_for` algorithm to perform matrix multiplication.|  
|[How to: Write a parallel_for_each Loop](../VS_csharp/how-to--write-a-parallel_for_each-loop.md)|Shows how to use the `parallel_for_each` algorithm to compute the count of prime numbers in a [std::array](../VS_csharp/array-class--stl-.md) object in parallel.|  
...
|[How to: Perform Map and Reduce Operations in Parallel](../VS_csharp/how-to--perform-map-and-reduce-operations-in-parallel.md)|Shows how to use the `parallel_transform` and `parallel_reduce` algorithms to perform a map and reduce operation that counts the occurrences of words in files.|  
|[Parallel Patterns Library (PPL)](../VS_csharp/parallel-patterns-library--ppl-.md)|Describes the PPL, which provides an imperative programming model that promotes scalability and ease-of-use for developing concurrent applications.|  
|[Cancellation in the PPL](../VS_csharp/cancellation-in-the-ppl.md)|Explains the role of cancellation in the PPL, how to cancel parallel work, and how to determine when a task group is canceled.|  
|[Exception Handling in the Concurrency Runtime](../VS_csharp/exception-handling-in-the-concurrency-runtime.md)|Explains the role of exception handling in the Concurrency Runtime.|  
  
## Reference  
 [parallel_for Function](../VS_csharp/parallel_for-function.md)  
  
 [parallel_for_each Function](../VS_csharp/parallel_for_each-function.md)  
  
 [parallel_invoke Function](../VS_csharp/parallel_invoke-function.md)  
  
 [affinity_partitioner Class](../VS_csharp/affinity_partitioner-class.md)  
  
 [auto_partitioner Class](../VS_csharp/auto_partitioner-class.md)  
  
 [simple_partitioner Class](../VS_csharp/simple_partitioner-class.md)  
  
 [static_partitioner Class](../VS_csharp/static_partitioner-class.md)  
  
 [parallel_sort Function](../VS_csharp/parallel_sort-function.md)  
  
 [parallel_buffered_sort Function](../VS_csharp/parallel_buffered_sort-function.md)  
  
 [parallel_radixsort Function](../VS_csharp/parallel_radixsort-function.md)