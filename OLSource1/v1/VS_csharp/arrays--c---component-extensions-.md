...
The `Platform::Array<T>` type in [!INCLUDE[cppwrt_short](../VS_csharp/includes/cppwrt_short_md.md)], or the `array` keyword in [!INCLUDE[cppcli](../VS_csharp/includes/cppcli_md.md)], declares an array of a specified type and initial value.  
...
-   [Variable Argument Lists (...) (C++/CLI)](../VS_csharp/variable-argument-lists--...---c---cli-.md)  
 ...
 *qualifiers* [optional]  
 One or more of these storage class specifiers: [mutable](../VS_csharp/mutable-data-members--c---.md), [volatile](../VS_csharp/volatile--c---.md), [const](../VS_csharp/const--c---.md), [extern](../VS_csharp/using-extern-to-specify-linkage.md), [static](../Topic/Static%20\(C++\).md).  
  
 `array-type`  
 The type of the array variable. Valid types are [!INCLUDE[wrt](../VS_csharp/includes/wrt_md.md)] classes and fundamental types, ref classes and structs, value classes and structs, and native pointers (`type``*`).  
 ...
 **Remarks**  
 ...
 You can detect at compile time whether a type is a reference-counted array with `__is_ref_array(``type``)`. For more information, see [Intrinsic Support for Type Traits](../VS_csharp/compiler-support-for-type-traits--c---component-extensions-.md).  
 ...
 *qualifiers* [optional]  
 ...
 `array-type`  
 The type of the array variable. Valid types are [!INCLUDE[wrt](../VS_csharp/includes/wrt_md.md)] classes and fundamental types, ref classes and structs, value classes and structs, native pointers (`type``*`), and native POD (plain old data) types.  
 ...
 **Remarks**  
 ...
 `array` is in the [cli](../VS_csharp/platform--default--and-cli-namespaces---c---component-extensions-.md) namespace.  
 ...
 When you allocate an array whose element type is a value type `V`, the default constructor for `V` is applied to each array element. For more information, see [.NET Framework Equivalents to C++ Native Types (C++/CLI)](../VS_csharp/.net-framework-equivalents-to-c---native-types--c---cli-.md).  
  
 At compile time, you can detect whether a type is a common language runtime (CLR) array with`__is_ref_array(``type``)`. For more information, see [Intrinsic Support for Type Traits](../VS_csharp/compiler-support-for-type-traits--c---component-extensions-.md).  
