...
>  `CToolBar` supports bitmaps with a maximum of 16 colors. When you load an image into a toolbar editor, Visual Studio automatically converts the image to a 16-color bitmap, if necessary, and displays a warning message if the image was converted. If you use an image with more than 16 colors (using an external editor to edit the image), the application might behave unexpectedly.  
 ...
 To create a radio button, call a [CCmdUI](../VS_csharp/ccmdui-class.md) object's [SetRadio](../VS_csharp/ccmdui-class.md#ccmdui__setradio) member function from an `ON_UPDATE_COMMAND_UI` handler. Pass `SetRadio` an argument of 0 for unchecked or nonzero for checked. In order to provide a radio group's mutually exclusive behavior, you must have `ON_UPDATE_COMMAND_UI` handlers for all of the buttons in the group.  
  
 For more information on using `CToolBar`, see the article [MFC Toolbar Implementation](../VS_csharp/mfc-toolbar-implementation.md) and [Technical Note 31: Control Bars](../VS_csharp/tn031--control-bars.md).  
...
### Example  
 [!code[NVC_MFCDocView#179](../VS_csharp/codesnippet/CPP/ctoolbar-class_1.cpp)]  
...
### Parameters  
 ...
 `dwCtrlStyle`  
 Additional styles for the creation of the embedded [CToolBarCtrl](../VS_csharp/ctoolbarctrl-class.md) object. By default, this value is set to **TBSTYLE_FLAT**. For a complete list of toolbar styles, see `dwStyle`.  
  
 `dwStyle`  
 The toolbar style. See                                 [Toolbar Control and Button Styles](http://msdn.microsoft.com/library/windows/desktop/bb760439) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)] for a list of appropriate styles.  
  
 *rcBorders*  
 A [CRect](../VS_csharp/crect-class.md) object that defines the widths of the toolbar window borders. These borders are set to 0,0,0,0 by default, thereby resulting in a toolbar window with no borders.  
...
### Example  
 [!code[NVC_MFCDocView#180](../VS_csharp/codesnippet/CPP/ctoolbar-class_2.cpp)]  
...
### Parameters  
 ...
 `rString`  
 A reference to a [CString](../VS_csharp/cstringt-class.md) object that will contain the text to be retrieved.  
...
### Parameters  
 ...
 `lpRect`  
 Address of the [RECT](../VS_csharp/rect-structure.md) structure that will contain the item's coordinates.  
...
### Remarks  
 Use `GetToolBarCtrl` to take advantage of the functionality of the Windows toolbar common control, and to take advantage of the support [CToolBarCtrl](../VS_csharp/ctoolbarctrl-class.md) provides for toolbar customization.  
  
 For more information about using common controls, see the article [Controls](../VS_csharp/controls--mfc-.md) and                         [Common Controls](http://msdn.microsoft.com/library/windows/desktop/bb775493) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
  
### Example  
 [!code[NVC_MFCDocViewSDI#15](../VS_csharp/codesnippet/CPP/ctoolbar-class_3.cpp)]  
...
### Remarks  
 See [toolbar editor](../VS_csharp/toolbar-editor.md) in for more information about creating a toolbar resource.  
...
> [!NOTE]
>  You can also set button states using the `nStyle` parameter; however, because button states are controlled by the [ON_UPDATE_COMMAND_UI](../VS_csharp/on_update_command_ui.md) handler, any state you set using `SetButtonInfo` will be lost during the next idle processing. See [How to Update User-Interface Objects](../VS_csharp/how-to--update-user-interface-objects.md) and [TN031: Control Bars](../VS_csharp/tn031--control-bars.md) for more information.  
  
 For information on bitmap images and buttons, see the [CToolBar](../VS_csharp/ctoolbar-class.md) Overview and [CToolBar::LoadBitmap](#ctoolbar__loadbitmap).  
...
> [!NOTE]
>  You can also set button states using the `nStyle` parameter; however, because button states are controlled by the [ON_UPDATE_COMMAND_UI](../VS_csharp/on_update_command_ui.md) handler, any state you set using `SetButtonStyle` will be lost during the next idle processing. See [How to Update User-Interface Objects](../VS_csharp/how-to--update-user-interface-objects.md) and [TN031: Control Bars](../VS_csharp/tn031--control-bars.md) for more information.  
...
### Example  
 [!code[NVC_MFCListView#8](../VS_csharp/codesnippet/CPP/ctoolbar-class_4.cpp)]  
  
## See Also  
 [MFC Sample CTRLBARS](../VS_csharp/visual-c---samples.md)   
 [MFC Sample DLGCBR32](../VS_csharp/visual-c---samples.md)   
 [MFC Sample DOCKTOOL](../VS_csharp/visual-c---samples.md)   
 ...
 [CCmdUI::SetCheck](../VS_csharp/ccmdui-class.md#ccmdui__setcheck)   
 [CCmdUI::SetRadio](../VS_csharp/ccmdui-class.md#ccmdui__setradio)