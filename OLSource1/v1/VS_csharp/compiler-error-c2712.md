...
cannot use __try in functions that require object unwinding  
 ...
 Error C2712 can occur if you use [/EHsc](../VS_csharp/-eh--exception-handling-model-.md), and a function with structured exception handling also has objects that require unwinding (destruction).  
...
-   Compile without /EHsc  
 ...
 Error C2712 can also occur if you call a method declared by using the [__event](../VS_csharp/__event.md) keyword. Because the event might be used in a multithreaded environment, the compiler generates code that prevents manipulation of the underlying event object, and then encloses the generated code in an SEH [try-finally statement](../VS_csharp/try-finally-statement.md). Consequently, error C2712 will occur if you call the event method and pass by value an argument whose type has a destructor. One solution in this case is to pass the argument as a constant reference.  
