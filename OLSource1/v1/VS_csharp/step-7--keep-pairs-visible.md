...
1.  Add the following `if` statement to the `label_Click()` event handler method, near the end of the code just above the statement where you start the timer. Take a close look at the code while adding it to the program. Consider how the code works.  
     ...
     [!code[VbExpressTutorial4Step7#9](../VS_csharp/codesnippet/CSharp/step-7--keep-pairs-visible_1.cs)]
[!code[VbExpressTutorial4Step7#9](../VS_csharp/codesnippet/VisualBasic/step-7--keep-pairs-visible_1.vb)]  
...
-   To go to the next tutorial step, see [Step 8: Add a Method to Verify Whether the Player Won](../VS_csharp/step-8--add-a-method-to-verify-whether-the-player-won.md).  
  
-   To return to the previous tutorial step, see [Step 6: Add a Timer](../VS_csharp/step-6--add-a-timer.md).