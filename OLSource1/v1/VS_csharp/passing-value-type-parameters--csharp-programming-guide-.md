...
A [value-type](../VS_csharp/value-types--csharp-reference-.md) variable contains its data directly as opposed to a [reference-type](../VS_csharp/reference-types--csharp-reference-.md) variable, which contains a reference to its data. Passing a value-type variable to a method by value means passing a copy of the variable to the method. Any changes to the parameter that take place inside the method have no affect on the original data stored in the argument variable. If you want the called method to change the value of the parameter, you must pass it by reference, using the [ref](../VS_csharp/ref--csharp-reference-.md) or [out](../VS_csharp/out--csharp-reference-.md) keyword. For simplicity, the following examples use `ref`.  
...
## Passing Value Types by Value  
 ...
 [!code[csProgGuideParameters#3](../VS_csharp/codesnippet/CSharp/passing-value-type-parameters--csharp-programming-guide-_1.cs)]  
...
## Passing Value Types by Reference  
 ...
 [!code[csProgGuideParameters#4](../VS_csharp/codesnippet/CSharp/passing-value-type-parameters--csharp-programming-guide-_2.cs)]  
  
 In this example, it is not the value of `n` that is passed; rather, a reference to `n` is passed. The parameter `x` is not an [int](../VS_csharp/int--csharp-reference-.md); it is a reference to an `int`, in this case, a reference to `n`. Therefore, when `x` is squared inside the method, what actually is squared is what `x` refers to, `n`.  
...
## Swapping Value Types  
 ...
 [!code[csProgGuideParameters#5](../VS_csharp/codesnippet/CSharp/passing-value-type-parameters--csharp-programming-guide-_3.cs)]  
  
 When you call the `SwapByRef` method, use the `ref` keyword in the call, as shown in the following example.  
  
 [!code[csProgGuideParameters#6](../VS_csharp/codesnippet/CSharp/passing-value-type-parameters--csharp-programming-guide-_4.cs)]  
