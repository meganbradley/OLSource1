...
>  A program must not contain an invalid `__assume` statement on a reachable path. If the compiler can reach an invalid `__assume` statement, the program might cause unpredictable and potentially dangerous behavior.  
 ...
 Use `__assume` in an [ASSERT](../VS_csharp/_assert--_asserte--_assert_expr-macros.md) only when the assert is not recoverable. Do not use `__assume` in an assert for which you have subsequent error recovery code because the compiler might optimize away the error-handling code.  
...
|---------------|------------------|  
|`__assume`|x86, ARM, [!INCLUDE[vcprx64](../VS_csharp/includes/vcprx64_md.md)]|  
