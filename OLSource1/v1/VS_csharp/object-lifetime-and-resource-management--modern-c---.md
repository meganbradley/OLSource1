...
### Heap-based lifetime  
 For heap object lifetime, use [smart pointers](../VS_csharp/smart-pointers--modern-c---.md). Use `shared_ptr` and `make_shared` as the default pointer and allocator. Use `weak_ptr` to break cycles, do caching, and observe objects without affecting or assuming anything about their lifetimes.  
...
```  
 ...
 Use `unique_ptr` for unique ownership, for example, in the *pimpl* idiom. (See [Pimpl For Compile-Time Encapsulation](../VS_csharp/pimpl-for-compile-time-encapsulation--modern-c---.md).) Make a `unique_ptr` the primary target of all explicit `new` expressions.  
