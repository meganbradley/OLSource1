...
---
# Extern Element
The Extern element references any external header (.h) files to merge with the .vsct file at compile time. The files to be merged must be on the Include path given to the VSCT compiler or referenced by an [Include element](../VS_csharp/include-element.md). The files may be other .vsct files or C++ header files.  
...
|language|Optional. The default language of all [<Strings\>](../VS_csharp/strings-element.md) elements in the command table:<br /><br /> language="en-us"|  
...
|Element|Description|  
|-------------|-----------------|  
|[CommandTable Element](../VS_csharp/commandtable-element.md)|Defines all of the elements that represent commands — that is, menu items, menus, toolbars, and combo boxes — that a VSPackage provides to the IDE.|  
