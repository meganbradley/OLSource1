...
|----------|-----------------|-------------------|  
|[List](../VS_csharp/collections.list--t--union--fsharp-.md)|An ordered, immutable series of elements of the same type. Implemented as a linked list.|[Lists (F#)](../VS_csharp/lists--fsharp-.md)<br /><br /> [List Module](../VS_csharp/collections.list-module--fsharp-.md)|  
|[Array](../VS_csharp/collections.array-module--fsharp-.md)|A fixed-size, zero-based, mutable collection of consecutive data elements that are all of the same type.|[Arrays (F#)](../VS_csharp/arrays--fsharp-.md)<br /><br /> [Array Module](../VS_csharp/collections.array-module--fsharp-.md)<br /><br /> [Array2D Module](../VS_csharp/collections.array2d-module--fsharp-.md)<br /><br /> [Array3D Module](../VS_csharp/collections.array3d-module--fsharp-.md)|  
|[seq](../VS_csharp/collections.seq--t--type-abbreviation--fsharp-.md)|A logical series of elements that are all of one type. Sequences are particularly useful when you have a large, ordered collection of data but don't necessarily expect to use all the elements. Individual sequence elements are computed only as required, so a sequence can perform better than a list if not all the elements are used. Sequences are represented by the seq<'T> type, which is an alias for IEnumerable<T\>. Therefore, any .NET Framework type that implements <xref:System.Collections.Generic.IEnumerable`1*> can be used as a sequence.|[Sequences (F#)](../VS_csharp/sequences--fsharp-.md)<br /><br /> [Seq Module](../VS_csharp/collections.seq-module--fsharp-.md)|  
|[Map](../VS_csharp/collections.map--key--value--class--fsharp-.md)|An immutable dictionary of elements. Elements are accessed by key.|[Map Module](../VS_csharp/collections.map-module--fsharp-.md)|  
|[Set](../VS_csharp/collections.set--t--class--fsharp-.md)|An immutable set that's based on binary trees, where comparison is the F# structural comparison function, which potentially uses implementations of the <xref:System.IComparable*> interface on key values.|[Set Module](../VS_csharp/collections.set-module--fsharp-.md)|  
...
|find|O(N)|O(N)|O(N)|O(log N)|-|Returns the first element for which the given function returns `true`. Returns <xref:System.Collections.Generic.KeyNotFoundException*> if no such element exists.|  
|findIndex|O(N)|O(N)|O(N)|-|-|Returns the index of the first element in the array that satisfies the given predicate. Raises <xref:System.Collections.Generic.KeyNotFoundException*> if no element satisfies the predicate.|  
|findKey|-|-|-|O(log N)|-|Evaluates the function on each mapping in the collection, and returns the key for the first mapping where the function returns `true`. If no such element exists, this function raises <xref:System.Collections.Generic.KeyNotFoundException*>.|  
...
|max|O(N)|O(N)|O(N)|-|-|Returns the greatest element in the collection, compared by using the [max](../VS_csharp/operators.max--t--function--fsharp-.md) operator.|  
|maxBy|O(N)|O(N)|O(N)|-|-|Returns the greatest element in the collection, compared by using [max](../VS_csharp/operators.max--t--function--fsharp-.md) on the function result.|  
|maxElement|-|-|-|-|O(log N)|Returns the greatest element in the set according to the ordering that's used for the set.|  
|min|O(N)|O(N)|O(N)|-|-|Returns the least element in the collection, compared by using the [min](../VS_csharp/operators.min--t--function--fsharp-.md) operator.|  
|minBy|O(N)|O(N)|O(N)|-|-|Returns the least element in the collection, compared by using the [min](../VS_csharp/operators.min--t--function--fsharp-.md) operator on the function result.|  
...
|pick|O(N)|O(N)|O(N)|O(log N)|-|Applies the given function to successive elements, returning the first result where the function returns Some. If the function never returns Some, <xref:System.Collections.Generic.KeyNotFoundException*> is raised.|  
...
|sort|O(N log N) average<br /><br /> O(N^2) worst case|O(N log N)|O(N log N)|-|-|Sorts the collection by element value. Elements are compared using [compare](../VS_csharp/operators.compare--t--function--fsharp-.md).|  
|sortBy|O(N log N) average<br /><br /> O(N^2) worst case|O(N log N)|O(N log N)|-|-|Sorts the given list by using keys that the given projection provides. Keys are compared using [compare](../VS_csharp/operators.compare--t--function--fsharp-.md).|  
|sortInPlace|O(N log N) average<br /><br /> O(N^2) worst case|-|-|-|-|Sorts the elements of an array by mutating it in place and using the given comparison function. Elements are compared by using [compare](../VS_csharp/operators.compare--t--function--fsharp-.md).|  
|sortInPlaceBy|O(N log N) average<br /><br /> O(N^2) worst case|-|-|-|-|Sorts the elements of an array by mutating it in place and using the given projection for the keys. Elements are compared by using [compare](../VS_csharp/operators.compare--t--function--fsharp-.md).|  
