...
> [!NOTE]
>  The `ProjectItem` element accepts different attributes depending on whether the template is for a project or an item. This topic explains the `ProjectItem` element for item. For an explanation of the `ProjectItem` element for project templates, see [ProjectItem Element (Visual Studio Project Templates)](../VS_csharp/projectitem-element--visual-studio-project-templates-.md).  
...
|---------------|-----------------|  
|`SubType`|Optional attribute.<br /><br /> Specifies the subtype of an item in a multi-file item template. This value is used to determine the editor that [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] will use to open the item.|  
...
|Element|Description|  
|-------------|-----------------|  
|[TemplateContent](../VS_csharp/templatecontent-element--visual-studio-templates-.md)|Specifies the contents of the template.|  
