...
## Remarks  
 If there is no scheduler (see                 [Scheduler](../VS_csharp/scheduler-class.md)) associated with the calling context, many methods within the                 `CurrentScheduler` class will result in attachment of the process' default scheduler. This may also imply that the process' default scheduler is created during such a call.  
...
### Remarks  
 ...
 You must invoke the                         [Release](../VS_csharp/schedulegroup-class.md#schedulegroup__release_method) method on a schedule group when you are done scheduling work to it. The scheduler will destroy the schedule group when all work queued to it has completed.  
...
### Remarks  
 ...
 If there is no scheduler attached to the calling context, calling this method will result in a                         [scheduler_not_attached](../VS_csharp/scheduler_not_attached-class.md) exception being thrown.  
  
 Calling this method from a context that is internal to and managed by a scheduler, or a context that was attached using a method other than the                         [Scheduler::Attach](../VS_csharp/scheduler-class.md#scheduler__attach_method) or                         [CurrentScheduler::Create](#currentscheduler__create_method) methods, will result in an                         [improper_scheduler_detach](../VS_csharp/improper_scheduler_detach-class.md) exception being thrown.  
