...
-   Are linked in tab order to other MDI windows.  
 ...
 For more information about window behavior in Visual Studio, see [Window Management](../VS_csharp/customizing-window-layouts-in-visual-studio.md).  
  
## Document Window Implementation  
 Document windows are created by implementing an editor. The <xref:Microsoft.VisualStudio.Shell.Interop.IVsEditorFactory*> interface creates document windows as part of instantiating an editor. For more information, see [Custom Editors](../VS_csharp/legacy-interfaces-in-the-editor.md).  
...
## The Running Document Table  
 The IDE uses the running document table (RDT) to track the status of every document window. The RDT is the mechanism through which document windows are notified of events, such as when a solution is closed or when a file has been edited. For more information, see [Running Document Table](../VS_csharp/running-document-table.md).  
  
## See Also  
 [Delayed Document Loading](../VS_csharp/delayed-document-loading.md)