...
## Remarks  
 ...
 A `Try…Catch` statement consists of a `Try` block followed by one or more `Catch` clauses, which specify handlers for various exceptions. When an exception is thrown in a `Try` block, [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] looks for the `Catch` statement that handles the exception. If a matching `Catch` statement is not found, [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] examines the method that called the current method, and so on up the call stack. If no `Catch` block is found, [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] displays an unhandled exception message to the user and stops execution of the program.  
...
-   The statement has no `exception` variable, for example: `Catch`  
 ...
 When a `Try…Catch…Finally` statement is nested in another `Try` block, [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] first examines each `Catch` statement in the innermost `Try` block. If no matching `Catch` statement is found, the search proceeds to the `Catch` statements of the outer `Try…Catch…Finally` block.  
...
> [!TIP]
>  The `Try…Catch…Finally` statement is available as an IntelliSense code snippet. In the Code Snippets Manager, expand **Code Patterns - If, For Each, Try Catch, Property, etc**, and then **Error Handling (Exceptions)**. For more information, see [Code Snippets](../VS_csharp/code-snippets.md).  
...
-   An [End Statement](../VS_csharp/end-statement.md) is encountered in the `Try` or `Catch` block.  
...
-   Exception handling makes a program more complex.  
 ...
 [!code[VbVbalrStatements#94](../VS_csharp/codesnippet/VisualBasic/try...catch...finally-statement--visual-basic-_1.vb)]  
...
## Async Methods  
 If you mark a method with the [Async](../VS_csharp/async--visual-basic-.md) modifier, you can use the [Await](../VS_csharp/await-operator--visual-basic-.md) operator in the method. A statement with the `Await` operator suspends execution of the method until the awaited task completes. The task represents ongoing work. When the task that's associated with the `Await` operator finishes, execution resumes in the same method. For more information, see [Control Flow in Async Programs (C# and Visual Basic)](../VS_csharp/control-flow-in-async-programs--csharp-and-visual-basic-.md).  
...
## Iterators  
 An iterator function or `Get` accessor performs a custom iteration over a collection. An iterator uses a [Yield](../VS_csharp/yield-statement--visual-basic-.md) statement to return each element of the collection one at a time. You call an iterator function by using a [For Each...Next Statement (Visual Basic)](../VS_csharp/for-each...next-statement--visual-basic-.md).  
  
 A `Yield` statement can be inside a `Try` block. A `Try` block that contains a `Yield` statement can have `Catch` blocks, and can have a `Finally` block. See the "Try Blocks in Visual Basic" section of [Iterators (C# and Visual Basic)](../VS_csharp/iterators--csharp-and-visual-basic-.md) for an example.  
...
## Partial-Trust Situations  
 ...
 [!code[VbVbalrStatements#85](../VS_csharp/codesnippet/VisualBasic/try...catch...finally-statement--visual-basic-_2.vb)]  
...
## Example  
 ...
 [!code[VbVbalrStatements#86](../VS_csharp/codesnippet/VisualBasic/try...catch...finally-statement--visual-basic-_3.vb)]  
...
## Example  
 ...
 [!code[VbVbalrStatements#91](../VS_csharp/codesnippet/VisualBasic/try...catch...finally-statement--visual-basic-_4.vb)]  
...
## Example  
 ...
 [!code[VbVbalrStatements#92](../VS_csharp/codesnippet/VisualBasic/try...catch...finally-statement--visual-basic-_5.vb)]  
...
## Example  
 ...
 [!code[VbVbalrStatements#93](../VS_csharp/codesnippet/VisualBasic/try...catch...finally-statement--visual-basic-_6.vb)]  
...
## Example  
 ...
 [!code[csAsyncExceptions#1](../VS_csharp/codesnippet/VisualBasic/try...catch...finally-statement--visual-basic-_7.vb)]  
...
## Example  
 ...
 [!code[csAsyncExceptions#3](../VS_csharp/codesnippet/VisualBasic/try...catch...finally-statement--visual-basic-_8.vb)]  
  
## See Also  
 <xref:Microsoft.VisualBasic.Information.Err*>   
 <xref:System.Exception*>   
 [Exit Statement](../VS_csharp/exit-statement--visual-basic-.md)   
 [On Error Statement](../VS_csharp/on-error-statement--visual-basic-.md)   
 [Best Practices for Using IntelliSense Code Snippets](../VS_csharp/best-practices-for-using-code-snippets.md)   
 [Exception Handling (Task Parallel Library)](assetId:///beb51e50-9061-4d3d-908c-56a4f7c2e8c1)   
 [Throw Statement (Visual Basic)](../VS_csharp/throw-statement--visual-basic-.md)