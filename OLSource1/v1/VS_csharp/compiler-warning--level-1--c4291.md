...
'declaration' : no matching operator delete found; memory will not be freed if initialization throws an exception  
 ...
 A placement [new](../VS_csharp/new-operator--c---.md) is used for which there is no placement [delete](../VS_csharp/delete-operator--c---.md).  
 ...
 If you use the operator **new** without any extra arguments and compile with [/GX](../VS_csharp/-gx--enable-exception-handling-.md), [/EHs](../VS_csharp/-eh--exception-handling-model-.md), or /EHa options to enable exception handling, the compiler will generate code to call operator **delete** if the constructor throws an exception.  
