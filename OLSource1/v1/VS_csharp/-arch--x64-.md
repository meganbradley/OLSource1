...
---
# -arch (x64)
Specifies the architecture for code generation on x64. Also see [/arch (x86)](../VS_csharp/-arch--x86-.md) and [/arch (ARM)](../VS_csharp/-arch--arm-.md).  
 ...
 **/arch** only affects code generation for native functions. When you use [/clr](../VS_csharp/-clr--common-language-runtime-compilation-.md) to compile, **/arch** has no effect on code generation for managed functions.  
 ...
 The `__AVX__` preprocessor symbol is defined when the **/arch:AVX** compiler option is specified. The `__AVX2__` preprocessor symbol is defined when the **/arch:AVX2** compiler option is specified. For more information, see [Predefined Macros](../VS_csharp/predefined-macros.md). The **/arch:AVX2** option was introduced in Visual Studio 2013 Update 2, version 12.0.34567.1.  
