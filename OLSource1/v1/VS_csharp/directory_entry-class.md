...
Describes an object that is returned by `*X`, where *X* is a [directory_iterator](../VS_csharp/directory_iterator-class.md) or a [recursive_directory_iterator](../VS_csharp/recursive_directory_iterator-class.md).  
...
## Remarks  
 The class stores an object of type [path](../VS_csharp/path-class--c---standard-template-library-.md). The stored `path` can be an instance of the [path Class](../VS_csharp/path-class--c---standard-template-library-.md) or of a type that is derived from `path`. It also stores two [file_type](../VS_csharp/file_type-enumeration.md) values; one that represents what is known about the status of the stored file name, and another that represents what is known about the symbolic link status of the file name.  
