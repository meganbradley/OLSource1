...
|Visual Studio Visualization and Modeling SDK|[http://go.microsoft.com/fwlink/?LinkID=186128](http://go.microsoft.com/fwlink/?LinkID=186128)|  
 ...
 To create a new domain-specific language, you create a new [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] solution using the Domain-Specific Language project template.  
...
2.  Click each template to see a description. Choose the solution that most closely resembles what you want to create.  
    ...
    -   Select **Minimal WinForm Designer** or **Minimal WPF Designer** to create a DSL that is displayed on a Windows Forms or WPF surface. You will have to write code to define the editor. For more information, see the following topics:  
         ...
         [Walkthrough: Creating a Windows Forms-Based Domain-Specific Language](../VS_csharp/creating-a-windows-forms-based-domain-specific-language.md)  
  
         [Walkthrough: Creating a WPF-Based Domain-Specific Language](../VS_csharp/creating-a-wpf-based-domain-specific-language.md)  
...
### Test the Solution  
 ...
 To test the solution, press F5 or CTRL+F5. A new instance of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] opens in experimental mode.  
  
 In the new instance of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)], in Solution Explorer, open the Sample file. It opens as a diagram, with a toolbox.  
  
 If you run a solution that you have created from the **Minimal Language** template, your experimental [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] will resemble the following example:  
  
 ![](../VS_csharp/media/dsl_min.png "DSL_min")  
  
 Experiment with the tools. Create elements and connect them.  
  
 Close the experimental instance of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)].  
...
>  After adding a feature, do not forget to click **Transform All Templates** in the toolbar of Solution Explorer before you build and running your DSL.  
 ...
 ![Embedding and Reference relationships](../VS_csharp/media/music_classes.png "Music_Classes")  
  
 The next figure is an example model of this DSL:  
  
 ![Instance model of generated DSL](../VS_csharp/media/music_instance.png "Music_Instance")  
...
##  <a name="classes"></a> Defining Domain Classes  
 ...
 For more information, see [Properties of Domain Classes](../VS_csharp/properties-of-domain-classes.md).  
...
|Not directly displayed.|The class of which it forms part.||  
 ...
 For more information, see [Properties of Relationships](../VS_csharp/properties-of-domain-relationships.md) and [Properties of Roles](../VS_csharp/properties-of-domain-roles.md).  
...
2.  **Build and run the DSL.** Press F5 or CTRL+F5 to run a new instance of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] in experimental mode. In the experimental instance of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)], open or create a file that has the file name extension of your DSL.  
...
6.  **Map the properties to the text decorators.**  
    ...
    4.  Make sure that a check mark appears alongside each decorator name.  
     ...
     ![Shape Mappings and DSL Details window](../VS_csharp/media/dsldetailswindow.png "DslDetailsWindow")  
...
7.  **Make a toolbox item for creating elements of the domain class.**  
 ...
 For more information, see [Properties of Geometry Shapes](../VS_csharp/properties-of-geometry-shapes.md) and [Properties of Image Shapes](../VS_csharp/properties-of-image-shapes.md).  
...
5.  **Verify that each text decorator appears,** and that:  
 ...
 After you first test a shape, you might want to adjust some its properties and add some more advanced features. For more information, see [Customizing Shape Definitions](../VS_csharp/customizing-and-extending-a-domain-specific-language.md).  
 ...
 **Set the Allows Duplicates property of the relationship,** if more than one link of the same class can exist between the same pair of model elements. For example, you could allow a Teacher to teach more than one Subject to the same Student.  
 ...
 ![Shape maps for connectors](../VS_csharp/media/music_connector.png "Music_Connector")  
...
8.  **Verify that each text decorator appears,** and that:  
 ...
 After you first test a connector, you might want to adjust some its properties and add some more advanced features. For more information, see [Customizing Shape Definitions](../VS_csharp/customizing-and-extending-a-domain-specific-language.md).  
...
##  <a name="compartments"></a> Defining Shapes that Contain Lists: Compartment Shapes  
 ...
 ![Compartment Shape](../VS_csharp/media/compartmentshape.png "CompartmentShape")  
 ...
 ![Shape map](../VS_csharp/media/music_mapcomp.png "Music_MapComp")  
  
 For more information, see [Properties of Compartment Shapes](../VS_csharp/properties-of-compartment-shapes.md).  
...
7.  Open the language Explorer. Verify that you can see the container nodes with the list item nodes inside.  
 ...
 ![Generated explorer of DSL](../VS_csharp/media/music_explorer.png "Music_Explorer")  
  
 After you first test a compartment shape, you might want to adjust some of its properties and add some more advanced features. For more information, see [Customizing Shape Definitions](../VS_csharp/customizing-and-extending-a-domain-specific-language.md).  
...
-   The port domain classes are mapped to port shapes. You can either map the derived classes to separate port shape classes, or map the base class to one port shape class.  
 ...
 For more information, see [Properties of Port Shapes](../VS_csharp/properties-of-port-shapes.md).  
...
##  <a name="swimlanes"></a> Defining a DSL that has Swimlanes  
 ...
 For more information, see [Properties of Swimlanes](../VS_csharp/properties-of-swimlanes.md).  
...
1.  Fine-tune the DSL by using more features of the DSL Definition. For example, you can make a single connector tool that can create several types of connector, and you can control the rules by which deleting one element also deletes related elements. These techniques are mostly achieved by setting values in the DSL Definition, and some require a few lines of program code.  
     ...
     For more information, see [Customizing and Extending a Domain-Specific Language](../VS_csharp/customizing-and-extending-a-domain-specific-language.md).  
  
2.  Extend your modeling tools by using program code to achieve more advanced effects. For example, you can create menu commands that can change the model, and you can create tools that integrate two or more DSLs. VMSDK is designed specifically to make it easy to integrate your extensions with the code that is generated from the DSL Definition.  For more information, see [Programming Domain Specific Languages](../VS_csharp/writing-code-to-customise-a-domain-specific-language.md).  
...
### Changing the DSL Definition  
 ...
 See also [How to: Change the Namespace of a Domain-Specific Language](../VS_csharp/how-to--change-the-namespace-of-a-domain-specific-language.md).  
...
|The toolbox of my DSL does not appear after I changed the assembly names.|Inspect and update **DslPackage\GeneratedCode\Package.tt** For more information, see [How to Change the Namespace](../VS_csharp/how-to--change-the-namespace-of-a-domain-specific-language.md).|  
|The toolbox of my DSL does not appear, but I have not changed the assembly name.<br /><br /> Or, a message box appears reporting the failure to load an extension.|Reset the experimental instance, and rebuild your solution.<br /><br /> 1.  At the Windows Start menu, under **All Programs**, expand [!INCLUDE[vssdk_current_long](../VS_csharp/includes/vssdk_current_long_md.md)], then **Tools**, and then click **Reset the Microsoft Visual Studio Experimental Instance**.<br />2.  On the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)]**Build** menu, click **Rebuild Solution**.|  
  
## See Also  
 [Getting Started with DSLs](../VS_csharp/getting-started-with-domain-specific-languages.md)   
 [Walkthrough: Creating a Windows Forms-Based Domain-Specific Language](../VS_csharp/creating-a-windows-forms-based-domain-specific-language.md)   
 [Walkthrough: Creating a WPF-Based Domain-Specific Language](../VS_csharp/creating-a-wpf-based-domain-specific-language.md)