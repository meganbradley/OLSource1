...
## Remarks  
 The object orders the sequence it controls by calling two stored objects, a comparison function object of type[key_equal](#unordered_set__key_equal) and a hash function object of type[hasher](#unordered_set__hasher). You access the first stored object by calling the member function[key_eq](#unordered_set__key_eq)`()`; and you access the second stored object by calling the member function[hash_function](#unordered_set__hash_function)`()`. Specifically, for all values`X` and`Y` of type`Key`, the call`key_eq()(X, Y)` returns true only if the two argument values have equivalent ordering; the call`hash_function()(keyval)` yields a distribution of values of type`size_t`. Unlike template class[unordered_multiset](../VS_csharp/unordered_multiset-class.md), an object of template class`unordered_set` ensures that`key_eq()(X, Y)` is always false for any two elements of the controlled sequence. (Keys are unique.)  
...
### Remarks  
 ...
 For a code example, see[set::emplace](../VS_csharp/set-class.md#set__emplace).  
...
### Remarks  
 ...
 For a code example, see[set::emplace_hint](../VS_csharp/set-class.md#set__emplace_hint).  
...
### Remarks  
 For a code example, see[set::erase](../VS_csharp/set-class.md#set__erase).  
...
|`ValTy`|Template parameter that specifies the argument type that the unordered_set can use to construct an element of[value_type](../VS_csharp/map-class.md#map__value_type), and perfect-forwards`Val` as an argument.|  
...
### Return Value  
 The single-element member functions, (1) and (2), return a[pair](../VS_csharp/pair-structure.md) whose`bool` component is true if an insertion was made, and false if the unordered_set already contained an element whose key had an equivalent value in the ordering. The iterator component of the return-value pair points to the newly inserted element if the`bool` component is true, or to the existing element if the`bool` component is false.  
...
### Remarks  
 ...
 The[value_type](../VS_csharp/map-class.md#map__value_type) of a container is a typedef that belongs to the container, and, for set,                         `unordered_set<V>::value_type` is type`const V`.  
 ...
 The initializer list member function (6) uses an[initializer_list](../VS_csharp/-initializer_list-.md) to copy elements into the unordered_set.  
  
 For insertion of an element constructed in placeâ€”that is, no copy or move operations are performedâ€”see[unordered_set::emplace](../VS_csharp/set-class.md#set__emplace) and[unordered_set::emplace_hint](../VS_csharp/set-class.md#set__emplace_hint).  
  
 For a code example, see[set::insert](../VS_csharp/set-class.md#set__insert).  
  
##  <a name="unordered_set__iterator"></a>  unordered_set::iterator  
 A type that provides a constant[forward iterator](../VS_csharp/forward_iterator_tag-struct.md) that can read elements in an unordered_set.  
...
### Example  
  See the example for[begin](../VS_csharp/set-class.md#set__begin) for an example of how to declare and use an**iterator**.  
...
|-|-|  
|Parameter|Description|  
|`_Right`|The[unordered_set](../VS_csharp/unordered_set-class.md) being copied into the`unordered_set`.|  
 ...
 **[d] [c] [b] [a]**    
## See Also  
 [<unordered_set>](../VS_csharp/-unordered_set-.md)   
