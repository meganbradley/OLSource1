---
title: "Project Types"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "vs-ide-sdk"
ms.tgt_pltfrm: na
ms.topic: "article"
helpviewer_keywords: 
  - "project types, adding"
  - "projects [Visual Studio SDK], adding new types"
ms.assetid: 263a084f-f97a-4e09-add7-f0e8a6a27daf
caps.latest.revision: 26
ms.author: "gregvanl"
translation.priority.mt: 
  - "de-de"
  - "ja-jp"
---
# Project Types
[!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] includes several project types for languages such as [!INCLUDE[csprcs](../VS_csharp/includes/csprcs_md.md)] and [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)]. [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] also lets you create your own project types.  
  
## In This Section  
 [Project Type Essentials](../VS_csharp/project-type-essentials.md)  
 Presents the important information that you must have to start working with project types.  
  
 [Creating Project Types](../VS_csharp/creating-project-types.md)  
 Discusses the design of project types.  
  
 [Walkthrough: Adding a Command to the Solution Explorer Toolbar (C#)](../VS_csharp/adding-a-command-to-the-solution-explorer-toolbar.md)  
 Details the steps that you must follow to add a button to the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] **Solution Explorer** toolbar.  
  
 [How to: Add Web Services to Project Systems](../Topic/Adding%20Web%20Services%20to%20Project%20Systems.md)  
 Shows how to integrate Web services into your VSPackage project system.  
  
 [How to: Rename Project Hierarchy Nodes (C++)](../Topic/Renaming%20Project%20Hierarchy%20Nodes%20\(C++\).md)  
 Shows how to rename a project folder hierarchy node.  
  
 [Adding Project and Project Item Templates](../VS_csharp/adding-project-and-project-item-templates.md)  
 Discusses how you can add templates to your project types so that users can create new projects and project items according to a pattern.  
  
 [Opening and Saving Project Items](../VS_csharp/opening-and-saving-project-items.md)  
 Provides information about how to manage the items your project type supports.  
  
 [Managing Configuration Options](../VS_csharp/managing-configuration-options.md)  
 Discusses how your project types can support configuration options like Debug and Release that control how projects are built, debugged, and so forth.  
  
 [Supporting Source Control](../VS_csharp/supporting-source-control.md)  
 Provides information about how to add support for source-control systems to your project types.  
  
 [Nesting Projects](../VS_csharp/nesting-projects.md)  
 Explains how your project types can support *nesting*, so that projects can be grouped together in **Solution Explorer**.  
  
 [Upgrading Projects](../VS_csharp/upgrading-projects.md)  
 Describes how your project type can participate in the Upgrade Wizard to upgrade project files from an earlier version of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)].  
  
 [Project Types Architecture](../VS_csharp/project-types-architecture.md)  
 Provides detailed technical information about project types.  
  
## Related Sections  
 [Hierarchies and Selection](../VS_csharp/hierarchies-and-selection.md)  
 Provides an overview of how the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] integrated development environment (IDE) displays a project as a hierarchy.  
  
 [Project Subtypes](../VS_csharp/project-subtypes.md)  
 Provides links to project subtype topics. Project subtypes enable extension of most kinds of project types, including your own.  
  
 [Projects (Visual Studio SDK)](../VS_csharp/projects.md)  
 Describes how to extend the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] project system.