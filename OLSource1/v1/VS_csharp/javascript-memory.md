...
-   Reduce non-actionable info in the JavaScript heap data.  
 ...
 For a tutorial that leads you through the process of identifying a memory leak in a working app, see [Walkthrough: Find a memory leak](../VS_csharp/walkthrough--find-a-memory-leak--javascript-.md).  
...
##  <a name="Run"></a> Run the JavaScript memory analyzer  
 You can use the memory analyzer when you have a working Windows Store app open in Visual Studio or installed on a computer that is running [!INCLUDE[win8](../VS_csharp/includes/win8_md.md)] or later.  
...
2.  If you're running the app from Visual Studio, in the **Start Debugging** list on the **Standard** toolbar, choose the debug target for your project: either a Windows Phone Emulator or, for a Windows Store app, **Local Machine**, **Simulator**, or **Remote Machine**.  
     ...
     For more info about these options, see [Run apps from Visual Studio](../VS_csharp/run-store-apps-from-visual-studio.md).  
...
3.  On the menu bar, choose **Debug**, **Performance Profiler...**.  
     ...
     ![Change analysis target](../VS_csharp/media/js_tools_target.png "JS_Tools_Target")  
...
6.  Take a baseline heap snapshot by choosing **Take Heap Snapshot**.  
     ...
     ![Baseline snapshot](../VS_csharp/media/js_mem_leak_workflow_baseline.png "JS_Mem_Leak_Workflow_Baseline")  
...
10. Switch to Visual Studio and take a third snapshot.  
     ...
     ![Second and third snapshot](../VS_csharp/media/js_mem_leak_workflow.png "JS_Mem_Leak_Workflow")  
...
12. Choose one of the following **Scope** filter options to help identify memory usage issues:  
    ...
    >  Use the filtered view of objects left over from the previous snapshot to investigate memory leaks. For example, if the differential object count is +205 / -195, this view will show the 10 objects left over, and these are likely candidates for memory leaks.  
     ...
     ![Snapshot diff view showing types](../VS_csharp/media/js_mem_snapshot_diff.png "JS_Mem_Snapshot_Diff")  
...
##  <a name="LiveMemory"></a> View live memory usage summary  
 ...
 ![JavaScript Memory Analyzer memory graph](../VS_csharp/media/js_mem_memory_graph.png "JS_Mem_Memory_Graph")  
...
>  The JavaScript memory analyzer forces a garbage collection before each snapshot. This helps ensure more consistent results among runs.  
 ...
 ![Snapshot summary](../VS_csharp/media/js_mem_snapshot_summary.png "JS_Mem_Snapshot_Summary")  
...
##  <a name="SnapshotDetails"></a> View snapshot details  
 ...
 ![Snapshot details view showing potential issues](../VS_csharp/media/js_mem_snapshot_details.png "JS_Mem_Snapshot_Details")  
...
-   **All objects in Snapshot #<number\>**. This filter setting doesn't filter out any objects on the heap.  
 ...
 To show object references that don't match the current **Scope** filter, select **Show non-matching references** in the settings list ![Settings drop&#45;down list in memory analyzer](../VS_csharp/media/js_mem_settings.png "JS_Mem_Settings") in the upper-right corner of the pane. If you enable this setting, non-matching references are displayed with gray text.  
...
##  <a name="FoldObjects"></a> View objects by dominator  
 ...
 To toggle the view of objects by dominator, choose the **Fold in objects by dominator** button. ![Folding objects into their dominators](../VS_csharp/media/js_mem_fold_objects.png "JS_Mem_Fold_Objects")  
...
##  <a name="BuiltInValues"></a> Show built-in objects  
 ...
 To display these objects, choose **Show built-ins** in the settings list ![Settings drop&#45;down list in memory analyzer](../VS_csharp/media/js_mem_settings.png "JS_Mem_Settings") in the upper-right corner of the pane.  
...
-   `console.takeHeapSnapshot` takes a heap snapshot that appears in the JavaScript memory analyzer. This command is one of the [JavaScript Console Commands](../VS_csharp/javascript-console-commands.md).  
...
```  
 ...
 ![Using a Profile Mark](../VS_csharp/media/js_mem_performance_marks.png "JS_Mem_Performance_Marks")  
...
-   Look for objects that are retained in memory inadvertently after the user has navigated to a new page, which is a common cause of memory issues. For example:  
    ...
    -   You might need to remove one or more event listeners. For more info, see [View DOM event listeners](../VS_csharp/view-dom-event-listeners.md).  
...
## See Also  
 [Walkthrough: Find a memory leak](../VS_csharp/walkthrough--find-a-memory-leak--javascript-.md)