...
```  
 ...
 Using `#elif` is simpler, because each `#if` requires a [#endif](../VS_csharp/sharpendif--csharp-reference-.md), whereas a `#elif` can be used without a matching `#endif`.  
  
 See [#if](../VS_csharp/sharpif--csharp-reference-.md) for an example of how to use `#elif`.  
