...
|`CDynamicParameterAccessor`|Automatic, but can be [overridden](../VS_csharp/overriding-a-dynamic-accessor.md).|Yes, if the provider supports `ICommandWithParameters`. Parameters bound automatically.|Slower than `CDynamicAccessor` but useful for calling generic stored procedures.|  
