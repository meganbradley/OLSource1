...
You can generate text strings in your application at run time by using [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] runtime text templates. The computer where the application executes does not have to have [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)]. Runtime templates are sometimes called "preprocessed text templates" because at compile time, the template generates code that is executed at run time.  
...
2.  In the **Add New Item** dialog box, select **Runtime Text Template**. (In [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] look under **Common Items\General**.)  
...
## Converting an Existing File to a Run-Time Template  
 A good way to create a template is to convert an existing example of the output. For example, if your application will generate HTML files, you can start by creating a plain HTML file. Make sure that it works correctly and that its appearance is correct. Then include it into your [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] project and convert it to a template.  
  
#### To convert an existing text file to a run-time template  
  
1.  Include the file into your [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] project. In Solution Explorer, on the shortcut menu of the project, choose **Add**, **Existing Item**.  
...
```  
 ...
 Notice that statements are inserted between `<# ... #>` and expressions are inserted between `<#= ... #>`. For more information, see [Writing a Text Template](../VS_csharp/writing-a-t4-text-template.md).  
...
#### Constructor parameters in Visual Basic  
 In [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)], the separate file **MyWebPageCode.vb** contains:  
...
## Related Topics  
 Design Time Templates: If you want to use a template to generate code that becomes part of your application, see [Generating Files in a Visual Studio Solution](../VS_csharp/design-time-code-generation-by-using-t4-text-templates.md).  
  
 Runtime templates can be used in any application where the templates and their content are determined at compile time. But if you want to write a [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] extension that generates text from templates that change at run time, see [Invoking Text Transformation in a Visual Studio Extension](../VS_csharp/invoking-text-transformation-in-a-vs-extension.md).  
  
## See Also  
 [Generating artifacts by using text templates](../VS_csharp/code-generation-and-t4-text-templates.md)   
 [Writing a Text Template](../VS_csharp/writing-a-t4-text-template.md)   
