...
## Remarks  
 ...
 To use the synchronization classes [CSemaphore](../VS_csharp/csemaphore-class.md), [CMutex](../VS_csharp/cmutex-class.md), and [CEvent](../VS_csharp/cevent-class.md), you can create either a **CMultiLock** or [CSingleLock](../VS_csharp/csinglelock-class.md) object to wait on and release the synchronization object. Use **CMultiLock** when there are multiple objects that you could use at a particular time. Use `CSingleLock` when you only need to wait on one object at a time.  
 ...
 **CMultiLock** objects are most useful when a thread has a large number of `CEvent` objects it can respond to. Create an array containing all the `CEvent` pointers, and call `Lock`. This will cause the thread to wait until one of the events is signaled.  
 ...
 For more information on how to use **CMultiLock** objects, see the article [Multithreading: How to Use the Synchronization Classes](../VS_csharp/multithreading--how-to-use-the-synchronization-classes.md).  
 ...
 *dwTimeOut*  
 ...
 `dwWakeMask`  
 Specifies other conditions that are allowed to abort the wait. For a full list of the available options for this parameter, see                                 [MsgWaitForMultipleObjects](http://msdn.microsoft.com/library/windows/desktop/ms684242) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
...
### Remarks  
 ...
 The first form of `Unlock` tries to unlock the synchronization object managed by `CMultiLock`. The second form of `Unlock` tries to unlock the `CSemaphore` objects owned by `CMultiLock`. If `CMultiLock` does not own any locked `CSemaphore` object, the function returns **FALSE**; otherwise, it returns **TRUE**. `lCount` and `lpPrevCount` are exactly the same as the parameters of [CSingleLock::Unlock](../VS_csharp/csinglelock-class.md#csinglelock__unlock). The second form of `Unlock` is rarely applicable to multilock situations.  
