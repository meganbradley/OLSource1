...
Converts a sequence of multibyte characters to a corresponding sequence of wide characters. Versions of [mbstowcs](../VS_csharp/mbstowcs--_mbstowcs_l.md) with security enhancements as described in [Security Enhancements in the CRT](../VS_csharp/security-features-in-the-crt.md).  
...
#### Parameters  
 ...
 [in] `count`  
 The maximum number of wide characters to store in the `wcstr` buffer, not including the terminating null, or [_TRUNCATE](../VS_csharp/_truncate.md).  
...
-   The number of wide characters stored in the `wcstr` buffer equals `count`.  
 ...
 If `count` is the special value [_TRUNCATE](../VS_csharp/_truncate.md), then `mbstowcs_s` converts as much of the string as will fit into the destination buffer, while still leaving room for a null terminator.  
...
>  Ensure that `wcstr` and `mbstr` do not overlap, and that `count` correctly reflects the number of multibyte characters to convert.  
 ...
 `mbstowcs_s` uses the current locale for any locale-dependent behavior; `_mbstowcs_s_l` is identical except that it uses the locale passed in instead. For more information, see [Locale](../VS_csharp/locale.md).  
