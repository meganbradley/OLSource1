...
## Remarks  
 ...
 For more information on `CGdiObject`, see [Graphic Objects](../VS_csharp/graphic-objects.md).  
...
### Parameters  
 `nIndex`  
 A constant specifying the type of stock object desired. See the parameter                                 *fnObject* for                                 [GetStockObject](http://msdn.microsoft.com/library/windows/desktop/dd144925) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)] for a description of appropriate values.  
...
### Example  
 [!code[NVC_MFCDocView#175](../VS_csharp/codesnippet/CPP/cgdiobject-class_1.cpp)]  
...
|Object|Buffer type|  
|------------|-----------------|  
|`CPen`|[LOGPEN](../VS_csharp/logpen-structure.md)|  
|`CBrush`|[LOGBRUSH](../VS_csharp/logbrush-structure.md)|  
|`CFont`|[LOGFONT](http://msdn.microsoft.com/library/windows/desktop/dd145037)|  
|`CBitmap`|[BITMAP](../VS_csharp/bitmap-structure.md)|  
|`CPalette`|**WORD**|  
|`CRgn`|Not supported|  
  
 If the object is a `CBitmap` object, `GetObject` returns only the width, height, and color format information of the bitmap. The actual bits can be retrieved by using [CBitmap::GetBitmapBits](../VS_csharp/cbitmap-class.md#cbitmap__getbitmapbits).  
  
 If the object is a `CPalette` object, `GetObject` retrieves a **WORD** that specifies the number of entries in the palette. The function does not retrieve the                         [LOGPALETTE](http://msdn.microsoft.com/library/windows/desktop/dd145040) structure that defines the palette. An application can get information on palette entries by calling [CPalette::GetPaletteEntries](../VS_csharp/cpalette-class.md#cpalette__getpaletteentries).  
...
### Example  
  See the example for [CWnd::IsWindowEnabled](../VS_csharp/cwnd-class.md#cwnd__iswindowenabled).  
