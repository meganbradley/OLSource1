...
> [!NOTE]
>  This topic applies to objects derived from `CRecordset` in which bulk row fetching has not been implemented. If bulk row fetching is implemented, the architecture is similar. To understand the differences, see [Recordset: Fetching Records in Bulk (ODBC)](../VS_csharp/recordset--fetching-records-in-bulk--odbc-.md).  
  
##  <a name="_core_a_sample_class"></a> Sample Class  
 When you use the [MFC ODBC Consumer Wizard](../VS_csharp/adding-an-mfc-odbc-consumer.md) from **Add Class** wizard to declare a recordset class derived from `CRecordset`, the resulting class has the general structure shown in the following simple class:  
...
> [!NOTE]
>  The order of parameters is important. For information about this and more information about parameters, see [Recordset: Parameterizing a Recordset (ODBC)](../VS_csharp/recordset--parameterizing-a-recordset--odbc-.md).  
...
##  <a name="_core_using_m_nfields_and_m_nparams"></a> Using m_nFields and m_nParams  
 ...
 For more information and examples, see [Record Field Exchange: Using RFX](../VS_csharp/record-field-exchange--using-rfx.md).  
