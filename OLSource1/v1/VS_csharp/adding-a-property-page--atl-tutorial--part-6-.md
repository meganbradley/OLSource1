...
1.  Replace the `Apply` function in PolyProp.h with the following code:  
     ...
     [!code[NVC_ATL_Windowing#58](../VS_csharp/codesnippet/CPP/adding-a-property-page--atl-tutorial--part-6-_1.h)]  
  
 A property page can have more than one client attached to it at a time, so the `Apply` function loops around and calls `put_Sides` on each client with the value retrieved from the edit box. You are using the [CComQIPtr](../VS_csharp/ccomqiptr-class.md) class, which performs the `QueryInterface` on each object to obtain the `IPolyCtl` interface from the **IUnknown** interface (stored in the `m_ppUnk` array).  
...
1.  Add the following code in Polyprop.cpp to the `OnEnChangeSides` method (deleting any code that the wizard put there):  
     ...
     [!code[NVC_ATL_Windowing#59](../VS_csharp/codesnippet/CPP/adding-a-property-page--atl-tutorial--part-6-_2.cpp)]  
...
1.  Open PolyCtl.h and add this line to the property map:  
     ...
     [!code[NVC_ATL_Windowing#60](../VS_csharp/codesnippet/CPP/adding-a-property-page--atl-tutorial--part-6-_3.h)]  
  
 The control's property map now looks like this:  
  
 [!code[NVC_ATL_Windowing#61](../VS_csharp/codesnippet/CPP/adding-a-property-page--atl-tutorial--part-6-_4.h)]  
...
## Building and Testing the Control  
 ...
 [Back to Step 5](../VS_csharp/adding-an-event--atl-tutorial--part-5-.md) &#124; [On to Step 7](../VS_csharp/putting-the-control-on-a-web-page--atl-tutorial--part-7-.md)  
