...
---
# Debugger Components
The [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] debugger is implemented as a VSPackage and manages the entire debug session. The debug session comprises the following elements:  
  
-   **Debug Package:** The [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] debugger provides the same user interface no matter what is being debugged.  
  
-   **Session debug manager (SDM):** Provides a consistent programmatic interface to the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] Debugger for the management of a variety of debug engines. It is implemented by [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)].  
  
-   **Process debug manager (PDM):** Manages, for all running instances of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)], a list of all programs that can be or are being debugged. It is implemented by [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)].  
  
-   **Debug engine (DE):** Is responsible for monitoring a program being debugged, communicating the state of the running program to the SDM and the PDM, and interacting with the expression evaluator and symbol provider to provide real-time analysis of the state of a program's memory and variables. It is implemented by [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] (for the languages it supports) and third-party vendors who want to support their own run time.  
...
## In This Section  
 ...
 [Operational Modes](../VS_csharp/operational-modes.md)  
 ...
 [Expression Evaluator](../VS_csharp/expression-evaluator.md)  
 Explains the purpose of the EE at run time.  
  
 [Symbol Provider](../VS_csharp/symbol-provider.md)  
...
## See Also  
 [Getting Started (Visual Studio Debugging SDK)](../VS_csharp/getting-started-with-debugger-extensibility.md)