...
### Remarks  
 ...
 An object is pinned only while a `pin_ptr` points to it. The object is no longer pinned when its pinning pointer goes out of scope, or is set to [nullptr](../VS_csharp/nullptr---c---component-extensions-.md). After the `pin_ptr` goes out of scope, the object that was pinned can be moved in the heap by the garbage collector. Any native pointers that still point to the object will not be updated, and de-referencing one of them could raise an unrecoverable exception.  
  
 If no pinning pointers point to the object (all pinning pointers went out of scope, were reassigned to point to other objects, or were assigned [nullptr](../VS_csharp/nullptr---c---component-extensions-.md)), the object is guaranteed not to be pinned.  
...
-   the target type of a cast.  
 ...
 `pin_ptr` is in the `cli` namespace. For more information, see [cli Namespace](../VS_csharp/platform--default--and-cli-namespaces---c---component-extensions-.md).  
  
 For more information about interior pointers, see [interior_ptr (C++/CLI)](../VS_csharp/interior_ptr--c---cli-.md).  
  
 For more information about pinning pointers, see [How to: Pin Pointers and Arrays](../VS_csharp/how-to--pin-pointers-and-arrays.md) and [How to: Declare Pinning Pointers and Value Types](../VS_csharp/how-to--declare-pinning-pointers-and-value-types.md).  
