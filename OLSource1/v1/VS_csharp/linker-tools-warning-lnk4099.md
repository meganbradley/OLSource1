...
1.  Extract an object file from the library with [lib](../VS_csharp/lib-reference.md) **/extract:**`objectname`**.obj** `xyz`**.lib**.  
  
2.  Check the path to the .pdb file with **dumpbin /section:.debug$T /rawdata** `objectname`**.obj**.  
  
 You could also compile with [/Z7](../VS_csharp/-z7---zi---zi--debug-information-format-.md), so the pdb doesn't need to be used, or remove the [/DEBUG](../VS_csharp/-debug--generate-debug-info-.md) linker option if you do not have .pdb files for the objects you are linking.