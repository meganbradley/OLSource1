...
Like [printf](../VS_csharp/printf.printf--t--function--fsharp-.md), but calls the specified function to generate the result. For example, these let the printing force a flush after all output has been entered onto the channel, but not before.  
...
#### Parameters  
 `continutation`  
 Type: [string](../VS_csharp/core.string-type-abbreviation--fsharp-.md) `-> 'Result`  
  
 The function called after formatting to generate the format result.  
  
 `format`  
 Type: [StringFormat](../VS_csharp/printf.stringformat--t--result--type-abbreviation--fsharp-.md)`<'T,'Result>`  
