...
## Remarks  
 ...
 The `assert` macro prints a diagnostic message when `expression` evaluates to `false` (0) and calls [abort](../VS_csharp/abort.md) to terminate program execution. No action is taken if `expression` is `true` (nonzero). The diagnostic message includes the failed expression, the name of the source file and line number where the assertion failed.  
 ...
 For more information about CRT debugging, see [CRT Debugging Techniques](../VS_csharp/crt-debugging-techniques.md).  
 ...
 The `assert` macro is enabled in both the release and debug versions of the C run-time libraries when `NDEBUG` is not defined. When `NDEBUG` is defined, the macro is available but does not evaluate its argument and has no effect. When it is enabled, the `assert` macro calls `_wassert` for its implementation. Other assertion macros, [_ASSERT](../VS_csharp/_assert--_asserte--_assert_expr-macros.md), [_ASSERTE](../VS_csharp/_assert--_asserte--_assert_expr-macros.md) and [_ASSERT_EXPR](../VS_csharp/_assert--_asserte--_assert_expr-macros.md), are also available, but they only evaluate the expressions passed to them when the [_DEBUG](../VS_csharp/_debug.md) macro has been defined and when they are in code linked with the debug version of the C run-time libraries.  
...
## See Also  
 ...
 [raise](../VS_csharp/raise.md)   
 [signal](../VS_csharp/signal.md)   
 [_ASSERT, _ASSERTE, _ASSERT_EXPR Macros](../VS_csharp/_assert--_asserte--_assert_expr-macros.md)   
