...
#### Parameters  
 `lpwsaData`  
 A pointer to a [WSADATA](../VS_csharp/wsadata-structure.md) structure. If `lpwsaData` is not equal to `NULL`, then the address of the `WSADATA` structure is filled by the call to `WSAStartup`. This function also ensures that `WSACleanup` is called for you before the application terminates.  
...
## Code  
 [!code[NVC_MFCAsyncSocket#4](../VS_csharp/codesnippet/CPP/afxsocketinit_1.cpp)]  
...
## See Also  
 ...
 [CWinApp::InitInstance](../Topic/CWinApp::InitInstance.md)