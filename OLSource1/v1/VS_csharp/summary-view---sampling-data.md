...
## Timeline Graph  
 The timeline graph in the Summary view shows the percentage of processor (CPU) utilization of the profiled application over the time that the profiling occurred. You can use the timeline graph to filter the view to a selected time span. For more information, see [How to: Filter Profiling Reports from the Summary Timeline](../VS_csharp/how-to--filter-report-views-from-the-summary-timeline.md).  
