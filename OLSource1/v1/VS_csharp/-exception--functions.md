...
### Return Value  
 An [exception_ptr](../VS_csharp/-exception--typedefs.md#exception_ptr) object pointing to the current exception.  
...
### Remarks  
 ...
 The `current_exception` function captures the exception that is in flight regardless of whether the `catch` statement specifies an [exception-declaration](../VS_csharp/try--throw--and-catch-statements--c---.md) statement.  
...
##  <a name="make_exception_ptr"></a>  make_exception_ptr  
 Creates an [exception_ptr](../VS_csharp/-exception--typedefs.md#exception_ptr) object that holds a copy of an exception.  
...
### Parameters  
 `Except`  
 The class with the exception to copy. Usually, you specify an [exception class](../VS_csharp/exception-class.md) object as the argument to the `make_exception_ptr` function, although any class object can be the argument.  
  
### Return Value  
 An [exception_ptr](../VS_csharp/-exception--typedefs.md#exception_ptr) object pointing to a copy of the current exception for `Except`.  
  
### Remarks  
 Calling the `make_exception_ptr` function is equivalent to throwing a C++ exception, catching it in a catch block, and then calling the [current_exception](../VS_csharp/-exception--functions.md#current_exception) function to return an `exception_ptr` object that references the exception. The Microsoft implementation of the `make_exception_ptr` function is more efficient than throwing and then catching an exception.  
...
### Parameters  
 `P`  
 The caught exception to re-throw. If `P` is a null [exception_ptr](../VS_csharp/-exception--typedefs.md#exception_ptr), the function throws [std::bad_exception](../VS_csharp/bad_exception-class.md).  
...
### Remarks  
 The function establishes a new [terminate_handler](../VS_csharp/-exception--typedefs.md#terminate_handler) as the function * `fnew`. Thus, `fnew` must not be a null pointer. The function returns the address of the previous terminate handler.  
...
### Return Value  
 Returns `true` after completing evaluation of a throw expression and before completing initialization of the exception declaration in the matching handler or calling [unexpected](../VS_csharp/-exception--functions.md#unexpected) as a result of the throw expression. In particular, `uncaught_exception` will return `true` when called from a destructor that is being invoked during an exception unwind. On devices, `uncaught_exception` is only supported on Windows CE 5.00 and higher versions, including Windows Mobile 2005 platforms.  
...
### Remarks  
 ...
 The function calls an unexpected handler, a function of type `void`. If `unexpected` is called directly by the program, the unexpected handler is the one most recently set by a call to [set_unexpected](../VS_csharp/-exception--functions.md#set_unexpected).  
...
-   Throwing an object of type [bad_exception](../VS_csharp/bad_exception-class.md).  
  
-   Calling [terminate](../VS_csharp/-exception--functions.md#terminate), **abort** or **exit**( `int`).  
  
 At program startup, the unexpected handler is a function that calls [terminate](../VS_csharp/-exception--functions.md#terminate).  
  
### Example  
  See [set_unexpected](../VS_csharp/-exception--functions.md#set_unexpected) for an example of the use of **unexpected.**  
  
## See Also  
 [&lt;exception&gt;](../VS_csharp/-exception-.md)