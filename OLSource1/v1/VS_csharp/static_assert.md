...
|`string-literal`|An message that is displayed if the `constant-expression` parameter is zero. The message is a string of characters in the [base character set](../VS_csharp/ascii-character-set.md) of the compiler; that is, not [multibyte or wide characters](../VS_csharp/multibyte-and-wide-characters.md).|  
...
## Remarks  
 ...
 The `static_assert` declaration tests a software assertion at compile time. In contrast, the [assert (CRT)](../VS_csharp/assert-macro--_assert--_wassert.md) macro tests a software assertion at run time and incurs a run time cost in space or time. The `static_assert` declaration is especially useful for debugging templates because template arguments can be included in the `constant-expression` parameter.  
...
## See Also  
 [Error Handling (C++)](../VS_csharp/assertion-and-user-supplied-messages--c---.md)   
 ...
 [The ASCII Character Set](../VS_csharp/ascii-character-set.md)   
