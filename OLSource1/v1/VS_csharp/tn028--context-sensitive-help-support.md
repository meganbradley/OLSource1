...
> [!NOTE]
>  In addition to implementing context-sensitive help using WinHelp, MFC also supports using HTML Help. For more information on this support and programming with HTML Help, see [HTML Help: Context-Sensitive Help for Your Programs](../VS_csharp/html-help--context-sensitive-help-for-your-programs.md).  
...
## Help Files  
 The Microsoft Foundation classes assume a single Help file. That Help file must have the same name and path as the application. For example, if the executable is C:\MyApplication\MyHelp.exe the help file must be C:\MyApplication\MyHelp.hlp. You set the path through the `m_pszHelpFilePath` member variable of the [CWinApp Class](../VS_csharp/cwinapp-class.md).  
...
```  
 ...
 The source code for MAKEHM is available in the MFC Programming Utilities sample [MAKEHM](../VS_csharp/visual-c---samples.md).  
