...
### Remarks  
 `safe_cast` throws `InvalidCastException` if it cannot convert *expression* to the type specified by *type-id*. To catch `InvalidCastException`, specify the [/EHsc](../VS_csharp/-eh--exception-handling-model-.md) compiler option, and use a try/catch statement.  
 ...
 **Example**  
 ...
 The following code example demonstrates how to use `safe_cast` with the [!INCLUDE[wrt](../VS_csharp/includes/wrt_md.md)].  
...
### Remarks  
 ...
 The compiler will accept a [static_cast](../VS_csharp/static_cast-operator.md) in most places that it will accept a `safe_cast`.  However, `safe_cast` is guaranteed to produce verifiable MSIL, where as a `static_cast` could produce unverifiable MSIL.  See [Pure and Verifiable Code](../VS_csharp/pure-and-verifiable-code--c---cli-.md) and [PEVerify Tool (Peverify.exe)](assetId:///f4f46f9e-8d08-4e66-a94b-0c69c9b0bbfa) for more information on verifiable code.  
  
 Like `static_cast`, `safe_cast` invokes user-defined conversions.  
  
 For more information about casts, see [Casting Operators](../VS_csharp/casting-operators.md).  
  
 `safe_cast` does not apply a **const_cast** (cast away **const**).  
  
 `safe_cast` is in the cli namespace.  See [cli Namespace](../VS_csharp/platform--default--and-cli-namespaces---c---component-extensions-.md) for more information.  
  
 For more information on **safe_cas**t, see:  
  
-   [C-Style Casts with /clr](../VS_csharp/c-style-casts-with--clr--c---cli-.md)  
  
-   [How to: Upcast with safe_cast](../VS_csharp/how-to--use-safe_cast-in-c---cli.md)  
  
-   [How to: Downcast with safe_cast](../Topic/How%20to:%20Downcast%20with%20safe_cast.md)  
  
-   [How to: Using safe_cast and Generic Types](../Topic/How%20to:%20Use%20safe_cast%20and%20Generic%20Types.md)  
