...
#### Parameters  
 `dwFlags`  
 [in] Specifies different actions. Currently, only the [INTERCEPT_EXCEPTION_ACTION](../VS_csharp/intercept_exception_action.md) value `IEA_INTERCEPT` is supported and must be specified.  
...
## Remarks  
 ...
 When the debugger wants to know if the exception should be intercepted, it calls this method on the current stack frame object. This method is responsible for handling all details of the exception. If the [IDebugStackFrame3](../VS_csharp/idebugstackframe3.md) interface is not implemented or the `InterceptStackException` method returns any error, then the debugger continues processing the exception normally.  
...
>  Exceptions can be intercepted only in managed code, that is, when the program being debugged is running under the .NET run time. Of course, third-party language implementers can implement `InterceptStackException` in their own debug engines if they so choose.  
 ...
 After the interception is complete, an [IDebugInterceptExceptionCompleteEvent2](../VS_csharp/idebuginterceptexceptioncompleteevent2.md) is signaled.  
  
## See Also  
 [IDebugStackFrame3](../VS_csharp/idebugstackframe3.md)   
 [INTERCEPT_EXCEPTION_ACTION](../VS_csharp/intercept_exception_action.md)   
 [IDebugInterceptExceptionCompleteEvent2](../VS_csharp/idebuginterceptexceptioncompleteevent2.md)