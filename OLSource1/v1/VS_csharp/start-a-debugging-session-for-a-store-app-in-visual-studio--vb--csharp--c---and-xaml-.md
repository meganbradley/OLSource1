...
> [!NOTE]
>  For apps written in JavaScript and HTML see [Start a debugging session for Store Apps in Visual Studio (JavaScript)](../VS_csharp/start-a-debugging-session-for-store-apps-in-visual-studio--javascript-.md).  
...
2.  Do this to open the debug property page for the project :  
    ...
    -   For Visual C# and Visual Basic apps, choose **Debug**.  
         ...
         ![C&#35; &#47; VB project debug property page](../VS_csharp/media/dbg_csvb_debugpropertypage.png "DBG_CsVb_DebugPropertyPage")  
  
    -   For Visual C++ apps, expand the **Configuration Properties**  node and then choose **Debugging**.  
  
         ![C&#43;&#43; Windows Store app debugging property page](../VS_csharp/media/dbg_cpp_debugpropertypage.png "DBG_CPP_DebugPropertyPage")  
...
###  <a name="BKMK_Choose_the_deployment_target"></a> Choose the deployment target  
 ![Applies to Windows only](../VS_csharp/media/windows_only_content.png "windows_only_content")  
...
|||  
|-|-|  
|**Local Machine**|Debug the app in the current session on your local machine. See [Running Windows Metro style apps on the local machine](../VS_csharp/run-windows-store-apps-on-the-local-machine.md).|  
|**Simulator**|Debug the app in the Visual Studio simulator for [!INCLUDE[win8_appname_long](../VS_csharp/includes/win8_appname_long_md.md)] apps. The simulator is a Desktop window that enables you to debug device functionality—such as touch gestures and device rotation—that are not available on the local machine. See [Running Windows Metro style apps in the simulator](../VS_csharp/run-windows-store-apps-in-the-simulator.md).|  
|**Remote Machine**|Debug the app on a device that is connected to the local machine over an intranet or directly connected by using an Ethernet cable. To debug remotely, the Visual Studio Remote Tools must be installed and running on hte remote device. See [Running Windows Metro style apps on a remote machine](../VS_csharp/run-windows-store-apps-on-a-remote-machine.md).|  
...
-   Choose the remote machine from the **Select Remote Debugger Connection** dialog box.  
    ...
    -   For C++ apps, choose the down arrow in the **Machine Name** box and choose **<Locate...>**.  
     ...
     ![Select Remote Debugger Connection dialog box](../VS_csharp/media/vsrun_selectremotedebuggerdlg.png "VSRUN_SelectRemoteDebuggerDlg")  
 ...
 ![Applies to Windows Phone only](../VS_csharp/media/phone_only_content.png "phone_only_content")  
...
-   Start your app from the Start menu, an execution contract, or by another procedure.  
 ...
 . For more information about debugging background tasks, see [How to trigger suspend, resume, and background events in Metro style apps](../VS_csharp/how-to-trigger-suspend--resume--and-background-events-for-windows-store-apps-in-visual-studio.md).  
...
2.  Choose one of the following options from the list:  
    ...
    |**Local Machine**|Debug the app in the current session on your local machine. See [Running Windows Metro style apps on the local machine](../VS_csharp/run-windows-store-apps-on-the-local-machine.md).|  
    |**Simulator**|Debug the app in the Visual Studio simulator for [!INCLUDE[win8_appname_long](../VS_csharp/includes/win8_appname_long_md.md)] apps. The simulator is a Desktop window that enables you to debug device functionality—such as touch gestures and device rotation—that are not available on the local machine. See [Running Windows Metro style apps in the simulator](../VS_csharp/run-windows-store-apps-in-the-simulator.md).|  
    |**Remote Machine**|Debug the app on a device that is connected to the local machine over an intranet or directly connected by using an Ethernet cable. To debug remotely, the Visual Studio Remote Tools must be installed and running on the remote device. See [Running Windows Metro style apps on a remote machine](../VS_csharp/run-windows-store-apps-on-a-remote-machine.md).|  
...
###  <a name="BKMK_Attach_the_debugger_to_a_running_app_"></a> Attach the debugger to a running app  
 To attach the debugger to a [!INCLUDE[win8_appname_long](../VS_csharp/includes/win8_appname_long_md.md)] app, you must use the Debuggable Package Manager to set the app to run in debug mode. The Debuggable Package Manager is installed with the Visual Studio Remote Tools.  
  
 Attaching the debugger to an app is useful when you need to debug an already-installed app, such as an app that was installed from the [!INCLUDE[win8_appstore_long](../VS_csharp/includes/win8_appstore_long_md.md)]. Attaching is required when you have the source files for the app, but you do not have a Visual Studio project for the app. For example, you might have a custom build system that does not use Visual Studio projects or solutions.  
...
## See Also  
 ...
 [Quick Start: Debugger navigation (Xaml and C#)](../VS_csharp/navigate-a-debugging-session-in-visual-studio--xaml-and-csharp-.md)