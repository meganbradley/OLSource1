...
```  
 ...
 An iterator `Iterator` other than an object pointer must also define the member types required by the specialization `iterator_traits<Iterator>`. Note that these requirements can be met by deriving `Iterator` from the public base class [iterator](../VS_csharp/iterator-struct.md).  
...
>  You can avoid using iterators explicitly by using range-for loops. For more information, see [Loops (Modern C++)](assetId:///b1b2779c-750e-4576-a514-a84178eae9da).  
 ...
 [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)] now offers checked iterators and debug iterators to ensure that you do not overwrite the bounds of your container. For more information, see [Checked Iterators](../VS_csharp/checked-iterators.md) and [Debug Iterator Support](../VS_csharp/debug-iterator-support.md).  
