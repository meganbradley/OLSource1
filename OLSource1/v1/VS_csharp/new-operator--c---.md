...
> [!NOTE]
>  Microsoft C++ Component Extensions provides support for the `new` keyword to add vtable slot entries. For more information, see [new (new slot in vtable) (C++ Component Extensions)](../VS_csharp/new--new-slot-in-vtable----c---component-extensions-.md)  
...
## Remarks  
 If unsuccessful, **new** returns zero or throws an exception; see [The new and delete Operators](../VS_csharp/new-and-delete-operators.md) for more information. You can change this default behavior by writing a custom exception-handling routine and calling the [_set_new_handler](../VS_csharp/_set_new_handler.md) run-time library function with your function name as its argument.  
  
 For information on how to create an object on the managed heap, see [gcnew](../VS_csharp/ref-new--gcnew---c---component-extensions-.md).  
  
 When **new** is used to allocate memory for a C++ class object, the object's constructor is called after the memory is allocated.  
  
 Use the [delete](../VS_csharp/delete-operator--c---.md) operator to deallocate the memory allocated with the **new** operator.  
...
```  
 ...
 If you use the operator **new** without any extra arguments, and compile with the [/GX](../VS_csharp/-gx--enable-exception-handling-.md), [/EHa](../VS_csharp/-eh--exception-handling-model-.md), or [/EHs](../VS_csharp/-eh--exception-handling-model-.md) option, the compiler will generate code to call operator **delete** if the constructor throws an exception.  
...
-   The class has a default constructor (a constructor that can be called with no arguments).  
 ...
 No explicit per-element initialization can be done when allocating arrays using the **new** operator; only the default constructor, if present, is called. See [Default Arguments](../VS_csharp/default-arguments.md) for more information.  
...
>  The argument to `operator new` is of type **size_t**. This type is defined in DIRECT.H, MALLOC.H, MEMORY.H, SEARCH.H, STDDEF.H, STDIO.H, STDLIB.H, STRING.H, and TIME.H.  
 ...
 An option in the grammar allows specification of *placement* (see the Grammar for [new Operator](../VS_csharp/new-operator--c---.md)). The *placement* parameter can be used only for user-defined implementations of `operator new`; it allows extra information to be passed to `operator new`. An expression with a *placement* field such as `T *TObject = new ( 0x0040 ) T;` is translated to `T *TObject = T::operator new( sizeof( T ), 0x0040 );` if class T has member operator new, otherwise to `T *TObject = ::operator new( sizeof( T ), 0x0040 );`.  
