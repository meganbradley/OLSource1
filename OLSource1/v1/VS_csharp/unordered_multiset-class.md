...
## Remarks  
 The object orders the sequence it controls by calling two stored objects, a comparison function object of type [key_equal](#unordered_multiset__key_equal) and a hash function object of type [hasher](#unordered_multiset__hasher). You access the first stored object by calling the member function [key_eq](#unordered_multiset__key_eq)`()`; and you access the second stored object by calling the member function [hash_function](#unordered_multiset__hash_function)`()`. Specifically, for all values `X` and `Y` of type `Key`, the call `key_eq()(X, Y)` returns true only if the two argument values have equivalent ordering; the call `hash_function()(keyval)` yields a distribution of values of type `size_t`. Unlike template class [unordered_set](../VS_csharp/unordered_set-class.md), an object of template class `unordered_multiset` does not ensure that `key_eq()(X, Y)` is always false for any two elements of the controlled sequence. (Keys need not be unique.)  
...
### Remarks  
 ...
 For a code example, see [multiset::emplace](../VS_csharp/multiset-class.md#multiset__emplace).  
...
|`ValTy`|Template parameter that specifies the argument type that the unordered_multiset can use to construct an element of [value_type](../VS_csharp/map-class.md#map__value_type), and perfect-forwards `Val` as an argument.|  
...
### Remarks  
 ...
 The [value_type](../VS_csharp/map-class.md#map__value_type) of a container is a typedef that belongs to the container, and, for set, `unordered_multiset<V>::value_type` is type `const V`.  
 ...
 The initializer list member function (6) uses an [initializer_list](../VS_csharp/-initializer_list-.md) to copy elements into the unordered_multiset.  
 ...
 For a code example, see [multiset::insert](../VS_csharp/multiset-class.md#multiset__insert).  
  
##  <a name="unordered_multiset__iterator"></a>  unordered_multiset::iterator  
 A type that provides a constant [forward iterator](../VS_csharp/forward_iterator_tag-struct.md) that can read elements in an unordered_multiset.  
...
### Example  
  See the example for [begin](../VS_csharp/multiset-class.md#multiset__begin) for an example of how to declare and use an **iterator**.  
...
|-|-|  
|Parameter|Description|  
|`_Right`|The [unordered_multiset](../VS_csharp/unordered_multiset-class.md) being copied into the `unordered_multiset`.|  
