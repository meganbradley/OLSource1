...
#### Parameters  
 `CharType`  
 The data type of a single character to be stored in the string. The Standard C++ Library provides specializations of this template class, with the type definitions [string](../VS_csharp/-string--typedefs.md#string) for elements of type `char`, [wstring](../VS_csharp/-string--typedefs.md#wstring), for `wchar_t`, [u16string](../VS_csharp/-string--typedefs.md#u16string) for `char16_t`, and [u32string](../VS_csharp/-string--typedefs.md#u32string) for `char32_t`.  
...
## Remarks  
 If a function is asked to generate a sequence longer than [max_size](#basic_string__max_size) elements, the function reports a length error by throwing an object of type [length_error](../VS_csharp/length_error-class.md).  
...
### Remarks  
 ...
 The member `operator[]` does not check whether the index passed as a parameter is valid but the member function **at** does and so should be used if the validity is not certain. An invalid index, which is an index less that zero or greater than or equal to the size of the string, passed to the member function **at** throws an [out_of_range Class](../VS_csharp/out_of_range-class.md) exception. An invalid index passed to the `operator[]` results in undefined behavior, but the index equal to the length of the string is a valid index for const strings and the operator returns the null-character when passed this index.  
...
### Remarks  
 A exception of type [length_error Class](../VS_csharp/length_error-class.md) is thrown when an operation produces a string with a length greater than the maximum size.  
...
### Remarks  
 ...
 `operator[]` does not check whether the index passed as a parameter is valid, but the member function **at** does and so should be used in the validity is not certain. An invalid index (an index less that zero or greater than or equal to the size of the string) passed to the member function **at** throws an [out_of_range Class](../VS_csharp/out_of_range-class.md) exception. An invalid index passed to `operator[]` results in undefined behavior, but the index equal to the length of the string is a valid index for const strings and the operator returns the null character when passed this index.  
  
 The reference returned may be invalidated by string reallocations or modifications for the non- **const** strings.  
  
 When compiling with _SECURE_SCL 1, a runtime error will occur if you attempt to access an element outside the bounds of the string.  See [Checked Iterators](../VS_csharp/checked-iterators.md) for more information.  
...
### Example  
  See the example for [copy](../VS_csharp/char_traits-struct.md#char_traits__copy) for an example of how to declare and use `traits_type`.  
