...
This topic describes how to add a toolbar button that contains a Windows control to a toolbar. In MFC, a toolbar button must be a [CMFCToolBarButton Class](../VS_csharp/cmfctoolbarbutton-class.md)-derived class, for example [CMFCToolBarComboBoxButton](../VS_csharp/cmfctoolbarcomboboxbutton-class.md), [CMFCToolBarEditBoxButton](../VS_csharp/cmfctoolbareditboxbutton-class.md), [CMFCDropDownToolbarButton](../VS_csharp/cmfcdropdowntoolbarbutton-class.md), or [CMFCToolBarMenuButton](../VS_csharp/cmfctoolbarmenubutton-class.md).  
  
## Adding Controls to Toolbars  
 To add a control to a toolbar, follow these steps:  
  
1.  Reserve a dummy resource ID for the button in the parent toolbar resource. For more information about how to create buttons by using the Toolbar Editor in Visual Studio, see the [Toolbar Editor](../VS_csharp/toolbar-editor.md) topic.  
...
## Toolbar Controls and Customization  
 ...
 When you enable customization, you create the **Customize** dialog box in the customization handler `OnViewCustomize` by using the [CMFCToolbarsCustomizeDialog](../VS_csharp/cmfctoolbarscustomizedialog-class.md) class. Before you display the **Customize** dialog box by calling [CMFCToolbarsCustomizeDialog::Create](../Topic/CMFCToolBarsCustomizeDialog::Create.md), call [CMFCToolbarsCustomizeDialog::ReplaceButton](../Topic/CMFCToolBarsCustomizeDialog::ReplaceButton.md) to replace the standard button with the new control.  
  
## Example: Creating a Find Combo Box  
 This section describes how to create a `Find` combo box control that appears on a toolbar and contains recently-used search strings. The user can type a string in the control and then press the enter key to search a document, or press the escape key to return the focus to the main frame. This example assumes that the document is displayed in a [CEditView](../VS_csharp/ceditview-class.md)-derived view.  
...
2.  Create a new class, `CFindComboBox`, derived from [CComboBox](../VS_csharp/ccombobox-class.md).  
...
4.  Create a class for the `Find` combo box button, derived from [CMFCToolbarComboBoxButton](../VS_csharp/cmfctoolbarcomboboxbutton-class.md). In this example, it is named `CFindComboButton`.  
...
5.  The constructor of `CMFCToolbarComboBoxButton` takes three parameters: the command ID of the button, the button image index, and the style of the combo box. Set these parameters as follows:  
    ...
    3.  For a list of available combo box styles, see [Combo-Box Styles](../VS_csharp/combo-box-styles.md).  
...
7.  Use the [IMPLEMENT_SERIAL](../VS_csharp/implement_serial.md) macro to make the combo button persistent. The workspace manager automatically loads and saves the button's state in the Windows registry.  
...
## See Also  
 ...
 [CMFCToolBarButton Class](../VS_csharp/cmfctoolbarbutton-class.md)   
