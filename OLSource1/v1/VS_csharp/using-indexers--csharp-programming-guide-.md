...
## Remarks  
 The type of an indexer and the type of its parameters must be at least as accessible as the indexer itself. For more information about accessibility levels, see [Access Modifiers](../VS_csharp/access-modifiers--csharp-reference-.md).  
  
 For more information about how to use indexers with an interface, see [Interface Indexers](../VS_csharp/indexers-in-interfaces--csharp-programming-guide-.md).  
 ...
 An indexer value is not classified as a variable; therefore, you cannot pass an indexer value as a [ref](../VS_csharp/ref--csharp-reference-.md) or [out](../VS_csharp/out--csharp-reference-.md) parameter.  
...
### Description  
 The following example shows how to declare a private array field, `temps`, and an indexer. The indexer enables direct access to the instance `tempRecord[i]`. The alternative to using the indexer is to declare the array as a [public](../VS_csharp/public--csharp-reference-.md) member and access its members, `tempRecord.temps[i]`, directly.  
  
 Notice that when an indexer's access is evaluated, for example, in a `Console.Write` statement, the [get](../VS_csharp/get--csharp-reference-.md) accessor is invoked. Therefore, if no `get` accessor exists, a compile-time error occurs.  
  
### Code  
 [!code[csProgGuideIndexers#1](../VS_csharp/codesnippet/CSharp/using-indexers--csharp-programming-guide-_1.cs)]  
...
### Code  
 [!code[csProgGuideIndexers#2](../VS_csharp/codesnippet/CSharp/using-indexers--csharp-programming-guide-_2.cs)]  
...
-   Set the accessibility of the `get` and [set](../VS_csharp/set--csharp-reference-.md) accessors to be as restrictive as is reasonable. This is important for the `set` accessor in particular. For more information, see [Accessor Accessibility](../VS_csharp/restricting-accessor-accessibility--csharp-programming-guide-.md).  
