...
## Remarks  
 These functions differ from `_vscprintf` and `_vscwprintf` only in that they support the ability to specify the order in which the arguments are used. For more information, see [printf Positional Parameters](../VS_csharp/printf_p-positional-parameters.md).  
  
 The versions of these functions with the `_l` suffix are identical except that they use the locale parameter passed in instead of the current thread locale.  
  
 If `format` is a null pointer, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, the functions return -1 and set `errno` to `EINVAL`.  
...
## Example  
 See the example for [vsprintf](../VS_csharp/vsprintf--_vsprintf_l--vswprintf--_vswprintf_l--__vswprintf_l.md).  
  
## See Also  
 [vprintf Functions](../VS_csharp/vprintf-functions.md)   
 [_scprintf_p, _scprintf_p_l, _scwprintf_p, _scwprintf_p_l](../VS_csharp/_scprintf_p--_scprintf_p_l--_scwprintf_p--_scwprintf_p_l.md)   
 [_vscprintf, _vscprintf_l, _vscwprintf, _vscwprintf_l](../VS_csharp/_vscprintf--_vscprintf_l--_vscwprintf--_vscwprintf_l.md)