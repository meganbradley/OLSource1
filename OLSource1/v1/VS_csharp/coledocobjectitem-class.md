...
-   The **Help** menu contains menu items for both the Active document container and server.  
 ...
 For more information on menu merging and Active document activation, see Overview of [Active Document Containment](../VS_csharp/active-document-containment.md).  
...
### Parameters  
 `pCaller`  
 A pointer to a [CView](../VS_csharp/cview-class.md) object that is sending the print command.  
  
 `pInfo`  
 A pointer to a [CPrintInfo](../VS_csharp/cprintinfo-structure.md) object that describes the job to be printed.  
...
### Return Value  
 For a complete listing of return values, see                         [IOleCommandTarget::QueryStatus](http://msdn.microsoft.com/library/windows/desktop/ms688491) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
  
### Remarks  
 This member function emulates the functionality of the                         [IOleCommandTarget::QueryStatus](http://msdn.microsoft.com/library/windows/desktop/ms688491) method, as described in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
...
### Parameters  
 `dwCloseOption`  
 Flag specifying under what circumstances the OLE item is saved when it returns to the loaded state. For a list of possible values, see [COleClientItem::Close](../VS_csharp/coleclientitem-class.md#coleclientitem__close).  
