...
## Remarks  
 ...
 initializes the controlled sequence with the sequence `[``right``.`[begin](../VS_csharp/list--begin--stl-clr-.md)`(),` `right``.`[end](../VS_csharp/list--end--stl-clr-.md)`())`. You use it to specify an initial controlled sequence that is a copy of the sequence controlled by the list object `right`.  
  
 The constructor:  
  
 `list(list<Value>^ right);`  
  
 initializes the controlled sequence with the sequence `[``right``->`[begin](../VS_csharp/list--begin--stl-clr-.md)`(),` `right``->`[end](../VS_csharp/list--end--stl-clr-.md)`())`. You use it to specify an initial controlled sequence that is a copy of the sequence controlled by the list object whose handle is `right`.  
