...
'instance_dereference_operator' : the meaning of dereferencing a handle can change, when a user-defined 'operator' operator exists; write the operator as a static function to be explicit about the operand  
 ...
 For more information, see [^ (Handle to Object on Managed Heap)](../VS_csharp/handle-to-object-operator--^----c---component-extensions-.md) and [% (Tracking Reference)](../VS_csharp/tracking-reference-operator--c---component-extensions-.md).  
  
 Also, an instance operator is not available to other language compilers via referenced metadata. For more information, see [User-Defined Operators](../VS_csharp/user-defined-operators--c---cli-.md).  
