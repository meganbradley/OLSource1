...
When the user saves an instance, or *model*, of a domain-specific language (DSL) in [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)], an XML file is created or updated. The file can be reloaded to recreate the model in the Store.  
...
-   Enable validation for the DSL. In DSL Explorer, select Editor\Validation, and set the **Uses...** properties to `true`.  
     ...
     For more information, see [Validation](../VS_csharp/validation-in-a-domain-specific-language.md).  
