...
#### Parameters  
 ...
 The `filename` and `linenumber` parameters are only available when `_calloc_dbg` has been called explicitly or the [_CRTDBG_MAP_ALLOC](../VS_csharp/_crtdbg_map_alloc.md) preprocessor constant has been defined.  
  
## Return Value  
 On successful completion, this function returns a pointer to the user portion of the last allocated memory block, calls the new handler function, or returns `NULL`. For a complete description of the return behavior, see the Remarks section. For more information about how the new handler function is used, see the [calloc](../VS_csharp/calloc.md) function.  
  
## Remarks  
 `_calloc_dbg` is a debug version of the [calloc](../VS_csharp/calloc.md) function. When [_DEBUG](../VS_csharp/_debug.md) is not defined, each call to `_calloc_dbg` is reduced to a call to `calloc`. Both `calloc` and `_calloc_dbg` allocate `num` memory blocks in the base heap, but `_calloc_dbg` offers several debugging features:  
