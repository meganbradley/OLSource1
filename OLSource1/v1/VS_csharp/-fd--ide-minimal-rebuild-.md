...
**/FD** is not exposed to users except in the [Command Line](../VS_csharp/command-line-property-pages.md) property page of a C++ project's **Property Pages** dialog box, if and only if [/Gm (Enable Minimal Rebuild)](../VS_csharp/-gm--enable-minimal-rebuild-.md) is not also selected. **/FD** has no effect other than from the development environment. **/FD** is not exposed in the output of **cl /?**.  
