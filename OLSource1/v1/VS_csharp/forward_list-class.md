...
## Remarks  
 A `forward_list` object allocates and frees storage for the sequence it controls through a stored object of class `Allocator` that is based on [allocator Class](../VS_csharp/allocator-class.md) (commonly known as `std::allocator)`. For more information, see [Allocators](../VS_csharp/allocators.md). An allocator object must have the same external interface as an object of template class `allocator`.  
...
### Remarks  
 ...
 Erasing `N` elements causes `N` destructor calls. [Reallocation](../VS_csharp/forward_list-class.md) occurs, so iterators and references become invalid for the erased elements.  
...
### Remarks  
 All constructors store an [allocator](../VS_csharp/allocator-class.md) and initialize the controlled sequence. The allocator object is the argument `Al`, if present. For the copy constructor, it is `_Right.get_allocator()`. Otherwise, it is `Allocator()`.  
 ...
 The ninth and tenth constructors are the same as the sixth, but with an [rvalue](../VS_csharp/rvalue-reference-declarator----.md) reference.  
...
### Return Value  
 The stored [allocator](../VS_csharp/allocator-class.md) object.  
...
### Remarks  
 ...
 The last member function is the same as the first, but with an [rvalue](../VS_csharp/rvalue-reference-declarator----.md) reference.  
  
 Inserting `N` elements causes `N` constructor calls. [Reallocation](../VS_csharp/forward_list-class.md) occurs, but no iterators or references become invalid.  
...
### Remarks  
 ...
 The third member operator is the same as the first, but with an [rvalue](../VS_csharp/rvalue-reference-declarator----.md) reference.  
...
## See Also  
 [<forward_list>](../VS_csharp/-forward_list-.md)