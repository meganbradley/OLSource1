...
---
# Command Window
The **Command** window is used to execute commands or aliases directly in the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] integrated development environment (IDE). You can execute both menu commands and commands that do not appear on any menu. To display the **Command** window, choose **Other Windows** from the **View** menu, and select **Command Window**.  
  
## Displaying the Values of Variables  
 To check the value of a variable `varA`, use the [Print Command](../VS_csharp/print-command.md):  
...
## Parameters, Switches, and Values  
 Some [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] commands commands have required and optional arguments, switches and values. Certain rules apply when dealing with such commands. The following is an example of a rich command to clarify the terminology.  
...
-   `var[1-3]+` and `oldpar` are parameters  
 ...
 The position of switches and parameters can be interchanged freely on the command line with the exception of the [Shell](../VS_csharp/shell-command.md) command, which requires its switches and parameters in a specific order.  
...
## See Also  
 [Predefined Visual Studio Command Aliases](../VS_csharp/visual-studio-command-aliases.md)   
 [Visual Studio Commands with Arguments](../VS_csharp/visual-studio-commands.md)