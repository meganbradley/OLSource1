...
### Remarks  
 The type is a signed integer that describes an object that can store a byte offset involved in various stream positioning operations. It is typically a synonym for [streamoff](../VS_csharp/-ios--typedefs.md#streamoff), but it has essentially the same properties as that type.  
...
### Remarks  
 The type describes an object that can store all the information needed to restore an arbitrary file-position indicator within a stream. It is typically a synonym for [streampos](../VS_csharp/-ios--typedefs.md#streampos), but in any case it has essentially the same properties as that type.  
