...
## Remarks  
 The **SECURITY_DESCRIPTOR** structure contains the security information associated with an object. Applications use this structure to set and query an object's security status. See also [AtlGetSecurityDescriptor](../VS_csharp/atlgetsecuritydescriptor.md).  
...
### Parameters  
 `pSid`  
 Pointer to a [CSid](../VS_csharp/csid-class.md) (security identifier) that receives a copy of the group stored in the CDacl.  
...
### Parameters  
 `Sid`  
 The [CSid](../VS_csharp/csid-class.md) object for the security descriptor's new primary owner. This parameter must not be NULL.  
