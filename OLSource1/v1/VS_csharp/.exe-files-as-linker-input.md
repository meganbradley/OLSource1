---
title: ".Exe Files as Linker Input"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "article"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "executable files, as linker output"
  - ".exe files"
ms.assetid: 73714c30-6f03-4e72-881e-090e5240ba93
caps.latest.revision: 11
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# .Exe Files as Linker Input
The [MS-DOS Stub File Name](../VS_csharp/-stub--ms-dos-stub-file-name-.md) (/STUB) option specifies the name of an .exe file that runs with MS-DOS. LINK examines the specified file to be sure that it is a valid MS-DOS program.  
  
## See Also  
 [LINK Input Files](../VS_csharp/link-input-files.md)   
 [Linker Options](../VS_csharp/linker-options.md)