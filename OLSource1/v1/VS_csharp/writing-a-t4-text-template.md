...
-   **Control blocks** - program code that inserts variable values into the text, and controls conditional or repeated parts of the text.  
 ...
 To try the examples in this topic, copy them into a template file as described in [Generating Application Code and Files from Templates](../VS_csharp/design-time-code-generation-by-using-t4-text-templates.md). After editing the template file, save it, and then inspect the output **.txt** file.  
...
```  
 ...
 For more information about directives, see [Text Template Directives](../VS_csharp/t4-text-template-directives.md).  
...
## Control blocks  
 Control blocks are sections of program code that are used to transform the templates. The default language is C#, but to use [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)], you can write this directive at the beginning of the file:  
...
```  
 ...
 For more information about control blocks, see [Text Template Control Blocks](../VS_csharp/text-template-control-blocks.md).  
...
```  
 ...
 For a list of macros, see [Macros for Build Commands and Properties](../VS_csharp/common-macros-for-build-commands-and-properties.md).  
  
 The assembly directive has no effect in a [preprocessed text template](../VS_csharp/run-time-text-generation-with-t4-text-templates.md).  
  
 For more information, see [Assembly Directive](../VS_csharp/t4-assembly-directive.md).  
...
```  
 ...
 For more information, see [Import Directive](../VS_csharp/t4-import-directive.md).  
...
###  <a name="Include"></a> Including code and text  
 ...
 For more information, see [Include Directive](../VS_csharp/t4-include-directive.md).  
...
### Utility methods  
 ...
 For more information, see [Text Template Utility Functions](../VS_csharp/text-template-utility-methods.md).  
 ...
 **Edit the model file in a diagram or form.** [!INCLUDE[dsl](../VS_csharp/includes/dsl_md.md)] provides tools that let you edit a model as a diagram or Windows form. This makes it easier to discuss the model with users of the generated application. [!INCLUDE[dsl](../VS_csharp/includes/dsl_md.md)] also creates a set of strongly-typed classes that reflect the structure of the model. For more information, see [Using Text Templates in Domain-Specific Language Solutions](../VS_csharp/generating-code-from-a-domain-specific-language.md).  
  
 **Use a UML model**. You can generate code from a UML model. This has the advantage that the model can be edited as a diagram in a familiar notation. Also, you do not have to design the diagram. For more information, see [How to Generate Files from a UML Model](../VS_csharp/generate-files-from-a-uml-model.md).  
  
### Relative file paths in design-time templates  
 In a [design-time text template](../VS_csharp/design-time-code-generation-by-using-t4-text-templates.md), if you want to reference a file in a location relative to the text template, use `this.Host.ResolvePath()`. You must also set `hostspecific="true"` in the `template` directive:  
...
### Design-time Text Templates run in a separate AppDomain  
 You should be aware that a [design-time text template](../VS_csharp/design-time-code-generation-by-using-t4-text-templates.md) runs in an AppDomain that is separate from the main application. In most cases this is not important, but you might discover restrictions in certain complex cases. For example, if you want to pass data in or out of the template from a separate service, then the service must provide a serializable API.  
  
 (This isn’t true of a [run-time text template](../VS_csharp/run-time-text-generation-with-t4-text-templates.md), which provides code that is compiled along with the rest of your code.)  
...
|Task|Topic|  
|----------|-----------|  
|Writing a template.|[Guidelines for Writing Text Templates](../VS_csharp/guidelines-for-writing-t4-text-templates.md)|  
|Generate text by using program code.|[Text Template Structure](../VS_csharp/writing-a-t4-text-template.md)|  
|Generate files in a [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] solution.|[Generating Text in a Visual Studio Solution](../VS_csharp/design-time-code-generation-by-using-t4-text-templates.md)|  
