...
```  
 ...
 Exceptions in C++ resemble those in languages such as C# and Java. In the `try` block, if an exception is *thrown* it will be *caught* by the first associated `catch` block whose type matches that of the exception. In other words, execution jumps from the `throw` statement to the `catch` statement. If no usable catch block is found, `std::terminate` is invoked and the program exits. In C++, any type may be thrown; however, we recommend that you throw a type that derives directly or indirectly from `std::exception`. In the previous example, the exception type, [invalid_argument](../VS_csharp/invalid_argument-class.md), is defined in the standard library in the [<stdexcept\>](../VS_csharp/-stdexcept-.md) header file. C++ does not provide, and does not require, a `finally` block to make sure that all resources are released if an exception is thrown. The resource acquisition is initialization (RAII) idiom, which uses smart pointers, provides the required functionality for resource cleanup. For more information, see [How to: Design for Exception Safety (C++)](../VS_csharp/how-to--design-for-exception-safety.md). For information about the C++ stack-unwinding mechanism, see [Overview: Exceptions in C++](../VS_csharp/exceptions-and-stack-unwinding-in-c--.md).  
...
-   For every function that might throw or propagate an exception, provide one of the three exception guarantees: the strong guarantee, the basic guarantee, or the nothrow (noexcept) guarantee. For more information, see [How to: Design for Exceptions](../VS_csharp/how-to--design-for-exception-safety.md).  
...
-   Use standard library exception types when they apply. Derive custom exception types from the [exception Class](../VS_csharp/exception-class.md) hierarchy. For more information, see [(NOTINBUILD)How to: Use the Standard Library Exception Objects](assetId:///ad1fb785-ed4e-4d94-8e84-964353aed7b6).  
...
## C++ exceptions versus Windows SEH exceptions  
 ...
 For more information about SEH, see [Structured Exception Handling (C++)](../VS_csharp/structured-exception-handling--c-c---.md).  
  
## Exception specifications and noexcept  
 Exception specifications were introduced in C++ as a way to specify the exceptions that a function might throw. However, exception specifications proved problematic in practice, and are deprecated in the C++11 draft standard. We recommend that you do not use exception specifications except for `throw()`, which indicates that the exception allows no exceptions to escape. If you must use exception specifications of the type `throw(`*type*`)`, be aware that [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)] departs from the standard in certain ways. For more information, see [Exception Specifications](../VS_csharp/exception-specifications--throw---c---.md). The `noexcept` specifier is introduced in C++11 as the preferred alternative to `throw()`.  
  
## See Also  
 [How to: Interface Between Exceptional and Non-Exceptional Code](../VS_csharp/how-to--interface-between-exceptional-and-non-exceptional-code.md)   
