...
In the program code for an extension to Visual Studio, you can control how model elements are displayed on diagrams. To see which versions of Visual Studio support UML models, see [Version support for architecture and modeling tools](../VS_csharp/what-s-new-for-design-in-visual-studio.md#VersionSupport).  
...
>  An instance of `IDiagram` (and its subtypes such as `IClassDiagram`) is valid only within the command you are processing. It is not recommended to keep an `IDiagram` object in a variable that persists while control is returned to the user.  
 ...
 For more information, see [How to Add Commands and Gestures to a UML Diagram](../VS_csharp/define-a-menu-command-on-a-modeling-diagram.md).  
...
## To access the diagrams in a project  
 The [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] API can be used to open and create modeling projects and diagrams.  
...
```  
 ...
 Instances of `IDiagram` and its subtypes are not valid after you return control to [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)].  
  
 You can also obtain the model store from a [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] project:  
...
##  <a name="AlignCommand"></a> Example: Command for Aligning Shapes  
 ...
 To make the command available, add this code to a menu command project, and then deploy the resulting extension to your users. For more information, see [How to Define a Menu Command](../VS_csharp/define-a-menu-command-on-a-modeling-diagram.md).  
