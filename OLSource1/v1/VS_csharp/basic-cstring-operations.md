...
This topic explains the following basic [CString](../VS_csharp/cstringt-class.md) operations:  
...
-   [Converting CString objects](#_core_converting_cstring_objects)  
 ...
 `Class CString` is based on class template [CStringT](../VS_csharp/cstringt-class.md). `CString` is a `typedef` of `CStringT`. More exactly, `CString` is a `typedef` of an *explicit specialization* of `CStringT`, which is a common way to use a class template to define a class. Similarly defined classes are `CStringA` and `CStringW`. For more information on explicit specialization, see [Class Template Instantiation](../Topic/Class%20Template%20Instantiation.md).  
...
-   Assign the value of a C literal string to a `CString` object.  
     ...
     [!code[NVC_ATLMFC_Utilities#183](../VS_csharp/codesnippet/CPP/basic-cstring-operations_1.cpp)]  
  
-   Assign the value of one `CString` to another `CString` object.  
  
     [!code[NVC_ATLMFC_Utilities#184](../VS_csharp/codesnippet/CPP/basic-cstring-operations_2.cpp)]  
  
     The contents of a `CString` object are copied when one `CString` object is assigned to another. Therefore, the two strings do not share a reference to the actual characters that make up the string. For more information about how to use `CString` objects as values, see [CString Semantics](../VS_csharp/cstring-semantics.md).  
  
    > [!NOTE]
    >  To write your application so that it can be compiled for Unicode or for ANSI, code literal strings by using the _T macro. For more information, see [Unicode and Multibyte Character Set (MBCS) Support](../VS_csharp/unicode-and-multibyte-character-set--mbcs--support.md).  
...
##  <a name="_core_concatenating_two_cstring_objects"></a> Concatenating Two CString Objects  
 ...
 [!code[NVC_ATLMFC_Utilities#185](../VS_csharp/codesnippet/CPP/basic-cstring-operations_3.cpp)]  
...
##  <a name="_core_comparing_cstring_objects"></a> Comparing CString Objects  
 ...
 The following table shows the available [CString](../VS_csharp/cstringt-class.md) comparison functions and their equivalent Unicode/MBCS-portable functions in the C run-time library.  
...
|`Collate`|`_mbscoll`|`wcscoll`|  
 ...
 [!code[NVC_ATLMFC_Utilities#186](../VS_csharp/codesnippet/CPP/basic-cstring-operations_4.cpp)]  
  
##  <a name="_core_converting_cstring_objects"></a> Converting CString Objects  
 For information about converting CString objects to other string types, see [How to: Convert Between Various String Types](../VS_csharp/how-to--convert-between-various-string-types.md).  
...
## See Also  
 ...
 [Partial Specialization of Class Templates](../VS_csharp/template-specialization--c---.md)   
 [How to: Convert Between Various String Types](../VS_csharp/how-to--convert-between-various-string-types.md)