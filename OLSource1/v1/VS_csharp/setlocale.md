...
## Remarks  
 ...
 The "C" locale maps each character in the string to its value as a `wchar_t` (unsigned short). Other values that are valid for `setlocale` are those entries that are found in the [Language Strings](../VS_csharp/language-strings.md) list. For example, you could issue:  
