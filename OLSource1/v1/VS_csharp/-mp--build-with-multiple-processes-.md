...
|--------------------------------|-----------------|  
|[#import](../VS_csharp/sharpimport-directive--c---.md) preprocessor directive|Converts the types in a type library into C++ classes, and then writes those classes to a header file.|  
|[/E](../VS_csharp/-e--preprocess-to-stdout-.md), [/EP](../VS_csharp/-ep--preprocess-to-stdout-without-sharpline-directives-.md)|Copies preprocessor output to the standard output (**stdout**).|  
|[/Gm](../VS_csharp/-gm--enable-minimal-rebuild-.md)|Enables an incremental rebuild.|  
|[/showIncludes](../VS_csharp/-showincludes--list-include-files-.md)|Writes a list of include files to the standard error (**stderr**).|  
|[/Yc](../VS_csharp/-yc--create-precompiled-header-file-.md)|Writes a precompiled header file.|  
...
|------------------------|-----------------|-----------------------|  
|`C2813`|The **#import** directive is not compatible with the **/MP** option.|The compilation ends unless a [compiler warning level](../VS_csharp/-w---w0---w1---w2---w3---w4---w1---w2---w3---w4---wall---wd---we---wo---wv---wx--warning-level-.md) option specifies otherwise.|  
...
-   In your remaining source files, insert [#include](../VS_csharp/sharpinclude-directive--c-c---.md) directives that specify the generated headers, and then compile your remaining source files by using the **/MP** option.  
  
### Visual Studio Project Settings  
  
#### The MSBUILD.exe Tool  
 [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] uses the [MSBuild.exe](../VS_csharp/msbuild-reference.md) tool to build solutions and projects. The **/maxcpucount:**`number` (or **/m:**`number`) command-line option of the MSBuild.exe tool can build multiple projects at the same time. And the **/MP** compiler option can build multiple compilation units at the same time. If it is appropriate for your application, improve your solution's build time by using either or both **/MP** and **/maxcpucount**.  
  
 The build time of your solution partly depends on the number of processes that perform the build. The `number` argument of the [/maxcpucount](../VS_csharp/msbuild-command-line-reference.md) MSBuild option specifies the maximum number of projects to build at the same time. Similarly, the `processMax` argument of the **/MP** compiler option specifies the maximum number of compilation units to build at the same time. If the **/maxcpucount** option specifies *P* projects and the **/MP** option specifies *C* processes, a maximum of *P*x*C* processes execute at the same time.  
