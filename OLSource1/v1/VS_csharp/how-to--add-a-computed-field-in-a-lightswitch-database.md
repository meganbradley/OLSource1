...
6.  Add code to the *FieldName*`_Compute` method that sets the value of the `result` parameter. The following example sets the value of the `Subtotal` field by multiplying the value of the `UntiPrice` field by the value of the `OrderQuantity` field.  
     ...
     [!code[LS_Data#1](../VS_csharp/codesnippet/CSharp/how-to--add-a-computed-field-in-a-lightswitch-database_1.cs)]
[!code[LS_Data#1](../VS_csharp/codesnippet/VisualBasic/how-to--add-a-computed-field-in-a-lightswitch-database_1.vb)]  
 ...
 In most cases, the value of a computed field is recalculated based on changes to any field that you use to derive the value of the computed field. If the value of the computed field doesn't update, users can refresh the screen to view the updated value. You can also write custom code that refreshes the screen when certain events occur. See [How to: Handle Data Events](../VS_csharp/how-to--handle-data-events.md).  
