...
---
# ios_base Class
The class describes the storage and member functions common to both input and output streams that do not depend on the template parameters. (The template class [basic_ios](../VS_csharp/basic_ios-class.md) describes what is common and is dependent on template parameters.)  
...
|||  
|-|-|  
|[failure](#ios_base__failure)|The member class serves as the base class for all exceptions thrown by the member function [clear](../VS_csharp/basic_ios-class.md#basic_ios__clear) in template class [basic_ios](../VS_csharp/basic_ios-class.md).|  
...
-   **copyfmt_event**, to identify a callback that occurs near the end of a call to [copyfmt](../VS_csharp/basic_ios-class.md#basic_ios__copyfmt), just before the [exception mask](../VS_csharp/ios_base-class.md) is copied.  
  
-   **erase_event**, to identify a callback that occurs at the beginning of a call to [copyfmt](../VS_csharp/basic_ios-class.md#basic_ios__copyfmt), or at the beginning of a call to the destructor for **\*this**.  
...
### Remarks  
 Supports the manipulators in [ios](../VS_csharp/-ios-.md).  
...
-   `internal`, to pad to a field width as needed by inserting fill characters at a point internal to a generated numeric field. (For information on setting the field width, see [setw](../VS_csharp/-iomanip--functions.md#setw)).  
...
-   `floatfield`, defined as `fixed` &#124; `scientific`  
 ...
 For examples of functions that modify these format flags, see [<iomanip\>](../VS_csharp/-iomanip-.md).  
...
### Example  
  See [basic_ios::imbue](../VS_csharp/basic_ios-class.md#basic_ios__imbue) for a sample.  
...
### Remarks  
 The (protected) constructor does nothing. A later call to **basic_ios::**[init](../VS_csharp/basic_ios-class.md#basic_ios__init) must initialize the object before it can be safely destroyed. Thus, the only safe use for class ios_base is as a base class for template class [basic_ios](../VS_csharp/basic_ios-class.md).  
...
### Remarks  
 The member function returns a reference to element `idx` of the extensible array with elements of type **long**. All elements are effectively present and initially store the value zero. The returned reference is invalid after the next call to `iword` for the object, after the object is altered by a call to **basic_ios::**[copyfmt](../VS_csharp/basic_ios-class.md#basic_ios__copyfmt), or after the object is destroyed.  
  
 If `idx` is negative or if unique storage is unavailable for the element, the function calls [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)**(badbit)** and returns a reference that might not be unique.  
...
### Return Value  
 The first member function returns the stored [display precision](../VS_csharp/ios_base-class.md). The second member function stores `_Prec` in the display precision and returns its previous stored value.  
  
### Remarks  
 Floating-point numbers are displayed in fixed notation with [fixed](../VS_csharp/-ios--functions.md#fixed).  
...
### Remarks  
 ...
 If _                        *Idx* is negative, or if unique storage is unavailable for the element, the function calls [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)**(badbit)** and returns a reference that might not be unique.  
...
### Remarks  
 The member function pushes the pair `{``pfn`, `idx``}` onto the stored callback stack [callback stack](../VS_csharp/ios_base-class.md). When a callback event **ev** is reported, the functions are called, in reverse order of registry, by the expression ( **\***`pfn`)( **ev**, ``**\*this**, ```idx`).  
...
### Remarks  
 The static member function stores a **stdio** sync flag, which is initially **true**. When **true**, this flag ensures that operations on the same file are properly synchronized between the [iostreams](../VS_csharp/iostreams-conventions.md) functions and those defined in the Standard C++ Library. Otherwise, synchronization may or may not be guaranteed, but performance may be improved. The function stores `_Sync` in the **stdio** sync flag and returns its previous stored value. You can call it reliably only before performing any operations on the standard streams.  
