...
```  
 ...
 Because the Visual C++ compiler uses name decoration for C++ functions, you must either use the decorated name as the `entryname` or `internalname`, or define the exported functions by using `extern "C"` in the source code. The compiler also decorates C functions that use the [__stdcall](../VS_csharp/__stdcall.md) calling convention with an underscore (_) prefix and a suffix composed of the at sign (@) followed by the number of bytes (in decimal) in the argument list.  
  
 To find the decorated names produced by the compiler, use the [DUMPBIN](../VS_csharp/dumpbin-reference.md) tool or the linker [/MAP](../VS_csharp/-map--generate-mapfile-.md) option. The decorated names are compiler-specific. If you export the decorated names in the .DEF file, executables that link to the DLL must also be built by using the same version of the compiler. This ensures that the decorated names in the caller match the exported names in the .DEF file.  
...
1.  The [__declspec(dllexport)](../VS_csharp/dllexport--dllimport.md) keyword in the source code  
  
2.  An `EXPORTS` statement in a .DEF file  
  
3.  An [/EXPORT](../VS_csharp/-export--exports-a-function-.md) specification in a LINK command  
  
4.  A [comment](../VS_csharp/comment--c-c---.md) directive in the source code, of the form `#pragma comment(linker, "/export:``definition``")`  
