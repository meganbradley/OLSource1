...
|||  
|-|-|  
|[AFX_ODBC_CALL](../VS_csharp/afx_odbc_call.md)|Calls an ODBC API function that returns `SQL_STILL_EXECUTING`. `AFX_ODBC_CALL` will repeatedly call the function until it no longer returns `SQL_STILL_EXECUTING`.|  
|[AFX_SQL_ASYNC](../VS_csharp/afx_sql_async.md)|Calls `AFX_ODBC_CALL`.|  
|[AFX_SQL_SYNC](../VS_csharp/afx_sql_sync.md)|Calls an ODBC API function that does not return `SQL_STILL_EXECUTING`.|  
  
### Database Globals  
  
|||  
|-|-|  
|[AfxGetHENV](../VS_csharp/afxgethenv.md)|Retrieves a handle to the ODBC environment currently in use by MFC. You can use this handle in direct ODBC calls.|  
