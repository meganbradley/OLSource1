...
This article explains the default printing process in Windows in terms of the MFC framework.  
 ...
 In MFC applications, the view class has a member function named `OnDraw` that contains all the drawing code. `OnDraw` takes a pointer to a [CDC](../VS_csharp/cdc-class.md) object as a parameter. That `CDC` object represents the device context to receive the image produced by `OnDraw`. When the window displaying the document receives a [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) message, the framework calls `OnDraw` and passes it a device context for the screen (a [CPaintDC](../VS_csharp/cpaintdc-class.md) object, to be specific). Accordingly, `OnDraw`'s output goes to the screen.  
 ...
 That protocol is described in the article [Multipage Documents](../VS_csharp/multipage-documents.md).  
