...
Searches for a file by using environment paths. These versions of [_searchenv, _wsearchenv](../VS_csharp/_searchenv--_wsearchenv.md) have security enhancements, as described in [Security Enhancements in the CRT](../VS_csharp/security-features-in-the-crt.md).  
...
|any|any|any|<= 0|`EINVAL`|not changed|  
 ...
 If any of these error conditions occurs, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, these functions set `errno` to `EINVAL` and return `EINVAL`.  
