...
## Example  
 The following code example illustrates how to use `Async.AwaitIAsyncResult` to set up and execute a computation that is triggered when a previous .NET Framework asynchronous operation that produces an <xref:System.IAsyncResult*> finishes. In this case, the call to `AwaitIAsyncResult` causes the operation to wait for a file write operation to be completed before opening the file for reading.  
  
 [!code[FsAsyncAPIs#15](../VS_csharp/codesnippet/FSharp/async.awaitiasyncresult-method--fsharp-_1.fs)]  
