...
---
# qsort
Performs a quick sort. A more secure version of this function is available; see [qsort_s](../VS_csharp/qsort_s.md).  
...
|> 0|`elem1` greater than `elem2`|  
 ...
 This function validates its parameters. If `compare` or `num` is `NULL`, or if `base` is `NULL` and *`num` is nonzero, or if `width` is less than zero, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, the function returns and `errno` is set to `EINVAL`.  
