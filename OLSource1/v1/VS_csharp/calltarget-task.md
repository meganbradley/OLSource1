...
|---------------|-----------------|  
|`RunEachTargetSeparately`|Optional `Boolean` output parameter.<br /><br /> If `true`, the [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] engine is called once per target. If `false`, the [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] engine is called once to build all targets. The default value is `false`.|  
...
## Remarks  
 ...
 If you want to build the default targets, use the [MSBuild Task](../VS_csharp/msbuild-task.md) and set the `Projects` parameter equal to `$(MSBuildProjectFile)`.  
