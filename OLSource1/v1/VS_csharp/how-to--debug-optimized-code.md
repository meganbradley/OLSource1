...
> [!NOTE]
>  The [/Zo (Enhance Optimized Debugging)](../VS_csharp/-zo--enhance-optimized-debugging-.md)compiler option (introduced in Visual Studio Update 3) generates richer debugging information for optimized code (projects that are not built with the **/Od** compiler option. See [/O Options (Optimize Code)](../VS_csharp/-o-options--optimize-code-.md)). This includes improved support for debugging local variables and inlined functions.  
>   
>  [Edit and Continue](../VS_csharp/edit-and-continue--visual-csharp-.md) is disabled when the **/Zo** ocompiler option is used.  
...
7.  In the properties list on the right, find `Optimization`. The setting next to it probably says `Disabled (`[/Od](../VS_csharp/-od--disable--debug--.md)`)`. Choose one of the other options (`Minimum Size``(`[/O1](../VS_csharp/-o1---o2--minimize-size--maximize-speed-.md)`)`, `Maximum Speed``(`[/O2](../VS_csharp/-o1---o2--minimize-size--maximize-speed-.md)`)`, `Full Optimization``(`[/Ox](../VS_csharp/-ox--full-optimization-.md)`)`, or `Custom`).  
...
9. Select the Configuation Properties, C/C++, Command Line node of the project properties page, and add `(`[/Zo](../VS_csharp/-zo--enhance-optimized-debugging-.md)`)` to the **Additional Options** text box.  
  
    > [!WARNING]
    >  `/Zo` requires Visual Studio 2013 Update 3 or a later version.  
    >   
    >  Adding `/Zo` will disable [Edit and Continue](../VS_csharp/edit-and-continue--visual-csharp-.md).  
