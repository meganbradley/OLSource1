...
---
# tmpnam_s, _wtmpnam_s
Generate names you can use to create temporary files. These are versions of [tmpnam and _wtmpnam](../VS_csharp/_tempnam--_wtempnam--tmpnam--_wtmpnam.md) with security enhancements as described in [Security Enhancements in the CRT](../VS_csharp/security-features-in-the-crt.md).  
...
|not `NULL` (points to valid memory)|too short|`ERANGE`|not modified|  
 ...
 If `str` is `NULL`, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, these functions set `errno` to `EINVAL` and return `EINVAL`.  
