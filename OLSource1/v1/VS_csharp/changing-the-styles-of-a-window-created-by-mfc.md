...
##  <a name="_core_changing_styles_in_an_existing_application"></a> Changing Styles in an Existing Application  
 ...
 To change the default window attributes used by a framework application created with the Application Wizard, override the window's [PreCreateWindow](../Topic/CWnd::PreCreateWindow.md) virtual member function. `PreCreateWindow` allows an application to access the creation process normally managed internally by the [CDocTemplate](../VS_csharp/cdoctemplate-class.md) class. The framework calls `PreCreateWindow` just prior to creating the window. By modifying the [CREATESTRUCT](../VS_csharp/createstruct-structure.md) structure passed to `PreCreateWindow`, your application can change the attributes used to create the window. For example, to ensure that a window does not use a caption, use the following bitwise operation:  
  
 [!code[NVC_MFCDocView#15](../VS_csharp/codesnippet/CPP/changing-the-styles-of-a-window-created-by-mfc_1.cpp)]  
  
 The [CTRLBARS](../VS_csharp/visual-c---samples.md) sample application demonstrates this technique for changing window attributes. Depending on what your application changes in `PreCreateWindow`, it may be necessary to call the base class implementation of the function.  
...
##  <a name="_core_the_sdi_case"></a> The SDI Case  
 ...
 [!code[NVC_MFCDocViewSDI#11](../VS_csharp/codesnippet/CPP/changing-the-styles-of-a-window-created-by-mfc_2.cpp)]  
...
##  <a name="_core_the_mdi_case"></a> The MDI Case  
 A little more work is required to change the window style of a child window in a multiple document interface (MDI) application. By default, an MDI application created with the Application Wizard uses the default [CMDIChildWnd](../VS_csharp/cmdichildwnd-class.md) class defined in MFC. To change the window style of an MDI child window, you must derive a new class from `CMDIChildWnd` and replace all references to `CMDIChildWnd` in your project with references to the new class. Most likely, the only reference to `CMDIChildWnd` in the application is located in your application's `InitInstance` member function.  
 ...
 [!code[NVC_MFCDocView#16](../VS_csharp/codesnippet/CPP/changing-the-styles-of-a-window-created-by-mfc_3.cpp)]  
  
 This code creates MDI child windows without a Maximize button.  
  
### What do you want to know more about?  
  
-   [Windows styles](../VS_csharp/window-styles.md)  
