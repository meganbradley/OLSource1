...
This article shows how to define and consume user-defined reference types and value types in [!INCLUDE[cppcli](../VS_csharp/includes/cppcli_md.md)].  
...
##  <a name="BKMK_Implicitly_abstract_classes"></a> Implicitly abstract classes  
 ...
 If you are unable to construct objects from a class that's derived from an interface, the reason might be that the class is implicitly abstract. For more information about abstract classes, see [abstract](../VS_csharp/abstract---c---component-extensions-.md).  
...
##  <a name="BKMK_Type_visibility"></a> Type visibility  
 ...
 By default prior to Visual C++ 2005, native types had public accessibility outside the assembly. Enable [Compiler Warning (level 1) C4692](../VS_csharp/compiler-warning--level-1--c4692.md) to help you see where private native types are used incorrectly. Use the [make_public](../VS_csharp/make_public.md) pragma to give public accessibility to a native type in a source code file that you can't modify.  
  
 For more information, see [The #using Directive](../VS_csharp/sharpusing-directive--c---.md).  
...
|Specifier|Effect|  
|---------------|------------|  
|public|Member is accessible inside and outside the assembly.  See [public (C++)](../VS_csharp/public--c---.md) for more information.|  
|private|Member is not accessible, neither inside nor outside the assembly.  See [private (C++)](../VS_csharp/private--c---.md) for more information.|  
|protected|Member is accessible inside and outside the assembly, but only to derived types.  See [protected (C++)](../VS_csharp/protected--c---.md) for more information.|  
|internal|Member is public inside the assembly but private outside the assembly.  `internal` is a context-sensitive keyword.  For more information, see [Context-Sensitive Keywords](../VS_csharp/context-sensitive-keywords---c---component-extensions-.md).|  
...
##  <a name="BKMK_Static_constructors"></a> Static constructors  
 ...
 For more information about static constructors, see [Interface Static Constructor](../VS_csharp/how-to--define-an-interface-static-constructor--c---cli-.md) .  
...
-   [^ (Handle to Object on Managed Heap)](../VS_csharp/handle-to-object-operator--^----c---component-extensions-.md)  
  
-   [interior_ptr](../VS_csharp/interior_ptr--c---cli-.md)  
...
##  <a name="BKMK_Destructors_and_finalizers"></a> Destructors and finalizers  
 Destructors in a reference type perform a deterministic clean-up of resources. Finalizers clean up unmanaged resources and can be called deterministically by the destructor or nondeterministically by the garbage collector. For information about destructors in standard C++, see [Destructors (C++)](../VS_csharp/destructors--c---.md).  
...
```  
 ...
 The behavior of destructors in a managed Visual C++ class differs from Managed Extensions for C++. For more information about this change, see [Changes in Destructor Semantics](../VS_csharp/changes-in-destructor-semantics.md).  
...
-   An object that's created by using stack semantics goes out of scope. For more information, see [Stack Semantics for Reference Types](../VS_csharp/c---stack-semantics-for-reference-types.md).  
  
-   An exception is thrown during the object's construction.  
  
-   The object is a member in an object whose destructor is running.  
  
-   You call the [delete](../VS_csharp/delete-operator--c---.md) operator on a handle ([^ (Handle to Object on Managed Heap)](../VS_csharp/handle-to-object-operator--^----c---component-extensions-.md)).  
...
-   If the type goes out of scope in a C# `using` statement.  
 ...
 If you create an object of a reference type on the managed heap (not using stack semantics for reference types), use [try-finally](../VS_csharp/try-finally-statement.md) syntax to ensure that an exception doesn't prevent the destructor from running.  
...
```  
 ...
 If a type has either a finalizer or a destructor, the compiler generates a `Dispose(bool)` method, according to the design pattern. (For information, see [Implementing Finalize and Dispose to Clean Up Unmanaged Resources](assetId:///31a6c13b-d6a2-492b-9a9f-e5238c983bcb)). You cannot explicitly author or call `Dispose(bool)` in Visual C++.  
 ...
 At compile time, you can detect whether a type has a finalizer or a destructor. For more information, see [Intrinsic Support for Type Traits](../VS_csharp/compiler-support-for-type-traits--c---component-extensions-.md).  
...
## See Also  
 [Classes and Structs](../VS_csharp/classes-and-structs---c---component-extensions-.md)   
