...
## Specifying dynamic layout settings for an MFC dialog box  
 ...
 ![Dialog before being resized.](../VS_csharp/media/mfcdynamiclayout4.PNG "MFCDynamicLayout4")  
 ...
 ![Dialog after being resized.](../VS_csharp/media/mfcdynamiclayout5.PNG "MFCDynamicLayout5")  
...
1.  With an MFC project open, open the dialog you want to work with in the dialog editor.  
     ...
     ![Open the dialog in the resource editor.](../VS_csharp/media/mfcdynamiclayout3.PNG "MFCDynamicLayout3")  
...
3.  If you want a control such as a button to be at a fixed size and stay in place at the bottom right, as is common for the **OK** or **Cancel** buttons, set the **Sizing Type** to **None**, and set the **Moving Type** to **Both**. For the **Moving X** and **Moving Y** values under **Moving Type**, set 100% to cause the control to stay a fixed distance from the bottom right corner.  
     ...
     ![Dynamic Layout](../VS_csharp/media/mfcdynamiclayout1.PNG "MFCDynamicLayout1")  
...
4.  Suppose you also have a control that you want to expand as the dialog expands. Typically, a user might expand a dialog in order to expand a multiline editbox to increase the size of the text area, or they might expand a list control to see more data. For this case, set the **Sizing Type** to Both, and set the **Moving Type** to none. Then, set the **Sizing X** and **Sizing Y** values to 100.  
     ...
     ![Dynamic Layout Settings](../VS_csharp/media/mfcdynamiclayout2.PNG "MFCDynamicLayout2")  
...
3.  For the first control to which you want to add dynamic behavior, use the static methods on the dynamic layout class to create the [MoveSettings](../VS_csharp/cmfcdynamiclayout--movesettings-structure.md) structure that encodes the way the control should be adjusted. You do this by first choosing the appropriate static method: [MoveHorizontal](../Topic/CMFCDynamicLayout::MoveHorizontal.md), [MoveVertical](../Topic/CMFCDynamicLayout::MoveVertical.md), [MoveNone](../Topic/CMFCDynamicLayout::MoveNone.md), or [MoveHorizontalAndVertical](../Topic/CMFCDynamicLayout::MoveHorizontalAndVertical.md). You pass in a percentage for the horizontal and/or vertical aspects of the move. These static methods all return a newly created MoveSettings object that you can use to specify a control's move behavior.  
...
4.  Do the same thing for the size behavior, which uses the [SizeSettings](../VS_csharp/cmfcdynamiclayout--sizesettings-structure.md) type. For example, to specify that a control does not change size when the dialog resizes, use the following code:  
...
1.  Use the [LoadResource](../Topic/CMFCDynamicLayout::MoveHorizontalAndVertical.md) method to specify a resource name in the relevant resource script file (.rc file) that specifies dynamic layout information, as in the following example:  
...
## See Also  
 ...
 [Control Classes](../VS_csharp/control-classes.md)   
 [Dialog Box Classes](../VS_csharp/dialog-box-classes.md)   
