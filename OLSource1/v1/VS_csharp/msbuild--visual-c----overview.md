...
## Prerequisites  
 ...
 Lists the MSBuild XML Schema elements, together with their attributes, and parent and child elements. Especially note the [ItemGroup](../VS_csharp/itemgroup-element--msbuild-.md), [PropertyGroup](../VS_csharp/propertygroup-element--msbuild-.md), [Target](../VS_csharp/target-element--msbuild-.md), and [Task](../VS_csharp/task-element--msbuild-.md) elements.  
 ...
 Describes MSBuild tasks. Especially note these tasks, which are specific to Visual C++: [BscMake Task](../VS_csharp/bscmake-task.md), [CL Task](../VS_csharp/cl-task.md), [CPPClean Task](../VS_csharp/cppclean-task.md), [LIB Task](../VS_csharp/lib-task.md), [Link Task](../VS_csharp/link-task.md), [MIDL Task](../VS_csharp/midl-task.md), [MT Task](../VS_csharp/mt-task.md), [RC Task](../VS_csharp/rc-task.md), [SetEnv Task](../VS_csharp/setenv-task.md), [VCMessage Task](../VS_csharp/vcmessage-task.md), [XDCMake Task](../VS_csharp/xdcmake-task.md), [XSD Task](../VS_csharp/xsd-task.md).  
  
## MSBuild on the Command Line  
 The following statement from the [MSBuild Command Line Reference](../VS_csharp/msbuild-command-line-reference.md) document illustrates that the msbuild.exe tool takes an implicit or explicit `project file` argument (a .vcxproj file for Visual C++ projects) and zero or more command-line `options`.  
 ...
 **msbuild.exe [** `project file` **] [** `options` **]**  
 ...
 Each target consists of a sequence of one or more *tasks*. Each task is represented by a .NET Framework class that contains one executable command. For example, the [CL task](../VS_csharp/cl-task.md) contains the [cl.exe](../VS_csharp/compiling-a-c-c---program.md) command.  
 ...
 Additional task parameters support the MSBuild infrastructure. For example, the `Sources` task parameter specifies a set of tasks that can be consumed by other tasks. For more information about MSBuild tasks, see [MSBuild Task Reference](../VS_csharp/msbuild-task-reference.md).  
...
### PlatformToolset Property  
 ...
 Set the `PlatformToolset` property to `v110` to use [!INCLUDE[cpp_dev11_long](../VS_csharp/includes/cpp_dev11_long_md.md)] tools and libraries to build your application.  
  
 `msbuild myProject.vcxproj /p:PlatformToolset=v110`  
  
 Set the `PlatformToolset` property to `v100` to use [!INCLUDE[cpp_dev10_long](../VS_csharp/includes/cpp_dev10_long_md.md)] tools and libraries to build your application.  
