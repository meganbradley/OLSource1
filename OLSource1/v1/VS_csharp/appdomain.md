...
---
# appdomain
Specifies that each application domain of your managed application should have its own copy of a particular global variable or static member variable. See [Application Domains and Visual C++](../VS_csharp/application-domains-and-visual-c--.md) for more information.  
 ...
 Using `__declspec(appdomain)` is similar to using [Thread Local Storage (TLS)](../VS_csharp/thread-local-storage--tls-.md). Threads have their own storage, as do application domains. Using `__declspec(appdomain)` ensures the global variable has its own storage in each application domain created for this application.  
  
 There are limitations to mixing the use of per process and per appdomain variables; see [process](../VS_csharp/process.md) for more information.  
 ...
 For information on how to call a function in a specific application domain, see [call_in_appdomain Function](../VS_csharp/call_in_appdomain-function.md).  
