...
This article presents an example of using class [CSocket](../VS_csharp/csocket-class.md). The example employs `CArchive` objects to serialize data through a socket. Note that this is not document serialization to or from a file.  
 ...
 [!code[NVC_MFCSimpleSocket#1](../VS_csharp/codesnippet/CPP/windows-sockets--example-of-sockets-using-archives_1.cpp)]  
  
 The most important thing about this example is that its structure parallels that of an MFC `Serialize` function. The **PacketSerialize** member function consists of an **if** statement with an **else** clause. The function receives two [CArchive](../VS_csharp/carchive-class.md) references as parameters: `arData` and `arAck`. If the `arData` archive object is set for storing (sending), the **if** branch executes; otherwise, if `arData` is set for loading (receiving) the function takes the **else** branch. For more information about serialization in MFC, see [Serialization](../VS_csharp/how-to--make-a-type-safe-collection.md).  
