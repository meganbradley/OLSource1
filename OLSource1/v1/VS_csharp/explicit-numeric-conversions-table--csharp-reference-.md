...
Explicit numeric conversion is used to convert any numeric type to any other numeric type, for which there is no implicit conversion, by using a cast expression. The following table shows these conversions.  
 ...
 For more information about conversions, see [Casting and Type Conversions](../VS_csharp/casting-and-type-conversions--csharp-programming-guide-.md).  
...
-   When you convert `decimal` to `float` or `double`, the `decimal` value is rounded to the nearest `double` or `float` value.  
 ...
 For more information on explicit conversion, see Explicit in the C# Language Specification. For more information on how to access the spec, see [C# Language Specification](../VS_csharp/csharp-language-specification.md).  
