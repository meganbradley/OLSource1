...
Tier interaction profiling provides additional information about the execution times of synchronous [!INCLUDE[vstecado](../VS_csharp/includes/vstecado_md.md)] calls in functions of multi-tiered applications that communicate with one or more databases.  
 ...
 **Visual Studio editions**  
 ...
 Tier interaction profiling can be collected using [!INCLUDE[vsUltLong](../VS_csharp/includes/vsultlong_md.md)], [!INCLUDE[vsPreLong](../VS_csharp/includes/vsprelong_md.md)], or [!INCLUDE[vs_pro_current_short](../VS_csharp/includes/vs_pro_current_short_md.md)]. However, tier interaction profiling data can be viewed only in [!INCLUDE[vsUltLong](../VS_csharp/includes/vsultlong_md.md)] and [!INCLUDE[vsPreLong](../VS_csharp/includes/vsprelong_md.md)].  
 ...
 **TIP reports**  
 ...
 Tier interaction data can only be viewed in the [!INCLUDE[vsUltLong](../VS_csharp/includes/vsultlong_md.md)] IDE. File based tier interaction reports through [VSPerfReport](../VS_csharp/vsperfreport.md) are not available.  
...
### Profiling stand-alone applications  
 To add tier interaction data to an application that is not run by another process, such as a Windows desktop application that makes synchronous [!INCLUDE[vstecado](../VS_csharp/includes/vstecado_md.md)] calls to a SQLServer database, use the **VSPerfClrEnv /InteractionOn** option to set the environment variables, and the **VSPerfClrEnv /InteractionOff** option to remove them.  
...
6.  Clear the TIP environment variables. Type the following command:  
 ...
 For more information, see [Command-Line Profiling of Stand-Alone Applications](../VS_csharp/command-line-profiling-of-stand-alone-applications.md).  
  
### Profiling services  
 To profile services, including [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] applications, use the **VSPerfClrEnv /GlobalInteractionOn** option to set the environment variables, and the **VSPerfClrEnv /GlobalInteractionOff** option to remove them.  
  
 When you are profiling services, including [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] Web applications, you will often need to restart the computer to enable profiling.  
...
## Adding tier interaction data with VSPerfASPNETCmd  
 ...
 To add tier interaction to profiling data collected by using VSPerfASPNETCmd, add the **/TIP** option to the command line. For example, use the following command line to collect tier interaction data for an [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] Web application by using the instrumentation method:  
  
```  
vsperfaspnetcmd /tip /trace http://localhost/MyWebApp  
```  
  
 For more information about VSPerfASPNETCmd, see [Rapid Web Site Profiling with VSPerfASPNETCmd](../VS_csharp/rapid-web-site-profiling-with-vsperfaspnetcmd.md).