...
**Microsoft Specific**  
 ...
 For functions declared with the `naked` attribute, the compiler generates code without prolog and epilog code. You can use this feature to write your own prolog/epilog code sequences using inline assembler code. Naked functions are particularly useful in writing virtual device drivers.  Note that the `naked` attribute is only valid on x86 and ARM, and is not available on [!INCLUDE[vcprx64](../VS_csharp/includes/vcprx64_md.md)].  
...
## Remarks  
 Because the `naked` attribute is only relevant to the definition of a function and is not a type modifier, naked functions must use extended attribute syntax and the [__declspec](../VS_csharp/__declspec.md) keyword.  
