...
## Remarks  
 The `Message` task allows [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] projects to issue messages to loggers at different steps in the build process.  
  
 If the `Condition` parameter evaluates to `true`, the value of the `Text` parameter will be logged and the build will continue to execute. If a `Condition` parameter does not exist, the message text is logged. For more information on logging, see [MSBuild Logging](../VS_csharp/obtaining-build-logs-with-msbuild.md).  
