...
```  
 ...
 Mixing inline and non-inline compile options on different modules can sometimes cause problems. If a C++ library is created with function inlining turned on ([/Ob1](../VS_csharp/-ob--inline-function-expansion-.md) or [/Ob2](../VS_csharp/-ob--inline-function-expansion-.md)) but the corresponding header file describing the functions has inlining turned off (no option), you will get error LNK2001. The functions do not get inlined into the code from the header file, but since they are not in the library file there is no address to resolve the reference.  
...
## See Also  
 [Linker Tools Error LNK2019](../VS_csharp/linker-tools-error-lnk2019.md)