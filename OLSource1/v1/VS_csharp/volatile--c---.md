...
## Remarks  
 You can use the [/volatile](../VS_csharp/-volatile--volatile-keyword-interpretation-.md) compiler switch to modify how the compiler interprets this keyword.  
 ...
 When `volatile` is used on a variable that also has the [__restrict](../VS_csharp/__restrict.md) keyword, `volatile` takes precedence.  
...
## ISO Compliant  
 If you are familiar with the [C# volatile](../VS_csharp/volatile--csharp-reference-.md) keyword, or familiar with the behavior of `volatile` in earlier versions of Visual C++, be aware that the C++11 ISO Standard `volatile` keyword is different and is supported in Visual Studio when the [/volatile:iso](../VS_csharp/-volatile--volatile-keyword-interpretation-.md) compiler option is specified. (For ARM, it's specified by default). The `volatile` keyword in C++11 ISO Standard code is to be used only for hardware access; do not use it for inter-thread communication. For inter-thread communication, use mechanisms such as [std::atomic<T\>](../VS_csharp/-atomic-.md) from the [C++ Standard Template Library](../VS_csharp/c---standard-library-reference.md).  
...
## See Also  
 ...
 [const](../VS_csharp/const--c---.md)   
 [const and volatile Pointers](../VS_csharp/const-and-volatile-pointers.md)