...
```  
 ...
 ![Structural elements of a lambda expression](../VS_csharp/media/lambdaexpsyntax.png "LambdaExpSyntax")  
...
```  
 ...
 A `capture` followed by an ellipsis is a pack expansion, as shown in this [variadic template](../VS_csharp/ellipses-and-variadic-templates.md) example:  
...
```  
 ...
 A lambda expression can take another lambda expression as its argument. For more information, see "Higher-Order Lambda Expressions" in the topic [Examples of Lambda Expressions](../VS_csharp/examples-of-lambda-expressions.md).  
...
### Exception Specification  
 You can use the `throw()` exception specification to indicate that the lambda expression does not throw any exceptions. As with ordinary functions, the Visual C++ compiler generates warning [C4297](../VS_csharp/compiler-warning--level-1--c4297.md) if a lambda expression declares the `throw()` exception specification and the lambda body throws an exception, as shown here:  
...
```  
 ...
 For more information, see [Exception Specifications](../VS_csharp/exception-specifications--throw---c---.md).  
  
### Return Type  
 The return type of a lambda expression is automatically deduced. You don't have to use the [auto](../VS_csharp/auto--c---.md) keyword unless you specify a *trailing-return-type*. The *trailing-return-type* resembles the return-type part of an ordinary method or function. However, the return type must follow the parameter list, and you must include the trailing-return-type keyword `->` before the return type.  
...
```  
 ...
 A lambda expression can produce another lambda expression as its return value. For more information, see "Higher-Order Lambda Expressions" in [Examples of Lambda Expressions](../VS_csharp/examples-of-lambda-expressions.md).  
...
## Microsoft-Specific  
 ...
 If you are using a Microsoft-specific modifier such as [__declspec](../VS_csharp/__declspec.md), you can insert it into a lambda expression immediately after the `parameter-declaration-clause`—for example:  
...
```  
 ...
 To determine whether a modifier is supported by lambdas, see the article about it in the [Microsoft-Specific Modifiers](../VS_csharp/microsoft-specific-modifiers.md) section of the documentation.  
  
 Visual Studio supports C++11 Standard lambda expression syntax and functionality, with these exceptions:  
  
-   Like all other classes, lambdas don't get automatically generated move constructors and move assignment operators. For more information about support for rvalue reference behaviors, see the "Rvalue References" section in [Support For C++11 Features](../VS_csharp/support-for-c--11-14-17-features--modern-c---.md).  
...
## See Also  
 ...
 [Function Objects](../VS_csharp/function-objects-in-the-stl.md)   
 [Function Call (C++)](../VS_csharp/function-call--c---.md)   
