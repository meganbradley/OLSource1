...
## Remarks  
 ...
 These functions validate their parameters. If `strSource` is a null pointer, or `strDest` is a NULL pointer (unless count is zero), or if `count` is greater than `INT_MAX`, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md) . If execution is allowed to continue, these functions set `errno` to `EINVAL` and return `INT_MAX`.  
