...
|[Function call operator](../VS_csharp/function-call-operator----.md)|**( )**|  
|[Explicit type conversion operator](../VS_csharp/explicit-type-conversion-operator----.md)|*type-name* **( )**|  
|[Member access operator](../VS_csharp/member-access-operators--.-and---.md)|**.** or **–>**|  
|[Postfix increment operator](../VS_csharp/postfix-increment-and-decrement-operators-----and---.md)|`++`|  
|[Postfix decrement operator](../VS_csharp/postfix-increment-and-decrement-operators-----and---.md)|**––**|  
...
-   Each formal argument is initialized with its corresponding actual argument in the expression list. (A formal argument is an argument that is declared in the function header and used in the body of a function.) Conversions are done as if by initialization — both standard and user-defined conversions are performed in converting an actual argument to the correct type. The initialization performed is illustrated conceptually by the following code:  
    ...
    ```  
     ...
     Note that the initialization is performed as if using the equal-sign syntax instead of the parentheses syntax. A copy of `i` is made prior to passing the value to the function. (For more information, see [Initializers](../VS_csharp/initializers.md) and [Conversions](../VS_csharp/user-defined-type-conversions--c---.md), [Initialization Using Special Member Functions](assetId:///82223d73-64cb-4923-b678-78f9568ff3ca), and [Explicit Initialization](assetId:///c89724f8-ddd3-4d77-b86d-77fcd8bd8595).  
  
     Therefore, if the function prototype (declaration) calls for an argument of type **long**, and if the calling program supplies an actual argument of type `int`, the actual argument is promoted using a standard type conversion to type **long** (see [Standard Conversions](../VS_csharp/standard-conversions.md)).  
     ...
     For actual arguments of class type, the formal argument is initialized by calling the class's constructor. (See [Constructors](../VS_csharp/constructors--c---.md) for more about these special class member functions.)  
...
-   Any argument of class type is passed by value as a data structure; the copy is created by binary copying instead of by invoking the class's copy constructor (if one exists).  
 ...
 Ellipses, if used, must be declared last in the argument list. For more information about passing a variable number of arguments, see the discussion of [va_arg, va_start, and va_list](../VS_csharp/va_arg--va_copy--va_end--va_start.md) in the *Run-Time Library Reference*.  
  
 For information on default arguments in CLR programming, see [... (Variable Argument Lists)](../VS_csharp/variable-argument-lists--...---c---cli-.md).  
  
 Default arguments enable you to specify the value an argument should assume if none is supplied in the function call. The following code fragment shows how default arguments work. For more information about restrictions on specifying default arguments, see [Default Arguments](../VS_csharp/default-arguments.md).  
