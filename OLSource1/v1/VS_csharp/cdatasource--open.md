 ...
 *pPropSet*  
 [in] A pointer to an array of [DBPROPSET](https://msdn.microsoft.com/en-us/library/ms714367.aspx) structures containing properties and values to be set. See [Property Sets and Property Groups](https://msdn.microsoft.com/en-us/library/ms713696.aspx) in the *OLE DB Programmer's Reference* in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
 ...
 *pPassword*  
 ...
 `nInitMode`  
 [in] Database initialization mode. See [Initialization Properties](https://msdn.microsoft.com/en-us/library/ms723127.aspx)in the *OLE DB Programmer's Reference* in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)] for a list of valid initialization modes. If `nInitMode` is zero, no initialization mode is included in the property set used to open the connection.  
  
 `szProgID`  
 [in] A program identifier.  
  
 `enumerator`  
 [in] A [CEnumerator](../VS_csharp/cenumerator-class.md) object used to obtain a moniker for opening the connection when the caller does not specify a **CLSID**.  
...
## Remarks  
 ...
 The method overloads that do not use the `hWnd` parameter open a data source object without using the service components in oledb32.dll. A [CDataSource](../VS_csharp/cdatasource-class.md) object opened with these function overloads will be unable to utilize any of the functionality of Service Components.  
...
## Example  
 ...
 [!code[NVC_OLEDB_Consumer#7](../VS_csharp/codesnippet/CPP/cdatasource--open_1.cpp)]  
  
## Requirements  
 **Header:** atldbcli.h  
  
## See Also  
 [CDataSource Class](../VS_csharp/cdatasource-class.md)