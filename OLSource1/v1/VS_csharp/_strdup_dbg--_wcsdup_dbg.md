...
Versions of [_strdup and _wcsdup](../VS_csharp/_strdup--_wcsdup--_mbsdup.md) that use the debug version of `malloc`.  
...
## Remarks  
 The `_strdup_dbg` and `_wcsdup_dbg` functions are identical to `_strdup` and `_wcsdup` except that, when `_DEBUG` is defined, these functions use the debug version of `malloc`, `_malloc_dbg`, to allocate memory for the duplicated string. For information on the debugging features of `_malloc_dbg`, see [_malloc_dbg](../VS_csharp/_malloc_dbg.md).  
  
 You do not need to call these functions explicitly in most cases. Instead, you can define the flag `_CRTDBG_MAP_ALLOC`. When `_CRTDBG_MAP_ALLOC` is defined, calls to `_strdup` and `_wcsdup` are remapped to `_strdup_dbg` and `_wcsdup_dbg`, respectively, with the `blockType` set to `_NORMAL_BLOCK`. Thus, you do not need to call these functions explicitly unless you want to mark the heap blocks as `_CLIENT_BLOCK`. For more information on block types, see [Types of Blocks on the Debug Heap](../VS_csharp/crt-debug-heap-details.md#BKMK_Types_of_blocks_on_the_debug_heap).  
...
## Libraries  
 All debug versions of the [C run-time libraries](../VS_csharp/crt-library-features.md).  
...
## See Also  
 ...
 [_strdup, _wcsdup, _mbsdup](../VS_csharp/_strdup--_wcsdup--_mbsdup.md)   
