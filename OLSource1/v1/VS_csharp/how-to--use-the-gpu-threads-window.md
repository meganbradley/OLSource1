...
In the GPU Threads window, you can examine and work with threads that are running on the GPU in the application that you are debugging. For more information about applications that run on the GPU, see [Overview of C++ Accelerated Massive Parallelism (C++ AMP)](../VS_csharp/c---amp-overview.md).  
