...
This topic shows how to convert .NET collections to their equivalent STL/CLR containers. As an example we show how to convert a .NET <xref:System.Collections.Generic.List`1*> to a STL/CLR [vector](../VS_csharp/vector--stl-clr-.md) and how to convert a .NET <xref:System.Collections.Generic.Dictionary`2*> to a STL/CLR [map](../VS_csharp/map--stl-clr-.md), but the procedure is similar for all collections and containers.  
...
1.  Create a generic STL/CLR container by creating a [collection_adapter](../VS_csharp/collection_adapter--stl-clr-.md) object. This template class takes a .NET collection interface as an argument. To verify which interfaces are supported, see [collection_adapter (STL/CLR)](../VS_csharp/collection_adapter--stl-clr-.md).  
  
2.  Copy the contents of the .NET collection to the container. This can be done by using a STL/CLR [algorithm](../VS_csharp/algorithm--stl-clr-.md), or by iterating over the .NET collection and inserting a copy of each element into the STL/CLR container.  
...
## See Also  
 ...
 [adapter (STL/CLR)](../VS_csharp/adapter--stl-clr-.md)   
