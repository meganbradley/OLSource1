...
-   The [__declspec(dllexport)](../VS_csharp/dllexport--dllimport.md) keyword in the source code  
  
-   [EXPORTS](../VS_csharp/exports.md) statement in a .def file  
  
-   An [/EXPORT](../VS_csharp/-export--exports-a-function-.md) specification in a LINK command  
  
 LINK ignores /IMPLIB when an import library is not being created. If no exports are specified, LINK does not create an import library. If an export file is used in the build, LINK assumes that an import library already exists and does not create one. For information on import libraries and export files, see [LIB Reference](../VS_csharp/lib-reference.md).  
