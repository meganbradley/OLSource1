...
## General Rules for Indentation  
 When indentation is required, you must use spaces, not tabs. At least one space is required. Your organization can create coding standards to specify the number of spaces to use for indentation; three or four spaces of indentation at each level where indentation occurs is typical. You can configure [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] to match your organization's indentation standards by changing the options in the **Options** dialog box, which is available from the **Tools** menu. In the **Text Editor** node, expand **F#** and then click **Tabs**. For a description of the available options, see [Tabs, All Languages, Text Editor, Options Dialog Box](../VS_csharp/options--text-editor--all-languages--tabs.md).  
...
-   The first character in the keywords `let`, `if`, and `module` introduce offside lines.  
 ...
 [!code[FsCodeFormatting#1](../VS_csharp/codesnippet/FSharp/code-formatting-guidelines--fsharp-_1.fs)]  
 ...
 [!code[FsCodeFormatting#2](../VS_csharp/codesnippet/FSharp/code-formatting-guidelines--fsharp-_2.fs)]  
...
## Indentation in Modules  
 ...
 [!code[FsCodeFormatting#3](../VS_csharp/codesnippet/FSharp/code-formatting-guidelines--fsharp-_3.fs)]  
  
 [!code[FsCodeFormatting#4](../VS_csharp/codesnippet/FSharp/code-formatting-guidelines--fsharp-_4.fs)]  
  
 For more information, see [Modules (F#)](../VS_csharp/modules--fsharp-.md).  
...
## Exceptions to the Basic Indentation Rules  
 ...
 [!code[FsCodeFormatting#5](../VS_csharp/codesnippet/FSharp/code-formatting-guidelines--fsharp-_5.fs)]  
 ...
 [!code[FsCodeFormatting#6](../VS_csharp/codesnippet/FSharp/code-formatting-guidelines--fsharp-_6.fs)]  
  
 [!code[FsCodeFormatting#7](../VS_csharp/codesnippet/FSharp/code-formatting-guidelines--fsharp-_7.fs)]  
 ...
 [!code[FsCodeFormatting#8](../VS_csharp/codesnippet/FSharp/code-formatting-guidelines--fsharp-_8.fs)]  
 ...
 [!code[FsCodeFormatting#9](../VS_csharp/codesnippet/FSharp/code-formatting-guidelines--fsharp-_9.fs)]  
...
-   Bodies of anonymous functions. In the following code, the print expression starts at a column position that is farther to the left than the `fun` keyword. However, the line must not start at a column to the left of the start of the previous indentation level (that is, to the left of the `L` in `List`).  
     ...
     [!code[FsCodeFormatting#10](../VS_csharp/codesnippet/FSharp/code-formatting-guidelines--fsharp-_10.fs)]  
...
-   Bodies of modules, classes, interfaces, and structures delimited by `begin...end`, `{...}`, `class...end`, or `interface...end`. This allows for a style in which the opening keyword of a type definition can be on the same line as the type name without forcing the whole body to be indented farther than the opening keyword.  
     ...
     [!code[FsCodeFormatting#13](../VS_csharp/codesnippet/FSharp/code-formatting-guidelines--fsharp-_11.fs)]  
