...
-   Indirectly construct it using a document template.  
 ...
 Before you call either **Create** or `LoadFrame`, you must construct the frame-window object on the heap using the C++ **new** operator. Before calling **Create**, you can also register a window class with the [AfxRegisterWndClass](../VS_csharp/afxregisterwndclass.md) global function to set the icon and class styles for the frame.  
...
-   If the frame window is the main application window, the frame window is the context for running WinHelp. Closing the frame window will shut down WINHELP.EXE if it was launched for help for this application.  
 ...
 For more information on `CFrameWnd`, see [Frame Windows](../VS_csharp/frame-windows.md).  
...
### Parameters  
 `nCmdShow`  
 Specifies the parameter to pass to [CWnd::ShowWindow](../VS_csharp/cwnd-class.md#cwnd__showwindow). By default, the frame is shown and correctly restored.  
...
### Example  
 [!code[NVC_MFCWindowing#1](../VS_csharp/codesnippet/CPP/cframewnd-class_1.cpp)]  
...
### Parameters  
 ...
 `dwStyle`  
 Specifies the window [style](../VS_csharp/window-styles.md) attributes. Include the **FWS_ADDTOTITLE** style if you want the title bar to automatically display the name of the document represented in the window.  
 ...
 *lpszMenuName*  
 ...
 `dwExStyle`  
 Specifies the window extended [style](../VS_csharp/extended-window-styles.md) attributes.  
  
 `pContext`  
 Specifies a pointer to a [CCreateContext](../VS_csharp/ccreatecontext-structure.md) structure. This parameter can be **NULL**.  
...
### Remarks  
 The control bar will be docked to one of the sides of the frame window specified in the calls to both [CControlBar::EnableDocking](../VS_csharp/ccontrolbar-class.md#ccontrolbar__enabledocking) and [CFrameWnd::EnableDocking](#cframewnd__enabledocking). The side chosen is determined by `nDockBarID`.  
...
### Example  
  See the example for [CToolBar::Create](../VS_csharp/ctoolbar-class.md#ctoolbar__create).  
...
### Return Value  
 A pointer to the current [CDocument](../VS_csharp/cdocument-class.md). If there is no current document, returns **NULL**.  
...
### Return Value  
 A pointer to the current [CView](../VS_csharp/cview-class.md). If there is no current view, returns **NULL**.  
...
### Remarks  
 ...
 [!code[NVC_MFCWindowing#2](../VS_csharp/codesnippet/CPP/cframewnd-class_2.cpp)]  
...
### Remarks  
 The `nID` parameter refers to the unique identifier passed to the **Create** method of the control bar. For more information on control bars, refer to the topic entitled [Control Bars](../VS_csharp/control-bars.md).  
...
### Remarks  
 If a runtime error occurs, this method asserts in Debug mode and raises an exception derived from the [CException](../VS_csharp/cexception-class.md) class.  
...
### Return Value  
 A [CString](../VS_csharp/cstringt-class.md) object containing the current title of the window object.  
...
### Remarks  
 ...
 The settings you want to restore must be written to the registry before you call `LoadBarState`. Write the information to the registry by calling [CWinApp::SetRegistryKey](../VS_csharp/cwinapp-class.md#cwinapp__setregistrykey). Write the information to the INI file by calling [SaveBarState](#cframewnd__savebarstate).  
 ...
 *dwDefaultStyle*  
 The frame's [style](../VS_csharp/window-styles.md). Include the **FWS_ADDTOTITLE** style if you want the title bar to automatically display the name of the document represented in the window.  
  
 `pParentWnd`  
 A pointer to the frame's parent.  
  
 `pContext`  
 A pointer to a [CCreateContext](../VS_csharp/ccreatecontext-structure.md) structure. This parameter can be **NULL**.  
...
### Example  
 [!code[NVC_MFCWindowing#3](../VS_csharp/codesnippet/CPP/cframewnd-class_3.cpp)]  
...
-   **borderSet** = 3  
 ...
 `lpRectBorder`  
 Pointer to a [RECT](../VS_csharp/rect-structure.md) structure or a [CRect](../VS_csharp/crect-class.md) object that specifies the coordinates of the border.  
...
### Remarks  
 ...
 [!code[NVC_MFCDocViewSDI#16](../VS_csharp/codesnippet/CPP/cframewnd-class_4.cpp)]  
...
### Parameters  
 `lpcs`  
 A pointer to a Windows [CREATESTRUCT](../VS_csharp/createstruct-structure.md) structure                                *.*  
  
 `pContext`  
 A pointer to a [CCreateContext](../VS_csharp/ccreatecontext-structure.md) structure                                *.*  
...
### Remarks  
 ...
 Override this function to override values passed in the `CCreateContext` object or to change the way controls in the main client area of the frame window are created. The `CCreateContext` members you can override are described in the [CCreateContext](../VS_csharp/ccreatecontext-structure.md) class.  
  
> [!NOTE]
>  Do not replace values passed in the `CREATESTRUCT` structure. They are for informational use only. If you want to override the initial window rectangle, for example, override the `CWnd` member function [PreCreateWindow](../VS_csharp/cwnd-class.md#cwnd__precreatewindow).  
...
### Parameters  
 `pCmdUI`  
 A pointer to a [CCmdUI](../VS_csharp/ccmdui-class.md) object representing the menu that generated the update command. The update handler calls the [Enable](../VS_csharp/ccmdui-class.md#ccmdui__enable) member function of the `CCmdUI` object through `pCmdUI` to update the user interface.  
...
### Parameters  
 *pViewNew*  
 Specifies a pointer to a [CView](../VS_csharp/cview-class.md) object, or **NULL** for no active view.  
...
### Remarks  
 If the value of the `nStyle` parameter is not valid, this method asserts in Debug mode and raises [CInvalidArgException](../VS_csharp/cinvalidargexception-class.md) in Release mode. In case of other runtime errors, this method asserts in Debug mode and raises an exception derived from the [CException](../VS_csharp/cexception-class.md) class.  
  
 This method affects the state of menus in applications written for [!INCLUDE[windowsver](../VS_csharp/includes/windowsver_md.md)] and later.  
...
## See Also  
 ...
 [CRuntimeClass](../VS_csharp/cruntimeclass-structure.md)