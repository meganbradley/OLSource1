...
## Remarks  
 ...
 Key framework classes derived from `CCmdTarget` include [CView](../VS_csharp/cview-class.md), [CWinApp](../VS_csharp/cwinapp-class.md), [CDocument](../VS_csharp/cdocument-class.md), [CWnd](../VS_csharp/cwnd-class.md), and [CFrameWnd](../VS_csharp/cframewnd-class.md). If you intend for a new class to handle messages, derive the class from one of these `CCmdTarget`-derived classes. You will rarely derive a class from `CCmdTarget` directly.  
  
 For an overview of command targets and `OnCmdMsg` routing, see [Command Targets](../VS_csharp/command-targets.md), [Command Routing](../VS_csharp/command-routing.md), and [Mapping Messages](../VS_csharp/mapping-messages.md).  
...
### Example  
 [!code[NVC_MFCDocView#43](../VS_csharp/codesnippet/CPP/ccmdtarget-class_1.cpp)]  
...
### Remarks  
 This function is typically called from the constructor of your object and should only be called if a dispatch map has been declared for the class. For more information on automation see the articles [Automation Clients](../VS_csharp/automation-clients.md) and [Automation Servers](../VS_csharp/automation-servers.md).  
...
### Remarks  
 Derived classes should override this member function (if not overridden, `GetDispatchIID` returns FALSE). See [COleControl](../VS_csharp/colecontrol-class.md).  
...
### Remarks  
 ...
 Derived classes should override this function to return the number of type information interfaces provided (either 0 or 1). If not overridden, **GetTypeInfoCount** returns 0. To override, use the [IMPLEMENT_OLETYPELIB](../VS_csharp/implement_oletypelib.md) macro, which also implements `GetTypeLib` and `GetTypeLibCache`.  
...
### Remarks  
 Derived classes should override this member function (if not overridden, `GetTypeLib` returns TYPE_E_CANTLOADLIBRARY). Use the [IMPLEMENT_OLETYPELIB](../VS_csharp/implement_oletypelib.md) macro, which also implements `GetTypeInfoCount` and `GetTypeLibCache`.  
...
### Remarks  
 Derived classes should override this member function (if not overridden, **GetTypeLibCache** returns NULL). Use the [IMPLEMENT_OLETYPELIB](../VS_csharp/implement_oletypelib.md) macro, which also implements `GetTypeInfoCount` and `GetTypeLib`.  
...
### Remarks  
 ...
 Derived classes can override this function to return appropriate values (if not overridden, `IsInvokeAllowed` returns TRUE). See in particular [COleControl::IsInvokeAllowed](../VS_csharp/colecontrol-class.md#colecontrol__isinvokeallowed).  
...
### Remarks  
 ...
 At run time, `OnCmdMsg` dispatches a command to other objects or handles the command itself by calling the root class `CCmdTarget::OnCmdMsg`, which does the actual message-map lookup. For a complete description of the default command routing, see [Message Handling and Mapping Topics](../VS_csharp/message-handling-and-mapping.md).  
  
 On rare occasions, you may want to override this member function to extend the framework's standard command routing. Refer to [Technical Note 21](../VS_csharp/tn021--command-and-message-routing.md) for advanced details of the command-routing architecture.  
...
|-------------------|--------------------|  
|CN_COMMAND|[CCmdUI](../VS_csharp/ccmdui-class.md)*|  
|CN_EVENT|AFX_EVENT*|  
|CN_UPDATE_COMMAND_UI|CCmdUI*|  
|CN_OLECOMMAND|[COleCmdUI](../VS_csharp/colecmdui-class.md)*|  
|CN_OLE_UNREGISTER|NULL|  
  
### Example  
 [!code[NVC_MFCDocView#44](../VS_csharp/codesnippet/CPP/ccmdtarget-class_2.cpp)]  
  
 [!code[NVC_MFCDocView#45](../VS_csharp/codesnippet/CPP/ccmdtarget-class_3.cpp)]  
...
## See Also  
 [MFC Sample ACDUAL](../VS_csharp/visual-c---samples.md)   
