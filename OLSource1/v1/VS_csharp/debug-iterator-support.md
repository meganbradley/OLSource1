...
---
# Debug Iterator Support
The Visual C++ run-time library detects incorrect iterator use, and asserts and displays a dialog box at run time. To enable debug iterator support, you must use a debug version of a C run-time library to compile your program. For more information, see [C Run-Time Libraries](../VS_csharp/crt-library-features.md). For information about how to use iterators, see [Checked Iterators](../VS_csharp/checked-iterators.md).  
...
-   Increasing the size of a [vector](../VS_csharp/-vector-.md) (push or insert) causes iterators into the `vector` to become invalid.  
...
## Example  
 You can use the symbol [_HAS_ITERATOR_DEBUGGING](../VS_csharp/_has_iterator_debugging.md) to turn off the iterator debugging feature in a debug build. The following program does not assert, but still triggers undefined behavior.  
  
> [!IMPORTANT]
>  Use `_ITERATOR_DEBUG_LEVEL` to control `_HAS_ITERATOR_DEBUGGING`. For more information, see [_ITERATOR_DEBUG_LEVEL](../VS_csharp/_iterator_debug_level.md).  
