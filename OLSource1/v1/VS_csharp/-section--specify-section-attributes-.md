...
|P|Pageable|Marks the section as not pageable|  
 ...
 K and P are peculiar in that the section flags that correspond to them are in the negative sense. If you specify one of them on the .text section (/SECTION:.text,K), there will be no difference in the section flags when you run [DUMPBIN](../VS_csharp/dumpbin-options.md) with the [/HEADERS](../VS_csharp/-headers.md) option; it was already implicitly cached. To remove the default, specify /SECTION:.text,!K and DUMPBIN will reveal section characteristics, including "Not Cached."  
  
 A section in the PE file that does not have E, R, or W set is probably invalid.  
  
 The ALIGN*=#* lets you specify an alignment value for a particular section. See [/ALIGN](../VS_csharp/-align--section-alignment-.md) for more information.  
