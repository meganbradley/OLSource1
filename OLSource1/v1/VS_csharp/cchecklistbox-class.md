...
### Example  
 [!code[NVC_MFCControlLadenDialog#60](../VS_csharp/codesnippet/CPP/cchecklistbox-class_1.cpp)]  
...
### Parameters  
 `dwStyle`  
 Specifies the style of the checklist box. The style must be **LBS_HASSTRINGS** and either **LBS_OWNERDRAWFIXED** (all items in the list are the same height) or **LBS_OWNERDRAWVARIABLE** (items in the list are of varying heights). This style can be combined with other [list-box styles](../VS_csharp/list-box-styles.md) except **LBS_USETABSTOPS**.  
  
 `rect`  
 Specifies the checklist-box size and position. Can be either a [CRect](../VS_csharp/crect-class.md) object or a [RECT](../VS_csharp/rect-structure.md) structure.  
...
### Remarks  
 ...
 When **Create** executes, Windows sends the [WM_NCCREATE](../VS_csharp/cwnd-class.md#cwnd__onnccreate), [WM_CREATE](../VS_csharp/cwnd-class.md#cwnd__oncreate), [WM_NCCALCSIZE](../VS_csharp/cwnd-class.md#cwnd__onnccalcsize), and [WM_GETMINMAXINFO](../VS_csharp/cwnd-class.md#cwnd__ongetminmaxinfo) messages to the checklist-box control.  
 ...
 Apply the following [window styles](../VS_csharp/window-styles.md) to a checklist-box control:  
...
### Remarks  
 By default, this member function does nothing. Override this member function and fill in the `MEASUREITEMSTRUCT` structure to inform Windows of the dimensions of checklist-box items. If the checklist box is created with the [LBS_OWNERDRAWVARIABLE](../VS_csharp/list-box-styles.md) style, the framework calls this member function for each item in the list box. Otherwise, this member is called only once.  
...
|**BST_INDETERMINATE**|Set the specified check box state to indeterminate.<br /><br /> This state is only available if the check box style is `BS_AUTO3STATE` or `BS_3STATE`. For more information, see [Button Styles](../VS_csharp/button-styles.md).|  
...
-   **BS_3STATE**  
 ...
 For information on these styles, see [Button Styles](../VS_csharp/button-styles.md).  
  
## See Also  
 [MFC Sample TSTCON](../VS_csharp/visual-c---samples.md)   
 [Base Class](../VS_csharp/clistbox-class.md)   
