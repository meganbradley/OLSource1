...
---
# _CrtSetReportMode
Specifies the destination or destinations for a specific report type generated by `_CrtDbgReport` and any macros that call [_CrtDbgReport, _CrtDbgReportW](../VS_csharp/_crtdbgreport--_crtdbgreportw.md), such as [_ASSERT, _ASSERTE, _ASSERT_EXPR Macros](../VS_csharp/_assert--_asserte--_assert_expr-macros.md), [_ASSERT, _ASSERTE, _ASSERT_EXPR Macros](../VS_csharp/_assert--_asserte--_assert_expr-macros.md), [_RPT, _RPTF, _RPTW, _RPTFW Macros](../VS_csharp/_rpt--_rptf--_rptw--_rptfw-macros.md), and [_RPT, _RPTF, _RPTW, _RPTFW Macros](../VS_csharp/_rpt--_rptf--_rptw--_rptfw-macros.md) (debug version only).  
...
## Return Value  
 On successful completion, `_CrtSetReportMode` returns the previous report mode or modes for the report type specified in `reportType`. If an invalid value is passed in as `reportType` or an invalid mode is specified for `reportMode`, `_CrtSetReportMode` invokes the invalid parameter handler as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, this function sets `errno` to `EINVAL` and returns -1. For more information, see [errno, _doserrno, _sys_errlist, and _sys_nerr](../VS_csharp/errno--_doserrno--_sys_errlist--and-_sys_nerr.md).  
...
## Remarks  
 ...
 When [_DEBUG](../VS_csharp/_debug.md) is not defined, calls to `_CrtSetReportMode` are removed during preprocessing.  
...
|`_CRTDBG_MODE_FILE`|Writes the message to a user-supplied file handle. [_CrtSetReportFile](../VS_csharp/_crtsetreportfile.md) should be called to define the specific file or stream to use as the destination.|  
