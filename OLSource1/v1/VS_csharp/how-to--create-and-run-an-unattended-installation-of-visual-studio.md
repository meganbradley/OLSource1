...
You can run the installation application for [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] as an unattended (that is, customized silent) installation over an intranet instead of from media such as DVDs. This topic describes how to prepare [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] for this type of installation from a network share.  
  
## Creating a Network Image  
 First, create a network image of the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] media.  
...
3.  Share the IDEinstall folder on the network, and then set the appropriate security settings.  
     ...
     The network path of the installation application for [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] resembles \\\\*ServerName*\IDEinstall\\*Product*.exe.  
  
    > [!NOTE]
    >  Installation may fail if any path and file name combination exceeds 260 characters. The maximum length of a path in [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] is 221 characters.  The local path name should not exceed 70 characters, and the network path name should not exceed 39 characters.  
...
## Deploying Visual Studio in unattended mode  
 To deploy [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] in unattended mode, you must modify the AdminDeployment.xml file. To do this, you must first create the AdminDeployment.xml file by using the `/CreateAdminFile` *<file location\>* command-line parameter. Then, you can use this file to either push a deployment of Visual Studio to your network or pull it into an installation if you put that file in the *Drive*:\IDEinstall\packages directory. The AdminDeployment.xml file is not unique to an operating system, architecture, edition of Visual Studio, or operating-system language.  
...
>   
>  ![Installation items at the end of the AdminDeployment.xml file](../VS_csharp/media/vs2015_install_endoffileadmindeploy.PNG "vs2015_Install_EndOfFileAdminDeploy")  
>   
...
-   Choose [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)], and then choose **Change**.  
...
2.  Click the **Details** tab, and then make note of the **Product version** property.  
     ...
     ![Example of the Properties dialog box in an unattended installation of Visual Studio](../VS_csharp/media/unattended-install---properties-dialog-box.PNG "Unattended Install - Properties Dialog Box")  
...
## Registering the product  
 After installation is complete, you can register your copy of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] from within [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)].  
...
2.  Enter the product key.  
     ...
     (For more information, see the [How to: Locate the Visual Studio Product Key](../VS_csharp/how-to--locate-the-visual-studio-product-key.md) and the [How to: Automatically apply product keys when deploying Visual Studio](../VS_csharp/how-to--automatically-apply-product-keys-when-deploying-visual-studio.md) topics.)  
