...
2.  Pass the `CFile` object to the constructor for `CArchive`, as shown in the following example:  
     ...
     [!code[NVC_MFCSerialization#5](../VS_csharp/codesnippet/CPP/two-ways-to-create-a-carchive-object_1.cpp)]  
 ...
 When you are finished storing or loading data to or from the `CArchive` object, close it. Although the `CArchive` (and `CFile`) objects will automatically close the archive (and file), it is good practice to explicitly do so since it makes recovery from errors easier. For more information about error handling, see the article [Exceptions: Catching and Deleting Exceptions](../VS_csharp/exceptions--catching-and-deleting-exceptions.md).  
  
#### To close the CArchive object  
  
1.  The following example illustrates how to close the `CArchive` object:  
  
     [!code[NVC_MFCSerialization#6](../VS_csharp/codesnippet/CPP/two-ways-to-create-a-carchive-object_2.cpp)]  
