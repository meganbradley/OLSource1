...
Use the `condition_variable` class to wait for an event when you have a `mutex` of type `unique_lock<mutex>`. Objects of this type may have better performance than objects of type [condition_variable_any<unique_lock<mutex\>>](../VS_csharp/condition_variable_any-class.md).  
...
### Parameters  
 `Lck`  
 A [unique_lock<mutex\>](../VS_csharp/unique_lock-class.md) object.  
