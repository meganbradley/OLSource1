...
## Example  
 ...
 Consider the following code.  Pinning is not needed because `ppchar` is an interior pointer, and if the garbage collector moves the string it points to, it will also update `ppchar`. Without a [pin_ptr](../VS_csharp/pin_ptr--c---cli-.md), the code will work and not have the potential performance hit caused by pinning.  
