...
---
# Upgrading Projects
Changes to the project model from one version of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] to the next may require that projects and solutions be upgraded so that they can run on the newer version. The [!INCLUDE[vsipsdk](../VS_csharp/includes/vsipsdk_md.md)] provides interfaces that can be used to implement upgrade support in your own projects.  
...
## How Upgrade Works  
 When a solution created in an earlier version of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] is opened in a newer version, the IDE checks the solution file to determine if it needs to be upgraded. If upgrading is required, the **Upgrade Wizard** is automatically launched to walk the user through the upgrade process.  
...
## See Also  
 ...
 [How to: Upgrade Project Items](../Topic/Upgrading%20Project%20Items.md)