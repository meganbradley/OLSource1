 ...
 **(void) fseek(** `stream`**, 0L,** `SEEK_SET` **);**  
 ...
 If stream is a `NULL` pointer, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, this function returns and `errno` is set to `EINVAL`.  
