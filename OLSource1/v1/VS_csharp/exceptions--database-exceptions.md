...
### Error Codes Used for ODBC Exceptions  
 In addition to return codes defined by the framework, which have names of the form **AFX_SQL_ERROR_XXX**, some [CDBExceptions](../VS_csharp/cdbexception-class.md) are based on [ODBC](../VS_csharp/odbc-basics.md) return codes. The return codes for such exceptions have names of the form **SQL_ERROR_XXX**.  
...
### Error Codes Used for DAO Exceptions  
 For DAO exceptions, more information is typically available. You can access error information through three data members of a caught [CDaoException](../VS_csharp/cdaoexception-class.md) object:  
  
-   [m_pErrorInfo](../Topic/CDaoException::m_pErrorInfo.md) contains a pointer to a [CDaoErrorInfo](../VS_csharp/cdaoerrorinfo-structure.md) object that encapsulates error information in DAO's collection of error objects associated with the database.  
...
-   [m_scode](../Topic/CDaoException::m_scode.md) contains an OLE `SCODE` from DAO, if applicable. You'll seldom need to work with this error code, however. Usually more information is available in the other two data members. See the data member for more about `SCODE` values.  
 ...
 Additional information about DAO errors, the DAO Error object type, and the DAO Errors collection is available under class [CDaoException](../VS_csharp/cdaoexception-class.md).  
  
##  <a name="_core_a_database_exception.2d.handling_example"></a> A Database Exception-Handling Example  
 The following example attempts to construct a [CRecordset](../VS_csharp/crecordset-class.md)-derived object on the heap with the **new** operator, and then open the recordset (for an ODBC data source). For a similar example for the DAO classes, see "DAO Exception Example" below.  
  
### ODBC Exception Example  
 The [Open](../Topic/CRecordset::Open.md) member function could throw an exception (of type [CDBException](../VS_csharp/cdbexception-class.md) for the ODBC classes), so this code brackets the **Open** call with a **try** block. The subsequent **catch** block will catch a `CDBException`. You could examine the exception object itself, called `e`, but in this case it is enough to know that the attempt to create a recordset has failed. The **catch** block displays a message box and cleans up by deleting the recordset object.  
  
 [!code[NVC_MFCDatabase#36](../VS_csharp/codesnippet/CPP/exceptions--database-exceptions_1.cpp)]  
...
### DAO Exception Example  
 ...
 [!code[NVC_MFCDatabase#37](../VS_csharp/codesnippet/CPP/exceptions--database-exceptions_2.cpp)]  
 ...
 For a discussion of the error information returned by a `CDaoException` object, see classes [CDaoException](../VS_csharp/cdaoexception-class.md) and [CDaoErrorInfo](../VS_csharp/cdaoerrorinfo-structure.md).  
