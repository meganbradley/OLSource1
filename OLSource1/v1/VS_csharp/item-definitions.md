...
---
# Item Definitions
[!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] 2.0 enables the static declaration of items in project files by using the [ItemGroup](../VS_csharp/itemgroup-element--msbuild-.md) element. However, metadata may be added only at the item level, even if the metadata is identical for all items. Starting in [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] 3.5, a project element named [ItemDefinitionGroup](../VS_csharp/itemdefinitiongroup-element--msbuild-.md) overcomes this limitation. *ItemDefinitionGroup* lets you define a set of item definitions, which add default metadata values to all items in the named item type.  
  
 The *ItemDefinitionGroup* element appears immediately after the [Project](../VS_csharp/project-element--msbuild-.md) element of the project file. Item definitions provide the following functionality:  
...
```  
 ...
 Beginning in [!INCLUDE[vstecmsbuild](../VS_csharp/includes/vstecmsbuild_md.md)] 3.5, ItemGroups can also be self\-referential. For example:  
