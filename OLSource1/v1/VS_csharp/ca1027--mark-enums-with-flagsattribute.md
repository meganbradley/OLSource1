...
## Example  
 In the following example, `DaysEnumNeedsFlags` is an enumeration that meets the requirements for using <xref:System.FlagsAttribute*>, but does not have it. The `ColorEnumShouldNotHaveFlag` enumeration does not have values that are powers of two, but incorrectly specifies <xref:System.FlagsAttribute*>. This violates rule [Do not mark enums with FlagsAttribute](../VS_csharp/ca2217--do-not-mark-enums-with-flagsattribute.md).  
  
 [!code[FxCop.Design.EnumFlags#1](../VS_csharp/codesnippet/CSharp/ca1027--mark-enums-with-flagsattribute_1.cs)]  
