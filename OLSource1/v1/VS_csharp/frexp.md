...
## Return Value  
 `frexp` returns the mantissa. If `x` is 0, the function returns 0 for both the mantissa and the exponent. If `expptr` is `NULL`, the invalid parameter handler is invoked as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, this function sets `errno` to `EINVAL` and returns 0.  
