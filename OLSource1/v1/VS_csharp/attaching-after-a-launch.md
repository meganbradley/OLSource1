...
## Implementation  
 Programmatically, when the session debug manager (SDM) first receives the [IDebugProgram2](../VS_csharp/idebugprogram2.md) object that represents the program to be launched, it calls the [IDebugProgram2::Attach](../VS_csharp/idebugprogram2--attach.md) method, passing it an [IDebugEventCallback2](../VS_csharp/idebugeventcallback2.md) object, which is later used to pass debug events back to the SDM. The `IDebugProgram2::Attach` method then calls the [IDebugProgramNodeAttach2::OnAttach](../VS_csharp/idebugprogramnodeattach2--onattach.md) method. For more information on how the SDM receives the `IDebugProgram2` interface, see [Notifying the Port](../VS_csharp/notifying-the-port.md).  
...
## See Also  
 [Attaching Directly to a Program](../VS_csharp/attaching-directly-to-a-program.md)   
 [Notifying the Port](../VS_csharp/notifying-the-port.md)   
 ...
 [IDebugProgram2::Attach](../VS_csharp/idebugprogram2--attach.md)   
