...
---
# CMemFile Class
The [CFile](../VS_csharp/cfile-class.md)-derived class that supports memory files.  
...
## Remarks  
 ...
 The most common use of `CMemFile` is to create a `CMemFile` object and use it by calling [CFile](../VS_csharp/cfile-class.md) member functions. Note that creating a `CMemFile` automatically opens it: you do not call [CFile::Open](../VS_csharp/cfile-class.md#cfile__open), which is only used for disk files. Because `CMemFile` doesn't use a disk file, the data member `CFile::m_hFile` is not used and has no meaning.  
  
 The `CFile` member functions [Duplicate](../VS_csharp/cfile-class.md#cfile__duplicate), [LockRange](../VS_csharp/cfile-class.md#cfile__lockrange), and [UnlockRange](../VS_csharp/cfile-class.md#cfile__unlockrange) are not implemented for `CMemFile`. If you call these functions on a `CMemFile` object, you will get a [CNotSupportedException](../VS_csharp/cnotsupportedexception-class.md).  
  
 `CMemFile` uses the run-time library functions [malloc](../VS_csharp/malloc.md), [realloc](../VS_csharp/realloc.md), and [free](../VS_csharp/free.md) to allocate, reallocate, and deallocate memory; and the intrinsic [memcpy](../VS_csharp/memcpy--wmemcpy.md) to block copy memory when reading and writing. If you'd like to change this behavior or the behavior when `CMemFile` grows a file, derive your own class from `CMemFile` and override the appropriate functions.  
  
 For more information on `CMemFile`, see the articles [Files in MFC](../VS_csharp/files-in-mfc.md) and [Memory Management (MFC)](../VS_csharp/memory-management.md) and see [File Handling](../VS_csharp/file-handling.md) in the                 *Run-Time Library Reference*.  
...
### Remarks  
 ...
 The default implementation uses the run-time library function [malloc](../VS_csharp/malloc.md) to allocate memory.  
...
### Remarks  
 Note that the file is opened by the constructor and that you should not call [CFile::Open](../VS_csharp/cfile-class.md#cfile__open).  
...
### Example  
 [!code[NVC_MFCFiles#36](../VS_csharp/codesnippet/CPP/cmemfile-class_1.cpp)]  
...
##  <a name="cmemfile__memcpy"></a>  CMemFile::Memcpy  
 This function is called by the `CMemFile` overrides of [CFile::Read](../VS_csharp/cfile-class.md#cfile__read) and [CFile::Write](../VS_csharp/cfile-class.md#cfile__write) to transfer data to and from the memory file.  
