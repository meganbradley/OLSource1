...
|**_DllMainCRTStartup**|A DLL; calls `DllMain`, which must be defined with `__stdcall`, if it exists|  
 ...
 If the [/DLL](../VS_csharp/-dll--build-a-dll-.md) or [/SUBSYSTEM](../VS_csharp/-subsystem--specify-subsystem-.md) option is not specified, the linker selects a subsystem and entry point depending on whether **main** or `WinMain` is defined.  
 ...
 For information on how to define your own DllMain entry point, see [Run-Time Library Behavior](../VS_csharp/run-time-library-behavior.md) .  
