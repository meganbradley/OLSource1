...
### Remarks  
 The member function returns an object of class [valarray](../VS_csharp/valarray-class.md)**<Type\>**, of length [size](#valarray__size), each of whose elements `I` is **func**(( **\*this**)[ `I`]).  
...
### Return Value  
 ...
 The second member operator is the same as the first, but with an [rvalue reference](../VS_csharp/rvalue-reference-declarator----.md).  
...
### Remarks  
 ...
 When compiling with _SECURE_SCL 1, a runtime error will occur if you attempt to access an element outside the bounds of the valarray.  See [Checked Iterators](../VS_csharp/checked-iterators.md) for more information.  
  
### Example  
  See the examples for [slice::slice](../VS_csharp/slice-class.md#slice__slice) and [gslice::gslice](../VS_csharp/gslice-class.md#gslice__gslice) for an example of how to declare and use the operator.  
...
-   For `valarray(const Type* Ptr, Count)`, the element at position `I` is initialized with `Ptr`[ `I`].  
 ...
 The last constructor is the same as the next to last, but with an [rvalue reference](../VS_csharp/rvalue-reference-declarator----.md).  
