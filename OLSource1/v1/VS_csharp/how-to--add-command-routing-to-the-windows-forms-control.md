...
[CWinFormsView](../VS_csharp/cwinformsview-class.md) routes commands and update-command UI messages to the user control to allow it to handle MFC commands (for example, frame menu items and toolbar buttons).  
  
 The user control uses [ICommandTarget::Initialize](../VS_csharp/icommandtarget--initialize.md) to store a reference to the command source object in `m_CmdSrc`, as shown in the following example. To use `ICommandTarget` you must add a reference to mfcmifc80.dll.  
  
 `CWinFormsView` handles several of the common MFC view notifications by forwarding them to the managed user control. These notifications include the [OnInitialUpdate](../VS_csharp/iview--oninitialupdate.md), [OnUpdate](../VS_csharp/iview--onupdate.md) and [OnActivateView](../VS_csharp/iview--onactivateview.md) methods of the [IView Interface](../VS_csharp/iview-interface.md).  
  
 This topic assumes you have previously completed [How to: Create the User Control and Host in a Dialog Box](../VS_csharp/how-to--create-the-user-control-and-host-in-a-dialog-box.md) and [How to: Create the User Control and Host MDI View](../VS_csharp/how-to--create-the-user-control-and-host-mdi-view.md).  
  
### To create the MFC host application  
  
1.  Open Windows Forms Control Library you created in [How to: Create the User Control and Host in a Dialog Box](../VS_csharp/how-to--create-the-user-control-and-host-in-a-dialog-box.md).  
...
7.  Open the MFC application you created in [How to: Create the User Control and Host MDI View](../VS_csharp/how-to--create-the-user-control-and-host-mdi-view.md).  
...
## See Also  
 ...
 [CommandHandler Delegate](../VS_csharp/commandhandler-delegate.md)