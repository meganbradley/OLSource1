...
Converts a sequence of wide characters to a corresponding sequence of multibyte characters. A version of [wcstombs](../VS_csharp/wcstombs--_wcstombs_l.md) with security enhancements as described in [Security Enhancements in the CRT](../VS_csharp/security-features-in-the-crt.md).  
...
#### Parameters  
 ...
 [in] `count`  
 The maximum number of bytes to store in the `mbstr` buffer, not including the terminating null character, or [_TRUNCATE](../VS_csharp/_truncate.md).  
...
|The destination buffer is too small to contain the converted string (unless `count` is `_TRUNCATE`; see Remarks below)|`ERANGE`|  
 ...
 If any of these conditions occurs, the invalid parameter exception is invoked as described in [Parameter Validation](../VS_csharp/parameter-validation.md) . If execution is allowed to continue, the function returns an error code and sets `errno` as indicated in the table.  
...
-   The number of bytes stored in the `mbstr` buffer equals `count`.  
 ...
 If `count` is the special value [_TRUNCATE](../VS_csharp/_truncate.md), then `wcstombs_s` converts as much of the string as will fit into the destination buffer, while still leaving room for a null terminator.  
...
>  Ensure that `wcstr` and `mbstr` do not overlap, and that `count` correctly reflects the number of wide characters to convert.  
 ...
 `wcstombs_s` uses the current locale for any locale-dependent behavior; `_wcstombs_s_l` is identical to `wcstombs` except that it uses the locale passed in instead. For more information, see [Locale](../VS_csharp/locale.md).  
  
 In C++, using these functions is simplified by template overloads; the overloads can infer buffer length automatically (eliminating the need to specify a size argument) and they can automatically replace older, non-secure functions with their newer, secure counterparts. For more information, see [Secure Template Overloads](../VS_csharp/secure-template-overloads.md).  
...
## See Also  
 ...
 [wctomb_s, _wctomb_s_l](../VS_csharp/wctomb_s--_wctomb_s_l.md)   
