...
## Return Value  
 Each of these functions returns a pointer to the stream. A null pointer value indicates an error. If `filename` or `mode` is `NULL` or an empty string, these functions invoke the invalid parameter handler, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, these functions return `NULL` and set `errno` to `EINVAL`.  
...
|`D`|Specifies a file as temporary. It is deleted when the last file pointer is closed.|  
 ...
 If `t` or `b` is not given in `mode`, the translation mode is defined by the default-mode variable `_fmode`. If `t` or `b` is prefixed to the argument, the function fails and returns `NULL`. For a discussion of text and binary modes, see [Text and Binary Mode File I/O](../VS_csharp/text-and-binary-mode-file-i-o.md).  
