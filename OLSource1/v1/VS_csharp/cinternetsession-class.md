...
## Remarks  
 If your Internet connection must be maintained for the duration of an application, you can create a `CInternetSession` member of the class [CWinApp](../VS_csharp/cwinapp-class.md).  
  
 Once you have established an Internet session, you can call [OpenURL](#cinternetsession__openurl). `CInternetSession` then parses the URL for you by calling the global function [AfxParseURL](../VS_csharp/afxparseurl.md). Regardless of its protocol type, `CInternetSession` interprets the URL and manages it for you. It can handle requests for local files identified with the URL resource "file://". `OpenURL` will return a pointer to a [CStdioFile](../VS_csharp/cstdiofile-class.md) object if the name you pass it is a local file.  
...
-   [GetFtpConnection](#cinternetsession__getftpconnection) to open a connection to an FTP service.  
 ...
 For more information about basic Internet programming tasks, see the article [Internet First Steps: WinInet](../VS_csharp/wininet-basics.md). For general information about using the MFC WinInet classes, see the article [Internet Programming with WinInet](../VS_csharp/win32-internet-extensions--wininet-.md).  
  
> [!NOTE]
>  `CInternetSession` will throw an [AfxThrowNotSupportedException](../VS_csharp/afxthrownotsupportedexception.md) for unsupported service types. Only the following service types are currently supported: FTP, HTTP, gopher, and file.  
...
### Parameters  
 `pstrAgent`  
 A pointer to a string that identifies the name of the application or entity calling the Internet functions (for example, "Microsoft Internet Browser"). If `pstrAgent` is **NULL** (the default), the framework calls the global function [AfxGetAppName](../VS_csharp/afxgetappname.md), which returns a null-terminated string containing an application's name. Some protocols use this string to identify your application to the server.  
...
-   `INTERNET_OPEN_TYPE_PROXY` Connect through a CERN proxy.  
 ...
 For information on connecting with different types of proxies, see [Steps in a Typical FTP Client Application](../VS_csharp/steps-in-a-typical-ftp-client-application.md).  
 ...
 **CInternetSession** is the first Internet function called by an application. It initializes internal data structures and prepares for future calls from the application.  
 ...
 If no Internet connection can be opened, `CInternetSession` throws an [AfxThrowInternetException](../VS_csharp/afxthrowinternetexception.md).  
  
### Example  
  See the example for [CFtpFileFind](../VS_csharp/cftpfilefind-class.md).  
...
### Return Value  
 Nonzero if successful; otherwise 0. If the call fails, determine the cause of the failure by examining the thrown [CInternetException](../VS_csharp/cinternetexception-class.md) object.  
...
### Remarks  
 ...
 For more information about asynchronous operations, see the article [Internet First Steps: WinInet](../VS_csharp/wininet-basics.md).  
  
##  <a name="cinternetsession__getcookie"></a>  CInternetSession::GetCookie  
 This member function implements the behavior of the Win32 function                 [InternetGetCookie](http://msdn.microsoft.com/library/windows/desktop/aa384710), as described in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
...
### Parameters  
 ...
 `pstrCookieData`  
 In the first overload, a pointer to a string containing the address of the buffer that receives the cookie data. This value can be **NULL**. In the second overload, a reference to a [CString](../VS_csharp/cstringt-class.md) object to receive the cookie data.  
...
### Return Value  
 A pointer to a [CFtpConnection](../VS_csharp/cftpconnection-class.md) object. If the call fails, determine the cause of the failure by examining the thrown [CInternetException](../VS_csharp/cinternetexception-class.md) object.  
  
### Remarks  
 `GetFtpConnection` connects to an FTP server, and creates and returns a pointer to a **CFTPConnection** object. It does not perform any specific operation on the server. If you intend to read or write to files, for example, you must perform those operations as separate steps. See the classes [CFtpConnection](../VS_csharp/cftpconnection-class.md) and [CFtpFileFind](../VS_csharp/cftpfilefind-class.md) for information about searching for files, opening files, and reading or writing to files. See the article [Internet Programming with WinInet](../VS_csharp/win32-internet-extensions--wininet-.md) for steps in performing common FTP connection tasks.  
...
### Return Value  
 A pointer to a [CGopherConnection](../VS_csharp/cgopherconnection-class.md) object. If the call fails, determine the cause of the failure by examining the thrown [CInternetException](../VS_csharp/cinternetexception-class.md) object.  
  
### Remarks  
 `GetGopherConnection` connects to a gopher server, and creates and returns a pointer to a `CGopherConnection` object. It does not perform any specific operation on the server. If you intend to read or write data, for example, you must perform those operations as separate steps. See the classes [CGopherConnection](../VS_csharp/cgopherconnection-class.md), [CGopherFile](../VS_csharp/cgopherfile-class.md), and [CGopherFileFind](../VS_csharp/cgopherfilefind-class.md) for information about searching for files, opening files, and reading or writing to files. For information about browsing an FTP site, see the member function [OpenURL](#cinternetsession__openurl). See the article [Internet Programming with WinInet](../VS_csharp/win32-internet-extensions--wininet-.md) for steps in performing common gopher connection tasks.  
 ...
 *dwflags*  
 Any combination of the **INTERNET_ FLAG_\*** flags. See the table in the **Remarks** section of [CHttpConnection::OpenRequest](../VS_csharp/chttpconnection-class.md#chttpconnection__openrequest) for a description of `dwFlags` values.  
  
### Return Value  
 A pointer to a [CHttpConnection](../VS_csharp/chttpconnection-class.md) object. If the call fails, determine the cause of the failure by examining the thrown [CInternetException](../VS_csharp/cinternetexception-class.md) object.  
  
### Remarks  
 `GetHttpConnection` connects to an HTTP server, and creates and returns a pointer to a `CHttpConnection` object. It does not perform any specific operation on the server. If you intend to query an HTTP header, for example, you must perform this operation as a separate step. See the classes [CHttpConnection](../VS_csharp/chttpconnection-class.md) and [CHttpFile](../VS_csharp/chttpfile-class.md) for information about operations you can perform by using a connection to an HTTP server. For information about browsing an HTTP site, see the member function [OpenURL](#cinternetsession__openurl). See the article [Internet Programming with WinInet](../VS_csharp/win32-internet-extensions--wininet-.md) for steps in performing common HTTP connection tasks.  
...
|`INTERNET_STATUS_CONNECTING_TO_SERVER`|Connecting to the socket address ( [SOCKADDR](../VS_csharp/sockaddr-structure.md)) pointed to by `lpvStatusInformation`.|  
...
>  Status callbacks need thread-state protection. If you are using MFC in a shared library, add the following line to the beginning of your override:  
 ...
 [!code[NVC_MFCHtmlHttp#8](../VS_csharp/codesnippet/CPP/cinternetsession-class_1.cpp)]  
...
-   **INTERNET_FLAG_PASSIVE** Used for an FTP site. Uses passive FTP semantics. Used with [CInternetConnection](../VS_csharp/cinternetconnection-class.md) of `OpenURL`.  
...
### Remarks  
 ...
 `OpenURL`, which wraps the Win32 function **InternetOpenURL**, allows only downloading, retrieving, and reading the data from an Internet server. `OpenURL` allows no file manipulation on a remote location, so it requires no [CInternetConnection](../VS_csharp/cinternetconnection-class.md) object.  
...
### Remarks  
 This member function implements the behavior of the Win32 message                         [InternetSetCookie](http://msdn.microsoft.com/library/windows/desktop/aa385107), as described in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
...
### Parameters  
 `dwOption`  
 The Internet option to set. See                                 [Option Flags](http://msdn.microsoft.com/library/windows/desktop/aa385328) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)]for a list of the possible options.  
