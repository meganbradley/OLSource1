...
[!INCLUDE[cpp_fp_under_construction](../VS_csharp/includes/cpp_fp_under_construction_md.md)]  
 ...
 A control derived from [CMFCToolBar](../VS_csharp/cmfctoolbar-class.md) that can be inserted into an Outlook bar ( [CMFCOutlookBar](../VS_csharp/cmfcoutlookbar-class.md)). The Outlook bar pane contains a column of large buttons. The user can scroll up and down the list of buttons if it is larger than the pane. When the user detaches an Outlook bar pane from the Outlook bar, it can float or dock in the main frame window.  
...
|[CMFCOutlookBarPane::CanBeAttached](#cmfcoutlookbarpane__canbeattached)|Determines whether the pane can be docked to another pane or frame window. (Overrides [CBasePane::CanBeAttached](../VS_csharp/cbasepane-class.md#cbasepane__canbeattached).)|  
|`CMFCOutlookBarPane::CanBeRestored`|Determines whether the system can restore a toolbar to its original state after customization. (Overrides [CMFCToolBar::CanBeRestored](../VS_csharp/cmfctoolbar-class.md#cmfctoolbar__canberestored).)|  
...
|`CMFCOutlookBarPane::GetThisClass`|Used by the framework to obtain a pointer to the [CRuntimeClass](../VS_csharp/cruntimeclass-structure.md) object that is associated with this class type.|  
...
|`CMFCOutlookBarPane::OnBeforeFloat`|Called by the framework when a pane is about to float. (Overrides [CPane::OnBeforeFloat](../VS_csharp/cpane-class.md#cpane__onbeforefloat).)|  
|[CMFCOutlookBarPane::RemoveButton](#cmfcoutlookbarpane__removebutton)|Removes the button that has a specified command ID.|  
|`CMFCOutlookBarPane::RestoreOriginalstate`|Restores the original state of a toolbar. (Overrides [CMFCToolBar::RestoreOriginalstate](../VS_csharp/cmfctoolbar-class.md#cmfctoolbar__restoreoriginalstate).)|  
...
|[CMFCOutlookBarPane::RemoveAllButtons](#cmfcoutlookbarpane__removeallbuttons)|Removes all the buttons from the Outlook bar pane. (Overrides [CMFCToolBar::RemoveAllButtons](../VS_csharp/cmfctoolbar-class.md#cmfctoolbar__removeallbuttons).)|  
  
## Remarks  
 For information about how to implement an Outlook bar, see [CMFCOutlookBar](../VS_csharp/cmfcoutlookbar-class.md).  
  
 For an example of an Outlook bar, see the OutlookDemo sample project.  
  
## Example  
 The following example demonstrates how to use various methods of the `CMFCOutlookBarPane` class. The example shows how to create an Outlook bar pane, enable the page scroll mode, enable docking, and set the background color of the Outlook bar. This code snippet is part of the [Outlook Multi Views sample](../VS_csharp/visual-c---samples.md).  
  
 [!code[NVC_MFC_OutlookMultiViews#3](../VS_csharp/codesnippet/CPP/cmfcoutlookbarpane-class_1.h)]  
[!code[NVC_MFC_OutlookMultiViews#4](../VS_csharp/codesnippet/CPP/cmfcoutlookbarpane-class_2.cpp)]  
...
### Remarks  
 This method directly calls [CMFCToolBarImages::Clear](../VS_csharp/cmfctoolbarimages-class.md#cmfctoolbarimages__clear), which is called on the images that are used by the Outlook bar pane.  
...
### Parameters  
 ...
 [in] `dwStyle`  
 The window style.  For a list of window styles, see [Window Styles](../VS_csharp/window-styles.md).  
...
### Remarks  
 ...
 For more information about `dwControlBarStyle` see [CBasePane::CreateEx](../VS_csharp/cbasepane-class.md#cbasepane__createex).  
...
### Remarks  
 ...
 The default implementation checks the customization mode ( [CMFCToolBar::IsCustomizeMode](../VS_csharp/cmfctoolbar-class.md#cmfctoolbar__iscustomizemode)) and if it is set to `TRUE`, disables all the shortcut menu items except **Delete**. Then, it just passes the input parameters to `CMFCToolBar::EnableContextMenuItems`.  
...
### Remarks  
 Call this method to set the Outlook bar's background image. The list of background images is managed by the embedded [CMFCToolBarImages](../VS_csharp/cmfctoolbarimages-class.md) object.  
