...
---
# basic_istream Class
Describes an object that controls extraction of elements and encoded objects from a stream buffer with elements of type `Elem`, also known as [char_type](../VS_csharp/basic_ios-class.md#basic_ios__char_type), whose character traits are determined by the class             *Tr*, also known as [traits_type](../VS_csharp/basic_ios-class.md#basic_ios__traits_type).  
...
```  
 ...
 Both groups of functions call [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)( **eofbit**) if they encounter end of file while extracting elements.  
  
 An object of class `basic_istream`< `Elem`,                 *Tr*> stores:  
  
-   A virtual public base object of class [basic_ios](../VS_csharp/basic_ios-class.md)< `Elem`,                         *Tr*> `.`  
  
-   An extraction count for the last unformatted input operation (called **count** in the previous code).  
  
## Example  
 See the example for [basic_ifstream Class](../VS_csharp/basic_ifstream-class.md) to learn more about input streams.  
...
### Parameters  
 `_Strbuf`  
 An object of type [basic_streambuf](../VS_csharp/basic_streambuf-class.md).  
...
### Remarks  
 The first constructor initializes the base class by calling [init](../VS_csharp/basic_ios-class.md#basic_ios__init)(_S `trbuf`). It also stores zero in the extraction count. For more information about this extraction count, see the Remarks section of the [basic_istream Class](../VS_csharp/basic_istream-class.md) overview topic.  
...
### Example  
  See the example for [basic_ifstream::basic_ifstream](../VS_csharp/basic_ifstream-class.md#basic_ifstream__basic_ifstream) to learn more about input streams.  
...
### Remarks  
 The first of these unformatted input functions extracts an element, if possible, as if by returning `rdbuf`-> `sbumpc`. Otherwise, it returns **traits_type::**[eof](../VS_csharp/char_traits-struct.md#char_traits__eof). If the function extracts no element, it calls [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)( **failbit**).  
  
 The second function extracts the [int_type](../VS_csharp/basic_ios-class.md#basic_ios__int_type) element `meta` the same way. If `meta` compares equal to **traits_type::eof**, the function calls `setstate`( **failbit**). Otherwise, it stores **traits_type::**[to_char_type](../VS_csharp/char_traits-struct.md#char_traits__to_char_type)( `meta`) in `_Ch`. The function returns **\*this**.  
...
-   After the function extracts `_Count` - 1 elements.  
 ...
 If the function extracts no elements or `_Count` - 1 elements, it calls [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)( **failbit**). In any case, it returns **\*this**.  
...
### Remarks  
 The `<istream>` header also defines several global extraction operators. For more information, see [operator>> (<istream\>)](../VS_csharp/-istream--operators.md#operator_gt__gt_).  
  
 The first member function ensures that an expression of the form **istr** >> `ws` calls [ws](../VS_csharp/-istream--functions.md#ws)( **istr**), and then returns **\*this**. The second and third functions ensure that other manipulators, such as [hex](../VS_csharp/-ios--functions.md#hex), behave similarly. The remaining functions constitute the formatted input functions.  
...
```  
 ...
 extracts a field and converts it to a Boolean value by calling [use_facet](../VS_csharp/basic_filebuf-class.md#basic_filebuf__open) < `num_get`< **Elem**, **InIt**>( [getloc](../VS_csharp/ios_base-class.md#ios_base__getloc)). [get](../VS_csharp/ios_base-class.md#ios_base__getloc)( **InIt**( [rdbuf](../VS_csharp/basic_ios-class.md#basic_ios__rdbuf)), `Init`(0), **\*this**, `getloc`, `_Val`). Here, **InIt** is defined as [istreambuf_iterator](../VS_csharp/istreambuf_iterator-class.md)< **Elem**, **Tr**>. The function returns **\*this**.  
...
```  
 ...
 If the converted value cannot be represented as the type of `_Val`, the function calls [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)( **failbit**). In any case, the function returns **\*this**.  
...
### Remarks  
 The unformatted input function extracts an element, if possible, as if by returning `rdbuf` -> [sgetc](../VS_csharp/basic_streambuf-class.md#basic_streambuf__sgetc). Otherwise, it returns **traits_type::**[eof](../VS_csharp/char_traits-struct.md#char_traits__eof).  
...
### Remarks  
 The [unformatted input function](../VS_csharp/basic_istream-class.md) puts back `_Ch`, if possible, as if by calling [rdbuf](../VS_csharp/basic_ios-class.md#basic_ios__rdbuf)`->`[sputbackc](../VS_csharp/basic_streambuf-class.md#basic_streambuf__sputbackc). If rdbuf is a null pointer, or if the call to `sputbackc` returns **traits_type::**[eof](../VS_csharp/char_traits-struct.md#char_traits__eof), the function calls [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)( **badbit**). In any case, it returns **\*this**.  
...
### Remarks  
 The unformatted input function extracts up to `count` elements and stores them in the array beginning at _ `Str`. Extraction stops early on end of file, in which case the function calls [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)( `failbit`). In any case, it returns `*this`.  
...
### Parameters  
 ...
 `way`  
 One of the [ios_base::seekdir](../VS_csharp/ios_base-class.md#ios_base__seekdir) enumerations.  
...
>  Do not use the second member function with text files, because Standard C++ does not support relative seeks in text files.  
 ...
 If [fail](../VS_csharp/basic_ios-class.md#basic_ios__fail) is false, the first member function calls **newpos** = [rdbuf](../VS_csharp/basic_ios-class.md#basic_ios__rdbuf) -> [pubseekpos](../VS_csharp/basic_streambuf-class.md#basic_streambuf__pubseekpos)( `pos`), for some **pos_type** temporary object **newpos**. If **fail** is false, the second function calls **newpos** = **rdbuf** -> [pubseekoff](../VS_csharp/basic_streambuf-class.md#basic_streambuf__pubseekoff)( `off`, `way`). In either case, if ( `off_type`) **newpos** == ( `off_type`)(-1) (the positioning operation fails), the function calls **istr**. [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)( **failbit**). Both functions return **\*this**.  
  
 If [fail](../VS_csharp/basic_ios-class.md#basic_ios__fail) is true, the member functions do nothing.  
...
### Remarks  
 If `_Istr``.`[good](../VS_csharp/basic_ios-class.md#basic_ios__good) is true, the constructor:  
  
-   Calls `_Istr`. [tie](../VS_csharp/basic_ios-class.md#basic_ios__tie) -> [flush](../VS_csharp/basic_ostream-class.md#basic_ostream__flush) if `_Istr`. `tie` is not a null pointer  
  
-   Effectively calls [ws](../VS_csharp/-istream--functions.md#ws)( `_Istr`) if `_Istr`. [flags](../VS_csharp/ios_base-class.md#ios_base__flags)**&**[skipws](../VS_csharp/-ios--functions.md#skipws) is nonzero  
  
 If, after any such preparation, `_Istr`. **good** is false, the constructor calls `_Istr`. [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)( **failbit**). In any case, the constructor stores the value returned by `_Istr`. **good** in **status**. A later call to **operator bool** delivers this stored value.  
...
### Remarks  
 The member function calls [swap](../VS_csharp/basic_ios-class.md#basic_ios__swap)`(``_Right``)`. It also exchanges the extraction count with the extraction count for `_Right`.  
...
### Return Value  
 If [rdbuf](../VS_csharp/basic_ios-class.md#basic_ios__rdbuf) is a null pointer, the function returns -1. Otherwise, it calls `rdbuf` -> [pubsync](../VS_csharp/basic_streambuf-class.md#basic_streambuf__pubsync). If that returns -1, the function calls [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)( **badbit**) and returns -1. Otherwise, the function returns zero.  
...
### Remarks  
 If [fail](../VS_csharp/basic_ios-class.md#basic_ios__fail) is false, the member function returns [rdbuf](../VS_csharp/basic_ios-class.md#basic_ios__rdbuf) -> [pubseekoff](../VS_csharp/basic_streambuf-class.md#basic_streambuf__pubseekoff)(0, `cur`, **in**). Otherwise, it returns `pos_type`(-1).  
...
### Remarks  
 The [unformatted input function](../VS_csharp/basic_istream-class.md) puts back the previous element in the stream, if possible, as if by calling `rdbuf` -> [sungetc](../VS_csharp/basic_streambuf-class.md#basic_streambuf__sungetc). If [rdbuf](../VS_csharp/basic_ios-class.md#basic_ios__rdbuf) is a null pointer, or if the call to `sungetc` returns **traits_type::**[eof](../VS_csharp/basic_ios-class.md#basic_ios__eof), the function calls [setstate](../VS_csharp/basic_ios-class.md#basic_ios__setstate)( **badbit**). In any case, it returns **\*this**.  
  
 For information on how `unget` might fail, see [basic_streambuf::sungetc](../VS_csharp/basic_streambuf-class.md#basic_streambuf__sungetc).  
