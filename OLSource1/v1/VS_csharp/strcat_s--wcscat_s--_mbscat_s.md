...
Appends a string. These versions of [strcat, wcscat, _mbscat](../VS_csharp/strcat--wcscat--_mbscat.md) have security enhancements, as described in [Security Enhancements in the CRT](../VS_csharp/security-features-in-the-crt.md).  
...
```  
 ...
 If `strDestination` is a null pointer, or is not null-terminated, or if `strSource` is a `NULL` pointer, or if the destination string is too small, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, these functions return `EINVAL` and set `errno` to `EINVAL`.  
...
## Example  
 See the code example in [strcpy_s, wcscpy_s, _mbscpy_s](../VS_csharp/strcpy_s--wcscpy_s--_mbscpy_s.md).  
