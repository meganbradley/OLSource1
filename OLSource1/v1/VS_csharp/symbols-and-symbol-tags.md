...
Debug information about a compiled program is stored in the program database (.pdb) file as symbols that are accessible using the Debug Interface Access (DIA) SDK APIs. All symbols have a [IDiaSymbol::get_symTag](../VS_csharp/idiasymbol--get_symtag.md) and a [IDiaSymbol::get_symIndexId](../VS_csharp/idiasymbol--get_symindexid.md) property. The `symTag` property indicates the kind of symbol as defined by the [SymTagEnum Enumeration](../VS_csharp/symtagenum.md) enumeration. The `symIndexId` property is a `DWORD` value that contains the unique identifier for every instance of a symbol.  
  
 Symbols also have properties that can specify additional information about the symbol as well as references to other symbols, most often a [IDiaSymbol::get_lexicalParent](../VS_csharp/idiasymbol--get_lexicalparent.md) or [IDiaSymbol::get_classParent](../VS_csharp/idiasymbol--get_classparent.md). When you query a property that contains a reference, the reference is returned as an [IDiaSymbol](../VS_csharp/idiasymbol.md) object. Such properties are always paired with another property by the same name but suffixed with "Id", for example, [IDiaSymbol::get_lexicalParentId](../VS_csharp/idiasymbol--get_lexicalparentid.md) and [IDiaSymbol::get_classParentId](../VS_csharp/idiasymbol--get_classparentid.md). The tables in [Symbol Locations](../VS_csharp/symbol-locations.md), [Lexical Hierarchy of Symbol Types](../VS_csharp/lexical-hierarchy-of-symbol-types.md), and [Class Hierarchy of Symbol Types](../VS_csharp/class-hierarchy-of-symbol-types.md) outline the properties for each of the different kinds of symbols. These properties may have relevant information about or references to other symbols. Because the `*Id` properties are simply numeric ordinal identifiers of their related properties, they are omitted from further discussions. They are referred to only where needed for parameter clarification.  
...
## See Also  
 [Debug Interface Access SDK](../VS_csharp/debug-interface-access-sdk.md)