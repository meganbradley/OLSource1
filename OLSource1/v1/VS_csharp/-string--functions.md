...
### Remarks  
 ...
 The second function of each pair is an analog to the first one to support [rvalue references](../VS_csharp/lvalues-and-rvalues--visual-c---.md).  
...
-   After the function extracts `str.`[max_size](../VS_csharp/basic_string-class.md#basic_string__max_size) elements, in which case the internal state flag of `is` is set to `ios_base::failbit`.  
...
-   Some other error other than those previously listed, in which case the internal state flag of `is` is set to `ios_base::badbit`  
 ...
 For information about internal state flags, see [ios_base::iostate](../VS_csharp/ios_base-class.md#ios_base__iostate).  
...
### Remarks  
 The template function executes the specialized member function _                        *Left*. [swap](../VS_csharp/basic_string-class.md#basic_string__swap)(\_                        *Right*) for strings, which guarantees constant complexity.  
