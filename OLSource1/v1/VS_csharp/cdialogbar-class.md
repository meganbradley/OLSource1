...
## Remarks  
 ...
 Creating and using a dialog bar is similar to creating and using a `CFormView` object. First, use the [dialog editor](../VS_csharp/dialog-editor.md) to define a dialog template with the style **WS_CHILD** and no other style. The template must not have the style **WS_VISIBLE**. In your application code, call the constructor to construct the `CDialogBar` object, then call **Create** to create the dialog-bar window and attach it to the `CDialogBar` object.  
  
 For more information on `CDialogBar`, see the article [Dialog Bars](../VS_csharp/dialog-bars.md) and [Technical Note 31](../VS_csharp/tn031--control-bars.md), Control Bars.  
  
> [!NOTE]
>  In the current release, a `CDialogBar` object cannot host Windows Forms controls. For more information about Windows Forms controls in Visual C++, see [Using Windows Forms in MFC](../VS_csharp/using-a-windows-form-user-control-in-mfc.md).  
...
### Example  
 [!code[NVC_MFCMessageMaps#13](../VS_csharp/codesnippet/CPP/cdialogbar-class_1.cpp)]  
