...
## Remarks  
 Use the `marshal_context` class for data conversions that require a context. See [Marshaling Overview in C++](../VS_csharp/overview-of-marshaling-in-c--.md) for more information about which conversions require a context and which marshaling file has to be included. The result of marshaling when you use a context is valid only until the `marshal_context` object is destroyed. To preserve your result, you must copy the data.  
