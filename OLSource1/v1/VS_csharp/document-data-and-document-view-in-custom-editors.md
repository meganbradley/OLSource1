...
## Document Data Object  
 ...
 <xref:EnvDTE80.Window2.DocumentData*> and [Document Windows](../VS_csharp/document-windows.md).  
...
## Document View Object  
 ...
 If your editor supports multiple views of a document data object, then your document data and document view objects must be separate. Otherwise, they can be grouped together. For more information, see [Supporting Multiple Document Views](../VS_csharp/supporting-multiple-document-views.md).  
  
 The IDE notifies views about events (for example, when a solution containing a document is closed) by matching an item identifier (ItemID) for each entry in the running document table. For more information on this, see [Running Document Table](../VS_csharp/running-document-table.md).  
  
 There are two options for creating a view for a custom editor. One is the in-place activation model, where the view is hosted in a window using either an ActiveX control or a document data object. The second is the simplified embedding model, where the view is hosted by [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] and <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowPane*> is implemented to handle window commands. For information about the in-place activation model, see [In-Place Activation Editors](../Topic/In-Place%20Activation.md). For information about the simplified embedding model, see [Simplified Embedding Editors](../VS_csharp/simplified-embedding.md).  
  
## See Also  
 [Supporting Multiple Document Views](../VS_csharp/supporting-multiple-document-views.md)   
 [Simplified Embedding](../VS_csharp/simplified-embedding.md)   
 [How to: Attach Views to Document Data Objects](../VS_csharp/how-to--attach-views-to-document-data.md)   
 [Document Lock Holder Management](../VS_csharp/document-lock-holder-management.md)   
 [Single and Multi-tab Views](../VS_csharp/single-and-multi-tab-views.md)   
 [Saving a Standard Document](../VS_csharp/saving-a-standard-document.md)   
 [Persistence and the Running Document Table](../VS_csharp/persistence-and-the-running-document-table.md)   
 [Determining Which Editor Opens a File in a Project](../VS_csharp/determining-which-editor-opens-a-file-in-a-project.md)   
 [Editor Factories](../VS_csharp/editor-factories.md)