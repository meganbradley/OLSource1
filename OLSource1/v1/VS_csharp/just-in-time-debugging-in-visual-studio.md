...
## Using Just-In-Time Debugging  
 ...
 In Visual Studio, create a C# console app (**File / New / Project / Visual C# / Console Application**) named **ThrowsNullException**. For more information about creating projects in Visual Studio, see [Walkthrough: Create a Simple Application with Visual C# or Visual Basic](../VS_csharp/walkthrough--create-a-simple-application-with-visual-csharp-or-visual-basic.md).  
...
> [!IMPORTANT]
>  In order for this procedure to work in a [release configuration](../VS_csharp/how-to--set-debug-and-release-configurations.md), you need to turn off [Just My Code](../VS_csharp/just-my-code.md). In Visual Studio, click **Tools / Options**. In the **Options** dialog, select **Debugging**. Remove the check from **Enable Just My Code**.  
  
 Build the solution (in Visual Studio, choose **Build / Rebuild Solution**). You can choose either the Debug or the Release configuration. For more information about build configurations, see [Understanding Build Configurations](../VS_csharp/understanding-build-configurations.md).  
 ...
 ![ThrowsNullExceptionConsole](../VS_csharp/media/throwsnullexceptionconsole.png "ThrowsNullExceptionConsole")  
  
 After a few seconds, an error window appears:  
  
 ![ThrowsNullExceptionError](../VS_csharp/media/throwsnullexceptionerror.png "ThrowsNullExceptionError")  
...
>  If your application contains untrusted code, a dialog box with a security warning appears. This dialog box enables you to decide whether or not to proceed with debugging. Before you continue with debugging, decide whether you trust the code. Did you write the code yourself? Do you trust the coder? If the application is running on a remote machine, do you recognize the name of the process? Even if the application is running locally, that does not necessarily mean it can be trusted. Consider the possibility of malicious code running on your computer. If you decide that the code you are about to debug is trustworthy, click **Debug**. Otherwise, click **Don't Debug**.  
 ...
 ![JustInTimeDialog](../VS_csharp/media/justintimedialog.png "JustInTimeDialog")  
 ...
 ![NullReferenceSecondInstance](../VS_csharp/media/nullreferencesecondinstance.png "NullReferenceSecondInstance")  
...
1.  By default, Windows Forms applications have a top-level exception handler that allows the program to continue to run if it can recover. For example, if your Windows Forms application throws an unhandled exception, you will see a dialog like the following:  
     ...
     ![WindowsFormsUnhandledException](../VS_csharp/media/windowsformsunhandledexception.png "WindowsFormsUnhandledException")  
...
## See Also  
 ...
 [Debugger Basics](../VS_csharp/debugger-basics.md)   
 [Just-In-Time, Debugging, Options Dialog Box](../VS_csharp/just-in-time--debugging--options-dialog-box.md)   
 [Security Warning: Attaching to an Untrusted Process Can Be Dangerous](../VS_csharp/security-warning--attaching-to-a-process-owned-by-an-untrusted-user-can-be-dangerous.-if-the-following-information-looks-suspicious-or-you-are-unsure--do-not-attach-to-this-process.md)