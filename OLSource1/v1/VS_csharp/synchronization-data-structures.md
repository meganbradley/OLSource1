...
##  <a name="critical_section"></a> critical_section  
 The [concurrency::critical_section](../VS_csharp/critical_section-class.md) class represents a cooperative mutual exclusion object that yields to other tasks instead of preempting them. Critical sections are useful when multiple threads require exclusive read and write access to shared data.  
  
 The `critical_section` class is non-reentrant. The [concurrency::critical_section::lock](../VS_csharp/critical_section--lock-method.md) method throws an exception of type [concurrency::improper_lock](../VS_csharp/improper_lock-class.md) if it is called by the thread that already owns the lock.  
...
|Method|Description|  
|------------|-----------------|  
|[lock](../VS_csharp/critical_section--lock-method.md)|Acquires the critical section. The calling context blocks until it acquires the lock.|  
|[try_lock](../VS_csharp/critical_section--try_lock-method.md)|Tries to acquire the critical section, but does not block.|  
|[unlock](../VS_csharp/critical_section--unlock-method.md)|Releases the critical section.|  
  
 [[Top](#top)]  
  
##  <a name="reader_writer_lock"></a> reader_writer_lock  
 The [concurrency::reader_writer_lock](../VS_csharp/reader_writer_lock-class.md) class provides thread-safe read/write operations to shared data. Use reader/writer locks when multiple threads require concurrent read access to a shared resource but rarely write to that shared resource. This class gives only one thread write access to an object at any time.  
 ...
 Like the `critical_section` class, the `reader_writer_lock` class is non-reentrant. The [concurrency::reader_writer_lock::lock](../VS_csharp/reader_writer_lock--lock-method.md) and [concurrency::reader_writer_lock::lock_read](../VS_csharp/reader_writer_lock--lock_read-method.md) methods throw an exception of type `improper_lock` if they are called by a thread that already owns the lock.  
...
|Method|Description|  
|------------|-----------------|  
|[lock](../VS_csharp/reader_writer_lock--lock-method.md)|Acquires read/write access to the lock.|  
|[try_lock](../VS_csharp/reader_writer_lock--try_lock-method.md)|Tries to acquire read/write access to the lock, but does not block.|  
|[lock_read](../VS_csharp/reader_writer_lock--lock_read-method.md)|Acquires read-only access to the lock.|  
|[try_lock_read](../VS_csharp/reader_writer_lock--try_lock_read-method.md)|Tries to acquire read-only access to the lock, but does not block.|  
|[unlock](../VS_csharp/reader_writer_lock--unlock-method.md)|Releases the lock.|  
...
##  <a name="scoped_lock"></a> scoped_lock and scoped_lock_read  
 ...
 The `critical_section` class contains the [concurrency::critical_section::scoped_lock](../VS_csharp/critical_section--scoped_lock-class.md) class. The constructor acquires access to the provided `critical_section` object; the destructor releases access to that object. The `reader_writer_lock` class contains the [concurrency::reader_writer_lock::scoped_lock](../VS_csharp/reader_writer_lock--scoped_lock-class.md) class, which resembles `critical_section::scoped_lock`, except that it manages write access to the provided `reader_writer_lock` object. The `reader_writer_lock` class also contains the [concurrency::reader_writer_lock::scoped_lock_read](../VS_csharp/reader_writer_lock--scoped_lock_read-class.md) class. This class manages read access to the provided `reader_writer_lock` object.  
...
##  <a name="event"></a> event  
 The [concurrency::event](../VS_csharp/event-class.md) class represents a synchronization object whose state can be signaled or non-signaled. Unlike synchronization objects, such as critical sections, whose purpose is to protect access to shared data, events synchronize flow of execution.  
...
|Method|Description|  
|------------|-----------------|  
|[wait](../VS_csharp/event--wait-method.md)|Waits for the event to become signaled.|  
|[set](../VS_csharp/event--set-method.md)|Sets the event to the signaled state.|  
|[reset](../VS_csharp/event--reset-method.md)|Sets the event to the non-signaled state.|  
|[wait_for_multiple](../VS_csharp/event--wait_for_multiple-method.md)|Waits for multiple events to become signaled.|  
  
### Example  
 For an example that shows how to use the `event` class, see [Comparing Synchronization Data Structures to the Windows API](../VS_csharp/comparing-synchronization-data-structures-to-the-windows-api.md).  
