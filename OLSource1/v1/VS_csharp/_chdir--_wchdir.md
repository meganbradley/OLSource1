...
## Return Value  
 These functions return a value of 0 if successful. A return value of –1 indicates failure. If the specified path could not be found, `errno` is set to `ENOENT`. If `dirname` is NULL, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, `errno` is set to `EINVAL` and the function returns -1.  
