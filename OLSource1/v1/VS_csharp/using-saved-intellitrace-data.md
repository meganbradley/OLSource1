...
-   Matching source files and symbol (.pdb) files for your application code. Otherwise, Visual Studio can't resolve the source locations and shows the message "Symbols not found." See [Specify Symbol (.pdb) and Source Files](../VS_csharp/specify-symbol--.pdb--and-source-files-in-the-visual-studio-debugger.md) and [Diagnose problems in deployment](../VS_csharp/diagnose-problems-after-deployment.md).  
...
-   An .iTrace file from one of these sources:  
    ...
    |**Source**|**See**|  
    |----------------|-------------|  
    |An IntelliTrace session in Visual Studio Enterprise (but not Professional or Community editions)|[Record Execution History to Debug Your App with IntelliTrace in Visual Studio](../VS_csharp/intellitrace-features.md)|  
    ...
    |Microsoft Monitoring Agent, either alone or with System Center 2012 R2 Operations Manager, for ASP.NET web apps and SharePoint applications running in deployment|-   [Diagnose problems in deployment](../VS_csharp/diagnose-problems-after-deployment.md)<br />-   [What's New for System Center 2012 R2 Operations Manager](http://technet.microsoft.com/library/dn249700.aspx)|  
...
3.  Expand that call to review any nested calls and parameter values that were recorded at that point in time.  
     ...
     Now you can review other recorded values, the call stack, step through your code, or use the **IntelliTrace** window to [move backwards or forwards "in time" between other methods](../VS_csharp/intellitrace.md) that were called during this performance event.  
...
1.  Under **Exception Data**, review the recorded exception events, their types, messages, and when the exceptions happened. To dig deeper into the code, start debugging from the most recent event in a group of exceptions.  
     ...
     Now you can review other recorded values, the call stack, or use the **IntelliTrace** window to [move backwards or forwards "in time" between other recorded events](../VS_csharp/intellitrace.md), related code, and the values recorded at those points in time.  
...
1.  Copy the SharePoint correlation ID from its source.  
     ...
     ![IntelliTrace &#45; SharePoint error &#45; correlation ID](../VS_csharp/media/sharepointerror_intellitrace.png "SharePointError_IntelliTrace")  
...
2.  Open the .iTrace file, then go to **Analysis** and enter the SharePoint correlation ID to review the matching web request and recorded events.  
     ...
     ![IntelliTrace log &#45; Enter SharePoint correlation ID](../VS_csharp/media/entersharepointcorrelationid.png "EnterSharePointCorrelationID")  
...
3.  Under **Request Events**, examine the events. Starting from the top, events appear in the order that they happened.  
    ...
    2.  Choose **Start Debugging** to start debugging at the point where the event happened.  
     ...
     ![IntelliTrace log file &#45; View web request &#43; events](../VS_csharp/media/entersharepointcorrelationid2.png "EnterSharePointCorrelationID2")  
...
3.  Choose **Debug Exception** to start debugging at the point where and when the exception happened.  
     ...
     ![IntelliTrace log &#45; SharePoint unhandled exceptions](../VS_csharp/media/sharepointunhandledexceptions_intellitrace.png "SharePointUnhandledExceptions_IntelliTrace")  
  
 For a walkthrough, see [Walkthrough: Debug a SharePoint Application Using IntelliTrace](assetId:///4bd80d2f-f680-4bf4-81c3-f14e8185f6a4). For the kinds of data that the agent records, see [Record Execution History to Debug Your App with IntelliTrace in Visual Studio](../VS_csharp/intellitrace-features.md).  
...
2.  At the bottom of **Threads List**, choose **Start Debugging**. You can also double-click a thread.  
     ...
     To start debugging from where the app begins, double-click **Main Thread**. See [Record Execution History to Debug Your App with IntelliTrace in Visual Studio](../VS_csharp/intellitrace-features.md).  
...
### Where can I get more information?  
 [Collect IntelliTrace Data for Applications in Production with Microsoft Monitoring Agent](../VS_csharp/using-the-intellitrace-stand-alone-collector.md)  
  
 [Record Execution History to Debug Your App with IntelliTrace in Visual Studio](../VS_csharp/intellitrace-features.md)  
  
 [Collect more diagnostic data in manual tests](assetId:///bb5a2cc0-84f5-4dfe-9560-ca3d313aefd2)  
  
 [Debug Apps by Recording Execution History with IntelliTrace](../VS_csharp/intellitrace.md)  
