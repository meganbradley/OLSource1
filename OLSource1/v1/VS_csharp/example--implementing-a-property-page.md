...
This example shows how to build a property page that displays (and allows you to change) properties of the [Document Classes](../VS_csharp/document-classes.md) interface. This interface is exposed by documents in Visual Studio's [Common Environment Object Model Examples](assetId:///919cdb58-9f6d-45e6-a0f1-9ca8f6e7051f) (although the property page that you'll create won't care where the objects it manipulates come from as long as they support the correct interface).  
  
 The example is based on the [ATLPages sample](../VS_csharp/visual-c---samples.md).  
...
##  <a name="vcconusing_the_atl_object_wizard"></a> Adding the ATL Property Page Class  
 First, create a new ATL project for a DLL server called `ATLPages7`. Now use the [ATL Property Page Wizard](../VS_csharp/atl-property-page-wizard.md) to generate a property page. Give the property page a **Short Name** of **DocProperties** then switch to the **Strings** page to set property-page-specific items as shown in the table below.  
...
##  <a name="vcconediting_the_dialog_resource"></a> Editing the Dialog Resource  
 ...
 ![Editing a dialog resource](../VS_csharp/media/ppgresourcelabeled.gif "PPGResourceLabeled")  
...
##  <a name="vcconadding_message_handlers"></a> Adding Message Handlers  
 ...
 [!code[NVC_ATL_Windowing#73](../VS_csharp/codesnippet/CPP/example--implementing-a-property-page_1.h)]  
  
 This code responds to changes made to the edit control or check box by calling [IPropertyPageImpl::SetDirty](../VS_csharp/ipropertypageimpl--setdirty.md), which informs the page site that the page has changed. Typically the page site will respond by enabling or disabling an **Apply** button on the property page frame.  
...
##  <a name="vcconhousekeeping"></a> Housekeeping  
 ...
 [!code[NVC_ATL_Windowing#74](../VS_csharp/codesnippet/CPP/example--implementing-a-property-page_2.h)]  
  
 You'll also need to refer to the `IPropertyPageImpl` base class; add the following `typedef` to the **CDocProperties** class:  
  
 [!code[NVC_ATL_Windowing#75](../VS_csharp/codesnippet/CPP/example--implementing-a-property-page_3.h)]  
  
##  <a name="vcconoverriding_ipropertypageimpl_setobjects"></a> Overriding IPropertyPageImpl::SetObjects  
 The first `IPropertyPageImpl` method that you need to override is [SetObjects](../VS_csharp/ipropertypageimpl--setobjects.md). Here you'll add code to check that only a single object has been passed and that it supports the **Document** interface that you're expecting:  
  
 [!code[NVC_ATL_Windowing#76](../VS_csharp/codesnippet/CPP/example--implementing-a-property-page_4.h)]  
...
##  <a name="vcconoverriding_ipropertypageimpl_activate"></a> Overriding IPropertyPageImpl::Activate  
 ...
 [!code[NVC_ATL_Windowing#77](../VS_csharp/codesnippet/CPP/example--implementing-a-property-page_5.h)]  
  
 The base class implementation of the [Activate](../VS_csharp/ipropertypageimpl--activate.md) method is responsible for creating the dialog box and its controls, so you can override this method and add your own initialization after calling the base class:  
  
 [!code[NVC_ATL_Windowing#78](../VS_csharp/codesnippet/CPP/example--implementing-a-property-page_6.h)]  
  
 This code uses the COM methods of the **Document** interface to get the properties that you're interested in. It then uses the Win32 API wrappers provided by [CDialogImpl](../VS_csharp/cdialogimpl-class.md) and its base classes to display the property values to the user.  
  
##  <a name="vcconoverride_ipropertypageimpl_apply"></a> Overriding IPropertyPageImpl::Apply  
 When users want to apply their changes to the objects, the property page site will call the [Apply](../VS_csharp/ipropertypageimpl--apply.md) method. This is the place to do the reverse of the code in **Activate** — whereas **Activate** took values from the object and pushed them into the controls on the property page, **Apply** takes values from the controls on the property page and pushes them into the object.  
  
 [!code[NVC_ATL_Windowing#79](../VS_csharp/codesnippet/CPP/example--implementing-a-property-page_7.h)]  
  
> [!NOTE]
>  The check against [m_bDirty](../VS_csharp/ipropertypageimpl--m_bdirty.md) at the beginning of this implementation is an initial check to avoid unnecessary updates of the objects if **Apply** is called more than once. There are also checks against each of the property values to ensure that only changes result in a method call to the **Document**.  
...
##  <a name="vccontesting_the_property_page"></a> Displaying the Property Page  
 ...
 Use the [Add Class dialog box](../VS_csharp/add-class-dialog-box.md) and the [ATL Simple Object Wizard](../VS_csharp/atl-simple-object-wizard.md) to generate a new class and use `Helper` as its short name. Once created, add a method as shown in the table below.  
...
|Parameters|`[in] BSTR bstrCaption, [in] BSTR bstrID, [in] IUnknown* pUnk`|  
 ...
 [!code[NVC_ATL_Windowing#80](../VS_csharp/codesnippet/CPP/example--implementing-a-property-page_8.cpp)]  
...
## See Also  
 [Property Pages](../VS_csharp/atl-com-property-pages.md)   
 [ATLPages Sample](../VS_csharp/visual-c---samples.md)