...
### Remarks  
 Within the Standard Template Library, the argument must refer to one of the three sequence containers that have the member function `push_back`: [deque Class](../VS_csharp/deque-class.md), [list Class](../VS_csharp/list-class.md), or [vector Class](../VS_csharp/vector-class.md).  
...
### Remarks  
 This function works with all STL containers and with [initializer_list](../VS_csharp/initializer_list-class.md).  
  
 You can use this member function in place of the `begin()` template function to guarantee that the return value is `const_iterator`. Typically, it's used in conjunction with the [auto](../VS_csharp/auto--c---.md) type deduction keyword, as shown in the following example. In the example, consider `Container` to be a modifiable (non- `const`) container or `initializer_list` of any kind that supports `begin()` and `cbegin()`.  
...
### Remarks  
 ...
 You can use this member function in place of the [end()](../VS_csharp/-iterator--functions.md#end) template function to guarantee that the return value is `const_iterator`. Typically, it's used in conjunction with the [auto](../VS_csharp/auto--c---.md) type deduction keyword, as shown in the following example. In the example, consider `Container` to be a modifiable (non- `const`) container or `initializer_list` of any kind that supports `end()` and `cend()`.  
...
### Remarks  
 For a code example, see [begin](../VS_csharp/-iterator--functions.md#begin).  
...
### Remarks  
 The member function [front_insert_iterator](../VS_csharp/front_insert_iterator-class.md#front_insert_iterator__front_insert_iterator) of the front_insert_iterator class may also be used.  
  
 Within the Standard Template Library, the argument must refer to one of the two sequence containers that have the member function `push_back`: [deque Class](../VS_csharp/deque-class.md) or [list Class](../VS_csharp/list-class.md).  
...
### Remarks  
 The template function returns [insert_iterator](../VS_csharp/insert_iterator-class.md#insert_iterator__insert_iterator)`<Container>(``_Cont``,` `_Where``)`.  
...
##  <a name="make_checked_array_iterator"></a>  make_checked_array_iterator  
 Creates a [checked_array_iterator](../VS_csharp/checked_array_iterator-class.md) that can be used by other algorithms.  
...
### Remarks  
 ...
 This function takes a raw pointer—which would ordinarily cause concern about bounds overrun—and wraps it in a [checked_array_iterator](../VS_csharp/checked_array_iterator-class.md) class that does checking. Because that class is marked as checked, the STL doesn't warn about it. For more information and code examples, see [Checked Iterators](../VS_csharp/checked-iterators.md).  
  
### Example  
  In the following example, a [vector](../VS_csharp/vector-class.md) is created and populated with 10 items. The contents of the vector are copied into an array by using the copy algorithm, and then `make_checked_array_iterator` is used to specify the destination. This is followed by an intentional violation of the bounds checking so that a debug assertion failure is triggered.  
...
##  <a name="make_unchecked_array_iterator"></a>  make_unchecked_array_iterator  
 Creates an [unchecked_array_iterator](../VS_csharp/unchecked_array_iterator-class.md) that can be used by other algorithms.  
...
### Remarks  
 ...
 This function takes a raw pointer and wraps it in a class that performs no checking and therefore optimizes away to nothing, but it also silences compiler warnings such as [C4996](../VS_csharp/compiler-warning--level-3--c4996.md). Therefore, this is a targeted way to deal with unchecked-pointer warnings without globally silencing them or incurring the cost of checking. For more information and code examples, see [Checked Iterators](../VS_csharp/checked-iterators.md).  
  
### Example  
  In the following example, a [vector](../VS_csharp/vector-class.md) is created and populated with 10 items. The contents of the vector are copied into an array by using the copy algorithm, and then `make_unchecked_array_iterator` is used to specify the destination.  
