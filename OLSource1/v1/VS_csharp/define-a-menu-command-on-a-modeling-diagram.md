...
2.  Add the following references to your project.  
    ...
    |Microsoft.VisualStudio.Modeling.Sdk.[version]|Define model event handlers.<br /><br /> Encapsulate series of changes to your model. For more information, see [How to Link Model Updates using Transactions](../VS_csharp/link-uml-model-updates-by-using-transactions.md).|  
    ...
    |Microsoft.VisualStudio.ArchitectureTools.Extensibility.Layer<br /><br /> Required only for commands on layer diagrams. For more information, see [Creating extensions for Layer diagrams](../VS_csharp/extend-layer-diagrams.md).|Define commands on a Layer diagram.|  
...
### Navigating and Updating the Model  
 The elements of the UML model are all available through the API. From the current selection or from the root of the model, you can access all the other elements. For more information, see [How to: Navigate the UML Model](../VS_csharp/navigate-the-uml-model.md) and [Programming with the UML API](../VS_csharp/programming-with-the-uml-api.md).  
...
1.  Press **F5**, or on the **Debug** menu, choose **Start Debugging**.  
    ...
    -   In Solution Explorer, on the shortcut menu of the startup or only project, choose **Properties**. In the project properties editor, select the **Debug** tab. Make sure that the string in the **Start external program** field is the full pathname of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)], typically:  
  
         `C:\Program Files\Microsoft Visual Studio [version]\Common7\IDE\devenv.exe`  
  
2.  In the experimental [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)], open or create a modeling project, and open or create a modeling diagram. Use a diagram that belongs to one of the types that are listed in the attributes of your menu command class.  
...
##  <a name="MenuExample"></a> Example  
 The following example shows the code for a menu command that will interchange the names of two elements on a class diagram. This code must be built in a [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] Extension project and installed as described in the previous sections.  
...
## See Also  
 ...
 [How to Define a Gesture Handler on a UML Diagram](../VS_csharp/define-a-gesture-handler-on-a-modeling-diagram.md)   
 [How to Define a Toolbox Item](../VS_csharp/define-a-custom-modeling-toolbox-item.md)   
 [How to use validation constraints](../VS_csharp/define-validation-constraints-for-uml-models.md)   
 [How to Edit Interactions using the UML API](../VS_csharp/edit-uml-sequence-diagrams-by-using-the-uml-api.md)   
