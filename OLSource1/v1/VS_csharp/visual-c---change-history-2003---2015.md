...
-   Explicit specializations no longer find template parameters from primary template ([C2146](../VS_csharp/compiler-error-c2146.md)).  
  
-   A protected member (n) can only be accessed through a member function of a class (B) that inherits from the class (A) of which it (n) is a member ([C2247](../VS_csharp/compiler-error-c2247.md)).  
  
-   Improved accessibility checks in compiler now detect inaccessible base classes ([C2248](../VS_csharp/compiler-error-c2248.md)).  
  
-   An exception cannot be caught if the destructor and/or copy constructor is inaccessible (C2316).  
  
-   Default arguments on pointers to functions no longer allowed ([C2383](../VS_csharp/compiler-error-c2383.md)).  
  
-   A static data member cannot be initialized via derived class ([C2477](../VS_csharp/compiler-error-c2477.md)).  
  
-   The initialization of a typedef is not allowed by the standard and now generates a compiler error ([C2513](../VS_csharp/compiler-error-c2513.md)).  
  
-   bool is now a proper type ([C2632](../VS_csharp/compiler-error-c2632.md)).  
  
-   A UDC can now create ambiguity with overloaded operators ([C2666](8364d15-c6eb-439a-9088-e04a0176692b)).  
  
-   More expressions are now considered valid null pointer constants ([C2668](../VS_csharp/compiler-error-c2668.md)).  
  
-   template<> is now required in places where the compiler would previously imply it ([C2768](../VS_csharp/compiler-error-c2768.md)).  
  
-   The expilicit specialization of a member function ourside the class is not valid if the function has already been explicitly specialized via a template class specialization ([C2910](../VS_csharp/compiler-error-c2910.md)).  
  
-   Floating point non-type template parameters are no longer allowed ([C2993](../VS_csharp/compiler-error-c2993.md)).  
  
-   Class templates are not allowed as template type arguments (C3206).  
  
-   Friend function names are no longer introduced into containing namespace ([C3767](../VS_csharp/compiler-error-c3767.md)).  
...
-   typename is now required if a dependent name is to be treated as a type ([C4346](../VS_csharp/compiler-warning--level-1--c4346.md)).  
...
-   A class template specialization needs to be defined before it was used in a return type ([C4686](../VS_csharp/compiler-warning--level-3--c4686.md)).  
...
-   The file descriptor value -2 is now used to indicate that stdout and stderr are not available for output, as for example in a Windows application that has no console window. The previous value used was -1. For more information, see [_fileno](../VS_csharp/_fileno.md).  
...
-   When linking with Setargv.obj or Wsetargv.obj, it is no longer possible to suppress the expansion of a wildcard character on the command line by enclosing it in double quotes. For more information, see [Expanding Wildcard Arguments](../VS_csharp/expanding-wildcard-arguments.md).  
...
-   Debug Iterators: Applications built with a debug version of the C-Runtime Library and which use iterators incorrectly might begin to see asserts at runtime. To disable these asserts, you must define _HAS_ITERATOR_DEBUGGING to 0. For more information, see [Debug Iterator Support](../VS_csharp/debug-iterator-support.md)  
...
-   The syntax for SAL Annotations has changed. For more information, see [SAL Annotations](../VS_csharp/sal-annotations.md).  
  
-   The IEEE filter now supports the SSE 4.1 instruction set. For more information, see [_fpieee_flt](../VS_csharp/_fpieee_flt.md)_fpieee_flt.  
...
-   The /Yl compiler option has changed. By default, the compiler uses this option, which can lead to LNK2011 errors under certain conditions. For more information, see [/Yl (Inject PCH Reference for Debug Library)](../VS_csharp/-yl--inject-pch-reference-for-debug-library-.md).  
...
-   The compiler may issue warnings [C4703](../VS_csharp/compiler-warning--level-4--c4703.md) and C4701 where previously it did not. The compiler applies stronger checks for use of                                 uninitialized local variables of pointer type.  
...
-   **new and delete** In previous versions of the library, the implementation-defined operator new and delete functions were exported from the runtime library DLL (for example, msvcr120.dll). These operator functions are now always statically linked into your binaries, even when using the runtime library DLLs.  
     ...
     This is not a breaking change for native or mixed code (/clr), however for code compiled as [/clr:pure](../VS_csharp/-clr--common-language-runtime-compilation-.md), this might cause your code to fail to compile. If you compile code as /clr:pure, you may need to add #include <new> or #include <new.h> to work around build errors due to this change. Note that /clr:pure is deprecated in [!INCLUDE[vs_dev14](../VS_csharp/includes/vs_dev14_md.md)] and might be removed in future releases.  
  
#### <process.h>  
  
-   **_beginthread and _beginthreadex** The [_beginthread](../VS_csharp/_beginthread--_beginthreadex.md) and [_beginthreadex](../VS_csharp/_beginthread--_beginthreadex.md) functions now hold a reference to the module in which the thread procedure is defined for the duration of the thread. This helps to ensure that modules are not unloaded until a thread has run to completion.  
  
#### <stdarg.h>  
  
-   **va_start and reference types** When compiling C++ code, [va_start](../VS_csharp/va_arg--va_copy--va_end--va_start.md) now validates at compile-time that the argument passed to it is not of reference type. Reference-type arguments are prohibited by the C++ Standard.  
...
-   **The printf and scanf family of functions are now defined inline.** The definitions of all of the printf and scanf functions have been moved inline into <stdio.h>, <conio.h>, and other CRT headers. This is a breaking change that leads to a linker error (LNK2019, unresolved external symbol) for any programs that declared these functions locally without including the appropriate CRT headers. If possible, you should update the code to include the CRT headers (that is, add #include <stdio.h>) and the inline functions, but if you do not want to modify your code to include these header files, an alternative solution is to add an additional library to your linker input, legacy_stdio_definitions.lib.  
    ...
    >  If you are linking with Windows SDK 8.1 or earlier, you might encounter these unresolved external symbol errors. In that case, you should resolve the error by adding legacy_stdio_definitions.lib to the linker input as described previously.  
     ...
     To troubleshoot unresolved symbol errors, you can try using [dumpbin.exe](../VS_csharp/dumpbin-reference.md) to examine the symbols defined in a binary. Try the following command line to view symbols defined in a library.  
  
    ```  
    dumpbin.exe /LINKERMEMBER somelibrary.lib  
    ```  
  
-   **gets and _getws** The [gets](../VS_csharp/gets--_getws.md) and [_getws](../VS_csharp/gets--_getws.md) functions have been removed. The gets function was removed from the C Standard Library in C11 because it cannot be used securely. The _getws function was a Microsoft extension that was equivalent to gets but for wide strings. As alternatives to these functions, consider use of [fgets](../VS_csharp/fgets--fgetws.md), [fgetws](../VS_csharp/fgets--fgetws.md), [gets_s](../VS_csharp/gets_s--_getws_s.md), and [_getws_s](../VS_csharp/gets_s--_getws_s.md).  
  
-   **_cgets and _cgetws** The [_cgets](../VS_csharp/_cgets--_cgetws.md) and [_cgetws](../VS_csharp/_cgets--_cgetws.md) functions have been removed. As alternatives to these functions, consider use of [_cgets_s](../VS_csharp/_cgets_s--_cgetws_s.md) and [_cgetws_s](../VS_csharp/_cgets_s--_cgetws_s.md).  
...
-   **Infinity and NaN Formatting** In previous versions, infinities and NaNs would be formatted using a set of Visual C++-specific sentinel strings.  
    ...
    -   Indefinite NaN:nan(ind)  
     ...
     The [scanf](../VS_csharp/scanf--_scanf_l--wscanf--_wscanf_l.md) functions have been modified to parse these new strings, so these strings will round-trip through printf and scanf.  
  
-   **Floating point formatting and parsing** New floating point formatting and parsing algorithms have been introduced to improve correctness. This change affects the [printf](../VS_csharp/printf--_printf_l--wprintf--_wprintf_l.md) and [scanf](../VS_csharp/scanf--_scanf_l--wscanf--_wscanf_l.md) families of functions, as well as functions like [strtod](../VS_csharp/strtod--_strtod_l--wcstod--_wcstod_l.md).  
...
-   **_O_U8TEXT mode**  
     ...
     The [_setmode](../VS_csharp/_setmode.md) function now correctly reports the mode for streams opened in_O_U8TEXT mode. In previous versions of the library, it would report such streams as being opened in _O_WTEXT.  
...
-   **snprintf and vsnprintf** The [snprintf](../VS_csharp/snprintf--_snprintf--_snprintf_l--_snwprintf--_snwprintf_l.md) and [vsnprintf](../VS_csharp/vsnprintf--_vsnprintf--_vsnprintf_l--_vsnwprintf--_vsnwprintf_l.md) functions are now implemented. Older code often provided definitions macro versions of these functions because they were not implemented by the CRT library, but these are no longer needed in newer versions. If [snprintf](../VS_csharp/snprintf--_snprintf--_snprintf_l--_snwprintf--_snwprintf_l.md) or [vsnprintf](../VS_csharp/vsnprintf--_vsnprintf--_vsnprintf_l--_vsnwprintf--_vsnwprintf_l.md) is defined as a macro before including <stdio.h>, compilation now fails with an error that indicates where the macro was defined.  
...
-   **_outp and _inp** The functions [_outp](../VS_csharp/_outp--_outpw--_outpd.md), [_outpw](../VS_csharp/_outp--_outpw--_outpd.md), [_outpd](../VS_csharp/_outp--_outpw--_outpd.md), [_inp](../VS_csharp/_inp--_inpw--_inpd.md), [_inpw](../VS_csharp/_inp--_inpw--_inpd.md), and [_inpd](../VS_csharp/_inp--_inpw--_inpd.md) have been removed.  
...
-   **smallheap** The smalheap link option has been removed. See [Link Options](../VS_csharp/link-options.md).  
...
-   **asctime** In previous versions, the [asctime](../VS_csharp/asctime--_wasctime.md) function would pad single-digit days with a leading zero, for example: Fri Jun 06 08:00:00 2014. The specification requires that such days be padded with a leading space, e.g. Fri Jun  6 08:00:00 2014. This has been fixed.  
...
-   **hash_map and hash_set** The non-standard header files hash_map and hash_set are deprecated in [!INCLUDE[vs_dev14](../VS_csharp/includes/vs_dev14_md.md)] and will be removed in a future release. Use unordered_map and unordered_set instead.  
...
-   **type traits** The old names for type traits from an earlier version of the C++ draft standard have been removed. These were changed in C++11 and have been updated to the C++11 values in [!INCLUDE[vs_dev14](../VS_csharp/includes/vs_dev14_md.md)]. The following table shows the old and new names.  
...
## Compiler Conformance Improvements in Visual C++ 2015  
 When upgrading code from previous versions, you might also encounter compiler errors that are due to conformance improvements made in Visual C++ 2015. These improvements do not break binary compatibility from earlier versions of Visual C++, but they can produce compiler errors where none were emitted before. For more information, see [Compiler Conformance Improvements in Visual C++ 2015](../VS_csharp/compiler-conformance-improvements-in-visual-c---2015.md).