...
C++ supports dynamic allocation and deallocation of objects using the [new](../VS_csharp/new-operator--c---.md) and [delete](../VS_csharp/delete-operator--c---.md) operators. These operators allocate memory for objects from a pool called the free store. The `new` operator calls the special function [operator new](../Topic/operator%20new%20Function.md), and the `delete` operator calls the special function [operator delete](../Topic/operator%20delete%20Function.md).  
  
 In [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)] .NET 2002, the `new` function in the Standard C++ Library will support the behavior specified in the C++ standard, which is to throw a std::bad_alloc exception if the memory allocation fails.  
 ...
 For a list of the library files that comprise the C Runtime Library and the Standard C++ Library, see [C Run-Time Libraries](../VS_csharp/crt-library-features.md).  
...
```  
 ...
 If the request is for zero bytes of storage, **operator new** returns a pointer to a distinct object (that is, repeated calls to **operator new** return different pointers). If there is insufficient memory for the allocation request, **operator new** returns **NULL** or throws an exception (see [The new and delete Operators](../VS_csharp/new-and-delete-operators.md) for more information).  
  
 You can write a routine that attempts to free memory and retry the allocation; see [_set_new_handler](../VS_csharp/_set_new_handler.md) for more information. For more details on the recovery scheme, see the following topic, [Handling Insufficient Memory Conditions](../Topic/Handling%20Insufficient%20Memory%20Conditions.md).  
...
```  
 ...
 There is another ways to handle failed memory allocation requests: write a custom recovery routine to handle such a failure, then register your function by calling the [_set_new_handler](../VS_csharp/_set_new_handler.md) run-time function.  
...
```  
 ...
 The **operator delete** function is static; therefore, it cannot be virtual. The `operator delete` function obeys access control, as described in [Member-Access Control](../VS_csharp/member-access-control--c---.md).  
