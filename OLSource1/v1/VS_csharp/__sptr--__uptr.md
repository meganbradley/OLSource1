...
```  
 ...
 Use the `__sptr` and `__uptr` modifiers with pointer declarations. Use the modifiers in the position of a [pointer type qualifier](../VS_csharp/pointer-declarations.md), which means the modifier must follow the asterisk. You cannot use the modifiers with [pointers to members](../VS_csharp/pointers-to-members.md). The modifiers do not affect non-pointer declarations.  
  
 If you do not use the `__sptr` or `__uptr` modifier, and you enable [Compiler Warning (level 2) C4826](../VS_csharp/compiler-warning--level-2--c4826.md), the compiler issues a warning when a 32-bit pointer is converted to 64 bits.  
...
## See Also  
 [Microsoft-Specific Modifiers](../VS_csharp/microsoft-specific-modifiers.md)