...
-   [How-to: Use a Message Block Filter to Improve Performance](../VS_csharp/how-to--use-a-message-block-filter.md)  
  
-   [Walkthrough: Creating a Dataflow Agent](../VS_csharp/walkthrough--creating-a-dataflow-agent.md)  
  
 We also recommend that you understand the basics of [!INCLUDE[ndptecgdiplus](../VS_csharp/includes/ndptecgdiplus_md.md)] before you start this walkthrough. For more information about [!INCLUDE[ndptecgdiplus](../VS_csharp/includes/ndptecgdiplus_md.md)], see [GDI+](_gdiplus_GDI_start_cpp).  
...
##  <a name="functionality"></a> Defining Image Processing Functionality  
 ...
 [!code[concrt-image-processing-filter#2](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_1.cpp)]  
  
 The following function, `ProcessImage`, calls the given [std::function](../VS_csharp/function-class.md) object to transform the color value of each pixel in a [!INCLUDE[ndptecgdiplus](../VS_csharp/includes/ndptecgdiplus_md.md)] [Bitmap](https://msdn.microsoft.com/en-us/library/ms534420.aspx) object. The `ProcessImage` function uses the [concurrency::parallel_for](../VS_csharp/parallel_for-function.md) algorithm to process each row of the bitmap in parallel.  
  
 [!code[concrt-image-processing-filter#3](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_2.cpp)]  
 ...
 [!code[concrt-image-processing-filter#4](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_3.cpp)]  
  
 The following function, `GetColorDominance`, also calls the `ProcessImage` function. However, instead of changing the value of each color, this function uses [concurrency::combinable](../VS_csharp/combinable-class.md) objects to compute whether the red, green, or blue color component dominates the image.  
  
 [!code[concrt-image-processing-filter#5](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_4.cpp)]  
 ...
 [!code[concrt-image-processing-filter#6](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_5.cpp)]  
  
 [[Top](#top)]  
  
##  <a name="network"></a> Creating the Image Processing Network  
 This section describes how to create a network of asynchronous message blocks that perform image processing on every [!INCLUDE[TLA#tla_jpeg](../VS_csharp/includes/tlasharptla_jpeg_md.md)] (.jpg) image in a given directory. The network performs the following image-processing operations:  
...
3.  For any other image, apply sepia toning.  
 ...
 The following steps show how to create a function that implements this image processing network and applies that network to every [!INCLUDE[TLA#tla_jpeg](../VS_csharp/includes/tlasharptla_jpeg_md.md)] image in a given directory.  
  
#### To create the image processing network  
  
1.  Create a function, `ProcessImages`, that takes the name of a directory on disk.  
  
     [!code[concrt-image-processing-filter#7](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_6.cpp)]  
...
2.  In the `ProcessImages` function, create a `countdown_event` variable. The `countdown_event` class is shown later in this walkthrough.  
     ...
     [!code[concrt-image-processing-filter#8](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_7.cpp)]  
  
3.  Create a [std::map](../VS_csharp/map-class.md) object that associates a `Bitmap` object with its original file name.  
  
     [!code[concrt-image-processing-filter#9](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_8.cpp)]  
  
4.  Add the following code to define the members of the image-processing network.  
  
     [!code[concrt-image-processing-filter#10](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_9.cpp)]  
  
5.  Add the following code to connect the network.  
  
     [!code[concrt-image-processing-filter#11](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_10.cpp)]  
  
6.  Add the following code to send to the head of the network the full path of each [!INCLUDE[TLA#tla_jpeg](../VS_csharp/includes/tlasharptla_jpeg_md.md)] file in the directory.  
  
     [!code[concrt-image-processing-filter#12](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_11.cpp)]  
  
7.  Wait for the `countdown_event` variable to reach zero.  
  
     [!code[concrt-image-processing-filter#13](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_12.cpp)]  
  
 The following table describes the members of the network.  
  
|Member|Description|  
|------------|-----------------|  
|`load_bitmap`|A [concurrency::transformer](../VS_csharp/transformer-class.md) object that loads a `Bitmap` object from disk and adds an entry to the `map` object to associate the image with its original file name.|  
|`loaded_bitmaps`|A [concurrency::unbounded_buffer](../VS_csharp/unbounded_buffer-class.md) object that sends the loaded images to the image processing filters.|  
...
|`decrement`|A [concurrency::call](../VS_csharp/call-class.md) object that acts as the terminal node in the network. It decrements the `countdown_event` object to signal to the main application that an image has been processed.|  
 ...
 ![Image processing network](../VS_csharp/media/concrt_imageproc.png "ConcRT_ImageProc")  
  
 The `countdown_event` object in this example enables the image processing network to inform the main application when all images have been processed. The `countdown_event` class uses a [concurrency::event](../VS_csharp/event-class.md) object to signal when a counter value reaches zero. The main application increments the counter every time that it sends a file name to the network. The terminal node of the network decrements the counter after each image has been processed. After the main application traverses the specified directory, it waits for the `countdown_event` object to signal that its counter has reached zero.  
  
 The following example shows the `countdown_event` class:  
  
 [!code[concrt-image-processing-filter#14](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_13.cpp)]  
  
 [[Top](#top)]  
  
##  <a name="complete"></a> The Complete Example  
 The following code shows the complete example. The `wmain` function manages the [!INCLUDE[ndptecgdiplus](../VS_csharp/includes/ndptecgdiplus_md.md)] library and calls the `ProcessImages` function to process the [!INCLUDE[TLA#tla_jpeg](../VS_csharp/includes/tlasharptla_jpeg_md.md)] files in the `Sample Pictures` directory.  
  
 [!code[concrt-image-processing-filter#15](../VS_csharp/codesnippet/CPP/walkthrough--creating-an-image-processing-network_14.cpp)]  
  
 The following illustration shows sample output. Each source image is above its corresponding modified image.  
  
 ![Sample output for the example](../VS_csharp/media/concrt_imageout.png "ConcRT_ImageOut")  
