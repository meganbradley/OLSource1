...
### Remarks  
 **allocator_type** is a synonym for the template parameter [Allocator](../VS_csharp/set-class.md).  
  
 Returns the function object that a multiset uses to order its elements, which is the template parameter `Allocator`.  
  
 For more information on `Allocator`, see the Remarks section of the [set Class](../VS_csharp/set-class.md) topic.  
...
|`Where`|The place to start searching for the correct point of insertion. (If that point immediately precedes `Where`, insertion can occur in amortized constant time instead of logarithmic time.)|  
|`ValTy`|Template parameter that specifies the argument type that the set can use to construct an element of [value_type](../VS_csharp/map-class.md#map__value_type), and perfect-forwards `Val` as an argument.|  
...
### Return Value  
 The single-element member functions, (1) and (2), return a [pair](../VS_csharp/pair-structure.md) whose `bool` component is true if an insertion was made, and false if the set already contained an element of equivalent value in the ordering. The iterator component of the return-value pair points to the newly inserted element if the `bool` component is true, or to the existing element if the `bool` component is false.  
...
### Remarks  
 ...
 The [value_type](../VS_csharp/map-class.md#map__value_type) of a container is a typedef that belongs to the container, and, for set, `set<V>::value_type` is type `const V`.  
 ...
 The initializer list member function (6) uses an [initializer_list](../VS_csharp/-initializer_list-.md) to copy elements into the set.  
...
##  <a name="set__iterator"></a>  set::iterator  
 A type that provides a constant [bidirectional iterator](../VS_csharp/bidirectional_iterator_tag-struct.md) that can read any element in a set.  
...
### Return Value  
 ...
 For more information on `Traits` see the [set Class](../VS_csharp/set-class.md) topic.  
...
### Remarks  
 ...
 For more information on `Key`, see the Remarks section of the [set Class](../VS_csharp/set-class.md) topic.  
...
### Remarks  
 The first version of `operator=` uses an [lvalue reference](../VS_csharp/lvalue-reference-declarator---.md) for `_Right`, to copy elements from `_Right` to this `set`.  
  
 The second version uses an [rvalue reference](../VS_csharp/rvalue-reference-declarator----.md) for _Right. It moves elements from `_Right` to this `set`.  
