...
#### Parameters  
 `pCallback`  
 [in] An [IDebugEventCallback2](../VS_csharp/idebugeventcallback2.md) object to be used for debug event notification.  
...
## Remarks  
 A debug engine (DE) never calls this method to attach to a program. If the DE runs in the program's address space, the [IDebugProgramNodeAttach2::OnAttach](../VS_csharp/idebugprogramnodeattach2--onattach.md) method is called. If the DE runs in the session debug manager's (SDM) address space, the [Attach](../VS_csharp/idebugengine2--attach.md) method is called.  
