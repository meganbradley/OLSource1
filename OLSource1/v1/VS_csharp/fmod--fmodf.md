...
## Return Value  
 `fmod` returns the floating-point remainder of `x` / `y`. If the value of `y` is 0.0, `fmod` returns a quiet NaN. For information about representation of a quiet NaN by the `printf` family, see [printf](../VS_csharp/printf--_printf_l--wprintf--_wprintf_l.md).  
