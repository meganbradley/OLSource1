...
## Supported Platforms  
 Debugging is supported on [!INCLUDE[win7](../VS_csharp/includes/win7_md.md)], [!INCLUDE[win8](../VS_csharp/includes/win8_md.md)], [!INCLUDE[winsvr08_r2](../VS_csharp/includes/winsvr08_r2_md.md)], and [!INCLUDE[winserver8](../VS_csharp/includes/winserver8_md.md)]. For debugging on the software emulator, [!INCLUDE[win8](../VS_csharp/includes/win8_md.md)], or [!INCLUDE[winserver8](../VS_csharp/includes/winserver8_md.md)] is required. For debugging on the hardware, you must install the drivers for your graphics card. Not all hardware vendors implement all debugger features. See the vendor documentation for limitations.  
...
## Launching and Attaching to Applications  
 You can use the Visual Studio debugging commands to start and stop GPU debugging. For more information, see [Execution Control](../VS_csharp/navigating-through-code-with-the-debugger.md). You can also attach the GPU debugger to a running process, but only if that process executes GPU code. For more information, see [Attach to Running Processes with the Visual Studio Debugger](../VS_csharp/attach-to-running-processes-with-the-visual-studio-debugger.md).  
...
1.  The **Run to Cursor** command runs your app until it reaches the cursor location and then breaks. This does not imply that the current thread runs to the cursor; rather, it means that the first thread that reaches the cursor point triggers the break. See [Start, Break, Step, Run through Code, and Stop Debugging in Visual Studio](../VS_csharp/navigating-through-code-with-the-debugger.md)  
...
### Error: There is a configuration issue with the selected Debugging Accelerator Type.  
 This error happens when there is an inconsistency between the project settings and the configuration of the PC that you are debugging on. For more information, see [Project Settings for a C++ Debug Configuration](../VS_csharp/project-settings-for-a-c---debug-configuration.md).  
...
## See Also  
 [Walkthrough: Debugging a C++ AMP Application](../VS_csharp/walkthrough--debugging-a-c---amp-application.md)   
 [Project Settings for a C++ Debug Configuration](../VS_csharp/project-settings-for-a-c---debug-configuration.md)   
