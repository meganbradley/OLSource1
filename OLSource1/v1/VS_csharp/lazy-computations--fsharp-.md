...
## Remarks  
 In the previous syntax, `expression` is code that is evaluated only when a result is required, and `identifier` is a value that stores the result. The value is of type [Lazy<'T>](../VS_csharp/control.lazy--t--type-abbreviation--fsharp-.md), where the actual type that is used for `'T` is determined from the result of the expression.  
 ...
 [!code[FsLangRef2#73011](../VS_csharp/codesnippet/FSharp/lazy-computations--fsharp-_1.fs)]  
  
 Lazy evaluation, but not the `Lazy` type, is also used for sequences. For more information, see [Sequences (F#)](../VS_csharp/sequences--fsharp-.md).  
  
## See Also  
 [F# Language Reference](../VS_csharp/fsharp-language-reference.md)   
 [LazyExtensions module](../VS_csharp/control.lazyextensions-module--fsharp-.md)