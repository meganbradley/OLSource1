...
## Remarks  
 The template class describes an object that points to a resource that is managed by one or more [shared_ptr](../VS_csharp/shared_ptr-class.md) objects. The `weak_ptr` objects that point to a resource do not affect the resource's reference count. Thus, when the last `shared_ptr` object that manages that resource is destroyed the resource will be freed, even if there are `weak_ptr` objects pointing to that resource. This is essential for avoiding cycles in data structures.  
...
### Remarks  
 The member function returns an empty shared_ptr object if `*this` has expired; otherwise it returns a [shared_ptr](../VS_csharp/shared_ptr-class.md)`<Ty>` object that owns the resource that `*this` points to.  
