...
This topic describes how to use [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] Profiling Tools command-line tools to attach the Profiler to an ASP.NET application and collect process and thread concurrency data.  
...
1.  Start the profiler by typing the following command:  
    ...
    -   The [/start](../VS_csharp/start.md) option initializes the profiler to collect resource contention data.  
    ...
    |Option|Description|  
    |------------|-----------------|  
    |[/user](../VS_csharp/user--vsperfcmd-.md) **:**[`Domain\`]`UserName`|Specifies the optional domain and user name of the account to be granted access to the profiler.|  
    |[/crosssession](../VS_csharp/crosssession.md)|Enables profiling of processes in other logon sessions.|  
...
3.  Attach the profiler to the ASP.NET worker process by typing the following command:**VSPerfCmd /attach:**`PID` [**/targetclr:**`Version`]  
    ...
    -   [/targetclr](../VS_csharp/targetclr.md) **:** `Version` specifies the version of the common language runtime (CLR) to profile when more than one version of the runtime is loaded in an application. This parameter is optional.  
...
## See Also  
 ...
 [Rapid Web Site Profiling with VSPerfASPNETCmd](../VS_csharp/rapid-web-site-profiling-with-vsperfaspnetcmd.md)