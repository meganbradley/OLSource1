...
|None of the constants match the constants in the **case** labels; **default** label is not present.|Control is transferred to the statement after the `switch` statement.|  
 ...
 If a matching expression is found, control is not impeded by subsequent **case** or **default** labels. The [break](../VS_csharp/break-statement--c---.md) statement is used to stop execution and transfer control to the statement after the `switch` statement. Without a **break** statement, every statement from the matched **case** label to the end of the `switch`, including the **default**, is executed. For example:  
...
## Microsoft Specific  
 ...
 The default for Microsoft C is that the Microsoft extensions are enabled. Use the [/Za](../VS_csharp/-za---ze--disable-language-extensions-.md) compiler option to disable these extensions.  
