...
## Notes for Callers  
 Using the interface ID (IID) argument given to [IDebugEventCallback2::Event](../VS_csharp/idebugeventcallback2--event.md) or [IDebugPortEvents2::Event](../VS_csharp/idebugportevents2--event.md), the session debug manager (SDM) calls [QueryInterface](../VS_csharp/queryinterface.md) on the `IDebugEvent2` interface to obtain the appropriate event interface.  
...
|Method|Description|  
|------------|-----------------|  
|[GetAttributes](../VS_csharp/idebugevent2--getattributes.md)|Gets the attributes for this debug event.|  
  
## Remarks  
 The more specific event interfaces, such as [IDebugBreakpointEvent2](../VS_csharp/idebugbreakpointevent2.md), do not derive from the IDebugEvent2 interface but are instead implemented as a separate interface on the same object as `IDebugEvent2`.  
