...
|`uniform_real_distribution::operator()`|`uniform_real_distribution::b`|[uniform_real_distribution::param_type](#uniform_real_distribution__param_type)|  
 ...
 The property member `a()` returns the currently stored minimum bound of the distribution, while `b()` returns the currently stored maximum bound. For this distribution class, these minimum and maximum values are the same as those returned by the common property functions `min()` and `max()` described in the [<random\>](../VS_csharp/-random-.md) topic.  
...
### Parameters  
 See parent topic [uniform_real_distribution Class](../VS_csharp/uniform_real_distribution-class.md).  
