...
>  The <xref:Microsoft.VisualStudio.VSConstants*> class contains constants for common HRESULTS, for example, <xref:Microsoft.VisualStudio.VSConstants.S_OK*> and <xref:Microsoft.VisualStudio.VSConstants.E_NOTIMPL*>, and [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] HRESULTS, for example, <xref:Microsoft.VisualStudio.VSConstants.VS_E_INCOMPATIBLEDOCDATA*> and <xref:Microsoft.VisualStudio.VSConstants.VS_E_UNSUPPORTEDFORMAT*>. <xref:Microsoft.VisualStudio.VSConstants*> also provides the <xref:Microsoft.VisualStudio.ErrorHandler.Succeeded*> and <xref:Microsoft.VisualStudio.ErrorHandler.Failed*> methods, which correspond to the SUCCEEDED and FAILED macros in COM.  
 ...
 [!code[VSSDKHRESULTInformation#1](../VS_csharp/codesnippet/VisualBasic/using-visual-studio-interop-assemblies_1.vb)]
[!code[VSSDKHRESULTInformation#1](../VS_csharp/codesnippet/CSharp/using-visual-studio-interop-assemblies_1.cs)]  
 ...
 [!code[VSSDKHRESULTInformation#2](../VS_csharp/codesnippet/VisualBasic/using-visual-studio-interop-assemblies_2.vb)]
[!code[VSSDKHRESULTInformation#2](../VS_csharp/codesnippet/CSharp/using-visual-studio-interop-assemblies_2.cs)]  
...
## IUnknown parameters passed as Type void**  
 Look for [out] parameters that are defined as type `void **` in the COM interface, but that are defined as `[``iid_is``]` in the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] interop assembly method prototype.  
...
## Optional [out] Parameters  
 Look for parameters that are defined as an [out] data type (`int`, `object`, and so on) in the COM interface, but that are defined as arrays of the same data type in the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] interop assembly method prototype.  
 ...
 Because the CLR does not allow the value of an [out] parameter to be `null`, part of the designed behavior of these interfaces is not directly available within managed code. The [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] interop assembly methods for affected interfaces work around the issue by defining the relevant parameters as arrays because the CLR allows the passing of `null` arrays.  
...
## Passing Constants in Pointer Parameters  
 Look for parameters that are defined as [in] pointers in the COM interface, but that are defined as a <xref:System.IntPtr*> type in the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] interop assembly method prototype.  
  
 A similar issue occurs when a COM interface passes a special value, such as 0, -1, or –2, instead of an object pointer. Unlike [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)], the CLR does not allow constants to be cast as objects. Instead, the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] interop assembly defines the parameter as a <xref:System.IntPtr*> type.  
...
## OLE Return Values Passed as [out] Parameters  
 Look for methods that have a `retval` return value in the COM interface, but that have an `int` return value and an additional [out] array parameter in the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] interop assembly method prototype. It should be clear that these methods require special handling because the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] interop assembly method prototypes have one more parameter than the COM interface methods.  
  
 Many COM interfaces that deal with OLE activity send information about OLE status back to the calling program stored in the `retval` return value of the interface. Instead of using a return value, the corresponding [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] interop assembly methods send the information back to the calling program stored in an [out] array parameter.  
