...
|----------|------------|------------------------|  
|Decide whether to use the MFC ODBC or DAO classes.|Use ODBC for new MFC projects. Use DAO only to maintain existing applications. See [Should I use DAO or ODBC?](../VS_csharp/should-i-use-dao-or-odbc-.md). For general information, see the article [Data Access Programming](../VS_csharp/data-access-programming--mfc-atl-.md).|The framework supplies classes that support database access.|  
...
|Create recordset objects as needed in your code. Use each recordset to manipulate records...|Your recordsets are based on the classes derived from [CRecordset](../VS_csharp/crecordset-class.md) with the wizards.|ODBC uses record field exchange (RFX) to exchange data between the database and your recordset's field data members. If you are using a record view, dialog data exchange (DDX) exchanges data between the recordset and the controls on the record view.|  
|...or create an explicit [CDatabase](../VS_csharp/cdatabase-class.md) in your code for each database you want to open.|Base your recordset objects on the database objects.|The database object provides an interface to the data source.|  
|Bind data columns to your recordset dynamically.|In ODBC, add code to your derived recordset class to manage the binding. See the article [Recordset: Dynamically Binding Data Columns (ODBC)](../VS_csharp/recordset--dynamically-binding-data-columns--odbc-.md).||  
  
## See Also  
 [Building on the Framework](../VS_csharp/building-on-the-framework.md)   
 [Sequence of Operations for Building MFC Applications](../VS_csharp/sequence-of-operations-for-building-mfc-applications.md)   
