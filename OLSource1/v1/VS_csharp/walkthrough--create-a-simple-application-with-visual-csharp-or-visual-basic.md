...
By completing this walkthrough, you’ll become familiar with many of the tools, dialog boxes, and designers that you can use when you develop applications with Visual Studio. You’ll create a simple “Hello, World”-style application, design the UI, add code, and debug errors, while you learn more about working in the integrated development environment (IDE).  
 ...
 [Configure the IDE](../VS_csharp/walkthrough--create-a-simple-application-with-visual-csharp-or-visual-basic.md#BKMK_ConfigureIDE)  
  
 [Create a simple application](../VS_csharp/walkthrough--create-a-simple-application-with-visual-csharp-or-visual-basic.md#BKMK_CreateApp)  
  
 [Debug and test the application](../VS_csharp/walkthrough--create-a-simple-application-with-visual-csharp-or-visual-basic.md#BKMK_DebugTest)  
...
##  <a name="BKMK_ConfigureIDE"></a> Configure the IDE  
 ...
 ![IDE with General Settings Applied](../VS_csharp/media/exploreide-idewithgeneralsettings.png "ExploreIDE-IDEwithgeneralsettings")  
 ...
 ![Options dialog box wirh Show all settings option](../VS_csharp/media/exploreide-optionsdialogbox.png "ExploreIDE-Optionsdialogbox")  
...
2.  Change the **Color theme** to **Dark**, then click **OK**.  
     ...
     ![Dark color theme selected](../VS_csharp/media/exploreide-darkthemeoptionsdlgbox.png "ExploreIDE-Darkthemeoptionsdlgbox")  
  
 The colors in Visual Studio should match the following image:  
  
 ![IDE with Dark Theme applied](../VS_csharp/media/exploreide-darkthemeide.png "ExploreIDE-DarkThemeIDE")  
...
1.  Create a new project. On the menu bar, choose **File**, **New**, **Project…**.  
     ...
     ![In the Quick Launch box, specify new project](../VS_csharp/media/exploreide-quicklaunchnewprojectsmall.png "ExploreIDE-QuickLaunchNewProjectsmall")  
...
2.  Choose the Visual Basic or the Visual C# WPF Application template by choosing in the left pane **Installed**, **Templates**, **Visual C#**, **Windows**, for example, and then choosing WPF Application in the middle pane.  Name the project HelloWPFApp at the bottom of the New Project dialog.  
     ...
     ![Create a Visual Basic WPF Project, HelloWPFApp](../VS_csharp/media/exploreide-newprojectvb.png "ExploreIDE-NewProjectVB")  
  
     OR  
  
     ![Create a Visual C&#35; WPF Project, HelloWPFApp](../VS_csharp/media/exploreide-newprojectcsharp.png "ExploreIDE-NewProjectcsharp")  
 ...
 ![Solution Explorer with HelloWPFApp files loaded](../VS_csharp/media/exploreide-hellowpfappfiles.png "ExploreIDE-HelloWPFAppFiles")  
...
1.  In the following procedure, you’ll give MainWindow a more specific name. In **Solution Explorer**, select MainWindow.xaml. You should see the **Properties** window, but if you don’t, choose the **View** menu and the **Property Window** item. Change the **File Name** property to `Greetings.xaml`.  
     ...
     ![Properties window with File Name highlighted](../VS_csharp/media/exploreide-filenameinpropertieswindow.png "ExploreIDE-FilenameinPropertiesWindow")  
...
2.  In the **Toolbox**, search for the TextBlock control.  
     ...
     ![Toolbox with the TextBlock control highlighted](../VS_csharp/media/exploreide-textblocktoolbox.png "ExploreIDE-TextBlockToolbox")  
...
3.  Add a TextBlock control to the design surface by choosing the TextBlock item and dragging it to the window on the design surface.  Center the control near the top of the window.  
 ...
 ![TextBlock control on the Greetings form](../VS_csharp/media/exploreide-greetingswithtextblockonly.png "ExploreIDE-GreetingswithTextblockonly")  
...
1.  In the **Toolbox**, search for the RadioButton control.  
     ...
     ![Toolbox window with RadioButton control selected](../VS_csharp/media/exploreide-radiobuttontoolbox.png "ExploreIDE-RadioButtonToolbox")  
...
2.  Add two RadioButton controls to the design surface by choosing the RadioButton item and dragging it to the window on the design surface twice, and move the buttons (by selecting them and using the arrow keys) so that the buttons appear side by side under the TextBlock control.  
     ...
     ![Greetings form with textblock and two radiobuttons](../VS_csharp/media/exploreide-greetingswithradiobuttons.png "ExploreIDE-Greetingswithradiobuttons")  
...
2.  In the XAML view, change the value of **Content** for the Button control from `Content=”Button”` to `Content=”Display”`, and then save the changes (Ctrl-s or use the **File** menu).  
 ...
 ![Greetings form with control labels](../VS_csharp/media/exploreide-greetingswithconrollabels.png "ExploreIDE-Greetingswithconrollabels")  
...
##  <a name="BKMK_DebugTest"></a> Debug and test the application  
 Next, you’ll debug the application to look for errors and test that both message boxes appear correctly. The following instructions tell you how to build and launch the debugger, but later you might read [Building a WPF Application (WPF)](assetId:///a58696fd-bdad-4b55-9759-136dfdf8b91c) and [Debugging WPF](../VS_csharp/debugging-wpf.md) for more information.  
...
1.  Start the debugger by selecting **Debug**, then **Start Debugging**.  
     ...
     ![Start Debugging command on the Debug menu](../VS_csharp/media/exploreide-startdebugging.png "ExploreIDE-StartDebugging")  
...
2.  Choose the **OK** button, and then stop the debugger.  
     ...
     ![Stop Debugging command on the Debug menu](../VS_csharp/media/exploreide-stopdebugging.png "ExploreIDE-StopDebugging")  
...
2.  Add a breakpoint from the menu by selecting **Debug**, then **Toggle Breakpoint**.  
     ...
     ![Toggle Breakpoint command on the Debug menu](../VS_csharp/media/exploreide-togglebreakpoint.png "ExploreIDE-ToggleBreakpoint")  
...
1.  On the main menu, select **Build**, then **Clean solution** to delete intermediate files and output files that were created during previous builds.  This is not necessary, but it cleans up the debug build outputs.  
     ...
     ![The Clean Solution command on the Build menu](../VS_csharp/media/exploreide-cleansolution.png "ExploreIDE-CleanSolution")  
...
2.  Change the build configuration for HelloWPFApp from **Debug** to **Release** by using the dropdown control on the toolbar (it says “Debug” currently).  
     ...
     ![The Standard toolbar with Release selected](../VS_csharp/media/exploreide-releaseversion.png "ExploreIDE-ReleaseVersion")  
...
3.  Build the solution by choosing **Build**, then **Build Solution** or press the F6 key.  
 ...
 Congratulations on completing this walkthrough! You can find the .exe you built under your solution and project directory (…\HelloWPFApp\HelloWPFApp\bin\Release\\). If you want to explore more examples, see [Visual Studio Samples](../VS_csharp/visual-studio-samples.md).  
  
## See Also  
 [What’s New in Visual Studio](../VS_csharp/what-s-new-in-visual-studio-2015.md)   
 [Getting Started with Visual Studio](../VS_csharp/get-started-developing-with-visual-studio.md)   
