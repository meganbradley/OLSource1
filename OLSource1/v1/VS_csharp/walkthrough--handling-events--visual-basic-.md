...
This is the second of two topics that demonstrate how to work with events. The first topic, [Walkthrough: Declaring and Raising Events](../VS_csharp/walkthrough--declaring-and-raising-events--visual-basic-.md), shows how to declare and raise events. This section uses the form and class from that walkthrough to show how to handle events when they take place.  
  
 The `Widget` class example uses traditional event-handling statements. [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] provides other techniques for working with events. As an exercise, you can modify this example to use the `AddHandler` and `Handles` statements.  
  
### To handle the PercentDone event of the Widget class  
  
1.  Place the following code in `Form1`:  
  
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#4](../VS_csharp/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_1.vb)]  
...
3.  Add the following code to the `mWidget_PercentDone` event handler:  
     ...
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#5](../VS_csharp/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_2.vb)]  
...
4.  Add the following code for the `Button2_Click` event handler:  
     ...
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#6](../VS_csharp/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_3.vb)]  
...
3.  Add the following code for the `Form1_Load` event procedure to create the `Widget`:  
     ...
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#7](../VS_csharp/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_4.vb)]  
  
 When this code executes, [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] creates a `Widget` object and connects its events to the event procedures associated with `mWidget`. From that point on, whenever the `Widget` raises its `PercentDone` event, the `mWidget_PercentDone` event procedure is executed.  
  
#### To call the LongTask method  
  
-   Add the following code to the `Button1_Click` event handler:  
  
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#8](../VS_csharp/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_5.vb)]  
...
3.  Click the **Cancel** button to stop the task. Notice that the appearance of the **Cancel** button does not change immediately when you click it. The `Click` event cannot happen until the `My.Application.DoEvents` statement allows event processing.  
    ...
    > [!NOTE]
    >  The `My.Application.DoEvents` method does not process events in exactly the same way as the form does. For example, in this walkthrough, you must click the **Cancel** button twice. To allow the form to handle the events directly, you can use multithreading. For more information, see [Threading (C# and Visual Basic)](../VS_csharp/threading--csharp-and-visual-basic-.md).  
...
-   Add the following line of code to the `Button1_Click` procedure, immediately preceding the line that reads `mWidget.LongTask(12.2, 0.33)`:  
     ...
     [!code[VbVbcnWalkthroughDeclaringAndRaisingEvents#9](../VS_csharp/codesnippet/VisualBasic/walkthrough--handling-events--visual-basic-_6.vb)]  
...
## See Also  
 [Walkthrough: Declaring and Raising Events](../VS_csharp/walkthrough--declaring-and-raising-events--visual-basic-.md)   
