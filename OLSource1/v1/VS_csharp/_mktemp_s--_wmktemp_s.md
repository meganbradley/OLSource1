...
Creates a unique file name. These are versions of [_mktemp, _wmktemp](../VS_csharp/_mktemp--_wmktemp.md) with security enhancements as described in [Security Enhancements in the CRT](../VS_csharp/security-features-in-the-crt.md).  
...
|any|<= number of X's|`EINVAL`|empty string|  
 ...
 If any of the above error conditions occurs, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, `errno` is set to `EINVAL` and the functions returns `EINVAL`.  
...
## See Also  
 ...
 [tmpfile_s](../VS_csharp/tmpfile_s.md)