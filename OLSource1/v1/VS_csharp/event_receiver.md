 ...
 **event_receiver** is used with the [event_source](../VS_csharp/event_source.md) attribute and the [__hook](../VS_csharp/__hook.md) and [__unhook](../VS_csharp/__unhook.md) keywords. Use **event_source** to create event sources. Use `__hook` within an event receiver's methods to associate ("hook") event receiver methods to the events of an event source. Use `__unhook` to dissociate them.  
