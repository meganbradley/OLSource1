...
MFC supports views that scroll and views that are automatically scaled to the size of the frame window that displays them. Class `CScrollView` supports both kinds of views.  
 ...
 For more information about scrolling and scaling, see class [CScrollView](../VS_csharp/cscrollview-class.md) in the *MFC Reference*. For a scrolling example, see the [Scribble sample](../VS_csharp/visual-c---samples.md).  
