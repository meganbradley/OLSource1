...
## Remarks  
 ...
 The comparison behavior of `_mbsncmp` and `_mbsncmp_l` is affected by the setting of the `LC_CTYPE` category setting of the locale. This controls detection of leading and trailing bytes of multibyte characters. For more information, see [setlocale](../VS_csharp/setlocale--_wsetlocale.md). The `_mbsncmp` function uses the current locale for this locale-dependent behavior. The `_mbsncmp_l` function is identical except that it uses the `locale` parameter instead. For more information, see [Locale](../VS_csharp/locale.md). If the locale is a single-byte locale, the behavior of these functions is identical to `strncmp`.  
