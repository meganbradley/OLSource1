...
### Example  
  The following example demonstrates some uses for the `iota` function by filling a [list](../VS_csharp/-list-.md) of integers and then filling a [vector](../VS_csharp/-vector-.md) with the `list` so that the [random_shuffle](../VS_csharp/-algorithm--functions.md#random_shuffle) function can be used.  
