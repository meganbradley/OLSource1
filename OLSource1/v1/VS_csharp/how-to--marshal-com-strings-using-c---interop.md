...
## Example  
 The following example demonstrates how a BSTR (a string format used in COM programming) can be passed from a managed to an unmanaged function. The calling managed function uses <xref:System.Runtime.InteropServices.Marshal.StringToBSTR*> to obtain the address of a BSTR representation of the contents of a .NET System.String. This pointer is pinned using [pin_ptr](../VS_csharp/pin_ptr--c---cli-.md) to ensure that its physical address is not changed during a garbage collection cycle while the unmanaged function executes. The garbage collector is prohibited from moving the memory until the [pin_ptr](../VS_csharp/pin_ptr--c---cli-.md) goes out of scope.  
