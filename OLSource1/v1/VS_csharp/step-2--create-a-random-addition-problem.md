...
> [!NOTE]
>  This topic is part of a tutorial series about basic coding concepts. For an overview of the tutorial, see [Tutorial 2: Create a Timed Math Quiz](../VS_csharp/tutorial-2--create-a-timed-math-quiz.md).  
...
3.  Create a `Random` object by adding a `new` statement near the top of the code, like the following.  
     ...
     [!code[VbExpressTutorial3Step2#1](../VS_csharp/codesnippet/CSharp/step-2--create-a-random-addition-problem_1.cs)]
[!code[VbExpressTutorial3Step2#1](../VS_csharp/codesnippet/VisualBasic/step-2--create-a-random-addition-problem_1.vb)]  
     ...
     Soon you'll build a method to check the answers, so your quiz must use variables to store the random numbers that it generates for each problem. See [Variables in Visual Basic](../VS_csharp/variables-in-visual-basic.md) or [Types (C# Programming Guide](../VS_csharp/types--csharp-programming-guide-.md). To properly use variables, you must declare them, which means listing their names and data types.  
...
4.  Add two integer variables to the form, and name them **addend1** and **addend2**.  
    ...
    >  An integer variable is known as an int in C# or an Integer in Visual Basic. This kind of variable stores a positive or negative number from -2147483648 through 2147483647 and can store only whole numbers, not decimals.  
     ...
     [!code[VbExpressTutorial3Step2#2](../VS_csharp/codesnippet/CSharp/step-2--create-a-random-addition-problem_2.cs)]
[!code[VbExpressTutorial3Step2#2](../VS_csharp/codesnippet/VisualBasic/step-2--create-a-random-addition-problem_2.vb)]  
...
5.  Add a method that's named `StartTheQuiz()` and that uses the `Random` object's `Next()` method to show the random numbers in the labels. `StartTheQuiz()` will eventually fill in all of the problems and then start the timer, so add a comment. The function should look like the following.  
     ...
     [!code[VbExpressTutorial3Step2#3](../VS_csharp/codesnippet/CSharp/step-2--create-a-random-addition-problem_3.cs)]
[!code[VbExpressTutorial3Step2#3](../VS_csharp/codesnippet/VisualBasic/step-2--create-a-random-addition-problem_3.vb)]  
     ...
     ![Next method](../VS_csharp/media/express_randomwhite.png "Express_RandomWhite")  
Next method  
    ...
    >  When you use the `Next()` method with the `Random` object, such as when you call `randomizer.Next(50)`, you get a random number that's less than 50 (from 0 through 49). In this example, you called `randomizer.Next(51)`. You used 51 and not 50 so that the two random numbers will add up to an answer that's from 0 through 100. If you pass 50 to the `Next()` method, it chooses a number from 0 through 49, so the highest possible answer is 98, not 100. After the first two statements in the method run, each of the two integer variables, `addend1` and `addend2`, hold a random number from 0 through 50. This screenshot shows Visual C# code, but IntelliSense works the same way for Visual Basic.  
     ...
     [!code[VbExpressTutorial3Step2#18](../VS_csharp/codesnippet/CSharp/step-2--create-a-random-addition-problem_4.cs)]
[!code[VbExpressTutorial3Step2#18](../VS_csharp/codesnippet/VisualBasic/step-2--create-a-random-addition-problem_4.vb)]  
...
7.  Add the following two statements.  
     ...
     [!code[VbExpressTutorial3Step2#4](../VS_csharp/codesnippet/CSharp/step-2--create-a-random-addition-problem_5.cs)]
[!code[VbExpressTutorial3Step2#4](../VS_csharp/codesnippet/VisualBasic/step-2--create-a-random-addition-problem_5.vb)]  
...
8.  Save your code, run it, and then choose the **Start** button.  
     ...
     ![Random addition problem](../VS_csharp/media/express_additionproblem.png "Express_AdditionProblem")  
...
-   To go to the next tutorial step, see [Step 3: Add a Countdown Timer](../VS_csharp/step-3--add-a-countdown-timer.md).  
  
-   To return to the previous tutorial step, see [Step 1: Create a Project and Add Labels to Your Form](../VS_csharp/step-1--create-a-project-and-add-labels-to-your-form.md).