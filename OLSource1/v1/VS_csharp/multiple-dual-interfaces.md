...
## Exposing a Single IDispatch Interface  
 It is possible to expose multiple dual interfaces on a single object by deriving from two or more specializations of `IDispatchImpl`. However, if you allow clients to query for the `IDispatch` interface, you will need to use the [COM_INTERFACE_ENTRY2](../VS_csharp/com_interface_entry2.md) macro (or [COM_INTERFACE_ENTRY_IID](../VS_csharp/com_interface_entry_iid.md)) to specify which base class to use for the implementation of `IDispatch`.  
  
 [!code[NVC_ATL_COM#23](../VS_csharp/codesnippet/CPP/multiple-dual-interfaces_1.h)]  
