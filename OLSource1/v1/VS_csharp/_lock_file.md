...
## Remarks  
 The `_lock_file` function locks the `FILE` object specified by `file`. The underlying file is not locked by `_lock_file`. Use [_unlock_file](../VS_csharp/_unlock_file.md) to release the lock on the file. Calls to `_lock_file` and `_unlock_file` must be matched in a thread.  
...
## See Also  
 ...
 [_unlock_file](../VS_csharp/_unlock_file.md)