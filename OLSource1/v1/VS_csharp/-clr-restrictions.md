...
-   In a structured exception handler, there are restrictions on using `_alloca` when compiling with **/clr**. For more information, see [_alloca](../VS_csharp/_alloca.md).  
  
-   The use of run-time error checks is not valid with **/clr**. For more information, see [Run-Time Error Checks](../VS_csharp/how-to--use-native-run-time-checks.md).  
...
-   The [vprintf Functions](../VS_csharp/vprintf-functions.md) cannot be called from a program compiled with **/clr**.  
  
-   The [naked](../VS_csharp/naked--c---.md) [__declspec](../VS_csharp/__declspec.md) modifier is ignored under /clr.  
  
-   The translator function set by [_set_se_translator](../VS_csharp/_set_se_translator.md) will affect only catches in unmanaged code. See [Exception Handling](../VS_csharp/exception-handling---c---component-extensions-.md) for more information.  
...
-   The following compiler options are not supported with **/clr**:  
    ...
    -   **/EHsc** and **/EHs** (**/clr** implies **/EHa** (see [/EH (Exception Handling Model)](../VS_csharp/-eh--exception-handling-model-.md))  
  
    -   **/fp:strict** and **/fp:except** (see [/fp (Specify Floating-Point Behavior)](../VS_csharp/-fp--specify-floating-point-behavior-.md))  
  
    -   [/Zd](../VS_csharp/-z7---zi---zi--debug-information-format-.md)  
  
    -   [/Gm](../VS_csharp/-gm--enable-minimal-rebuild-.md)  
  
    -   [/MT](../VS_csharp/-md---mt---ld--use-run-time-library-.md)  
  
    -   [/RTC](../VS_csharp/-rtc--run-time-error-checks-.md)  
...
-   [/J](../VS_csharp/-j--default-char-type-is-unsigned-.md) is not supported with **/clr:safe** or **/clr:pure**.  
...
-   When using **/Zi** with **/clr**, there are performance implications. For more information, see [/Zi](../VS_csharp/-z7---zi---zi--debug-information-format-.md).  
  
-   Passing a wide character to a .NET Framework output routine without also specifying [/Zc:wchar_t](../VS_csharp/-zc-wchar_t--wchar_t-is-native-type-.md) or without casting the character to `__wchar_t` will cause the output to appear as an `unsigned short int`. For example:  
...
-   [/GS](../VS_csharp/-gs--buffer-security-check-.md) is ignored when compiling with **/clr**, unless a function is under `#pragma` [unmanaged](../VS_csharp/managed--unmanaged.md) or if the function must be compiled to native, in which case the compiler will generate warning C4793, which is off by default.  
  
-   See [/ENTRY](../VS_csharp/-entry--entry-point-symbol-.md) for function signature requirements of a managed application.  
  
-   Applications compiled with **/openmp** and **/clr** can only be run in a single appdomain process.  See [/openmp (Enable OpenMP 2.0 Support)](../VS_csharp/-openmp--enable-openmp-2.0-support-.md) for more information.  
...
-   When using the va_arg macro, you may get unexpected results when compiling with **/clr:pure**.  For more information, see [va_arg, va_end, va_start](../VS_csharp/va_arg--va_copy--va_end--va_start.md).  
  
-   If your application passes an argument of type [va_list](../VS_csharp/va_arg--va_copy--va_end--va_start.md) to a function declared to take a [variable number of arguments](../Topic/Variable%20Argument%20Lists.md), and your application is compiled with **/clr:pure**, the CLR throws <xref:System.NotSupportedException*>. If **/clr**  is used instead, the affected functions are compiled to native code and execute properly. If **/clr:safe** is used, an error diagnostic is emitted.  
...
-   You cannot use the [Compiler COM Support](../VS_csharp/compiler-com-support.md) classes with **/clr:pure** or **/clr:safe**.  
