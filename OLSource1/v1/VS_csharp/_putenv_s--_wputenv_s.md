...
Creates, modifies, or removes environment variables. These are versions of [_putenv, _wputenv](../VS_csharp/_putenv--_wputenv.md) but have security enhancements, as described in [Security Enhancements in the CRT](../VS_csharp/security-features-in-the-crt.md).  
...
|any|`NULL`|`EINVAL`|  
 ...
 If one of the error conditions occurs, these functions invoke an invalid parameter handler, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, these functions return `EINVAL` and set `errno` to `EINVAL`.  
...
## Example  
 For a sample that shows how to use `_putenv_s`, see [getenv_s, _wgetenv_s](../VS_csharp/getenv_s--_wgetenv_s.md).  
...
## See Also  
 ...
 [_searchenv, _wsearchenv](../VS_csharp/_searchenv--_wsearchenv.md)