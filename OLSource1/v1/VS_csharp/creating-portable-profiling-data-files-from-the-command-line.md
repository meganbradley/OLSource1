...
To make sharing of profiling data easier, you can use the [VSPerfReport](../VS_csharp/vsperfreport.md) command-line tool to embed the symbols for a profiling run into the .vsp file.  
  
 You can also create a pre-analyzed profiling data (.vsps) file that is smaller and is quicker to load in the IDE.  
  
> [!NOTE]
>  Make sure the symbol (.pdb) files are available to **VSPerfReport**. For more information, see [How to: Specify Symbol File Locations from the Command Line](../VS_csharp/how-to--specify-symbol-file-locations-from-the-command-line.md).  
>   
>  For information about the path to **VSReport**, see [Specifying the Path to Profiling Tools Command Line Tools](../VS_csharp/specifying-the-path-to-profiling-tools-command-line-tools.md).  
>   
