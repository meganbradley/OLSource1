...
## Remarks  
 ...
 Object code that's generated when `declarator` is defined is put in the text segment specified by `segname`, which is a narrow-string literal. The name `segname` does not have to be specified in a [section](../VS_csharp/section.md) pragma before it can be used in a declaration. By default, when no `code_seg` is specified, object code is put in a segment named .text. A `code_seg` attribute overrides any existing [#pragma code_seg](../VS_csharp/code_seg.md) directive. A `code_seg` attribute applied to a member function overrides any `code_seg` attribute applied to the enclosing class.  
 ...
 Lambdas inherit `code_seg` attributes from their enclosing scope. To specify a segment for a lambda, apply a `code_seg` attribute after the parameter-declaration clause and before any mutable or exception specification, any trailing return-type specification, and the lambda body. For more information, see [Lambda Expression Syntax](../VS_csharp/lambda-expression-syntax.md). This example defines a lambda in a segment named PagedMem:  
