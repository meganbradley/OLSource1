...
## Example  
 The PPL provides a programming model that resembles the Standard Template Library (STL). The following example demonstrates many features of the PPL. It computes several Fibonacci numbers serially and in parallel. Both computations act on a [std::array](../VS_csharp/array-class--stl-.md) object. The example also prints to the console the time that is required to perform both computations.  
  
 The serial version uses the STL [std::for_each](../Topic/for_each.md) algorithm to traverse the array and stores the results in a [std::vector](../VS_csharp/vector-class.md) object. The parallel version performs the same task, but uses the PPL [concurrency::parallel_for_each](../VS_csharp/parallel_for_each-function.md) algorithm and stores the results in a [concurrency::concurrent_vector](../VS_csharp/concurrent_vector-class.md) object. The `concurrent_vector` class enables each loop iteration to concurrently add elements without the requirement to synchronize write access to the container.  
 ...
 [!code[concrt-parallel-fibonacci#1](../VS_csharp/codesnippet/CPP/parallel-patterns-library--ppl-_1.cpp)]  
...
|Title|Description|  
|-----------|-----------------|  
|[Task Parallelism](../VS_csharp/task-parallelism--concurrency-runtime-.md)|Describes the role of tasks and task groups in the PPL.|  
|[Parallel Algorithms](../VS_csharp/parallel-algorithms.md)|Describes how to use parallel algorithms such as `parallel_for` and `parallel_for_each`.|  
|[Parallel Containers and Objects](../VS_csharp/parallel-containers-and-objects.md)|Describes the various parallel containers and objects that are provided by the PPL.|  
|[Cancelation](../VS_csharp/cancellation-in-the-ppl.md)|Explains how to cancel the work that is being performed by a parallel algorithm.|  
