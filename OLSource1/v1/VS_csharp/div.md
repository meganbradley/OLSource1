...
## Remarks  
 The `div` function divides `numer` by `denom` and thereby computes the quotient and the remainder. The [div_t](../VS_csharp/standard-types.md) structure contains the quotient, `int``quot`, and the remainder, `int``rem`. The sign of the quotient is the same as that of the mathematical quotient. Its absolute value is the largest integer that is less than the absolute value of the mathematical quotient. If the denominator is 0, the program terminates with an error message.  
  
 The overloads that take arguments of type `long` or `long long` are only available to C++ code. The return type [ldiv_t](../VS_csharp/standard-types.md) contains the members `long``quot` and `long``rem`, and the return type [lldiv_t](../VS_csharp/standard-types.md) contains the members `long long quot` and `long long rem`, which have the same meanings as the members of `div_t`.  
