...
|`m_BoxSize`|A [CSize](../VS_csharp/csize-class.md) object that specifies the size of a cell in a color bar grid.|  
...
|`m_bStdColorDlg`|A Boolean that indicates whether to show the standard system color dialog box or the [CMFCColorDialog](../VS_csharp/cmfccolordialog-class.md) dialog box. For more information, see [CMFCColorBar::EnableOtherButton](#cmfccolorbar__enableotherbutton).|  
...
|`m_ColorNames`|An [CMap](../VS_csharp/cmap-class.md) object that associates a set of RGB colors with their names.|  
|`m_colors`|A [CArray](../VS_csharp/carray-class.md) of                                         [COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449) values that contains the colors that are displayed in the color bar control.|  
...
|`m_lstDocColors`|A [CList](../VS_csharp/clist-class.md) of                                         [COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449) values that contains the colors that are currently used in a document.|  
...
|`m_Palette`|A [CPalette](../VS_csharp/cpalette-class.md) of the colors that are used in the color bar control.|  
|`m_pParentBtn`|A pointer to a [CMFCColorButton](../VS_csharp/cmfccolorbutton-class.md) object that is the parent of the current button. This value is significant if the color button is in a hierarchy of toolbar controls or is in a color property grid control.|  
|`m_pParentRibbonBtn`|A pointer to a [CMFCRibbonColorButton](../VS_csharp/cmfcribboncolorbutton-class.md) object that is on the ribbon and is the parent button of the current button. This value is significant if the color button is in a hierarchy of toolbar controls or is in a color property grid control.|  
|`m_pWndPropList`|A pointer to a [CMFCPropertyGridCtrl](../VS_csharp/cmfcpropertygridctrl-class.md) object.|  
|`m_strAutoColor`|A [CString](../VS_csharp/cstringt-class.md) that is the text that is displayed on the **Automatic** button. For more information, see [CMFCColorBar::EnableAutomaticButton](#cmfccolorbar__enableautomaticbutton).|  
|`m_strDocColors`|A [CString](../VS_csharp/cstringt-class.md) that is the text that is displayed on the document colors button. For more information, see [CMFCColorBar::SetDocumentColors](#cmfccolorbar__setdocumentcolors).|  
|`m_strOtherColor`|A [CString](../VS_csharp/cstringt-class.md) that is the text that is displayed on the *other* button. For more information, see [CMFCColorBar::EnableOtherButton](#cmfccolorbar__enableotherbutton).|  
  
## Remarks  
 Usually, you do not create a `CMFCColorBar` object directly. Instead, the [CMFCColorMenuButton](../VS_csharp/cmfccolormenubutton-class.md) (used in menus and toolbars) or the [CMFCColorButton](../VS_csharp/cmfccolorbutton-class.md) creates the `CMFCColorBar` object.  
...
-   Uses the [CMFCColorPickerCtrl](../VS_csharp/cmfccolorpickerctrl-class.md) control to select a custom color.  
  
-   Supports a "tear-off" state (if it is created by using the [CMFCColorMenuButton](../VS_csharp/cmfccolormenubutton-class.md)).  
...
2.  In your frame window class, override the [CFrameWndEx::OnShowPopupMenu](../VS_csharp/cframewndex-class.md#cframewndex__onshowpopupmenu) method and replace the regular menu button with a [CMFCColorMenuButton](../VS_csharp/cmfccolormenubutton-class.md) object (by calling [CMFCToolBar::ReplaceButton](../VS_csharp/cmfctoolbar-class.md#cmfctoolbar__replacebutton)).  
  
3.  Set all the styles and enable or disable the features of the `CMFCColorBar` object during [CMFCColorMenuButton](../VS_csharp/cmfccolormenubutton-class.md) creation. The `CMFCColorMenuButton` object dynamically creates the `CMFCColorBar` object after the framework calls the `CreatePopupMenu` method.  
  
 When the user clicks a color bar control button, the framework uses the `ON_COMMAND` macro to notify the parent of the color bar control. In the macro, the command ID parameter is the value that you assigned to the color bar control button in step 1 (ID_CHAR_COLOR in this example). For more information, see the [CMFCColorMenuButton](../VS_csharp/cmfccolormenubutton-class.md), [CMFCColorButton](../VS_csharp/cmfccolorbutton-class.md), [CMFCColorPickerCtrl](../VS_csharp/cmfccolorpickerctrl-class.md), [CMFCFrameWnd](../VS_csharp/cframewndex-class.md), and [CMFCToolBar](../VS_csharp/cmfctoolbar-class.md) classes.  
  
## Example  
 The following example demonstrates how to configure a color bar by using various methods in the `CMFCColorBar` class. The methods set the horizontal and vertical margins, enable the other button, create a color bar control window, and sets the currently selected color. This example is part of the [New Controls sample](../VS_csharp/visual-c---samples.md).  
  
 [!code[NVC_MFC_NewControls#1](../VS_csharp/codesnippet/CPP/cmfccolorbar-class_1.h)]  
[!code[NVC_MFC_NewControls#2](../VS_csharp/codesnippet/CPP/cmfccolorbar-class_2.cpp)]  
...
## Inheritance Hierarchy  
 ...
 [CMFCColorBar](../VS_csharp/cmfccolorbar-class.md)  
...
### Parameters  
 ...
 [in] `bAltColorDlg`  
 `TRUE` to display the [CMFCColorDialog](../VS_csharp/cmfccolordialog-class.md) dialog box; `FALSE` to display the standard [CColorDialog](../VS_csharp/ccolordialog-class.md) dialog box. The default value is `TRUE`.  
...
### Return Value  
 A [CSize](../VS_csharp/csize-class.md) object whose `cx` component contains the number of columns and whose `cy` component contains the number of rows.  
