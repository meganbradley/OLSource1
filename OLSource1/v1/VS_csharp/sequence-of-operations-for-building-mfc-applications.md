...
|Add [dialog boxes](../VS_csharp/dialog-boxes.md).|Design dialog-template resources with the dialog editor. Then create a dialog class and the code that handles the dialog box.|The framework manages the dialog box and facilitates retrieving information entered by the user.|  
...
|Implement your view class.|Implement one or more view classes corresponding to your documents. Implement the view's member functions that you mapped to the user interface with Class View. A variety of [CView](../VS_csharp/cview-class.md)-derived classes are available, including [CListView](../VS_csharp/clistview-class.md) and [CTreeView](../VS_csharp/ctreeview-class.md).|The framework manages most of the relationship between a document and its view. The view's member functions access the view's document to render its image on the screen or printed page and to update the document's data structures in response to user editing commands.|  
...
|Add scrolling.|If you need to support scrolling, derive your view class or classes from [CScrollView](../VS_csharp/cscrollview-class.md).|The view automatically adds scroll bars when the view window becomes too small.|  
|Create form views.|If you want to base your views on dialog-template resources, derive your view class or classes from [CFormView](../VS_csharp/cformview-class.md).|The view uses the dialog-template resource to display controls. The user can tab from control to control in the view.|  
|Create database forms.|If you want a form-based data-access application, derive your view class from [CRecordView](../VS_csharp/crecordview-class.md) (for ODBC programming).|The view works like a form view, but its controls are connected to the fields of a [CRecordset](../VS_csharp/crecordset-class.md) object representing a database table. MFC moves data between the controls and the recordset for you.|  
|Create a simple text editor.|If you want your view to be a simple text editor, derive your view class or classes from [CEditView](../VS_csharp/ceditview-class.md) or [CRichEditView](../VS_csharp/cricheditview-class.md).|The view provides editing functions, Clipboard support, and file input/output. `CRichEditView` provides styled text.|  
|Add splitter windows.|If you want to support window splitting, add a [CSplitterWnd](../VS_csharp/csplitterwnd-class.md) object to your SDI frame window or MDI child window and hook it up in the window's [OnCreateClient](../Topic/CFrameWnd::OnCreateClient.md) member function.|The framework supplies splitter-box controls next to the scroll bars and manages splitting your view into multiple panes. If the user splits a window, the framework creates and attaches additional view objects to the document.|  
...
## See Also  
 [Sequence of Operations for Creating OLE Applications](../VS_csharp/sequence-of-operations-for-creating-ole-applications.md)   
 [Sequence of Operations for Creating ActiveX Controls](../VS_csharp/sequence-of-operations-for-creating-activex-controls.md)   
 [Sequence of Operations for Creating Database Applications](../VS_csharp/sequence-of-operations-for-creating-database-applications.md)   
