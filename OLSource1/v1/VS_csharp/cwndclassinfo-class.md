...
|Macro|Description|  
|-----------|-----------------|  
|[DECLARE_WND_CLASS](../VS_csharp/declare_wnd_class.md)|`CWndClassInfo` registers information for a new window class.|  
|[DECLARE_WND_CLASS_EX](../VS_csharp/declare_wnd_class_ex.md)|`CWndClassInfo` registers information for a new window class, including the class parameters.|  
|[DECLARE_WND_SUPERCLASS](../VS_csharp/declare_wnd_superclass.md)|`CWndClassInfo` registers information for a window class that is based on an existing class but uses a different window procedure. This technique is called superclassing.|  
  
 By default, [CWindowImpl](../VS_csharp/cwindowimpl-class.md) includes the `DECLARE_WND_CLASS` macro to create a window based on a new window class. DECLARE_WND_CLASS provides default styles and background color for the control. If you want to specify the style and background color yourself, derive your class from `CWindowImpl` and include the `DECLARE_WND_CLASS_EX` macro in your class definition.  
 ...
 [!code[NVC_ATL_Windowing#43](../VS_csharp/codesnippet/CPP/cwndclassinfo-class_1.h)]  
  
 For more information about window classes, see [Window Classes](http://msdn.microsoft.com/library/windows/desktop/ms632596) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
...
### Remarks  
 ...
 `CWndClassInfo` uses `m_bSystemCursor` only when the [DECLARE_WND_CLASS](../VS_csharp/declare_wnd_class.md) (the default in [CWindowImpl](../VS_csharp/cwindowimpl-class.md)) or the [DECLARE_WND_CLASS_EX](../VS_csharp/declare_wnd_class_ex.md) macro is specified. In this case, `m_bSystemCursor` is initialized to **TRUE**. For more information, see the [CWndClassInfo](../VS_csharp/cwndclassinfo-class.md) overview.  
...
### Remarks  
 ...
 `CWndClassInfo` uses `m_lpszCursorID` only when the [DECLARE_WND_CLASS](../VS_csharp/declare_wnd_class.md) (the default in [CWindowImpl](../VS_csharp/cwindowimpl-class.md)) or the [DECLARE_WND_CLASS_EX](../VS_csharp/declare_wnd_class_ex.md) macro is specified. In this case, `m_lpszCursorID` is initialized to **IDC_ARROW**. For more information, see the [CWndClassInfo](../VS_csharp/cwndclassinfo-class.md) overview.  
...
### Remarks  
 `CWndClassInfo` uses `m_lpszOrigName` only when you include the [DECLARE_WND_SUPERCLASS](../VS_csharp/declare_wnd_superclass.md) macro in your class definition. In this case, `CWndClassInfo` registers a window class based on the class named by `m_lpszOrigName`. For more information, see the [CWndClassInfo](../VS_csharp/cwndclassinfo-class.md) overview.  
...
### Remarks  
 `CWndClassInfo` uses `m_szAutoName` only if **NULL** is passed for the `WndClassName` parameter to [DECLARE_WND_CLASS](../VS_csharp/declare_wnd_class.md), the [DECLARE_WND_CLASS_EX](../VS_csharp/declare_wnd_class_ex.md) or [DECLARE_WND_SUPERCLASS](../VS_csharp/declare_wnd_superclass.md). ATL will construct a name when the window class is registered.  
...
### Remarks  
 `CWndClassInfo` uses `pWndProc` only when you include the [DECLARE_WND_SUPERCLASS](../VS_csharp/declare_wnd_superclass.md) macro in your class definition. In this case, `CWndClassInfo` registers a window class that is based on an existing class but uses a different window procedure. The existing window class's window procedure is saved in `pWndProc`. For more information, see the [CWndClassInfo](../VS_csharp/cwndclassinfo-class.md) overview.  
...
### Remarks  
 If you have specified the [DECLARE_WND_CLASS](../VS_csharp/declare_wnd_class.md) (the default in [CWindowImpl](../VS_csharp/cwindowimpl-class.md)) or the [DECLARE_WND_CLASS_EX](../VS_csharp/declare_wnd_class_ex.md) macro, `Register` registers a new window class. In this case, the `pProc` parameter is not used.  
  
 If you have specified the [DECLARE_WND_SUPERCLASS](../VS_csharp/declare_wnd_superclass.md) macro, `Register` registers a superclass — a window class that is based on an existing class but uses a different window procedure. The existing window class's window procedure is returned in `pProc`.  
