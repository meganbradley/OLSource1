...
## Remarks  
 The *cast-expression* argument must be a pointer to a block of memory previously allocated for an object created with the [new operator](../VS_csharp/new-operator--c---.md). The **delete** operator has a result of type `void` and therefore does not return a value. For example:  
...
## Using delete  
 There are two syntactic variants for the [delete operator](../VS_csharp/delete-operator--c---.md): one for single objects and the other for arrays of objects. The following code fragment shows how these differ:  
