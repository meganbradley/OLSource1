---
title: "Using CReBarCtrl"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "CReBarCtrl"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "child windows, rebar controls"
  - "combo boxes, in rebar controls"
  - "rebar controls"
ms.assetid: 2c0aeec2-ffc3-44b8-97b5-0f56e116a338
caps.latest.revision: 12
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# Using CReBarCtrl
A rebar control acts as a container for child windows. These child windows, often other controls, are assigned to a rebar control band. A rebar control can contain one or more bands, with each band having any combination of a gripper bar, a bitmap, a text label, and a child window. However, bands cannot contain more than one child window.  
  
 The following illustration shows a rebar control that has two bands. One contains a gripper bar, a text label ("Address"), and a combo box child window. The other band contains a gripper bar, a text label, and a flat toolbar (implemented with a child window).  
  
 ![Rebar control that has two bands](../VS_csharp/media/vc4ruz1.gif "vc4RUZ1")  
  
## What do you want to know more about?  
  
-   [CReBar vs. CReBarCtrl](../VS_csharp/crebar-vs.-crebarctrl.md)  
  
-   [Creating a Rebar Control](../VS_csharp/creating-a-rebar-control.md)  
  
-   [Rebar Controls and Bands](../VS_csharp/rebar-controls-and-bands.md)  
  
-   [Using an Image List with a Rebar Control](../VS_csharp/using-an-image-list-with-a-rebar-control.md)  
  
-   [Using a Dialog Bar with a Rebar Control](../VS_csharp/using-a-dialog-bar-with-a-rebar-control.md)  
  
-   [Processing Notification Messages in a Rebar Control](../VS_csharp/processing-notification-messages-in-a-rebar-control.md)  
  
## See Also  
 [Controls (MFC)](../VS_csharp/controls--mfc-.md)