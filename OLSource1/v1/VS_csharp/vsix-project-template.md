...
> [!NOTE]
>  To use VSIX projects, you must install the Visual Studio SDK. For more information about the Visual Studio SDK, see [Visual Studio SDK](../VS_csharp/visual-studio-sdk.md).  
...
## Packaging an Extension in an Empty VSIX Project  
 You can package an existing extension, or an extension that does not already have VSIX support, by wrapping it in an empty VSIX project. The extension to be wrapped must be of a type that is supported by the [VSIX schema](../VS_csharp/vsix-extension-schema-2.0-reference.md).  
...
8.  Build the solution.  
     ...
     [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] builds a .vsix file that contains a VSIX manifest file, a [Content_Types].xml file, and all of the extension assets that you added to the project.  
  
## See Also  
 [VSX Schema for Extensions](../VS_csharp/vsix-extension-schema-2.0-reference.md)   
 [Managing Visual Studio Extensions](../VS_csharp/finding-and-using-visual-studio-extensions.md)