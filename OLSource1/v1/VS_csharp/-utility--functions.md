...
### Remarks  
 `make_pair` converts object of type [reference_wrapper](../VS_csharp/reference_wrapper-class.md) to reference types and converts decaying arrays and functions to pointers.  
...
-   Otherwise, the returned type `Type1` is `decay<Type1>::type`. If [decay](../VS_csharp/decay-class.md) is not supported, the returned type `Type1` is the same as the input type `Type1`.  
...
### Example  
  For an example about how to use the helper function `make_pair` to declare and initialize a pair, see [pair](../VS_csharp/pair-structure.md).  
...
##  <a name="swap"></a>  swap  
 Exchanges the elements of two [pair](../VS_csharp/pair-structure.md) objects.  
