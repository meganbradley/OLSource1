...
-   Visual Basic--[How to: Specify Build Events](../VS_csharp/how-to--specify-build-events--visual-basic-.md).  
  
-   Visual C# and F#--[How to: Specify Build Events (C#)](../VS_csharp/how-to--specify-build-events--csharp-.md).  
  
-   Visual C++--[Specifying Build Events](../VS_csharp/specifying-build-events.md).  
  
## Syntax  
 Build events follow the same syntax as DOS commands, but you can use macros to create build events more easily. For a list of available macros, see [Pre-build Event/Post-build Event Command Line Dialog Box](../VS_csharp/pre-build-event-post-build-event-command-line-dialog-box.md).  
...
-   Enclose file paths in quotation marks.  
     ...
     Example (for [!INCLUDE[win8](../VS_csharp/includes/win8_md.md)]): "%ProgramFiles(x86)%\Microsoft SDKs\Windows\v8.0A\Bin\NETFX 4.0 Tools\gacutil.exe" -if "$(TargetPath)"  
...
## See Also  
 ...
 [Pre-build Event/Post-build Event Command Line Dialog Box](../VS_csharp/pre-build-event-post-build-event-command-line-dialog-box.md)   
 [MSBuild Special Characters](../VS_csharp/msbuild-special-characters.md)   
