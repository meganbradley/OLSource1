...
## Return Value  
 ...
 If either `timeZoneName` is `NULL`, or `sizeInBytes` is zero or less than zero (but not both), an invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, this function sets `errno` to `EINVAL` and returns `EINVAL`.  
...
## See Also  
 ...
 [Global Variables _doserrno, errno, _sys_errlist, and _sys_nerr](../VS_csharp/errno--_doserrno--_sys_errlist--and-_sys_nerr.md)   
 [_get_daylight](../VS_csharp/_get_daylight.md)   
 [_get_dstbias](../VS_csharp/_get_dstbias.md)   
 [_get_timezone](../VS_csharp/_get_timezone.md)   
 [TZNAME_MAX](../VS_csharp/tzname_max.md)