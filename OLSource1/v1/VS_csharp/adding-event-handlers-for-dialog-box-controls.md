...
2.  In the [Properties window](../VS_csharp/properties-window.md), click the **ControlEvents** button to display the list of common Windows events associated with the control. For example, the standard **OK** button on the **About** dialog box lists the following notification events:  
...
3.  In the **Properties** window, click the right column next to the event to handle, and then select the suggested notification event name (for example, **OnBnClickedOK** handles **BN_CLICKED**).  
 ...
 If you want to add the event handler to a class other than the one implementing the dialog box, use the [Event Handler wizard](../VS_csharp/event-handler-wizard.md). For more information, see [Adding an Event Handler](../VS_csharp/adding-an-event-handler--visual-c---.md).  
...
## See Also  
 [Default Control Events](../VS_csharp/default-control-events.md)   
 [Defining Member Variables for Dialog Controls](../VS_csharp/defining-member-variables-for-dialog-controls.md)   
