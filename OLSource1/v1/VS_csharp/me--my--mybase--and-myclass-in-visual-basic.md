...
`Me`, `My`, `MyBase`, and `MyClass` in [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] have similar names, but different purposes. This topic describes each of these entities in order to distinguish them.  
...
## My  
 The `My` feature provides easy and intuitive access to a number of [!INCLUDE[dnprdnshort](../VS_csharp/includes/dnprdnshort_md.md)] classes, enabling the [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] user to interact with the computer, application, settings, resources, and so on.  
