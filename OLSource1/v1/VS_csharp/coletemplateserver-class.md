...
## Remarks  
 ...
 For more information about servers, see the article [Servers: Implementing a Server](../VS_csharp/servers--implementing-a-server.md).  
...
### Remarks  
 For a brief description of the use of the `COleTemplateServer` class, see the [COleLinkingDoc](../VS_csharp/colelinkingdoc-class.md) class overview.  
  
##  <a name="coletemplateserver__connecttemplate"></a>  COleTemplateServer::ConnectTemplate  
 Connects the document template pointed to by `pDocTemplate` to the underlying [COleObjectFactory](../VS_csharp/coleobjectfactory-class.md) object.  
...
### Remarks  
 The registration information is loaded by means of a call to [CDocTemplate::GetDocString](../VS_csharp/cdoctemplate-class.md#cdoctemplate__getdocstring). The substrings retrieved are those identified by the indexes **regFileTypeId**, **regFileTypeName**, and **fileNewName**, as described in the `GetDocString` reference pages.  
 ...
 The information in the arguments `rglpszRegister` and `rglpszOverwrite` is written to the registry through a call to [AfxOleRegisterServerClass](../VS_csharp/afxoleregisterserverclass.md). The default information, which is registered when the two arguments are **NULL**, is suitable for most applications. For information on the structure of the information in these arguments, see `AfxOleRegisterServerClass`.  
...
## See Also  
 ...
 [Base Class](../VS_csharp/coleobjectfactory-class.md)   
