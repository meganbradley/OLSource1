...
Appends a string. More secure versions of these functions are available; see [strcat_s, wcscat_s, _mbscat_s](../VS_csharp/strcat_s--wcscat_s--_mbscat_s.md).  
...
> [!IMPORTANT]
>  Because `strcat` does not check for sufficient space in `strDestination` before appending `strSource`, it is a potential cause of buffer overruns. Consider using [strncat](../VS_csharp/strncat--_strncat_l--wcsncat--_wcsncat_l--_mbsncat--_mbsncat_l.md) instead.  
...
## Example  
 See the example for [strcpy](../VS_csharp/strcpy--wcscpy--_mbscpy.md).  
