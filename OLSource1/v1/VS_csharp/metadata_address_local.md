...
## Remarks  
 This structure is part of the union in the [DEBUG_ADDRESS_UNION](../VS_csharp/debug_address_union.md) structure when the `dwKind` field of the `DEBUG_ADDRESS_UNION` structure is set to `ADDRESS_KIND_LOCAL` (a value from the [ADDRESS_KIND](../VS_csharp/address_kind.md) enumeration).  
  
 `Warning:` [C++ only]  If `pLocal` is not null, then you must call `Release` on the token pointer (`addr` is a field in the [DEBUG_ADDRESS](../VS_csharp/debug_address.md) structure):  
