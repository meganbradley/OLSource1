...
## Remarks  
 An object of type `error_code` class stores an error code value and a pointer to an object that represents a [category](../VS_csharp/error_category-class.md) of error codes that describe reported low-level system errors.  
...
### Return Value  
 The [error_condition](../VS_csharp/error_condition-class.md) specified by [default_error_condition](../VS_csharp/error_category-class.md#error_category__default_error_condition).  
...
### Remarks  
 The member operator stores `(value_type)_Errcode` as the error code value and a pointer to the [generic_category](../VS_csharp/-system_error--functions.md#generic_category). It returns `*this`.  
