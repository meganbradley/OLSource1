...
6.  In the **Run the post-build event** box, specify under what conditions to run the post-build event.  
    ...
    > [!NOTE]
    >  To add lengthy syntax, or to select any build macros from the [Pre-build Event/Post-build Event Command Line Dialog Box](../VS_csharp/pre-build-event-post-build-event-command-line-dialog-box.md), click the Ellipsis button (**…**) to display an edit box.  
...
## See Also  
 [Build Events Page, Project Designer (C#)](../VS_csharp/build-events-page--project-designer--csharp-.md)   
