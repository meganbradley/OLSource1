...
---
# fopen, _wfopen
Opens a file. More-secure versions of these functions that perform additional parameter validation and return error codes are available; see [fopen_s, _wfopen_s](../VS_csharp/fopen_s--_wfopen_s.md).  
...
## Return Value  
 Each of these functions returns a pointer to the open file. A null pointer value indicates an error. If `filename` or `mode` is `NULL` or an empty string, these functions trigger the invalid parameter handler, which is described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, these functions return `NULL` and set `errno` to `EINVAL`.  
  
 For more information, see [errno, _doserrno, _sys_errlist, and _sys_nerr](../VS_csharp/errno--_doserrno--_sys_errlist--and-_sys_nerr.md).  
...
## Remarks  
 ...
 Always check the return value to see whether the pointer is NULL before you perform any additional operations on the file. If an error occurs, the global variable `errno` is set and may be used to obtain specific error information. For more information, see [errno, _doserrno, _sys_errlist, and _sys_nerr](../VS_csharp/errno--_doserrno--_sys_errlist--and-_sys_nerr.md).  
...
|`_tfopen`|`fopen`|`fopen`|`_wfopen`|  
 ...
 If `t` or `b` is not given in `mode`, the default translation mode is defined by the global variable [_fmode](../VS_csharp/_fmode.md). If `t` or `b` is prefixed to the argument, the function fails and returns `NULL`.  
  
 For more information about how to use text and binary modes in Unicode and multibyte stream-I/O, see [Text and Binary Mode File I/O](../VS_csharp/text-and-binary-mode-file-i-o.md) and [Unicode Stream I/O in Text and Binary Modes](../VS_csharp/unicode-stream-i-o-in-text-and-binary-modes.md).  
  
 `c`  
 Enable the commit flag for the associated `filename` so that the contents of the file buffer are written directly to disk if either `fflush` or `_flushall` is called.  
  
 `n`  
 Reset the commit flag for the associated `filename` to "no-commit." This is the default. It also overrides the global commit flag if you link your program with COMMODE.OBJ. The global commit flag default is "no-commit" unless you explicitly link your program with COMMODE.OBJ (see [Link options](../VS_csharp/link-options.md)).  
 ...
 Valid characters for the `mode` string that is used in `fopen` and `_fdopen` correspond to `oflag` arguments that are used in [_open](../VS_csharp/_open--_wopen.md) and [_sopen](../VS_csharp/_sopen--_wsopen.md), as follows.  
...
|`_wfopen`|<stdio.h> or <wchar.h>|  
 ...
 `_wfopen` is a Microsoft extension. For more information about compatibility, see [Compatibility](../VS_csharp/compatibility.md).  
...
## See Also  
 ...
 [_sopen, _wsopen](../VS_csharp/_sopen--_wsopen.md)