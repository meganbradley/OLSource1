...
After you [insert a control](../VS_csharp/inserting-the-control-into-a-visual-c---application.md) and generate one or more [wrapper classes](../VS_csharp/wrapper-classes.md), you can invoke the control's methods and program the control's event handlers.  
  
 The control's [wrapper classes](../VS_csharp/wrapper-classes.md) specify the functions you can use to modify the run-time behavior of the control. Include the appropriate wrapper class header file and use the methods. To set a property, look for an accessor method with the property name prefixed by Set. To retrieve a property, look for an accessor method with the property name prefixed by Get. Event handlers can be written later.  
  
 Because the controls are implemented using Automation, the types passed can only be Automation-safe types such as BSTR and VARIANT. While you can use system calls to allocate and set BSTRs and VARIANTs, you might want to use the ATL wrapper classes ([CComBSTR](../VS_csharp/ccombstr-class.md), [CComVariant](../VS_csharp/ccomvariant-class.md)), the Visual C++ COM Compiler support wrapper classes ([_bstr_t](../VS_csharp/_bstr_t-class.md), [_variant_t](../VS_csharp/_variant_t-class.md)), or the MFC wrapper class ([COleVariant](../VS_csharp/colevariant-class.md)).  
 ...
 If you want to use ADO and RDO directly, you should connect to the ADO or RDO DLLs directly (Msado15.dll or Msrdo20.dll), either with the [compiler COM support classes](../VS_csharp/compiler-com-support-classes.md), which support the [#import directive](../VS_csharp/preprocessor-directives.md), or with the respective SDK.  
