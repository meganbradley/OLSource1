...
1.  Create a new `TextFieldParser`. The following code creates the `TextFieldParser` named `MyReader` and opens the file `test.txt`.  
     ...
     [!code[VbFileIORead#15](../VS_csharp/codesnippet/VisualBasic/how-to--read-from-comma-delimited-text-files-in-visual-basic_1.vb)]  
...
2.  Define the `TextField` type and delimiter. The following code defines the `TextFieldType` property as `Delimited` and the delimiter as ",".  
     ...
     [!code[VbFileIORead#16](../VS_csharp/codesnippet/VisualBasic/how-to--read-from-comma-delimited-text-files-in-visual-basic_2.vb)]  
...
3.  Loop through the fields in the file. If any lines are corrupt, report an error and continue parsing. The following code loops through the file, displaying each field in turn and reporting any fields that are formatted incorrectly.  
     ...
     [!code[VbFileIORead#17](../VS_csharp/codesnippet/VisualBasic/how-to--read-from-comma-delimited-text-files-in-visual-basic_3.vb)]  
  
4.  Close the `While` and `Using` blocks with `End While` and `End Using`.  
  
     [!code[VbFileIORead#18](../VS_csharp/codesnippet/VisualBasic/how-to--read-from-comma-delimited-text-files-in-visual-basic_4.vb)]  
  
## Example  
 This example reads from the file `test.txt`.  
  
 [!code[VbFileIORead#19](../VS_csharp/codesnippet/VisualBasic/how-to--read-from-comma-delimited-text-files-in-visual-basic_5.vb)]  
...
## See Also  
 ...
 [How to: Read From a Fixed-width Text File in Visual Basic](../VS_csharp/how-to--read-from-fixed-width-text-files-in-visual-basic.md)   
 [How to: Read From a Text File with Multiple Formats in Visual Basic](../VS_csharp/how-to--read-from-text-files-with-multiple-formats-in-visual-basic.md)   
 ...
 [Troubleshooting: Reading from and Writing to Text Files](../VS_csharp/troubleshooting--reading-from-and-writing-to-text-files--visual-basic-.md)   
