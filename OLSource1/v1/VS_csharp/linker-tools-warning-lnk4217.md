...
locally defined symbol 'symbol' imported in function 'function'  
 ...
 [__declspec(dllimport)](../VS_csharp/dllexport--dllimport.md) was specified for a symbol even though the symbol is defined locally. Remove the `__declspec` modifier to resolve this warning.  
  
 `symbol` is the symbol name that's defined within the image. `function` is the function that is importing the symbol.  
  
 This warning will not appear when you compile by using the option [/clr](../VS_csharp/-clr--common-language-runtime-compilation-.md).  
