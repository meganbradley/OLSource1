...
The Parallel Patterns Library (PPL) includes several containers and objects that provide thread-safe access to their elements.  
 ...
 A *concurrent container* provides concurrency-safe access to the most important operations. The functionality of these containers resembles those that are provided by the Standard Template Library (STL). For example, the [concurrency::concurrent_vector](../VS_csharp/concurrent_vector-class.md) class resembles the [std::vector](../VS_csharp/vector-class.md) class, except that the `concurrent_vector` class lets you append elements in parallel. Use concurrent containers when you have parallel code that requires both read and write access to the same container.  
  
 A *concurrent object* is shared concurrently among components. A process that computes the state of a concurrent object in parallel produces the same result as another process that computes the same state serially. The [concurrency::combinable](../VS_csharp/combinable-class.md) class is one example of a concurrent object type. The `combinable` class lets you perform computations in parallel, and then combine those computations into a final result. Use concurrent objects when you would otherwise use a synchronization mechanism, for example, a mutex, to synchronize access to a shared variable or resource.  
...
##  <a name="vector"></a> concurrent_vector Class  
 The [concurrency::concurrent_vector](../VS_csharp/concurrent_vector-class.md) class is a sequence container class that, just like the [std::vector](../VS_csharp/vector-class.md) class, lets you randomly access its elements. The `concurrent_vector` class enables concurrency-safe append and element access operations. Append operations do not invalidate existing pointers or iterators. Iterator access and traversal operations are also concurrency-safe.  
...
-   A `concurrent_vector` object does not use [move semantics](../VS_csharp/rvalue-reference-declarator----.md) when you append to it.  
  
-   The `concurrent_vector` class does not provide the `erase` or `pop_back` methods. As with `vector`, use the [clear](../VS_csharp/concurrent_vector--clear-method.md) method to remove all elements from a `concurrent_vector` object.  
...
-   The `concurrent_vector` class defines the [grow_by](../VS_csharp/concurrent_vector--grow_by-method.md) and [grow_to_at_least](../VS_csharp/concurrent_vector--grow_to_at_least-method.md) methods. These methods resemble the [resize](../VS_csharp/concurrent_vector--resize-method.md) method, except that they are concurrency-safe.  
...
||||  
|-|-|-|  
|[at](../VS_csharp/concurrent_vector--at-method.md)|[end](../VS_csharp/concurrent_vector--end-method.md)|[operator&#91;&#93;](../VS_csharp/concurrent_vector--operatoroperator.md)|  
|[begin](../VS_csharp/concurrent_vector--begin-method.md)|[front](../VS_csharp/concurrent_vector--front-method.md)|[push_back](../VS_csharp/concurrent_vector--push_back-method.md)|  
|[back](../VS_csharp/concurrent_vector--back-method.md)|[grow_by](../VS_csharp/concurrent_vector--grow_by-method.md)|[rbegin](../VS_csharp/concurrent_vector--rbegin-method.md)|  
|[capacity](../VS_csharp/concurrent_vector--capacity-method.md)|[grow_to_at_least](../VS_csharp/concurrent_vector--grow_to_at_least-method.md)|[rend](../VS_csharp/concurrent_vector--rend-method.md)|  
|[empty](../VS_csharp/concurrent_vector--empty-method.md)|[max_size](../VS_csharp/concurrent_vector--max_size-method.md)|[size](../VS_csharp/concurrent_vector--size-method.md)|  
...
|||  
|-|-|  
|[assign](../VS_csharp/concurrent_vector--assign-method.md)|[reserve](../VS_csharp/concurrent_vector--reserve-method.md)|  
|[clear](../VS_csharp/concurrent_vector--clear-method.md)|[resize](../VS_csharp/concurrent_vector--resize-method.md)|  
|[operator=](../VS_csharp/concurrent_vector--operator=-operator.md)|[shrink_to_fit](../VS_csharp/concurrent_vector--shrink_to_fit-method.md)|  
  
 Operations that modify the value of existing elements are not concurrency-safe. Use a synchronization object such as a [reader_writer_lock](../VS_csharp/reader_writer_lock-class.md) object to synchronize concurrent read and write operations to the same data element. For more information about synchronization objects, see [Synchronization Data Structures](../VS_csharp/synchronization-data-structures.md).  
...
##  <a name="queue"></a> concurrent_queue Class  
 The [concurrency::concurrent_queue](../VS_csharp/concurrent_queue-class.md) class, just like the [std::queue](../VS_csharp/queue-class.md) class, lets you access its front and back elements. The `concurrent_queue` class enables concurrency-safe enqueue and dequeue operations. The `concurrent_queue` class also provides iterator support that is not concurrency-safe.  
...
-   The `concurrent_queue` class does not provide the `front` or `pop` methods. The `concurrent_queue` class replaces these methods by defining the [try_pop](../VS_csharp/concurrent_queue--try_pop-method.md) method.  
  
-   The `concurrent_queue` class does not provide the `back` method. Therefore, you cannot reference the end of the queue.  
  
-   The `concurrent_queue` class provides the [unsafe_size](../VS_csharp/concurrent_queue--unsafe_size-method.md) method instead of the `size` method. The `unsafe_size` method is not concurrency-safe.  
...
|||  
|-|-|  
|[empty](../VS_csharp/concurrent_queue--empty-method.md)|[push](../VS_csharp/concurrent_queue--push-method.md)|  
|[get_allocator](../VS_csharp/concurrent_queue--get_allocator-method.md)|[try_pop](../VS_csharp/concurrent_queue--try_pop-method.md)|  
...
|||  
|-|-|  
|[clear](../VS_csharp/concurrent_queue--clear-method.md)|[unsafe_end](../VS_csharp/concurrent_queue--unsafe_end-method.md)|  
|[unsafe_begin](../VS_csharp/concurrent_queue--unsafe_begin-method.md)|[unsafe_size](../VS_csharp/concurrent_queue--unsafe_size-method.md)|  
...
##  <a name="unordered_map"></a> concurrent_unordered_map Class  
 The [HYPERLINK "file:///C:\\\Users\\\thompet\\\AppData\\\Local\\\Temp\\\DxEditor\\\DduePreview\\\Default\\\798d7037-df37-4310-858b-6f590bbf6ebf\\\HTM\\\html\\\a217b4ac-af2b-4d41-94eb-09a75ee28622" concurrency::concurrent_unordered_map](../VS_csharp/concurrent_unordered_map-class.md) class is an associative container class that, just like the [std::unordered_map](../VS_csharp/unordered_map-class.md) class, controls a varying-length sequence of elements of type [std::pair<const Key, Ty>](../VS_csharp/pair-structure.md). Think of an unordered map as a dictionary that you can add a key and value pair to or look up a value by key. This class is useful when you have multiple threads or tasks that have to concurrently access a shared container, insert into it, or update it.  
 ...
 [!code[concrt-unordered-map-structure#1](../VS_csharp/codesnippet/CPP/parallel-containers-and-objects_2.cpp)]  
  
 For an example that uses `concurrent_unordered_map` to perform a map and reduce operation in parallel, see [How to: Perform Map and Reduce Operations in Parallel](../VS_csharp/how-to--perform-map-and-reduce-operations-in-parallel.md).  
...
|||||  
|-|-|-|-|  
|[at](../VS_csharp/concurrent_unordered_map--at-method.md)|`count`|`find`|[key_eq](../VS_csharp/concurrent_unordered_map--key_eq-method.md)|  
|`begin`|`empty`|`get_allocator`|`max_size`|  
|`cbegin`|`end`|`hash_function`|[operator&#91;&#93;](../VS_csharp/concurrent_unordered_map--operatoroperator.md)|  
|`cend`|`equal_range`|[insert](../VS_csharp/concurrent_unordered_map--insert-method.md)|`size`|  
...
|`load_factor`|[operator=](../VS_csharp/concurrent_unordered_map--operator=-operator.md)|[swap](../VS_csharp/concurrent_unordered_map--swap-method.md)|  
...
##  <a name="unordered_multimap"></a> concurrent_unordered_multimap Class  
 The [concurrency::concurrent_unordered_multimap](../VS_csharp/concurrent_unordered_multimap-class.md) class closely resembles the `concurrent_unordered_map` class except that it allows for multiple values to map to the same key. It also differs from `concurrent_unordered_map` in the following ways:  
  
-   The [concurrent_unordered_multimap::insert](../VS_csharp/concurrent_unordered_multimap--insert-method.md) method returns an iterator instead of `std::pair<iterator, bool>`.  
...
-   The `concurrent_unordered_multimap` class does not provide `operator[]` nor the `at` method.  
 ...
 [!code[concrt-unordered-multimap-structure#1](../VS_csharp/codesnippet/CPP/parallel-containers-and-objects_3.cpp)]  
  
 [[Top](#top)]  
  
##  <a name="unordered_set"></a> concurrent_unordered_set Class  
 The [concurrency::concurrent_unordered_set](../VS_csharp/concurrent_unordered_set-class.md) class closely resembles the `concurrent_unordered_map` class except that it manages values instead of key and value pairs. The `concurrent_unordered_set` class does not provide `operator[]` nor the `at` method.  
 ...
 [!code[concrt-unordered-set#1](../VS_csharp/codesnippet/CPP/parallel-containers-and-objects_4.cpp)]  
  
 [[Top](#top)]  
  
##  <a name="unordered_multiset"></a> concurrent_unordered_multiset Class  
 The [concurrency::concurrent_unordered_multiset](../VS_csharp/concurrent_unordered_multiset-class.md) class closely resembles the `concurrent_unordered_set` class except that it allows for duplicate values. It also differs from `concurrent_unordered_set` in the following ways:  
  
-   The [concurrent_unordered_multiset::insert](../VS_csharp/concurrent_unordered_multiset--insert-method.md) method returns an iterator instead of `std::pair<iterator, bool>`.  
...
-   The `concurrent_unordered_multiset` class does not provide `operator[]` nor the `at` method.  
 ...
 [!code[concrt-unordered-multiset#1](../VS_csharp/codesnippet/CPP/parallel-containers-and-objects_5.cpp)]  
  
 [[Top](#top)]  
  
##  <a name="combinable"></a> combinable Class  
 The [concurrency::combinable](../VS_csharp/combinable-class.md) class provides reusable, thread-local storage that lets you perform fine-grained computations and then merge those computations into a final result. You can think of a `combinable` object as a reduction variable.  
...
###  <a name="combinable-features"></a> Methods and Features  
 The following table shows some of the important methods of the `combinable` class. For more information about all the `combinable` class methods, see [combinable Class](../VS_csharp/combinable-class.md).  
  
|Method|Description|  
|------------|-----------------|  
|[local](../VS_csharp/combinable--local-method.md)|Retrieves a reference to the local variable that is associated with the current thread context.|  
|[clear](../VS_csharp/combinable--clear-method.md)|Removes all thread-local variables from the `combinable` object.|  
|[combine](../VS_csharp/combinable--combine-method.md)<br /><br /> [combine_each](../VS_csharp/combinable--combine_each-method.md)|Uses the provided combine function to generate a final value from the set of all thread-local computations.|  
 ...
 You can store additional data in a `combinable` object after you call the [combine](../VS_csharp/combinable--combine-method.md) or [combine_each](../VS_csharp/combinable--combine_each-method.md) methods. You can also call the `combine` and `combine_each` methods multiple times. If no local value in a `combinable` object changes, the `combine` and `combine_each` methods produce the same result every time that they are called.  
...
-   [How to: Use combinable to Combine Sets](../VS_csharp/how-to--use-combinable-to-combine-sets.md)  
  
 [[Top](#top)]  
  
## Related Topics  
 [How-to: Use Parallel Containers](../VS_csharp/how-to--use-parallel-containers-to-increase-efficiency.md)  
