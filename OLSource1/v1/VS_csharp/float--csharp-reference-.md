...
## Literals  
 By default, a real numeric literal on the right side of the assignment operator is treated as [double](../VS_csharp/double--csharp-reference-.md). Therefore, to initialize a float variable, use the suffix `f` or `F`, as in the following example:  
...
```  
 ...
 If you do not use the suffix in the previous declaration, you will get a compilation error because you are trying to store a [double](../VS_csharp/double--csharp-reference-.md) value into a `float` variable.  
  
## Conversions  
 You can mix numeric integral types and floating-point types in an expression. In this case, the integral types are converted to floating-point types. The evaluation of the expression is performed according to the following rules:  
  
-   If one of the floating-point types is [double](../VS_csharp/double--csharp-reference-.md), the expression evaluates to [double](../VS_csharp/double--csharp-reference-.md) or [bool](../VS_csharp/bool--csharp-reference-.md) in relational or Boolean expressions.  
  
-   If there is no [double](../VS_csharp/double--csharp-reference-.md) type in the expression, the expression evaluates to `float` or [bool](../VS_csharp/bool--csharp-reference-.md) in relational or Boolean expressions.  
...
## Example  
 In the following example, an [int](../VS_csharp/int--csharp-reference-.md), a [short](../VS_csharp/short--csharp-reference-.md), and a `float` are included in a mathematical expression giving a `float` result. (Remember that `float` is an alias for the <xref:System.Single*?displayProperty=fullName> type.) Notice that there is no [double](../VS_csharp/double--csharp-reference-.md) in the expression.  
  
 [!code[csrefKeywordsTypes#13](../VS_csharp/codesnippet/CSharp/float--csharp-reference-_1.cs)]  
