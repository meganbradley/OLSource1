...
*XML documentation comments* are JavaScript comments that you add to a script to provide information about code elements such as functions, fields, and variables. In Visual Studio, these text descriptions are displayed with IntelliSense when you reference the script function.  
 ...
 This topic provides a basic tutorial on using XML documentation comments. For information about using other elements, such as [<var\>](../VS_csharp/-var---javascript-.md) and [<value\>](../VS_csharp/-value---javascript-.md), and for additional code examples, see [XML Documentation Comments](../VS_csharp/xml-documentation-comments--javascript-.md). For information about providing IntelliSense information for an asynchronous callback such as a `Promise`, see [<returns\>](../VS_csharp/-returns---javascript-.md).  
...
-   In the function, add [<summary\>](../VS_csharp/-summary---javascript-.md), [<param\>](../VS_csharp/-param---javascript-.md), and [<returns\>](../VS_csharp/-returns---javascript-.md) elements, and precede each element with three slash marks (///).  
...
-   In a constructor function or object definition, add a [<field\>](../VS_csharp/-field---javascript-.md) element preceded by three slash marks (///).  
  
     The following example shows the use of the `<field>` element in a constructor function. For additional examples, see [<field\>](../VS_csharp/-field---javascript-.md).  
...
1.  In the function, add a [<signature\>](../VS_csharp/-signature---javascript-.md) element for each overload. In these elements, add other elements, such as `<summary>`, `<param>`, and `<returns>`, preceding each element with three slash marks (///).  
...
1.  Create an XML file that has documentation comments in the OpenAjax MessageBundle format.  
    ...
    > [!IMPORTANT]
    >  MessageBundle is the recommended format. This format is not supported in Microsoft Ajax or in .winmd files. For information about using the alternative `VSDoc` format, see [<loc\>](../VS_csharp/-loc---javascript-.md).  
...
2.  In your .js file, add the following code. The `<loc>` element must be declared before any script, and follows the same usage rules as the `<reference>` element. For more information, see [JavaScript IntelliSense](../VS_csharp/javascript-intellisense.md) and [<loc\>](../VS_csharp/-loc---javascript-.md).  
...
## See Also  
 [JavaScript IntelliSense Overview](../VS_csharp/javascript-intellisense.md)   
