...
Scheduler instances let you associate specific scheduling policies with various kinds of workloads. This topic contains two basic examples that show how to create and manage a scheduler instance.  
 ...
 The examples create schedulers that use the default scheduler policies. For an example that creates a scheduler that uses a custom policy, see [How-to: Specify Specific Scheduler Policies](../VS_csharp/how-to--specify-specific-scheduler-policies.md).  
  
### To manage a scheduler instance in your application  
  
1.  Create a [concurrency::SchedulerPolicy](../VS_csharp/schedulerpolicy-class.md) object that contains the policy values for the scheduler to use.  
  
2.  Call the [concurrency::CurrentScheduler::Create](../VS_csharp/currentscheduler--create-method.md) method or the [concurrency::Scheduler::Create](../VS_csharp/scheduler--create-method.md) method to create a scheduler instance.  
  
     If you use the `Scheduler::Create` method, call the [concurrency::Scheduler::Attach](../VS_csharp/scheduler--attach-method.md) method when you need to associate the scheduler with the current context.  
...
4.  Pass the handle to the event object that you just created to the [concurrency::CurrentScheduler::RegisterShutdownEvent](../VS_csharp/currentscheduler--registershutdownevent-method.md) method or the [concurrency::Scheduler::RegisterShutdownEvent](../VS_csharp/scheduler--registershutdownevent-method.md) method. This registers the event to be set when the scheduler is destroyed.  
  
5.  Perform the tasks that you want the current scheduler to schedule.  
  
6.  Call the [concurrency::CurrentScheduler::Detach](../VS_csharp/currentscheduler--detach-method.md) method to detach the current scheduler and restore the previous scheduler as the current one.  
  
     If you use the `Scheduler::Create` method, call the [concurrency::Scheduler::Release](../VS_csharp/scheduler--release-method.md) method to decrement the reference count of the `Scheduler` object.  
...
## See Also  
 [Scheduler Instances](../VS_csharp/scheduler-instances.md)   
