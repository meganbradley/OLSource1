...
## Remarks  
 ...
 An object of class basic_stringbuf< `Elem`, `Tr`, `Alloc`> stores a copy of the `ios_base::`[openmode](../VS_csharp/ios_base-class.md#ios_base__openmode) argument from its constructor as its `stringbuf` mode **mode**:  
  
-   If `mode & ios_base::in` is nonzero, the input buffer is accessible. For more information, see [basic_streambuf Class](../VS_csharp/basic_streambuf-class.md).  
...
### Parameters  
 ...
 `_Str`  
 An object of type [basic_string](../VS_csharp/basic_string-class.md).  
  
### Remarks  
 The first constructor stores a null pointer in all the pointers controlling the input buffer and the output buffer. For more information, see the Remarks section of the [basic_streambuf Class](../VS_csharp/basic_streambuf-class.md). It also stores `_Mode` as the stringbuf mode. For more information, see the Remarks section of the [basic_stringbuf Class](../VS_csharp/basic_stringbuf-class.md).  
...
### Remarks  
 If _                        *Meta* does not compare equal to **traits_type::**[eof](../VS_csharp/char_traits-struct.md#char_traits__eof), the protected virtual member function tries to insert the element **traits_type::**[to_char_type](../VS_csharp/char_traits-struct.md#char_traits__to_char_type)(\_                        *Meta*) into the output buffer. It can do so in various ways:  
...
### Parameters  
 ...
 `_Way`  
 The starting point for offset operations. See [ios_base::seekdir](../VS_csharp/ios_base-class.md#ios_base__seekdir) for possible values.  
  
 `_Mode`  
 Specifies the mode for the pointer position. The default is to allow you to modify the read and write positions. For more information, see [ios_base::openmode](../VS_csharp/ios_base-class.md#ios_base__openmode).  
...
### Return Value  
 Returns an object of class [basic_string](../VS_csharp/basic_string-class.md)< **Elem**, **Tr**, Alloc **>,** whose controlled sequence is a copy of the sequence controlled by **\*this**.  
...
-   If **mode & ios_base::out** is nonzero and an output buffer exists, the sequence is the entire output buffer ( [epptr](../VS_csharp/basic_streambuf-class.md#basic_streambuf__epptr) - [pbase](../VS_csharp/basic_streambuf-class.md#basic_streambuf__pbase) elements beginning with `pbase`).  
  
-   If **mode & ios_base::in** is nonzero and an input buffer exists, the sequence is the entire input buffer ( [egptr](../VS_csharp/basic_streambuf-class.md#basic_streambuf__egptr) - [eback](../VS_csharp/basic_streambuf-class.md#basic_streambuf__eback) elements beginning with `eback`).  
...
### Return Value  
 If the function cannot succeed, it returns **traits_type::**[eof](../VS_csharp/char_traits-struct.md#char_traits__eof). Otherwise, it returns the current element in the input stream, which are converted.  
  
### Remarks  
 The protected virtual member function tries to extract the current element **byte** from the input buffer, advance the current stream position, and return the element as **traits_type::**[to_int_type](../VS_csharp/char_traits-struct.md#char_traits__to_int_type)( **byte**). It can do so in one way: If a read position is available, it takes **byte** as the element stored in the read position and advances the next pointer for the input buffer.  
