...
## Remarks  
 ...
 [CFrameWnd::GetDockState](../VS_csharp/cframewnd-class.md#cframewnd__getdockstate) retrieves the state information of all the frame window's `CControlBar` objects and puts it into the `CDockState` object. You can then write the contents of the `CDockState` object to storage with [Serialize](../VS_csharp/cobject-class.md#cobject__serialize) or [CDockState::SaveState](#cdockstate__savestate). If you later want to restore the state of the control bars in the frame window, you can load the state with `Serialize` or [CDockState::LoadState](#cdockstate__loadstate), then use [CFrameWnd::SetDockState](../VS_csharp/cframewnd-class.md#cframewnd__setdockstate) to apply the saved state to the frame window's control bars.  
  
 For more information on docking control bars, see the articles [Control Bars](../VS_csharp/control-bars.md), [Toolbars: Docking and Floating](../VS_csharp/docking-and-floating-toolbars.md), and [Frame Windows](../VS_csharp/frame-windows.md).  
