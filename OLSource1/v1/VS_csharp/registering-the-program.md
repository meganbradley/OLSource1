...
After the debug engine has acquired a port, represented by an [IDebugPort2](../VS_csharp/idebugport2.md) interface, the next step in enabling the program to be debugged is to register it with the port. Once registered, the program is available for debugging by one of the following means:  
...
1.  Call the [AddProgramNode](../VS_csharp/idebugportnotify2--addprogramnode.md) method implemented by the port.  
  
     `IDebugPortNotify2::AddProgramNode` requires a pointer to an [IDebugProgramNode2](../VS_csharp/idebugprogramnode2.md) interface.  
