...
Causes the compiler to process `#pragma`[omp](../VS_csharp/omp.md).  
...
## Remarks  
 `#pragma omp` is used to specify [Directives](../VS_csharp/openmp-directives.md) and [Clauses](../VS_csharp/openmp-clauses.md). If **/openmp** is not specified in a compilation, the compiler ignores OpenMP clauses and directives. [OpenMP Function](../VS_csharp/openmp-functions.md) calls are processed by the compiler even if **/openmp** is not specified.  
  
 An application compiled with **/openmp** and using the [OpenMP Libraries](../VS_csharp/openmp-libraries.md) can only be run on Windows 2000 or later operating systems.  
  
 Applications compiled with **/openmp** and **/clr** can only be run in a single application domain process; multiple application domains are not supported. That is, when the module constructor (.cctor) is run, it will detect the process is compiled with **/openmp** and if the application is being loaded into a non-default runtime. For more information, see [appdomain](../VS_csharp/appdomain.md), [/clr (Common Language Runtime Compilation)](../VS_csharp/-clr--common-language-runtime-compilation-.md), and [Initialization of Mixed Assemblies](../VS_csharp/initialization-of-mixed-assemblies.md).  
