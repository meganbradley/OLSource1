...
> [!NOTE]
>  For information about the `sizeof ...` operator, see [Ellipses and Variadic Templates](../VS_csharp/ellipses-and-variadic-templates.md).  
...
```  
 ...
 When the `sizeof` operator is applied to a `class`, `struct`, or `union` type, the result is the number of bytes in an object of that type, plus any padding added to align members on word boundaries. The result does not necessarily correspond to the size calculated by adding the storage requirements of the individual members. The [/Zp](../VS_csharp/-zp--struct-member-alignment-.md) compiler option and the [pack](../VS_csharp/pack.md) pragma affect alignment boundaries for members.  
