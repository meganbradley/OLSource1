...
>  This topic applies to objects derived from `CRecordset` in which bulk row fetching has not been implemented. If you are using bulk row fetching, see [Recordset: Fetching Records in Bulk (ODBC)](../VS_csharp/recordset--fetching-records-in-bulk--odbc-.md).  
 ...
 As a supplement, you might want to read [Record Field Exchange: How RFX Works](../VS_csharp/record-field-exchange--how-rfx-works.md), which describes the corresponding role of RFX in update operations.  
...
-   If **::SQLSetPos** cannot be used, MFC does the following:  
    ...
    3.  **Update** commits the changes — executes the **UPDATE** statement — and the record is changed on the data source, but not committed if a transaction is in progress (see [Transaction: Performing a Transaction in a Recordset (ODBC)](../VS_csharp/transaction--performing-a-transaction-in-a-recordset--odbc-.md) for information about how the transaction affects the update). ODBC keeps a copy of the record, which also changes.  
...
-   If **::SQLSetPos** cannot be used, MFC does the following:  
 ...
 For information about the SQL statements used in update operations, see [SQL](../VS_csharp/sql.md).  
  
## See Also  
 [Recordset (ODBC)](../VS_csharp/recordset--odbc-.md)   
 [Recordset: More About Updates (ODBC)](../VS_csharp/recordset--more-about-updates--odbc-.md)   
