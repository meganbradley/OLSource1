...
|Name|Description|  
|----------|-----------------|  
|[ICommandTarget::Initialize](../VS_csharp/icommandtarget--initialize.md)|Initializes the command target object.|  
  
## Remarks  
 When you host a user control in an MFC View, [CWinFormsView](../VS_csharp/cwinformsview-class.md) routes commands and update command UI messages to the user control to allow it to handle MFC commands (for example, frame menu items and toolbar buttons). By implementing `ICommandTarget`, you give the user control a reference to the [ICommandSource](../VS_csharp/icommandsource-interface.md) object.  
