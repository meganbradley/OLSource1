...
This topic describes how to use [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] Profiling Tools command-line tools to collect detailed .NET memory allocation and object lifetime data for a dynamically compiled [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] Web application by using the instrumentation profiling method.  
...
>  Command-line tools of the Profiling Tools are located in the \Team Tools\Performance Tools subdirectory of the [!INCLUDE[vs_current_short](../VS_csharp/includes/vs_current_short_md.md)] installation directory. On 64 bit computers, both 64 bit and 32 bit versions of the tools are available. To use the profiler command-line tools, you must add the tools path to the PATH environment variable of the command prompt window or add it to to the command itself. For more information, see [Specifying the Path to Profiling Tools Command Line Tools](../VS_csharp/specifying-the-path-to-profiling-tools-command-line-tools.md).  
 ...
 To collect performance data from a [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] Web application, you modify the web.config file of the target application to enable the [VSInstr.exe](../VS_csharp/vsinstr.md) tool to instrument the dynamically compiled application files. You then use the [VSPerfCLREnv.cmd](../VS_csharp/vsperfclrenv.md) tool to configure the server that hosts the [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] Web application and enable .NET memory profiling by setting the appropriate environment variables, and then restart the computer.  
...
1.  Modify the web.config file of the target application. See [How to: Modify Config.Web Files to Instrument and Profile ASP.NET Web Applications](../VS_csharp/how-to--modify-web.config-files-to-instrument-and-profile-dynamically-compiled-asp.net-web-applications.md).  
...
1.  Start the profiler. Type:  
     ...
     **VSPerfCmd** [/start](../VS_csharp/start.md) **:trace** [/output](../VS_csharp/output.md) **:** `OutputFile` [`Options`]  
    ...
    |Option|Description|  
    |------------|-----------------|  
    |[/user](../VS_csharp/user--vsperfcmd-.md) **:**[`Domain`**\\**]`UserName`|Specifies the optional domain and user name of the account that owns the [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] worker process. This option is required if the process is running as a user other than the logged on user. The name is listed in the User Name column on the Processes tab of Windows Task Manager.|  
    |[/crosssession](../VS_csharp/crosssession.md)|Enables profiling of processes in other sessions. This option is required if the application is running in a different session. The session id is listed in the Session ID column on the the Processes tab of Windows Task Manager. **/CS** can be specified as an abbreviation for **/crosssession**.|  
    |[/globaloff](../VS_csharp/globalon-and-globaloff.md)|Starts the profiler with data collection paused. Use [/globalon](../VS_csharp/globalon-and-globaloff.md) to resume profiling.|  
    |[/counter](../VS_csharp/counter.md) **:** `Config`|Collects information from the processor performance counter specified in `Config`. Counter information is added to the data collected at each profiling event.|  
...
2.  Start the [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] Web application in the typical way.  
...
-   The following pairs of options start and stop data collection. Specify each option on a separate command line. You can turn data collection on and off multiple times.  
    ...
    |[/threadon](../VS_csharp/threadon-and-threadoff.md) **:** `TID` [/threadoff](../VS_csharp/threadon-and-threadoff.md) **:** `TID`|Starts (**/threadon**) or stops (**/threadoff**) data collection for the thread specified by the thread ID (`TID`).|  
  
-   You can also use the **VSPerfCmd.exe**[/mark](../VS_csharp/mark.md) option to insert a profiling mark into the data file. The **/mark** command adds an identifier, a timestamp, and an optional user-defined text string. Marks can be used to filter the data in profiler reports and data views.  
  
## Ending the Profiling Session  
 To end a profiling session, close the target [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] Web application, stop Internet Information Services (IIS) to stop the profiled process, and then shut down the profiler. Then restart IIS.  
  
#### To end a profiling session  
  
1.  Close the [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] Web application.  
  
2.  Close the [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] worker process by resetting Internet Information Services (IIS). Type:  
  
     **IISReset /stop**  
  
3.  Shut down the profiler. Type:  
  
     **VSPerfCmd** [/shutdown](../VS_csharp/shutdown.md)  
...
## Restoring the Application and Computer Configuration  
 When you have completed all profiling, replace the web.config file, clear the profiling environment variables, and restart the computer to restore the server and the [!INCLUDE[vstecasp](../VS_csharp/includes/vstecasp_md.md)] application to their original states.  
...
## See Also  
 [Command-Line Profiling of ASP.NET Applications](../VS_csharp/command-line-profiling-of-asp.net-web-applications.md)   
