...
## Remarks  
 MFC does not encapsulate DAO error objects in a class. Instead, the [CDaoException](../VS_csharp/cdaoexception-class.md) class supplies an interface for accessing the Errors collection contained in the DAO **DBEngine** object, the object that also contains all workspaces. When an MFC DAO operation throws a `CDaoException` object that you catch, MFC fills a `CDaoErrorInfo` structure and stores it in the exception object's [m_pErrorInfo](../Topic/CDaoException::m_pErrorInfo.md) member. (If you choose to call DAO directly, you must call the exception object's [GetErrorInfo](../Topic/CDaoException::GetErrorInfo.md) member function yourself to fill `m_pErrorInfo`.)  
  
 For more information about handling DAO errors, see the article [Exceptions: Database Exceptions](../VS_csharp/exceptions--database-exceptions.md). For related information, see the topic "Error Object" in DAO Help.  
...
## See Also  
 ...
 [CDaoException Class](../VS_csharp/cdaoexception-class.md)