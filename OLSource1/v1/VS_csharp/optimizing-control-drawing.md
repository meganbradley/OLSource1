...
When a control is instructed to draw itself into a container-supplied device context, it typically selects GDI objects (such as pens, brushes, and fonts) into the device context, performs its drawing operations, and restores the previous GDI objects. If the container has multiple controls that are to be drawn into the same device context, and each control selects the GDI objects it requires, time can be saved if the controls do not individually restore previously selected objects. After all the controls have been drawn, the container can automatically restore the original objects.  
 ...
 [!code[NVC_MFC_AxOpt#15](../VS_csharp/codesnippet/CPP/optimizing-control-drawing_1.cpp)]  
 ...
 To prevent the [CPen](../VS_csharp/cpen-class.md) and [CBrush](../VS_csharp/cbrush-class.md) objects from being destroyed when `OnDraw` finishes, store them in member variables instead of local variables. In the control's class declaration, add declarations for two new member variables:  
  
 [!code[NVC_MFC_AxOpt#16](../VS_csharp/codesnippet/CPP/optimizing-control-drawing_2.h)]  
[!code[NVC_MFC_AxOpt#17](../VS_csharp/codesnippet/CPP/optimizing-control-drawing_3.h)]  
  
 Then, the `OnDraw` function can be rewritten as follows:  
  
 [!code[NVC_MFC_AxOpt#18](../VS_csharp/codesnippet/CPP/optimizing-control-drawing_4.cpp)]  
 ...
 [!code[NVC_MFC_AxOpt#19](../VS_csharp/codesnippet/CPP/optimizing-control-drawing_5.cpp)]  
  
 Finally, to eliminate unnecessary `SelectObject` calls, modify `OnDraw` as follows:  
  
 [!code[NVC_MFC_AxOpt#20](../VS_csharp/codesnippet/CPP/optimizing-control-drawing_6.cpp)]  
...
## See Also  
 ...
 [Serialization](../VS_csharp/mfc-activex-controls.md)   
 [Basic Components of an ActiveX Control](../VS_csharp/mfc-activex-controls.md)   
 [MFC ActiveX Control Wizard](../VS_csharp/mfc-activex-control-wizard.md)   
 [MFC ActiveX Controls: Painting an ActiveX Control](../VS_csharp/mfc-activex-controls--painting-an-activex-control.md)