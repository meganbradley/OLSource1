...
A Domain-Specific Language (DSL) solution generates an API that you can use to read and update instances of the DSL in [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)]. This API is defined in the code that is generated from the DSL definition. This topic describes the generated API.  
...
>  The Component Diagrams DSL template is not related to the UML component diagrams that you can create by using the Architecture menu in Visual Studio. In the **New Project** dialog box, expand **Other Project Types\Extensibility** and then click **Domain-Specific Language Designer**.  
 ...
 ![Components and interconnected ports](../VS_csharp/media/componentsample.png "ComponentSample")  
  
## The Structure of the DSL Solution  
 The **Dsl** project defines the API for your DSL. The **DslPackage** project defines how it integrates with [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)]. You can also add your own projects, which can also contain code generated from the model.  
 ...
 *YourDsl* `Schema.xsd`  
 ...
 For more information, see [Generating Code from a DSL](../VS_csharp/generating-code-from-a-domain-specific-language.md) and [Creating Custom Directive Processors](../VS_csharp/creating-custom-t4-text-template-directive-processors.md).  
...
-   A property definition and a nested handler class for each domain property. You can override OnValueChanging() and OnValueChanged(). For more information, see [Domain Property Change Handlers](../VS_csharp/domain-property-value-change-handlers.md).  
...
>  This is not the same as the root class of the model.  
 ...
 Copy and Delete Closures define what other elements should be included when an element is copied or deleted. You can control this behavior by setting the **Propagates Copy** and **Propagates Delete** properties of the roles at each side of every relationship. If you want the values to be determined dynamically, you can write code to override the methods of the Closure classes. For more information see [Customizing Copy and Delete](../Topic/How%20to:%20Program%20Copy%20and%20Paste%20Behavior%20-%20redirect.md).  
 ...
 For these constraints to be executed, you must have set one of the **Uses...** options in the **Editor\Validation** node in DSL Explorer. For more information, see [Validation](../VS_csharp/validation-in-a-domain-specific-language.md).  
  
 `PropertiesGrid.cs`  
  
 This file contains code only if you have attached a Custom Type Descriptor to a domain property. For more information, see [Customizing the Properties Window](../VS_csharp/customizing-the-properties-window.md).  
  
 `SerializationHelper.cs`  
  
-   A validation method to ensure that no two elements are referenced by the same moniker. For more information, see [Customizing File Storage](../VS_csharp/customizing-file-storage-and-xml-serialization.md).  
...
-   SerializationHelper class, which provides functions that are used in common by the serialization classes.  
 ...
 A class for every shape class in the DSL Definition. Shapes are derived from <xref:Microsoft.VisualStudio.Modeling.Diagrams.NodeShape*>. For more information, see [Customizing Files](../VS_csharp/customizing-file-storage-and-xml-serialization.md).  
 ...
 You could override `CreateElementPrototype()` to define a toolbox item that creates a group of several objects. For example, you could define an item to represent objects that have sub-components. After changing the code, reset the experimental instance of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] to clear the toolbox cache.  
  
## Generated files in the DslPackage project  
 DslPackage couples the DSL model to the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] shell, managing the window, toolbox, and menu commands. Most of the classes are double derived, so that you can override any of their methods.  
  
 `CommandSet.cs`  
  
 The context menu commands that are visible on the diagram. You can adapt or add to this set. This file contains the code for the commands. The location of the commands on menus is determined by the Commands.vsct file. For more information, see [Commands](../VS_csharp/writing-user-commands-and-actions.md).  
...
```  
 ...
 Instantiates `DocData` and `DocView`. It fulfills a standard interface that [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] uses to open an editor when your DSL package starts. It is referenced in the `ProvideEditorFactory` attribute in Package.cs  
  
 `GeneratedVSCT.vsct`  
  
 Locates the standard menu commands on menus, such as the diagram context menu, the **Edit** menu, and so on. The code for the commands is in CommandSet.cs. You can relocate or modify the standard commands, and you can add your own commands. For more information, see [Commands](../VS_csharp/writing-user-commands-and-actions.md).  
...
```  
 ...
 This file defines how the DSL integrates into [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)]. Attributes on the package class register the DSL as the handler for files that have your file extension, define its toolbox, and define how to open a new window. The Initialize() method is called one time when the first DSL is loaded into a [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] instance.  
...
>  If you edit the .tt file to include resources such as icons or images, make sure that the resource is included in the VSIX build. In Solution Explorer, select the file and make sure that the **Include in VSIX** property is `True`.  
 ...
 This file controls how the DSL is packaged into a Visual Studio Integration Extension (VSIX). For more information, see [Deployment](../VS_csharp/deploying-domain-specific-language-solutions.md).  
  
## See Also  
 [How to Define a Domain-Specific Language](../VS_csharp/how-to-define-a-domain-specific-language.md)   
 [Understanding Models, Relationships, and Classes](../VS_csharp/understanding-models--classes-and-relationships.md)   
 [Customizing and Extending a DSL](../VS_csharp/customizing-and-extending-a-domain-specific-language.md)   
 [Writing Code to Customize a DSL](../VS_csharp/writing-code-to-customise-a-domain-specific-language.md)