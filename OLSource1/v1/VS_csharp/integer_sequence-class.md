...
## Example  
 ...
 The apply function accepts a [std::tuple](../VS_csharp/tuple-class.md), and produces an integer_sequence by using the  `tuple_size` helper class. Note that [std::decay_t](../VS_csharp/decay-class.md)_is necessary because [tuple_size](../VS_csharp/tuple_size-class--tuple-.md) does not work with reference types. The `apply_` function unpacks the tuple members and forwards them as separate arguments to a function call. In this example the function is a simple lambda expression that prints out the values.  
...
## See Also  
 [Ellipses and Variadic Templates](../VS_csharp/ellipses-and-variadic-templates.md)