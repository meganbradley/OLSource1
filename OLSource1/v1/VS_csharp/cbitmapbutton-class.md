...
3.  Call the [Create](../VS_csharp/cbutton-class.md#cbutton__create) function to create the Windows button control and attach it to the `CBitmapButton` object.  
...
6.  In the `CDialog` object's [OnInitDialog](../VS_csharp/cdialog-class.md#cdialog__oninitdialog) routine, call the `CBitmapButton` object's [AutoLoad](#cbitmapbutton__autoload) function, using as parameters the button's control ID and the `CDialog` object's **this** pointer.  
  
 If you want to handle Windows notification messages, such as **BN_CLICKED**, sent by a bitmap-button control to its parent (usually a class derived from **CDialog)**, add to the `CDialog`-derived object a message-map entry and message-handler member function for each message. The notifications sent by a `CBitmapButton` object are the same as those sent by a [CButton](../VS_csharp/cbutton-class.md) object.  
  
 The class [CToolBar](../VS_csharp/ctoolbar-class.md) takes a different approach to bitmap buttons.  
  
 For more information on `CBitmapButton`, see [Controls](../VS_csharp/controls--mfc-.md).  
...
### Remarks  
 Use the `AutoLoad` function to initialize an owner-draw button in a dialog box as a bitmap button. Instructions for using this function are in the remarks for the [CBitmapButton](../VS_csharp/cbitmapbutton-class.md) class.  
  
### Example  
 [!code[NVC_MFCControlLadenDialog#75](../VS_csharp/codesnippet/CPP/cbitmapbutton-class_1.cpp)]  
...
### Remarks  
 After creating the C++ `CBitmapButton` object, call [CButton::Create](../VS_csharp/cbutton-class.md#cbutton__create) to create the Windows button control and attach it to the `CBitmapButton` object.  
  
### Example  
 [!code[NVC_MFCControlLadenDialog#57](../VS_csharp/codesnippet/CPP/cbitmapbutton-class_2.cpp)]  
...
### Example  
 [!code[NVC_MFCControlLadenDialog#58](../VS_csharp/codesnippet/CPP/cbitmapbutton-class_3.cpp)]  
...
### Example  
 [!code[NVC_MFCControlLadenDialog#59](../VS_csharp/codesnippet/CPP/cbitmapbutton-class_4.cpp)]  
  
## See Also  
 [MFC Sample CTRLTEST](../VS_csharp/visual-c---samples.md)   
 [Base Class](../VS_csharp/cbutton-class.md)   
