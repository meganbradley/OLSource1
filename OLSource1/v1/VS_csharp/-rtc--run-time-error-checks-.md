...
Used to enable and disable the run-time error checks feature, in conjunction with the [runtime_checks](../VS_csharp/runtime_checks.md) pragma.  
...
-   Detection of overruns and underruns of local variables such as arrays. **/RTC**`s` will not detect overruns when accessing memory that results from compiler padding within a structure. Padding could occur by using [align](../VS_csharp/align--c---.md), [/Zp (Struct Member Alignment)](../VS_csharp/-zp--struct-member-alignment-.md), or [pack](../VS_csharp/pack.md), or if you order structure elements in such a way as to require the compiler to add padding.  
  
-   Stack pointer verification, which detects stack pointer corruption. Stack pointer corruption can be caused by a calling convention mismatch. For example, using a function pointer, you call a function in a DLL that is exported as [__stdcall](../VS_csharp/__stdcall.md) but you declare the pointer to the function as [__cdecl](../VS_csharp/__cdecl.md).  
  
 `u`  
 Reports when a variable is used without having been initialized. For example, an instruction that generates `C4701` may also generate a run-time error under **/RTC**`u`. Any instruction that generates [Compiler Warning (level 1 and level 4) C4700](../VS_csharp/compiler-warning--level-1-and-level-4--c4700.md) will generate a run-time error under **/RTC**`u`.  
...
## Remarks  
 Run-time error checks are a way for you to find problems in your running code; for more information, see [How to: Use Native Run-Time Checks](../VS_csharp/how-to--use-native-run-time-checks.md).  
  
 If you compile your program at the command line using any of the **/RTC** compiler options, any pragma [optimize](../VS_csharp/optimize.md) instructions in your code will silently fail. This is because run-time error checks are not valid in a release (optimized) build.  
  
 You should use **/RTC** for development builds; **/RTC** should not be used for a retail build. **/RTC** cannot be used with compiler optimizations ([/O](../VS_csharp/-o-options--optimize-code-.md)). A program image built with **/RTC** will be slightly larger and slightly slower than an image built with **/Od** (up to 5 percent slower than an **/Od** build).  
  
 The __MSVC_RUNTIME_CHECKS preprocessor directive will be defined when you use any **/RTC** option or [/GZ](../VS_csharp/-gz--enable-stack-frame-run-time-error-checking-.md).  
...
## See Also  
 ...
 [How to: Use Native Run-Time Checks](../VS_csharp/how-to--use-native-run-time-checks.md)