...
## Example  
 Assume we have an application, appl.exe, that's built by using [!INCLUDE[vcprvc](../VS_csharp/includes/vcprvc_md.md)]. The application manifest either is embedded in appl.exe as the binary resource RT_MANIFEST, which has an ID equal to 1, or is stored as the separate file appl.exe.manifest. The content of this manifest resembles this:  
