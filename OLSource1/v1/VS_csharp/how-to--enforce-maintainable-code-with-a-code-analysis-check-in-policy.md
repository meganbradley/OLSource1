...
Developers can use the Code Metrics tool to measure the complexity and maintainability of their code, but they cannot invoke code metrics as part of a check-in policy. However, a team  can enable Code Analysis rules that verify the compliance of their code with Code Metrics standards and enforce the rules through check-in policies. For more information about code metrics, see the [Code Metrics Overview](../VS_csharp/code-metrics-values.md).  
 ...
 Administrators of version control for [!INCLUDE[esprfound](../VS_csharp/includes/esprfound_md.md)] can add the Code Analysis Maintainability Rules to the check-in policy requirements. These check-in policies require developers to run Code Analysis based on these rule changes before initiating a check-in.  
...
## See Also  
 ...
 [Code Analysis Check-in Policies](../VS_csharp/creating-and-using-code-analysis-check-in-policies.md)