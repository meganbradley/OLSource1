---
title: "Enabling Authorization and Creating Permissions in LightSwitch"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "lightswitch"
ms.tgt_pltfrm: na
ms.topic: "article"
helpviewer_keywords: 
  - "Visual Studio LightSwitch, authorization lessons"
  - "Visual Studio LightSwitch, authentication lessons"
ms.assetid: 473d8bc4-5f02-438a-9257-0a4b9922779f
caps.latest.revision: 14
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# Enabling Authorization and Creating Permissions in LightSwitch
You can use the authorization and authentication features in LightSwitch to control access to screens and data. For example, you could create a permission to view a payroll screen, and the application administrator can grant or deny that permission to individuals or to groups of users defined as *roles*.  
  
 In this set of lessons you will learn how to enable authentication and create permissions.  
  
> [!NOTE]
>  These lessons build upon the My First Application project that you last modified in the [Setting the Startup Screen for a LightSwitch Application](../VS_csharp/setting-the-startup-screen-for-a-lightswitch-application.md) lesson. If your application isn't already open in Visual Studio, on the menu bar, choose **File**, **Open Project** to open it.  
  
## Lessons  
  
|Read this lesson|To learn|  
|----------------------|--------------|  
|[Enabling Authentication](../VS_csharp/enabling-authentication-in-lightswitch.md)|How to prevent unauthorized users from accessing your LightSwitch application.|  
|[Creating Permissions](../VS_csharp/creating-permissions-in-lightswitch.md)|How to create a permission to view a screen.|  
|[Restricting Access to Screens](../VS_csharp/restricting-access-to-a-lightswitch-screen.md)|How to apply a permission in code.|  
|[Using Debug Permissions to Test Code](../VS_csharp/using-debug-permissions-to-test-lightswitch-code.md)|How to test your code with different sets of permissions.|  
  
## See Also  
 [LightSwitch Guided Tour](../VS_csharp/lightswitch-guided-tour.md)   
 [How to: Enable Authentication in an HTML Client App](../VS_csharp/how-to--enable-authentication-in-an-html-client-app.md)