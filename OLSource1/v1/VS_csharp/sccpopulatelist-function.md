...
#### Parameters  
 ...
 nCommand  
 [in] The source control command that will be applied to all files in the `lpFileNames` array (see [Command Code Enumerator](../VS_csharp/command-code-enumerator.md) for a list of possible commands).  
 ...
 pfnPopulate  
 [in] The IDE callback function to call to add and remove files (see [POPLISTFUNC](../VS_csharp/poplistfunc.md) for details).  
 ...
 fOptions  
 [in] Command flags (see the "PopulateList flag" section of [Bitflags Used by Specific Commands](../VS_csharp/bitflags-used-by-specific-commands.md) for details).  
...
## Example  
 The IDE builds a list of files that it thinks the user can get. Before it displays this list, it calls the `SccPopulateList` function, giving the source control plug-in the opportunity to add and delete files from the list. The plug-in modifies the list by calling the given callback function (see [Building File Lists Using POPLISTFUNC Function](../VS_csharp/poplistfunc.md) for more details).  
...
## See Also  
 ...
 [Command Code Enumerator](../VS_csharp/command-code-enumerator.md)