...
# hash_map Class
> [!NOTE]
>  This API is obsolete. The alternative is [unordered_map Class](../VS_csharp/unordered_map-class.md).  
...
-   A template class, because the functionality it provides is generic and so independent of the specific type of data contained as elements or keys. The data types to be used for elements and keys are, instead, specified as parameters in the class template along with the comparison function and allocator.  
 ...
 The hash_map orders the sequence it controls by calling a stored hash `Traits` object of class [value_compare](../VS_csharp/value_compare-class.md). This stored object may be accessed by calling the member function [key_comp](#hash_map__key_comp). Such a function object must behave the same as an object of class [hash_compare](../VS_csharp/hash_compare-class.md)<Key, less<Key\>>. Specifically, for all values `Key` of type `Key`, the call `Traits`( `Key` ) yields a distribution of values of type `size_t`.  
...
### Remarks  
 If the argument key value is not found, then the function throws an object of class [out_of_range](../VS_csharp/out_of_range-class.md).  
...
### Return Value  
 A const bidirectional iterator addressing the first element in the [hash_map](../VS_csharp/hash_map-class.md) or the location succeeding an empty `hash_map`.  
...
### Return Value  
 A const bidirectional iterator that addresses the location succeeding the last element in a [hash_map](../VS_csharp/hash_map-class.md). If the `hash_map` is empty, then `hash_map::cend == hash_map::begin`.  
...
### Return Value  
 A const reverse bidirectional iterator addressing the first element in a reversed [hash_map](../VS_csharp/hash_map-class.md) or addressing what had been the last element in the unreversed `hash_map`.  
...
### Return Value  
 A const reverse bidirectional iterator that addresses the location succeeding the last element in a reversed [hash_map](../VS_csharp/hash_map-class.md) (the location that had preceded the first element in the unreversed `hash_map`).  
...
|-|-|  
|Parameter|Description|  
|`_Val`|The value used to move construct an element to be inserted into the [hash_map](../VS_csharp/hash_map-class.md) unless the `hash_map` already contains that element (or, more generally, an element whose key is equivalently ordered).|  
...
### Remarks  
 ...
 Beginning with Visual C++ .NET 2003, members of the [<hash_map>](../VS_csharp/-hash_map-.md) and [<hash_set>](../VS_csharp/-hash_set-.md) header files are no longer in the std namespace, but rather have been moved into the stdext namespace. See [The stdext Namespace](../VS_csharp/stdext-namespace.md) for more information.  
...
### Return Value  
 The [emplace](../VS_csharp/hash_multimap-class.md#hash_multimap__emplace) member function returns an iterator that points to the position where the new element was inserted into the `hash_map`, or where the existing element with equivalent ordering is located.  
...
### Remarks  
 ...
 For more information on `Traits` see the [hash_map Class](../VS_csharp/hash_map-class.md) topic.  
...
### Remarks  
 ...
 For more information on `Key`, see the Remarks section of the [hash_map Class](../VS_csharp/hash_map-class.md) topic.  
...
### Remarks  
 ...
 For more information on `Type` see the [hash_map Class](../VS_csharp/hash_map-class.md) topic.  
...
### Remarks  
 ...
 When using `operator[]` to insert elements, the returned reference does not indicate whether an insertion is changing a preexisting element or creating a new one. The member functions [find](../VS_csharp/map-class.md#map__find) and [insert](../VS_csharp/map-class.md#map__insert) can be used to determine whether an element with a specified key is already present before an insertion.  
...
|-|-|  
|Parameter|Description|  
|`_Right`|The [hash_map Class](../VS_csharp/hash_map-class.md) being copied into the `hash_map`.|  
