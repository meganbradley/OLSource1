...
---
# CScrollView Class
A [CView](../VS_csharp/cview-class.md) with scrolling capabilities.  
...
## Remarks  
 You can handle standard scrolling yourself in any class derived from `CView` by overriding the message-mapped [OnHScroll](../VS_csharp/cwnd-class.md#cwnd__onhscroll) and [OnVScroll](../VS_csharp/cwnd-class.md#cwnd__onvscroll) member functions. But `CScrollView` adds the following features to its `CView` capabilities:  
...
-   It scrolls automatically in response to messages from the keyboard, a non-scrolling mouse, or the IntelliMouse wheel.  
 ...
 You can handle mouse wheel scrolling yourself by overriding the message-mapped [OnMouseWheel](../VS_csharp/cwnd-class.md#cwnd__onmousewheel) and [OnRegisteredMouseWheel](../VS_csharp/cwnd-class.md#cwnd__onregisteredmousewheel) member functions. As they are for `CScrollView`, these member functions support the recommended behaviour for                 [WM_MOUSEWHEEL](http://msdn.microsoft.com/library/windows/desktop/ms645617), the wheel rotation message.  
  
 To take advantage of automatic scrolling, derive your view class from `CScrollView` instead of from `CView`. When the view is first created, if you want to calculate the size of the scrollable view based on the size of the document, call the `SetScrollSizes` member function from your override of either [CView::OnInitialUpdate](../VS_csharp/cview-class.md#cview__oninitialupdate) or [CView::OnUpdate](../VS_csharp/cview-class.md#cview__onupdate). (You must write your own code to query the size of the document. For an example, see the [Scribble sample](../VS_csharp/visual-c---samples.md).)  
...
-   The framework also forwards scroll messages from a `CSplitterWnd` splitter control to the currently active splitter pane (a view). When placed in a [CSplitterWnd](../VS_csharp/csplitterwnd-class.md) with shared scroll bars, a `CScrollView` object will use the shared ones rather than creating its own.  
  
 For more information on using `CScrollView`, see [Document/View Architecture](../VS_csharp/document-view-architecture.md) and [Derived View Classes Available in MFC](../VS_csharp/derived-view-classes-available-in-mfc.md).  
...
### Example  
 [!code[NVC_MFCDocView#164](../VS_csharp/codesnippet/CPP/cscrollview-class_1.cpp)]  
...
### Remarks  
 ...
 [!code[NVC_MFCDocView#165](../VS_csharp/codesnippet/CPP/cscrollview-class_2.cpp)]  
...
### Remarks  
 ...
 [!code[NVC_MFCDocView#166](../VS_csharp/codesnippet/CPP/cscrollview-class_3.cpp)]  
  
 Alternatively, you might sometimes need to set a fixed size, as in the following code:  
  
 [!code[NVC_MFCDocView#167](../VS_csharp/codesnippet/CPP/cscrollview-class_4.cpp)]  
...
### Example  
 [!code[NVC_MFCDocView#168](../VS_csharp/codesnippet/CPP/cscrollview-class_5.cpp)]  
  
 [!code[NVC_MFCDocView#169](../VS_csharp/codesnippet/CPP/cscrollview-class_6.cpp)]  
