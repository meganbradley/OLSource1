...
## Command IDs  
 See Visual C++ for an explanation of the command routing and binding process. [Technical Note 20](../VS_csharp/tn020--id-naming-and-numbering-conventions.md) contains information on ID naming.  
 ...
 **CFrameWnd** also handles the **WM_ENTERIDLE** message to describe the current menu item selected on the status bar (also known as the message line).  
 ...
 The same sort of **ON_UPDATE_COMMAND_UI** processing is done for toolbars (and other control bars) when the application enters its idle loop. See the *Class Library Reference* and [Technical Note 31](../VS_csharp/tn031--control-bars.md) for more information on control bars.  
...
## Window Message Routing  
 ...
 Please refer to [Technical Note 17](../VS_csharp/tn017--destroying-window-objects.md) for details on Window cleanup, a very important topic for all **CWnd**-derived classes.  
