...
-   "LNK2005 Errors on New and Delete When Defining _ATL_MIN_CRT" (Q184235).  
 ...
 This error is followed by fatal error [LNK1169](../VS_csharp/linker-tools-error-lnk1169.md).  
  
### To fix by checking the following possible causes  
  
1.  Mixing static and dynamic libraries when also using [/clr](../VS_csharp/-clr--common-language-runtime-compilation-.md).  
  
2.  The symbol is a packaged function (created by compiling with [/Gy](../VS_csharp/-gy--enable-function-level-linking-.md)) and was included in more than one file but was changed between compilations. Recompile all files that include `symbol`.  
...
5.  A header file declared and defined a variable. Possible solutions include:  
    ...
    -   Declare the variable [selectany](../VS_csharp/selectany.md).  
...
6.  If you use uuid.lib in combination with other .lib files that define GUIDs (for example, oledb.lib and adsiid.lib). For example:  
    ...
    ```  
     ...
     To fix, add [/FORCE:MULTIPLE](../VS_csharp/-force--force-file-output-.md) to the linker command line options, and make sure that uuid.lib is the first library referenced.