...
---
# CSharedFile Class
The [CMemFile](../VS_csharp/cmemfile-class.md)-derived class that supports shared memory files.  
 ...
 **GlobalAlloc** returns an `HGLOBAL` handle rather than a pointer to memory, such as the pointer returned by [malloc](../VS_csharp/malloc.md). The `HGLOBAL` handle is needed in certain applications. For example, to put data on the Clipboard you need an `HGLOBAL` handle.  
 ...
 For more information, see the article [Files in MFC](../VS_csharp/files-in-mfc.md) and [File Handling](../VS_csharp/file-handling.md) in the                 *Run-Time Library Reference*.  
...
## See Also  
 [Base Class](../VS_csharp/cmemfile-class.md)   
