...
## Return Value  
 ...
 If invalid parameters are passed to this function, the invalid-parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, this function returns –1 and sets `errno` to either `EBADF`, which indicates an invalid file descriptor, or `EINVAL`, which indicates an invalid `mode` argument.  
...
> [!CAUTION]
>  If you write data to a file stream, explicitly flush the code by using [fflush](../VS_csharp/fflush.md) before you use `_setmode` to change the mode. If you do not flush the code, you might get unexpected behavior. If you have not written data to the stream, you do not have to flush the code.  
