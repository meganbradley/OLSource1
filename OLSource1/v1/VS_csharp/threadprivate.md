...
## Remarks  
 ...
 For more information, see [2.7.1 threadprivate Directive](../VS_csharp/2.7.1-threadprivate-directive.md).  
  
 The `threadprivate` directive is based on the [thread](../VS_csharp/thread.md)`__declspec` attribute; limits on **__declspec(thread)** apply to `threadprivate`.  
  
 You cannot use `threadprivate` in any DLL that will be loaded via [LoadLibrary](http://msdn.microsoft.com/library/windows/desktop/ms684175).  This includes DLLs that are loaded with [/delayload](../VS_csharp/-delayload--delay-load-import-.md), which also uses **LoadLibrary**.  
...
## Example  
 For a sample of using `threadprivate`, see [private](../VS_csharp/private--openmp-.md).  
