...
The <xref:Microsoft.Build.Tasks.Windows.UpdateManifestForBrowserApplication*> task is run to add the **<hostInBrowser /\>** element to the application manifest (*projectname*.exe.manifest) when a [!INCLUDE[TLA#tla_xbap](../VS_csharp/includes/tlasharptla_xbap_md.md)] project is built.  
...
## Remarks  
 [!INCLUDE[TLA2#tla_xbap#plural](../VS_csharp/includes/tla2sharptla_xbapsharpplural_md.md)] are run by using [!INCLUDE[TLA#tla_clickonce](../VS_csharp/includes/tlasharptla_clickonce_md.md)] deployment and, therefore, must by published with supporting deployment and application manifests. [!INCLUDE[TLA#tla_msbuild](../VS_csharp/includes/tlasharptla_msbuild_md.md)] uses the [GenerateApplicationManifest](http://msdn2.microsoft.com/library/6wc2ccdc.aspx) task to generate an application manifest.  
...
```  
 ...
 The <xref:Microsoft.Build.Tasks.Windows.UpdateManifestForBrowserApplication*> task is run when an [!INCLUDE[TLA2#tla_xbap](../VS_csharp/includes/tla2sharptla_xbap_md.md)] project is built in order to add the `<hostInBrowser />` element.  
