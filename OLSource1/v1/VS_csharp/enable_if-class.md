...
-   Do not use `enable_if` to enforce requirements.  If you want to validate template parameters, and if the validation fails, cause an error instead of selecting another implementation, use [static_assert](../VS_csharp/static_assert.md).  
...
## Example  
 This example explains how the STL template function [std::make_pair()](../VS_csharp/-utility--functions.md#make_pair) takes advantage of `enable_if`.  
