...
1.  The given `symbol` may be a packaged function, created by compiling with [/Gy](../VS_csharp/-gy--enable-function-level-linking-.md). This symbol was included in more than one file but was changed between compilations. Recompile all files that include the `symbol`.  
