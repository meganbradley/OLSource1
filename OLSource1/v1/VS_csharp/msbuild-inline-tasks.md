...
MSBuild tasks are typically created by compiling a class that implements the <xref:Microsoft.Build.Framework.ITask*> interface. For more information, see [MSBuild Tasks](../VS_csharp/msbuild-tasks.md).  
...
## The Structure of an Inline Task  
 An inline task is contained by a [UsingTask](../VS_csharp/usingtask-element--msbuild-.md) element. The inline task and the `UsingTask` element that contains it are typically included in a .targets file and imported into other project files as required. Here is a basic inline task. Notice that it does nothing.  
...
> [!NOTE]
>  When defining the task class in the source file, the class name must agree with the `TaskName` attribute of the corresponding [UsingTask](../VS_csharp/usingtask-element--msbuild-.md) element.  
...
## See Also  
 ...
 [Walkthrough: Creating an Inline Task](../VS_csharp/walkthrough--creating-an-inline-task.md)