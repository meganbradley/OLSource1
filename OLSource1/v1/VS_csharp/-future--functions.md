...
### Parameters  
 `policy`  
 A [launch](../VS_csharp/-future--enums.md#launch_enumeration) value.  
...
|*Ty*|The type `result_of<Fn(ArgTypes…)>::type`.|  
 ...
 If `policy` is `launch::async`, the function creates a thread that evaluates `INVOKE(dfn, dargs..., Ty)`. The function returns after it creates the thread without waiting for results. If the system can't start a new thread, the function throws a [system_error](../VS_csharp/system_error-class.md) that has an error code of `resource_unavailable_try_again`.  
...
> [!NOTE]
>  For a `future`—or the last [shared_future](../VS_csharp/shared_future-class.md)—that's attached to a task started with `std::async`, the destructor blocks if the task has not completed; that is, it blocks if this thread did not yet call `.get()` or `.wait()` and the task is still running. If a `future` obtained from `std::async` is moved outside the local scope, other code that uses it must be aware that its destructor may block for the shared state to become ready.  
  
 The pseudo-function `INVOKE` is defined in [<functional\>](../VS_csharp/-functional-.md).  
  
##  <a name="future_category_function"></a>  future_category Function  
 Returns a reference to the [error_category](../VS_csharp/error_category-class.md) object that characterizes errors that are associated with `future` objects.  
...
##  <a name="make_error_code_function"></a>  make_error_code Function  
 Creates an [error_code](../VS_csharp/error_code-class.md) together with the [error_category](../VS_csharp/error_category-class.md) object that characterizes [future](../VS_csharp/future-class.md) errors.  
...
### Parameters  
 `Errno`  
 A [future_errc](../VS_csharp/-future--enums.md#future_errc_enumeration) value that identifies the reported error.  
  
### Return Value  
 `error_code(static_cast<int>(Errno), future_category());`  
  
##  <a name="make_error_condition_function"></a>  make_error_condition Function  
 Creates an [error_condition](../VS_csharp/error_condition-class.md) together with the [error_category](../VS_csharp/error_category-class.md) object that characterizes [future](../VS_csharp/future-class.md) errors.  
