...
One option for deploying a [!INCLUDE[smb_current_short](../VS_csharp/includes/smb_current_short_md.md)]-based application to a server running Internet Information Services (IIS) is to package the application in a .zip file. This package can be installed by a server administrator by using the **Import Application Package** wizard in the IIS Manager application.  
