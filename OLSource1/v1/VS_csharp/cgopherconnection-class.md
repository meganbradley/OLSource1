...
|Name|Description|  
|----------|-----------------|  
|[CGopherConnection::CreateLocator](#cgopherconnection__createlocator)|Creates a [CGopherLocator](../VS_csharp/cgopherlocator-class.md) object to find files on a gopher server.|  
...
## Remarks  
 ...
 To communicate with a gopher Internet server, you must first create an instance of [CInternetSession](../VS_csharp/cinternetsession-class.md), and then call [CInternetSession::GetGopherConnection](../VS_csharp/cinternetsession-class.md#cinternetsession__getgopherconnection), which creates the `CGopherConnection` object and returns a pointer to it. You never create a `CGopherConnection` object directly.  
  
 To learn more about how `CGopherConnection` works with the other MFC Internet classes, see the article [Internet Programming with WinInet](../VS_csharp/win32-internet-extensions--wininet-.md). For more information about using the other two supported Internet services, FTP and HTTP see the classes [CHttpConnection](../VS_csharp/chttpconnection-class.md) and [CFtpConnection](../VS_csharp/cftpconnection-class.md).  
...
### Remarks  
 You never create a `CGopherConnection` directly. Rather, call [CInternetSession::GetGopherConnection](../VS_csharp/cinternetsession-class.md#cinternetsession__getgopherconnection), which creates a `CGopherConnection` object and returns a pointer to it.  
 ...
 *dwGopherType*  
 This specifies whether `pstrSelectorString` refers to a directory or document, and whether the request is gopher or gopher+. See the attributes for the structure                                 [GOPHER_FIND_DATA](http://msdn.microsoft.com/library/windows/desktop/aa384215) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)].  
  
 `pstrLocator`  
 A pointer to a string identifying the file to open. Generally, this string is returned from a call to [CGopherFileFind::GetLocator](../VS_csharp/cgopherfilefind-class.md#cgopherfilefind__getlocator).  
...
### Return Value  
 A [CGopherLocator](../VS_csharp/cgopherlocator-class.md) object.  
 ...
 *strRequestedAttributes*  
 ...
 `strResult`  
 A reference to a [CString](../VS_csharp/cstringt-class.md) that receives the locator type.  
...
### Parameters  
 ...
 `dwFlags`  
 Any combination of INTERNET_FLAG_* flags. See [CInternetSession::OpenUrl](../VS_csharp/cinternetsession-class.md#cinternetsession__openurl) for further information on INTERNET_FLAG_\* flags.  
...
### Return Value  
 A pointer to the [CGopherFile](../VS_csharp/cgopherfile-class.md) object to be opened.  
  
### Remarks  
 Override the `dwContext` default to set the context identifier to a value of your choosing. The context identifier is associated with this specific operation of the `CGopherConnection` object created by its [CInternetSession](../VS_csharp/cinternetsession-class.md) object. The value is returned to [CInternetSession::OnStatusCallback](../VS_csharp/cinternetsession-class.md#cinternetsession__onstatuscallback) to provide status on the operation with which it is identified. See the article [Internet First Steps: WinInet](../VS_csharp/wininet-basics.md) for more information about the context identifier.  
