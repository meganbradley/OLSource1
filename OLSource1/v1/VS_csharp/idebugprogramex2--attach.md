...
#### Parameters  
 `pCallback`  
 [in] An [IDebugEventCallback2](../VS_csharp/idebugeventcallback2.md) object that represents the callback function that the attached debug engine sends events to.  
  
 `dwReason`  
 [in] A value from the [ATTACH_REASON](../VS_csharp/attach_reason.md) enumeration that describes the reason for the attach operation.  
