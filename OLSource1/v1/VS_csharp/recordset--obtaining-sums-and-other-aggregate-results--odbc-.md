...
This topic applies to the MFC ODBC classes.  
 ...
 This topic explains how to obtain aggregate results using the following [SQL](../VS_csharp/sql.md) keywords:  
...
1.  Create a recordset as described in [Adding an MFC ODBC Consumer](../VS_csharp/adding-an-mfc-odbc-consumer.md) containing the columns from which you want to obtain aggregate results.  
  
2.  Modify the [DoFieldExchange](../Topic/CRecordset::DoFieldExchange.md) function for the recordset. Replace the string representing the column name (the second argument of the [RFX](../VS_csharp/record-field-exchange--using-rfx.md) function calls) with a string representing the aggregation function on the column. For example, replace:  
...
>  The wizard actually assigns data member names without Hungarian prefixes. For example, the wizard would produce `m_Sales` for a Sales column, rather than the `m_lSales` name used earlier for illustration.  
 ...
 If you are using a [CRecordView](../VS_csharp/crecordview-class.md) class to view the data, you have to change the DDX function call to display the new data member value; in this case, changing it from:  
