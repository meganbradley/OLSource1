...
-   if it was constructed from a [weak_ptr](../VS_csharp/weak_ptr-class.md) object that points to that resource, or  
...
### Remarks  
 The constructors each construct an object that owns the resource named by the operand sequence. The constructor `shared_ptr(const weak_ptr<Other>& wp)` throws an exception object of type [bad_weak_ptr](../VS_csharp/bad_weak_ptr-class.md) if `wp.expired()`.  
...
**sp1.use_count() == 2**    
## See Also  
 [weak_ptr](../VS_csharp/weak_ptr-class.md)   
