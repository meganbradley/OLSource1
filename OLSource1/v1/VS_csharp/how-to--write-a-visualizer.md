...
-   The *debuggee side* runs within the process Visual Studio is debugging (the *debuggee*).  
 ...
 Support for generic types is limited. You can write a visualizer for a target that is a generic type only if the generic type is an open type. This restriction is the same as the restriction when using the `DebuggerTypeProxy` attribute. For details, see [Using DebuggerTypeProxy Attribute](../VS_csharp/using-debuggertypeproxy-attribute.md).  
  
 Custom visualizers may have security considerations. See [Visualizer Security Considerations](../VS_csharp/visualizer-security-considerations.md).  
  
 The procedures below, give a high-level view of what you need to do to create a visualizer. For a more detailed explanation, see [Walkthrough: Writing a Visualizer](../VS_csharp/walkthrough--writing-a-visualizer-in-csharp.md).  
...
## See Also  
 ...
 [Visualizer Security Considerations](../VS_csharp/visualizer-security-considerations.md)