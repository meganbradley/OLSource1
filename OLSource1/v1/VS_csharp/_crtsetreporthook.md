...
## Remarks  
 `_CrtSetReportHook` allows an application to use its own reporting function into the C run-time debug library reporting process. As a result, whenever [_CrtDbgReport](../VS_csharp/_crtdbgreport--_crtdbgreportw.md) is called to generate a debug report, the application's reporting function is called first. This functionality enables an application to perform operations such as filtering debug reports so it can focus on specific allocation types or send a report to destinations not available by using `_CrtDbgReport`. When [_DEBUG](../VS_csharp/_debug.md) is not defined, calls to `_CrtSetReportHook` are removed during preprocessing.  
  
 For a more robust version of `_CrtSetReportHook`, see [_CrtSetReportHook2](../VS_csharp/_crtsetreporthook2--_crtsetreporthookw2.md).  
...
## See Also  
 ...
 [_CrtGetReportHook](../VS_csharp/_crtgetreporthook.md)