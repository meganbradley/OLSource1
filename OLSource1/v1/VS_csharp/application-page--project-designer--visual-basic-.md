...
> [!NOTE]
>  If you use the Global keyword in a [Namespace Statement](../VS_csharp/namespace-statement.md), you can define a namespace out of the root namespace of your project. If you clear the **Root Namespace**, `Global` becomes the top-level namespace, which removes the need for the `Global` keyword in a `Namespace` statement. For more information, see "Global Keyword in Namespace Statements" in [Namespaces in Visual Basic](../VS_csharp/namespaces-in-visual-basic.md).  
  
 For information about how to create namespaces in your code, see [Namespace Statement](../VS_csharp/namespace-statement.md).  
  
 For more information about the root namespace property, see [/rootnamespace](../VS_csharp/-rootnamespace.md).  
...
> [!NOTE]
>  The prerequisite packages that are listed in the [Prerequisites Dialog Box](../VS_csharp/prerequisites-dialog-box.md) are set automatically when you open the dialog box for the first time. If you subsequently change the project's target framework, you must specify the prerequisites manually to match the new target framework.  
  
 For more information, see [How To: Target a Specific .NET Framework](../VS_csharp/how-to--target-a-version-of-the-.net-framework.md) and [.NET Framework Multi-Targeting Overview](../VS_csharp/visual-studio-multi-targeting-overview.md).  
  
 **Application type**  
 Specifies the type of application to build. For [!INCLUDE[win8_appname_long](../VS_csharp/includes/win8_appname_long_md.md)] apps, you can specify **Windows Store App**, **Class Library**, or **WinMD File**. For most other application types, you can specify **Windows Application**, **Console Application**, **Class Library**, **Windows Service**, or **Web Control Library**.  
  
 For a web application project, you must specify **Class Library**.  
  
 If you specify the **WinMD File** option, types can be projected into any Windows Runtime programming language. By packaging the project's output as a WinMD file, you can code an application in multiple languages and have code interoperate as if you wrote it all in the same language. You can use the **WinMD File** option for solutions that target the Windows Runtime libraries, including [!INCLUDE[win8_appname_long](../VS_csharp/includes/win8_appname_long_md.md)] apps. For more information, see [Creating Windows Runtime Components in C# and Visual Basic](http://go.microsoft.com/fwlink/?LinkId=231895).  
...
>  The Windows Runtime can project types so that they appear as native objects in whichever language uses them. For example, JavaScript applications that interact with Windows Runtime use it as a set of JavaScript objects, and C# applications use the library as a collection of .NET objects. By packaging the project’s output as a WinMD file, you can take advantage of the same technology that Windows Runtime uses.  
 ...
 For more information about the **Application type** property, see [/target (Visual Basic)](../VS_csharp/-target--visual-basic-.md). For information about how to access that property programmatically, see <xref:VSLangProj.ProjectProperties.OutputType*>.  
  
 **Icon**  
 Sets the .ico file that you want to use as your program icon. Select **<Browse...>** to browse for an existing graphic. See [/win32icon](../VS_csharp/-win32icon.md) (or [/win32icon (Import an .ico File) (C# Compiler Options)](../VS_csharp/-win32icon--csharp-compiler-options-.md)) for more information. To access this property programmatically, see <xref:VSLangProj.ProjectProperties.ApplicationIcon*>.  
 ...
 **Startup object** defines the entry point to be called when the application loads. Generally this is set to either the main form in your application or to the `Sub Main` procedure that should run when the application starts. Because class libraries do not have an entry point, their only option for this property is **(None)**. For more information, see [/main](../VS_csharp/-main.md). To access this property programmatically, see <xref:VSLangProj.ProjectProperties.StartupObject*>.  
  
 **Assembly Information**  
 Click this button to display the [Assembly Information Dialog Box](../VS_csharp/assembly-information-dialog-box.md).  
 ...
 **View Windows Settings**  
 ...
 For more information about manifest generation, see [ClickOnce Deployment on Windows Vista](../VS_csharp/clickonce-deployment-on-windows-vista.md).  
...
## See Also  
 [Managing Application Properties](../VS_csharp/application-properties.md)   
