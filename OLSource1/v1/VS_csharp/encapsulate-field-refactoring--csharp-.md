...
The **Encapsulate Field** refactoring operation enables you to quickly create a property from an existing field, and then seamlessly update your code with references to the new property.  
 ...
 When a [field](../VS_csharp/fields--csharp-programming-guide-.md) is [public](../VS_csharp/public--csharp-reference-.md), other objects have direct access to that field and can modify it, undetected by the object that owns that field. By using [properties](../VS_csharp/properties--csharp-programming-guide-.md) to encapsulate that field, you can disallow direct access to fields.  
  
 To create the new property, the **Encapsulate Field** operation changes the access modifier for the field that you want to encapsulate to [private](../VS_csharp/private--csharp-reference-.md), and then generates [get](../VS_csharp/get--csharp-reference-.md) and [set](../VS_csharp/set--csharp-reference-.md) accessors for that field. In some cases, only a `get` accessor is generated, such as when the field is declared read-only.  
...
2.  In the [Code Editor](../VS_csharp/writing-code-in-the-code-and-text-editor.md), place the cursor in the declaration, on the name of the field that you want to encapsulate. In the example below, place the cursor on the word `width`:  
...
## Remarks  
 ...
 The code that is generated by this refactoring operation is modeled by the encapsulate field code snippets feature. Code Snippets are modifiable. For more information, see [Code Snippets](../VS_csharp/code-snippets.md).  
