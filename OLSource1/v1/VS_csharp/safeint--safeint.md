...
## Remarks  
 For more information about the template types `T` and `E`, see [SafeInt Class](../VS_csharp/safeint-class.md).  
  
 The input parameter for the constructor, `i` or `u`, must be a Boolean, character, or integer type. If it is another type of parameter, the `SafeInt` class calls [static_assert](../VS_csharp/static_assert.md) to indicate an invalid input parameter.  
