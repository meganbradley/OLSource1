...
1.  Click **Start**, click **Control Panel**, and then open **Regional and Language Options** (or **Region** in [!INCLUDE[win8](../VS_csharp/includes/win8_md.md)]).  
...
## Changing the Language Used for the UI Text in Visual Studio  
 When you install multiple language versions of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] on the same computer, the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] UI defaults to **Same as Microsoft Windows**. This setting indicates that [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] will display the UI text in the language that is specified as the display language for the operating system.  
...
3.  In the **Language** list, choose the language in which the UI text should appear in the development environment.  
 ...
 You can also use the devenv command to set the language that's used for UI. For more information, see [/lcid (devenv.exe)](../VS_csharp/-lcid--devenv.exe-.md).  
  
## See Also  
 [International Settings, Environment, Options Dialog Box](../VS_csharp/international-settings--environment--options-dialog-box.md)