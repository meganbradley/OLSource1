...
#### Parameters  
 `scanner`  
 Type: `'Msg ->` [Async](../VS_csharp/control.async--t--type--fsharp-.md)`<'T>` [option](../VS_csharp/core.option--t--union--fsharp-.md)  
...
## Return Value  
 An asynchronous computation ([Async](../VS_csharp/control.async-class--fsharp-.md) object) that `scanner` built off the read message.  
  
## Remarks  
 If a timeout period is exceeded, `None` is returned. This method is for use within the body of the agent. For each agent, at most one concurrent reader can be active, so no more than one concurrent call to [Receive](../VS_csharp/mailboxprocessor.receive--msg--method--fsharp-.md), [TryReceive](../VS_csharp/mailboxprocessor.tryreceive--msg--method--fsharp-.md), [Scan](../VS_csharp/mailboxprocessor.scan--msg--t--method--fsharp-.md) or `TryScan` can be active. The body of the `scanner` function is locked during its execution, but the lock is released before the execution of the asynchronous workflow.  
...
## Example  
 ...
 [!code[FsMailboxProcessor#22](../VS_csharp/codesnippet/FSharp/mailboxprocessor.tryscan--msg--t--method--fsharp-_1.fs)]  
