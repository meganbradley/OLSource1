...
> [!NOTE]
>  MFC does not use the [Run-Time Type Information](../VS_csharp/run-time-type-information.md) (RTTI) support introduced in Visual C++ 4.0.  
  
 If you have derived your class from [CObject](../VS_csharp/cobject-class.md) and used the **DECLARE**_**DYNAMIC** and `IMPLEMENT_DYNAMIC`, the `DECLARE_DYNCREATE` and `IMPLEMENT_DYNCREATE`, or the `DECLARE_SERIAL` and `IMPLEMENT_SERIAL` macros explained in the article [Deriving a Class from CObject](../VS_csharp/deriving-a-class-from-cobject.md), the `CObject` class has the ability to determine the exact class of an object at run time.  
...
1.  Use `RUNTIME_CLASS` with the name of the class, as shown here for the class `CObject`:  
     ...
     [!code[NVC_MFCCObjectSample#4](../VS_csharp/codesnippet/CPP/accessing-run-time-class-information_1.cpp)]  
...
2.  Call the `IsKindOf` member function for objects of that class, using the `RUNTIME_CLASS` macro to generate the `CRuntimeClass` argument, as shown here:  
     ...
     [!code[NVC_MFCCObjectSample#2](../VS_csharp/codesnippet/CPP/accessing-run-time-class-information_2.h)]  
  
     [!code[NVC_MFCCObjectSample#5](../VS_csharp/codesnippet/CPP/accessing-run-time-class-information_3.cpp)]  
 ...
 One use of run-time class information is in the dynamic creation of objects. This process is discussed in the article [Dynamic Object Creation](../VS_csharp/dynamic-object-creation.md).  
  
 For more detailed information on serialization and run-time class information, see the articles [Files in MFC](../VS_csharp/files-in-mfc.md) and [Serialization](../VS_csharp/serialization-in-mfc.md).  
