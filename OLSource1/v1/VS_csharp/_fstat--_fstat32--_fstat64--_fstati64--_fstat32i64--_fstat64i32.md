...
## Return Value  
 Returns 0 if the file-status information is obtained. A return value of –1 indicates an error. If the file descriptor is invalid or `buffer` is `NULL`, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, `errno` is set to `EBADF`, in the case of an invalid file descriptor, or to `EINVAL`, if `buffer` is `NULL`.  
...
## Remarks  
 ...
 Because Stat.h uses the [_dev_t](../VS_csharp/standard-types.md) type, which is defined in Types.h, you must include Types.h before Stat.h in your code.  
...
## See Also  
 ...
 [_filelength, _filelengthi64](../VS_csharp/_filelength--_filelengthi64.md)   
