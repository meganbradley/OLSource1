...
---
# _umask_s
Sets the default file-permission mask. A version of [_umask](../VS_csharp/_umask.md) with security enhancements as described in [Security Enhancements in the CRT](../VS_csharp/security-features-in-the-crt.md).  
...
|invalid mode|any|`EINVAL`|not modified|  
 ...
 If one of the above conditions occurs, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, `_umask_s` returns `EINVAL` and sets `errno` to `EINVAL`.  
