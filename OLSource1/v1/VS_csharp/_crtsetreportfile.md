...
## Return Value  
 On successful completion, `_CrtSetReportFile` returns the previous report file defined for the report type specified in `reportType`. If an invalid value is passed in for `reportType`, this function invokes the invalid parameter handler, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, `errno` is set to `EINVAL` and the function returns `_CRTDBG_HFILE_ERROR`. For more information, see [errno, _doserrno, _sys_errlist, and _sys_nerr](../VS_csharp/errno--_doserrno--_sys_errlist--and-_sys_nerr.md).  
  
## Remarks  
 `_CrtSetReportFile` is used with the [_CrtSetReportMode](../VS_csharp/_crtsetreportmode.md) function to define the destination or destinations for a specific report type generated by `_CrtDbgReport`. When `_CrtSetReportMode` has been called to assign the `_CRTDBG_MODE_FILE` reporting mode for a specific report type, `_CrtSetReportFile` should then be called to define the specific file or stream to use as the destination. When [_DEBUG](../VS_csharp/_debug.md) is not defined, calls to `_CrtSetReportFile` are removed during preprocessing.  
