...
#### Parameters  
 `start_address`  
 Start address of a routine that begins execution of a new thread. For `_beginthread`, the calling convention is either [__cdecl](../VS_csharp/__cdecl.md) (for native code) or [__clrcall](../VS_csharp/__clrcall.md) (for managed code); for `_beginthreadex`, it is either [__stdcall](../VS_csharp/__stdcall.md) (for native code) or [__clrcall](../VS_csharp/__clrcall.md) (for managed code).  
...
## Return Value  
 ...
 If `startaddress` is NULL, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, these functions set `errno` to `EINVAL` and return -1.  
...
-   A thread that's created by using `_beginthreadex` is terminated by a call to [_endthreadex](../VS_csharp/_endthread--_endthreadex.md).  
...
>  For an executable file linked with Libcmt.lib, do not call the Win32 `ExitThread` API so that you don't prevent the run-time system from reclaiming allocated resources. `_endthread` and `_endthreadex` reclaim allocated thread resources and then call `ExitThread`.  
 ...
 The locale of the new thread is inherited from its parent thread. If per-thread locale is enabled by a call to [_configthreadlocale](../VS_csharp/_configthreadlocale.md) (either globally or for new threads only), the thread can change its locale independently from its parent by calling `setlocale` or `_wsetlocale`. For more information, see [Locale](../VS_csharp/locale.md).  
  
 For mixed and pure code, `_beginthread` and `_beginthreadex` each have two overloads—one takes a native calling-convention function pointer, the other takes a `__clrcall` function pointer. The first overload is not application domain-safe and never will be. If you are writing mixed or pure code you must ensure that the new thread enters the correct application domain before it accesses managed resources. You can do this, for example, by using [call_in_domain](../VS_csharp/call_in_appdomain-function.md). The second overload is application domain-safe; the newly created thread will always end up in the application domain of the caller of `_beginthread` or `_beginthreadex`.  
...
## Libraries  
 Multithreaded versions of the [C run-time libraries](../VS_csharp/crt-library-features.md) only.  
...
## See Also  
 ...
 [_endthread, _endthreadex](../VS_csharp/_endthread--_endthreadex.md)   
