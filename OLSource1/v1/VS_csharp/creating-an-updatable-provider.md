...
-   If you inherit from [IRowsetChangeImpl](../VS_csharp/irowsetchangeimpl-class.md), calling these three methods immediately changes the data store.  
  
-   If you inherit from [IRowsetUpdateImpl](../VS_csharp/irowsetupdateimpl-class.md), the methods defer changes to the data store until you call **Update**, `GetOriginalData`, or **Undo**. If the update involves several changes, they are performed in batch mode (note that batching changes can add considerable memory overhead).  
