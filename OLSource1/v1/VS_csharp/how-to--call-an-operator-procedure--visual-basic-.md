...
You call an operator procedure by using the operator symbol in an expression. In the case of a conversion operator, you call the [CType Function](../VS_csharp/ctype-function--visual-basic-.md) to convert a value from one data type to another.  
  
 You do not call operator procedures explicitly. You just use the operator, or the `CType` function, in an assignment statement or an expression, the same way you ordinarily use an operator. [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)] makes the call to the operator procedure.  
...
## Example  
 ...
 [!code[VbVbcnProcedures#29](../VS_csharp/codesnippet/VisualBasic/how-to--call-an-operator-procedure--visual-basic-_1.vb)]  
 ...
 For an example of calling a conversation operator procedure, see [How to: Use a Class that Defines Operators](../VS_csharp/how-to--use-a-class-that-defines-operators--visual-basic-.md).  
