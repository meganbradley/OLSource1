...
## Remarks  
 The Resource Manager invokes the `RemoveVirtualProcessors` method to take back a set of virtual processor roots from a scheduler. The scheduler is expected to invoke the [Remove](../VS_csharp/iexecutionresource--remove-method.md) method on each interface when it is done with the virtual processor roots. Do not use an `IVirtualProcessorRoot` interface once you have invoked the `Remove` method on it.  
...
## See Also  
 ...
 [IScheduler::RemoveVirtualProcessors Method](../VS_csharp/ischeduler--removevirtualprocessors-method.md)