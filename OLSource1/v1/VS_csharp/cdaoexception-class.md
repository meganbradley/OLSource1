...
|[CDaoException::m_pErrorInfo](#cdaoexception__m_perrorinfo)|A pointer to a [CDaoErrorInfo](../VS_csharp/cdaoerrorinfo-structure.md) object that contains information about one DAO error object.|  
...
>  The DAO database classes are distinct from the MFC database classes based on Open Database Connectivity (ODBC). All DAO database class names have the "CDao" prefix. You can still access ODBC data sources with the DAO classes. In general, the MFC classes based on DAO are more capable than the MFC classes based on ODBC; the DAO-based classes can access data, including through ODBC drivers, via their own database engine. The DAO-based classes also support Data Definition Language (DDL) operations, such as adding tables via the classes, without having to call DAO directly. For information on exceptions thrown by the ODBC classes, see [CDBException](../VS_csharp/cdbexception-class.md).  
 ...
 For more information about exception handling in general, or about `CDaoException` objects, see the articles [Exception Handling (MFC)](../VS_csharp/exception-handling-in-mfc.md) and [Exceptions: Database Exceptions](../VS_csharp/exceptions--database-exceptions.md). The second article contains example code that illustrates exception handling in DAO.  
...
### Remarks  
 Ordinarily, the framework creates exception objects when its code throws an exception. You seldom need to construct an exception object explicitly. If you want to throw a `CDaoException` from your own code, call the global function [AfxThrowDaoException](../VS_csharp/afxthrowdaoexception.md).  
...
4.  Examine the current [CDaoErrorInfo](../VS_csharp/cdaoerrorinfo-structure.md) structure that `GetErrorInfo` returns in the [m_pErrorInfo](#cdaoexception__m_perrorinfo) data member. Its members provide information on the DAO error.  
...
6.  If you constructed the exception object on the heap, delete it with the **delete** operator when you finish.  
 ...
 For more information about handling errors in the MFC DAO classes, see the article [Exceptions: Database Exceptions](../VS_csharp/exceptions--database-exceptions.md).  
...
|**m_lHelpContext**|Help Context|The context ID for a topic in the DAO Help file|  
 ...
 For full details about the information contained in the `CDaoErrorInfo` object, see the [CDaoErrorInfo](../VS_csharp/cdaoerrorinfo-structure.md) structure.  
...
### Remarks  
 ...
 For information about `SCODE`, see the topic                         [Structure of OLE Error Codes](http://msdn.microsoft.com/library/windows/desktop/ms690088) in the [!INCLUDE[winSDK](../VS_csharp/includes/winsdk_md.md)]. The `SCODE` data type maps to the `HRESULT` data type.  
