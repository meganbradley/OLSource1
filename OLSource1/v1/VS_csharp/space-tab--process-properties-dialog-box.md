...
Use the **Space** tab to examine the address space of a process. To display the [Process Properties Dialog Box](../VS_csharp/process-properties-dialog-box.md), move the focus to a [Processes View](../VS_csharp/processes-view.md) window. Select any process node in the tree, then choose **Properties** from the **View** menu.  
