...
## Remarks  
 The first member function removes the element of the controlled sequence pointed to by `where`, and returns an iterator that designates the first element remaining beyond the element removed, or [end](../VS_csharp/map--end--stl-clr-.md)`()` if no such element exists. You use it to remove a single element.  
...
## See Also  
 ...
 [clear](../VS_csharp/map--clear--stl-clr-.md)