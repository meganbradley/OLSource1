...
## Remarks  
 All overloads require `n * sizeof(T)` additional space, where `n` is the number of elements to be sorted, and `T` is the element type. In most cases parallel_buffered_sort will show an improvement in performance over [parallel_sort](../VS_csharp/parallel_sort-function.md), and you should use it over parallel_sort if you have the memory available.  
