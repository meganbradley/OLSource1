...
---
# _lfind_s
Performs a linear search for the specified key. A version of [_lfind](../VS_csharp/_lfind.md) with security enhancements as described in [Security Enhancements in the CRT](../VS_csharp/security-features-in-the-crt.md).  
...
## Return Value  
 ...
 If invalid parameters are passed to the function, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, `errno` is set to `EINVAL` and the function returns `NULL`.  
