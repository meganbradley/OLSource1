...
```  
 ...
 A [throw](../VS_csharp/throw--csharp-reference-.md) statement can be used in a `catch` block to re-throw the exception that is caught by the `catch` statement. The following example extracts source information from an <xref:System.IO.IOException*> exception, and then throws the exception to the parent method.  
...
```  
 ...
 For more information about catch, see [try-catch-finally](../VS_csharp/try-catch-finally--csharp-reference-.md).  
  
## Exceptions in Async Methods  
 An async method is marked  by an  [async](../VS_csharp/async--csharp-reference-.md) modifier and usually contains one or more await expressions or statements. An await expression applies the [await](../VS_csharp/await--csharp-reference-.md) operator to a <xref:System.Threading.Tasks.Task*> or <xref:System.Threading.Tasks.Task`1*>.  
  
 When control reaches an `await` in the async method, progress in the method is suspended until the awaited task completes. When the task  is complete, execution can resume in the method. For more information, see [Asynchronous Programming with Async and Await (C# and Visual Basic)](../VS_csharp/asynchronous-programming-with-async-and-await--csharp-and-visual-basic-.md) and [Control Flow in Async Programs (C# and Visual Basic)](../VS_csharp/control-flow-in-async-programs--csharp-and-visual-basic-.md).  
  
 The completed task to which `await` is applied might be in a faulted state because of an unhandled exception in the method that returns the task. Awaiting the task throws an exception. A task can also end up in a canceled state if the asynchronous process that returns it is canceled. Awaiting a canceled task throws  an `OperationCanceledException`. For more information about how to cancel an asynchronous process, see [Fine Tuning Your Async Application (C# and Visual Basic)](../VS_csharp/fine-tuning-your-async-application--csharp-and-visual-basic-.md).  
...
## Example  
 ...
 [!code[csrefKeywordsExceptions#2](../VS_csharp/codesnippet/CSharp/try-catch--csharp-reference-_1.cs)]  
...
## Example  
 ...
 [!code[csrefKeywordsExceptions#3](../VS_csharp/codesnippet/CSharp/try-catch--csharp-reference-_2.cs)]  
...
## Example  
 ...
 [!code[csAsyncExceptions#2](../VS_csharp/codesnippet/CSharp/try-catch--csharp-reference-_3.cs)]  
...
## Example  
 ...
 [!code[csAsyncExceptions#4](../VS_csharp/codesnippet/CSharp/try-catch--csharp-reference-_4.cs)]  
...
## See Also  
 ...
 [try, throw, and catch Statements (C++)](../VS_csharp/try--throw--and-catch-statements--c---.md)   
 [Exception Handling Statements](../VS_csharp/exception-handling-statements--csharp-reference-.md)   
 [throw](../VS_csharp/throw--csharp-reference-.md)   
