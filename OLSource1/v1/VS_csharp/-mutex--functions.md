...
##  <a name="adopt_lock_variable"></a>  adopt_lock Variable  
 Represents an object that can be passed to constructors for [lock_guard](../VS_csharp/lock_guard-class.md) and [unique_lock](../VS_csharp/unique_lock-class.md) to indicate that the mutex object that is also being passed to the constructor is locked.  
...
### Parameters  
 `Flag`  
 A [once_flag](../VS_csharp/once_flag-structure.md) object that ensures that the callable object is only called once.  
...
### Remarks  
 If `Flag` is not valid, the function throws a [system_error](../VS_csharp/system_error-class.md) that has an error code of `invalid_argument`. Otherwise, the template function uses its `Flag` argument to ensure that it calls `F(A...)` successfully exactly once, regardless of how many times the template function is called. If `F(A...)` exits by throwing an exception, the call was not successful.  
  
##  <a name="defer_lock_variable"></a>  defer_lock Variable  
 Represents an object that can be passed to the constructor for [unique_lock](../VS_csharp/unique_lock-class.md). This indicates that the constructor should not lock the mutex object that's also being passed to it.  
...
##  <a name="try_to_lock_variable"></a>  try_to_lock Variable  
 Represents an object that can be passed to the constructor for [unique_lock](../VS_csharp/unique_lock-class.md) to indicate that the constructor should try to unlock the `mutex` that is also being passed to it without blocking.  
