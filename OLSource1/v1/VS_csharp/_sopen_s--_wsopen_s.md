...
---
# _sopen_s, _wsopen_s
Opens a file for sharing. These versions of [_sopen and _wsopen](../VS_csharp/_sopen--_wsopen.md) have security enhancements, as described in [Security Enhancements in the CRT](../VS_csharp/security-features-in-the-crt.md).  
...
## Return Value  
 ...
 If an invalid argument is passed to the function, the invalid parameter handler is invoked, as described in [Parameter Validation](../VS_csharp/parameter-validation.md). If execution is allowed to continue, `errno` is set to `EINVAL` and `EINVAL` is returned.  
...
|`_tsopen_s`|`_sopen_s`|`_sopen_s`|`_wsopen_s`|  
 ...
 `_O_BINARY`  
 Opens a file in binary (untranslated) mode. (See [fopen](../VS_csharp/fopen--_wfopen.md) for a description of binary mode.)  
...
>  The `_O_TRUNC` flag destroys the contents of the specified file.  
 ...
 `_sopen_s` applies the current file-permission mask to `pmode` before the permissions are set. (See [_umask](../VS_csharp/_umask.md).)  
...
|`_wsopen_s`|<io.h> or <wchar.h>|<fcntl.h>, <sys/types.h>, <sys/stat.h>, <share.h>|  
 ...
 `_sopen_s` and `_wsopen_s` are Microsoft extensions. For more compatibility information, see [Compatibility](../VS_csharp/compatibility.md).  
  
## Example  
 See the example for [_locking](../VS_csharp/_locking.md).  
