...
This article explains how a [CSocket](../VS_csharp/csocket-class.md) object, a [CSocketFile](../VS_csharp/csocketfile-class.md) object, and a [CArchive](../VS_csharp/carchive-class.md) object are combined to simplify sending and receiving data through a Windows Socket.  
  
 The article [Windows Sockets: Example of Sockets Using Archives](../VS_csharp/windows-sockets--example-of-sockets-using-archives.md) presents the **PacketSerialize** function. The archive object in the **PacketSerialize** example works much like an archive object passed to an MFC [Serialize](../Topic/CObject::Serialize.md) function. The essential difference is that for sockets, the archive is attached not to a standard [CFile](../VS_csharp/cfile-class.md) object (typically associated with a disk file) but to a `CSocketFile` object. Rather than connecting to a disk file, the `CSocketFile` object connects to a `CSocket` object.  
 ...
 Class `CSocketFile` derives from `CFile`, but it does not support [CFile](../VS_csharp/cfile-class.md) member functions such as the positioning functions (`Seek`, `GetLength`, `SetLength`, and so on), the locking functions (`LockRange`, `UnlockRange`), or the `GetPosition` function. All the [CSocketFile](../VS_csharp/csocketfile-class.md) object must do is write or read sequences of bytes to or from the associated `CSocket` object. Because a file is not involved, operations such as `Seek` and `GetPosition` make no sense. `CSocketFile` is derived from `CFile`, so it would normally inherit all of these member functions. To prevent this, the unsupported `CFile` member functions are overridden in `CSocketFile` to throw a [CNotSupportedException](../VS_csharp/cnotsupportedexception-class.md).  
...
CArchive, CSocketFile, and CSocket  
 ...
 [!code[NVC_MFCSimpleSocket#2](../VS_csharp/codesnippet/CPP/windows-sockets--how-sockets-with-archives-work_1.cpp)]  
