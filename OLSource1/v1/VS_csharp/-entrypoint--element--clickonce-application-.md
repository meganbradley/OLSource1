...
Identifies the assembly that should be executed when this [!INCLUDE[ndptecclick](../VS_csharp/includes/ndptecclick_md.md)] application is run on a client computer.  
...
## assemblyIdentity  
 Required. The role of `assemblyIdentity` and its attributes is defined in [<assemblyIdentity\> Element (ClickOnce Reference)](../VS_csharp/-assemblyidentity--element--clickonce-application-.md).  
...
|Attribute|Description|  
|---------------|-----------------|  
|`file`|Required. A local reference to the startup assembly for the [!INCLUDE[ndptecclick](../VS_csharp/includes/ndptecclick_md.md)] application. This value cannot contain forward slash (/) or backslash (\\) path separators.|  
...
## customHostRequired  
 ...
 If this element is present, the `assemblyIdentity` and `commandLine` elements must not also be present. If they are, [!INCLUDE[ndptecclick](../VS_csharp/includes/ndptecclick_md.md)] will raise a validation error during installation.  
...
## Example  
 The following code example illustrates an `entryPoint` element in an application manifest for a [!INCLUDE[ndptecclick](../VS_csharp/includes/ndptecclick_md.md)] application. This code example is part of a larger example provided for the [ClickOnce Application Manifest](../VS_csharp/clickonce-application-manifest.md) topic.  
