...
These general guidelines might be helpful if you are generating program code or other application resources in [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)]. They are not fixed rules.  
  
## Guidelines for Design-Time T4 Templates  
 Design-time T4 templates are templates that generate code in your [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] project at design time. For more information, see [Design-time templates](../VS_csharp/design-time-code-generation-by-using-t4-text-templates.md).  
 ...
 Allow for code that you write by hand in addition to the generated code. It is unusual for a code generation scheme to be able to account for all possible variations that might arise. Therefore, you should expect to add to or override some of the generated code. Where the generated material is in a .NET language such as [!INCLUDE[csprcs](../VS_csharp/includes/csprcs_md.md)] or [!INCLUDE[vbprvb](../VS_csharp/includes/vbprvb_md.md)], two strategies are especially useful:  
...
-   Classes should be generated in pairs, one inheriting from the other. The base class should contain all the generated methods and properties, and the derived class should contain only the constructors. This allows your hand-written code to override any of the generated methods.  
 ...
 To avoid repeating similar blocks of text and code in multiple templates, use the `<#@ include #>` directive. For more information, see [Include Directive](../VS_csharp/t4-include-directive.md).  
 ...
 If you have large code blocks and class feature blocks, it might be useful to move some of this code into methods that you compile in a separate project. You can use the `<#@ assembly #>` directive to access the code in the template. For more information, see [Assembly Directive](../VS_csharp/t4-assembly-directive.md).  
  
 You can put the methods in an abstract class that the template can inherit. The abstract class must inherit from <xref:Microsoft.VisualStudio.TextTemplating.TextTransformation*?displayProperty=fullName>. For more information, see [Template Directive](../VS_csharp/t4-template-directive.md).  
...
## Guidelines for Run-Time (Preprocessed) T4 Templates  
 ...
 You can use inheritance to share methods and text blocks between T4 text templates. For more information, see [Template Directive](../VS_csharp/t4-template-directive.md).  
...
## Guidelines for All T4 Templates  
 ...
 Use a different file name extension such as `.ttinclude` for include files. Use `.tt` only for files that you want to be processed either as run-time or design-time text templates. In some cases, [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] recognizes `.tt` files and automatically sets their properties for processing.  
...
```  
 ...
 UML class and activity diagrams can often be adapted for these purposes. You can also design your own type of diagram as a domain-specific language (DSL). Code can be generated from both UML and DSLs. For more information, see [UML](../VS_csharp/analyze-and-model-your-architecture.md) and [VMSDK](../VS_csharp/analyze-and-model-your-architecture.md).  
  
## See Also  
 [Code Generation in a Visual Studio Project](../VS_csharp/design-time-code-generation-by-using-t4-text-templates.md)   
 [Generating Text Files at Run Time](../VS_csharp/run-time-text-generation-with-t4-text-templates.md)