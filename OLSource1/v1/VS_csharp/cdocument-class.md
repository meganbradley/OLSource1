...
-   Override the [CObject::Serialize](../VS_csharp/cobject-class.md#cobject__serialize) member function in your document class to write and read the document's data to and from disk.  
  
 **CDocument** supports sending your document via mail if mail support (MAPI) is present. See the articles [MAPI](../VS_csharp/mapi.md) and [MAPI Support in MFC](../VS_csharp/mapi-support-in-mfc.md).  
  
 For more information on **CDocument**, see [Serialization](../VS_csharp/serialization-in-mfc.md), [Document/View Architecture Topics](../VS_csharp/document-view-architecture.md), and [Document/View Creation](../VS_csharp/document-view-creation.md).  
...
### Remarks  
 ...
 Call this function only if you are manually creating and attaching a view. Typically you will let the framework connect documents and views by defining a [CDocTemplate](../VS_csharp/cdoctemplate-class.md) object to associate a document class, view class, and frame window class.  
  
### Example  
 [!code[NVC_MFCDocViewSDI#12](../VS_csharp/codesnippet/CPP/cdocument-class_1.cpp)]  
...
### Example  
 [!code[NVC_MFCDocView#57](../VS_csharp/codesnippet/CPP/cdocument-class_2.cpp)]  
...
### Example  
 [!code[NVC_MFCDocView#58](../VS_csharp/codesnippet/CPP/cdocument-class_3.cpp)]  
...
### Parameters  
 ...
 `nOpenFlags`  
 Sharing and access mode. Specifies the action to take when opening the file. You can combine options listed in the CFile constructor [CFile::CFile](../VS_csharp/cfile-class.md#cfile__cfile) by using the bitwise OR (&#124;) operator. One access permission and one share option are required; the **modeCreate** and **modeNoInherit** modes are optional.  
...
### Example  
 [!code[NVC_MFCDocView#59](../VS_csharp/codesnippet/CPP/cdocument-class_4.cpp)]  
...
### Remarks  
 Document events may affect multiple classes. This method is responsible for handling document events that affect classes other than the [CDocument Class](../VS_csharp/cdocument-class.md). Currently, the only class that must respond to document events is the [CDataRecoveryHandler Class](../VS_csharp/cdatarecoveryhandler-class.md). The `CDocument` class has other overrideable methods responsible for handling the effect on the `CDocument`.  
...
### Remarks  
 ...
 A special implementation of `OnFileSendMail` for [COleDocument](../VS_csharp/coledocument-class.md) handles compound files correctly.  
  
 **CDocument** supports sending your document via mail if mail support (MAPI) is present. See the articles [MAPI Topics](../VS_csharp/mapi.md) and [MAPI Support in MFC](../VS_csharp/mapi-support-in-mfc.md).  
...
### Example  
 ...
 [!code[NVC_MFCDocView#60](../VS_csharp/codesnippet/CPP/cdocument-class_5.cpp)]  
  
 [!code[NVC_MFCDocView#61](../VS_csharp/codesnippet/CPP/cdocument-class_6.cpp)]  
  
 [!code[NVC_MFCDocView#62](../VS_csharp/codesnippet/CPP/cdocument-class_7.cpp)]  
...
### Example  
 ...
 [!code[NVC_MFCDocView#63](../VS_csharp/codesnippet/CPP/cdocument-class_8.cpp)]  
...
### Remarks  
 The default implementation of this function opens the specified file, calls [CObject::Serialize](../VS_csharp/cobject-class.md#cobject__serialize) to write the document's data to the file, and then marks the document as clean. Override this function if you want to perform special processing when the framework saves a document. For example, you might write an application where documents represent records in a database rather than separate files.  
...
### Parameters  
 `pCmdUI`  
 A pointer to the [CCmdUI](../VS_csharp/ccmdui-class.md) object associated with the **ID_FILE_SEND_MAIL** command.  
...
### Parameters  
 `pFrame`  
 Pointer to the [CFrameWnd](../VS_csharp/cframewnd-class.md) that holds the associated **CDocument** object.  
  
### Remarks  
 It can be overridden to provide custom cleanup, but be sure to call the base class as well.  
  
 The default of `PreCloseFrame` does nothing in **CDocument**. The **CDocument**-derived classes [COleDocument](../VS_csharp/coledocument-class.md) and [CRichEditDoc](../VS_csharp/cricheditdoc-class.md) use this member function.  
...
### Parameters  
 ...
 `bAbort`  
 Specifies whether the file is to be released by using either `CFile::Close` or `CFile::Abort`. **FALSE** if the file is to be released using [CFile::Close](../VS_csharp/cfile-class.md#cfile__close); **TRUE** if the file is to be released using [CFile::Abort](../VS_csharp/cfile-class.md#cfile__abort).  
...
### Remarks  
 ...
 Call this function only if you are manually detaching a view. Typically you will let the framework detach documents and views by defining a [CDocTemplate](../VS_csharp/cdoctemplate-class.md) object to associate a document class, view class, and frame window class.  
...
##  <a name="cdocument__reportsaveloadexception"></a>  CDocument::ReportSaveLoadException  
 Called if an exception is thrown (typically a [CFileException](../VS_csharp/cfileexception-class.md) or [CArchiveException](../VS_csharp/carchiveexception-class.md)) while saving or loading the document.  
...
### Remarks  
 ...
 This function calls the [CView::OnUpdate](../VS_csharp/cview-class.md#cview__onupdate) member function for each of the document's views except the sending view, passing `pHint` and `lHint`. Use these parameters to pass information to the views about the modifications made to the document. You can encode information using `lHint` and/or you can define a [CObject](../VS_csharp/cobject-class.md)-derived class to store information about the modifications and pass an object of that class using `pHint`. Override the `CView::OnUpdate` member function in your [CView](../VS_csharp/cview-class.md)-derived class to optimize the updating of the view's display based on the information passed.  
  
### Example  
 [!code[NVC_MFCDocView#64](../VS_csharp/codesnippet/CPP/cdocument-class_9.cpp)]  
  
## See Also  
 [MFC Sample MDIDOCVW](../VS_csharp/visual-c---samples.md)   
 [MFC Sample SNAPVW](../VS_csharp/visual-c---samples.md)   
 [MFC Sample NPP](../VS_csharp/visual-c---samples.md)   
