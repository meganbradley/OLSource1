...
## Remarks  
 The `_pclose` function looks up the process ID of the command processor (Cmd.exe) started by the associated `_popen` call, executes a [_cwait](../VS_csharp/_cwait.md) call on the new command processor, and closes the stream on the associated pipe.  
