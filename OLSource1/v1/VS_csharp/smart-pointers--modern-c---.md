...
## Uses for smart pointers  
 ...
 [!code[smart_pointers_intro#1](../VS_csharp/codesnippet/CPP/smart-pointers--modern-c---_1.cpp)]  
...
>  Always create smart pointers on a separate line of code, never in a parameter list, so that a subtle resource leak won't occur due to certain parameter list allocation rules.  
 ...
 [!code[smart_pointers_intro#2](../VS_csharp/codesnippet/CPP/smart-pointers--modern-c---_2.cpp)]  
...
5.  Let the smart pointer delete the object.  
 ...
 [!code[smart_pointers_intro#3](../VS_csharp/codesnippet/CPP/smart-pointers--modern-c---_3.cpp)]  
 ...
 [!code[smart_pointers_intro#4](../VS_csharp/codesnippet/CPP/smart-pointers--modern-c---_4.cpp)]  
...
-   `unique_ptr`   
     Allows exactly one owner of the underlying pointer. Use as the default choice for POCO unless you know for certain that you require a `shared_ptr`. Can be moved to a new owner, but not copied or shared. Replaces `auto_ptr`, which is deprecated. Compare to `boost::scoped_ptr`. `unique_ptr` is small and efficient; the size is one pointer and it supports rvalue references for fast insertion and retrieval from STL collections. Header file: `<memory>`. For more information, see [How to: Create and use unique_ptr instances](../VS_csharp/how-to--create-and-use-unique_ptr-instances.md) and [unique_ptr Class](../VS_csharp/unique_ptr-class.md).  
  
-   `shared_ptr`   
     Reference-counted smart pointer. Use when you want to assign one raw pointer to multiple owners, for example, when you return a copy of a pointer from a container but want to keep the original. The raw pointer is not deleted until all `shared_ptr` owners have gone out of scope or have otherwise given up ownership. The size is two pointers; one for the object and one for the shared control block that contains the reference count. Header file: `<memory>`. For more information, see [How to: Create and Use shared_ptr Instances](../VS_csharp/how-to--create-and-use-shared_ptr-instances.md) and [shared_ptr Class](../VS_csharp/shared_ptr-class.md).  
  
-   `weak_ptr`   
    Special-case smart pointer for use in conjunction with `shared_ptr`. A `weak_ptr` provides access to an object that is owned by one or more `shared_ptr` instances, but does not participate in reference counting. Use when you want to observe an object, but do not require it to remain alive. Required in some cases to break circular references between `shared_ptr` instances. Header file: `<memory>`. For more information, see [How to: Create and Use weak_ptr Instances](../VS_csharp/how-to--create-and-use-weak_ptr-instances.md) and [weak_ptr Class](../VS_csharp/weak_ptr-class.md).  
 ...
 **Smart Pointers for COM Objects (Classic Windows Programming)**  
 ...
 Use this unless you cannot use ATL. Performs reference counting by using the `AddRef` and `Release` methods. For more information, see [How to: Create and Use CComPtr and CComQIPtr Instances](../VS_csharp/how-to--create-and-use-ccomptr-and-ccomqiptr-instances.md).  
  
 [CComQIPtr Class](../VS_csharp/ccomqiptr-class.md)  
 Resembles `CComPtr` but also provides simplified syntax for calling `QueryInterface` on COM objects. For more information, see [How to: Create and Use CComPtr and CComQIPtr Instances](../VS_csharp/how-to--create-and-use-ccomptr-and-ccomqiptr-instances.md).  
  
 [CComHeapPtr Class](../VS_csharp/ccomheapptr-class.md)  
 Smart pointer to objects that use `CoTaskMemFree` to free memory.  
  
 [CComGITPtr Class](../VS_csharp/ccomgitptr-class.md)  
 ...
 **ATL Smart Pointers for POCO Objects**  
 ...
 [CAutoPtr Class](../VS_csharp/cautoptr-class.md)  
 ...
 Smart pointer for objects that are allocated by using the C [malloc](../VS_csharp/malloc.md) function.  
  
 [CAutoVectorPtr Class](../VS_csharp/cautovectorptr-class.md)  
 Smart pointer for arrays that are allocated by using `new[]`.  
  
 [CAutoPtrArray Class](../VS_csharp/cautoptrarray-class.md)  
