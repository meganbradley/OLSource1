...
##  <a name="BKMK_AfxDebugBreak"></a> AfxDebugBreak  
 MFC provides a special [AfxDebugBreak](../VS_csharp/afxdebugbreak--mfc-.md) function for hard-coding breakpoints in source code:  
...
##  <a name="BKMK_The_TRACE_macro"></a> The TRACE macro  
 To display messages from your program in the debugger [Output window](../VS_csharp/output-window.md), you can use the [ATLTRACE](../VS_csharp/atltrace--atl-.md) macro or the MFC [TRACE](../VS_csharp/trace.md) macro. Like [assertions](../VS_csharp/c-c---assertions.md), the trace macros are active only in the Debug version of your program and disappear when compiled in the Release version.  
...
```  
 ...
 For more information on the **TRACE** macro, see [Diagnostic Services](../VS_csharp/diagnostic-services.md).  
...
-   Call the global function [AfxEnableMemoryTracking](../VS_csharp/afxenablememorytracking.md) to enable or disable the diagnostic memory allocator. Because memory diagnostics are on by default in the debug library, you will typically use this function to temporarily turn them off, which increases program execution speed and reduces diagnostic output.  
  
 **To select specific memory diagnostic features with afxMemDF**  
  
-   If you want more precise control over the memory diagnostic features, you can selectively turn individual memory diagnostic features on and off by setting the value of the MFC global variable [afxMemDF](../VS_csharp/afxmemdf.md). This variable can have the following values as specified by the enumerated type **afxMemDF**.  
    ...
    |**checkAlwaysMemDF**|Call [AfxCheckMemory](../VS_csharp/afxcheckmemory.md) every time memory is allocated or freed.|  
...
3.  Create a third `CMemoryState` object and call its [CMemoryState::Difference](../Topic/CMemoryState::Difference.md) member function, supplying as arguments the two previous `CMemoryState` objects. If there is a difference between the two memory states, the `Difference` function returns a nonzero value. This indicates that some memory blocks have not been deallocated.  
    ...
    ```  
     ...
     Notice that the memory-checking statements are bracketed by `#ifdef`[_DEBUG](../VS_csharp/_debug.md)/ **#endif** blocks so that they are compiled only in Debug versions of your program.  
...
> [!NOTE]
>  Before you can use MFC object dumping, you must [enable diagnostic tracing](../VS_csharp/mfc-debugging-techniques.md#BKMK_Enabling_Memory_Diagnostics).  
...
```  
 ...
 The C run-time library has a similar function, [_CrtSetBreakAlloc](../VS_csharp/_crtsetbreakalloc.md), that you can use for C run-time allocations.  
...
####  <a name="BKMK_Customizing_object_dumps"></a> Customizing object dumps  
 When you derive a class from [CObject](../VS_csharp/cobject-class.md), you can override the `Dump` member function to provide additional information when you use [DumpAllObjectsSince](../Topic/CMemoryState::DumpAllObjectsSince.md) to dump objects to the [Output window](../VS_csharp/output-window.md).  
  
 The `Dump` function writes a textual representation of the object's member variables to a dump context ([CDumpContext](../VS_csharp/cdumpcontext-class.md)). The dump context is similar to an I/O stream. You can use the append operator (**<<**) to send data to a `CDumpContext`.  
...
1.  Rebuild the MFC libraries using the [/Z7, /Zi, /ZI (Debug Information Format)](../VS_csharp/-z7---zi---zi--debug-information-format-.md) option, instead of **/Z7**. These options build a single program database (PDB) file that contains debug information for the entire library, reducing redundancy and saving space.  
...
7.  From the **Build** menu, select **Build** to rebuild project files that are out of date.  
 ...
 As an alternative to the technique described in this topic, you can use an external makefile to define individual options for each file. In that case, to link with the MFC debug libraries, you must define the [_DEBUG](../VS_csharp/_debug.md) flag for each module. If you want to use MFC release libraries, you must define NDEBUG. For more information on writing external makefiles, see the [NMAKE Reference](../VS_csharp/running-nmake.md).  
  
 [In this topic](#BKMK_In_this_topic)  
  
## See Also  
 [Debugging Visual C++](../VS_csharp/debugging-native-code.md)