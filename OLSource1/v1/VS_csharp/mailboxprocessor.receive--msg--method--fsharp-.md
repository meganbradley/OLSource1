...
## Return Value  
 An asynchronous computation ([Async](../VS_csharp/control.async-class--fsharp-.md) object) that returns the received message.  
  
## Remarks  
 This method is for use within the body of the agent. For each agent, at most one concurrent reader may be active, so no more than one concurrent call to `Receive`, [TryReceive](../VS_csharp/mailboxprocessor.tryreceive--msg--method--fsharp-.md), [Scan](../VS_csharp/mailboxprocessor.scan--msg--t--method--fsharp-.md) or [TryScan](../VS_csharp/mailboxprocessor.tryscan--msg--t--method--fsharp-.md) may be active.  
  
## Example  
 The following example shows how to use the `Receive` method. In this case, a timeout of 10 seconds is specified. In general, the message processing function runs on a different thread from the [Post](../VS_csharp/mailboxprocessor.post--msg--method--fsharp-.md) function, so you must catch the timeout exception in the message processor function. In this example, the timeout exception just causes the loop to continue, and increases the message number by 1.  
  
 [!code[FsMailboxProcessor#10](../VS_csharp/codesnippet/FSharp/mailboxprocessor.receive--msg--method--fsharp-_1.fs)]  
