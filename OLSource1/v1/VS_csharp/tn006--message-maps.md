...
## Defining a Message Map  
 The [DECLARE_MESSAGE_MAP](../VS_csharp/declare_message_map.md) macro declares three members for a class.  
...
```  
 ...
 The message map's table is defined by using a set of macros that expand to message map entries. A table starts with a [BEGIN_MESSAGE_MAP](../VS_csharp/begin_message_map.md) macro call, which defines the class that is handled by this message map and the parent class to which unhandled messages are passed. The table ends with the [END_MESSAGE_MAP](../VS_csharp/end_message_map.md) macro call.  
 ...
 A standard function signature has been defined for unpacking the parameters of each Windows message and providing type safety. These signatures may be found in the file Afxwin.h in the declaration of [CWnd](../VS_csharp/cwnd-class.md). Each one is marked with the keyword `afx_msg` for easy identification.  
...
## User Defined Windows Messages  
 User-defined messages may be included in a message map by using the [ON_MESSAGE](../VS_csharp/on_message.md) macro. This macro accepts a message number and a method of the form:  
...
```  
 ...
 Command update messages are routed through the same mechanism, but use the `ON_UPDATE_COMMAND_UI` macro instead. Command update handler member functions take a single parameter, a pointer to a [CCmdUI](../VS_csharp/ccmdui-class.md) object, and return `void`. The macro has the form  
...
-   In the implementation file  
 ...
 Advanced users can handle a range of commands by using a single command handler: [ON_COMMAND_RANGE](../VS_csharp/on_command_range.md) or `ON_COMMAND_RANGE_EX`. See the product documentation for more information about these macros.  
...
-   The control ID (`wParam`) matches the control ID specified in the message-map entry.  
 ...
 Custom control notification messages may use the [ON_CONTROL](../VS_csharp/on_control.md) macro to define a message map entry with a custom notification code. This macro has the form  
  
```  
ON_CONTROL(wNotificationCode, id, memberFxn)  
```  
  
 For advanced usage [ON_CONTROL_RANGE](../VS_csharp/on_control_range.md) can be used to handle a specific control notification from a range of controls with the same handler.  
