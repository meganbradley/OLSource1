...
-   `Value` is of the same type as the underlying type. If `HasValue` is `True`, `Value` contains a meaningful value. If `HasValue` is `False`, accessing `Value` will throw an invalid operation exception.  
 ...
 For more information about nullable types in Visual C#, see [Nullable Types (C# Programming Guide)](../VS_csharp/nullable-types--csharp-programming-guide-.md). For more information about nullable types in Visual Basic, see [Value Types that Might Not Have a Defined Value](../VS_csharp/nullable-value-types--visual-basic-.md).  
