...
You can embed a DSL diagram in a Windows Control, which appears in the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] window.  
...
1.  In the form designer, add a **ListBox** named `listBox1`. This will display a list of the elements in the model. It will be kept in synchronism with the model using *store events*. For more information, see [Event Handlers Propagate Changes Outside the Model](../VS_csharp/event-handlers-propagate-changes-outside-the-model.md).  
...
4.  To test the DSL, press F5 and in the experimental instance of [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)], open a sample model file.  
