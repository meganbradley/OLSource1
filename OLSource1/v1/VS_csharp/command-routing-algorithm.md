...
In Visual Studio commands are handled by a number of different components. Commands are routed from the innermost context, which is based on the current selection, to the outermost (also known as global) context. For more information, see [Command Availability](../VS_csharp/command-availability.md).  
...
## See Also  
 [Command Implementation](../VS_csharp/command-design.md)