...
-   Indirectly construct it through a document template.  
 ...
 The `CMDIChildWnd` class inherits much of its default implementation from `CFrameWnd`. For a detailed list of these features, please refer to the [CFrameWnd](../VS_csharp/cframewnd-class.md) class description. The `CMDIChildWnd` class has the following additional features:  
...
-   The currently active MDI child window menu entirely replaces the MDI frame window's menu, and the caption of the currently active MDI child window is added to the MDI frame window's caption. For further examples of MDI child window functions that are implemented in conjunction with an MDI frame window, see the `CMDIFrameWnd` class description.  
 ...
 For more information on `CMDIChildWnd`, see [Frame Windows](../VS_csharp/frame-windows.md).  
...
### Parameters  
 `lpszClassName`  
 Points to a null-terminated character string that names the Windows class (a                                 [WNDCLASS](http://msdn.microsoft.com/library/windows/desktop/ms633576) structure). The class name can be any name registered with the [AfxRegisterWndClass](../VS_csharp/afxregisterwndclass.md) global function. Should be **NULL** for a standard `CMDIChildWnd`.  
  
 `lpszWindowName`  
 Points to a null-terminated character string that represents the window name. Used as text for the title bar.  
  
 `dwStyle`  
 Specifies the window [style](../VS_csharp/window-styles.md) attributes. The **WS_CHILD** style is required.  
 ...
 `pContext`  
 Specifies a [CCreateContext](../VS_csharp/ccreatecontext-structure.md) structure. This parameter can be **NULL**.  
...
### Example  
 ...
 [!code[NVC_MFCWindowing#7](../VS_csharp/codesnippet/CPP/cmdichildwnd-class_1.cpp)]  
  
### Example  
 Example 2:  
  
 [!code[NVC_MFCWindowing#8](../VS_csharp/codesnippet/CPP/cmdichildwnd-class_2.cpp)]  
  
 [!code[NVC_MFCWindowing#9](../VS_csharp/codesnippet/CPP/cmdichildwnd-class_3.cpp)]  
...
### Remarks  
 The frame returned is two parents removed from the `CMDIChildWnd` and is the parent of the window of type **MDICLIENT** that manages the `CMDIChildWnd` object. Call the [GetParent](../VS_csharp/cwnd-class.md#cwnd__getparent) member function to return the `CMDIChildWnd` object's immediate **MDICLIENT** parent as a temporary `CWnd` pointer.  
  
### Example  
  See the example for [CMDIFrameWnd::MDISetMenu](../VS_csharp/cmdiframewnd-class.md#cmdiframewnd__mdisetmenu).  
...
### Example  
  See the example for [CMDIFrameWnd::GetWindowMenuPopup](../VS_csharp/cmdiframewnd-class.md#cmdiframewnd__getwindowmenupopup).  
...
### Example  
 [!code[NVC_MFCWindowing#10](../VS_csharp/codesnippet/CPP/cmdichildwnd-class_4.cpp)]  
...
### Example  
 [!code[NVC_MFCWindowing#11](../VS_csharp/codesnippet/CPP/cmdichildwnd-class_5.cpp)]  
...
### Example  
 [!code[NVC_MFCWindowing#12](../VS_csharp/codesnippet/CPP/cmdichildwnd-class_6.cpp)]  
