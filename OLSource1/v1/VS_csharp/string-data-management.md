...
-   [String Manipulation](../VS_csharp/string-manipulation--crt-.md) for working with C-style null-terminated strings  
  
-   Win32 API functions for managing strings  
  
-   MFC's class [CString](../VS_csharp/cstringt-class.md), which provides flexible, resizable string objects  
  
-   Class [CStringT Class](../VS_csharp/cstringt-class.md), which provides an MFC-independent string object with the same functionality as `CString`  
...
>  which the compiler treats as a Unicode string. For example, the following code:  
 ...
 [!code[NVC_ATLMFC_Utilities#187](../VS_csharp/codesnippet/CPP/string-data-management_1.cpp)]  
  
> [!NOTE]
>  is translated as a Unicode string if **_UNICODE** is defined or as an ANSI string if not. For more information, see the article [Unicode and Multibyte Character Set (MBCS) Support](../VS_csharp/unicode-and-multibyte-character-set--mbcs--support.md).  
