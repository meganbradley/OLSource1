...
##  <a name="BKMK_Trace_and_Debug_Requirements"></a> Trace and Debug Requirements  
 If you create your project using the [!INCLUDE[vsprvs](../VS_csharp/includes/vsprvs_md.md)] wizards, the TRACE symbol is defined by default in both Release and Debug configurations. The DEBUG symbol is defined by default only in the Debug build.  
...
-   `/d:TRACE` in Visual C# and C++  
 ...
 C++ does not support the <xref:System.Diagnostics.Debug*> class methods. You can achieve the same effect by using the <xref:System.Diagnostics.Trace*> class with conditional compilation, such as `#ifdef DEBUG`... `#endif`. You can define these symbols in the **<Project\> Property Pages** dialog box. For more information, see [Changing Project Settings for a Visual Basic Debug Configuration](../VS_csharp/project-settings-for-a-visual-basic-debug-configuration.md) or [Changing Project Settings for a C or C++ Debug Configuration](../VS_csharp/project-settings-for-a-c---debug-configuration.md).  
