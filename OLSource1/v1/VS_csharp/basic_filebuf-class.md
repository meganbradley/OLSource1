...
|[uflow](../VS_csharp/basic_streambuf-class.md#basic_streambuf__uflow)|Protected, virtual function to extract the current element from the input stream.|  
...
### Remarks  
 The member function opens the file with filename                         *filename*, by calling [fopen](../VS_csharp/fopen--_wfopen.md)(                        *filename*, **strmode**). **strmode** is determined from **mode &**~( [ate](../VS_csharp/ios_base-class.md#ios_base__openmode) & &#124; [binary](../VS_csharp/ios_base-class.md#ios_base__openmode)):  
  
-   **ios_base::in** becomes **"r"** (open existing file for reading).  
  
-   [ios_base::out](../VS_csharp/ios_base-class.md#ios_base__fmtflags) or **ios_base::out &#124; ios_base::trunc** becomes **"w"** (truncate existing file or create for writing).  
...
-   **ios_base::in &#124; ios_base::out &#124; ios_base::app** becomes **"a+"** (open existing file for reading and for appending all writes).  
 ...
 If the file pointer is not a null pointer, the function determines the file conversion facet: `use_facet`< `codecvt`< **Elem**, `char`, **traits_type::**[state_type](../VS_csharp/char_traits-struct.md#char_traits__state_type)> >( [getloc](../VS_csharp/basic_streambuf-class.md#basic_streambuf__getloc)), for use by [underflow](#basic_filebuf__underflow) and [overflow](#basic_filebuf__overflow).  
...
### Parameters  
 `_Right`  
 An rvalue reference to a [basic_filebuf](../VS_csharp/basic_filebuf-class.md) object.  
  
### Return Value  
 Returns *this.  
  
### Remarks  
 The member operator replaces the contents of the object by using the contents of `_Right`, treated as an rvalue reference. For more information, see [Rvalue Reference Declarator: &&](../VS_csharp/rvalue-reference-declarator----.md).  
...
### Return Value  
 If the function cannot succeed, it returns **traits_type::eof**. Otherwise, it returns **traits_type::**[not_eof](../VS_csharp/char_traits-struct.md#char_traits__not_eof)(_                        *Meta*).  
  
### Remarks  
 If _                        *Meta***!= traits_type::**[eof](../VS_csharp/char_traits-struct.md#char_traits__eof), the protected virtual member function endeavors to insert the element **ch = traits_type::**[to_char_type](../VS_csharp/char_traits-struct.md#char_traits__to_char_type)(\_                        *Meta*) into the output buffer. It can do so in various ways:  
...
### Remarks  
 The protected virtual member function endeavors to alter the current positions for the controlled streams. For an object of class [basic_filebuf](../VS_csharp/basic_filebuf-class.md)< `Elem`, `Tr`>, a stream position can be represented by an object of type `fpos_t`, which stores an offset and any state information needed to parse a wide stream. Offset zero designates the first element of the stream. (An object of type [pos_type](../VS_csharp/basic_streambuf-class.md#basic_streambuf__pos_type) stores at least an `fpos_t` object.)  
  
 For a file opened for both reading and writing, both the input and output streams are positioned in tandem. To switch between inserting and extracting, you must call either [pubseekoff](../VS_csharp/basic_streambuf-class.md#basic_streambuf__pubseekoff) or [pubseekpos](../VS_csharp/basic_streambuf-class.md#basic_streambuf__pubseekpos). Calls to `pubseekoff` (and hence to `seekoff`) have various limitations for [text streams](../VS_csharp/text-and-binary-streams.md), [binary streams](../VS_csharp/text-and-binary-streams.md), and [wide streams](../VS_csharp/byte-and-wide-streams.md).  
...
### Return Value  
 If the function cannot succeed, it returns **traits_type::**[eof](../VS_csharp/char_traits-struct.md#char_traits__eof). Otherwise, it returns **ch**, converted as described in the Remarks section.  
  
### Remarks  
 The protected virtual member function endeavors to extract the current element **ch** from the input stream, and return the element as **traits_type::**[to_int_type](../VS_csharp/char_traits-struct.md#char_traits__to_int_type)( **ch**). It can do so in various ways:  
