...
-   Passing run-time arguments to a predefined query.  
     ...
     To pass parameters to a stored procedure, you must specify a complete custom ODBC **CALL** statement — with parameter placeholders — when you call **Open**, overriding the recordset's default SQL statement. For more information, see [CRecordset::Open](../Topic/CRecordset::Open.md) in the *Class Library Reference* and [SQL: Customizing Your Recordset's SQL Statement (ODBC)](../VS_csharp/sql--customizing-your-recordset’s-sql-statement--odbc-.md) and [Recordset: Declaring a Class for a Predefined Query (ODBC)](../VS_csharp/recordset--declaring-a-class-for-a-predefined-query--odbc-.md).  
...
-   Efficiently performing numerous requeries with different parameter information.  
    ...
    ```  
     ...
     For a discussion of how to use quotes correctly for filter strings, see [Recordset: Filtering Records (ODBC)](../VS_csharp/recordset--filtering-records--odbc-.md).  
 ...
 For more information about filters, see [Recordset: Filtering Records (ODBC)](../VS_csharp/recordset--filtering-records--odbc-.md).  
...
1.  Run the [MFC ODBC Consumer Wizard](../VS_csharp/adding-an-mfc-odbc-consumer.md) from **Add Class** to create the class.  
...
5.  In the constructor of your recordset class, increment the count of parameters, `m_nParams`.  
     ...
     For information, see [Record Field Exchange: Working with the Wizard Code](../VS_csharp/record-field-exchange--working-with-the-wizard-code.md).  
