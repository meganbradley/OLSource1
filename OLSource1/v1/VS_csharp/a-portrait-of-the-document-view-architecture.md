...
## Gaining Access to Document Data from the View  
 ...
 For example, from the view's [OnDraw](../Topic/CView::OnDraw.md) member function, the view uses **GetDocument** to obtain a document pointer. Then it uses that pointer to access a `CString` data member in the document. The view passes the string to the `TextOut` function. To see the code for this example, see [Drawing in a View](../VS_csharp/drawing-in-a-view.md).  
