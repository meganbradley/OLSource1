...
-   To display what your custom build step is actually doing, add <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> as the first command. The build events and build steps are put in a temporary .bat file and run when the project is built. Therefore, you can add error checking to your build event or build step commands.  
...
-   Verify the values of any file name or directory macros you are using. You can echo macros individually, or you can add <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> to the start of your custom build step, which will copy your custom build step's commands to command.bat with all macros expanded.  
