---
title: "Badly formed XML. File &#39;&lt;commentfile&gt;&#39; cannot be included"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "vbc42321"
  - "bc42321"
helpviewer_keywords: 
  - "BC42321"
ms.assetid: 96611980-2323-466c-82c3-8bb678290b68
caps.latest.revision: 10
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# Badly formed XML. File &#39;&lt;commentfile&gt;&#39; cannot be included
An <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> tag in one of the XML comments refers to a file that does not exist or a file that contains badly formed XML.  
  
 **Error ID:** BC42321  
  
### To correct this error  
  
1.  Verify that the file exists.  
  
2.  Verify that the XML in the file is valid.  
  
## See Also  
 [\<include> (Visual Basic)](../vs140/-include---visual-basic-.md)