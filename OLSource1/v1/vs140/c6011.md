...
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 You must allocate memory inside the function whose parameters are annotated by using the Null property in a Pre condition before dereferencing the parameter. The following code generates warning C6011 because an attempt is made to dereference a null pointer (<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>) inside the function without first allocating memory:  
