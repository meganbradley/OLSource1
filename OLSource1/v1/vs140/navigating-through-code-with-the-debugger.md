...
-   On a nested function call, **Step Into** steps into the most deeply nested function. If you use **Step Into** on a call like <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, the debugger steps into the function <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>.  
  
-   The debugger actually steps through code statements rather than physical lines. For example an <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> clause can be written on one line:  
...
## Step into non-user code  
 By default, the debugger tries to show you only your app code while debugging, which is determined by a debugger setting called *Just My Code*. (See [Just My Code](../vs140/just-my-code.md) to see how this works for different project types and languages and how you might customize the behavior.) However, sometimes while you are debugging, you might want to look at framework code, third-party library code, or calls to the operating system (system calls).  
