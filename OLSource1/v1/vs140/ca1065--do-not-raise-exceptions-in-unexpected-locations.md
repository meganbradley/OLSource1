...
-   [M:IEquatable.Equals](http://go.microsoft.com/fwlink/?LinkId=113472)  
 ...
 An **Equals** method should return <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> instead of throwing an exception. For example, if Equals is passed two mismatched types it should just return <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> instead of throwing an \<xref:System.ArgumentException*>.  
...
### Dispose Methods  
 A \<xref:System.IDisposable.Dispose*?displayProperty=fullName> method should not throw an exception. Dispose is often called as part of the clean up logic in a <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> clause. Therefore, explicitly throwing an exception from Dispose forces the user to add exception handling inside the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> clause.  
  
 The **Dispose(false)** code path should never throw exceptions, because this is almost always called from a finalizer.  
  
### Equality Operators (==, !=)  
 Like Equals methods, equality operators should return either <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> and should not throw exceptions.  
...
## Related Rules  
 [Do not raise exceptions in exception clauses](../vs140/ca2219--do-not-raise-exceptions-in-exception-clauses.md)  
