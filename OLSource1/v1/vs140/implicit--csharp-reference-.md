...
The <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> keyword is used to declare an implicit user-defined type conversion operator. Use it to enable implicit conversions between a user-defined type and another type, if the conversion is guaranteed not to cause a loss of data.  
  
## Example  
 [!code[csrefKeywordsConversion#5](../vs140/codesnippet/CSharp/implicit--csharp-reference-_1.cs)]  
  
 By eliminating unnecessary casts, implicit conversions can improve source code readability. However, because implicit conversions do not require programmers to explicitly cast from one type to the other, care must be taken to prevent unexpected results. In general, implicit conversion operators should never throw exceptions and never lose information so that they can be used safely without the programmer's awareness. If a conversion operator cannot meet those criteria, it should be marked <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>. For more information, see [Using Conversion Operators](../vs140/using-conversion-operators--csharp-programming-guide-.md).  
