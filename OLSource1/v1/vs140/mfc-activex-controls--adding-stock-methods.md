...
A stock method differs from a custom method in that it is already implemented by class [COleControl](../vs140/colecontrol-class.md). For example, <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> contains a predefined member function that supports the Refresh method for your control. The dispatch map entry for this stock method is **DISP_STOCKFUNC_REFRESH**.  
  
 <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> supports two stock methods: DoClick and Refresh. Refresh is invoked by the control's user to immediately update the control's appearance; DoClick is invoked to fire the control's Click event.  
  
|Method|Dispatch map entry|Comment|  
|------------|------------------------|-------------|  
|<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>|**DISP_STOCKPROP_DOCLICK( )**|Fires a Click event.|  
...
##  \<a name="_core_classwizard_changes_for_stock_methods">\</a> Add Method Wizard Changes for Stock Methods  
 ...
 [!code[NVC_MFC_AxUI#16](../vs140/codesnippet/CPP/mfc-activex-controls--adding-stock-methods_1.cpp)]  
  
 This makes the Refresh method available to the control's users.  
  
 The following line is added to the control's .IDL file:  
  
 [!code[NVC_MFC_AxUI#17](../vs140/codesnippet/CPP/mfc-activex-controls--adding-stock-methods_2.idl)]  
