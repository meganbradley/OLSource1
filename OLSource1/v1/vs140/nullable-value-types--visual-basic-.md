...
### Retrieving Values  
 To retrieve the value of a variable of a nullable type, you should first test its <xref:System.Nullable`1.HasValue*> property to confirm that it has a value. If you try to read the value when <xref:System.Nullable`1.HasValue*> is `False`, [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] throws an <xref:System.InvalidOperationException*> exception. The following example shows the recommended way to read the variable `numberOfChildren` of the previous examples.  
