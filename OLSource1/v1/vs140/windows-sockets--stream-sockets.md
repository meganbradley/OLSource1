...
This article describes stream sockets, one of the two Windows Socket types available. (The other type is the [datagram socket](../vs140/windows-sockets--datagram-sockets.md).)  
 ...
 The network transport layer may break up or group data into packets of reasonable size. The <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> class will handle the packing and unpacking for you.  
...
-   The final model takes advantage of the ability to communicate between Unicode and ANSI socket applications that class CArchive lends to class CSocket.  
    ...
    > [!NOTE]
    >  If you use class <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, you must use a stream. An MFC assertion fails if you specify the socket type as **SOCK_DGRAM**.  
