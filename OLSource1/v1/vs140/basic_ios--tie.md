...
## Return Value  
 The first member function returns the stored tie pointer. The second member function stores <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> in the tie pointer and returns its previous stored value.  
  
## Remarks  
 <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> causes two streams to be synchronized, such that, operations on one stream occur after operations on the other stream are complete.  
