...
You can add precision and flexibility to your async applications by using the methods and properties that the <xref:System.Threading.Tasks.Task?qualifyHint=False> type makes available. The topics in this section show examples that use <xref:System.Threading.CancellationToken?qualifyHint=False> and important `Task` methods such as <xref:System.Threading.Tasks.Task.WhenAll?qualifyHint=True> and <xref:System.Threading.Tasks.Task.WhenAny?qualifyHint=True>.  
