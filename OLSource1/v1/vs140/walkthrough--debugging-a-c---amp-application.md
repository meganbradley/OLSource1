...
4.  Choose **Win32 Console Application**, type <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> in the **Name** box, and then choose the **OK** button.  
...
6.  In the **Locals** window, observe the value for <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> until the breakpoint at line 70 is reached.  
  
7.  On the menu bar, choose **Debug**, **Stop Debugging**.  
  
## Debugging the GPU Code  
 This section shows how to debug the GPU code, which is the code contained in the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> function. The GPU code computes the sum of integers for each "block" in parallel.  
...
Parallel Stacks window  
     ...
     32 threads went from <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> to the lambda statement in the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> function call and then to the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> function, where the parallel reduction occurs. 28 out of the 32 threads have progressed to the [tile_barrier::wait](../vs140/tile_barrier--wait-method.md) statement and remain blocked at line 22, whereas the other 4 threads remain active in the <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> function at line 30.  
...
GPU thread DataTip  
     ...
     For more information about the Parallel Stacks window, see [Using the Parallel Stacks Window](../vs140/using-the-parallel-stacks-window.md).  
...
3.  Select the **Add Watch** column header, enter <CodeContentPlaceHolder>12\</CodeContentPlaceHolder>, and then choose the Enter key.  
  
4.  Select the **Add Watch** column header again, type <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>, and then choose the Enter key.  
  
5.  Select the **Add Watch** column header again, type <CodeContentPlaceHolder>14\</CodeContentPlaceHolder>, and then choose the Enter key.  
...
6.  In the upper-right corner of the Parallel Watch window, there's a filter control that you can use to filter the content by using Boolean expressions. Enter <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> in the filter control text box and then choose the Enter key.  
  
     The window now contains only threads on which the <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> value is greater than 20000. The content is still sorted by the <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> column, which is the sorting action you performed earlier.  
