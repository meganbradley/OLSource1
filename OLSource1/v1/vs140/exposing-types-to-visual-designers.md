...
-   <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> must be set to 1 in the registry.  
...
-   The generated code must be in the same language as the global project setting.  
     ...
     The temporary PE is compiled regardless of what the custom tool reports as the requested extension in \<xref:Microsoft.VisualStudio.Shell.Interop.IVsSingleFileGenerator.DefaultExtension*> provided that <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is set to 1 in the registry. The extension does not need to be .vb, .cs, or .jsl; it can be any extension.  
