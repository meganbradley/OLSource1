...
A query is basically a set of instructions for how to retrieve and organize data. To execute the query requires a call to its <xref:System.Collections.Generic.IEnumerable`1.GetEnumerator?qualifyHint=False> method. This call is made when you use a `foreach` loop to iterate over the elements. To evaluate a query and store its results without executing a `foreach` loop, just call one of the following methods on the query variable:  
  
-   <xref:System.Linq.Enumerable.ToList``1?qualifyHint=False>  
  
-   <xref:System.Linq.Enumerable.ToArray``1?qualifyHint=False>  
  
-   <xref:System.Linq.Enumerable.ToDictionary``3?qualifyHint=False>  
  
-   <xref:System.Linq.Enumerable.ToLookup``3?qualifyHint=False>  
