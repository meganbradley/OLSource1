...
## UI Constraints on Source Control VSPackages  
 ...
 Every source control VSPackage must implement the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> interface. Two methods on the interface, \<xref:Microsoft.VisualStudio.Shell.Interop.IVsSccProvider.SetActive*> and \<xref:Microsoft.VisualStudio.Shell.Interop.IVsSccProvider.SetInactive*>, must be implemented by the VSPackage.  
