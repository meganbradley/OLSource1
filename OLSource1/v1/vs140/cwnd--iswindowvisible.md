...
## Return Value  
 Nonzero if <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is visible (has the [WS_VISIBLE](../vs140/window-styles.md) style bit set, and parent window is visible). Because the return value reflects the state of the **WS_VISIBLE** style bit, the return value may be nonzero even though <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> is totally obscured by other windows.  
