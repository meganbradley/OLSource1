...
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 Function pointers are also strictly type-safe in C++, so the following C code requires modification. In C++ it's best to create a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> that defines the function pointer type, and then use that type to cast function pointers:  
  
<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>  
 C++ also requires that functions either be prototyped or fully defined before they can be referenced or invoked.  
  
 Identifiers used in C code that happen to be keywords in C++ (such as <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>, etc.) must be renamed. This can generally be done with simple search-and-replace operations.  
  
 Finally, whereas C-style COM calls require explicit use of the v-table and <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> pointer, C++ does not:  
...
### Type Visibility  
 Native types are now private by default. In Visual C++ .NET 2002 and Visual C++ 2003, native types were public by default. This can result in a native type not being visible outside the DLL. Resolve this error by adding <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> to these types. See [Type and Member Visibility](../vs140/type-and-member-visibility.md) for more information.  
  
### Floating Point and Alignment Issues  
 <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> is not supported on the common language runtime (see [_control87, _controlfp, \__control87_2](../vs140/_control87--_controlfp--__control87_2.md) for more information). The CLR will also not respect [align (C++)](../vs140/align--c---.md).  
...
### COM Initialization  
 ...
 The common language runtime starts COM as MTA by default; use [/CLRTHREADATTRIBUTE (Set CLR Thread Attribute)](../vs140/-clrthreadattribute--set-clr-thread-attribute-.md) to modify this.  
...
### Program Crashes on Shutdown  
 In some cases, the CLR can shutdown before your managed code is finished running. Using <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> can cause this. See [signal Constants](../vs140/signal-constants.md) and [set_terminate (\<exception>)](../vs140/set_terminate---exception--.md) for more information.  
