...
## Return Value  
 A pointer to the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> object used to store the conversion formats.  
  
## Remarks  
 If you want your server application to offer data in a variety of formats during data transfer operations, register those formats with the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> object returned by this function. For example, if you want to supply a **CF_TEXT** representation of the OLE item for Clipboard or drag-and-drop operations, you would register the format with the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> object this function returns, and then override the **OnRenderXxxData** member function to provide the data.  
