...
>  If your ODBC driver supports **SQLSetPos**, which MFC uses if available, this topic does not apply to you.  
 ...
 The second issue concerns limitations of class [CTime](../vs140/ctime-class.md) when used with the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> function to transfer time and date information to or from a table. Processing the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> object imposes some overhead in the form of extra intermediate processing during the data transfer. The date range of <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> objects might also be too limiting for some applications. A new version of the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> function takes an ODBC **TIMESTAMP_STRUCT** parameter instead of a <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> object. For more information, see <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> in [Macros and Globals](../vs140/macros--global-functions--and-global-variables.md) in the *MFC Reference*.  
  
##  \<a name="_core_using_the_cursor_library">\</a> Using the Cursor Library  
 When you connect to a data source — by calling [CDatabase::OpenEx](../vs140/cdatabase--openex.md) or [CDatabase::Open](../vs140/cdatabase--open.md) — you can specify whether to use the cursor library for the data source. If you will be creating snapshots on that data source, specify the **CDatabase::useCursorLib** option in the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> parameter to <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> or specify **TRUE** for the **bUseCursorLib** parameter to **Open** (the default value is **TRUE**). If your ODBC driver supports dynasets and you want to open dynasets on the data source, do not use the cursor library (it masks some driver functionality needed for dynasets). In that case, do not specify **CDatabase::useCursorLib** in <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> or specify **FALSE** for the **bUseCursorLib** parameter in **Open**.  
