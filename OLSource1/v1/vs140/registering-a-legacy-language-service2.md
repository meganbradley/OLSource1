...
|Page|REG_SZ|*GUID*|The GUID of the property page to request from the VSPackage by calling the <xref:Microsoft.VisualStudio.Shell.Interop.IVsPackage.GetPropertyPage?qualifyHint=False> method. If this registry entry is not present, the registry key describes a node, not a page.|  
...
## Registry Entries for Managed Package Framework Language Service Options  
 ...
 These registry entries are accessed through the <xref:Microsoft.VisualStudio.Package.LanguagePreferences?qualifyHint=False> class.  
