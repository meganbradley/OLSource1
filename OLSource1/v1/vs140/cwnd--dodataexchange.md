...
## Remarks  
 Never call this function directly. It is called by the [UpdateData](../vs140/cwnd--updatedata.md) member function. Call <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> to initialize a dialog box's controls or retrieve data from a dialog box.  
 ...
 To automatically generate an overridden version of this member function, first create a dialog resource with the dialog editor, then derive an application-specific dialog class. Then use the Add Variable wizard to associate variables, data, and validation ranges with various controls in the new dialog box. The wizard then writes the overridden <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>, which contains a data map. The following is an example DDX/DDV code block generated by the Add Variable wizard:  
  
 [!code[NVC_MFCWindowing#90](../vs140/codesnippet/CPP/cwnd--dodataexchange_1.cpp)]  
  
 The <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> overridden member function must precede the macro statements in your source file.  
