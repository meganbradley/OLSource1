...
'operator' : integral constant overflow  
 ...
 The operator is used in an expression that results in an integer constant overflowing the space allocated for it. You may need to use a larger type for the constant. A **signed int** holds a smaller value than an <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> because the **signed int** uses one bit to represent the sign.  
