...
[!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] unifies the syntax for structures and classes, with the result that both entities support most of the same features. However, there are also important differences between structures and classes.  
...
-   Members of both can have individualized access levels. For example, one member can be declared <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> and another <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
...
-   All structure elements are <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> by default; class variables and constants are <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> by default, while other class members are <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> by default. This behavior for class members provides compatibility with the Visual Basic 6.0 system of defaults.  
...
-   Structure elements cannot be declared as <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>; class members can.  
  
-   A structure procedure can handle events only if it is a [Shared (Visual Basic)](../vs140/shared--visual-basic-.md)<CodeContentPlaceHolder>6\</CodeContentPlaceHolder> procedure, and only by means of the [AddHandler Statement](../vs140/addhandler-statement.md); any class procedure can handle events, using either the [Handles](../vs140/handles-clause--visual-basic-.md) keyword or the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> statement. For more information, see [Events (Visual Basic)](../vs140/events--visual-basic-.md).  
...
-   **Initialization.** A structure variable implicitly includes an initialization of the elements using the structure's parameterless constructor. Therefore, <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> is equivalent to <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>.  
...
-   **Assigning Nothing.** You can assign the value [Nothing (Visual Basic)](../vs140/nothing--visual-basic-.md) to a structure variable, but the instance continues to be associated with the variable. You can still call its methods and access its data elements, although variable elements are reinitialized by the assignment.  
  
     In contrast, if you set an object variable to <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>, you dissociate it from any class instance, and you cannot access any members through the variable until you assign another instance to it.  
...
-   **Multiple Instances.** An object variable can have different class instances assigned to it at different times, and several object variables can refer to the same class instance at the same time. Changes you make to the values of class members affect those members when accessed through another variable pointing to the same instance.  
     ...
     Structure elements, however, are isolated within their own instance. Changes to their values are not reflected in any other structure variables, even in other instances of the same <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> declaration.  
