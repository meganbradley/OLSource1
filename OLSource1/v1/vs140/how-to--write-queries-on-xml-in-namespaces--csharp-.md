...
To write a query on XML that is in a namespace, you must use <xref:System.Xml.Linq.XName?qualifyHint=False> objects that have the correct namespace.  
 ...
 For C#, the most common approach is to initialize an <xref:System.Xml.Linq.XNamespace?qualifyHint=False> using a string that contains the URI, then use the addition operator overload to combine the namespace with the local name.  
