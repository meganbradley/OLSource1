...
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 Type inference on composite keys depends on the names of the properties in the keys, and the order in which they occur. If the properties in the source sequences do not have the same names, you must assign new names in the keys. For example, if the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> table and <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> table each used different names for their columns, you could create composite keys by assigning identical names in the anonymous types:  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 Composite keys can be also used in a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> clause.  
...
-   Modify program.cs to call the new method from <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>.  
