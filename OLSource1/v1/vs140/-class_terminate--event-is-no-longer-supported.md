...
'Class_Terminate' event is no longer supported. Use 'Sub Finalize' to free resources.  
 ...
 The <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> event of previous versions of Visual Basic is replaced by class destructors.  
...
-   Declare a <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> procedure named <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> to terminate a class. <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is called when the garbage collector detects that there are no more active references to the instance.  
