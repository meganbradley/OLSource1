...
Dictionary member access, which uses an exclamation point (<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>) instead of a period (<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>), is valid only on a class or an interface.  
