...
-   **Subscribing to Events** Before you can subscribe to an event, create an event handler and delegate. Then use the <xref:Microsoft.VisualStudio.Modeling.Store.EventManagerDirectory?qualifyHint=False>property to subscribe to the event. For more information, see [How to: Register to be notified on an event](../vs140/event-handlers-propagate-changes-outside-the-model.md).  
