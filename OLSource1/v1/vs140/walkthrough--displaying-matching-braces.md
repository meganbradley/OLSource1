...
3.  Define a class `BraceMatchingTagger` that inherits from <xref:Microsoft.VisualStudio.Text.Tagging.ITagger`1?qualifyHint=False> of type <xref:Microsoft.VisualStudio.Text.Tagging.TextMarkerTag?qualifyHint=False>.  
  
     [!code[VSSDKBraceMatchingTest#2](../vs140/codesnippet/CSharp/walkthrough--displaying-matching-braces_2.cs)]
[!code[VSSDKBraceMatchingTest#2](../vs140/codesnippet/VisualBasic/walkthrough--displaying-matching-braces_2.vb)]
  
  
4.  Add properties for the text view, the source buffer, and the current snapshot point, and also a set of brace pairs.  
  
     [!code[VSSDKBraceMatchingTest#3](../vs140/codesnippet/CSharp/walkthrough--displaying-matching-braces_3.cs)]
[!code[VSSDKBraceMatchingTest#3](../vs140/codesnippet/VisualBasic/walkthrough--displaying-matching-braces_3.vb)]
  
  
5.  In the tagger constructor, set the properties and subscribe to the view change events <xref:Microsoft.VisualStudio.Text.Editor.ITextCaret.PositionChanged?qualifyHint=False> and <xref:Microsoft.VisualStudio.Text.Editor.ITextView.LayoutChanged?qualifyHint=False>. In this example, for illustrative purposes, the matching pairs are also defined in the constructor.  
  
     [!code[VSSDKBraceMatchingTest#4](../vs140/codesnippet/CSharp/walkthrough--displaying-matching-braces_4.cs)]
[!code[VSSDKBraceMatchingTest#4](../vs140/codesnippet/VisualBasic/walkthrough--displaying-matching-braces_4.vb)]
  
  
6.  As part of the <xref:Microsoft.VisualStudio.Text.Tagging.ITagger`1?qualifyHint=False> implementation, declare a TagsChanged event.  
...
8.  Implement the <xref:Microsoft.VisualStudio.Text.Tagging.ITagger`1.GetTags?qualifyHint=False> method to match braces either when the current character is an open brace or when the previous character is a close brace, as in Visual Studio. When the match is found, this method instantiates two tags, one for the open brace and one for the close brace.  
...
2.  Implement the <xref:Microsoft.VisualStudio.Text.Tagging.IViewTaggerProvider.CreateTagger``1?qualifyHint=False> method to instantiate a BraceMatchingTagger.  
