...
-   You can use an assignment statement to assign the value represented by a numeric expression to a variable, as the following example demonstrates.  
     ...
     [!code[VbVbalrOperators#82](../vs140/codesnippet/VisualBasic/how-to--calculate-numeric-values--visual-basic-_1.vb)]  
  
     In the preceding example, the value of the expression on the right side of the equal operator (<CodeContentPlaceHolder>4\</CodeContentPlaceHolder>) is assigned to the variable <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> on the left side of the operator, so <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> evaluates to 276.  
...
-   Use parentheses to enclose the operations you want to be performed first. The following example shows two different results with the same operands and operators.  
     ...
     [!code[VbVbalrOperators#83](../vs140/codesnippet/VisualBasic/how-to--calculate-numeric-values--visual-basic-_2.vb)]  
  
     In the preceding example, the calculation for <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> performs the addition operator (<CodeContentPlaceHolder>8\</CodeContentPlaceHolder>) first because the parentheses around <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> override normal precedence, and the value assigned to <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> is 276 (4 times 69). The calculation for <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> performs the operators in their normal precedence (<CodeContentPlaceHolder>12\</CodeContentPlaceHolder> before <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>), and the value assigned to <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> is 270 (268 plus 2).  
