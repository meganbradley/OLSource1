...
The <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> statement prevents the garbage collector from relocating a movable variable. The <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> statement is only permitted in an [unsafe](../vs140/unsafe--csharp-reference-.md) context. <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> can also be used to create [fixed size buffers](../vs140/fixed-size-buffers--csharp-programming-guide-.md).  
  
 The <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> statement sets a pointer to a managed variable and "pins" that variable during the execution of the statement. Without <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>, pointers to movable managed variables would be of little use since garbage collection could relocate the variables unpredictably. The C# compiler only lets you assign a pointer to a managed variable in a <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> statement.  
...
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 To initialize pointers of different types, simply nest <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> statements, as shown in the following example.  
  
 [!code[csrefKeywordsFixedLock#3](../vs140/codesnippet/CSharp/fixed-statement--csharp-reference-_3.cs)]  
  
 After the code in the statement is executed, any pinned variables are unpinned and subject to garbage collection. Therefore, do not point to those variables outside the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> statement.  
