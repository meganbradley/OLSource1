...
###  <a name="BKMK_Visual_throughput__FPS_"></a> Visual throughput (FPS)  
 ...
 The **Visual throughput (FPS)** line graph shows the frames per second (FPS) on the UI and composition thread for the app. The values are what you would see in the chrome of an app when <xref:Windows.UI.Xaml.DebugSettings.EnableFrameRateCounter?qualifyHint=False> is enabled.  
