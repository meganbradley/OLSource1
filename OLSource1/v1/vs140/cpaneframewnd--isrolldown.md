...
## Return Value  
 <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> if the mini-frame window must be rolled down; otherwise, <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
  
## Remarks  
 This method is called by the framework to determine whether a mini-frame window should be rolled down. The rollup/rolldown feature is enabled for a mini-frame window if it contains at least one pane that has the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> flag. This flag is set when a pane is created. For more information, see [CBasePane::CreateEx](../vs140/cbasepane--createex.md).  
