...
In [!INCLUDE[vs_current_short](../vs140/includes/vs_current_short_md.md)], [!INCLUDE[ndptecclick](../vs140/includes/ndptecclick_md.md)] deployment allows you to configure how an application is updated. However, if you need to use and customize advanced [!INCLUDE[ndptecclick](../vs140/includes/ndptecclick_md.md)] deployment features, you will need to access the deployment object model provided by \<xref:System.Deployment.Application*>. You can use the \<xref:System.Deployment.Application*> APIs for advanced tasks such as:  
...
6.  Debug what happens between the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>/<CodeContentPlaceHolder>3\</CodeContentPlaceHolder> method calls.  
...
8.  Attempt to attach the debugger to the version 1 application as it downloads an update for version 2. Alternatively you can use the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> method or simply <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> in Visual Basic. Of course, you should not leave these method calls in production code.  
 ...
 Use the \<xref:System.Deployment.Application.ApplicationDeployment.IsNetworkDeployed*> property to invoke the \<xref:System.Deployment.Application*> APIs only when the application is deployed; the APIs should not be invoked during debugging in [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)].  
