---
title: "CPane::GetExclusiveRowMode"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "reference"
f1_keywords: 
  - "CPane::GetExclusiveRowMode"
  - "GetExclusiveRowMode"
  - "CPane.GetExclusiveRowMode"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "GetExclusiveRowMode method"
ms.assetid: 934d006d-f1e7-4ed0-834a-b3216742af2c
caps.latest.revision: 11
robots: noindex,nofollow
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# CPane::GetExclusiveRowMode
Determines if the pane is in exclusive row mode.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Return Value  
 <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> if the pane is in exclusive row mode; otherwise, <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
  
## Remarks  
 For more information about exclusive row mode, see [CPane::SetExclusiveRowMode](../vs140/cpane--setexclusiverowmode.md).  
  
## Requirements  
 **Header:** afxPane.h  
  
## See Also  
 [CPane Class](../vs140/cpane-class.md)   
 [MFC Hierarchy Chart](../vs140/hierarchy-chart.md)