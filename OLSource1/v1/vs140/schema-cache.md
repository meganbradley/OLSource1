...
The XML Editor provides a schema cache located in the %InstallRoot%\Xml\Schemas directory. The schema cache is global to all users on your computer and includes standard XML Schemas that are used for IntelliSense and XML document validation.  
 ...
 The XML editor can also find schemas located in the solution, schemas specified in the **Schemas** field of the document **Properties** window, and schemas identified by the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> attributes.  
...
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 The <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> attribute can be any file path or http URL pointing to the schema. The file path can be relative to the catalog document. The following variables, delimited by %%, are recognized by the editor and will be expanded in the path:  
...
-   LCID  
 ...
 The catalog document can include a <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> element, which points to other catalogs. You can use the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> element to point to a central catalog shared by your team or company, or an online catalog shared with your business partners. The <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> attribute is the file path or http URL for the other catalogs. The following is an example of the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> element:  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 The catalog can also control how schemas are associated with XML documents using the special <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> element. This element associates schemas that have no target namespace with a particular file extension, which can be useful because the XML Editor does not do any auto-association of schemas that do not have a <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> attribute. In the following example the <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> element associates the dotNetConfig schema with all files that have the "config" file extension:  
...
## Localized Schemas  
 ...
 In the following example a new <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> element has been created that uses the %LCID% variable to point to the localized schema.  
...
2.  Add a new <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> element that points to the directory of additional schemas.  
