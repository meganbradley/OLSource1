...
|internal|Member is public inside the assembly but private outside the assembly.  `internal` is a context-sensitive keyword.  For more information, see [Context-Sensitive Keywords](../vs140/context-sensitive-keywords---c---component-extensions-.md).|  
...
```  
 ...
 Calling the destructor suppresses—by using [SuppressFinalize](assetId:///M:System.GC.SuppressFinalize(System.Object)?qualifyHint=False&autoUpgrade=True)—finalization of the object. If the destructor is not called, your type's finalizer will eventually be called by the garbage collector.  
...
```  
 ...
 If a finalizer deletes a native pointer in a managed type, you must ensure that references to or through the native pointer are not prematurely collected; call the destructor on the managed type instead of using [KeepAlive](assetId:///M:System.GC.KeepAlive(System.Object)?qualifyHint=False&autoUpgrade=True).  
