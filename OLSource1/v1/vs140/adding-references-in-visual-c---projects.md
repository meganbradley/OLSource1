 ...
 **Copy Local Satellite Assemblies**  
 Specifies whether to automatically copy the satellite assemblies of the referenced assembly to the target location during a build. Only used if **Copy Local** is <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>.  
  
 **Reference Assembly Output**  
 Specifies that this assembly is used in the build process. If <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, the assembly is used on the compiler command line during the build.  
 ...
 **Strong Name**  
 <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> if the referenced assembly has a strong name. A strong named assembly is uniquely versioned.  
