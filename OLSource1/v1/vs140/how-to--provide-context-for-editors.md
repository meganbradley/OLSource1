...
1.  Call <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> on your \<xref:Microsoft.VisualStudio.OLE.Interop.IServiceProvider*> interface for the \<xref:Microsoft.VisualStudio.Shell.Interop.SVsMonitorUserContext*> service.  
...
1.  Call <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> on the \<xref:Microsoft.VisualStudio.Shell.Interop.SVsTrackSelectionEx*> service to return a pointer to the \<xref:Microsoft.VisualStudio.Shell.Interop.IVsTrackSelectionEx*> interface.  
  
2.  Call \<xref:Microsoft.VisualStudio.Shell.Interop.IVsTrackSelectionEx.OnElementValueChange*>, specifying an element identifier (<CodeContentPlaceHolder>2\</CodeContentPlaceHolder> parameter) value of SEID_UserContext to indicate that you are passing context to the global level.  
...
2.  Call \<xref:Microsoft.VisualStudio.Shell.Interop.IVsUserContext.SetDirty*> on the context bag to indicate that the editor or designer has new context.  
    ...
    > [!NOTE]
    >  The <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> flag is automatically set to <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> whenever context is added or removed from the context bag. The **Dynamic Help** window only calls \<xref:Microsoft.VisualStudio.Shell.Interop.IVsUserContextUpdate.UpdateUserContext*> on the context bag if the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> flag is set to <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>. It is reset to <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> after the update.  
