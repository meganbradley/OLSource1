...
#### Parameters  
 *nBusyReply*  
 A value from the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> enumeration, which is defined in COMPOBJ.H. It can have any one of the following values:  
...
## Remarks  
 ...
 When an application has been made busy with a call to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, it responds to calls from the OLE system DLLs with a value determined by the last setting of <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>. The calling application uses this busy reply to determine what action to take.  
