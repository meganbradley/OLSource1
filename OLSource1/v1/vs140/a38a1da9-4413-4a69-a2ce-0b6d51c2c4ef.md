...
The [TryCast](../vs140/trycast-operator--visual-basic-.md) operator is used with a type parameter operand that is not guaranteed to be a reference type.  
 ...
 <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> operates only on reference types, such as classes or interfaces. When you pass a type parameter as an argument to <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, you must constrain that type parameter to be a reference type. You can do this by including one or more of the following in the type parameter's constraint list:  
...
-   If you need to pass this type parameter to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, constrain it with one or more of the constraints in the preceding list.  
  
-   If you cannot require the type parameter to accept only a reference type, you cannot use it with <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>. You might be able to use the [CType Function](../vs140/ctype-function--visual-basic-.md) instead.  
