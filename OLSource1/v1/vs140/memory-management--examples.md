...
1.  Use the **new** operator, which returns a pointer to the object, to allocate objects on the heap. Use the **delete** operator to delete them.  
     ...
     The following heap and frame examples assume that the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> constructor takes no arguments.  
  
     [!code[NVC_MFC_Utilities#7](../vs140/codesnippet/CPP/memory-management--examples_7.cpp)]  
  
     If the argument for the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> constructor is a pointer to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, the statement for frame allocation is:  
