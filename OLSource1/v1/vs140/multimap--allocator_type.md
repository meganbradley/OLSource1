---
title: "multimap::allocator_type"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "multimap.allocator_type"
  - "map/std::multimap::allocator_type"
  - "allocator_type"
  - "std::multimap::allocator_type"
  - "std.multimap.allocator_type"
  - "multimap::allocator_type"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "allocator_type typedef"
ms.assetid: d0dcf16a-f446-4f2a-9823-6cb7e19ca00e
caps.latest.revision: 17
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# multimap::allocator_type
A type that represents the allocator class for the multimap object.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Remark  
 **allocator_type** is a synonym for the template parameter **Allocator**.  
  
## Example  
 See the example for [get_allocator](../vs140/multimap--get_allocator.md) for an example using <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
  
## Requirements  
 **Header:** \<map>  
  
 **Namespace:** std  
  
## See Also  
 [multimap Class](../vs140/multimap-class.md)   
 [Standard Template Library](../vs140/standard-template-library.md)