...
## What Is a Query and What Does It Do?  
 ...
 Generally, the source data is organized logically as a sequence of elements of the same kind. A SQL database table contains a sequence of rows. Similarly, an [!INCLUDE[vstecado](../vs140/includes/vstecado_md.md)] <xref:System.Data.DataTable?qualifyHint=False> contains a sequence of <xref:System.Data.DataRow?qualifyHint=False> objects. In an XML file, there is a "sequence" of XML elements (although these are organized hierarchically in a tree structure). An in-memory collection contains a sequence of objects.  
  
 From an application's viewpoint, the specific type and structure of the original source data is not important. The application always sees the source data as an <xref:System.Collections.Generic.IEnumerable`1?qualifyHint=False> or <xref:System.Linq.IQueryable`1?qualifyHint=False> collection. In [!INCLUDE[sqltecxlinq](../vs140/includes/sqltecxlinq_md.md)], the source data is made visible as an `IEnumerable`<<xref:System.Xml.Linq.XElement?qualifyHint=False>>. In [!INCLUDE[linq_dataset](../vs140/includes/linq_dataset_md.md)], it is an `IEnumerable`<<xref:System.Data.DataRow?qualifyHint=False>>. In [!INCLUDE[vbtecdlinq](../vs140/includes/vbtecdlinq_md.md)], it is an `IEnumerable` or `IQueryable` of whatever custom objects you have defined to represent the data in the SQL table.  
