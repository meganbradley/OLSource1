...
1.  Implement basic support for the [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] settings mechanism.  
    ...
    -   Ensure that the VSPackage's implementation of the [QueryInterface](assetId:///M:System.Runtime.InteropServices.Marshal.QueryInterface(System.IntPtr,System.Guid@,System.IntPtr@)?qualifyHint=False&autoUpgrade=True) method supplies an <xref:Microsoft.VisualStudio.Shell.Interop.IVsUserSettings*> interface when called with `IID_IVsUserSettings`.  
...
2.  Optionally, alert the IDE of the need to export a particular setting.  
     ...
     A VSPackage can support more than one Custom Settings Point (settings category). Implementations of the [NeedExport](assetId:///M:Microsoft.VisualStudio.Shell.Interop.IVsUserSettingsQuery.NeedExport(System.String,System.Int32@)?qualifyHint=False&autoUpgrade=True) method must check the supplied Custom Settings Point's GUID or settings category argument to determine if a particular group of settings must be saved.  
...
3.  Write settings data to the settings file.  
     ...
     To support exporting settings, a VSPackage must always implement the [ExportSettings](assetId:///M:Microsoft.VisualStudio.Shell.Interop.IVsUserSettings.ExportSettings(System.String,Microsoft.VisualStudio.Shell.Interop.IVsSettingsWriter)?qualifyHint=False&autoUpgrade=True) method.  
    ...
    1.  A VSPackage can support more than one Custom Settings Point (settings category). In the example below, the assetId:///M:Microsoft.VisualStudio.Shell.Interop.IVsUserSettings.ExportSettings(System.String,Microsoft.VisualStudio.Shell.Interop.IVsSettingsWriter)?qualifyHint=False&autoUpgrade=True method calls a different implementation for persisting command bar state as opposed to persisting key binding state.  
    ...
    2.  A VSPackage must use the supplied <xref:Microsoft.VisualStudio.Shell.Interop.IVsSettingsWriter*> interface to save data to the setting file.  
        ...
        > [!NOTE]
        >  In addition to data explicitly written and transparent to the assetId:///M:Microsoft.VisualStudio.Shell.Interop.IVsUserSettings.ExportSettings(System.String,Microsoft.VisualStudio.Shell.Interop.IVsSettingsWriter)?qualifyHint=False&autoUpgrade=True implementation, the settings API also saves [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] version information. Saved settings can be compared against the version of the IDE that generated them during settings import.  
