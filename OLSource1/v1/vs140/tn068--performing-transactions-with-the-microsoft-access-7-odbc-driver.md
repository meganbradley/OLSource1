...
## Overview  
 If your database application performs transactions, you must be careful to call <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> in the correct sequence in your application. The Microsoft Access 7.0 driver uses the Microsoft Jet database engine, and Jet requires that your application not begin a transaction on any database that has an open cursor. For the MFC ODBC database classes, an open cursor equates to an open <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> object.  
  
 If you open a recordset before calling **BeginTrans**, you may not see any error messages. However, any recordset updates your application makes become permanent after calling <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>, and the updates will not be rolled back by calling **Rollback**. To avoid this problem, you must call **BeginTrans** first and then open the recordset.  
  
 MFC checks the driver functionality for cursor commit and rollback behavior. Class <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> provides two member functions, <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>, to determine the effect of any transaction on your open <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> object. For the Microsoft Access 7.0 ODBC driver, these member functions return <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> because the Access driver does not support cursor preservation. Therefore, you must call <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> following a **CommitTrans** or **Rollback** operation.  
...
> [!NOTE]
>  Do not use this technique if you need to change the recordset member variables **m_strFilter** or <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> between transactions. In that case, you should close the recordset after each **CommitTrans** or **Rollback** operation.  
