...
3.  Choose the **Framework** node, and then, in the **Framework** list, choose <xref:System.Data?qualifyHint=False>, <xref:System.Data.Entity?qualifyHint=False>,  and <xref:System.Data.Linq?qualifyHint=False>.  
...
1.  Add the following code to your program. In this example, you add two objects with a relationship between them, and you add an instructor and an office assignment. The table `OfficeAssignments` contains the `InstructorID` column, which references the `PersonID` column in the `Person` table.  
    ...
    ```  
     ...
     Nothing is changed in the database until you call <xref:System.Data.Objects.ObjectContext.SaveChanges?qualifyHint=False>.  
