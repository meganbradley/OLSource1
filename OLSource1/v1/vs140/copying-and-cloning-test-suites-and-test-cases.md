...
There are several ways to duplicate test suites and test cases when you are [planning tests](../vs140/planning-manual-tests-using-the-web-portal.md) in [!INCLUDE[vstsTfsExt](../vs140/includes/vststfsext_md.md)] (TFS).  
...
1.  Right click a test case and choose **Create copy**.  
     ...
     ![Right&#45;click a test case and choose Create copy](../vs140/media/almt_mtm_copy1.png "ALMT_mtm_copy1")  
...
2.  Edit the new test. You must at least change its title. Under **Links**, you might want to delete the link to the old test case.  
     ...
     ![Right&#45;click a test case and choose Create copy](../vs140/media/almt_mtm_copy2.png "ALMT_mtm_copy2")  
...
###  <a name="shallowCopyPlan"></a> Copy suites from another plan or suite  
 ...
 ![Copying test suites](../vs140/media/mtr_copy3.png "MTR_Copy3")  
  
1.  Create the test plan for the new sprint.  
  
2.  Select the destination suite or plan and then get the suites you want to copy.  
  
     ![Copy Test Suites From Another Test Plan](../vs140/media/copytestsuites_anotherplan.png "CopyTestSuites_AnotherPlan")  
...
### Clone a test plan and its test cases  
 ...
 ![Cloning test suites](../vs140/media/mtr_clone3.png "MTR_Clone3")  
...
2.  In the dialog, select the suites you want to copy and set the new area and iteration paths.  
     ...
     ![Clone a test plan](../vs140/media/mtm_testplanclone.png "MTM_TestPlanClone")  
...
3.  [Update any query-based suites](../vs140/organizing-test-cases-using-test-suites.md) that you copied to use the new area and iteration paths.  
  
4.  [Specify a build in the destination test plan](../vs140/automate-a-test-case-in-microsoft-test-manager.md) if you have cloned automated test cases.  
...
|----------------------|------------|-----------|  
|Test case|![Topic applies](../vs140/media/doesapply.gif "DoesApply")|Each new test case retains its shared steps.<br /><br /> A link is made between the source and new test cases.<br /><br /> The new test cases do not have test runs, bugs, test results, and build information.|  
|Shared steps referenced by cloned test cases|![Topic applies](../vs140/media/doesapply.gif "DoesApply")||  
|Test suite|![Topic applies](../vs140/media/doesapply.gif "DoesApply")|The following data is retained:<br /><br /> -   Names and hierarchical structure of the test suites<br />-   Order of the test cases<br />-   Assigned testers<br />-   Configurations|  
|Action Recordings linked from a cloned test case|![Topic applies](../vs140/media/doesapply.gif "DoesApply")||  
|Links and Attachments|![Topic applies](../vs140/media/doesapply.gif "DoesApply")||  
|Test configuration|![Topic applies](../vs140/media/doesapply.gif "DoesApply")|The test configuration is reapplied in the destination test plan.|  
|Test settings|![Topic does not apply](../vs140/media/doesnotapply.gif "DoesNotApply")|The test setting for the destination test plan is applied.|  
|Test results|![Topic does not apply](../vs140/media/doesnotapply.gif "DoesNotApply")||  
|Test runs and exploratory test sessions|![Topic does not apply](../vs140/media/doesnotapply.gif "DoesNotApply")|Because test runs are applicable only to the source test plan, they are not copied.|  
|Requirements-based suites|![Topic does not apply](../vs140/media/doesnotapply.gif "DoesNotApply")<br /><br /> Without `/clonerequirements`|Requirements-based test suites are converted to static test suites in the destination test plan. Cloned test cases will be referenced under this static test suite.<br /><br /> Cloned test cases do not include links to their original requirements work items.|  
|Requirements-based suites|![Topic applies](../vs140/media/doesapply.gif "DoesApply")<br /><br /> with `/clonerequirements`|Copied and linked to a new copy of the requirement work item.|  
...
|Bug work items<br /><br /> with `/clonerequirements`|with `/clonerequirements`|Cloned in a team project that uses the Scrum process template, or any project in which the Bug work item type is in the Requirements [work item category](assetId:///2fc6c411-89a9-4af5-8dd3-b2d4c2ecf540).<br /><br /> In other projects, bugs are not cloned.|  
  
#### Example test suite cloned by using tcm.exe  
 ![Source test plan](../vs140/media/mtr_clonesourcetestplan.png "MTR_CloneSourceTestPlan")  
Source Test Plan  
  
 ![Destination test plan](../vs140/media/mtr_clonedestinationtestplan.png "MTR_CloneDestinationTestPlan")  
