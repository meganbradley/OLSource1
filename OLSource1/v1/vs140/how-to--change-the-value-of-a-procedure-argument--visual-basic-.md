...
When you call a procedure, each argument you supply corresponds to one of the parameters defined in the procedure. In some cases, the procedure code can change the value underlying an argument in the calling code. In other cases, the procedure can change only its local copy of an argument.  
 ...
 If the underlying element in the calling code is a modifiable element and the argument is passed <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, the procedure code can use the direct reference to change the element's value in the calling code.  
...
## Changing Local Copies  
 If the underlying element in the calling code is a nonmodifiable element, or if the argument is passed <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, the procedure cannot change its value in the calling code. However, the procedure can change its local copy of such an argument.  
...
1.  In the procedure declaration, specify [ByVal](../vs140/byval--visual-basic-.md) for the parameter corresponding to the argument.  
     ...
     In the calling code, enclose the argument in parentheses in the argument list. This forces [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] to pass the argument by value, even if the corresponding parameter specifies <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
...
## Example  
 The following example shows two procedures that take an array variable and operate on its elements. The <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> procedure simply adds one to each element. The <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> procedure assigns a new array to the parameter <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> and then adds one to each element.  
  
 [!code[VbVbcnProcedures#35](../vs140/codesnippet/VisualBasic/how-to--change-the-value-of-a-procedure-argument--visual-basic-_1.vb)]  
  
 [!code[VbVbcnProcedures#36](../vs140/codesnippet/VisualBasic/how-to--change-the-value-of-a-procedure-argument--visual-basic-_2.vb)]  
  
 [!code[VbVbcnProcedures#37](../vs140/codesnippet/VisualBasic/how-to--change-the-value-of-a-procedure-argument--visual-basic-_3.vb)]  
  
 The first <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> call displays "After increase(n): 11, 21, 31, 41". Because the array <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> is a reference type, <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> can change its members, even though the passing mechanism is <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>.  
  
 The second <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> call displays "After replace(n): 101, 201, 301". Because <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> is passed <CodeContentPlaceHolder>12\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> can modify the variable <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> in the calling code and assign a new array to it. Because <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> is a reference type, <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> can also change its members.  
...
## Compiling the Code  
 When you pass a variable by reference, you must use the <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> keyword to specify this mechanism.  
