...
## Default Security Page  
 You can use the Default Security page to specify default permissions for objects on the system. The Default Security page has three sections: Access, Launch, and Configuration. To change a section's defaults, click the corresponding **Edit Default** button. These Default Security settings are stored in the registry under <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>.  
...
## Default Properties Page  
 On the Default Properties page, you must select the **Enable Distributed COM on this computer** check box if you want clients on other machines to access COM objects running on this machine. Selecting this option sets the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> value to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
...
-   The Location page allows you to specify where the application should run when a client calls <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> on the relevant CLSID. If you select the **Run application on the following computer** check box and enter a computer name, then a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> value is added under the AppID for that application. Clearing the **Run application on this computer** check box renames the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> value to <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> and, thereby, disables it.  
