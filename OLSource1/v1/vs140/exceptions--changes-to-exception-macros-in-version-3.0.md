...
##  \<a name="_core_exception_types_and_the_catch_macro">\</a> Exception Types and the CATCH Macro  
 ...
 [!code[NVC_MFCExceptions#1](../vs140/codesnippet/CPP/exceptions--changes-to-exception-macros-in-version-3.0_1.cpp)]  
 ...
 [!code[NVC_MFCExceptions#19](../vs140/codesnippet/CPP/exceptions--changes-to-exception-macros-in-version-3.0_2.cpp)]  
  
 is thrown as a **CException\***, even though it is constructed as a **CCustomException**. The **CATCH** macro in MFC versions 2.5 and earlier uses <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> to test the type at run time. Because the expression  
  
 [!code[NVC_MFCExceptions#20](../vs140/codesnippet/CPP/exceptions--changes-to-exception-macros-in-version-3.0_3.cpp)]  
  
 is true, the first catch block catches the exception. In version 3.0, which uses C++ exceptions to implement many of the exception-handling macros, the second catch block matches the thrown <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
...
##  \<a name="_core_re.2d.throwing_exceptions">\</a> Re-Throwing Exceptions  
 ...
 [!code[NVC_MFCExceptions#2](../vs140/codesnippet/CPP/exceptions--changes-to-exception-macros-in-version-3.0_4.cpp)]  
  
 Using **THROW** in the catch block causes the pointer <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> to be deleted, so that the outer catch site will receive an invalid pointer. Use <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> to re-throw <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>.  
