...
> [!NOTE]
>  The following managed code examples demonstrate how to provide lists of symbols through implementing the <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2?qualifyHint=False> interface. The object manager calls the methods in this interface and uses the obtained data to populate or update the symbol-browsing tools.  
>   
>  For native code symbol provider implementation, use the <xref:Microsoft.VisualStudio.Shell.Interop.IVsObjectList2?qualifyHint=False> interface.  
  
## Providing Lists of Symbols to the Object Manager  
  
#### To provide lists of symbols to the object manager  
  
1.  Get the number of items in the list of symbols by implementing the <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetItemCount?qualifyHint=False> method. The following example demonstrates how the object manager obtains the information on the number of items in the list.  
...
2.  Get information about the categories and the attributes of a given list item by implementing the <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetCategoryField2?qualifyHint=False> method. The item categories are specified in the <xref:Microsoft.VisualStudio.Shell.Interop.LIB_CATEGORY?qualifyHint=False> enumeration. The following example demonstrates how the object manager obtains attributes of items for a given category.  
...
3.  Get the text representation of a given list item by implementing the <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetTextWithOwnership?qualifyHint=False> method. The following example demonstrates how to obtain a full name of a given item.  
...
5.  Get the information on whether a given list item is expandable by implementing the <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetExpandable3?qualifyHint=False> method. The following example demonstrates how to obtain the information on whether a given item can be expanded.  
...
6.  Get a child list of symbols of a given list item by implementing the <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetList2?qualifyHint=False> method. The following example demonstrates how to obtain a child list of symbols of a given item for **Call** or **Callers** graphs.  
