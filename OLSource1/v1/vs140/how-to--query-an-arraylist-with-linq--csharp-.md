...
```  
 ...
 The use of an explicitly typed range variable in a query expression is equivalent to calling the <xref:System.Linq.Enumerable.Cast``1?qualifyHint=False> method. <xref:System.Linq.Enumerable.Cast``1?qualifyHint=False> throws an exception if the specified cast cannot be performed. <xref:System.Linq.Enumerable.Cast``1?qualifyHint=False> and <xref:System.Linq.Enumerable.OfType``1?qualifyHint=False> are the two Standard Query Operator methods that operate on non-generic <xref:System.Collections.IEnumerable?qualifyHint=False> types. For more information, see[Type Relationships in LINQ Query Operations (C#)](../vs140/type-relationships-in-linq-query-operations--csharp-.md).  
