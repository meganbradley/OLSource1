...
```  
 ...
 However, the <xref:System.Xml.XmlDocument*> approach generally does not perform as well as LINQ to XML, because the [SelectNodes](assetId:///M:System.Xml.XmlNode.SelectNodes(System.String)?qualifyHint=False&autoUpgrade=True) method must do the following internally every time it is called:  
