...
|---------------|-----------------|  
|[in] <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>|The width of the border, in pixels.|  
  
## Remarks  
 If this method succeeds, the border width is set to the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> parameter. Otherwise, the border width is reset to the default value that is specified by the current [theme](_inet_themes_visualstyles_overview), or zero if themes are not used.  
...
## Example  
 The following code example defines the variable, <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, that is used to programmatically access the month calendar control. This variable is used in the next example.  
