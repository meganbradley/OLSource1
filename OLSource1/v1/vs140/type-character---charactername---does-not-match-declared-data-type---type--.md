...
A variable is declared with one data type but referred to with a type character representing an incompatible data type, for example <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> when <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is declared <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
