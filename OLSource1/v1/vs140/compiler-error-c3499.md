...
a lambda that has been specified to have a void return type cannot return a value  
 ...
 The compiler generates this error when a lambda expression that specifies <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> as the return type returns a value; or when a lambda expression contains more than one statement and returns a value, but does not specify its return type.  
