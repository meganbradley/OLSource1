...
-   Couple the device to your computer, and run the tests using Microsoft Test Manager. This option allows you to capture screenshots and [collect diagnostic data](../vs140/collect-more-diagnostic-data-in-manual-tests.md) from the device.  
...
2.  Install the Remote Debugger on the device that you want to test. See [Installing the Remote Debugger](../vs140/run-windows-store-apps-on-a-remote-machine.md#BKMK_Installing_the_Remote_Tools). (This is only supported for Windows 8 client operating systems. Windows Server 2012 is not supported.)  
...
1.  On the machine that you are testing from, [open Microsoft Test Manager](../vs140/connect-microsoft-test-manager-to-your-team-project-and-test-plan.md).  
  
     [Create some test cases](../vs140/plan-manual-tests-with-microsoft-test-manager.md) if you haven’t already done so.  
  
2.  On the **Run Tests** page, choose the **Modify** link next to **Perform tests using:** to specify the remote Windows 8 device.  
  
     ![Select where to run your manual test](../vs140/media/mtr_win8_whererun.png "MTR_Win8_WhereRun")  
  
3.  Choose the **Remote device** option and enter the name of the device that you want to test.  
  
     ![Select where to run test](../vs140/media/mtr_win8_whererun2.png "MTR_Win8_WhereRun2")  
...
1.  Choose **Install Windows Store App**, and then enter the path and name of the .appx file for the Windows Store app that you want to install.  
     ...
     ![Install Windows Store app from MTM](../vs140/media/mtr_win8_installwindowsstyleapp.png "MTR_Win8_InstallwindowsStyleApp")  
  
2.  Follow the steps in the installation wizard.  
  
     ![Tailored Application Installation Steps](../vs140/media/mtr_win8_tailoredappinstallstepsdialog.png "MTR_Win8_TailoredAppInstallStepsDialog")  
  
     ![Test certificate dialog](../vs140/media/mtr_win8_testcertdialog.png "MTR_Win8_TestCertDialog")  
...
2.  Perform the steps in the test on the remote device.  
     ...
     As you complete each step of the test, [mark it passed or failed on the host computer](../vs140/run-manual-tests-with-microsoft-test-manager.md).  
    ...
    |[Event logs](../vs140/collect-more-diagnostic-data-in-manual-tests.md)|Yes.|  
    |Action record/playback|Windows Web apps – Yes.<br /><br /> Windows desktop and store apps – No.|  
    |Create a bug|Yes|  
    |Create environment snapshot of the servers in [an SCVMM lab environment](assetId:///386f79b2-dfdd-46c3-b481-5d7a627175d3).|Yes|  
