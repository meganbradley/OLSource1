...
## Remarks  
 The C++ standard defines an original and a revised meaning for the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> keyword. Before [!INCLUDE[cpp_dev10_long](../vs140/includes/cpp_dev10_long_md.md)], the keyword declares a variable in the automatic storage class; that is, a variable that has a local lifetime. Starting with [!INCLUDE[cpp_dev10_long](../vs140/includes/cpp_dev10_long_md.md)], the keyword deduces the type of a variable from the declaration's initialization expression. Use the **/Zc:auto[-]** compiler option to tell the compiler to use the original or revised meaning of the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> keyword.  
  
 The compiler issues an appropriate diagnostic message if your use of the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> keyword contradicts the current compiler option. For more information, see [auto Keyword](../vs140/auto-keyword.md). For more information about conformance issues with Visual C++, see [Nonstandard Behavior](../vs140/nonstandard-behavior.md).  
