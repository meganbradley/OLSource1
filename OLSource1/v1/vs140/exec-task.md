...
## Remarks  
 This task is useful when a specific [!INCLUDE[vstecmsbuild](../vs140/includes/vstecmsbuild_md.md)] task for the job that you want to perform is not available. However, the `Exec` task, unlike a more specific task, cannot gather output from the tool or command that it runs.  
