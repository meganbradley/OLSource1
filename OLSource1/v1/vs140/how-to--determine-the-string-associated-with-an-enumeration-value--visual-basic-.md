...
-   Use the \<xref:System.Enum.GetNames*> method to retrieve the strings associated with the enumeration members. This example declares an enumeration, <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, then uses the \<xref:System.Enum.GetNames*> method to display the strings associated with each member.  
