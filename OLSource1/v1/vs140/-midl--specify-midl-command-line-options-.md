...
## Remarks  
 All options for the conversion of an IDL file to a TLB file must be given in <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>; MIDL command-line options cannot be specified on the linker's command line. If /MIDL is not specified, the MIDL compiler will be invoked with only the IDL file name and no other options.  
