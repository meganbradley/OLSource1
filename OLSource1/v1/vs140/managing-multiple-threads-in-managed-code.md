...
2.  If you are on the UI thread and you want to synchronously block while you are performing work on a background thread, use the \<xref:System.Threading.Tasks.TaskScheduler*> property <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> inside \<xref:Microsoft.VisualStudio.Threading.JoinableTaskFactory.Run*>:  
...
<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>  
     ...
     If your background thread method isn’t asynchronous and you can’t make it asynchronous, you can still use the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> syntax to switch to the UI thread by wrapping your work with \<xref:Microsoft.VisualStudio.Threading.JoinableTaskFactory.Run*>, as in this example:  
