...
-   [Quick start: Creating and running unit tests for managed code](../vs140/walkthrough--creating-and-running-unit-tests-for-managed-code.md)  
...
##  \<a name="BKMK_The_MyBank_Solution_example">\</a> The MyBank Solution example  
 In this topic, we use the development of a fictional application called <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> as an example. You don’t need the actual code to follow the explanations in this topic. Test methods are written in C# and presented by using the Microsoft Unit Testing Framework for Managed Code, However, the concepts are easily transferred to other languages and frameworks.  
  
 ![MyBank Solution](../vs140/media/ute_mybanksolution.png "UTE_MyBankSolution")  
  
 Our first attempt at a design for the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> application includes an accounts component that represents an individual account and its transactions with the bank, and a database component that represents the functionality to aggregate and manage the individual accounts.  
  
 We create a <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> solution that contains two projects:  
  
-   <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>  
  
-   <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>  
  
 Our first attempt at designing the <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> project contain a class to hold basic information about an account, an interface that specifies the common functionality of any type of account, like depositing and withdrawing assets from the account, and a class derived from the interface that represents a checking account. We begin the Accounts projects by creating the following source files:  
  
-   <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> defines the basic information for an account.  
  
-   <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> defines a standard <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> interface for an account, including methods to deposit and withdraw assets from an account and to retrieve the account balance.  
  
-   <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> contains the <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> class that implements the <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> interface for a checking account.  
  
 We know from experience that one thing a withdrawal from a checking account must do is to make sure that the amount withdrawn is less than the account balance. So we override the <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> method in <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> with a method that checks for this condition. The method might look like this:  
 ...
 **Create your unit test project and unit tests manually**  
 ...
 A unit test project usually mirrors the structure of a single code project. In the MyBank example, you add two unit test projects named <CodeContentPlaceHolder>20\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> to the <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> solution. The test project names are arbitrary, but adopting a standard naming convention is a good idea.  
...
3.  To use one of the Microsoft unit test frameworks, choose **Unit Test Project** from the list of project templates. Otherwise, choose the project template of the unit test framework that you want to use. To test the <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> project of our example, you would name the project <CodeContentPlaceHolder>24\</CodeContentPlaceHolder>.  
...
4.  In your unit test project, add a reference to the code project under test, in our example to the Accounts project.  
 ...
 Each unit test project contains classes that mirror the names of the classes in the code project. In our example, the <CodeContentPlaceHolder>25\</CodeContentPlaceHolder> project would contain the following classes:  
  
-   <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> class contains the unit test methods for the <CodeContentPlaceHolder>27\</CodeContentPlaceHolder> class in the <CodeContentPlaceHolder>28\</CodeContentPlaceHolder> project  
  
-   <CodeContentPlaceHolder>29\</CodeContentPlaceHolder> class contains the unit test methods for <CodeContentPlaceHolder>30\</CodeContentPlaceHolder> class.  
...
-   The **Assert** section verifies that the action of the method under test behaves as expected.  
 ...
 To test the <CodeContentPlaceHolder>31\</CodeContentPlaceHolder> method of our example, we can write two tests: one that verifies the standard behavior of the method, and one that verifies that a withdrawal of more than the balance will fail. In the <CodeContentPlaceHolder>32\</CodeContentPlaceHolder> class, we add the following methods:  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 Note that <CodeContentPlaceHolder>33\</CodeContentPlaceHolder> uses an explicit <CodeContentPlaceHolder>34\</CodeContentPlaceHolder> statement to determine whether the test method passes or fails, while <CodeContentPlaceHolder>35\</CodeContentPlaceHolder> uses the <CodeContentPlaceHolder>36\</CodeContentPlaceHolder> attribute to determine the success of the test method. Under the covers, a unit test framework wraps test methods in try/catch statements. In most cases, if an exception is caught, the test method fails and the exception is ignored. The <CodeContentPlaceHolder>37\</CodeContentPlaceHolder> attribute causes the test method to pass if the specified exception is thrown.  
...
|![Test Explorer group button](../vs140/media/ute_groupby_btn.png "UTE_GroupBy_btn")|To group your tests by category, choose the **Group By** button.|  
 ...
 For more information, see [Running Unit Tests with Test Explorer](../vs140/run-unit-tests-with-test-explorer.md)  
 ...
 **A:** Yes. *Data-driven test methods* let you test a range of values with a single unit test method. Use a <CodeContentPlaceHolder>38\</CodeContentPlaceHolder> attribute for the test method that specifies the data source and table that contains the variable values that you want to test.  In the method body, you assign the row values to variables using the <CodeContentPlaceHolder>39\</CodeContentPlaceHolder>*ColumnName*<CodeContentPlaceHolder>40\</CodeContentPlaceHolder> indexer.  
...
>  These procedures apply only to test methods that you write by using the Microsoft unit test framework for managed code. If you’re using a different framework, consult the framework documentation for equivalent functionality.  
 ...
 For example, assume we add an unnecessary method to the <CodeContentPlaceHolder>41\</CodeContentPlaceHolder> class that is named <CodeContentPlaceHolder>42\</CodeContentPlaceHolder>. <CodeContentPlaceHolder>43\</CodeContentPlaceHolder> adds two integers.  
  
 To create a data-driven test for the <CodeContentPlaceHolder>44\</CodeContentPlaceHolder> method, we first create an Access database named <CodeContentPlaceHolder>45\</CodeContentPlaceHolder> and a table named <CodeContentPlaceHolder>46\</CodeContentPlaceHolder>. The <CodeContentPlaceHolder>47\</CodeContentPlaceHolder> table defines columns to specify the first and second operands of the addition and a column to specify the expected result. We fill a number of rows with appropriate values.  
...
<CodeContentPlaceHolder>5\</CodeContentPlaceHolder>  
 ...
 Learn more about [data-driven unit tests](../vs140/how-to--create-a-data-driven-unit-test.md).  
...
2.  *Shims* use runtime instrumentation to divert calls to a target method to a substitute shim method for non-virtual methods.  
 ...
 Learn more about [isolating unit test methods with Microsoft Fakes](../vs140/isolating-code-under-test-with-microsoft-fakes.md).  
