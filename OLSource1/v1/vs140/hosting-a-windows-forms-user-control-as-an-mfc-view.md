...
MFC uses the CWinFormsView class to host a Windows Forms user control in an MFC view. MFC Windows Forms views are ActiveX controls. The user control is hosted as a child of the native view and occupies the entire client area of the native view.  
 ...
 Because MFC Windows Forms views are ActiveX controls, they do not have the same <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> as MFC views. Also they cannot be passed as a pointer to a [CView](../vs140/cview-class.md) view. In general, use .NET Framework methods to work with Windows Forms views and rely less on Win32.  
