...
4.  Toolbar set command targets: These command targets are registered when you call \<xref:Microsoft.VisualStudio.Shell.Interop.IVsUIShell4.SetupToolbar2*>. The <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> parameter can be <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>. If it is not <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, then this command target is used to update any commands located on the toolbar you are setting up. If the shell is setting up your toolbar, then it passes the window frame as the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> so that all updates to the commands on your toolbar flow through the window frame, even when it is not in focus.  
