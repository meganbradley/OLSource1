...
#### Parameters  
 ...
 Identifies the control that sends the message if the message is from a control. Otherwise, <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> is 0.  
...
## Remarks  
 <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> processes the message map for control notification.  
  
 Override this member function in your derived class to handle the **WM_NOTIFY** message. An override will not process the message map unless the base class <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> is called.  
