...
## Remarks  
 ...
 These values are used to find the first memory context in a list that satisfies the specified comparison criteria. A memory context is given a list of memory contexts to compare itself against through the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> method. The first memory context in the list for which the comparison operator is <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is then returned.  
