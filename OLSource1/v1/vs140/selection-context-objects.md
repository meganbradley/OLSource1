...
The [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] integrated development environment (IDE) uses a global selection context object to determine what should be displayed in the IDE. Each window in the IDE can have its own selection context object pushed to the global selection context. The IDE updates the global selection context with values from a window when that window has the focus. For more information, see [Feedback to the User](../vs140/feedback-to-the-user.md).  
 ...
 Each window frame or site in the IDE has a service called \<xref:Microsoft.VisualStudio.Shell.Interop.STrackSelection*>. The object created by your VSPackage that is sited in the window frame must call the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> method to get a pointer to the \<xref:Microsoft.VisualStudio.Shell.Interop.ITrackSelection*> interface.  
 ...
 Modifying the global selection context triggers events that VSPackages can monitor. VSPackages can perform the following tasks by implementing <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> and \<xref:Microsoft.VisualStudio.Shell.Interop.IVsMonitorSelection*> interfaces:  
