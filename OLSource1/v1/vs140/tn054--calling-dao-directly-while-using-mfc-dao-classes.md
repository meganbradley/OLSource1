...
## A Brief Overview of DAO and MFC's Implementation  
 MFC's wrapping of DAO makes using DAO easier by handling many of the details so you do not have to worry about the little things. This includes the initialization of OLE, the creation and management of the DAO objects (especially the collection objects), error checking, and providing a strongly typed, simpler interface (no **VARIANT** or <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> arguments). You can make direct DAO calls and still take advantage of these features. All your code must do is call **Release** for any objects created by direct DAO calls and *not* modify any of the interface pointers that MFC may rely on internally. For example, do not modify the **m_pDAORecordset** member of an open <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> object unless you understand *all* the internal ramifications. You could, however, use the **m_pDAORecordset** interface to call DAO directly to get the Fields collection. In this case the **m_pDAORecordset** member would not be modified. You simply have to call **Release** on the Fields collection object when you are finished with the object.  
...
## Using the DAO OLE Interfaces  
 ...
 For many of the methods in the DAO interfaces, you will need to manipulate a <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> object (a length-prefixed string used in OLE automation). The <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> object typically is encapsulated within the **VARIANT** data type. The MFC class <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> itself inherits from the **VARIANT** data type. Depending on whether you build your project for ANSI or Unicode, the DAO interfaces will return ANSI or Unicode <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>s. Two macros, **V_BSTR** and **V_BSTRT**, are useful for assuring that the DAO interface gets the <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> of the expected type.  
  
 **V_BSTR** will extract the **bstrVal** member of a <CodeContentPlaceHolder>15\</CodeContentPlaceHolder>. This macro is typically used when you need to pass the contents of a <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> to a method of a DAO interface. The following code fragment shows both the declarations and actual use for two methods of the DAO DAOUser interface that take advantage of the **V_BSTR** macro:  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 Note that the <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> argument specified in the <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> constructor above ensures that there will be an ANSI <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> in the <CodeContentPlaceHolder>20\</CodeContentPlaceHolder> if you build an ANSI version of your application and a Unicode <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> for a Unicode version of your application. This is what DAO expects.  
  
 The other macro, **V_BSTRT**, will extract either an ANSI or Unicode **bstrVal** member of <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> depending on the type of build (ANSI or Unicode). The following code demonstrates how to extract the <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> value from a <CodeContentPlaceHolder>24\</CodeContentPlaceHolder> into a <CodeContentPlaceHolder>25\</CodeContentPlaceHolder>:  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 The **V_BSTRT** macro, along with other techniques to open other types that are stored in <CodeContentPlaceHolder>26\</CodeContentPlaceHolder>, is demonstrated in the DAOVIEW sample. Specifically, this translation is performed in the **CCrack::strVARIANT** method. This method, where possible, translates the value of a <CodeContentPlaceHolder>27\</CodeContentPlaceHolder> into an instance of <CodeContentPlaceHolder>28\</CodeContentPlaceHolder>.  
