...
Applies the given function to successive elements, returning the first value where the function returns a <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> value.  
...
|\<xref:System.Collections.Generic.KeyNotFoundException*>|Thrown when every item of the sequence evaluates to <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> when the given function is applied.|  
