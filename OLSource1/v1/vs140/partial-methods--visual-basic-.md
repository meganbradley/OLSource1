---
title: "Partial Methods (Visual Basic)"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-visual-basic"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "vb.PartialMethod"
  - "PartialMethod"
dev_langs: 
  - "VB"
helpviewer_keywords: 
  - "custom logic into code [Visual Basic]"
  - "partial methods [Visual Basic]"
  - "partial, methods [Visual Basic]"
  - "methods [Visual Basic], partial methods"
  - "inserting custom logic into code"
ms.assetid: 74b3368b-b348-44a0-a326-7d7dc646f4e9
caps.latest.revision: 20
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# Partial Methods (Visual Basic)
Partial methods enable developers to insert custom logic into code. Typically, the code is part of a designer-generated class. Partial methods are defined in a partial class that is created by a code generator, and they are commonly used to provide notification that something has been changed. They enable the developer to specify custom behavior in response to the change.  
  
 The designer of the code generator defines only the method signature and one or more calls to the method. Developers can then provide implementations for the method if they want to customize the behavior of the generated code. When no implementation is provided, calls to the method are removed by the compiler, resulting in no additional performance overhead.  
  
## Declaration  
 The generated code marks the definition of a partial method by placing the keyword <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> at the start of the signature line.  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 The definition must meet the following conditions:  
  
-   The method must be a <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, not a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>.  
  
-   The body of the method must be left empty.  
  
-   The access modifier must be <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>.  
  
## Implementation  
 The implementation consists primarily of filling in the body of the partial method. The implementation is typically in a separate partial class from the definition, and is written by a developer who wants to extend the generated code.  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 The previous example duplicates the signature in the declaration exactly, but variations are possible. In particular, other modifiers can be added, such as <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>. Only one <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> modifier is permitted. For more information about method modifiers, see [Sub Statement](../vs140/sub-statement--visual-basic-.md).  
  
## Use  
 You call a partial method as you would call any other <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> procedure. If the method has been implemented, the arguments are evaluated and the body of the method is executed. However, remember that implementing a partial method is optional. If the method is not implemented, a call to it has no effect, and expressions passed as arguments to the method are not evaluated.  
  
## Example  
 In a file named Product.Designer.vb, define a <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> class that has a <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> property.  
  
 [!code[VbVbalrPartialMeths#4](../vs140/codesnippet/VisualBasic/partial-methods--visual-basic-_1.vb)]  
  
 In a file named Product.vb, provide an implementation for <CodeContentPlaceHolder>12\</CodeContentPlaceHolder>.  
  
 [!code[VbVbalrPartialMeths#5](../vs140/codesnippet/VisualBasic/partial-methods--visual-basic-_2.vb)]  
  
 Finally, in the Main method of a project, declare a <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> instance and provide an initial value for its <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> property.  
  
 [!code[VbVbalrPartialMeths#6](../vs140/codesnippet/VisualBasic/partial-methods--visual-basic-_3.vb)]  
  
 A message box should appear that displays this message:  
  
 <CodeContentPlaceHolder>15\</CodeContentPlaceHolder>  
  
## See Also  
 [Sub Statement](../vs140/sub-statement--visual-basic-.md)   
 [Sub Procedure](../vs140/sub-procedures--visual-basic-.md)   
 [Optional Parameters](../vs140/optional-parameters--visual-basic-.md)   
 [Partial](../vs140/partial--visual-basic-.md)   
 [Code Generation in LINQ to SQL](assetId:///ddcbdaa1-e7fa-4d85-a379-313b49965c07)   
 [How to: Override Default Methods](assetId:///3a73991e-fd4e-4610-93fb-7ced4dc6b7f9)