...
-   Extension DLLs should be compiled with <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> defined. This forces **_AFXDLL** to be also defined and ensures that the proper declarations is pulled in from the MFC header files. It also ensures that **AFX_EXT_CLASS** is defined as **__declspec(dllexport)** while building the DLL, which is necessary if you are using this macro to declare the classes in your extension DLL.  
  
-   Extension DLLs should not instantiate a class derived from <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, but should rely on the client application (or DLL) to provide this object.  
  
-   Extension DLLs should, however, provide a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> function and do any necessary initialization there.  
...
-   It does not have a <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>-derived object. It must work with the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>-derived object of the client application. This means that the client application owns the main message pump, the idle loop, and so on.  
  
-   It calls <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> in its <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> function. The return value of this function should be checked. If a zero value is returned from <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>, return 0 from your <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> function.  
  
-   It creates a **CDynLinkLibrary** object during initialization if the extension DLL wants to export <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> objects or resources to the application.  
...
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 ...
 If your MFC extension DLL needs to maintain extra data for each application, you can derive a new class from **CDynLinkLibrary** and create it in <CodeContentPlaceHolder>17\</CodeContentPlaceHolder>. When running, the DLL can check the current application's list of **CDynLinkLibrary** objects to find the one for that particular extension DLL.  
