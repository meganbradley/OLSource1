...
Specifies that each application domain of your managed application should have its own copy of a particular global variable or static member variable. See [Application Domains and Visual C++](../vs140/application-domains-and-visual-c--.md) for more information.  
 ...
 Using `__declspec(appdomain)` is similar to using [Thread Local Storage (TLS)](../vs140/thread-local-storage--tls-.md). Threads have their own storage, as do application domains. Using `__declspec(appdomain)` ensures the global variable has its own storage in each application domain created for this application.  
