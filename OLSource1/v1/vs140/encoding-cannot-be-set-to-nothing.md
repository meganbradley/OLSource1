...
An attempt to read from or write to a file has failed because the parameter <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> has been set to <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> but requires a valid value.  
  
 \<xref:System.Text.Encoding*> is used to determine what encoding to use when writing to a file. The default is UTF-8.  
  
### To correct this error  
  
-   Supply a valid value for the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> parameter.  
