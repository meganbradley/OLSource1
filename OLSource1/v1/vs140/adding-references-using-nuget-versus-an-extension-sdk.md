...
|MSBuild automatically consumes the  .js or .css files.|Y|In **Solution Explorer**, you can expand the JavaScript SDK reference node to show individual .js or .css files and then generate <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> tags by dragging those files to their source files. The SDK supports F5 and automatic package setup.|Y||  
