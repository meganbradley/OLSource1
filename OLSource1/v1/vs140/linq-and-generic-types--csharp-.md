...
## IEnumerable<T\> variables in LINQ Queries  
 [!INCLUDE[vbteclinq](../vs140/includes/vbteclinq_md.md)] query variables are typed as <xref:System.Collections.Generic.IEnumerable`1*> or a derived type such as <xref:System.Linq.IQueryable`1*>. When you see a query variable that is typed as `IEnumerable<Customer>`, it just means that the query, when it is executed, will produce a sequence of zero or more `Customer` objects.  
