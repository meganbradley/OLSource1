---
title: "IConnectionPointImpl::m_vec"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "reference"
f1_keywords: 
  - "IConnectionPointImpl::m_vec"
  - "ATL.IConnectionPointImpl.m_vec"
  - "m_vec"
  - "IConnectionPointImpl.m_vec"
  - "ATL::IConnectionPointImpl::m_vec"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "m_vec"
ms.assetid: 0c99535b-0668-4620-977b-0291f2192849
caps.latest.revision: 14
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# IConnectionPointImpl::m_vec
Manages the connections between the connection point object and a sink.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Remarks  
 By default, <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is of type [CComDynamicUnkArray](../vs140/ccomdynamicunkarray-class.md).  
  
## Requirements  
 **Header:** atlcom.h  
  
## See Also  
 [IConnectionPointImpl Class](../vs140/iconnectionpointimpl-class.md)