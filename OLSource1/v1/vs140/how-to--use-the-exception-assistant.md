...
|\<xref:System.Exception.TargetSite*>|Method that throws the current exception. If the method that throws the exception is not available and the stack trace is not a null reference (<CodeContentPlaceHolder>0\</CodeContentPlaceHolder> in Visual Basic), \<xref:System.Exception.TargetSite*> obtains the method from the stack trace. If the stack trace is a null reference, \<xref:System.Exception.TargetSite*> also returns a null reference.|  
