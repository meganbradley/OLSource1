...
1.  Create an VSIX project. Name the solution <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
...
3.  Create a class named <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, and have it implement <xref:Microsoft.VisualStudio.Text.Tagging.ITagger<CodeContentPlaceHolder>4\</CodeContentPlaceHolder>1.GetTags*> method, which instantiates the tag spans. This example assumes that the spans in the \<xref:Microsoft.VisualStudio.Text.NormalizedSpanCollection*> passed in to the method are contiguous, although this may not always be the case. This method instantiates a new tag span for each of the outlining regions.  
...
7.  Declare a <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> event handler.  
...
8.  Add a <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> event handler that responds to \<xref:Microsoft.VisualStudio.Text.ITextBuffer.Changed*> events by parsing the text buffer.  
...
## Implementing a Tagger Provider  
 You must export a tagger provider for your tagger. The tagger provider creates an <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> for a buffer of the "text" content type, or else returns an <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> if the buffer already has one.  
  
#### To implement a tagger provider  
  
1.  Create a class named <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> that implements \<xref:Microsoft.VisualStudio.Text.Tagging.ITaggerProvider*>, and export it with the ContentType and TagType attributes.  
...
2.  Implement the \<xref:Microsoft.VisualStudio.Text.Tagging.ITaggerProvider.CreateTagger*> method by adding an <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> to the properties of the buffer.  
