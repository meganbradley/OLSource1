...
1.  Right-click the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> statement, point to **Breakpoint** and then click **Insert Breakpoint**.  
...
3.  Hover the pointer over the thread marker. A DataTip that appears. The DataTip tells you the name and thread ID number for each stopped thread. In this case, there is only one thread, whose name is probably <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>.  
