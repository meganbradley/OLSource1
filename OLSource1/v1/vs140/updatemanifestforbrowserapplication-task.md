...
The <xref:Microsoft.Build.Tasks.Windows.UpdateManifestForBrowserApplication?qualifyHint=False> task is run to add the **<hostInBrowser /\>** element to the application manifest (*projectname*.exe.manifest) when a [!INCLUDE[TLA#tla_xbap](../vs140/includes/tlasharptla_xbap_md.md)] project is built.  
...
```  
 ...
 The <xref:Microsoft.Build.Tasks.Windows.UpdateManifestForBrowserApplication?qualifyHint=False> task is run when an [!INCLUDE[TLA2#tla_xbap](../vs140/includes/tla2sharptla_xbap_md.md)] project is built in order to add the `<hostInBrowser />` element.  
