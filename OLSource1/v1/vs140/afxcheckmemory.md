...
## Remarks  
 ...
 All memory blocks currently allocated on the heap are checked, including those allocated by **new** but not those allocated by direct calls to underlying memory allocators, such as the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> function or the **GlobalAlloc** Windows function. If any block is found to be corrupted, a message is printed to the debugger output.  
  
 If you include the line  
  
 [!code[NVC_MFCCObjectSample#14](../vs140/codesnippet/CPP/afxcheckmemory_1.cpp)]  
  
 in a program module, then subsequent calls to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> show the filename and line number where the memory was allocated.  
  
> [!NOTE]
>  If your module contains one or more implementations of serializable classes, then you must put the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> line after the last <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> macro call.  
  
 This function works only in the Debug version of MFC.  
  
## Example  
 [!code[NVC_MFCCObjectSample#26](../vs140/codesnippet/CPP/afxcheckmemory_2.cpp)]  
