---
title: "CAtlDllModuleT::~CAtlDllModuleT"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "reference"
f1_keywords: 
  - "ATL::CAtlDllModuleT::~CAtlDllModuleT"
  - "~CAtlDllModuleT"
  - "ATL.CAtlDllModuleT.~CAtlDllModuleT"
  - "CAtlDllModuleT::~CAtlDllModuleT"
  - "ATL::CAtlDllModuleT<T>::~CAtlDllModuleT"
  - "CAtlDllModuleT<T>::~CAtlDllModuleT"
  - "CAtlDllModuleT.~CAtlDllModuleT"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "CAtlDllModuleT class, destructor"
  - "~CAtlDllModuleT destructor"
ms.assetid: 24252bbe-eb6d-4960-92ac-f04e1edec14a
caps.latest.revision: 14
robots: noindex,nofollow
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# CAtlDllModuleT::~CAtlDllModuleT
The destructor.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Requirements  
 **Header:** atlbase.h  
  
## See Also  
 [CAtlDllModuleT Class](../vs140/catldllmodulet-class.md)   
 [CAtlDllModuleT::CAtlDllModuleT](../vs140/catldllmodulet--catldllmodulet.md)