...
> [!NOTE]
>  This member function is not applicable on recordsets that are using bulk row fetching. If you have implemented bulk row fetching, then <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> will always return **FALSE** and will result in a failed assertion. For more information about bulk row fetching, see the article [Recordset: Fetching Records in Bulk (ODBC)](../vs140/recordset--fetching-records-in-bulk--odbc-.md).  
  
 <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is implemented through [DoFieldExchange](../vs140/crecordset--dofieldexchange.md).  
