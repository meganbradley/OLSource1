...
## Optimization Declspecs  
 In this section we will look at two declspecs that can be used in programs to help performance: <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>.  
  
 The <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> declspec can only be applied to function declarations that return a pointer, such as <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>  
  
 The <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> declspec is used on functions that return unaliased pointers. This keyword is used for the C-Runtime Library implementation of <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> since it will never return a pointer value that is already in use in the current program (unless you are doing something illegal, such as using memory after it has been freed).  
  
 The <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> declspec gives the compiler more information for performing compiler optimizations. One of the hardest things for a compiler to determine is what pointers alias other pointers, and using this information greatly helps the compiler.  
  
 It is worth pointing out that this is a promise to the compiler, not something that the compiler will verify. If your program uses this <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> declspec inappropriately, your program may have incorrect behavior.  
...
## Optimization Pragmas  
 There are also several useful pragmas for helping optimize code. The first one we’ll discuss is <CodeContentPlaceHolder>11\</CodeContentPlaceHolder>:  
...
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 ...
 <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> is useful for specifying whether or not you want the application to be able to inline a recursive call. By default it is off. For shallow recursion of small functions you may to turn this on. For more information, see [inline_recursion](../vs140/inline_recursion.md).  
  
 Another useful pragma for limiting the depth of inlining is <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>. This is typically useful in situations where you’re trying to limit the size of a program or function. For more information, see [inline_depth](../vs140/inline_depth.md).  
...
## __restrict and \__assume  
 ...
 First, it should be noted that <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> are two different things. While they are somewhat related, their semantics are different. <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> is a type qualifier, like <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>18\</CodeContentPlaceHolder>, but exclusively for pointer types.  
  
 A pointer that is modified with <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> is referred to as a *__restrict pointer*. A __restrict pointer is a pointer that can only be accessed through the \__restrict pointer. In other words, another pointer cannot be used to access the data pointed to by the \__restrict pointer.  
  
 <CodeContentPlaceHolder>20\</CodeContentPlaceHolder> can be a powerful tool for the Visual C++ optimizer, but use it with great care. If used improperly, the optimizer might perform an optimization that would break your application.  
  
 The <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> keyword replaces the **/Oa** switch from previous versions.  
  
 With <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> a developer can tell the compiler to make assumptions about the value of some variable.  
  
 For example <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> tells the optimizer that at that line of code the variable <CodeContentPlaceHolder>24\</CodeContentPlaceHolder> is less than 5. Again this is a promise to the compiler. If <CodeContentPlaceHolder>25\</CodeContentPlaceHolder> is actually 6 at this point in the program then the behavior of the program after the compiler has optimized may not be what you would expect. <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> is most useful prior to switch statements and/or conditional expressions.  
  
 There are some limitations to <CodeContentPlaceHolder>27\</CodeContentPlaceHolder>. First, like <CodeContentPlaceHolder>28\</CodeContentPlaceHolder>, it is only a suggestion, so the compiler is free to ignore it. Also, <CodeContentPlaceHolder>29\</CodeContentPlaceHolder> currently works only with variable inequalities against constants. It does not propagate symbolic inequalities, for example, assume(a < b).  
