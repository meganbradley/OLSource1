...
5.  In this class, export a <xref:Microsoft.VisualStudio.Utilities.ContentTypeDefinition?qualifyHint=False> named "hid" and declare its base definition to be "text".  
...
-   To map this content type to a file name extension, export a <xref:Microsoft.VisualStudio.Utilities.FileExtensionToContentTypeDefinition?qualifyHint=False> that has the extension ".hid" and the content type "hid".  
...
3.  When you export the extension class, add a <xref:Microsoft.VisualStudio.Utilities.ContentTypeAttribute?qualifyHint=False> of type "hid" to it.  
