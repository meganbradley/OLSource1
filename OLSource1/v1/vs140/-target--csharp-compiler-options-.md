...
The **/target** compiler option can be specified in one of four forms:  
 ...
 The assembly manifest is placed in the first .exe output file in the compilation or in the first DLL, if there is no .exe output file. For example, in the following command line, the manifest will be placed in <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>:  
