...
Could not obtain full operation system name due to internal error. This might be caused by WMI not existing on the current machine.  
 ...
 A call to the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> property failed. A possible cause for this failure is if Windows Management Instrumentation (WMI) is not installed on the current computer.  
  
### To correct this error  
  
1.  Add a <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> block around the call to the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> property.  
