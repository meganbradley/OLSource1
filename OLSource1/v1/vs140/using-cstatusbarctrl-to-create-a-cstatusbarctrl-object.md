...
5.  Call [SetText](../vs140/cstatusbarctrl--settext.md) to set the text in a given part of the status bar control. The message invalidates the portion of the control that has changed, causing it to display the new text when the control next receives the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> message.  
