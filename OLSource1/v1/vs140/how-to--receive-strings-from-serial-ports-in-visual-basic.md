...
This topic describes how to use <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> to receive strings from the computer's serial ports in [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)].  
...
2.  Determine which serial port should provide the strings. This example assumes it is <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
  
3.  Use the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> method to obtain a reference to the port. For more information, see \<xref:Microsoft.VisualBasic.Devices.Ports.OpenSerialPort*>.  
  
     The <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> block allows the application to close the serial port even if it generates an exception. All code that manipulates the serial port should appear within this block.  
  
     [!code[VbVbalrMyComputer#39](../vs140/codesnippet/VisualBasic/how-to--receive-strings-from-serial-ports-in-visual-basic_2.vb)]  
  
4.  Create a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> loop for reading lines of text until no more lines are available.  
...
6.  Use an <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> statement to determine if the \<xref:System.IO.Ports.SerialPort.ReadLine*> method returns <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> (which means no more text is available). If it does return <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>, exit the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> loop.  
  
     [!code[VbVbalrMyComputer#42](../vs140/codesnippet/VisualBasic/how-to--receive-strings-from-serial-ports-in-visual-basic_5.vb)]  
  
7.  Add an <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> block to the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> statement to handle the case if the string is actually read. The block appends the string from the serial port to the return string.  
...
## Compiling the Code  
 This example assumes the computer is using <CodeContentPlaceHolder>11\</CodeContentPlaceHolder>.  
  
## Robust Programming  
 This example assumes the computer is using <CodeContentPlaceHolder>12\</CodeContentPlaceHolder>. For more flexibility, the code should allow the user to select the desired serial port from a list of available ports. For more information, see [How to: Show Available Serial Ports in Visual Basic](../vs140/how-to--show-available-serial-ports-in-visual-basic.md).  
  
 This example uses a <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> block to make sure that the application closes the port and to catch any timeout exceptions. For more information, see [Try...Catch...Finally Statement (Visual Basic)](../vs140/try...catch...finally-statement--visual-basic-.md).  
