...
Processes that explicitly link to a DLL call the [FreeLibrary](http://go.microsoft.com/fwlink/p/?LinkID=259188) function when the DLL module is no longer needed. This function decrements the module's reference count and, if the reference count is zero, unmaps it from the address space of the process.  
 ...
 In an MFC application, use [AfxFreeLibrary](../vs140/afxfreelibrary.md) instead of <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> to unload an extension DLL. The interface (function prototype) for <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is the same as <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
