...
---
# Async.Sleep Method (F#)
Creates an asynchronous computation that will sleep for the given time. This is scheduled using a <xref:System.Threading.Timer?qualifyHint=False> object. The operation will not block operating system threads for the duration of the wait.  
...
|Exception|Condition|  
|---------------|---------------|  
|<xref:System.ArgumentOutOfRangeException?qualifyHint=False>|Thrown when the due time is negative and not infinite.|  
