...
The sample in this topic shows how to create AXHost and how to host an ActiveX control using various ATL functions. It also shows how to access the control and sink events (using [IDispEventImpl](../vs140/idispeventimpl-class.md)) from the control that is hosted. The sample hosts the Calendar control in a main window or in a child window.  
 ...
 Notice the definition of the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> symbol. You can change the value of this symbol to vary between 1 and 8. The value of the symbol determines how the control will be created:  
  
-   For even-numbered values of <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, the call to create the host subclasses a window and converts it into a control host. For odd-numbered values, the code creates a child window that acts as a host.  
  
-   For values of <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> between 1 and 4, access to the control and sinking of events are accomplished in the call that also creates the host. Values between 5 and 8 query the host for interfaces and hook the sink.  
...
|8|Main window|Multiple steps|AtlAxCreateControl|  
 ...
 [!code[NVC_ATL_AxHost#1](../vs140/codesnippet/CPP/hosting-activex-controls-using-atl-axhost_1.cpp)]  
