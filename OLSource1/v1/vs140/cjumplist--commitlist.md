...
## Remarks  
 The commit is atomic. An error will be returned if the commit fails.  When <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is called, the current list of removed items will be cleaned up. Calling this method resets the object so that it does not have an active list-building transaction. To update the list, <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> needs to be called again.  
