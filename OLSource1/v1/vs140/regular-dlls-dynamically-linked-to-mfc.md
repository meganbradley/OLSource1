...
A regular DLL dynamically linked to MFC is a DLL that uses MFC internally, and the exported functions in the DLL can be called by either MFC or non-MFC executables. As the name describes, this kind of DLL is built using the dynamic-link library version of MFC (also known as the shared version of MFC). Functions are usually exported from a regular DLL using the standard C interface.  
 ...
 You must add the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> macro at the beginning of all the exported functions in regular DLLs that dynamically link to MFC to set the current module state to the one for the DLL. This is done by adding the following line of code to the beginning of functions exported from the DLL:  
...
-   This type of DLL must instantiate a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>-derived class.  
  
-   This type of DLL uses the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> provided by MFC. Place all DLL-specific initialization code in the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> member function and termination code in <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> as in a normal MFC application.  
 ...
 Regular DLLs must have a <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>-derived class and a single object of that application class, as does an MFC application. However, the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> object of the DLL does not have a main message pump, as does the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> object of an application.  
  
 Note that the <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> mechanism does not apply to a DLL, because the application owns the main message pump. If your DLL brings up modeless dialogs or has a main frame window of its own, your application's main message pump must call a DLL-exported routine that calls <CodeContentPlaceHolder>12\</CodeContentPlaceHolder>.  
  
 Place all DLL-specific initialization in the <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> member function as in a normal MFC application. The <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> member function of your <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> derived class is called from the MFC provided <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> function before the DLL is unloaded.  
