...
## Return Value  
 <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> or any failure <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> value.  
  
## Remarks  
 Once the connection is broken, events will no longer be routed to the handler functions listed in the event sink map.  
  
> [!NOTE]
>  If your class derives from multiple <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> classes, you will need to disambiguate calls to this method by scoping the call with the particular base class you are interested in.  
  
 <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> breaks a connection that was established with the event source specified in <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>.  
