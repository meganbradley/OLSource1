...
-   Kerberos has been disabled on the domain controller.  
 ...
 If Kerberos authentication is not available, change the account that is used to run the Visual Studio Remote Debugging Monitor. For the procedure, see [Error: Remote Debugger Service On the Target Computer Cannot Connect](../vs140/89ecf99d-66bf-4da0-a840-aa95b0be1702.md).  
...
3.  The first line of the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> response shows the full computer name and IP address returned by DNS for the specified computer.  
  
4.  On the debugger host computer, open a **Command Prompt** window and run <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>.  
