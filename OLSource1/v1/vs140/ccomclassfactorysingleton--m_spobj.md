...
---
# CComClassFactorySingleton::m_spObj
The [CComObjectGlobal](../vs140/ccomobjectglobal-class.md) object constructed by <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
...
## Remarks  
 ...
 Note that the current form of <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> presents a breaking change from the way that <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> worked in previous versions of ATL. In previous versions the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> object was created at the same time as the class factory, during server initialization. In Visual C++.NET 2003, the object is created lazily, on the first request. This change could cause errors in programs that rely on early initialization.  
