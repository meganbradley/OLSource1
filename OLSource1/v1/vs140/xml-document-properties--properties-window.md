 ...
 **Output**  
 ...
 If a file is not specified, a default file name is generated based on the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> attribute on the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> element which determines the file extension. The default file is located in the current user's temporary directory.  
 ...
 **Stylesheet**  
 The XSLT file that is used to transform the document when the **Show XSLT Output** command is used. If this field is blank when the **Show XSLT Output** command is used, the editor uses the value provided in the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> processing instruction of the document, or it prompts you for the filename.  
