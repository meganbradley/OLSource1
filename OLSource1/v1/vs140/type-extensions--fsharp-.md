...
## Remarks  
 ...
 In the previous syntax, <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> represents the type that is being extended. Any type that can be accessed can be extended, but the type name must be an actual type name, not a type abbreviation. You can define multiple members in one type extension. The <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> represents the instance of the object being invoked, just as in ordinary members.  
  
 The <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> keyword is optional in lightweight syntax.  
 ...
 [!code[FsLangRef2#3701](../vs140/codesnippet/FSharp/type-extensions--fsharp-_1.fs)]  
 ...
 In the following example, an optional type extension extends the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> type with an extension method <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> that calls the static member <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>. The <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> method demonstrates that the new member is called just like any instance member.  
  
 [!code[FsLangRef2#3702](../vs140/codesnippet/FSharp/type-extensions--fsharp-_2.fs)]  
  
 The new instance member will appear like any other method of the <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> type in IntelliSense, but only when the module that contains the extension is open or otherwise in scope.  
