...
## Remarks  
 The data in all dirty field data members will be transferred to the record on the data source when the current record is updated by a call to the **Update** member function of <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> (following a call to **Edit** or <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>). With this knowledge, you can take further steps, such as unflagging the field data member to mark the column so it will not be written to the data source.  
  
 <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> is implemented through <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>.  
