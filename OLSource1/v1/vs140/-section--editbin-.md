...
## Remarks  
 ...
 To set or change the section's <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, specify a comma (**,**) followed by one or more attributes characters. To negate an attribute, precede its character with an exclamation point (!). The following characters specify memory attributes:  
...
|x|no alignment|  
 ...
 Specify the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> and *alignment* characters as a string with no white space. The characters are not case sensitive.  
