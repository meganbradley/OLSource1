...
#### Parameters  
 ...
 Index of the item to begin the search with, or -1 to start from the beginning. The item at <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is excluded from the search if <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> is not equal to -1.  
  
## Return Value  
 The index of the item if successful or -1 otherwise.  
  
## Remarks  
 The <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> parameter points to an **LVFINDINFO** structure, which contains information used to search for a list view item.  
