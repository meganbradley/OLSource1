...
---
# CScrollView::SetScaleToFitSize
Call <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> when you want to scale the viewport size to the current window size automatically.  
...
## Remarks  
 ...
 You'll typically place the call to <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> in your override of the view's <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> member function. If you do not want automatic scaling, call the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> member function instead.  
  
 <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> can be used to implement a "Zoom to Fit" operation. Use <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> to reinitialize scrolling.  
  
 <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> assumes that the size of the view window has been set. If the view window size has not been set when <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> is called, you will get an assertion. To ensure that this does not happen, make the following call before calling <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>:  
