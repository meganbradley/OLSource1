...
### Combining Variant Generic Delegates  
 You should not combine variant delegates. The [Combine](assetId:///M:System.Delegate.Combine(System.Delegate,System.Delegate)?qualifyHint=False&autoUpgrade=True) method does not support variant delegate conversion and expects delegates to be of exactly the same type. This can lead to a run-time exception when you combine delegates either by using the assetId:///M:System.Delegate.Combine(System.Delegate,System.Delegate)?qualifyHint=False&autoUpgrade=True method (in C# and Visual Basic) or by using the `+` operator (in C#), as shown in the following code example.  
