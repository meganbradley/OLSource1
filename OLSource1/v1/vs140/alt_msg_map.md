...
## Remarks  
 ATL identifies each message map by a number. The default message map (declared with the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> macro) is identified by 0. An alternate message map is identified by <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>.  
 ...
 Always begin a message map with <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>. You can then declare subsequent alternate message maps.  
  
 The [END_MSG_MAP](../vs140/end_msg_map.md) macro marks the end of the message map. Note that there is always exactly one instance of <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>.  
