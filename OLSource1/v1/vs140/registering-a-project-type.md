...
|`InprocServer32`|REG_SZ|`%MODULE%`|Path of the project type DLL. The IDE loads this DLL and passes the VSPackage CLSID to `DllGetClassObject` to get <xref:Microsoft.VisualStudio.OLE.Interop.IClassFactory?qualifyHint=False> to construct the <xref:Microsoft.VisualStudio.Shell.Interop.IVsPackage?qualifyHint=False> object.|  
