...
## Remarks  
 The <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> member function moves the caret only if it is owned by a window in the current task. <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> moves the caret whether or not the caret is hidden.  
