...
-   Return <xref:Microsoft.VisualStudio.VSConstants.S_OK?qualifyHint=False> if the location is valid, or <xref:Microsoft.VisualStudio.VSConstants.S_FALSE?qualifyHint=False> if it is not valid.  
...
### Example  
 ...
 This example assumes that you have added a `GetCodeSpan` method to the <xref:Microsoft.VisualStudio.Package.AuthoringSink?qualifyHint=False> class that validates the text span and returns `true` if it is a valid breakpoint location.  
