...
-   \<xref:Microsoft.VisualStudio.Shell.Interop.IVsSolutionLoadEvents.OnQueryBackgroundLoadProjectBatch*>: This is fired before the loading of a project (or projects). To ensure that other background processes are completed before the projects are loaded, set <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> to **true**.  
  
-   \<xref:Microsoft.VisualStudio.Shell.Interop.IVsSolutionLoadEvents.OnBeforeLoadProjectBatch*>: This is fired when a batch of projects is about to be loaded. If <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> is true, the projects are to be loaded in the background; if <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> is false, the projects are to be loaded synchronously as a result of a user request, for example if the user expands a pending project in Solution Explorer. You can implement this to do expensive work that otherwise would need to be done in \<xref:Microsoft.VisualStudio.Shell.Interop.IVsSolutionLoadEvents.OnAfterOpenProject*>.  
...
-   \<xref:Microsoft.VisualStudio.Shell.Interop.__VSPROPID4.VSPROPID_IsSolutionFullyLoaded*>: <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> returns <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> if the solution and all its projects are loaded, otherwise <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>.  
  
-   \<xref:Microsoft.VisualStudio.Shell.Interop.__VSPROPID4.VSPROPID_IsInBackgroundIdleLoadProjectBatch*>: <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> returns <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> if a batch of projects are currently being loaded in the background, otherwise <CodeContentPlaceHolder>11\</CodeContentPlaceHolder>.  
  
-   \<xref:Microsoft.VisualStudio.Shell.Interop.__VSPROPID4.VSPROPID_IsInSyncDemandLoadProjectBatch*>: <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> returns <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> if a batch of projects are currently being loaded synchronously as a result of a user command or other explicit load, otherwise <CodeContentPlaceHolder>14\</CodeContentPlaceHolder>.  
  
-   \<xref:Microsoft.VisualStudio.Shell.Interop.__VSPROPID2.VSPROPID_IsSolutionClosing*>: <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> returns <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> if the solution is currently being closed, otherwise <CodeContentPlaceHolder>17\</CodeContentPlaceHolder>.  
  
-   \<xref:Microsoft.VisualStudio.Shell.Interop.__VSPROPID.VSPROPID_IsSolutionOpening*>: <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> returns <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> if a solution is currently being opened, otherwise <CodeContentPlaceHolder>20\</CodeContentPlaceHolder>.  
...
-   \<xref:Microsoft.VisualStudio.Shell.Interop.IVsSolution4.EnsureProjectIsLoaded*>: calling this method forces the projects in <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> to load before the method returns.  
  
-   \<xref:Microsoft.VisualStudio.Shell.Interop.IVsSolution4.EnsureProjectsAreLoaded*>: calling this method forces the project in <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> to load before the method returns.  
