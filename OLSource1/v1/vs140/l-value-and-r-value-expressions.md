...
Expressions that refer to memory locations are called "l-value" expressions. An l-value represents a storage region's "locator" value, or a "left" value, implying that it can appear on the left of the equal sign (**=**). L-values are often identifiers.  
 ...
 An identifier is a modifiable l-value if it refers to a memory location and if its type is arithmetic, structure, union, or pointer. For example, if <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is a pointer to a storage region, then <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> is a modifiable l-value that designates the storage region to which <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> points.  
