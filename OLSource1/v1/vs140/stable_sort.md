...
## Remarks  
 ...
 The run-time complexity of <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> depends on the amount of memory available, but the best case (given sufficient memory) is *O*(*N* log *N*) and the worst case is *O*( *N* ( log *N* )2 ), where *N* = *_Last – First.* Usually, the **sort** algorithm is significantly faster than <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>.  
