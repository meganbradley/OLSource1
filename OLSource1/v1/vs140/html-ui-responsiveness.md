...
11. Use the [timeline details graph](#TimelineDetails) to get detailed information about events that are either running too frequently or taking too much time to complete. For example, look for the following:  
    ...
    -   Layout or scripting events that resulted in rendering elements, such as calls to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>. The associated DOM element for the event is provided.  
...
> [!NOTE]
>  <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, a Chrome command, also appears as a user mark.  
...
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 If the second user mark isn't specified, <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> uses a timestamp as the second user mark. The first user mark is required.  
...
-   <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> events, which typically occur in the activated event handler in your code. A tooltip for the event shows the specific event and the URL.  
...
|DOM event|Scripting|A DOM event occurred and was executed.\<br />\<br /> The <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> property for the DOM event, such as  <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>, is shown in parentheses.|  
...
|Windows Runtime async callback function|Scripting|An async operation that triggered a <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> callback function was completed by a Windows Runtime object.|  
...
|User measure|N/A|An app-specific scenario was measured using the <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> method. This is a tool-generated event used for analyzing code.|  
