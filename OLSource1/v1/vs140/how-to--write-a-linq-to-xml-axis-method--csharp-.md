...
## Example  
 The following example uses two extension methods. The first extension method, `GetXPath`, operates on <xref:System.Xml.Linq.XObject?qualifyHint=False>, and returns an XPath expression that when evaluated will return the node or attribute. The second extension method, `Find`, operates on <xref:System.Xml.Linq.XElement?qualifyHint=False>. It returns a collection of <xref:System.Xml.Linq.XAttribute?qualifyHint=False> objects and <xref:System.Xml.Linq.XElement?qualifyHint=False> objects that contain some specified text.  
