...
'Microsoft.VisualBasic.ComClassAttribute' on class '\<classname>' implicitly declares \<type> '\<membername>', which conflicts with a member of the same name in \<type> '\<typename>'. Use 'Microsoft.VisualBasic.ComClassAttribute(InterfaceShadows:=True)' if you want to hide the name on the base '\<typename>'.  
 ...
 A class using a <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> attribute block implicitly defines an interface with the same name as a member of the base class. In this situation, the interface name should shadow the base class member.  
...
1.  If you intend to hide the base class member, set <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> in the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> attribute block.  
