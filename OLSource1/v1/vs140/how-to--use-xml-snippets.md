...
> [!NOTE]
>  There may be cases when the XML snippet does not get invoked. For example, if you try to insert an <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> element inside an <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> node, the editor does not generate an XML snippet. When an <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> element is used inside an <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> node, there are no required attributes or subelements, so the editor does not have any data to insert.  
  
#### To insert snippets using the shortcut name  
  
1.  Position the cursor where you want to insert the XML snippet.  
  
2.  Type <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> in the editor pane.  
