...
1.  In Class View, expand the control project.  
     ...
     Note that the interface that ends in "UI" has one method, <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>. The interface that does not end in "UI" does not have any methods.  
  
2.  Add a method called <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> to the interface that does not end in "UI."  
...
4.  Add another method called <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>, only this time, add it to the interface that ends in "UI." Find the stubbed-out <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> method in the .cpp file and add code to display a message box, for example:  
  
     [!code[NVC_ATL_COM#6](../vs140/codesnippet/CPP/modifying-the-atl-dhtml-control_2.cpp)]  
  
5.  Add a third method, <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>, to the interface that does not end in "UI." Implement this method by calling [IWebBrowser2::Navigate](https://msdn.microsoft.com/en-us/library/aa752133.aspx), as follows:  
...
1.  In Solution Explorer, double-click the .htm file to display the HTML resource.  
     ...
     Examine the HTML, especially the calls to the external Windows dispatch methods. The HTML calls the project's <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> method, and the parameters indicate the body of the control (<CodeContentPlaceHolder>8\</CodeContentPlaceHolder>) and the color to assign ("<CodeContentPlaceHolder>9\</CodeContentPlaceHolder>"). The text following the method call is the label that appears on the button.  
  
2.  Add another <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> method, only change the color. For example:  
...
3.  Add the call to the <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> method you created. For example:  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
     This method will create a button, labeled **HelloHTML**, that the user can click to display the <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> message box.  
