...
## Multi-Process Execution  
 ...
 To avoid this problem but still enable multi-processor builds, [!INCLUDE[vstecmsbuild](../vs140/includes/vstecmsbuild_md.md)] uses "process isolation." By using process isolation, [!INCLUDE[vstecmsbuild](../vs140/includes/vstecmsbuild_md.md)] can create a maximum of <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> processes, where <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> equals the number of processors available on the system. For example, if [!INCLUDE[vstecmsbuild](../vs140/includes/vstecmsbuild_md.md)] builds a solution on a system that has two processors, then only two build processes are created. These processes are re-used to build all the projects in the solution.  
