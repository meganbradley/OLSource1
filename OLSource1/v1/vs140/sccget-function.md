...
## Remarks  
 This function is called with a count and an array of names of the files to be retrieved. If the IDE passes the flag <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>, this means that the items in <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> are not files but directories, and that all files under source control in the given directories are to be retrieved.  
  
 The <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> flag can be combined with the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> flag to retrieve all files in the given directories and all subdirectories as well.  
  
> [!NOTE]
>  <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> should never be passed without <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>. Also, note that if directories C:\A and C:\A\B are both passed on a recursive get, C:\A\B and all its subdirectories will actually be retrieved twice. It is the IDE's responsibility—and not the source control plug-in's—to make sure that duplicates such as this are kept out of the array.  
  
 Finally, even if a source control plug-in specified the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> flag on initialization, indicating that it does not have a user interface for a Get command, this function may still be called by the IDE to retrieve files. The flag simply means that the IDE does not display a Get menu item and that the plug-in is not expected to provide any UI.  
...
2.  Do the equivalent of "delete old" followed by "add new". The following algorithm is one way to accomplish this.  
    ...
    3.  Call the <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> function for both a.txt and b.txt.  
