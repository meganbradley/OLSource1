...
## Differences Between XPath and LINQ to XML  
 ...
 XPath expressions exist in isolation within a string. The C# compiler cannot help parse the XPath expression at compile time. By contrast, [!INCLUDE[sqltecxlinq](../vs140/includes/sqltecxlinq_md.md)] queries are parsed and compiled by the C# compiler. The compiler is able to catch many query errors.  
...
## Result Ordering  
 ...
 However, when iterating through a collection returned by a [!INCLUDE[sqltecxlinq](../vs140/includes/sqltecxlinq_md.md)] XPath axis method, the nodes in the collection are returned in document order. This is the case even when accessing the XPath axes where predicates are expressed in terms of reverse document order, such as <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>.  
  
 By contrast, most of the [!INCLUDE[sqltecxlinq](../vs140/includes/sqltecxlinq_md.md)] axes return collections in document order, but two of them, \<xref:System.Xml.Linq.XNode.Ancestors*> and \<xref:System.Xml.Linq.XElement.AncestorsAndSelf*>, return collections in reverse document order. The following table enumerates the axes, and indicates collection order for each:  
...
## Comparison of Composition  
 ...
 For example, if you have an element in a variable named <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>, and you want to find a grandchild element named <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> under all child elements named <CodeContentPlaceHolder>15\</CodeContentPlaceHolder>, you would write an XPath expression as follows:  
