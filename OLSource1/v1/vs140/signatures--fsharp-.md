...
## Remarks  
 ...
 For more information about the default accessibility, see [Access Control (F#)](../vs140/access-control--fsharp-.md).  
...
|---------------|-----------------|  
|<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>|For a type that has no abstract members, or that should not be extended.|  
|<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>|For a type that is an interface.|  
 ...
 Use the keyword <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> to create a signature for a value or function value. The keyword <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> introduces a type signature.  
  
 You can generate a signature file by using the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> compiler option. Generally, you do not write .fsi files manually. Instead, you generate .fsi files by using the compiler, add them to your project, if you have one, and edit them by removing methods and functions that you do not want to be accessible.  
...
-   Classes and structures that have constructors must expose the declarations of their base classes (the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> declaration). Also, classes and structures that have constructors must expose all of their abstract methods and interface declarations.  
  
-   Interface types must reveal all their methods and interfaces.  
  
 The rules for value signatures are as follows:  
  
-   Modifiers for accessibility (<CodeContentPlaceHolder>6\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>, and so on) and the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> modifiers in the signature must match those in the implementation.  
...
-   If the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> attribute is used, it must appear in both the signature and the implementation, and the same literal value must be used for both.  
...
-   The pattern of parameters (also known as the *arity*) of signatures and implementations must be consistent.  
 ...
 [!code[FsSignatures#9004](../vs140/codesnippet/FSharp/signatures--fsharp-_1.fs)]  
  
 The following code shows the implementation file.  
  
 [!code[FsSignatures#9001](../vs140/codesnippet/FSharp/signatures--fsharp-_2.fs)]  
  
## See Also  
 [F# Language Reference](../vs140/fsharp-language-reference.md)   
 [Access Control (F#)](../vs140/access-control--fsharp-.md)   
