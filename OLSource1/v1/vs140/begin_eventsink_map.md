...
## Remarks  
 In the implementation (.cpp) file that defines the member functions for your class, start the event sink map with the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> macro, then add macro entries for each event to be notified of, and complete the event sink map with the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> macro.  
