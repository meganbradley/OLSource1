...
##  \<a name="SlowStart">\</a> Emulator starts slowly, fails to start due to a timeout, or app deployment fails  
 Under certain conditions, the emulator takes several minutes to start or fails to start due to a timeout. When the emulator fails to start, you see the following message: <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>. The following conditions can result in this error.  
...
-   Remove/Disable third party drivers or software that may be interfering with virtual networking.  
    ...
    1.  Start the Network Connections manager (from the Start screen, type <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> and select this option to view the network connections.)  
...
-   Open Registry Editor by selecting **Run** from the Start buttons context menu, typing <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> in the dialog box, and choosing **OK**.  
