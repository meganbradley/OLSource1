...
This walkthrough puts a shortcut menu in a tool window. A shortcut menu is a menu that appears when a user right-clicks a button, text box, or window background. Commands on a shortcut menu behave the same as commands on other menus or toolbars. To support a shortcut menu, specify it in the .vsct file and display it in response to the right-click of the mouse.  
 ...
 A tool window consists of a WPF user control in a custom tool window class that inherits from <xref:Microsoft.VisualStudio.Shell.ToolWindowPane?qualifyHint=False>.  
 ...
 Alternatively, if your shortcut menu will not access Visual Studio functionality, you can use the <xref:System.Windows.FrameworkElement.ContextMenu?qualifyHint=False> property of a XAML element in the user control. For more information, see [ContextMenu](assetId:///2f40b2bb-b702-4706-9fc4-10bcfd7cc35d).  
...
1.  Create a VSIX project named `TWShortcutMenu` and add a tool window template named **ShortCutMenu** to it. For more information about creating a tool window, see [Creating an Extension with a Tool Window](../vs140/creating-an-extension-with-a-tool-window.md).  
...
6.  In ShortcutMenuControl.xaml, add a <xref:System.Windows.UIElement.MouseRightButtonDown?qualifyHint=False> event to the top level <xref:System.Windows.Controls.UserControl?qualifyHint=False> element. The XAML file should now look like this:  
...
9. Implement the `MyToolWindowMouseRightButtonDown` event as follows.  
    ...
    ```  
     ...
     This creates a <xref:System.ComponentModel.Design.CommandID?qualifyHint=False> object for the shortcut menu, identifies the location of the mouse click, and opens the shortcut menu in that location by using the <xref:Microsoft.VisualStudio.Shell.OleMenuCommandService.ShowContextMenu?qualifyHint=False> method.  
...
10. Implement the command handler.  
    ...
    ```  
     ...
     In this case, just one method handles events for all of the menu items by identifying the <xref:System.ComponentModel.Design.CommandID?qualifyHint=False> and setting the background color accordingly. If the menu items had contained unrelated commands, you would have created a separate event handler for each command.  
