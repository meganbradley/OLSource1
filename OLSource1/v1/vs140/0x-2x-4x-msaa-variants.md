...
## Remarks  
 These variants override the sample count and sample-quality arguments on calls to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> that create render targets. Specifically, these parameters are overridden when:  
  
-   The <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> object passed in <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> describes a render target; that is:  
...
-   The device supports the requested sample count (0, 2, or 4) and sample quality (0) for the requested render target format (D3D11_TEXTURE2D_DESC::Format member), as determined by <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>.  
...
## Restrictions and limitations  
 In Direct3D11, MSAA textures are more restricted than non-MSAA textures. For example, you can't call <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> on an MSAA texture, and calling <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> fails if the sample count and sample quality of the source resource and destination resource don't match, which can occur when this variant overrides the MSAA settings of one resource but not the other.  
...
## Example  
 These variants can be reproduced for render targets created by using <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> by using code like this:  
