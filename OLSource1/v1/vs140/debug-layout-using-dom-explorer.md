...
11. To see how the margin-left rule is applied, choose the **Computed** tab and look under the margin-left rule. You can see that this rule is set with a 5em value, but the computed value is either 66.66px or 146.66px, depending on your target device.  
    ...
    > [!TIP]
    >  The **Computed** tab shows that the margin-left rule is set in the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> CSS selector, found in hub.css. In this demo app, that is where you need to make the fix.  
...
13. Type <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> and press Enter. (You can also use the Up Arrow and Down Arrow keys to change the value.)  
