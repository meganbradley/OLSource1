...
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 In the example, if the parent group, <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, is a child of a top-level menu such as the **Tools** menu, the command will be visible on that menu, but the package that executes the command will not be loaded until the command is clicked by a user. However, by programming the command to implement the \<xref:Microsoft.VisualStudio.OLE.Interop.IOleCommandTarget*> interface, you can enable the package to be loaded when the menu that contains the command is first expanded.  
...
|---------------------|-----------------|  
|Active Project Type|For most project types, this <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> value is the same as the GUID of the VSPackage that implements the project. However, [!INCLUDE[vcprvc](../vs140/includes/vcprvc_md.md)] projects use the Project Type <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> as the value.|  
|Active Window|Typically, this is the last active document window that establishes the current UI context for key bindings. However, it could also be a tool window that has a key binding table that resembles the internal Web browser. For multi-tabbed document windows such as the HTML editor, every tab has a different command context <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>.|  
...
|Active Tool Window|A tool window that is open and has focus.|  
 ...
 A fifth major context area is the UI state of the IDE. UI contexts are identified by active command context <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>s, as follows:  
...
### Hiding and Displaying Commands Based on Context  
 You can display or hide a package command in the IDE without loading the package itself. To do this, define the command in the .vsct file of the package by using the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>, and <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> command flags and adding one or more [VisibilityItem](../vs140/visibilityitem-element.md) elements to the [VisibilityConstraints](../vs140/visibilityconstraints-element.md) section. When a specified command context <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> becomes active, the command is displayed without loading the package.  
...
-   Get the state of a context <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> (by calling the \<xref:Microsoft.VisualStudio.Shell.Interop.IVsMonitorSelection.IsCmdUIContextActive*> method).  
  
-   Turn context <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>s on and off (by calling the \<xref:Microsoft.VisualStudio.Shell.Interop.IVsMonitorSelection.SetCmdUIContext*> method).  
...
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 Also notice that every UI context must be given in a separate <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> element, as follows.  
  
<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>  
## See Also  
 [How to: Create and Handle Commands in VSPackages (C#)](../vs140/menucommands-vs.-olemenucommands.md)   
 [How Packages Contribute User Interface Elements](../vs140/how-vspackages-add-user-interface-elements.md)   
