...
## Remarks  
 The selection operator returns [get](../vs140/auto_ptr--get.md)<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, so that the expression *ap*->**member** behaves the same as ( *ap*.**get**( ) )->**member**, where *ap* is an object of class <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>\<**Type**>. Hence, the stored pointer must not be null, and **Type** must be a class, struct, or union type with a **member** member.  
