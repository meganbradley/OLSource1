...
##  \<a name="ShowInRootsView">\</a> Find an object in the object tree  
 In the Types and Dominators views, you can see the relationship of a particular object to the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> object. Objects rooted to the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> object will not be garbage-collected. You can easily find a known object in the Roots view without searching through the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> object tree. To do this, open the shortcut menu for an object in the Dominators or Type view, and then choose **Show in roots view**.  
...
-   <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> takes a heap snapshot that appears in the JavaScript memory analyzer. This command is one of the [JavaScript Console Commands](../vs140/javascript-console-commands.md).  
  
-   <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> sets a user mark (the inverted triangle) that appears in the timeline of the memory graph in the summary view while the app is running. This command takes one string argument that describes the event and appears as a tooltip in the memory graph. This description must not exceed 100 characters.  
  
> [!TIP]
>  Use <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> to speed up the analysis when repeating memory usage scenarios.  
  
 These commands throw an exception if you add them to your app and run the app outside the JavaScript memory analyzer. However, you can test whether the commands exist before using them. (The commands do not exist early in the session startup phase.) To check whether you can safely call <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>, use this code:  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 To check whether you can safely call <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>, use this code:  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 Here's a memory graph with several user marks and the tooltip for the currently selected user mark, for which the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> string argument is set to "data generated":  
...
-   Look for objects that are retained in memory inadvertently after the user has navigated to a new page, which is a common cause of memory issues. For example:  
    ...
    -   Some objects might provide a <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> method and recommendations for use. For example, you should call <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> on a [WinJS.Binding.List](http://msdn.microsoft.com/library/windows/apps/Hh700774.aspx) if you call the list's <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> method and then navigate away from a page.  
...
-   Consider temporarily modifying code to isolate problems. For example, you might want to:  
    ...
    -   Use the commands for the memory analyzer, <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>15\</CodeContentPlaceHolder>. (See [Associate source code with memory usage data](#JSConsoleCommands).)  
