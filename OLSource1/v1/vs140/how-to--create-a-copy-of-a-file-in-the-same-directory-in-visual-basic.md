...
Use the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> method to copy files. The parameters allow you to overwrite existing files, rename the file, show the progress of the operation, and allow the user to cancel the operation.  
  
### To create a copy of a file in the same folder  
  
-   Use the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> method, supplying the target file and the location. The following example creates a copy of <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> called <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>.  
...
-   Use the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> method, supplying the target file and location, and setting <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> to <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>. The following example creates a copy of <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> called <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> and overwrites any existing files by that name.  
...
-   The destination file exists and <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> is set to <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> (\<xref:System.IO.IOException*>).  
...
-   <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> is set to <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> is set to <CodeContentPlaceHolder>15\</CodeContentPlaceHolder>, and the user has cancelled the operation (\<xref:System.OperationCanceledException*>).  
  
-   <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> is set to <CodeContentPlaceHolder>17\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> is set to <CodeContentPlaceHolder>19\</CodeContentPlaceHolder>, and an unspecified I/O error occurs (\<xref:System.OperationCanceledException*>).  
