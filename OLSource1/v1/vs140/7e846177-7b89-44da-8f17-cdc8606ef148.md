...
A custom attribute is specified more than once on the same programming element, but an \<xref:System.AttributeUsageAttribute*> is applied to the custom attribute, and its \<xref:System.AttributeUsageAttribute.AllowMultiple*> property is set to <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>. \<xref:System.AttributeUsageAttribute.AllowMultiple*> controls whether the attribute is multi-use.  
...
-   Remove the extra specification of the custom attribute, or set \<xref:System.AttributeUsageAttribute.AllowMultiple*> to <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> in the \<xref:System.AttributeUsageAttribute*> applied to it.  
