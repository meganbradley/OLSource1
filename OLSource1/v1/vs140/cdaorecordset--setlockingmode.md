...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "CDaoRecordset class, setting locking mode"
  - "recordsets, setting locking mode"
...
## Remarks  
 ...
 If a page is locked, no other user can edit records on the same page. If you call <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> and pass a nonzero value and another user already has the page locked, an exception is thrown when you call **Edit**. Other users can read data from locked pages.  
  
 If you call <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> with a zero value and later call **Update** while the page is locked by another user, an exception occurs. To see the changes made to your record by another user (and lose your changes), call the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> member function with the bookmark value of the current record.  
