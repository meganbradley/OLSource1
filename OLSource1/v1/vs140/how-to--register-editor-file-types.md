...
1.  Provide the <xref:Microsoft.VisualStudio.Shell.ProvideEditorExtensionAttribute?qualifyHint=False> class with the appropriate parameters for your editor in the class of your VSPackage.  
    ...
    ```  
     ...
     The `projectGuid` is the GUID for miscellaneous file types, defined in <xref:Microsoft.VisualStudio.VSConstants.CLSID_MiscellaneousFilesProject?qualifyHint=False>. The miscellaneous file type is provided, so that the resulting file is not going to be a part of the build process.  
...
2.  Override the <xref:Microsoft.VisualStudio.Shell.Package.Initialize?qualifyHint=False> method.  
  
     In your implementation of the <xref:Microsoft.VisualStudio.Shell.Package.Initialize?qualifyHint=False> method, call the <xref:Microsoft.VisualStudio.Shell.Package.RegisterEditorFactory?qualifyHint=False> method and pass the instance of your editor factory as demonstrated below.  
...
3.  Unregister the editor factories.  
     ...
     Editor factories are automatically unregistered when the VSPackage is disposed. If the editor factory object implements the <xref:System.IDisposable?qualifyHint=False> interface, its `Dispose` method is called after the factory has been unregistered with [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)].  
...
3.  Register the editor factory.  
     ...
     The editor factory is registered in the <xref:Microsoft.VisualStudio.Shell.Interop.IVsRegisterEditors.RegisterEditor?qualifyHint=False> implementation.  
