...
## Managing Menus  
 The frame window participates in updating user-interface items using the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> mechanism described in [How to Update User-Interface Objects](../vs140/how-to--update-user-interface-objects.md). Buttons on toolbars and other control bars are updated during the idle loop. Menu items in drop-down menus on the menu bar are updated just before the menu drops down.  
