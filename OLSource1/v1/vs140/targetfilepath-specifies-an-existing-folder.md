---
title: "TargetFilePath specifies an existing folder"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: na
ms.topic: "article"
ms.assetid: 2334f49e-d080-4ee2-b036-f95c7f76c3ec
caps.latest.revision: 13
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# TargetFilePath specifies an existing folder
The parameter <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> specifies an existing directory but must specify the new directory's name.  
  
### To correct this error  
  
-   Change <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> to specify a new, rather than existing, directory.  
  
## See Also  
 [How to: Move a File in Visual Basic](../vs140/how-to--move-a-file-in-visual-basic.md)   
 [How to: Move a Directory in Visual Basic](assetId:///0f26d1ef-c0a0-4445-8eb0-9b7d0490411c)   
 [How to: Rename a File in Visual Basic](../vs140/how-to--rename-a-file-in-visual-basic.md)   
 [How to: Rename a Directory in Visual Basic](assetId:///780c7afc-a03c-4b01-865a-510fe331b1cc)