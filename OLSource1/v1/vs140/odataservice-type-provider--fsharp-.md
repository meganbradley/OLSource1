...
|?DataServiceCollection : bool|Generates collections derived from <xref:System.Data.Services.Client.DataServiceCollection`1?qualifyHint=False>. The default value is false.|  
  
## Remarks  
 For a walkthrough that shows how to use the ODataService type provider, see [Walkthrough: Accessing Typed Data From an OData Service (F#)](../vs140/walkthrough--accessing-an-odata-service-by-using-type-providers--fsharp-.md).  
...
|MyService.ServiceTypes.*ServiceTypeName*|The service context type, inherited from <xref:System.Data.Linq.DataContext?qualifyHint=False>.|  
|MyService.ServiceTypes.SimpleDataContextTypes.*ServiceTypeName*|Contains one property for each property of the full context type, which returns a <xref:System.Data.Services.Client.DataServiceQuery`1?qualifyHint=False>.<br /><br /> The Credentials property gets or sets the credentials used to query the OData service at runtime.<br /><br /> The DataContext property gets the full data context, of type <xref:System.Data.Linq.DataContext?qualifyHint=False>. This is the base type of the *ServiceTypeName* type generated by the type provider.|  
