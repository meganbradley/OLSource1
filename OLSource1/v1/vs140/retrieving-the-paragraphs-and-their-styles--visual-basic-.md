...
```  
 ...
 This expression is similar to the source of the query in the previous example, [Finding the Default Paragraph Style (Visual Basic)](../vs140/finding-the-default-paragraph-style--visual-basic-.md). The main difference is that it uses the <xref:System.Xml.Linq.XContainer.Descendants?qualifyHint=False> axis instead of the <xref:System.Xml.Linq.XContainer.Elements?qualifyHint=False> axis. The query uses the <xref:System.Xml.Linq.XContainer.Descendants?qualifyHint=False> axis because in documents that have sections, the paragraphs will not be the direct children of the body element; rather, the paragraphs will be two levels down in the hierarchy. By using the <xref:System.Xml.Linq.XContainer.Descendants?qualifyHint=False> axis, the code will work of whether or not the document uses sections.  
