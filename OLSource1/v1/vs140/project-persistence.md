...
## Commit Models  
 ...
 The IDE calls the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> interface on the project to indicate that the project should persist its project items as appropriate. Therefore, the object owns all aspects of its file and format. This includes the name of the format of the object.  
  
 In the case where items are not files, <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is still how non-file-based items are persisted. Project files, such as .vbp files for [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] projects or .vcproj files for [!INCLUDE[vcprvc](../vs140/includes/vcprvc_md.md)] projects, must also be persisted.  
