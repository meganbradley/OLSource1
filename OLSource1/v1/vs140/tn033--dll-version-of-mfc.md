...
-   An MFC Extension DLL does not have a <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>-derived class.  
  
-   An MFC Extension DLL must provide a special <CodeContentPlaceHolder>11\</CodeContentPlaceHolder>. AppWizard supplies a <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> function that you can modify.  
  
-   An MFC Extension DLL will usually provide an initialization routine to create a **CDynLinkLibrary** if the extension DLL wishes to export <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>es or resources to the application. A derived class of **CDynLinkLibrary** may be used if per-application data must be maintained by the extension DLL.  
...
-   Building an MFC Extension DLL. (Note the special flags such as <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> that are used in building an extension DLL)  
...
### Building an Extension DLL  
 You can use AppWizard to create an MFC extension DLL project, and it will automatically generate the appropriate compiler and linker settings. It was also generate a <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> function that you can modify.  
...
-   Add **/D_AFXEXT** to the compiler flags. On the Project Properties dialog, select the C/C++ node. Then select the Preprocessor category. Add <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> to the Define Macros field, separating each of the items with semicolons.  
...
### Changing your Header Files  
 ...
 To export your entire class, use **AFX_EXT_CLASS** in the class definition. This macro is defined by the framework as **__declspec(dllexport)** when **_AFXDLL** and <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> is defined, but defined as **__declspec(dllimport)** when <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> is not defined. <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> as described above, is only defined when building your extension DLL. For example:  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
### Not Exporting the Entire Class  
 Sometimes you may want to export just the individual necessary members of your class. For example, if you are exporting a <CodeContentPlaceHolder>20\</CodeContentPlaceHolder>-derived class, you might only need to export the constructor and the <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> call. You can export these members using the DLL's .DEF file, but you can also use **AFX_EXT_CLASS** in much the same way on the individual members you need to export.  
...
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 ...
 For example, the <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> macro is defined as follows when building an extension DLL:  
  
<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>  
 The line that begins "static <CodeContentPlaceHolder>23\</CodeContentPlaceHolder>" is declaring a static object inside of your class. To export this class correctly and access the runtime information from a client .EXE, you need to export this static object. Because the static object is declared with the modifier <CodeContentPlaceHolder>24\</CodeContentPlaceHolder>, you only need to define <CodeContentPlaceHolder>25\</CodeContentPlaceHolder> to be **__declspec(dllexport)** when building your DLL and define it as **__declspec(dllimport)** when building your client executable.  
  
 As discussed above, **AFX_EXT_CLASS** is already defined in this way. You just need to re-define <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> to be the same as **AFX_EXT_CLASS** around your class definition.  
  
 For example:  
  
<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>  
 MFC always uses the <CodeContentPlaceHolder>27\</CodeContentPlaceHolder> symbol on data items it defines within its macros, so this technique will work for all such scenarios. For example, it will work for <CodeContentPlaceHolder>28\</CodeContentPlaceHolder>.  
...
>  If you are exporting the entire class rather than selected members of the class, static data members are automatically exported.  
 ...
 You can use the same technique to automatically export the <CodeContentPlaceHolder>29\</CodeContentPlaceHolder> extraction operator for classes that use the <CodeContentPlaceHolder>30\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>31\</CodeContentPlaceHolder> macros. Export the archive operator by bracketing the class declarations (located in the .H file) with the following code:  
...
### Limitations of _AFXEXT  
 ...
 The problem is that in Win32, you must explicitly declare any data as **__declspec(dllexport)** if it is to be exported from a DLL, and **__declspec(dllimport)** if it is to be imported from a DLL. When you define <CodeContentPlaceHolder>32\</CodeContentPlaceHolder>, the MFC headers make sure that **AFX_EXT_CLASS** is defined correctly.  
...
<CodeContentPlaceHolder>5\</CodeContentPlaceHolder>  
 ...
 This type of layering cannot be done when using the built-in **AFX_EXT_CLASS** and <CodeContentPlaceHolder>33\</CodeContentPlaceHolder> preprocessor symbols. The technique described above solves this problem in a manner not unlike the mechanism MFC itself uses when building its OLE, Database, and Network extension DLLs.  
...
### DllMain  
 The following is the exact code you should place in your main source file for your extension DLL. It should come after the standard includes. Note that when you use AppWizard to create starter files for an extension DLL, it supplies a <CodeContentPlaceHolder>34\</CodeContentPlaceHolder> for you.  
  
<CodeContentPlaceHolder>7\</CodeContentPlaceHolder>  
 The call to <CodeContentPlaceHolder>35\</CodeContentPlaceHolder> captures the modules runtime-classes (<CodeContentPlaceHolder>36\</CodeContentPlaceHolder> structures) as well as its object factories (<CodeContentPlaceHolder>37\</CodeContentPlaceHolder> objects) for use later when the **CDynLinkLibrary** object is created. The (optional) call to <CodeContentPlaceHolder>38\</CodeContentPlaceHolder> allows MFC to cleanup the extension DLL when each process detaches (which happens when the process exits, or when the DLL is unloaded as a result of a **FreeLibrary** call) from the extension DLL. Since most extension DLLs are not dynamically loaded (usually, they are linked via their import libraries), the call to <CodeContentPlaceHolder>39\</CodeContentPlaceHolder> is usually not necessary.  
  
 If your application loads and frees extension DLLs dynamically, be sure to call <CodeContentPlaceHolder>40\</CodeContentPlaceHolder> as shown above. Also be sure to use <CodeContentPlaceHolder>41\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>42\</CodeContentPlaceHolder> (instead of Win32 functions **LoadLibrary** and **FreeLibrary**) if your application uses multiple threads or if it dynamically loads an extension DLL. Using <CodeContentPlaceHolder>43\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>44\</CodeContentPlaceHolder> insures that the startup and shutdown code that executes when the extension DLL is loaded and unloaded does not corrupt the global MFC state.  
  
 The header file AFXDLLX.H contains special definitions for structures used in extension DLLs, such as the definition for <CodeContentPlaceHolder>45\</CodeContentPlaceHolder> and **CDynLinkLibrary**.  
  
 The global *extensionDLL* must be declared as shown. Unlike the 16-bit version of MFC, you can allocate memory and call MFC functions during this time, since the MFCxx.DLL is fully initialized by the time your <CodeContentPlaceHolder>46\</CodeContentPlaceHolder> is called.  
...
### Sharing Resources and Classes  
 ...
 Exporting resources is done through a resource list. In each application is a singly linked list of **CDynLinkLibrary** objects. When looking for a resource, most of the standard MFC implementations that load resources look first at the current resource module (<CodeContentPlaceHolder>47\</CodeContentPlaceHolder>) and if not found walk the list of **CDynLinkLibrary** objects attempting to load the requested resource.  
  
 Dynamic creation of C++ objects given a C++ class name is similar. The MFC object deserialization mechanism needs to have all of the <CodeContentPlaceHolder>48\</CodeContentPlaceHolder> objects registered so that it can reconstruct by dynamically creating C++ object of the required type based on what was stored earlier.  
  
 If you want the client application to use classes in your extension DLL that are <CodeContentPlaceHolder>49\</CodeContentPlaceHolder>, then you will need to export your classes to be visible to the client application. This is also done by walking the **CDynLinkLibrary** list.  
...
### Initializing the DLL  
 ...
 Each client application that wants to use your DLL must call this initialization routine, if you use this approach. You may also allocate this **CDynLinkLibrary** object in your <CodeContentPlaceHolder>50\</CodeContentPlaceHolder> just after calling <CodeContentPlaceHolder>51\</CodeContentPlaceHolder>.  
...
<CodeContentPlaceHolder>9\</CodeContentPlaceHolder>  
 The routine name, *InitXxxDLL* in this example, can be anything you want. It does not need to be <CodeContentPlaceHolder>52\</CodeContentPlaceHolder>, but doing so makes the export list easier to maintain.  
...
### CWinApp vs. CDynLinkLibrary  
 An MFC Extension DLL does not have a <CodeContentPlaceHolder>53\</CodeContentPlaceHolder>-derived object of its own; instead it must work with the <CodeContentPlaceHolder>54\</CodeContentPlaceHolder>-derived object of the client application. This means that the client application owns the main message pump, the idle loop and so on.  
...
-   It must have a <CodeContentPlaceHolder>59\</CodeContentPlaceHolder> object and follow the standard rules for a message pump.  
...
### Memory Management  
 An application using MFCxx.DLL uses a common memory allocator provided by MSVCRTxx.DLL, the shared C-runtime DLL. The application, any extension DLLs, and well as the MFC DLLs themselves use this shared memory allocator. By using a shared DLL for memory allocation, the MFC DLLs can allocate memory that is later freed by the application or vice versa. Because both the application and the DLL must use the same allocator, you should not override the C++ global <CodeContentPlaceHolder>60\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>61\</CodeContentPlaceHolder>. The same rules apply to the rest of the C run-time memory allocation routines (such as <CodeContentPlaceHolder>62\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>63\</CodeContentPlaceHolder>, **free**, and others).  
  
### Ordinals and class __declspec(dllexport) and DLL naming  
 We do not use the <CodeContentPlaceHolder>64\</CodeContentPlaceHolder> **__declspec(dllexport)** functionality of the C++ compiler. Instead, a list of exports is included with the class library sources (MFCxx.DEF and MFCxxD.DEF). Only these select set of entry points (functions and data) are exported. Other symbols, such as MFC private implementation functions or classes, are not exported All exports are done by ordinal without a string name in the resident or non-resident name table.  
  
 Using <CodeContentPlaceHolder>65\</CodeContentPlaceHolder> **__declspec(dllexport)** may be a viable alternative for building smaller DLLs, but in the case of a large DLL like MFC, the default exporting mechanism has efficiency and capacity limits.  
