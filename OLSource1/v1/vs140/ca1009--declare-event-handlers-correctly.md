...
## Rule Description  
 ...
 Event handler methods should not return a value. In the C# programming language, this is indicated by the return type <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>. An event handler can invoke multiple methods in multiple objects. If the methods were allowed to return a value, multiple return values would occur for each event, and only the value of the last method that was invoked would be available.  
...
## Example  
 The following example shows a delegate that is suited to handling events. The methods that can be invoked by this event handler comply with the signature that is specified in the Design Guidelines. <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is the type name of the delegate. <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> derives from the base class for event data, \<xref:System.EventArgs*>, and holds alarm event data.  
