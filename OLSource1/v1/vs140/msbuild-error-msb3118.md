...
**MSB3118: Application is set to use application trust, but TargetFrameworkVersion is not v3.5.**  
 ...
 This error occurs when you set the \<xref:Microsoft.Build.Tasks.GenerateApplicationManifest.UseApplicationTrust*> property to <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> and set the \<xref:Microsoft.Build.Tasks.GenerateApplicationManifest.TargetFrameworkVersion*> property to a version lower than <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> (for example, <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>).  
  
### To correct this error  
  
-   Set the \<xref:Microsoft.Build.Tasks.GenerateApplicationManifest.TargetFrameworkVersion*> property to <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> or higher.  
