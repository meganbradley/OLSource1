...
#### Parameters  
 ...
 A pointer to a [FONTDESC](http://msdn.microsoft.com/library/windows/desktop/ms692782) structure containing values for initializing the default state of the font property when <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> is **NULL**.  
...
## Remarks  
 If the font property is being loaded from the medium to the control, the font's characteristics are retrieved from the medium and the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> object referenced by <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> is initialized with them. If the font property is being stored, the characteristics in the font object are written to the medium.  
