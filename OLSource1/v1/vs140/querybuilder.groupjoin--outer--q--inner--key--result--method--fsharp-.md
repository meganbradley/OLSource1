---
title: "QueryBuilder.GroupJoin&lt;&#39;Outer,&#39;Q,&#39;Inner,&#39;Key,&#39;Result&gt; Method (F#)"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-fsharp"
ms.tgt_pltfrm: na
ms.topic: "reference"
apilocation: 
  - "FSharp.Core.dll"
apitype: "Assembly"
f1_keywords: 
  - "Microsoft.FSharp.Linq.QueryBuilder.GroupJoin"
dev_langs: 
  - "FSharp"
helpviewer_keywords: 
  - "groupJoin query operator [F#]"
  - "GroupJoin method [F#]"
ms.assetid: a0a6c3b9-65fb-4bc6-bf05-c53ab4c6604f
caps.latest.revision: 10
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# QueryBuilder.GroupJoin&lt;&#39;Outer,&#39;Q,&#39;Inner,&#39;Key,&#39;Result&gt; Method (F#)
A query operator that correlates two sets of selected values based on matching keys and groups the results. Normal usage is <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
  
 **Namespace/Module Path**: Microsoft.FSharp.Linq  
  
 **Assembly**: FSharp.Core (in FSharp.Core.dll)  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
#### Parameters  
 <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>  
 Type: [QuerySource](../vs140/linq.querysource--t--q--class--fsharp-.md)\<'Outer,'Q>  
  
 The outer query.  
  
 <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>  
 Type: [QuerySource](../vs140/linq.querysource--t--q--class--fsharp-.md)\<'Inner,'Q>  
  
 The inner query.  
  
 <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>  
 Type: 'Outer -> 'Key  
  
 A function that specifies the correlation key from the outer query.  
  
 <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>  
 Type: 'Inner -> 'Key  
  
 A function that specifies the correlation key from the inner query.  
  
 <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>  
 Type: 'Outer ->   [seq](../vs140/collections.seq--t--type-abbreviation--fsharp-.md)\<'Inner> ->   'Result  
  
 A function that returns the results of the operation.  
  
## Return Value  
 The resulting query.  
  
## Remarks  
 For more information and examples, see [Query Expressions (F#)](../vs140/query-expressions--fsharp-.md).  
  
## Platforms  
 Windows 8, Windows 7, Windows Server 2012, Windows Server 2008 R2  
  
## Version Information  
 **F# Core Library Versions**  
  
 Supported in: 4.0, Portable  
  
## See Also  
 [Linq.QueryBuilder Class (F#)](../vs140/linq.querybuilder-class--fsharp-.md)   
 [Microsoft.FSharp.Linq Namespace (F#)](../vs140/microsoft.fsharp.linq-namespace--fsharp-.md)   
 [Query Expressions (F#)](../vs140/query-expressions--fsharp-.md)