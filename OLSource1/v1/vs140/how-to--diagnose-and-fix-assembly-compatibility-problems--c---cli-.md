...
This topic explains what can happen when the version of an assembly referenced at compile time doesn't match the version of the assembly referenced at runtime, and how to avoid the problem.  
 ...
 When an assembly is compiled, other assemblies may be referenced with the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> syntax. During the compilation, these assemblies are accessed by the compiler. Information from these assemblies is used to make optimization decisions.  
...
## Example  
 The following program illustrates the problem by reducing the accessibility of a method, and trying to access that method in another assembly without recompiling. Try compiling <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> first. This is the referenced assembly which will change. Then compile <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>. The compilation succeeds. Now, reduce the accessibility of the called method. Recompile <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> with the flag <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>. This makes the method protected, rather than private, so it can longer be called legally. Without recompiling <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>, rerun the application. An exception \<xref:System.MethodAccessException*> will result.  
