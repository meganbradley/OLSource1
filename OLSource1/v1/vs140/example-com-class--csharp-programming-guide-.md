...
-   Events must be declared in the event interface.  
 ...
 To expose properties and methods to COM, you must declare them on the class interface and mark them with a <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> attribute, and implement them in the class. The order in which the members are declared in the interface is the order used for the COM vtable.  
  
 To expose events from your class, you must declare them on the events interface and mark them with a <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> attribute. The class should not implement this interface.  
