...
1.  Create an event object using the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> function.  
  
2.  Create the threads.  
  
3.  Each thread monitors the event state by calling the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> function.  
  
4.  Each thread ends its own execution when the event is set to the signaled state (<CodeContentPlaceHolder>2\</CodeContentPlaceHolder> returns <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>).  
