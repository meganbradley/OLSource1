...
6.  Generate the deployment manifest with a call to Mage.exe. By default, Mage.exe will mark your [!INCLUDE[ndptecclick](../vs140/includes/ndptecclick_md.md)] deployment as an installed application, so that it can be run both online and offline. To make the application available only when the user is online, use the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> argument with a value of <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>. Since this application will take advantage of the multiple deployment feature, exclude the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> argument to Mage.exe. (In versions of the .NET Framework prior to version 3.5, excluding <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> for an offline application will result in an error.)  
