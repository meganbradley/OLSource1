 ...
 **Information for Programmers**  
 ...
 This error occurs because the program did not receive enough memory from the operating system to complete a [_beginthread](../vs140/_beginthread--_beginthreadex.md) or <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> call, or thread local storage has not been initialized by <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
 ...
 We recommend that an executable that calls the C runtime library (CRT) should use <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> for thread creation rather than the Windows API <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>. <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> initializes internal static storage used by many CRT functions in thread local storage. If you use <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> to create a thread, the CRT may terminate the process with R6016 when a call is made to a CRT function that requires initialized internal static storage.