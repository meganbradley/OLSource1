...
#### Parameters  
 ...
 The class from which <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> derives from.  
  
## Remarks  
 For each interface that is implemented, there is one or more <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> macro invocations. For each aggregate that the class uses, there is one **INTERFACE_AGGREGATE** macro invocation.  
