...
## Gaining Access to Document Data from the View  
 The view accesses its document's data either with the [GetDocument](../vs140/cview--getdocument.md) function, which returns a pointer to the document, or by making the view class a C++ <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> of the document class. The view then uses its access to the data to obtain the data when it is ready to draw or otherwise manipulate it.  
  
 For example, from the view's [OnDraw](../vs140/cview--ondraw.md) member function, the view uses **GetDocument** to obtain a document pointer. Then it uses that pointer to access a <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> data member in the document. The view passes the string to the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> function. To see the code for this example, see [Drawing in a View](../vs140/drawing-in-a-view.md).  
