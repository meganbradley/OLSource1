...
## Remarks  
 Because a single physical thread can run in multiple programs, more than one <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> from more than one program can represent the same physical thread.  
  
 When a breakpoint or exception occurs, an event is sent by calling [IDebugEventCallback2::Event](../vs140/idebugeventcallback2--event.md). One of the arguments to this method is an <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> interface representing the current thread. [IDebugThread2::EnumFrameInfo](../vs140/idebugthread2--enumframeinfo.md) is used to obtain the [IDebugStackFrame2](../vs140/idebugstackframe2.md) interface for the current stack frame.  
