...
|Name|Description|  
|----------|-----------------|  
|[COleMessageFilter::COleMessageFilter](#colemessagefilter__colemessagefilter)|Constructs a <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> object.|  
...
## Remarks  
 ...
 By default, a <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> object is allocated when the application is initialized. It can be retrieved with [AfxOleGetMessageFilter](../vs140/afxolegetmessagefilter.md).  
...
### Remarks  
 ...
 The <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> calls increment and decrement, respectively, a counter that determines whether the application is busy. For example, two calls to <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> and one call to <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> still result in a busy state. To cancel a busy state it is necessary to call <CodeContentPlaceHolder>20\</CodeContentPlaceHolder> the same number of times <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> has been called.  
...
##  \<a name="colemessagefilter__colemessagefilter">\</a>  COleMessageFilter::COleMessageFilter  
 Creates a <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> object.  
...
### Remarks  
 ...
 The <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>24\</CodeContentPlaceHolder> calls increment and decrement, respectively, a counter that determines whether the application is busy. For example, two calls to <CodeContentPlaceHolder>25\</CodeContentPlaceHolder> and one call to <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> still result in a busy state. To cancel a busy state it is necessary to call <CodeContentPlaceHolder>27\</CodeContentPlaceHolder> the same number of times <CodeContentPlaceHolder>28\</CodeContentPlaceHolder> has been called.  
  
 By default, the framework enters the busy state during idle processing, which is performed by [CWinApp::OnIdle](../vs140/cwinapp-class.md#cwinapp__onidle). While the application is handling <CodeContentPlaceHolder>29\</CodeContentPlaceHolder> notifications, incoming calls are handled after idle processing is complete.  
...
### Remarks  
 When a calling application is waiting for a call to be completed, the framework calls <CodeContentPlaceHolder>31\</CodeContentPlaceHolder> with a pointer to the pending message. By default, the framework dispatches <CodeContentPlaceHolder>32\</CodeContentPlaceHolder> messages, so that window updates can occur during a call that is taking a long time.  
...
### Parameters  
 *nBusyReply*  
 A value from the <CodeContentPlaceHolder>33\</CodeContentPlaceHolder> enumeration, which is defined in COMPOBJ.H. It can have any one of the following values:  
...
### Remarks  
 ...
 When an application has been made busy with a call to <CodeContentPlaceHolder>34\</CodeContentPlaceHolder>, it responds to calls from the OLE system DLLs with a value determined by the last setting of <CodeContentPlaceHolder>35\</CodeContentPlaceHolder>. The calling application uses this busy reply to determine what action to take.  
...
### Remarks  
 ...
 The caller's response is controlled by the functions <CodeContentPlaceHolder>38\</CodeContentPlaceHolder> and [SetMessagePendingDelay](#colemessagefilter__setmessagependingdelay). <CodeContentPlaceHolder>39\</CodeContentPlaceHolder> determines how long the calling application should wait between retries for a given call. <CodeContentPlaceHolder>40\</CodeContentPlaceHolder> determines how long the calling application waits for a response from the server before taking further action.  
  
 Usually the defaults are acceptable and do not need to be changed. The framework retries the call every <CodeContentPlaceHolder>41\</CodeContentPlaceHolder> milliseconds until the call goes through or the message-pending delay has expired. A value of 0 for <CodeContentPlaceHolder>42\</CodeContentPlaceHolder> specifies an immediate retry, and – 1 specifies cancellation of the call.  
