...
|Exception|Condition|  
|---------------|---------------|  
|\<xref:System.InvalidOperationException*>|Thrown when no element returns <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> when evaluated by the predicate.|  
  
## Return Value  
 The first element for which the Boolean function <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> returns <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>.  
