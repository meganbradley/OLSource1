---
title: "ordered (OpenMP Clauses)"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "ordered"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "ordered OpenMP clause"
ms.assetid: 9236ad6d-2b40-475c-873d-1a9b58cffe5b
caps.latest.revision: 12
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# ordered (OpenMP Clauses)
Required on a parallel [for](../vs140/for--openmp-.md) statement if an [ordered](../vs140/ordered--openmp-directives-.md) directive is to be used in the loop.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Remarks  
 <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> applies to the [for](../vs140/for--openmp-.md) directive.  
  
 For more information, see [2.4.1 for Construct](../vs140/2.4.1-for-construct.md).  
  
## Example  
 See [ordered](../vs140/ordered--openmp-directives-.md) for an example of using <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> clause.  
  
## See Also  
 [Clauses](../vs140/openmp-clauses.md)