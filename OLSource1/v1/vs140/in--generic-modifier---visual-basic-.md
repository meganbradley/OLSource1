---
title: "In (Generic Modifier) (Visual Basic)"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-visual-basic"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "vb.VarianceIn"
dev_langs: 
  - "VB"
helpviewer_keywords: 
  - "contravariance, In keyword [Visual Basic]"
  - "In keyword [Visual Basic]"
ms.assetid: 59bb13c5-fe96-42b8-8286-86293d1661c5
caps.latest.revision: 23
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# In (Generic Modifier) (Visual Basic)
For generic type parameters, the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> keyword specifies that the type parameter is contravariant.  
  
## Remarks  
 Contravariance enables you to use a less derived type than that specified by the generic parameter. This allows for implicit conversion of classes that implement variant interfaces and implicit conversion of delegate types.  
  
 For more information, see [Covariance and Contravariance (C# and Visual Basic)](../vs140/covariance-and-contravariance--csharp-and-visual-basic-.md).  
  
## Rules  
 You can use the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> keyword in generic interfaces and delegates.  
  
 A type parameter can be declared contravariant in a generic interface or delegate if it is used only as a type of method arguments and not used as a method return type. <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> parameters cannot be covariant or contravariant.  
  
 Covariance and contravariance are supported for reference types and not supported for value types.  
  
 In Visual Basic, you cannot declare events in contravariant interfaces without specifying the delegate type. Also, contravariant interfaces cannot have nested classes, enums, or structures, but they can have nested interfaces.  
  
## Behavior  
 An interface that has a contravariant type parameter allows its methods to accept arguments of less derived types than those specified by the interface type parameter. For example, because in .NET Framework 4, in the <xref:System.Collections.Generic.IComparer<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>IComparer(Of Person)<CodeContentPlaceHolder>4\</CodeContentPlaceHolder>IComparer(Of Employee)<CodeContentPlaceHolder>5\</CodeContentPlaceHolder>Person<CodeContentPlaceHolder>6\</CodeContentPlaceHolder>Employee`.  
  
 A contravariant delegate can be assigned another delegate of the same type, but with a less derived generic type parameter.  
  
## Example  
 The following example shows how to declare, extend, and implement a contravariant generic interface. It also shows how you can use implicit conversion for classes that implement this interface.  
  
 [!code[vbVarianceKeywords#1](../vs140/codesnippet/VisualBasic/in--generic-modifier---visual-basic-_1.vb)]  
  
## Example  
 The following example shows how to declare, instantiate, and invoke a contravariant generic delegate. It also shows how you can implicitly convert a delegate type.  
  
 [!code[vbVarianceKeywords#2](../vs140/codesnippet/VisualBasic/in--generic-modifier---visual-basic-_2.vb)]  
  
## See Also  
 [Covariance and Contravariance in Generic Interfaces (C# and Visual Basic)](../vs140/variance-in-generic-interfaces--csharp-and-visual-basic-.md)   
 [Out (Generic Modifier) (Visual Basic)](../vs140/out--generic-modifier---visual-basic-.md)