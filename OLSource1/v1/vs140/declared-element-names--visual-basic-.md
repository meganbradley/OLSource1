...
-   It must begin with an alphabetic character or an underscore (<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>).  
...
-   It must not be more than 1023 characters long.  
 ...
 The length limit of 1023 characters also applies to the entire string of a fully qualified name, such as <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
...
> [!CAUTION]
>  Element names starting with an underscore (<CodeContentPlaceHolder>8\</CodeContentPlaceHolder>) are not part of the [Common Language Specification](assetId:///4f0b77d0-4844-464f-af73-6e06bedeafc6) (CLS), so CLS-compliant code cannot use a component that defines such names. However, an underscore in any other position in an element name is CLS-compliant.  
...
## Escaped Names  
 Generally, an element name must not match any of the keywords reserved by [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)], such as <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>. However, you can define an *escaped name*, which is enclosed by brackets (<CodeContentPlaceHolder>11\</CodeContentPlaceHolder>). An escaped name can match any [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] keyword, since the brackets remove any ambiguity. You also use the brackets when you refer to the name later in your code.  
...
## Case Sensitivity in Names  
 Element names in [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] are case-insensitive. This means that when the compiler compares two names that differ in alphabetic case only, it interprets them as the same name. For example, it considers <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> to refer to the same declared element.  
  
 However, the common language runtime (CLR) uses case-sensitive binding. Therefore, when you produce an assembly or a DLL and make it available to other assemblies, your names are no longer case-insensitive. For example, if you define a class with an element called <CodeContentPlaceHolder>14\</CodeContentPlaceHolder>, and other assemblies make use of your class through the common language runtime, they must refer to the element as <CodeContentPlaceHolder>15\</CodeContentPlaceHolder>. If you subsequently recompile your class and change the element's name to <CodeContentPlaceHolder>16\</CodeContentPlaceHolder>, the other assemblies using your class could no longer access that element. Therefore, when you release an updated version of an assembly, you should not change the alphabetic case of any public elements.  
