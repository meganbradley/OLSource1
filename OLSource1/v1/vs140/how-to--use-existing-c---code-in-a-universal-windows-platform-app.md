...
1.  In the project properties for the UWP project, in the Linker section, add the path to the library in the Input property. For example, for a library in the project that places its output in *SolutionFolder*\Debug\MyNativeLibrary\MyNativeLibrary.lib, add the relative path <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>.  
