...
A class or interface is marked as <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> when it is nested in a type that is marked as <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> or is not marked.  
...
-   If you require that this type remain within its current containment hierarchy, remove the \<xref:System.CLSCompliantAttribute*> from its definition or mark it as <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>.  
