...
## Remarks  
 Currently, this method returns only the type or value of a constant. The caller must free the buffer returned in `prgBuffer` by calling COM's `CoTaskMemFree` function (C++) or [FreeCoTaskMem](assetId:///M:System.Runtime.InteropServices.Marshal.FreeCoTaskMem(System.IntPtr)?qualifyHint=False&autoUpgrade=True) (C#).  
