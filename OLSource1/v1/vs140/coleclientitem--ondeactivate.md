...
---
# COleClientItem::OnDeactivate
Called by the framework when the OLE item transitions from the in-place active state (<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>) to the loaded state, meaning that it is deactivated after an in-place activation.  
