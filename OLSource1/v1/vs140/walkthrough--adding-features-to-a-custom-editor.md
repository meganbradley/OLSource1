...
3.  Implement an editor factory by implementing the <xref:Microsoft.VisualStudio.Shell.Interop.IVsEditorFactory?qualifyHint=False> interface.  
...
5.  Implement the <xref:Microsoft.VisualStudio.OLE.Interop.IOleCommandTarget?qualifyHint=False> interface to handle commands.  
  
6.  Provide document persistence and response to external file changes by doing the following:  
  
    1.  To persist the file, implement <xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistDocData2?qualifyHint=False> and <xref:Microsoft.VisualStudio.Shell.Interop.IPersistFileFormat?qualifyHint=False> on your editor's document data object.  
  
    2.  To respond to external file changes, implement <xref:Microsoft.VisualStudio.Shell.Interop.IVsFileChangeEx?qualifyHint=False> and <xref:Microsoft.VisualStudio.Shell.Interop.IVsDocDataFileChangeControl?qualifyHint=False> on your editor's document data object.  
  
        > [!NOTE]
        >  Call `QueryService` on <xref:Microsoft.VisualStudio.Shell.Interop.SVsFileChangeEx?qualifyHint=False> to obtain a pointer to `IVsFileChangeEx`.  
  
7.  Coordinate document edit events with source code control. To do this:  
  
    1.  Obtain a pointer to `IVsQueryEditQuerySave2` by calling `QueryService` on <xref:Microsoft.VisualStudio.Shell.Interop.SVsQueryEditQuerySave?qualifyHint=False>.  
  
    2.  When the first edit event occurs, call the <xref:Microsoft.VisualStudio.Shell.Interop.IVsQueryEditQuerySave2.QueryEditFiles?qualifyHint=False> method.  
    ...
    3.  Similarly, before saving the file, call the <xref:Microsoft.VisualStudio.Shell.Interop.IVsQueryEditQuerySave2.QuerySaveFile?qualifyHint=False> method.  
...
8.  Enable the **Properties** window to display properties for text selected in the editor. To do this:  
    ...
    1.  Call <xref:Microsoft.VisualStudio.Shell.Interop.ITrackSelection.OnSelectChange?qualifyHint=False> each time text selection changes, passing in your implementation of <xref:Microsoft.VisualStudio.Shell.Interop.ISelectionContainer?qualifyHint=False>.  
  
    2.  Call `QueryService` on <xref:Microsoft.VisualStudio.Shell.Interop.STrackSelection?qualifyHint=False> service to obtain a pointer to <xref:Microsoft.VisualStudio.Shell.Interop.ITrackSelection?qualifyHint=False>.  
...
9. Enable users to drag and drop items between the editor and the **Toolbox**, or between external editors (like Microsoft Word) and the **Toolbox**. To do this:  
    ...
    2.  Implement the <xref:Microsoft.VisualStudio.Shell.Interop.IVsToolboxUser?qualifyHint=False> interface on the view so your editor can enable and disable items in the **Toolbox**.  
  
    3.  Implement <xref:Microsoft.VisualStudio.Shell.Interop.IVsPackage.ResetDefaults?qualifyHint=False> and call `QueryService` on <xref:Microsoft.VisualStudio.Shell.Interop.SVsToolbox?qualifyHint=False> service to obtain a pointer to the <xref:Microsoft.VisualStudio.Shell.Interop.IVsToolbox2?qualifyHint=False> and <xref:Microsoft.VisualStudio.Shell.Interop.IVsToolbox3?qualifyHint=False> interfaces.  
...
10. Decide whether you want any other optional features for your editor.  
    ...
    -   If you want your editor to support find and replace commands, implement <xref:Microsoft.VisualStudio.TextManager.Interop.IVsFindTarget?qualifyHint=False>.  
  
    -   If you want to use a document outline tool window in your editor, implement `IVsDocOutlineProvider`.  
  
    -   If you want to use a status bar in your editor, implement <xref:Microsoft.VisualStudio.Shell.Interop.IVsStatusbarUser?qualifyHint=False> and call `QueryService` for <xref:Microsoft.VisualStudio.Shell.Interop.SVsStatusbar?qualifyHint=False> to obtain a pointer to `IVsStatusBar`.  
...
-   The IDE implements `IOleUndoManager` as a service accessible from a <xref:Microsoft.VisualStudio.Shell.Interop.ILocalRegistry2.CreateInstance?qualifyHint=False> or as an object that can be instantiated through <xref:Microsoft.VisualStudio.Shell.Interop.ILocalRegistry2.CreateInstance?qualifyHint=False>. Your editor implements the `IOleUndoUnit` interface for each `Undo` action.  
