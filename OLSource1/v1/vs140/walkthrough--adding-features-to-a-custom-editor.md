...
6.  Provide document persistence and response to external file changes by doing the following:  
    ...
    2.  To respond to external file changes, implement \<xref:Microsoft.VisualStudio.Shell.Interop.IVsFileChangeEx*> and \<xref:Microsoft.VisualStudio.Shell.Interop.IVsDocDataFileChangeControl*> on your editor's document data object.  
        ...
        > [!NOTE]
        >  Call <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> on \<xref:Microsoft.VisualStudio.Shell.Interop.SVsFileChangeEx*> to obtain a pointer to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
  
7.  Coordinate document edit events with source code control. To do this:  
  
    1.  Obtain a pointer to <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> by calling <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> on \<xref:Microsoft.VisualStudio.Shell.Interop.SVsQueryEditQuerySave*>.  
...
8.  Enable the **Properties** window to display properties for text selected in the editor. To do this:  
    ...
    2.  Call <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> on \<xref:Microsoft.VisualStudio.Shell.Interop.STrackSelection*> service to obtain a pointer to \<xref:Microsoft.VisualStudio.Shell.Interop.ITrackSelection*>.  
...
9. Enable users to drag and drop items between the editor and the **Toolbox**, or between external editors (like Microsoft Word) and the **Toolbox**. To do this:  
    ...
    1.  Implement <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> on your editor to alert the IDE that your editor is a drop target.  
...
10. Decide whether you want any other optional features for your editor.  
    ...
    -   If you want to use a document outline tool window in your editor, implement <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>.  
  
    -   If you want to use a status bar in your editor, implement \<xref:Microsoft.VisualStudio.Shell.Interop.IVsStatusbarUser*> and call <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> for \<xref:Microsoft.VisualStudio.Shell.Interop.SVsStatusbar*> to obtain a pointer to <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>.  
  
         For example, an editor can display line / column information, selection mode (stream / box), and insertion mode (insert / overstrike).  
  
    -   If you want your editor to support the <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> command, the recommended method is to use the OLE undo manager model. As an alternative, you can have the editor handle the <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> command directly.  
...
13. Expose an Automation Object Model from your editor by implementing the <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> interface.  
  
     For more information, see [Contributing to the Automation Model](../vs140/contributing-to-the-automation-model.md).  
  
## Robust Programming  
  
-   The editor instance is created when the IDE calls the \<xref:Microsoft.VisualStudio.Shell.Interop.IVsEditorFactory.CreateEditorInstance*> method. If the editor supports multiple views, <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> creates both the document data and the document view objects. If the document data object is already open, a non-null <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> value is passed to <CodeContentPlaceHolder>16\</CodeContentPlaceHolder>. Your editor factory implementation must determine whether an existing document data object is compatible by querying for appropriate interfaces on it. For more information, see [Supporting Multiple Document Views](../vs140/supporting-multiple-document-views.md).  
...
-   <CodeContentPlaceHolder>20\</CodeContentPlaceHolder> can express both single and multiple selections. Each selected object is implemented as an <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> object.  
  
-   The IDE implements <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> as a service accessible from a \<xref:Microsoft.VisualStudio.Shell.Interop.ILocalRegistry2.CreateInstance*> or as an object that can be instantiated through \<xref:Microsoft.VisualStudio.Shell.Interop.ILocalRegistry2.CreateInstance*>. Your editor implements the <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> interface for each <CodeContentPlaceHolder>24\</CodeContentPlaceHolder> action.  
