 ...
 **/OPT:REF** eliminates functions and data that are never referenced; **/OPT:NOREF** keeps functions and data that are never referenced.  
 ...
 When /OFT:REF is enabled, LINK removes unreferenced packaged functions and data. An object contains packaged functions and data (COMDATs) if it was compiled by using the [/Gy](../vs140/-gy--enable-function-level-linking-.md) option. This optimization is known as transitive COMDAT elimination. By default, **/OPT:REF** is enabled in non-debug builds. To override this default and keep unreferenced COMDATs in the program, specify **/OPT:NOREF**. You can use the [/INCLUDE](../vs140/-include--force-symbol-references-.md) option to override the removal of a specific symbol.  
 ...
 If [/DEBUG](../vs140/-debug--generate-debug-info-.md) is specified, the default for **/OPT** is **NOREF**, and all functions are preserved in the image. To override this default and optimize a debugging build, specify **/OPT:REF**. Because **/OPT:REF** implies **/OPT:ICF**, we recommend that you also specify **/OPT:NOICF** to preserve identical functions in debugging builds. This makes it easier to read stack traces and set breakpoints in functions that would otherwise be folded together. The **/OPT:REF** option disables incremental linking.  
  
 You have to explicitly mark <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> data as a COMDAT; use [__declspec(selectany)](../vs140/selectany.md).  
  
 Specifying **/OPT:ICF** does not enable the **/OPT:REF** option.  
  
 **ICF[=**  <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> **] &#124; NOICF**  
 Use **/OPT:ICF[=**<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>**]** to perform identical COMDAT folding. Redundant COMDATs can be removed from the linker output. The optional <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> parameter specifies the number of times to traverse the symbols for duplicates. The default number of iterations is two. Additional iterations may locate more duplicates that are uncovered through folding in the previous iteration.  
  
 The linker behaves differently when **/OPT:REF** is specified—and **ICF** is in effect by default—than it does when **/OPT:REF,ICF** is specified explicitly. The form of **ICF** that's enabled with **/OPT:REF** alone does not fold read-only data—this includes .rdata, .pdata, and .xdata. Therefore, fewer functions are folded when images are produced for [!INCLUDE[vcprx64](../vs140/includes/vcprx64_md.md)] because functions in these modules are more dependent on read-only data—for example, .pdata and .xdata. To get full **ICF** folding behavior, explicitly specify **/OPT:ICF**.  
  
 To place functions in COMDATs, you use the **/Gy** compiler option; to place <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> data, you declare it <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>. For information about how to specify data for folding, see [selectany](../vs140/selectany.md).  
...
> [!NOTE]
>  Because **/OPT:ICF** can cause the same address to be assigned to different functions or read-only data members (<CodeContentPlaceHolder>7\</CodeContentPlaceHolder> variables compiled by using **/Gy**), it can break a program that depends on unique addresses for functions or read-only data members. For more information, see [/Gy (Enable Function-Level Linking)](../vs140/-gy--enable-function-level-linking-.md).  
...
## Remarks  
 ...
 You can use the [/VERBOSE](../vs140/-verbose--print-progress-messages-.md) option to see the functions that are removed by **/OPT:REF** and the functions that are folded by **/OPT:ICF**.  
