...
## Remarks  
 You must close the database specified by <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> before you repair it. In a multiuser environment, other users cannot have <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> open while you are repairing it. If <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> is not closed or is not available for exclusive use, an error occurs.  
