...
> [!NOTE]
>  A pointer to the main frame window is stored in the [m_pMainWnd](../vs140/cwinthread--m_pmainwnd.md) member variable of the application object. A call to <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> in your override of the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> member function of <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> sets <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> for you. If you do not call <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>, you must set the variable's value in <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> yourself. (SDI COM component (server) applications may not set the variable if /Embedding is on the command line.) Note that <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> is now a member of class <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> rather than <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>.  
