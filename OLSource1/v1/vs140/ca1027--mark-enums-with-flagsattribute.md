...
## Example  
 In the following example, <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> is an enumeration that meets the requirements for using \<xref:System.FlagsAttribute*>, but does not have it. The <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> enumeration does not have values that are powers of two, but incorrectly specifies \<xref:System.FlagsAttribute*>. This violates rule [Do not mark enums with FlagsAttribute](../vs140/ca2217--do-not-mark-enums-with-flagsattribute.md).  
  
 [!code[FxCop.Design.EnumFlags#1](../vs140/codesnippet/CSharp/ca1027--mark-enums-with-flagsattribute_1.cs)]  
