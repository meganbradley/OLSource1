...
## Example  
 In the following example, `DaysEnumNeedsFlags` is an enumeration that meets the requirements for using <xref:System.FlagsAttribute*>, but does not have it. The `ColorEnumShouldNotHaveFlag` enumeration does not have values that are powers of two, but incorrectly specifies <xref:System.FlagsAttribute*>. This violates rule [Do not mark enums with FlagsAttribute](../vs140/ca2217--do-not-mark-enums-with-flagsattribute.md).  
