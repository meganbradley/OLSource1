...
1.  Create a VSIX project named `TestToolWindowSearch` with a tool window named **TestSearch**. If you need help doing this, see [Creating an Extension with a Tool Window](../vs140/creating-an-extension-with-a-tool-window.md).  
  
## To create a tool window  
  
1.  In the `TestToolWindowSearch` project, open the TestSearchControl.xaml file.  
  
2.  Replace the existing `<StackPanel>` block with the following block, which adds a read-only <xref:System.Windows.Controls.TextBox?qualifyHint=False> to the <xref:System.Windows.Controls.UserControl?qualifyHint=False> in the tool window.  
...
4.  Remove the `button1_Click()` method.  
     ...
     This code adds a public <xref:System.Windows.Controls.TextBox?qualifyHint=False> property  named **SearchResultsTextBox** and a public string property named **SearchContent**. In the constructor, SearchResultsTextBox is set to the text box, and SearchContent is initialized to a newline-delimited set of strings. The content of the text box is also initialized to the set of strings.  
...
1.  In the TestSearch.cs file, add the following code to the `TestSearch` class. The code overrides the <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch.SearchEnabled?qualifyHint=False> property so that the get accessor returns `true`.  
  
     To enable search, you must override the <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch.SearchEnabled?qualifyHint=False> property. The <xref:Microsoft.VisualStudio.Shell.ToolWindowPane?qualifyHint=False> class implements <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch?qualifyHint=False> and provides a default implementation that doesn’t enable search.  
...
## To add the search implementation  
 When you enable search on a <xref:Microsoft.VisualStudio.Shell.ToolWindowPane?qualifyHint=False>, as in the previous procedure, the tool window creates a search host. This host sets up and manages search processes, which always occur on a background thread. Because the <xref:Microsoft.VisualStudio.Shell.ToolWindowPane?qualifyHint=False> class manages the creation of the search host and the setting up of the search, you need only create a search task and provide the search method. The search process occurs on a background thread, and calls to the tool window control occur on the UI thread. Therefore, you must use the <xref:Microsoft.VisualStudio.Shell.ThreadHelper.Invoke``1?qualifyHint=False> method to manage any calls that you make in dealing with the control.  
...
2.  In the `TestSearch` class, add the following code, which performs the following actions:  
    ...
    -   Overrides the <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch.CreateSearch?qualifyHint=False> method to create a search task.  
  
    -   Overrides the <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch.ClearSearch?qualifyHint=False> method to restore the state of the text box. This method is called when a user cancels a search task and when a user sets or unsets options or filters. Both <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch.CreateSearch?qualifyHint=False> and <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch.ClearSearch?qualifyHint=False> are called on the UI thread. Therefore, you don’t need to access the text box by means of the <xref:Microsoft.VisualStudio.Shell.ThreadHelper.Invoke``1?qualifyHint=False> method.  
  
    -   Creates a class that’s named `TestSearchTask` that inherits from <xref:Microsoft.VisualStudio.Shell.VsSearchTask?qualifyHint=False>, which provides a default implementation of <xref:Microsoft.VisualStudio.Shell.Interop.IVsSearchTask?qualifyHint=False>.  
  
         In `TestSearchTask`, the constructor sets a private field that references the tool window. To provide the search method, you override the <xref:Microsoft.VisualStudio.Shell.VsSearchTask.OnStartSearch?qualifyHint=False> and <xref:Microsoft.VisualStudio.Shell.VsSearchTask.OnStopSearch?qualifyHint=False> methods. The <xref:Microsoft.VisualStudio.Shell.VsSearchTask.OnStartSearch?qualifyHint=False> method is where you implement the search process. This process includes performing the search, displaying the search results in the text box, and calling the base class implementation of this method to report that the search is complete.  
...
1.  In the TestSearch.cs file, add the following code to the `TestSearch` class. The code implements `SearchFiltersEnum` by adding a <xref:Microsoft.VisualStudio.PlatformUI.WindowSearchSimpleFilter?qualifyHint=False> that specifies to filter the search results so that only even lines appear.  
