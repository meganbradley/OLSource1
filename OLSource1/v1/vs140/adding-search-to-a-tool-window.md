...
4.  Remove the `button1_Click()` method.  
    ...
    ```  
     ...
     [!code[ToolWindowSearch#1](../vs140/codesnippet/CSharp/adding-search-to-a-tool-window_1.cs)][!code[ToolWindowSearch#1](../vs140/codesnippet/VisualBasic/adding-search-to-a-tool-window_1.vb)]  
...
2.  In the `TestSearch` class, add the following code, which performs the following actions:  
    ...
    -   Overrides the [CreateSearch](assetId:///M:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch.CreateSearch(System.UInt32,Microsoft.VisualStudio.Shell.Interop.IVsSearchQuery,Microsoft.VisualStudio.Shell.Interop.IVsSearchCallback)?qualifyHint=False&autoUpgrade=True) method to create a search task.  
  
    -   Overrides the <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch.ClearSearch*> method to restore the state of the text box. This method is called when a user cancels a search task and when a user sets or unsets options or filters. Both assetId:///M:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch.CreateSearch(System.UInt32,Microsoft.VisualStudio.Shell.Interop.IVsSearchQuery,Microsoft.VisualStudio.Shell.Interop.IVsSearchCallback)?qualifyHint=False&autoUpgrade=True and <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch.ClearSearch*> are called on the UI thread. Therefore, you don’t need to access the text box by means of the <xref:Microsoft.VisualStudio.Shell.ThreadHelper.Invoke*> method.  
