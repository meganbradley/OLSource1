...
## Implementing the dynamic menu command  
 You create a dynamic menu command class that inherits from <xref:Microsoft.VisualStudio.Shell.OleMenuCommand?qualifyHint=False>. In this implementation, the constructor specifies a predicate to be used for matching commands. You must override the <xref:Microsoft.VisualStudio.Shell.OleMenuCommand.DynamicItemMatch?qualifyHint=False> method to use this predicate to set the <xref:Microsoft.VisualStudio.Shell.OleMenuCommand.MatchedCommandId?qualifyHint=False> property, which identifies the command to be invoked.  
  
1.  Create a new C# class file named DynamicItemMenuCommand.cs, and add a class named **DynamicItemMenuCommand** that inherits from <xref:Microsoft.VisualStudio.Shell.OleMenuCommand?qualifyHint=False>:  
...
4.  Add a constructor that inherits from the <xref:Microsoft.VisualStudio.Shell.OleMenuCommand?qualifyHint=False> constructor and specifies a command handler and a <xref:Microsoft.VisualStudio.Shell.OleMenuCommand.BeforeQueryStatus?qualifyHint=False> handler. Add a predicate for matching the command:  
...
5.  Override the <xref:Microsoft.VisualStudio.Shell.OleMenuCommand.DynamicItemMatch?qualifyHint=False> method so that it calls the matches predicate and sets the <xref:Microsoft.VisualStudio.Shell.OleMenuCommand.MatchedCommandId?qualifyHint=False> property:  
