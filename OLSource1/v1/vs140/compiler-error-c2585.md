...
1.  Converting from a class or structure type based on multiple inheritance. If the type inherits the same base class more than once, the conversion function or operator must use scope resolution (<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>) to specify which of the inherited classes to use in the conversion.  
