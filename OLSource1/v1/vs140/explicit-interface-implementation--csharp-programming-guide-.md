...
If a [class](../vs140/class--csharp-reference-.md) implements two interfaces that contain a member with the same signature, then implementing that member on the class will cause both interfaces to use that member as their implementation. In the following example, all the calls to <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> invoke the same method.  
 ...
 The class member <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is only available through the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> interface, and <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is only available through <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>. Both method implementations are separate, and neither is available directly on the class. For example:  
