...
## MFC OLE DB Classes  
 The MFC library has one class, [COleDBRecordView](../vs140/coledbrecordview-class.md), that displays database records in controls. The view is a form view directly connected to a <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> object and displays the fields of the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> object in the dialog template's controls. It also supplies a default implementation for moving to the first, next, previous, or last record and an interface for updating the record currently on view. For more information, see <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
