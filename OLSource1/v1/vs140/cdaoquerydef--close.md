...
## Remarks  
 Closing the querydef releases the underlying DAO object but does not destroy the saved DAO querydef object or the C++ <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> object. This is not the same as [CDaoDatabase::DeleteQueryDef](../vs140/cdaodatabase--deletequerydef.md), which deletes the querydef from the database's QueryDefs collection in DAO (if not a temporary querydef).  
