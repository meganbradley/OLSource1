...
This article family covers enabling tool tips for controls contained in a window that is not derived from [CFrameWnd](../vs140/cframewnd-class.md). The article [Toolbars Tool Tips](../vs140/toolbar-tool-tips.md) provides information about tool tips for controls in a <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>.  
...
-   [The TOOLTIPTEXT Structure](../vs140/tooltiptext-structure.md)  
 ...
 Tool tips are automatically displayed for buttons and other controls contained in a parent window derived from <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>. This is because <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> has a default handler for the [TTN_GETDISPINFO](http://msdn.microsoft.com/library/windows/desktop/bb760269) notification, which handles **TTN_NEEDTEXT** notifications from tool tip controls associated with controls.  
  
 However, this default handler is not called when the **TTN_NEEDTEXT** notification is sent from a tool tip control associated with a control in a window that is not a <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, such as a control on a dialog box or a form view. Therefore, it is necessary for you to provide a handler function for the **TTN_NEEDTEXT** notification message in order to display tool tips for child controls.  
