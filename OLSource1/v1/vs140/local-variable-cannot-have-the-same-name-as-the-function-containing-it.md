---
title: "Local variable cannot have the same name as the function containing it"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "vbc30290"
  - "bc30290"
helpviewer_keywords: 
  - "BC30290"
ms.assetid: 34c38cff-fb9c-406d-82e8-ca251ee77104
caps.latest.revision: 14
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# Local variable cannot have the same name as the function containing it
A <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> statement specifies a variable with the same name as the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> procedure containing it.  
  
 **Error ID:** BC30290  
  
### To correct this error  
  
-   Remove the variable declaration, or change the name of the variable.  
  
## See Also  
 [NOTINBUILD: Resolving a Reference When Multiple Variables Have the Same Name](assetId:///9601e39f-1911-44e1-ace5-3f6e090408b9)   
 [Function Procedures](../vs140/function-procedures--visual-basic-.md)