...
## Rule Description  
 ...
 If an enumeration that has the \<xref:System.FlagsAttribute*> applied defines a zero-valued member, its name should be 'None' to indicate that no values have been set in the enumeration. Using a zero-valued member for any other purpose is contrary to the use of the \<xref:System.FlagsAttribute*> in that the AND and OR bitwise operators are useless with the member. This implies that only one member should be assigned the value zero. Note that if multiple members that have the value zero occur in a flags-attributed enumeration, <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> returns incorrect results for members that are not zero.  
...
## Example  
 The following example shows two enumerations that satisfy the rule and an enumeration, <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, that violates the rule.  
