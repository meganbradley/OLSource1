...
-   If hint text is used, verify that the color is set to <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> using the color service.  
  
-   If the field is a required field that must be identified as such, verify:  
  
    -   that the background is set to <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> and the foreground is set to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>  
