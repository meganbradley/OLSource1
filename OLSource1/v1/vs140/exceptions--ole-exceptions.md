...
The techniques and facilities for handling exceptions in OLE are the same as those for handling other exceptions. For further information on exception handling, see the article [C++ Exception Handling](../vs140/c---exception-handling.md).  
 ...
 All exception objects are derived from the abstract base class <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>. MFC provides two classes for handling OLE exceptions:  
...
-   [COleDispatchException](../vs140/coledispatchexception-class.md) For generating and handling OLE dispatch (automation) exceptions.  
 ...
 The difference between these two classes is the amount of information they provide and where they are used. <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> has a public data member that contains the OLE status code for the exception. <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> supplies more information, including the following:  
...
-   The name of the application that generated the exception  
 ...
 <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> provides more information so that it can be used with products like Microsoft Visual Basic. The verbal error description can be used in a message box or other notification; the Help information can be used to help the user respond to the conditions that caused the exception.  
