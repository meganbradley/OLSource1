...
## Query Syntax  
 ...
 Note that the type of the queries is <xref:System.Collections.Generic.IEnumerable`1?qualifyHint=False>. All of these queries could be written using `var` as shown in the following example:  
...
## Example  
 ...
 In the previous queries, only Query #4 executes immediately. This is because it returns a single value, and not a generic <xref:System.Collections.Generic.IEnumerable`1?qualifyHint=False> collection. The method itself has to use `foreach` in order to compute its value.  
