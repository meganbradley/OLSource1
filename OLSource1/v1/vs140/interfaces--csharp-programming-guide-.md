...
An interface contains definitions for a group of related functionalities that a [class](../vs140/class--csharp-reference-.md) or a [struct](../vs140/struct--csharp-reference-.md) can implement.  
 ...
 Any class or struct that implements the <xref:System.IEquatable<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>1.Equals*> method that matches the signature that the interface specifies. As a result, you can count on a class that implements <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> to contain an <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> method with which an instance of the class can determine whether it's equal to another instance of the same class.  
  
 The definition of <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> doesn’t provide an implementation for <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>. The interface defines only the signature. In that way, an interface in C# is similar to an abstract class in which all the methods are abstract. However, a class or struct can implement multiple interfaces, but a class can inherit only a single class, abstract or not. Therefore, by using interfaces, you can include behavior from multiple sources in a class.  
 ...
 The following example shows an implementation of the IEquatable\<T> interface. The implementing class, <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>, must provide an implementation of the <xref:System.IEquatable<CodeContentPlaceHolder>6\</CodeContentPlaceHolder>get<CodeContentPlaceHolder>7\</CodeContentPlaceHolder>class ClassName : InterfaceName`). If the interface is inherited because you inherited a base class that implements the interface, the base class provides the implementation of the members of the interface. However, the derived class can reimplement the interface members instead of using the inherited implementation.  
