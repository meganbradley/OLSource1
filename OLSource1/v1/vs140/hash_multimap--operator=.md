---
title: "hash_multimap::operator="
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "hash_multimap.operator="
  - "hash_map/stdext::hash_multimap::operator="
  - "stdext::hash_multimap::operator="
  - "="
  - "operator="
  - "stdext.hash_multimap.operator="
  - "hash_multimap::operator="
  - "std.hash_multimap.operator="
  - "std::hash_multimap::operator="
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "operator="
  - "= operator, with specific standard C++ objects"
ms.assetid: 0f1d365d-1e19-48e7-89a5-6370e781da39
caps.latest.revision: 13
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# hash_multimap::operator=
> [!NOTE]
>  This API is obsolete. The alternative is [unordered_multimap Class](../vs140/unordered_multimap-class.md).  
  
 Replaces the elements of the hash_multimap with a copy of another hash_multimap.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
#### Parameters  
  
|||  
|-|-|  
|Parameter|Description|  
|<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>|The [hash_multimap](../vs140/hash_multimap-class.md) being copied into the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>.|  
  
## Remarks  
 After erasing any existing elements in a <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> either copies or moves the contents of <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> into the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>.  
  
## Example  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
## Output  
  
<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>  
## Requirements  
 **Header:** \<hash_map>  
  
 **Namespace:** std  
  
## See Also  
 [hash_multimap Class](../vs140/hash_multimap-class.md)   
 [Standard Template Library](../vs140/standard-template-library.md)