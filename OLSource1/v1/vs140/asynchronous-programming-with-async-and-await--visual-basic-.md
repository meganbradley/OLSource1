...
2.  `AccessTheWebAsync` creates an <xref:System.Net.Http.HttpClient*> instance and calls the [GetStringAsync](assetId:///M:System.Net.Http.HttpClient.GetStringAsync(System.String)?qualifyHint=False&autoUpgrade=True) asynchronous method to download the contents of a website as a string.  
...
##  <a name="BKMK_APIAsyncMethods"></a> API Async Methods  
 You might be wondering where to find methods such as `GetStringAsync` that support async programming. The  .NET Framework 4.5 or higher contains many members that work with `Async` and `Await`. You can recognize these members by the "Async" suffix that’s attached to the member name and a return type of <xref:System.Threading.Tasks.Task*> or <xref:System.Threading.Tasks.Task`1*>. For example, the `System.IO.Stream` class contains methods such as [CopyToAsync](assetId:///M:System.IO.Stream.CopyToAsync(System.IO.Stream)?qualifyHint=False&autoUpgrade=True), [ReadAsync](assetId:///M:System.IO.Stream.ReadAsync(System.Byte[],System.Int32,System.Int32)?qualifyHint=False&autoUpgrade=True), and [WriteAsync](assetId:///M:System.IO.Stream.WriteAsync(System.Byte[],System.Int32,System.Int32)?qualifyHint=False&autoUpgrade=True) alongside the synchronous methods [CopyTo](assetId:///M:System.IO.Stream.CopyTo(System.IO.Stream)?qualifyHint=False&autoUpgrade=True), [Read](assetId:///M:System.IO.Stream.Read(System.Byte[],System.Int32,System.Int32)?qualifyHint=False&autoUpgrade=True), and [Write](assetId:///M:System.IO.Stream.Write(System.Byte[],System.Int32,System.Int32)?qualifyHint=False&autoUpgrade=True).  
...
##  <a name="BKMK_Threads"></a> Threads  
 ...
 The `Async` and `Await` keywords don't cause additional threads to be created. Async methods don't require multithreading because an async method doesn't run on its own thread. The method runs on the current synchronization context and uses time on the thread only when the method is active. You can use [Task.Run](assetId:///M:System.Threading.Tasks.Task.Run(System.Action)?qualifyHint=True&autoUpgrade=True) to move CPU-bound work to a background thread, but a background thread doesn't help with a process that's just waiting for results to become available.  
  
 The async-based approach to asynchronous programming is preferable to existing approaches in almost every case. In particular, this approach is better than <xref:System.ComponentModel.BackgroundWorker*> for IO-bound operations because the code is simpler and you don't have to guard against race conditions. In combination with assetId:///M:System.Threading.Tasks.Task.Run(System.Action)?qualifyHint=True&autoUpgrade=True, async programming is better than <xref:System.ComponentModel.BackgroundWorker*> for CPU-bound operations because async programming separates the coordination details of running your code from the work that `Task.Run` transfers to the threadpool.  
...
##  <a name="BKMK_ReturnTypesandParameters"></a> Return Types and Parameters  
 ...
 You specify <xref:System.Threading.Tasks.Task`1*> as the return type if the method contains a [Return](../vs140/return-statement--visual-basic-.md) statement that specifies an operand of type `TResult`.  
...
|[How to: Extend the Async Walkthrough by Using Task.WhenAll (Visual Basic)](../vs140/how-to--extend-the-async-walkthrough-by-using-task.whenall--visual-basic-.md)|Adds [Task.WhenAll](assetId:///M:System.Threading.Tasks.Task.WhenAll(System.Threading.Tasks.Task[])?qualifyHint=True&autoUpgrade=True) to the previous walkthrough. The use of `WhenAll` starts all the downloads at the same time.||  
...
|[Async Cancellation: Bridging between the .NET Framework and the Windows Runtime (Visual Basic)](../vs140/async-cancellation--bridging-between-the-.net-framework-and-the-windows-runtime--visual-basic-.md)|Shows how to bridge between Task types in the .NET Framework and IAsyncOperations in the [!INCLUDE[wrt](../vs140/includes/wrt_md.md)] so that you can use <xref:System.Threading.CancellationTokenSource*> with a [!INCLUDE[wrt](../vs140/includes/wrt_md.md)] method.|[Async Sample: Bridging between .NET and Windows Runtime (AsTask & Cancellation)](http://go.microsoft.com/fwlink/p/?LinkId=263004)|  
...
||||  
|[Task-based Asynchronous Pattern (TAP)](assetId:///8cef1fcf-6f9f-417c-b21f-3fd8bac75007)|Describes a new pattern for asynchrony in the .NET Framework. The pattern is based on the <xref:System.Threading.Tasks.Task*> and <xref:System.Threading.Tasks.Task`1*> types.||  
