...
## Parallel Asynchronous I/O  
 ...
 The example closes all <xref:System.IO.FileStream?qualifyHint=False> instances in a `Finally` block after the tasks are complete. If each `FileStream` was instead created in a `Imports` statement, the `FileStream` might be disposed of before the task was complete.  
