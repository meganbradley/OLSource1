---
title: "Compiler Error C2928"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "C2928"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "C2928"
ms.assetid: 869e57f4-7024-4cbe-b47b-6e1e2a6005c5
caps.latest.revision: 11
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# Compiler Error C2928
explicit instantiation; 'identifier' is not a function or static data member of template-class 'class'  
  
 You cannot explicitly instantiate a member of <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> that is not a function or <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> variable.