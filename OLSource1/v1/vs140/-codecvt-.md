...
---
# &lt;codecvt&gt;
Defines several template classes that describe objects based on template class [codecvt](../vs140/codecvt-class.md). These objects can serve as [locale facets](../vs140/locale-class.md#facet_class) that control conversions between a sequence of values of type <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> and a sequence of values of type <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
...
-   UTF-16 is Unicode encoded within the program as either one or two 16-bit integers. (Note that this does not meet all the requirements of a valid wide-character encoding for Standard C or Standard C++. Nevertheless it is widely used as such.)  
 ...
 For byte streams (stored in a file, transmitted as a byte sequence, or stored within the program in an array of <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>):  
