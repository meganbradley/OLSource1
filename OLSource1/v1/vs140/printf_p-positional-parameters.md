---
title: "printf_p Positional Parameters"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "article"
apilocation: 
  - "msvcr120.dll"
  - "msvcr110.dll"
  - "msvcr110_clr0400.dll"
  - "msvcr90.dll"
  - "msvcr80.dll"
  - "msvcr100.dll"
apitype: "DLLExport"
dev_langs: 
  - "C++"
  - "C"
helpviewer_keywords: 
  - "_printf_p function, positional parameters"
  - "printf_p function, positional parameters"
ms.assetid: beb4fd85-a7aa-4665-9085-2c907a5b9ab0
caps.latest.revision: 19
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# printf_p Positional Parameters
Positional parameters provide the ability to specify by number which of the arguments is to be substituted into a field in a format string. The following positional parameter <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> functions are available:  
  
 [printf, wprintf](../vs140/printf--_printf_l--wprintf--_wprintf_l.md)  
 [_printf_p, _wprintf_p](../vs140/_printf_p--_printf_p_l--_wprintf_p--_wprintf_p_l.md)  
  
 [sprintf, swprintf](../vs140/sprintf--_sprintf_l--swprintf--_swprintf_l--__swprintf_l.md)  
 [_sprintf_p, _swprintf_p](../vs140/_sprintf_p--_sprintf_p_l--_swprintf_p--_swprintf_p_l.md)  
  
 [_cprintf, _cwprintf](../vs140/_cprintf--_cprintf_l--_cwprintf--_cwprintf_l.md)  
 [_cprintf_p, _cwprintf_p](../vs140/_cprintf_p--_cprintf_p_l--_cwprintf_p--_cwprintf_p_l.md)  
  
 [fprintf, fwprintf](../vs140/fprintf--_fprintf_l--fwprintf--_fwprintf_l.md)  
 [_fprintf_p, _fwprintf_p](../vs140/_fprintf_p--_fprintf_p_l--_fwprintf_p--_fwprintf_p_l.md)  
  
 [vprintf, vwprintf](../vs140/vprintf--_vprintf_l--vwprintf--_vwprintf_l.md)  
 [_vprintf_p, _vwprintf_p](../vs140/_vprintf_p--_vprintf_p_l--_vwprintf_p--_vwprintf_p_l.md)  
  
 [vfprintf, vfwprintf](../vs140/vfprintf--_vfprintf_l--vfwprintf--_vfwprintf_l.md)  
 [_vfprintf_p, _vfwprintf_p](../vs140/_vfprintf_p--_vfprintf_p_l--_vfwprintf_p--_vfwprintf_p_l.md)  
  
 [vsprintf, vswprintf](../vs140/vsprintf--_vsprintf_l--vswprintf--_vswprintf_l--__vswprintf_l.md)  
 [_vsprintf_p, _vswprintf_p](../vs140/_vsprintf_p--_vsprintf_p_l--_vswprintf_p--_vswprintf_p_l.md)  
  
## Specifying Positional Parameters  
  
##### Parameter Indexing  
 By default the positional functions behave identically to the non position ones, if no positional formatting is present. Positional parameters are specified using the format "<CodeContentPlaceHolder>10\</CodeContentPlaceHolder>", where <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> denotes a numeric ordinal number indicating the position of the parameter in the list of parameters, preceding the format string and <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> denotes the type field character type specified in the <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> function. The parameters in the list are indexed starting at the value 1 for the first element in the list and so forth. For additional information concerning type field characters, see [printf Type Field Characters](../vs140/printf-type-field-characters.md).  
  
 For an example of this behavior:  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 will print  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 The order of the numbers used need not match the order of the arguments given. Thus the following is valid:  
  
<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>  
 will print  
  
<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>  
 Parameter may be used more than once while formatting, unlike in traditional format strings, so that  
  
<CodeContentPlaceHolder>4\</CodeContentPlaceHolder>  
 will print  
  
<CodeContentPlaceHolder>5\</CodeContentPlaceHolder>  
 However, all arguments must be used at least once somewhere in the format string.  
  
 The maximum number of positional parameters allowed in a format string is given by <CodeContentPlaceHolder>14\</CodeContentPlaceHolder>.  
  
##### Width and Precision  
 When the * symbol is used to specify that the width or precision is to be determined from an argument, then the position of the width or precision value must appear immediately following the \* symbol. For example,  
  
<CodeContentPlaceHolder>6\</CodeContentPlaceHolder>  
 or  
  
<CodeContentPlaceHolder>7\</CodeContentPlaceHolder>  
##### Mixing positional and non positional arguments  
 Positional parameters may not be mixed with non-positional parameters in the same format string.  However, <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> and related functions still support non-positional parameters in format strings containing no positional parameters.  
  
## Example  
  
<CodeContentPlaceHolder>8\</CodeContentPlaceHolder>  
 **1 2 3**  
**3 1 2**  
**1 2 1**  
**abc def ghi**  
**ghi abc def**   
## See Also  
 [printf Type Field Characters](../vs140/printf-type-field-characters.md)   
 [printf Width Specification](../vs140/printf-width-specification.md)   
 [Precision Specification](../vs140/precision-specification.md)