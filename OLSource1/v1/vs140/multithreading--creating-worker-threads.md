...
-   [Example](#_core_controlling_function_example)  
 ...
 Creating a worker thread is a relatively simple task. Only two steps are required to get your thread running: implementing the controlling function and starting the thread. It is not necessary to derive a class from [CWinThread](../vs140/cwinthread-class.md). You can derive a class if you need a special version of <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, but it is not required for most simple worker threads. You can use <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> without modification.  
  
##  \<a name="_core_starting_the_thread">\</a> Starting the Thread  
 There are two overloaded versions of <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>: one that can only create worker threads, and one that can create both user-interface threads and worker threads. To begin execution of your worker thread using the first overload, call [AfxBeginThread](../vs140/afxbeginthread.md), providing the following information:  
