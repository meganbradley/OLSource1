...
-   The control is created. If the control is specified in a dialog box template or if you're using <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, creation is automatic when the dialog box or view is created. If you want to create the tree control as a child window of some other window, use the [Create](../vs140/ctreectrl--create.md) member function.  
...
-   Put data into the control by calling the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>'s [InsertItem](../vs140/ctreectrl--insertitem.md) function once for each data item. <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> returns a handle to the item you can use to refer to it later, such as when adding child items. A good time to initialize the data is in <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> (for controls in dialog boxes) or <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> (for views).  
  
-   As the user interacts with the control, it will send various notification messages. You can specify a function to handle each of the messages you want to handle by adding an **ON_NOTIFY_REFLECT** macro in your control window's message map or by adding an <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> macro to your parent window's message map. See [Tree Control Notification Messages](../vs140/tree-control-notification-messages.md) later in this topic for a list of possible notifications.  
...
-   When you're done with the control, make sure it's properly destroyed. If the tree control is in a dialog box or if it's a view, it and the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> object will be destroyed automatically. If not, you need to ensure that both the control and the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> object are properly destroyed.  
