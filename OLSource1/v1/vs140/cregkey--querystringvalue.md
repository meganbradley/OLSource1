...
#### Parameters  
 ...
 The size, in TCHARs, of the buffer pointed to by <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>. When the method returns, <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> contains the size, in TCHARs, of the string retrieved, including a terminating null character.  
  
## Return Value  
 If the method succeeds, ERROR_SUCCESS is returned. If the method fails to read a value, it returns a nonzero error code defined in WINERROR.H. If the data referenced is not of type REG_SZ, ERROR_INVALID_DATA is returned. If the method returns ERROR_MORE_DATA, <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> equals zero, not the required buffer size in bytes.  
