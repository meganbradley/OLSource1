...
-   Exists when a program has stopped at a breakpoint. The expression itself is a data structure representing a parsed expression that is ready for binding and evaluating within the given context.  
     ...
     Given a <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> and an [IDebugExpressionContext2](../vs140/idebugexpressioncontext2.md) interface, a debug engine (DE) can create an [IDebugExpression2](../vs140/idebugexpression2.md) interface by parsing an expression. Given an <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> interface, the DE can get a value through synchronous or asynchronous expression evaluation. This value, along with the name and type of the variable or argument, is sent to the IDE for display.  
