...
## Example  
 In the following example, a pointer to <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> is converted to a pointer to <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>. Notice that the pointer points to the lowest addressed byte of the variable. When you successively increment the result, up to the size of <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> (4 bytes), you can display the remaining bytes of the variable.  
