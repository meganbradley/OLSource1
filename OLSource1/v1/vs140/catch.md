...
## Remarks  
 The exception-processing code can interrogate the exception object, if appropriate, to get more information about the specific cause of the exception. Invoke the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> macro to shift processing to the next outer exception frame. End the **TRY** block with an <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> macro.  
  
 If *exception_class* is the class <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, then all exception types will be caught. You can use the [CObject::IsKindOf](../vs140/cobject--iskindof.md) member function to determine which specific exception was thrown. A better way to catch several kinds of exceptions is to use sequential <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> statements, each with a different exception type.  
...
## Example  
 [!code[NVC_MFCExceptions#26](../vs140/codesnippet/CPP/catch_1.cpp)]  
