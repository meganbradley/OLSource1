...
## Porting UNIX applications directly to Win32  
 ...
 Basic UNIX applications, including many CGI applications, should port easily to Visual C++ running on Windows. Functions like **open**, <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, **read**, **write** and others are available in the Visual C++ run-time library. Also, there is a one-to-one mapping between C UNIX APIs and Win32 APIs: **open** to **CreateFile**, **read** to **ReadFile**, **write** to **WriteFile**, <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> to **DeviceIOControl**, **close** to **CloseFile**, and so on.  
