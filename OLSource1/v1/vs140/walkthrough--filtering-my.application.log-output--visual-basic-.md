...
3.  In the <xref:System.Windows.Forms.Control.Click?qualifyHint=False> event handler for Button1, add the following code:  
...
6.  Close the application.  
    ...
    >  By default, the application flushes the log-file output when the application closes.  
     ...
     In the example above, the second call to the <xref:Microsoft.VisualBasic.Logging.Log.WriteEntry?qualifyHint=False> method and the call to the <xref:Microsoft.VisualBasic.Logging.Log.WriteException?qualifyHint=False> method produces log output, while the first and last calls to the `WriteEntry` method do not. This is because the severity levels of `WriteEntry` and `WriteException` are "Information" and "Error", both of which are allowed by the `My.Application.Log` object's default log filtering. However, events with "Start" and "Stop" severity levels are prevented from producing log output.  
  
## Filtering for All My.Application.Log Listeners  
 The `My.Application.Log` object uses a <xref:System.Diagnostics.SourceSwitch?qualifyHint=False> named `DefaultSwitch` to control which messages it passes from the `WriteEntry` and `WriteException` methods to the log listeners. You can configure `DefaultSwitch` in the application's configuration file by setting its value to one of the <xref:System.Diagnostics.SourceLevels?qualifyHint=False> enumeration values. By default, its value is "Information".  
...
9. Change the value of the `value` attribute back to "Information".  
    ...
    > [!NOTE]
    >  The `DefaultSwitch` switch setting controls only `My.Application.Log`. It does not change how the [!INCLUDE[dnprdnshort](../vs140/includes/dnprdnshort_md.md)] <xref:System.Diagnostics.Trace?qualifyHint=True> and <xref:System.Diagnostics.Debug?qualifyHint=True> classes behave.  
...
6.  Add this element to that `<sharedListeners>` section:  
    ...
    ```  
     ...
     The <xref:System.Diagnostics.EventTypeFilter?qualifyHint=False> filter takes one of the <xref:System.Diagnostics.SourceLevels?qualifyHint=False> enumeration values as its `initializeData` attribute.  
