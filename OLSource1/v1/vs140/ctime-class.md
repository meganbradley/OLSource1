...
|[CTime::GetAsDBTIMESTAMP](#ctime__getasdbtimestamp)|Converts the time information stored in the                                         `CTime` object to a Win32-compatible                                         <xref:System.Data.OleDb.OleDbType.DBTimeStamp?qualifyHint=False> structure.|  
...
## Remarks  
 ...
 `CTime` values are based on coordinated universal time (UTC), which is equivalent to Coordinated Universal time (Greenwich Mean Time, GMT). See                 [Time Management](../vs140/time-management.md) for information about how the time zone is determined.  
  
 When you create a                 `CTime` object, set the                 `nDST` parameter to 0 to indicate that standard time is in effect, or to a value larger than 0 to indicate that daylight saving time is in effect, or to a value less than zero to have the C run-time library code compute whether standard time or daylight saving time is in effect.                 `tm_isdst` is a required field. If not set, its value is undefined and the return value from                 [mktime](../vs140/mktime--_mktime32--_mktime64.md) is unpredictable. If                 `timeptr` points to a tm structure returned by a previous call to                 [asctime_s](../vs140/asctime_s--_wasctime_s.md),                 [_gmtime_s](../vs140/gmtime_s--_gmtime32_s--_gmtime64_s.md), or                 [localtime_s](../vs140/localtime_s--_localtime32_s--_localtime64_s.md), the                 `tm_isdst` field contains the correct value.  
...
>  The upper date limit is 12/31/3000. The lower limit is 1/1/1970 12:00:00 AM GMT.  
 ...
 For more information about using                 `CTime`, see the articles                 [Date and Time](../vs140/date-and-time.md), and                 [Time Management](../vs140/time-management.md) in the Run-Time Library Reference.  
