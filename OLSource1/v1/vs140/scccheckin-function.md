...
#### Parameters  
 ...
 lpComment  
 [in] Comment to be applied to each of the selected files being checked in. This is <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> if the source control plug-in should prompt for a comment.  
  
 fOptions  
 [in] Command flags, either 0 or <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
...
|SCC_E_CHECKINCONFLICT|Checkin could not be performed because:\<br />\<br /> -   Another user has checked in ahead and <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> was false.\<br />\<br /> -or-\<br />\<br /> -   The auto-merge cannot be done (for example, when files are binary).|  
...
## Remarks  
 The comment applies to all files being checked in. The comment argument can be a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> string, in which case the source control plug-in can prompt the user for a comment string for each file.  
  
 The <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> argument can be given a value of the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> flag to indicate the user's intent to check the file in and check it out again.  
