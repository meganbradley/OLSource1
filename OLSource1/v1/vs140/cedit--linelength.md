...
## Return Value  
 For single-line edit controls, the return value is the length, in <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>s, of the text in the edit control.  
  
 For multiline edit controls, the return value is the length, in <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>s, of the line specified by the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> parameter. For [!INCLUDE[vcpransi](../vs140/includes/vcpransi_md.md)] text, the length is the number of bytes in the line; for Unicode text, the length is the number of characters in the line. The length does not include the carriage-return character at the end of the line.  
  
 If the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> parameter is more than the number of characters in the control, the return value is zero.  
  
 If the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> parameter is –1, the return value is the number of unselected characters in the lines that contain selected characters. For example, if the selection extends from the fourth character of one line through the eighth character from the end of the next line, the return value is 10. That is, three characters on the first line and seven on the next.  
  
 For more information about the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> type, see the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> row in the table in [Windows Data Types](http://msdn.microsoft.com/library/windows/desktop/aa383751).  
