...
## LIB Command Line  
 To run LIB, type the command <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> followed by the options and file names for the task you are using LIB to perform. LIB also accepts command-line input in command files, which are described in the following section. LIB does not use an environment variable.  
  
> [!NOTE]
>  If you are accustomed to the LINK32.exe and LIB32.exe tools provided with the Microsoft Win32 Software Development Kit for Windows NT, you may have been using either the command <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> or the command <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> for managing libraries and creating import libraries. Be sure to change your makefiles and batch files to use the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> command instead.  
  
## LIB Command Files  
 You can pass command-line arguments to LIB in a command file using the following syntax:  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 The file <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> is a text file. No space or tab is allowed between the at sign (@) and the file name. There is no default extension; you must specify the full file name, including any extension. Wildcards cannot be used. You can specify an absolute or relative path with the file name.  
...
## Using LIB Options  
 ...
 /MACHINE  
 Specifies the target platform for the program. Usually, you do not need to specify /MACHINE. LIB infers the machine type from the .obj files. However, in some circumstances, LIB cannot determine the machine type and issues an error message. If such an error occurs, specify /MACHINE. In /EXTRACT mode, this option is for verification only. Use <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> at the command line to see available machine types.  
 ...
 /WX[:NO]  
 Treat warnings as errors. See [/WX (Treat Linker Warnings as Errors)](../vs140/-wx--treat-linker-warnings-as-errors-.md) for more information.  
