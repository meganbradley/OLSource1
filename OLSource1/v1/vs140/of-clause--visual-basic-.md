...
## Using the Of Keyword  
 The following code example uses the `Of` keyword to define the outline of a class that takes two type parameters. It *constrains* the `keyType` parameter by the <xref:System.IComparable*> interface, which means the consuming code must supply a type argument that implements <xref:System.IComparable*>. This is necessary so that the `add` procedure can call the [IComparable.CompareTo](assetId:///M:System.IComparable.CompareTo(System.Object)?qualifyHint=True&autoUpgrade=True) method. For more information on constraints, see [Type List](../vs140/type-list--visual-basic-.md).  
