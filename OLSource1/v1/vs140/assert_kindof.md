...
#### Parameters  
 *classname*  
 The name of a <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>-derived class.  
  
 *pobject*  
 A pointer to a class object.  
  
## Remarks  
 The *pobject* parameter should be a pointer to an object and can be **const**. The object pointed to and the class must support <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> run-time class information. As an example, to ensure that <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is a pointer to an object of the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> class, or any of its derivatives, you could code:  
  
 [!code[NVC_MFCDocView#194](../vs140/codesnippet/CPP/assert_kindof_1.cpp)]  
  
 Using the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> macro is exactly the same as coding:  
