...
#### Parameters  
 ...
 lpAuxProjPath  
 [in, out] Auxiliary string identifying the project (up to <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>SIZE, including the null terminator).  
  
 cFiles  
 [in] Number of files given by <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
 ...
 pbResults  
 [in, out] Array of flags that are set to indicate success (nonzero or TRUE) or failure (zero or FALSE) for each file (size of the array must be at least <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> long).  
...
|SCC_E_OPNOTPERFORMED|Connection is not to the same project as specified by <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>|  
