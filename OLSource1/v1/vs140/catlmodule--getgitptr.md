...
## Return Value  
 Returns S_OK on success, or an error code on failure. E_POINTER is returned if <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> is equal to NULL.  
...
## Remarks  
 ...
 In debug builds, an assertion error will occur if <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is equal to NULL, or if the Global Interface Table pointer cannot be obtained.  
