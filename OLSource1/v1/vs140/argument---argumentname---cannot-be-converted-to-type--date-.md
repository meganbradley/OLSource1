---
title: "Argument &#39;&lt;argumentname&gt;&#39; cannot be converted to type &#39;Date&#39;"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "vbrArgument_InvalidDateValue1"
ms.assetid: 928b8d75-f523-40c3-819a-03a1eb596ccc
caps.latest.revision: 14
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# Argument &#39;&lt;argumentname&gt;&#39; cannot be converted to type &#39;Date&#39;
A conversion attempted to change a variable to type <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, but the value is not recognized as a valid date.  
  
### To correct this error  
  
-   Find a conversion that is meaningful.  
  
## See Also  
 [Type Conversions in Visual Basic](../vs140/type-conversions-in-visual-basic.md)   
 [Date Data Type](../vs140/date-data-type--visual-basic-.md)