...
4.  In the list of available assemblies, choose the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> assemblies, and then choose the **Add** button to add references to these assemblies to your project.  
...
##  \<a name="BKMK_StoredProc">\</a> Calling a stored procedure  
 You can call stored procedures by using the EDMX type provider. In the following procedure, the School database contains a stored procedure, <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>, which updates a record, given new values for the columns. You can use this stored procedure because it's exposed as a method on the DataContext type.  
...
<CodeContentPlaceHolder>5\</CodeContentPlaceHolder>  
     The result is 1 if you succeed. Notice that <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> is used in the query expression to ensure that only one result is returned; otherwise, an exception is thrown. Also, to work with nullable values more easily, you can use the simple <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> function that's defined in this code to create a nullable value out of an ordinary value.  
...
7.  On the next page, choose your newly created School database as the data connection.  
     ...
     This connection should resemble <CodeContentPlaceHolder>11\</CodeContentPlaceHolder>.  
  
8.  Copy your entity connection string to the Clipboard because that string might be important later.  
  
9. Make sure the check box to save the entity connection string to the <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> file is selected, and make note of the string value in the text box, which should help you locate the connection string later, if needed.  
...
12. Select the **Finish** button to generate an .edmx file that contains an Entity Data Model that's based on the School database.  
     ...
     A file, <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>, is added to your project, and a database diagram appears.  
