...
The XML Editor allows you to associate an XSLT style sheet with an XML document, perform the transformation, and view the output. The resulting output from the XSLT transformation is displayed in a new document window.  
 ...
 The **Output** property specifies the filename for the output. If the **Output** property is blank, a filename is generated in your temporary directory. The file extension is based on the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> element in your style sheet and can be .xml, .txt or .htm.  
...
2.  Associate an XSLT style sheet with the XML document.  
    ...
    -   Add an <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> processing instruction to the XML document. For example, add the following line <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> to the document prolog.  
