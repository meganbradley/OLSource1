...
1.  Use the Visual Studio [Code and Text Editor](assetId:///508e1f18-99d5-48ad-b5ad-d011b21c6ab1) or Notepad.exe to create a text-only file named <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>.  
  
2.  In <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, type each user-defined keyword on a separate line.  
  
3.  Save <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>in the directory that contains devenv.exe. By default, the path of that directory is *\<drive>*:\Program Files\\[!INCLUDE[TLA#tla_visualstu](../vs140/includes/tlasharptla_visualstu_md.md)] *\<major.minor version number>*\Common7\\[!INCLUDE[TLA2#tla_ide](../vs140/includes/tla2sharptla_ide_md.md)]. Because that directory is read-only by default, you need administrative credentials to save <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>.  
  
4.  Exit Visual Studio, and then restart it.  
  
    > [!NOTE]
    >  You cannot rename or reload the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> file during an editing session because it is read during initialization. All previously defined color settings take precedence because the syntax coloring mechanism checks the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> file last.  
