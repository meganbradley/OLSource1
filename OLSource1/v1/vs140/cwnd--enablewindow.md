...
## Return Value  
 Indicates the state before the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> member function was called. The return value is nonzero if the window was previously disabled. The return value is 0 if the window was previously enabled or an error occurred.  
...
## Remarks  
 ...
 If disabled, all child windows are implicitly disabled, although they are not sent <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> messages.  
 ...
 By default, a window is enabled when it is created. An application can specify the **WS_DISABLED** style in the [Create](../vs140/cwnd--create.md) or [CreateEx](../vs140/cwnd--createex.md) member function to create a window that is initially disabled. After a window has been created, an application can also use the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> member function to enable or disable the window.  
