...
##  \<a name="top">\</a> NEON  
 ...
 NEON intrinsics are supported, as provided in the header file <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>. The Visual C++ compiler support for NEON intrinsics resembles that of the ARM compiler, which is documented in Appendix G of the [ARM Compiler toolchain, Version 4.1 Compiler Reference](http://go.microsoft.com/fwlink/p/?LinkId=251083) on the ARM Infocenter website.  
  
 The primary difference between the Visual C++ compiler and the ARM compiler is that the Visual C++ compiler adds <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> variants of the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> vector load and store instructions. The <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> variants take an additional parameter that specifies the alignment of the pointer argument but are otherwise identical to their non-<CodeContentPlaceHolder>11\</CodeContentPlaceHolder> counterparts.  
...
|__dmb|DMB|void __dmb(unsigned int <CodeContentPlaceHolder>12\</CodeContentPlaceHolder>)\<br />\<br /> Inserts a memory barrier operation into the instruction stream. The parameter <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> specifies the kind of restriction that the barrier enforces.\<br />\<br /> For more information about the kinds of restrictions that can be enforced, see [Memory Barrier Restrictions](#BarrierRestrictions).|  
|__dsb|DSB|void __dsb(unsigned int _Type)\<br />\<br /> Inserts a memory barrier operation into the instruction stream. The parameter <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> specifies the kind of restriction that the barrier enforces.\<br />\<br /> For more information about the kinds of restrictions that can be enforced, see [Memory Barrier Restrictions](#BarrierRestrictions).|  
|__isb|ISB|void __isb(unsigned int _Type)\<br />\<br /> Inserts a memory barrier operation into the instruction stream. The parameter <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> specifies the kind of restriction that the barrier enforces.\<br />\<br /> For more information about the kinds of restrictions that can be enforced, see [Memory Barrier Restrictions](#BarrierRestrictions).|  
|__emit||void __emit(unsigned \__int32 opcode)\<br />\<br /> Inserts a specified instruction into the stream of instructions that is output by the compiler.\<br />\<br /> The value of <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> must be a constant expression that is known at compile time. The size of an instruction word is 16 bits and the most significant 16 bits of <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> are ignored.\<br />\<br /> The compiler makes no attempt to interpret the contents of <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> and does not guarantee a CPU or memory state before the inserted instruction is executed.\<br />\<br /> The compiler assumes that the CPU and memory states are unchanged after the inserted instruction is executed. Therefore, instructions that do change state can have a detrimental impact on normal code that is generated by the compiler.\<br />\<br /> For this reason, use <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> only to insert instructions that affect a CPU state that the compiler does not normally process—for example, coprocessor state—or to implement functions that are declared by using <CodeContentPlaceHolder>20\</CodeContentPlaceHolder>.|  
...
|__prefetch|PLD|void __cdecl \__prefetch(const void *)\<br />\<br /> Provides a <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> memory hint to the system that memory at or near the specified address may be accessed soon. Some systems may choose to optimize for that memory access pattern to increase runtime performance. However, from the C++ language point of view, the function has no observable effect, and may do nothing at all.|  
...
###  \<a name="BarrierRestrictions">\</a> Memory Barrier Restrictions  
 The intrinsic functions <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> (data memory barrier), <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> (data synchronization barrier), and <CodeContentPlaceHolder>24\</CodeContentPlaceHolder> (instruction synchronization barrier) use the following predefined values to specify the memory barrier restriction in terms of the sharing domain and kind of access that are affected by the operation.  
...
|_ARM_BARRIER_OSHST|Outer sharable, writes only.|  
 ...
 For the <CodeContentPlaceHolder>25\</CodeContentPlaceHolder> intrinsic, the only restriction that is currently valid is _ARM_BARRIER_SY; all other values are reserved by the architecture.  
 ...
 **Remarks**  
 ...
 <CodeContentPlaceHolder>41\</CodeContentPlaceHolder> uses the MRRC instruction. The parameters correspond to bitfields that are encoded directly into the instruction word. The interpretation of the parameters is coprocessor-dependent. For more information, see the manual for the coprocessor in question.  
 ...
 **Remarks**  
 ...
 The values of the <CodeContentPlaceHolder>48\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>49\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>50\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>51\</CodeContentPlaceHolder>, and <CodeContentPlaceHolder>52\</CodeContentPlaceHolder> parameters of this intrinsic must be constant expressions that are known at compile time.  
  
 <CodeContentPlaceHolder>53\</CodeContentPlaceHolder> uses the MCR instruction; <CodeContentPlaceHolder>54\</CodeContentPlaceHolder> uses MCR2. The parameters correspond to bitfields that are encoded directly into the instruction word. The interpretation of the parameters is coprocessor-dependent. For more information, see the manual for the coprocessor in question.  
 ...
 **Remarks**  
 ...
 The values of the <CodeContentPlaceHolder>58\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>59\</CodeContentPlaceHolder>, and <CodeContentPlaceHolder>60\</CodeContentPlaceHolder> parameters of this intrinsic must be constant expressions that are known at compile time.  
  
 <CodeContentPlaceHolder>61\</CodeContentPlaceHolder> uses the MCRR instruction. The parameters correspond to bitfields that are encoded directly into the instruction word. The interpretation of the parameters is coprocessor-dependent. For more information, see the manual for the coprocessor in question.  
...
|__nop|void __nop(void) **Note:**  On ARM platforms, this function generates a NOP instruction if one is implemented in the target architecture; otherwise, an alternative instruction that does not change the state of the program or CPU is generated—for example, <CodeContentPlaceHolder>62\</CodeContentPlaceHolder>. This is functionally equivalent to the \__nop intrinsic for other hardware architectures. Because an instruction that has no effect on the state of the program or CPU might be ignored by the target architecture as an optimization, the instruction does not necessarily consume CPU cycles. Therefore, do not use the \__nop intrinsic to manipulate the execution time of a code sequence unless you are certain about how the CPU will behave. Instead, you can use the \__nop intrinsic to align the next instruction to a specific 32-bit boundary address.|  
...
## Interlocked Intrinsics  
 ...
 The following table summarizes the ARM support of the non-bittest interlocked intrinsics. Each cell in the table corresponds to a name that is derived by appending the operation name in the left-most cell of the row and the type name in the top-most cell of the column to <CodeContentPlaceHolder>63\</CodeContentPlaceHolder>. For example, the cell at the intersection of the <CodeContentPlaceHolder>64\</CodeContentPlaceHolder> row and the **8** column corresponds to <CodeContentPlaceHolder>65\</CodeContentPlaceHolder> and is fully supported. Most of the supported functions offer these optional suffixes: <CodeContentPlaceHolder>66\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>67\</CodeContentPlaceHolder>, and <CodeContentPlaceHolder>68\</CodeContentPlaceHolder>. The <CodeContentPlaceHolder>69\</CodeContentPlaceHolder> suffix indicates an "acquire" semantic and the <CodeContentPlaceHolder>70\</CodeContentPlaceHolder> suffix indicates a "release" semantic. The <CodeContentPlaceHolder>71\</CodeContentPlaceHolder> or “no fence” suffix is unique to ARM and is discussed in the next section.  
...
-   **Full**: supports plain, <CodeContentPlaceHolder>72\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>73\</CodeContentPlaceHolder>, and <CodeContentPlaceHolder>74\</CodeContentPlaceHolder> forms.  
  
-   **Partial**: supports plain, <CodeContentPlaceHolder>75\</CodeContentPlaceHolder>, and <CodeContentPlaceHolder>76\</CodeContentPlaceHolder> forms.  
  
-   **None**: Not supported  
  
###  \<a name="nf_suffix">\</a> _nf (no fence) Suffix  
 The <CodeContentPlaceHolder>77\</CodeContentPlaceHolder> or "no fence" suffix indicates that the operation does not behave as any kind of memory barrier. This is in contrast to the other three forms (plain, <CodeContentPlaceHolder>78\</CodeContentPlaceHolder>, and <CodeContentPlaceHolder>79\</CodeContentPlaceHolder>), which all behave as some kind of barrier. One possible use of the <CodeContentPlaceHolder>80\</CodeContentPlaceHolder> forms is to maintain a statistic counter that is updated by multiple threads at the same time but whose value is not otherwise used while multiple threads are executing.  
...
### _interlockedbittest Intrinsics  
 The plain interlocked bittest intrinsics are common to all platforms. ARM adds <CodeContentPlaceHolder>81\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>82\</CodeContentPlaceHolder>, and <CodeContentPlaceHolder>83\</CodeContentPlaceHolder> variants, which just modify the barrier semantics of an operation, as described in [_nf (no fence) Suffix](#nf_suffix) earlier in this article.  
