---
title: "CMFCDisableMenuAnimation::Restore"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "reference"
f1_keywords: 
  - "Restore"
  - "CMFCDisableMenuAnimation::Restore"
  - "CMFCDisableMenuAnimation.Restore"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "Restore method"
ms.assetid: e749adad-ec87-41a7-a120-3435a3930953
caps.latest.revision: 13
robots: noindex,nofollow
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# CMFCDisableMenuAnimation::Restore
Restores the previous animation that the framework used to display a pop-up menu.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Remarks  
 This method is called by the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> destructor to restore the previous animation that the framework used to display a pop-up menu.  
  
## Requirements  
 **Header:** afxpopupmenu.h  
  
## See Also  
 [CMFCDisableMenuAnimation Class](../vs140/cmfcdisablemenuanimation-class.md)   
 [Hierarchy Chart (MFC Feature Pack)](../vs140/hierarchy-chart.md)