...
|**Enable Created/Modified properties**|When selected, adds four fields to the entity to identify the user who created or most recently updated the record and the time when the record was created or most recently updated. The time is stored as a <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> data type. See [How to: Provide an Audit Trail](../vs140/how-to--implement-row-tracking.md). **Note:**  The **Enable Created/Modified properties** property appears for tables in the intrinsic database only.|  
...
|**Require Email Domain**|Applies to the **EmailAddress** business type. Indicates whether the user must provide an email domain as part of an email address that they type into a field of type <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.|  
