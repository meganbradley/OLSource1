...
## Rule Description  
 CSE indicates that the state of a process has been corrupted and not caught by the system. In the corrupted state scenario, a general handler only catches the exception if you mark your method with the proper <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> attribute. By default, the [Common Language Runtime (CLR)](https://msdn.microsoft.com/en-us/library/8bs2ecf4.aspx) will not invoke catch handlers for CSEs.  
...
## How to Fix Violations  
 ...
 1. Remove the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> attribute. This reverts to the default runtime behavior where CSEs are not passed to catch handlers.  
