---
title: "Significant Characters with External Linkage"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "article"
dev_langs: 
  - "C++"
  - "C"
ms.assetid: 6f3b7e26-3fb0-4975-a95d-23a2072c1186
caps.latest.revision: 10
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# Significant Characters with External Linkage
**ANSI 3.1.2** The number of significant characters with external linkage  
  
 Identifiers declared <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> in programs compiled with Microsoft C are significant to 247 characters. You can modify this default to a smaller number using the /H (restrict length of external names) option.  
  
## See Also  
 [Using extern to Specify Linkage](../vs140/using-extern-to-specify-linkage.md)