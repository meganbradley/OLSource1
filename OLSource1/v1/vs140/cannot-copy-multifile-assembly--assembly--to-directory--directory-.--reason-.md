...
Multifile assemblies are copied to a subdirectory of the project directory from which they will be run. For example, if the output path is c:\project1\bin, and there exists an assembly (whose <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> property is <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>) named Test containing files a.dll and b.dll, you will have the following file structure after the copy is complete:  
