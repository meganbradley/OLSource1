...
1.  From a pointer to one of the interfaces on \<xref:Microsoft.VisualStudio.TextManager.Interop.VsTextBuffer*>, call <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> for a pointer to \<xref:Microsoft.VisualStudio.OLE.Interop.IConnectionPointContainer*>.  
...
3.  Using this pointer, call the \<xref:Microsoft.VisualStudio.OLE.Interop.IConnectionPoint.Advise*> method, passing in a pointer to your implementation of the events interface for which you want to register, for example, the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> interface.  
