---
title: "&#39;RaiseEvent&#39; statements are not valid in the Immediate window"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "bc30739"
  - "vbc30739"
helpviewer_keywords: 
  - "BC30739"
ms.assetid: 275a5a79-ab57-4a76-9403-43f3e74fcc6c
caps.latest.revision: 13
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# &#39;RaiseEvent&#39; statements are not valid in the Immediate window
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder> statements cannot be used in the **Immediate** window.  
  
 **Error ID:** BC30739  
  
### To correct this error  
  
-   Test <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> statements from code in the **Code Editor**.  
  
## See Also  
 [RaiseEvent Statement](../vs140/raiseevent-statement.md)   
 [Debugging in Visual Studio](../vs140/debugging-in-visual-studio.md)