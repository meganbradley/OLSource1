...
|[ITransactionObject](https://msdn.microsoft.com/en-us/library/ms713659.aspx)|Optional|No|  
 ...
 The session object creates a rowset object. If the provider supports commands, the session also creates a command object (<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, implementing the OLE DB **TCommand**). The command object implements the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> interface and uses the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> method to execute commands on the rowset, as shown in the following figure.  
