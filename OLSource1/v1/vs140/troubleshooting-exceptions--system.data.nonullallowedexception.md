...
A \<xref:System.Data.NoNullAllowedException*> exception is thrown when there is an attempt to insert a null value into a column where \<xref:System.Data.DataColumn.AllowDBNull*> is set to <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>.  
  
## Associated Tips  
 **Check to determine if the value is DBNull before adding it to the column.**  
 If \<xref:System.Data.DataColumn.AllowDBNull*> is set to <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, you will not be able to insert a null value. For more information, see \<xref:System.DBNull*>.  
  
 **Set AllowDBNull to true.**  
 Setting this property to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> will allow you to inset null values. For more information, see \<xref:System.Data.DataColumn.AllowDBNull*>.  
