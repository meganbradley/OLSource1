...
An object variable contains a pointer to data that is stored elsewhere. The type of that data can change during run time. At any moment, you can use the [GetTypeCode](assetId:///M:System.Type.GetTypeCode(System.Type)?qualifyHint=False&autoUpgrade=True) method to determine the current run-time type, or the [TypeOf Operator (Visual Basic)](../vs140/typeof-operator--visual-basic-.md) to find out if the current run-time type is compatible with a specified type.  
...
2.  On the <xref:System.Type*?displayProperty=fullName> class, call the shared method assetId:///M:System.Type.GetTypeCode(System.Type)?qualifyHint=False&autoUpgrade=True to retrieve the <xref:System.TypeCode*> enumeration value for the object's type.  
...
## See Also  
 ...
 assetId:///M:System.Type.GetTypeCode(System.Type)?qualifyHint=False&autoUpgrade=True   
