...
-   Every update to a model or diagram must be made in the user interface (UI) thread. The background thread must use [Invoke](assetId:///M:System.Windows.Forms.Control.Invoke(System.Delegate)?qualifyHint=False&autoUpgrade=True) or `Dispatcher.`<xref:System.Windows.Threading.Dispatcher.Invoke*> to have the UI thread perform the actual updates.  
