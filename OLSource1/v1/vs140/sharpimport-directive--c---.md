...
#### Parameters  
 *filename*  
 Specifies the type library to import. <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> can be one of the following:  
...
|Syntax form|Action|  
|-----------------|------------|  
|Quoted form|Instructs the preprocessor to look for type library files first in the directory of the file that contains the <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> statement, and then in the directories of whatever files that include (<CodeContentPlaceHolder>14\</CodeContentPlaceHolder>) that file. The preprocessor then searches along the paths shown below.|  
...
##  \<a name="_predir_the_23import_directive_header_files_created_by_import">\</a> Header Files Created by Import  
 <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> creates two header files that reconstruct the type library contents in C++ source code. The primary header file is similar to that produced by the Microsoft Interface Definition Language (MIDL) compiler, but with additional compiler-generated code and data. The [primary header file](#_predir_the_primary_type_library_header_file) has the same base name as the type library, plus a .TLH extension. The secondary header file has the same base name as the type library, with a .TLI extension. It contains the implementations for compiler-generated member functions, and is included (<CodeContentPlaceHolder>16\</CodeContentPlaceHolder>) in the primary header file.  
 ...
 Both header files are placed in the output directory specified by the /Fo (name object file) option. They are then read and compiled by the compiler as if the primary header file was named by a <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> directive.  
  
 The following compiler optimizations come with the <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> directive:  
  
-   The header file, when created, is given the same timestamp as the type library.  
  
-   When <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> is processed, the compiler first checks if the header exists and is up to date. If yes, then it does not need to be re-created.  
  
 The <CodeContentPlaceHolder>20\</CodeContentPlaceHolder> directive also participates in minimal rebuild and can be placed in a precompiled header file. See [Creating Precompiled Header Files](../vs140/creating-precompiled-header-files.md) for more information.  
...
-   Heading boilerplate: Consists of comments, <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> statement for COMDEF.H (which defines some standard macros used in the header), and other miscellaneous setup information.  
  
-   Forward references and typedefs: Consists of structure declarations such as <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> and typedefs.  
  
-   Smart pointer declarations: The template class <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> is a smart-pointer implementation that encapsulates interface pointers and eliminates the need to call <CodeContentPlaceHolder>24\</CodeContentPlaceHolder>, **Release**, <CodeContentPlaceHolder>25\</CodeContentPlaceHolder> functions. In addition, it hides the <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> call in creating a new COM object. This section uses macro statement **_COM_SMARTPTR_TYPEDEF** to establish typedefs of COM interfaces to be template specializations of the [_com_ptr_t](../vs140/_com_ptr_t-class.md) template class. For example, for interface **IMyInterface**, the .TLH file will contain:  
  
<CodeContentPlaceHolder>6\</CodeContentPlaceHolder>  
     which the compiler will expand to:  
  
<CodeContentPlaceHolder>7\</CodeContentPlaceHolder>  
     Type <CodeContentPlaceHolder>27\</CodeContentPlaceHolder> can then be used in place of the raw interface pointer <CodeContentPlaceHolder>28\</CodeContentPlaceHolder>. Consequently, there is no need to call the various **IUnknown** member functions  
  
-   Typeinfo declarations: Primarily consists of class definitions and other items exposing the individual typeinfo items returned by **ITypeLib:GetTypeInfo**. In this section, each typeinfo from the type library is reflected in the header in a form dependent on the <CodeContentPlaceHolder>29\</CodeContentPlaceHolder> information.  
...
-   <CodeContentPlaceHolder>30\</CodeContentPlaceHolder> statement for the secondary type library header.  
  
-   Footer boilerplate: Currently includes <CodeContentPlaceHolder>31\</CodeContentPlaceHolder>.  
...
<CodeContentPlaceHolder>8\</CodeContentPlaceHolder>  
 immediately after the <CodeContentPlaceHolder>32\</CodeContentPlaceHolder> statement in the source code.  
  
 The namespace can be suppressed by using the [no_namespace](#_predir_no_namespace) attribute of the <CodeContentPlaceHolder>33\</CodeContentPlaceHolder> directive. However, suppressing the namespace may lead to name collisions. The namespace can also be renamed by the [rename_namespace](#_predir_rename_namespace) attribute.  
...
<CodeContentPlaceHolder>9\</CodeContentPlaceHolder>  
 The actual filename in the <CodeContentPlaceHolder>34\</CodeContentPlaceHolder> comment is the full path of the cross-referenced type library, as stored in the registry. If you encounter errors that are due to missing type definitions, check the comments at the head of the .TLH to see which dependent type libraries may need to be imported first. Likely errors are syntax errors (for example, C2143, C2146, C2321), C2501 (missing decl-specifiers), or C2433 ('inline' not permitted on data declaration) while compiling the .TLI file.  
  
 You must determine which of the dependency comments are not otherwise provided for by system headers and then provide an <CodeContentPlaceHolder>35\</CodeContentPlaceHolder> directive at some point before the <CodeContentPlaceHolder>36\</CodeContentPlaceHolder> directive of the dependent type library to resolve the errors.  
...
##  \<a name="_predir_the_23import_directive_import_attributes">\</a> #import Attributes  
 <CodeContentPlaceHolder>37\</CodeContentPlaceHolder> can optionally include one or more attributes. These attributes tell the compiler to modify the contents of the type-library headers. A backslash (**\\**) symbol can be used to include additional lines in a single <CodeContentPlaceHolder>38\</CodeContentPlaceHolder> statement. For example:  
