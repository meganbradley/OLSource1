...
The <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> keyword declares a user-defined type conversion operator that must be invoked with a cast. For example, this operator converts from a class called Fahrenheit to a class called Celsius:  
...
## Example  
 The following example provides a <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> and a <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> class, each of which provides an explicit conversion operator to the other class.  
  
 [!code[csrefKeywordsConversion#1](../vs140/codesnippet/CSharp/explicit--csharp-reference-_3.cs)]  
  
## Example  
 The following example defines a struct, <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>, that represents a single decimal digit. An operator is defined for conversions from <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> to <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>, but because not all bytes can be converted to a <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>, the conversion is explicit.  
