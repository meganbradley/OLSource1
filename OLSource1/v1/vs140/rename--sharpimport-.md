---
title: "rename (#import)"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "Rename"
dev_langs: 
  - "C++"
  - "C"
helpviewer_keywords: 
  - "rename attribute"
ms.assetid: 5c5c6153-1087-4b7b-87fb-fc59b90b9975
caps.latest.revision: 8
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# rename (#import)
**C++ Specific**  
  
 Works around name collision problems.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
#### Parameters  
 <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>  
 Old name in the type library.  
  
 <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>  
 Name to be used instead of the old name.  
  
## Remarks  
 If this attribute is specified, the compiler replaces all occurrences of *OldName* in a type library with the user-supplied *NewName* in the resulting header files.  
  
 This attribute can be used when a name in the type library coincides with a macro definition in the system header files. If this situation is not resolved, then various syntax errors will be generated, such as [Compiler Error C2059](../vs140/compiler-error-c2059.md) and [Compiler Error C2061](../vs140/compiler-error-c2061.md).  
  
> [!NOTE]
>  The replacement is for a name used in the type library, not for a name used in the resulting header file.  
  
 For example, suppose a property named <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> exists in a type library, and a macro <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> is defined in a header file and used before <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>. Since <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> is the default name of a wrapper function for the error-handling **get** property, a name collision will occur. To work around the problem, use the following attribute in the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> statement:  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 which renames the name <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> in the type library. An attempt to rename the <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> wrapper name will fail:  
  
<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>  
 This is because the name <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> only occurs in the resulting type library header file.  
  
 **END C++ Specific**  
  
## See Also  
 [#import Attributes](../vs140/sharpimport-attributes--c---.md)   
 [The #import Directive](../vs140/sharpimport-directive--c---.md)