...
1.  Create a project.  
     ...
     ![Create a coded UI test project](../vs140/media/cuit_datadriven_.png "CUIT_dataDriven_")  
...
5.  Use the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> method to verify that the test runs. Place the cursor in the test method shown above, open the context menu, and choose **Run Tests**. (Keyboard shortcut: Ctrl + R, T).  
...
6.  Because a data source can also be used for assertion parameter values—which are used by the test to verify expected values—let’s add an assertion to validate that the sum of the two numbers is correct. Place the cursor in the test method shown above, open the context menu and choose **Generate Code for Coded UI Test**, and then **Use Coded UI Test Builder**.  
     ...
     ![Map the UI text control](../vs140/media/cuit_datadriven_addassertion.png "CUIT_dataDriven_AddAssertion")  
...
8.  After configuring the assertion, generate code from the builder again. This creates a new method for the validation.  
     ...
     Because the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> method validates the results of the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> method, move it to the bottom of the code block.  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
9. Verify that the test runs by using the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> method. Place the cursor in the test method shown above, open the context menu, and choose **Run Tests**. (Keyboard shortcut:Ctrl + R, T).  
...
1.  Add a text file to the dataDrivenSample project named <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>.  
...
1.  To bind the data source, add a <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> attribute within the existing <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> attribute that is immediately above the test method.  
...
1.  Add <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> to the top of the CodedUITest.cs file:  
  
<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>  
2.  Add <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> in the <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> method which will apply values from the data source. The data source values override the constants assigned to UIMap controls by using the controls <CodeContentPlaceHolder>16\</CodeContentPlaceHolder>:  
...
<CodeContentPlaceHolder>4\</CodeContentPlaceHolder>  
    ...
    -   Choose the UI action and observe the corresponding UI control mapping. Notice how the mapping corresponds to the code, for example, <CodeContentPlaceHolder>17\</CodeContentPlaceHolder>.  
  
         ![Use the Coded UI Test Editor to assist with code](../vs140/media/cuit_datadriven_testeditor.png "CUIT_dataDriven_TestEditor")  
  
    -   In the Properties Window, open **Search Properties**. The search properties **Name** value is what is being manipulated in the code using the data source. For example, the <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> is being assigned the values in the first column of each data row: <CodeContentPlaceHolder>19\</CodeContentPlaceHolder>. For the three iterations, this test will change the **Name** value for the search property to 3, then 5, and finally 6.  
