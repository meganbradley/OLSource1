...
###  \<a name="BKMK_CancelAndRestart">\</a> Cancel and Restart the Operation  
 ...
 For more information about cancellation, see [Fine-Tuning Your Async Application (C#)](../vs140/fine-tuning-your-async-application--csharp-.md).  
...
2.  In <CodeContentPlaceHolder>24\</CodeContentPlaceHolder>, determine whether an operation is already underway. If the value of <CodeContentPlaceHolder>25\</CodeContentPlaceHolder> is null, no operation is already active. If the value isn't null, the operation that is already running is canceled.  
  
<CodeContentPlaceHolder>4\</CodeContentPlaceHolder>  
3.  Set <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> to a different value that represents the current process.  
  
<CodeContentPlaceHolder>5\</CodeContentPlaceHolder>  
4.  At the end of <CodeContentPlaceHolder>27\</CodeContentPlaceHolder>, the current process is complete, so set the value of <CodeContentPlaceHolder>28\</CodeContentPlaceHolder> back to null.  
  
<CodeContentPlaceHolder>6\</CodeContentPlaceHolder>  
 The following code shows all the changes in <CodeContentPlaceHolder>29\</CodeContentPlaceHolder>. The additions are marked with asterisks.  
  
<CodeContentPlaceHolder>7\</CodeContentPlaceHolder>  
 In <CodeContentPlaceHolder>30\</CodeContentPlaceHolder>, make the following changes.  
  
-   Add a parameter to accept the cancellation token from <CodeContentPlaceHolder>31\</CodeContentPlaceHolder>.  
  
-   Use the \<xref:System.Net.Http.HttpClient.GetAsync*> method to download the websites because <CodeContentPlaceHolder>32\</CodeContentPlaceHolder> accepts a \<xref:System.Threading.CancellationToken*> argument.  
  
-   Before calling <CodeContentPlaceHolder>33\</CodeContentPlaceHolder> to display the results for each downloaded website, check <CodeContentPlaceHolder>34\</CodeContentPlaceHolder> to verify that the current operation hasn’t been canceled.  
...
<CodeContentPlaceHolder>9\</CodeContentPlaceHolder>  
 To eliminate the partial lists, uncomment the first line of code in <CodeContentPlaceHolder>35\</CodeContentPlaceHolder> to clear the text box each time the user restarts the operation.  
...
###  \<a name="BKMK_RunMultipleOperations">\</a> Run Multiple Operations and Queue the Output  
 ...
 The operations share a global \<xref:System.Threading.Tasks.Task*>, <CodeContentPlaceHolder>36\</CodeContentPlaceHolder>, which serves as a gatekeeper for the display process.  
...
<CodeContentPlaceHolder>12\</CodeContentPlaceHolder>  
 The <CodeContentPlaceHolder>37\</CodeContentPlaceHolder> variable, <CodeContentPlaceHolder>38\</CodeContentPlaceHolder>, oversees the display process and prevents any group from interrupting another group's display operation. The character variable, <CodeContentPlaceHolder>39\</CodeContentPlaceHolder>, labels the output from different groups to verify that results appear in the expected order.  
  
#### The Click Event Handler  
 The event handler, <CodeContentPlaceHolder>40\</CodeContentPlaceHolder>, increments the group letter each time the user chooses the **Start** button. Then the handler calls <CodeContentPlaceHolder>41\</CodeContentPlaceHolder> to run the downloading operation.  
  
<CodeContentPlaceHolder>13\</CodeContentPlaceHolder>  
#### The AccessTheWebAsync Method  
 This example splits <CodeContentPlaceHolder>42\</CodeContentPlaceHolder> into two methods. The first method, <CodeContentPlaceHolder>43\</CodeContentPlaceHolder>, starts all the download tasks for a group and sets up <CodeContentPlaceHolder>44\</CodeContentPlaceHolder> to control the display process. The method uses a Language Integrated Query (LINQ query) and \<xref:System.Linq.Enumerable.ToArray*> to start all the download tasks at the same time.  
  
 <CodeContentPlaceHolder>45\</CodeContentPlaceHolder> then calls <CodeContentPlaceHolder>46\</CodeContentPlaceHolder> to await the completion of each download and display its length.  
  
 <CodeContentPlaceHolder>47\</CodeContentPlaceHolder> returns a task that's assigned to <CodeContentPlaceHolder>48\</CodeContentPlaceHolder> in <CodeContentPlaceHolder>49\</CodeContentPlaceHolder>. That value prevents interruption by another operation before the task is complete.  
...
#### The FinishOneGroupAsync Method  
 ...
 The first statement in <CodeContentPlaceHolder>50\</CodeContentPlaceHolder> uses <CodeContentPlaceHolder>51\</CodeContentPlaceHolder> to make sure that entering the method doesn't interfere with an operation that is already in the display process or that's already waiting. If such an operation is in progress, the entering operation must wait its turn.  
...
-   The <CodeContentPlaceHolder>52\</CodeContentPlaceHolder> task is null  at the start of <CodeContentPlaceHolder>53\</CodeContentPlaceHolder> only for group A, which started first. Group A hasn’t yet completed an await expression when it reaches <CodeContentPlaceHolder>54\</CodeContentPlaceHolder>. Therefore, control hasn't returned to <CodeContentPlaceHolder>55\</CodeContentPlaceHolder>, and the first assignment to <CodeContentPlaceHolder>56\</CodeContentPlaceHolder> hasn't occurred.  
  
-   The following two lines always appear together in the output. The code is never interrupted between starting a group's operation in <CodeContentPlaceHolder>57\</CodeContentPlaceHolder> and assigning a task for the group to <CodeContentPlaceHolder>58\</CodeContentPlaceHolder>.  
  
<CodeContentPlaceHolder>17\</CodeContentPlaceHolder>  
     After a group enters <CodeContentPlaceHolder>59\</CodeContentPlaceHolder>, the operation doesn't complete an await expression until the operation enters <CodeContentPlaceHolder>60\</CodeContentPlaceHolder>. Therefore, no other operation can gain control during that segment of code.  
...
5.  Name the project <CodeContentPlaceHolder>61\</CodeContentPlaceHolder>, and then choose the **OK** button.  
