---
title: "CMFCRibbonColorButton::GetAutomaticColor"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "reference"
f1_keywords: 
  - "CMFCRibbonColorButton.GetAutomaticColor"
  - "GetAutomaticColor"
  - "CMFCRibbonColorButton::GetAutomaticColor"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "GetAutomaticColor method"
ms.assetid: bc053737-f04c-4f77-8d1c-0168c6ee4929
caps.latest.revision: 13
robots: noindex,nofollow
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# CMFCRibbonColorButton::GetAutomaticColor
Retrieves the current automatic-button color.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Return Value  
 An RGB color value that represents the current automatic-button color.  
  
## Remarks  
 The automatic-button color is set by the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> parameter passed to the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> method.  
  
## Requirements  
 **Header:** afxribboncolorbutton.h  
  
## See Also  
 [CMFCRibbonColorButton Class](../vs140/cmfcribboncolorbutton-class.md)   
 [Hierarchy Chart (MFC Feature Pack)](../vs140/hierarchy-chart.md)