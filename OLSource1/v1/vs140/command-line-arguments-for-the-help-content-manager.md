...
|/locale|No|Specifies the product locale that’s used to view and manage content for the current instance of the Help viewer. For example, you specify <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> for English-United States.\<br />\<br /> If you don’t specify a locale, the locale of the operating system is used. If that locale can’t be determined, <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> is used.\<br />\<br /> If you specify a locale that isn’t valid, an error message is logged in the event log.|  
...
|/vendor|No|Specifies the vendor for the product content that will be removed (for example, <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>). The default argument for this switch is Microsoft.|  
