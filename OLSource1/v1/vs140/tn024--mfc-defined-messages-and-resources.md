 ...
 **MFC Private Windows Messages**  
 ...
 In the rare case of needing to handle one of these messages, you should use the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> message map macro and handle the message in the generic LRESULT/WPARAM/LPARAM format.  
 ...
 **WM_SIZEPARENT**  
 ...
 This message is sent by a frame window to its immediate children during resizing (**CFrameWnd::OnSize** calls <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> which calls <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>) to reposition the control bars around the side of the frame. The **AFX_SIZEPARENTPARAMS** structure contains the current available client rectangle of the parent and a HDWP (which may be NULL) with which to call <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> to minimize repainting.  
 ...
 **WM_IDLEUPDATECMDUI**  
 ...
 This message is sent in idle time to implement the idle-time update of update-command UI handlers. If the window (usually a control bar) handles the message, it creates a <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> object (or an object of a derived class) and call **CCmdUI::DoUpdate** for each of the "items" in the window. This will in turn check for an <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> handler for the objects in the command-handler chain.  
 ...
 *bDisableIfNoHandler* is nonzero to disable the UI object if there is neither an <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> nor an <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> handler.  
  
 **WM_EXITHELPMODE**  
  
 This message is posted to a <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> that to exit context sensitive help mode. The receipt of this message terminates the modal loop started by **CFrameWnd::OnContextHelp.**  
 ...
 **WM_INITIALUPDATE**  
 ...
 This message is sent by the document template to all descendants of a frame window when it is safe for them to do their initial update. It maps to a call to <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> but can be used in other <CodeContentPlaceHolder>12\</CodeContentPlaceHolder>-derived classes for other one-shot updating.  
 ...
 **WM_RECALCPARENT**  
 ...
 This message is sent by a view to its parent window (obtained via <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>) to force a layout recalculation (usually, the parent will call <CodeContentPlaceHolder>14\</CodeContentPlaceHolder>). This is used in OLE server applications where it is necessary for the frame to grow in size as the view's total size grows.  
  
 If the parent window processes this message it should return TRUE and fill the RECT passed in lParam with the new size of the client area. This is used in <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> to properly handle scrollbars (place then on the outside of the window when they are added) when a server object is in-place activated.  
 ...
 **WM_SIZECHILD**  
 ...
 This message is sent by <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> to its owner window (via <CodeContentPlaceHolder>17\</CodeContentPlaceHolder>) when the user resizes the resize bar with the resize handles. <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> responds to this message by attempting to reposition the frame window as the user has requested.  
 ...
 **WM_FLOATSTATUS**  
 ...
 This message is sent to all pop-up windows owned by a frame window when the frame is either activated or deactivated by another top-level frame window. This is used by the implementation of **MFS_SYNCACTIVE** in <CodeContentPlaceHolder>19\</CodeContentPlaceHolder>, to keep the activation of these pop-up windows in sync with the activation of the top level frame window.  
...
|lParam|Not used (0)|  
 ...
 The return value should be non-zero if **FS_SYNCACTIVE** is set and the window syncronizes its activation with the parent frame. <CodeContentPlaceHolder>20\</CodeContentPlaceHolder> returns non-zero when the style is set to **MFS_SYNCACTIVE.**  
  
 For more information, see the implementation of <CodeContentPlaceHolder>21\</CodeContentPlaceHolder>.  
