...
-   Construct a special type of category registry entry under [HKLM\SOFTWARE\Microsoft \Visual Studio\\*\<Visual Studio version>*\FontAndColors\\<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>]  
...
-   Populate the registry with two values:  
 ...
 The service specified in the registry must provide an implementation of <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> for the corresponding group.  
  
## To Implement IDE Support  
  
-   Implement \<xref:Microsoft.VisualStudio.Shell.Interop.IVsFontAndColorDefaultsProvider.GetObject*>, which returns either an \<xref:Microsoft.VisualStudio.Shell.Interop.IVsFontAndColorDefaults*> interface or an <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> interface to the IDE for each **Category** or group GUID supplied.  
...
-   The IDE uses the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> interface to define a union of several categories.  
