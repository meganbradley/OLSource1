...
MFC provides the template class [CWinFormsDialog](../vs140/cwinformsdialog-class.md) so that you can host a Windows Forms user control (<xref:System.Windows.Forms.UserControl*>) in a modal or modeless MFC dialog box. `CWinFormsDialog` is derived from the MFC class [CDialog](../vs140/cdialog-class.md), so the dialog box can be launched as modal or modeless.  
