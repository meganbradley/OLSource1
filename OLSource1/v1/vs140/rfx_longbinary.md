...
---
# RFX_LongBinary
Transfers binary large object (BLOB) data using class [CLongBinary](../vs140/clongbinary-class.md) between the field data members of a <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> object and the columns of a record on the data source of ODBC type **SQL_LONGVARBINARY** or **SQL_LONGVARCHAR**.  
 ...
 *value*  
 The value stored in the indicated data member — the value to be transferred. For a transfer from recordset to data source, the value, of type <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>, is taken from the specified data member. For a transfer from data source to recordset, the value is stored in the specified data member.  
