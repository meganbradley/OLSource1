...
A \<xref:System.Runtime.InteropServices.DispIdAttribute*> attribute block specifies a DISPID value less than 0, which is reserved by <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> for special functions on the class to which it is applied.  
  
 The dispatch identifier (DISPID) is used in COM as an argument to the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> method to access the properties and methods exposed by a COM object.  
  
 **Error ID:** BC32506  
  
### To correct this error  
  
-   Specify a DISPID value greater than zero in <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
