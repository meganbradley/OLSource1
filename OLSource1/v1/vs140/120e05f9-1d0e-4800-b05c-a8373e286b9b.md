...
An expression used for an attribute argument evaluates to a data type different from that of the corresponding attribute parameter, and [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] does not allow the required type conversion for attribute arguments.  
 ...
 Certain type conversions cannot produce values that are constant at compile time. For example, converting a <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> to a <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> or a <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> depends on the locale setting at run time. Other conversions, such as an array of a derived type to an array of <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, present a variety of problems that do not permit the compiler to allow them on attribute arguments.  
