...
## Remarks  
 The default implementation of this function calls the [OnEndPrinting](../vs140/cview--onendprinting.md) member function and restores the main frame window to the state it was in before print preview began. Override this function to perform special processing when preview mode is terminated. For example, if you want to maintain the user's position in the document when switching from preview mode to normal display mode, you can scroll to the position described by the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> parameter and the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> member of the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> structure that the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> parameter points to.  
  
 Always call the base class version of <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> from your override, typically at the end of the function.  
