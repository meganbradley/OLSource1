...
This example demonstrates how to create multicast delegates. A useful property of [delegate](../vs140/delegate--csharp-reference-.md) objects is that multiple objects can be assigned to one delegate instance by using the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> operator. The multicast delegate contains a list of the assigned delegates. When the multicast delegate is called, it invokes the delegates in the list, in order. Only delegates of the same type can be combined.  
  
 The <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> operator can be used to remove a component delegate from a multicast delegate.  
