...
-   An entry point event, which signals that the program has reached its main entry point, such as **Main** or <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>. This event is not typically sent if the DE attaches to a program that is already running.  
