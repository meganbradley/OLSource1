...
|Title|Description|  
|-----------|-----------------|  
|[Conditional Expressions](../vs140/conditional-expressions--if...-then...else--fsharp-.md)|Describes the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> expression, which runs different branches of code and also evaluates to a different value depending on the Boolean expression given.|  
|[Match Expression](../vs140/match-expressions--fsharp-.md)|Describes the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> expression, which provides branching control that is based on the comparison of an expression with a set of patterns.|  
|[Loops: for .. to Expression](../vs140/loops--for...to-expression--fsharp-.md)|Describes the <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> expression, which is used to iterate in a loop over a range of values of a loop variable.|  
|[Loops: for .. in Expression](../vs140/loops--for...in-expression--fsharp-.md)|Describes the <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> expression, a looping construct that is used to iterate over the matches of a pattern in an enumerable collection such as a range expression, sequence, list, array, or other construct that supports enumeration.|  
|[Loops: while .. do Expression](../vs140/loops--while...do-expression--fsharp-.md)|Describes the <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> expression, which is used to perform iterative execution (looping) while a specified test condition is true.|  
