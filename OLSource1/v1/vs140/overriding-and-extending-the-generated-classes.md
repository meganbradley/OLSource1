...
### Overriding methods in a partial class  
 ...
 For example, if in your DSL Definition you define a domain class named <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, you could write custom code that adds override methods:  
...
### Double-Derived Classes  
 ...
 For example, in the sample Library.dsl, the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> domain class has the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> property set to <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>. The generated code for that domain class contains two classes:  
  
-   <CodeContentPlaceHolder>11\</CodeContentPlaceHolder>, which is an abstract and which contains all the methods and properties.  
  
-   <CodeContentPlaceHolder>12\</CodeContentPlaceHolder>, which is derived from <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>. It is empty, except for its constructors.  
  
 To override any method, you create a partial definition of the derived class such as <CodeContentPlaceHolder>14\</CodeContentPlaceHolder>. You can override both the generated methods and the methods inherited from the modeling framework.  
...
### Custom Constructors  
 ...
 If you want to provide your own constructor, you can do this by setting <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> for the domain class in the DSL Definition. When you click **Transform All Templates**, the generated code will not include a constructor for that class. It will include a call to the missing constructor. This causes an error report when you build the solution. Double-click the error report to see a comment in the generated code that explains what you should provide.  
...
### Flagged Extension Points  
 A flagged extension point is a place in the DSL Definition where you can set a property or a check box to indicate that you will provide a custom method. Custom constructors are one example. Other examples include setting the <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> of a domain property to Calculated or Custom Storage or setting the **Is Custom** flag in a connection builder.  
...
### Rules  
 ...
 Rules are defined on a per-class basis, so that you do not have to have code that registers the rule for each object. For more information, see [How to create custom rules](../vs140/rules-propagate-changes-within-the-model.md).  
