...
|15|Transition state (1 if the key is being released, 0 if the key is being pressed).|  
 ...
 For a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> message, the key-transition bit (bit 15) is 1 and the context-code bit (bit 13) is 0.  
  
## Remarks  
 A nonsystem key is a keyboard key that is pressed when the ALT key is not pressed or a keyboard key that is pressed when the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> has the input focus.  
