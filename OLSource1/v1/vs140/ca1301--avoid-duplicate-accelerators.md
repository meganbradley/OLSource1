...
## Example  
 The following example shows a minimal form that contains two controls that have identical access keys. The keys are stored in a resource file, which is not shown; however, their values appear in the commented out <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> lines. The behavior of duplicate accelerators can be examined by exchanging the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> lines with their commented out counterparts. However, in this case, the example will not generate a warning from the rule.  
  
 [!code[FxCop.Globalization.AvoidDuplicateAccels#1](../vs140/codesnippet/CSharp/ca1301--avoid-duplicate-accelerators_1.cs)]  
