...
---
# _SECURE_SCL
Defines whether [Checked Iterators](../vs140/checked-iterators.md) are enabled. If defined as 1, unsafe iterator use causes a runtime error and the program is terminated. If defined as 0, checked iterators are disabled. In debug mode, the default value for **_SECURE_SCL** is 1, meaning checked iterators are enabled. In release mode, the default value for <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> is 0.  
  
> [!IMPORTANT]
>  Use <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> to control <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>. For more information, see [_ITERATOR_DEBUG_LEVEL](../vs140/_iterator_debug_level.md).  
