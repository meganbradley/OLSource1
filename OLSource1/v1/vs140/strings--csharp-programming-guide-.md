...
A string is an object of type \<xref:System.String*> whose value is text. Internally, the text is stored as a sequential read-only collection of \<xref:System.Char*> objects. There is no null-terminating character at the end of a C# string; therefore a C# string can contain any number of embedded null characters ('\0'). The \<xref:System.String.Length*> property of a string represents the number of <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> objects it contains, not the number of Unicode characters. To access the individual Unicode code points in a string, use the \<xref:System.Globalization.StringInfo*> object.  
  
## string vs. System.String  
 In C#, the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> keyword is an alias for \<xref:System.String*>. Therefore, <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> are equivalent, and you can use whichever naming convention you prefer. The <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> class provides many methods for safely creating, manipulating, and comparing strings. In addition, the C# language overloads some operators to simplify common string operations. For more information about the keyword, see [string (C# Reference)](../vs140/string--csharp-reference-.md). For more information about the type and its methods, see \<xref:System.String*>.  
...
## Immutability of String Objects  
 String objects are *immutable*: they cannot be changed after they have been created. All of the \<xref:System.String*> methods and C# operators that appear to modify a string actually return the results in a new string object. In the following example, when the contents of <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> are concatenated to form a single string, the two original strings are unmodified. The <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> operator creates a new string that contains the combined contents. That new object is assigned to the variable <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>, and the original object that was assigned to <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> is released for garbage collection because no other variable holds a reference to it.  
...
## Strings, Extension Methods and LINQ  
 Because the \<xref:System.String*> type implements <xref:System.Collections.Generic.IEnumerable<CodeContentPlaceHolder>11\</CodeContentPlaceHolder>+<CodeContentPlaceHolder>12\</CodeContentPlaceHolder>Stringbuilder<CodeContentPlaceHolder>13\</CodeContentPlaceHolder>String.Split` method to parse strings.|  
