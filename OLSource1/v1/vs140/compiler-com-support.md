...
## Microsoft Specific  
 ...
 By using the [#import preprocessor directive](../vs140/sharpimport-directive--c---.md), the compiler can read a type library and convert it into a C++ header file that describes the COM interfaces as classes. A set of <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> attributes is available for user control of the content for the resulting type library header files.  
  
 You can use the [__declspec](../vs140/__declspec.md) extended attribute [uuid](../vs140/uuid--c---.md) to assign a globally unique identifier (GUID) to a COM object. The keyword [__uuidof](../vs140/__uuidof-operator.md) can be used to extract the GUID associated with a COM object. Another <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> attribute, [property](../vs140/property--c---.md), can be used to specify the **get** and **set** methods for a data member of a COM object.  
  
 A set of COM support global functions and classes is provided to support the **VARIANT** and <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> types, implement smart pointers, and encapsulate the error object thrown by <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>:  
