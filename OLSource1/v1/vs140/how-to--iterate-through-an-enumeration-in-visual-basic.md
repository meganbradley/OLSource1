...
Enumerations provide a convenient way to work with sets of related constants, and to associate constant values with names. To iterate through an enumeration, you can move it into an array using the [GetValues](assetId:///M:System.Enum.GetValues(System.Type)?qualifyHint=False&autoUpgrade=True) method. You could also iterate through an enumeration using a `For...Each` statement, using the [GetNames](assetId:///M:System.Enum.GetNames(System.Type)?qualifyHint=False&autoUpgrade=True) or assetId:///M:System.Enum.GetValues(System.Type)?qualifyHint=False&autoUpgrade=True method to extract the string or numeric value.  
  
### To iterate through an enumeration  
  
-   Declare an array and convert the enumeration to it with the assetId:///M:System.Enum.GetValues(System.Type)?qualifyHint=False&autoUpgrade=True method before passing the array as you would any other variable. The following example displays each member of the enumeration <xref:Microsoft.VisualBasic.FirstDayOfWeek*> as it iterates through the enumeration.  
