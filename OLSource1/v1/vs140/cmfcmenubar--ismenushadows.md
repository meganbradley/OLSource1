---
title: "CMFCMenuBar::IsMenuShadows"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "reference"
f1_keywords: 
  - "CMFCMenuBar.IsMenuShadows"
  - "CMFCMenuBar::IsMenuShadows"
  - "IsMenuShadows"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "IsMenuShadows method"
ms.assetid: fbe2ed3d-3200-4f10-ad64-be9511efdf1d
caps.latest.revision: 13
robots: noindex,nofollow
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# CMFCMenuBar::IsMenuShadows
Indicates whether the framework draws shadows for pop-up menus.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Return Value  
 <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> if the framework draws menu shadows; otherwise <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
  
## Remarks  
 Use the [CMFCMenuBar::EnableMenuShadows](../vs140/cmfcmenubar--enablemenushadows.md) method to enable or disable this feature.  
  
## Requirements  
 **Header:** afxmenubar.h  
  
## See Also  
 [CMFCMenuBar Class](../vs140/cmfcmenubar-class.md)   
 [MFC Hierarchy Chart](../vs140/hierarchy-chart.md)