...
---
# _CrtGetDumpClient
Retrieves the current application-defined function for dumping the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> type memory blocks (debug version only).  
...
## Remarks  
 The <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> function retrieves the current hook function for dumping objects stored in the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> memory blocks for the C run-time debug memory dump process.  
