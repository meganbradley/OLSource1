...
-   [lock Statement (C# Programmer's Reference)](../vs140/lock-statement--csharp-reference-.md)  
  
-   [Monitors](assetId:///33fe4aef-b44b-42fd-9e72-c908e39e75db)  
  
## Monitors  
 Like the `lock` keyword, monitors prevent blocks of code from simultaneous execution by multiple threads. The [Enter](assetId:///M:System.Threading.Monitor.Enter(System.Object)?qualifyHint=False&autoUpgrade=True) method allows one and only one thread to proceed into the following statements; all other threads are blocked until the executing thread calls [Exit](assetId:///M:System.Threading.Monitor.Exit(System.Object)?qualifyHint=False&autoUpgrade=True). This is just like using the `lock` keyword. For example:  
...
## Synchronization Events and Wait Handles  
 ...
 Threads can be made to wait on events by calling one of the wait methods, such as <xref:System.Threading.WaitHandle.WaitOne*>, [WaitAny](assetId:///M:System.Threading.WaitHandle.WaitAny(System.Threading.WaitHandle[])?qualifyHint=False&autoUpgrade=True), or [WaitAll](assetId:///M:System.Threading.WaitHandle.WaitAll(System.Threading.WaitHandle[])?qualifyHint=False&autoUpgrade=True). <xref:System.Threading.WaitHandle.WaitOne*?displayProperty=fullName> causes the thread to wait until a single event becomes signaled, <xref:System.Threading.WaitHandle.WaitAny*?displayProperty=fullName> blocks a thread until one or more indicated events become signaled, and <xref:System.Threading.WaitHandle.WaitAll*?displayProperty=fullName> blocks the thread until all of the indicated events become signaled. An event becomes signaled when its <xref:System.Threading.EventWaitHandle.Set*> method is called.  
...
## See Also  
 ...
 [lock Statement (C# Reference)](../vs140/lock-statement--csharp-reference-.md)   
