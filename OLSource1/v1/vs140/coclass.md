...
## Remarks  
 ...
 If two header files contain classes with the **coclass** attribute and don't specify a GUID, the compiler will use the same GUID for both classes, and that will result in a MIDL error.  Therefore, you should use the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> attribute when you use **coclass**.  
...
-   [IProvideClassInfo2Impl](../vs140/iprovideclassinfo2impl-class.md) is added if the [noncreatable](../vs140/noncreatable.md) attribute is not specified for the target object.  
 ...
 The **coclass** attribute also makes the following functions available via injected code, or in the case of <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>, as a static method in the base class <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>:  
  
-   <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> registers the class factories of the target class.  
  
-   <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>, which is related to registration, can also be used to obtain the CLSID of the target class.  
  
-   **GetObjectFriendlyName** by default returns a string of the format "\<*target class name*> <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>". If this function is already present, it is not added. Add this function to the target class to return a friendlier name than the one automatically generated.  
