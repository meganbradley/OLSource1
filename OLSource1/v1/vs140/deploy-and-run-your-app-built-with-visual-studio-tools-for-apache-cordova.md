...
##  \<a name="BuildProcess">\</a> How the Cordova build process works in Visual Studio  
 ...
 Visual Studio automates the process of adding platforms (using the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> command) and installation/uninstallation of Cordova plugins (using the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> command) based on user selections in Visual Studio. Visual Studio uses the CLI to build for the requested platform for the selected configuration (for example, <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>).  
