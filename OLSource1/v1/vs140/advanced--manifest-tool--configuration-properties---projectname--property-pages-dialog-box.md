...
## UIElement List  
 **Update File Hashes**  
 Uses the /hashupdate option to specify that the manifest tool will compute the hash of files specified in <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> elements, and then update the hash attributes with the computed value.  
  
 **Update File Hashes Search Path**  
 Specifies the search path for files that are referenced in <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> elements. This option also uses the /hashupdate option.  
