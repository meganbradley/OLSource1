...
2.  Start the profiler by typing the following at a command prompt:  
    ...
    -   The [/output](../vs140/output.md)**:**<CodeContentPlaceHolder>2\</CodeContentPlaceHolder> option is required with **/start**. <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> specifies the name and the location of the profiling data (.vsp) file.  
...
3.  Attach the profiler to the service by typing the following command at a command prompt:  
     ...
     **VSPerfCmd /attach:** <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>  
  
     <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> specifies the process ID or process name of the target application. You can view the process IDs of all running processes in Windows Task Manager.  
