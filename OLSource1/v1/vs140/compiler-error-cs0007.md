...
Unexpected common language runtime initialization error — 'description'  
 ...
 This can happen if the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> file was changed. This file is configured during setup and should not be changed. If there is a possibility that the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> file was changed, check the file to make sure that the version of the runtime specified in the file is present on the machine. If the correct version is present, it may be corrupted. Reinstall the common language runtime.