...
##  \<a name="BKMK_Overview">\</a> Setting a function breakpoint in source code  
 ...
 At this point the value of <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is still 1.  
 ...
 You can set a breakpoint on any line of executable code. For example, in the C# code above you can set a breakpoint on the variable declaration, the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> loop, or any code inside the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> loop, but you cannot set a breakpoint on the namespace or class declarations or the method signature.  
...
2.  In the **Address** box, type a memory address or an expression that evaluates to a memory address. For example, type <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> to break when the contents of the variable <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> changes.  
  
3.  In the **Byte Count** dropdown, select the number of bytes you want the debugger to watch. For example, if you select **4**, the debugger will watch the four bytes starting at <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> and break if any of those bytes change value.  
...
-   The memory location is updated within the kernel. For example, if memory is passed to the 32-bit Windows <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> function, the memory will be updated from kernel mode and the debugger doesn't break on the memory write.  
...
## Setting a Breakpoint with a Memory Address (native C++ only)  
 ...
 For example, given an object of type <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> with the address, you can set a function breakpoint on a method named <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> called from that instance.  
  
1.  Set a breakpoint somewhere after that instance of the class is instantiated.  
  
2.  Find the address of the instance (we'll say it's <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>).  
 ...
 **Conditional Expression:** When you select Conditional Expression, you can then choose two conditions: **Is true** and **When changed**. Choose **Is true** if you want to break when the expression is satisfied, or choose **When changed** if you want to break when the value of the expression has changed.  
 ...
 In the following example we set the breakpoint to hit only when the value of <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> is **4**:  
  
 ![Breakpoint condition is true](../vs140/media/breakpointconditionistrue.png "BreakpointConditionIsTrue")  
  
 In the following example we set the breakpoint to hit only when the value of <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> changes:  
...
4.  Use the Object ID in the Conditional Expression field. For example, if there is a variable <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> referring to the object that is to be added to the collection, you would put **item == $n**, where **n** is the object ID number.  
...
-   ThreadName = "name"  
 ...
 Enclose string values in double quotes. You can combine clauses using <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> (AND), <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> (OR), <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> (NOT), and parentheses.  
