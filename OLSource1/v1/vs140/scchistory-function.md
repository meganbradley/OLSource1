...
#### Parameters  
 ...
 [in] Number of files specified in the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> array.  
...
## Remarks  
 The source control plug-in can display its own dialog box to show the history of each file, using <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> as the parent window. Alternatively, the optional text output callback function supplied to the [SccOpenProject Function](../vs140/sccopenproject-function.md) can be used, if it is supported.  
  
 Note that under certain circumstances, the file being examined may change during the execution of this call. For example, the [!INCLUDE[vsvss](../vs140/includes/vsvss_md.md)] history command gives the user a chance to get an old version of the file. In such a case, the source control plug-in returns <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> to warn the IDE that it needs to reload the file.  
