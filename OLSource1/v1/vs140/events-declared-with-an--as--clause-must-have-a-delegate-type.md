---
title: "Events declared with an &#39;As&#39; clause must have a delegate type"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "vbc31044"
  - "bc31044"
helpviewer_keywords: 
  - "BC31044"
ms.assetid: d0e86fcc-c638-4945-aa6f-9826aa38b23d
caps.latest.revision: 13
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# Events declared with an &#39;As&#39; clause must have a delegate type
A type was specified that is not a delegate.  
  
 **Error ID:** BC31044  
  
### To correct this error  
  
1.  Remove the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> clause.  
  
2.  Specify a delegate with the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> clause.  
  
## See Also  
 [Events in Visual Basic](../vs140/events--visual-basic-.md)   
 [Delegates in Visual Basic](../vs140/delegates--visual-basic-.md)   
 [Event Statement](../vs140/event-statement.md)   
 [Delegate Statement](../vs140/delegate-statement.md)