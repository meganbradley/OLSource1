 ...
 **Indirect Access.** Indirect access through a value type can also generate this error. Consider the following code example, which attempts to set the value of <xref:System.Drawing.Point?qualifyHint=False> by accessing it indirectly through <xref:System.Windows.Forms.Control.Location?qualifyHint=False>.  
...
```  
 ...
 The last statement of the preceding example fails because it creates only a temporary allocation for the <xref:System.Drawing.Point?qualifyHint=False> structure returned by the <xref:System.Windows.Forms.Control.Location?qualifyHint=False> property. A structure is a value type, and the temporary structure is not retained after the statement runs. The problem is resolved by declaring and using a variable for <xref:System.Windows.Forms.Control.Location?qualifyHint=False>, which creates a more permanent allocation for the <xref:System.Drawing.Point?qualifyHint=False> structure. The following example shows code that can replace the last statement of the preceding example.  
