...
### Remarks  
 [IWorkerThreadClient::Execute](../vs140/iworkerthreadclient--execute.md) will be called through <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> when the handle, <CodeContentPlaceHolder>16\</CodeContentPlaceHolder>, is signaled.  
...
### Remarks  
 [IWorkerThreadClient::Execute](../vs140/iworkerthreadclient--execute.md) will be called through <CodeContentPlaceHolder>20\</CodeContentPlaceHolder> when the timer is signaled.  
  
 Pass the timer handle from <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> to [CWorkerThread::RemoveHandle](../vs140/cworkerthread--removehandle.md) to close the timer.  
...
### Remarks  
 ...
 To have two or more <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> objects use the same worker thread, initialize one of them without passing any arguments then pass a pointer to that object to the <CodeContentPlaceHolder>24\</CodeContentPlaceHolder> methods of the others. The objects initialized using the pointer should be shut down before the object used to initialize them.  
...
### Remarks  
 When the handle is removed [IWorkerThreadClient::CloseHandle](../vs140/iworkerthreadclient--closehandle.md) will be called on the associated object that was passed to [AddHandle](../vs140/cworkerthread--addhandle.md). If this call fails, <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> will call the Windows [CloseHandle](http://msdn.microsoft.com/library/windows/desktop/ms724211) function on the handle.  
...
### Return Value  
 Returns S_OK on success, or an error HRESULT on failure, such as if the timeout value, <CodeContentPlaceHolder>28\</CodeContentPlaceHolder>, is exceeded.  
  
### Remarks  
 To reuse the object, call [CWorkerThread::Initialize](../vs140/cworkerthread--initialize.md) after calling this method.  
  
 Note that calling **Shutdown** on an object initialized with a pointer to another <CodeContentPlaceHolder>29\</CodeContentPlaceHolder> object has no effect and always returns S_OK.  
