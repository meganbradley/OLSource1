...
A generic method is a method that is declared with type parameters, as follows:  
 ...
 Use constraints to enable more specialized operations on type parameters in methods. This version of `Swap<T>`, now named `SwapIfGreater<T>`, can only be used with type arguments that implement <xref:System.IComparable`1?qualifyHint=False>.  
