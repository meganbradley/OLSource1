...
## Remarks  
 The **/cgthreads** option specifies the maximum number of threads cl.exe uses in parallel for the optimization and code generation phases of compilation. Notice that there can be no space between **/cgthreads** and the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> argument. By default, cl.exe uses four threads, as if **/cgthreads4** were specified. If more processor cores are available, a larger <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> value can improve build times. This option is especially useful when it's combined with [/GL (Whole Program Optimization)](../vs140/-gl--whole-program-optimization-.md).  
...
4.  Modify the **Additional Options** property to include **/cgthreads**<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, where <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> is a value from 1 to 8, and then select **OK**.  
