...
cannot open filename for writing  
 ...
 The linker can’t write to <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>. The file may be in use and its file handle locked by another process, or you may not have write permission for the file, or for the directory or network share it is located in. This error is often caused by a transient condition—for example, a lock held by an anti-virus program, a file search indexing process, or a delay in releasing a lock held by the [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] build system.  
  
 To fix this issue, verify that the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> file handle is not locked, and that you have write permission for the file. If it is an executable, verify that it is not already running.  
  
 You can use the Windows SysInternals utilities [Handle](http://technet.microsoft.com/sysinternals/bb896655.aspx) or [Process Explorer](http://technet.microsoft.com/sysinternals/bb896653) to determine which process has a file handle lock on <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>. You can also use Process Explorer to release locks on open file handles. For information about how to use these utilities, see the Help files that come with them.  
