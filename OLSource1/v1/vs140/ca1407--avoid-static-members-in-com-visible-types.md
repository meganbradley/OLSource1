...
## Cause  
 A type that is specifically marked as visible to Component Object Model (COM) contains a <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> method.  
  
## Rule Description  
 COM does not support <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> methods.  
 ...
 For this rule to occur, an assembly-level \<xref:System.Runtime.InteropServices.ComVisibleAttribute*> must be set to <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> and the class- \<xref:System.Runtime.InteropServices.ComVisibleAttribute*> must be set to <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>, as the following code shows.  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## How to Fix Violations  
 To fix a violation of this rule, change the design to use an instance method that provides the same functionality as the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> method.  
  
## When to Suppress Warnings  
 It is safe to suppress a warning from this rule if a COM client does not require access to the functionality that is provided by the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> method.  
  
## Example Violation  
  
### Description  
 The following example shows a <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> method that violates this rule.  
...
### Description  
 To fix the violation in the previous example, you could change the method to an instance method, but that does not make sense in this instance. A better solution is to explicitly apply <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> to the method to make it clear to other developers that the method cannot be seen from COM.  
