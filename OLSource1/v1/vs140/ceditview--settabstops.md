...
## Remarks  
 Only a single tab-stop width is supported. (<CodeContentPlaceHolder>2\</CodeContentPlaceHolder> objects support multiple tab widths.) Widths are in dialog units, which equal one-fourth of the average character width (based on uppercase and lowercase alphabetic characters only) of the font used at the time of printing or displaying. You should not use <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> because <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> must cache the tab-stop value.  
  
 This function modifies only the tabs of the object for which it is called. To change the tab stops for each <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> object in your application, call each object's <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> function.  
