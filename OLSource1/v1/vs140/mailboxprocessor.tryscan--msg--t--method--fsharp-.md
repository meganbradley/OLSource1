...
## Remarks  
 If a timeout period is exceeded, `None` is returned. This method is for use within the body of the agent. For each agent, at most one concurrent reader can be active, so no more than one concurrent call to [Receive](../vs140/mailboxprocessor.receive--msg--method--fsharp-.md), [TryReceive](../vs140/mailboxprocessor.tryreceive--msg--method--fsharp-.md), [Scan](../vs140/mailboxprocessor.scan--msg--t--method--fsharp-.md) or `TryScan` can be active. The body of the `scanner` function is locked during its execution, but the lock is released before the execution of the asynchronous workflow.  
