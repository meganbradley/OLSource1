...
## Remarks  
 <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> incorporates the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> macro to support serialization and dumping of its elements. If an array of doublewords is stored to an archive, either with the overloaded insertion ( **<<**) operator or with the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> member function, each element is, in turn, serialized.  
  
> [!NOTE]
>  Before using an array, use <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> to establish its size and allocate memory for it. If you do not use <CodeContentPlaceHolder>11\</CodeContentPlaceHolder>, adding elements to your array causes it to be frequently reallocated and copied. Frequent reallocation and copying are inefficient and can fragment memory.  
  
 If you need debug output from individual elements in the array, you must set the depth of the <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> object to 1 or greater.  
  
 For more information on using <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>, see the article [Collections](../vs140/collections.md).  
