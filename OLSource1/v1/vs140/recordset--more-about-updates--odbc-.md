...
###  \<a name="_core_how_transactions_affect_updates">\</a> How Transactions Affect Updates  
 Beyond understanding how <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, **Edit**, and **Delete** work, it is important to understand how the **BeginTrans**, **CommitTrans**, and **Rollback** member functions of [CDatabase](../vs140/cdatabase-class.md) work with the update functions of [CRecordset](../vs140/crecordset-class.md).  
  
 By default, calls to <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> and **Edit** affect the data source immediately when you call **Update**. **Delete** calls take effect immediately. But you can establish a transaction and execute a batch of such calls. The updates are not permanent until you commit them. If you change your mind, you can roll back the transaction instead of committing it.  
...
###  \<a name="_core_how_closing_the_recordset_affects_updates">\</a> How Closing the Recordset Affects Updates  
 If you close a recordset, or its associated <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> object, with a transaction in progress (you have not called [CDatabase::CommitTrans](../vs140/cdatabase--committrans.md) or [CDatabase::Rollback](../vs140/cdatabase--rollback.md)), the transaction is rolled back automatically (unless your database backend is the Microsoft Jet database engine).  
...
> [!TIP]
>  To force caching of an entire snapshot at once, call <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> immediately after you open the snapshot. Then call **MoveFirst** to begin working with the records. <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> is equivalent to scrolling over all the records, but it retrieves them all at once. Note, however, that this can lower performance and might not be required for some drivers.  
...
### Update Success and Failure  
 If **Update** succeeds, the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> or **Edit** mode ends. To begin an <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> or **Edit** mode again, call <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> or **Edit**.  
  
 If **Update** fails (returns **FALSE** or throws an exception), you remain in <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> or **Edit** mode, depending on which function you called last. You can then do one of the following:  
  
-   Modify a field data member and try the **Update** again.  
  
-   Call <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> to reset the field data members to Null, set the values of the field data members, and then call **Update** again.  
  
-   Call **Edit** to reload the values that were in the recordset before the first call to <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> or **Edit**, set the values of the field data members, and then call **Update** again. After a successful **Update** call (except after an <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> call), the field data members retain their new values.  
  
-   Call **Move** (including **Move** with a parameter of **AFX_MOVE_REFRESH**, or 0), which flushes any changes and ends any <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> or **Edit** mode in effect.  
...
-   **AFX_SQL_ERROR_MULTIPLE_ROWS_AFFECTED**  
 ...
 When these exceptions are thrown, you remain in the <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> or **Edit** state you were in when you called **Update** or **Delete**. Here are the most common scenarios in which you would see these exceptions. You're most likely to see:  
