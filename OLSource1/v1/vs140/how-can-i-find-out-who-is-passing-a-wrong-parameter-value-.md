...
4.  Enter an expression, such as <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, into the text box, where <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is the name of the parameter that contains the bad value, and <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> is the bad value passed to it.  
  
5.  Select the **is True** radio button, and click the **OK** button.  
  
6.  Now run the program again. The breakpoint causes the program to halt at the beginning of the function when the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> parameter has the value <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>.  
