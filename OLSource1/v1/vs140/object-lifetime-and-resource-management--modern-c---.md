...
## Concepts  
 ...
 The final concept is the DAG (Directed Acyclic Graph).  The structure of ownership in a program forms a DAG. No object can own itself—that's not only impossible but also inherently meaningless. But two objects can share ownership of a third object.  Several kinds of links are possible in a DAG like this: A is a member of B (B owns A), C stores a <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> (C owns each D element), E stores a <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> (E shares ownership of F, possibly with other objects), and so forth.  As long as there are no cycles and every link in the DAG is represented by an object that has a destructor (instead of a raw pointer, handle, or other mechanism), then resource leaks are impossible because the language prevents them. Resources are released immediately after they're no longer needed, without a garbage collector running. The lifetime tracking is overhead-free for stack scope, bases, members, and related cases, and inexpensive for <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>.  
  
### Heap-based lifetime  
 For heap object lifetime, use [smart pointers](../vs140/smart-pointers--modern-c---.md). Use <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> as the default pointer and allocator. Use <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> to break cycles, do caching, and observe objects without affecting or assuming anything about their lifetimes.  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 Use <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> for unique ownership, for example, in the *pimpl* idiom. (See [Pimpl For Compile-Time Encapsulation](../vs140/pimpl-for-compile-time-encapsulation--modern-c---.md).) Make a <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> the primary target of all explicit <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> expressions.  
