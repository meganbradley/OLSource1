...
## Remarks  
 ...
 The benefit of this class is that it provides a convenient way to change the state model for existing worker thread classes. <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> will create a single worker for the lifetime of the thread, so if the worker class holds state, it will hold it across multiple requests. By simply wrapping that class in the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> template before using it with <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>, the lifetime of the worker and the state it holds is limited to a single request.  
...
### Remarks  
 This class does not do any initialization in <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>.  
...
### Remarks  
 This class does not do any cleanup in <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>.  
