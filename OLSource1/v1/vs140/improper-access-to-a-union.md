...
**ANSI 3.3.2.3** A member of a union object is accessed using a member of a different type  
 ...
 For example, a union of **float** and <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> is declared. A **float** value is stored, but the program later accesses the value as an <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>. In such a situation, the value would depend on the internal storage of **float** values. The integer value would not be reliable.  
