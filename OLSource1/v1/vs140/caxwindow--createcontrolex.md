 ...
 *punkSink*  
 [in] A pointer to the **IUnknown** interface of the sink object to be connected to the connection point on the contained object specified by <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>.  
...
## Remarks  
 This method is similar to [CAxWindow::CreateControl](../vs140/caxwindow--createcontrol.md), but unlike that method, <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> also allows you to receive an interface pointer to the newly created control and set up an event sink to receive events fired by the control.  
...
## Example  
 See [Hosting ActiveX Controls Using ATL AXHost](../vs140/hosting-activex-controls-using-atl-axhost.md) for a sample that uses <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>.  
