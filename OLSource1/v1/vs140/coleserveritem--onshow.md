...
## Remarks  
 This function is typically called when the user of the container application creates an item or executes a verb, such as Edit, that requires the item to be shown. The default implementation attempts in-place activation. If this fails, the function calls the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> member function to display the OLE item in a separate window.  
