...
1.  In the package code file, add a public property called **OptionInteger** to the **MyToolsOptionsPackage** class.  
    ...
    ```  
     ...
     This code calls [GetDialogPage](assetId:///M:Microsoft.VisualStudio.Shell.Package.GetDialogPage(System.Type)?qualifyHint=False&autoUpgrade=True) to create or retrieve an `OptionPageGrid` instance. `OptionPageGrid` calls <xref:Microsoft.VisualStudio.Shell.DialogPage.LoadSettingsFromStorage*> to load its options, which are public properties.  
