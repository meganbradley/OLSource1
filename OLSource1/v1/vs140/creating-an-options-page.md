...
1.  Every Visual Studio extension starts with a VSIX deployment project which will contain the extension assets. Create a [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] VSIX project named `MyToolsOptionsExtension`. You can find the VSIX project template in the **New Project** dialog under **Visual C# / Extensibility**.  
...
3.  Declare an OptionPageGrid class and derive it from <xref:Microsoft.VisualStudio.Shell.DialogPage?qualifyHint=False>.  
...
4.  Apply a <xref:Microsoft.VisualStudio.Shell.ProvideOptionPageAttribute?qualifyHint=False> to the VSPackage class to assign to the class an options category and options page name for the OptionPageGrid. The result should look like this:  
...
5.  Add an `OptionInteger` property to the `OptionPageGrid` class.  
    ...
    -   Apply a <xref:System.ComponentModel.CategoryAttribute?qualifyHint=True> to assign to the property a property grid category.  
  
    -   Apply a <xref:System.ComponentModel.DisplayNameAttribute?qualifyHint=True> to assign to the property a name.  
  
    -   Apply a <xref:System.ComponentModel.DescriptionAttribute?qualifyHint=True> to assign to the property a description.  
    ...
    > [!NOTE]
    >  The default implementation of <xref:Microsoft.VisualStudio.Shell.DialogPage?qualifyHint=False> supports properties that have appropriate converters or that are structures or arrays that can be expanded into properties that have appropriate converters. For a list of converters, see the <xref:System.ComponentModel?qualifyHint=False> namespace.  
...
5.  Apply a second <xref:Microsoft.VisualStudio.Shell.ProvideOptionPageAttribute?qualifyHint=False> to the VSPackage class. This attribute assigns the class an options category and options page name.  
...
1.  In the package code file, add a public property called **OptionInteger** to the **MyToolsOptionsPackage** class.  
    ...
    ```  
     ...
     This code calls <xref:Microsoft.VisualStudio.Shell.Package.GetDialogPage?qualifyHint=False> to create or retrieve an `OptionPageGrid` instance. `OptionPageGrid` calls <xref:Microsoft.VisualStudio.Shell.DialogPage.LoadSettingsFromStorage?qualifyHint=False> to load its options, which are public properties.  
