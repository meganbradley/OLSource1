...
## Sections of the DslDefinition.dsl File  
 The root element is \<Dsl>, and its attributes identify the name of the domain-specific language, the namespace, and major and minor version numbers for versioning. The <CodeContentPlaceHolder>31\</CodeContentPlaceHolder> schema defines the content and structure for a valid DslDefinition.dsl file.  
 ...
 Types  
 This section defines each type and its namespace. Domain properties have two types. <CodeContentPlaceHolder>32\</CodeContentPlaceHolder> are defined in the model and generate types into DomainModel.cs. <CodeContentPlaceHolder>33\</CodeContentPlaceHolder> refer to types that are defined elsewhere (such as <CodeContentPlaceHolder>34\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>35\</CodeContentPlaceHolder>) and do not generate anything.  
...
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 Usually, the namespace of the referenced item (in this example, the <CodeContentPlaceHolder>36\</CodeContentPlaceHolder> domain class) is the same as the referencing item (in this case, the LibraryHasMembers domain relationship). In those cases, the moniker must give only the name of the class. Otherwise, you should use the full form /Namespace/Name:  
...
<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>  
 The <CodeContentPlaceHolder>37\</CodeContentPlaceHolder> attribute controls whether the generated code is prefixed by the <CodeContentPlaceHolder>38\</CodeContentPlaceHolder> Common Language Runtime (CLR) attribute, which determines whether values of the enumeration can be combined bitwise. If this attribute is set to true, you should specify power-of-two values for the literal values.  
  
## Classes  
 Most of the elements in any definition of a domain-specific language are either directly or indirectly instances of <CodeContentPlaceHolder>39\</CodeContentPlaceHolder>. Subclasses of <CodeContentPlaceHolder>40\</CodeContentPlaceHolder> include <CodeContentPlaceHolder>41\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>42\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>43\</CodeContentPlaceHolder>, and <CodeContentPlaceHolder>44\</CodeContentPlaceHolder>. The <CodeContentPlaceHolder>45\</CodeContentPlaceHolder> section of the DslDefinition.dsl file lists the domain classes.  
  
 Each class has a set of properties and might have a base class. In the Component Diagram example, <CodeContentPlaceHolder>46\</CodeContentPlaceHolder> is an abstract class that has a <CodeContentPlaceHolder>47\</CodeContentPlaceHolder> property, whose type is string:  
  
<CodeContentPlaceHolder>4\</CodeContentPlaceHolder>  
 <CodeContentPlaceHolder>48\</CodeContentPlaceHolder> is the base of several of the other classes such as <CodeContentPlaceHolder>49\</CodeContentPlaceHolder>, which has its own properties in addition to the <CodeContentPlaceHolder>50\</CodeContentPlaceHolder> property, which it inherited from <CodeContentPlaceHolder>51\</CodeContentPlaceHolder>. The BaseClass child node contains a moniker reference. Because the referenced class is in the same namespace, only its name is required in the moniker:  
...
-   A C# class is generated for each domain class that is listed in the <CodeContentPlaceHolder>53\</CodeContentPlaceHolder> section. The C# classes are generated in Dsl\GeneratedCode\DomainClasses.cs.  
  
### Properties  
 Each domain property has a name and a type. The name must be unique within the domain class and its transitive bases.  
  
 The type must refer to one of those listed in the <CodeContentPlaceHolder>54\</CodeContentPlaceHolder> section. Generally, the moniker must include the namespace.  
...
-   **IsElementName**. If this attribute is set to true, its value is automatically set to a unique value when an instance of the parent class is created. This attribute can be set to true for only one property in each class, which must have a string type. In the Component Diagram example, the <CodeContentPlaceHolder>55\</CodeContentPlaceHolder> property in <CodeContentPlaceHolder>56\</CodeContentPlaceHolder> has <CodeContentPlaceHolder>57\</CodeContentPlaceHolder> set to true. Whenever a user creates a <CodeContentPlaceHolder>58\</CodeContentPlaceHolder> element (which inherits from <CodeContentPlaceHolder>59\</CodeContentPlaceHolder>), the name is automatically initialized to something like "Component6."  
  
-   <CodeContentPlaceHolder>60\</CodeContentPlaceHolder>. If you have specified this attribute, the value that you specified is assigned to this attribute for new instances of this class. If <CodeContentPlaceHolder>61\</CodeContentPlaceHolder> is set, the DefaultValue attribute specifies the initial part of the new string.  
  
-   **Category** is the header under which the property will appear in the **Properties** window.  
  
## Relationships  
 The <CodeContentPlaceHolder>62\</CodeContentPlaceHolder> section lists all the relationships in the domain-specific language. Every <CodeContentPlaceHolder>63\</CodeContentPlaceHolder> is binary and directed, linking members of a source class to members of a target class. The source and target classes are typically domain classes, but relationships to other relationships are also permitted.  
...
-   The <CodeContentPlaceHolder>64\</CodeContentPlaceHolder> attribute references the domain class of the linked instances: OutPort for the source, InPort for the target.  
  
-   The <CodeContentPlaceHolder>65\</CodeContentPlaceHolder> attribute has four possible values (ZeroMany, ZeroOne, One, and OneMany). This attribute refers to the number of links of this relationship that can be associated with one role player.  
  
-   The <CodeContentPlaceHolder>66\</CodeContentPlaceHolder> attribute specifies the name that is used in the role playing class to access the objects at the other end. This name is used in template or custom code to traverse the relationship. For example, the <CodeContentPlaceHolder>67\</CodeContentPlaceHolder> attribute of the source role is set to <CodeContentPlaceHolder>68\</CodeContentPlaceHolder>. Therefore, the following code will work:  
  
<CodeContentPlaceHolder>7\</CodeContentPlaceHolder>  
     By convention, property names are plural if the multiplicity is ZeroMany or OneMany.  
  
     The multiplicity of a role refers to how many of the opposite role can be associated with each instance of this role. For example, in the relationship ComponentHasPorts, the target role has the <CodeContentPlaceHolder>69\</CodeContentPlaceHolder> attribute set to Port, the <CodeContentPlaceHolder>70\</CodeContentPlaceHolder> attribute set to Component, and the <CodeContentPlaceHolder>71\</CodeContentPlaceHolder> attribute set to ZeroOne. Therefore, the appropriate code to use this role is:  
  
<CodeContentPlaceHolder>8\</CodeContentPlaceHolder>  
-   The role's <CodeContentPlaceHolder>72\</CodeContentPlaceHolder> is the name that is used within the Relationship class to refer to that end of a link. By convention, a role name is always singular, because each link has only one instance at each end. The following code would work:  
  
<CodeContentPlaceHolder>9\</CodeContentPlaceHolder>  
-   By default, the <CodeContentPlaceHolder>73\</CodeContentPlaceHolder> attribute is set to true. If it is set to false, no property is created on the Role Player class. (In that case, <CodeContentPlaceHolder>74\</CodeContentPlaceHolder>, for example, would not work). However, it is still possible to use custom code to traverse the relationship or obtain access to the links themselves if the custom code uses the relationship explicitly:  
...
## Paths  
 ...
 Paths appear in the DslDefinition.dsl file in <CodeContentPlaceHolder>75\</CodeContentPlaceHolder> tags. Although paths can navigate through multiple links, most examples in practice traverse only one link.  
 ...
 Each segment starts with a relationship name. In an object-to-link hop, the relationship precedes a dot and the property name: "<CodeContentPlaceHolder>76\</CodeContentPlaceHolder>". In a link-to-object hop, the relationship precedes an exclamation mark and the role name: "<CodeContentPlaceHolder>77\</CodeContentPlaceHolder>".  
...
<CodeContentPlaceHolder>15\</CodeContentPlaceHolder>  
 ...
 The first link creation path navigates from the <CodeContentPlaceHolder>78\</CodeContentPlaceHolder> to the <CodeContentPlaceHolder>79\</CodeContentPlaceHolder> and then creates an instance of the embedding relationship <CodeContentPlaceHolder>80\</CodeContentPlaceHolder>. The second link creation path creates a link of the reference relationship CommentsReferenceComponents from the host Component to the new Comment. All link creation paths must start with the host class and must end at a link that steps towards the newly instantiated class.  
  
## XmlClassData  
 Each domain class (including relationships and other subtypes) can have extra information provided in an <CodeContentPlaceHolder>81\</CodeContentPlaceHolder> node, which appears under the <CodeContentPlaceHolder>82\</CodeContentPlaceHolder> section of the DslDefinition.dsl file. This information specifically concerns how instances of the class are stored in serialized form when a model is saved to a file.  
  
 Much of the generated code that <CodeContentPlaceHolder>83\</CodeContentPlaceHolder> influences is in <CodeContentPlaceHolder>84\</CodeContentPlaceHolder>.  
  
 Each <CodeContentPlaceHolder>85\</CodeContentPlaceHolder> node includes these child nodes and attributes:  
...
### XmlPropertyData  
 ...
 The **IsMonikerKey** and **IsMonikerQualifier** attributes give a property a role in identifying instances of the parent class. You can set **IsMonikerKey** to true for one property that is defined in or inherited by a class. This attribute identifies an individual instance of the parent class. The property that you set to <CodeContentPlaceHolder>86\</CodeContentPlaceHolder> is usually a name or other key identifier. For example, the <CodeContentPlaceHolder>87\</CodeContentPlaceHolder> string property is the moniker key for NamedElement and its derived classes. When the user saves a model to file, this attribute must contain unique values for each instance, among its siblings in the tree of embedding relationships.  
...
-   **Dsl** is the RootClass node and the class of the diagram. DomainClass, DomainRelationship, and other elements are embedded under <CodeContentPlaceHolder>88\</CodeContentPlaceHolder>.  
  
-   **Classes** is the **RoleElementName** of the relationship between Domain-Specific Language and DomainClass.  
  
<CodeContentPlaceHolder>24\</CodeContentPlaceHolder>  
-   The **XmlSerializationBehavior** attribute is embedded under the <CodeContentPlaceHolder>89\</CodeContentPlaceHolder> attribute, but the **OmitElement** attribute has been set on the embedding relationship. Therefore, no <CodeContentPlaceHolder>90\</CodeContentPlaceHolder> attribute intervenes. By contrast, a **ClassData** attribute is the <CodeContentPlaceHolder>91\</CodeContentPlaceHolder> attribute of the embedding relationship between an **XmlSerializationBehavior** attribute and an **XmlClassData** attribute.  
  
<CodeContentPlaceHolder>25\</CodeContentPlaceHolder>  
-   ConnectorHasDecorators is the embedding relationship between <CodeContentPlaceHolder>92\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>93\</CodeContentPlaceHolder>. <CodeContentPlaceHolder>94\</CodeContentPlaceHolder> has been set so that the name of the relationship appears with its list of properties for each link from the Connector object. However, <CodeContentPlaceHolder>95\</CodeContentPlaceHolder> has also been set so that no <CodeContentPlaceHolder>96\</CodeContentPlaceHolder> encloses the multiple links that are embedded inside <CodeContentPlaceHolder>97\</CodeContentPlaceHolder>:  
...
-   <CodeContentPlaceHolder>98\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>99\</CodeContentPlaceHolder> attributes.  
  
-   **ExposesFillColorAsProperty** and several similar attributes. These Boolean attributes make the corresponding property variable by the user. Generally, when a language user clicks a shape on the diagram, the properties that appear in the **Properties** window are those of the domain class instance to which the shape is mapped. If <CodeContentPlaceHolder>100\</CodeContentPlaceHolder> is set to true, a property of the shape itself also appears.  
  
-   **ShapeHasDecorators**. An instance of this attribute occurs for each text, icon, or expand/collapse decorator. (In the DslDefinition.dsl file, <CodeContentPlaceHolder>101\</CodeContentPlaceHolder> is a relationship with <CodeContentPlaceHolder>102\</CodeContentPlaceHolder> set to true.)  
  
## Shape Maps  
 Shape maps determine how instances of a given domain class appear on the screen, represented by a shape. Both shape and connector maps appear under the <CodeContentPlaceHolder>103\</CodeContentPlaceHolder> section of the DslDefinition.dsl file.  
  
 As in the following example, the <CodeContentPlaceHolder>104\</CodeContentPlaceHolder> elements have, at a minimum, the moniker of a domain class, the moniker of a shape, and a <CodeContentPlaceHolder>105\</CodeContentPlaceHolder> element:  
  
<CodeContentPlaceHolder>27\</CodeContentPlaceHolder>  
 The primary function of the <CodeContentPlaceHolder>106\</CodeContentPlaceHolder> element is so that the same class of objects can appear as a different shape in different contexts. For example, if an <CodeContentPlaceHolder>107\</CodeContentPlaceHolder> could also be embedded in a comment, the <CodeContentPlaceHolder>108\</CodeContentPlaceHolder> could appear as a different shape for that purpose.  
  
 Secondly, the path determines how the shape relates to its parent. No embedding structure is defined between the shapes in a DslDefinition.dsl file. You must infer the structure from the shape maps. The parent of a shape is the shape that is mapped to the domain element that the parent element path identifies. In this case, the path identifies the component to which the <CodeContentPlaceHolder>109\</CodeContentPlaceHolder> belongs. In another shape map, the Component class is mapped to ComponentShape. Therefore, the new <CodeContentPlaceHolder>110\</CodeContentPlaceHolder> shape is made a child shape of its component's <CodeContentPlaceHolder>111\</CodeContentPlaceHolder>.  
...
<CodeContentPlaceHolder>28\</CodeContentPlaceHolder>  
 The root of the model does not have a shape map. Instead, the root is referenced directly from the diagram, which has a <CodeContentPlaceHolder>112\</CodeContentPlaceHolder> element:  
