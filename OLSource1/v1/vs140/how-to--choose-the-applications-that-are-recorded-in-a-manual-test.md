...
-   How to capture the delay between actions  
 ...
 For more information about how to play back actions, see [Recording and Playing Back Manual Tests](../vs140/record-and-play-back-manual-tests.md).  
...
> [!CAUTION]
>  When a user runs a test using [!INCLUDE[mtrlong](../vs140/includes/mtrlong_md.md)], the user can decide whether to create an action recording, override an existing action recording, or not when they start each test. If the action log diagnostic data adapter has not been selected in the test settings for the role that runs the tests, the default values are used when the action recording is created.  
  
> [!NOTE]
>  In [!INCLUDE[vs_dev11_long](../vs140/includes/vs_dev11_long_md.md)], the ARM architecture is supported for action logs.  
  
 The following procedure describes how to configure your diagnostic data adapter from the configuration editor for action log and action recording. To create your tests settings, see [Create Test Settings for Manual Tests](../vs140/collect-more-diagnostic-data-in-manual-tests.md).  
...
## Configure recording of user interface actions for playback and as text  
 Before you follow the steps in this procedure, you must open your test settings for your test plan using [!INCLUDE[TCMext](../vs140/includes/tcmext_md.md)]. From the **Plan** view, select **Properties**. Select the test settings and then select the **Data and Diagnostics** page.  
...
## See Also  
 [Create Test Settings for Manual Tests](../vs140/collect-more-diagnostic-data-in-manual-tests.md)   
 [Setting Up Machines and Diagnostic Information to be Collected](../vs140/setting-up-machines-and-collecting-diagnostic-information-using-test-settings.md)   
