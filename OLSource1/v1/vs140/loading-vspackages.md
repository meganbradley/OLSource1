...
## Forcing a VSPackage to load  
 ...
 You can use the [LoadPackage](assetId:///M:Microsoft.VisualStudio.Shell.Interop.IVsShell.LoadPackage(System.Guid@,Microsoft.VisualStudio.Shell.Interop.IVsPackage@)?qualifyHint=False&autoUpgrade=True) method to force a VSPackage to load.  
...
## Using a custom attribute to register a VSPackage  
 In certain cases you may need to create a new registration attribute for your extension. You can use registration attributes to add new registry keys or to add new values to existing keys. The new attribute must derive from <xref:Microsoft.VisualStudio.Shell.RegistrationAttribute*>, and it must override the [Register](assetId:///M:Microsoft.VisualStudio.Shell.RegistrationAttribute.Register(Microsoft.VisualStudio.Shell.RegistrationAttribute.RegistrationContext)?qualifyHint=False&autoUpgrade=True) and [Unregister](assetId:///M:Microsoft.VisualStudio.Shell.RegistrationAttribute.Unregister(Microsoft.VisualStudio.Shell.RegistrationAttribute.RegistrationContext)?qualifyHint=False&autoUpgrade=True) methods.  
