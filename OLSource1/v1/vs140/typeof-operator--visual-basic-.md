...
## Example  
 ...
 The variable `refInteger` has a run-time type of `Integer`. It is compatible with `Integer` but not with `Double`. The variable `refForm` has a run-time type of <xref:System.Windows.Forms.Form?qualifyHint=False>. It is compatible with <xref:System.Windows.Forms.Form?qualifyHint=False> because that is its type, with <xref:System.Windows.Forms.Control?qualifyHint=False> because <xref:System.Windows.Forms.Form?qualifyHint=False> inherits from <xref:System.Windows.Forms.Control?qualifyHint=False>, and with <xref:System.ComponentModel.IComponent?qualifyHint=False> because <xref:System.Windows.Forms.Form?qualifyHint=False> inherits from <xref:System.ComponentModel.Component?qualifyHint=False>, which implements <xref:System.ComponentModel.IComponent?qualifyHint=False>. However, `refForm` is not compatible with <xref:System.Windows.Forms.Label?qualifyHint=False>.  
