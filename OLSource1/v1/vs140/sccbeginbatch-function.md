...
## Remarks  
 Source control batches are used to execute the same operations across multiple projects or multiple contexts. Batches can be used to eliminate redundant per-project dialog boxes from the user experience during a batched operation. The <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> function and the [SccEndBatch Function](../vs140/sccendbatch-function.md) are used as a function pair to indicate the beginning and end of an operation. They cannot be nested. <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> sets a flag indicating that a batch operation is in progress.  
