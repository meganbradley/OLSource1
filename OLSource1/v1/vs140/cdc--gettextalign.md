...
## Remarks  
 The text-alignment flags determine how the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> member functions align a string of text in relation to the string's starting point. The text-alignment flags are not necessarily single-bit flags and may be equal to 0. To test whether a flag is set, an application should follow these steps:  
...
2.  Apply the bitwise-AND operator to the result and the return value of <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>.  
