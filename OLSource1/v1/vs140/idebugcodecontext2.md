...
## Remarks  
 The key difference between an <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> interface and an [IDebugMemoryContext2](../vs140/idebugmemorycontext2.md) interface is that an <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> is always instruction-aligned. This means that an <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is always pointing to the beginning of an instruction, whereas an <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> may point to any byte of memory in the run-time architecture. <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> is incremented by instructions rather than by the basic storage size (typically byte).  
