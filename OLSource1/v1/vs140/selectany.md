...
## Example  
 This code shows how to use the `selectany` attribute to ensure data COMDAT folding when you also use the [/OPT:ICF](../vs140/-opt--optimizations-.md) linker option. Note that data must be marked with `selectany` and placed in a **const** (readonly) section. You must explicitly specify the read-only section.  
