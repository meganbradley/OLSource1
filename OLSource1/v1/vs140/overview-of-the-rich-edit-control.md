...
> [!IMPORTANT]
>  If you are using a rich edit control in a dialog box (regardless of whether your application is SDI, MDI, or dialog-based), you must call [AfxInitRichEdit](../vs140/afxinitrichedit.md) once before the dialog box is displayed. A typical place to call this function is in your program's <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> member function. You do not need to call it for each time you display the dialog box, only the first time. You do not have to call <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> if you are working with <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
