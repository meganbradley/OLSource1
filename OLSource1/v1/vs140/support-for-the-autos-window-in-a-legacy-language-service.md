...
### Example  
 The following example shows an implementation of the \<xref:Microsoft.VisualStudio.Package.LanguageService.GetProximityExpressions*> method that gets a list of expressions from the \<xref:Microsoft.VisualStudio.Package.LanguageService.ParseSource*> method using the parse reason \<xref:Microsoft.VisualStudio.Package.ParseReason.Autos*>. Each of the expressions is wrapped as a <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> that implements the \<xref:Microsoft.VisualStudio.TextManager.Interop.IVsEnumBSTR*> interface.  
  
 Note that the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> methods are custom methods on the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> object and were added to support this example. They represent one way in which expressions added to the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> object by the parser (by calling the \<xref:Microsoft.VisualStudio.Package.AuthoringSink.AutoExpression*> method) can be accessed outside the parser.  
