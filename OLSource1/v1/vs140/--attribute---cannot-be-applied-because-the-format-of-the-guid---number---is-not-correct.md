...
A <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> attribute block specifies a globally unique identifier (GUID) that does not conform to the proper format for a GUID. <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> uses GUIDs to uniquely identify the class, the interface, and the creation event.  
...
2.  Ensure that the GUID strings presented to the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> attribute block are copied correctly.  
