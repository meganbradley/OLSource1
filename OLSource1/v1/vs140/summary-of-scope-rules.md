...
## Global names  
 A name of an object, function, or enumerator is global if it is introduced outside any function or class or prefixed by the global unary scope operator (<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>), and if it is not used in conjunction with any of these binary operators:  
  
-   Scope-resolution (<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>)  
...
## Qualified names  
 Names used with the binary scope-resolution operator (<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>) are called "qualified names." The name specified after the binary scope-resolution operator must be a member of the class specified on the left of the operator or a member of its base class(es).  
...
1.  Names preceded by <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> force the search to begin at global scope.  
  
2.  Names preceded by the **class**, <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>, and **union** keywords force the compiler to search only for **class**, <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>, or **union** names.  
  
3.  Names on the left side of the scope-resolution operator (<CodeContentPlaceHolder>6\</CodeContentPlaceHolder>) can be only **class**, <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>, **namespace**, or **union** names.  
