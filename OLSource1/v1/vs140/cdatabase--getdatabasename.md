...
## Return Value  
 A [CString](../vs140/cstringt-class.md) containing the database name if successful; otherwise, an empty <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
  
## Remarks  
 This is not the same as the data source name (DSN) specified in the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> or **Open** call. What <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> returns depends on ODBC. In general, a database is a collection of tables. If this entity has a name, <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> returns it.  
  
 You might, for example, want to display this name in a heading. If an error occurs while retrieving the name from ODBC, <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> returns an empty **Cstring**.  
