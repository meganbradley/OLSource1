...
>  For a similar discussion from the MFC point of view, see [Multithreading: Programming Tips](../vs140/multithreading--programming-tips.md) and [Multithreading: When to Use the Synchronization Classes](../vs140/multithreading--when-to-use-the-synchronization-classes.md).  
 ...
 The Bounce.c program shown in [Sample Multithread C Program](../vs140/sample-multithread-c-program.md) uses a mutex named <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> to coordinate screen updates. Each time one of the display threads is ready to write to the screen, it calls **WaitForSingleObject** with the handle to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> and constant **INFINITE** to indicate that the **WaitForSingleObject** call should block on the mutex and not time out. If <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is clear, the wait function sets the mutex so other threads cannot interfere with the display and continues executing the thread. Otherwise, the thread blocks until the mutex clears. When the thread completes the display update, it releases the mutex by calling **ReleaseMutex**.  
