...
2.  Use the [StepIt](../vs140/cprogressctrl--stepit.md) member function to increment the position. This causes the control to redraw itself.  
    ...
    > [!NOTE]
    >  <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> will cause the position to wrap. For example, given a range of 1 –100, a step of 20, and a position of 90, <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> will set the position to 10.  
  
### To change the position by an arbitrary amount  
  
1.  Use the [OffsetPos](../vs140/cprogressctrl--offsetpos.md) member function to change the position. <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> will accept negative values.  
  
    > [!NOTE]
    >  <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, unlike <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>, will not wrap the position. The new position is adjusted to remain within the range.  
