...
-   **MSG_OOB** Send out-of-band data (**SOCK_STREAM** only).  
 ...
 The length of the address in <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> in bytes.  
  
## Return Value  
 If no error occurs, <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> returns the total number of characters sent. (Note that this can be less than the number indicated by <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>.) Otherwise, a value of **SOCKET_ERROR** is returned, and a specific error code can be retrieved by calling [GetLastError](../vs140/casyncsocket--getlasterror.md). The following errors apply to this member function:  
...
-   **WSAEFAULT** The <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> parameters are not part of the user address space, or the <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> argument is too small (less than the size of a [SOCKADDR](../vs140/sockaddr-structure.md) structure).  
...
-   <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> The Windows Sockets implementation reports a buffer deadlock.  
...
-   **WSAESHUTDOWN** The socket has been shut down; it is not possible to call <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> on a socket after <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> has been invoked with <CodeContentPlaceHolder>20\</CodeContentPlaceHolder> set to 1 or 2.  
...
## Remarks  
 <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> is used on datagram or stream sockets and is used to write outgoing data on a socket. For datagram sockets, care must be taken not to exceed the maximum IP packet size of the underlying subnets, which is given by the **iMaxUdpDg** element in the [WSADATA](../vs140/wsadata-structure.md) structure filled out by [AfxSocketInit](../vs140/afxsocketinit.md). If the data is too long to pass atomically through the underlying protocol, the error **WSAEMSGSIZE** is returned, and no data is transmitted.  
  
 Note that the successful completion of a <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> does not indicate that the data was successfully delivered.  
  
 <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> is only used on a **SOCK_DGRAM** socket to send a datagram to a specific socket identified by the <CodeContentPlaceHolder>24\</CodeContentPlaceHolder> parameter.  
  
 To send a broadcast (on a **SOCK_DGRAM** only), the address in the <CodeContentPlaceHolder>25\</CodeContentPlaceHolder> parameter should be constructed using the special IP address **INADDR_BROADCAST** (defined in the Windows Sockets header file WINSOCK.H) together with the intended port number. Or, if the <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> parameter is **NULL**, the socket is configured for broadcast. It is generally inadvisable for a broadcast datagram to exceed the size at which fragmentation can occur, which implies that the data portion of the datagram (excluding headers) should not exceed 512 bytes.  
