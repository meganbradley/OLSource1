...
## Implementing an Interface Event  
 As you develop UI components, you often start by creating a new form or a new control that inherits from an existing form or control. Events are frequently defined on an interface, and, in that case, you must implement the interface to implement the event. The <xref:System.ComponentModel.INotifyPropertyChanged?qualifyHint=False> interface defines a single <xref:System.ComponentModel.INotifyPropertyChanged.PropertyChanged?qualifyHint=False> event. The following code illustrates how to implement the event that this inherited interface defined:  
