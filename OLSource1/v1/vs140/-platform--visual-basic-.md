...
|`anycpu32bitpreferred`|Compiles your assembly to run on any platform. The application will run as a 32-bit application on both 32-bit and 64-bit versions of Windows. This flag is valid only for executables (.EXE) and requires [!INCLUDE[net_v45](../vs140/includes/net_v45_md.md)].|  
...
-   Incorrect platform invoke or COM declarations that use `Integer` for handles instead of <xref:System.IntPtr?qualifyHint=False>.  
  
-   Casting <xref:System.IntPtr?qualifyHint=False> to `Integer`.  
