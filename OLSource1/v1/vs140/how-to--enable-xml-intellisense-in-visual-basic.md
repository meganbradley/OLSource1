...
3.  Import the target namespace or namespaces into your code file or project. A target namespace is identified by the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> attribute of your XSD schema.  
  
     To import a target namespace, use the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> statement, or add a namespace for all code files in a project by using the **References** page of the Project Designer.  
  
 For more information on the capabilities of XML IntelliSense in Visual Basic, see [XML Intellisense in Visual Basic](../vs140/xml-intellisense-in-visual-basic.md). For more information on importing XML namespaces, see [Imports Statement (XML Namespace Prefix)](../vs140/imports-statement--xml-namespace-.md) or [References Page, Project Designer (Visual Basic)](../vs140/references-page--project-designer--visual-basic-.md).  
...
2.  At the beginning of the code file, add an <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> statement for the target XML namespace, as shown in the following example.  
  
     [!code[VbXMLSamples#1](../vs140/codesnippet/VisualBasic/how-to--enable-xml-intellisense-in-visual-basic_1.vb)]  
  
     To import an XML namespace as the default namespace, that is, the namespace that is applied to XML elements and attributes that do not have a namespace prefix, add an <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> statement for the target default XML namespace. Do not specify a namespace prefix. Following is an example of an <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> statement.  
  
     [!code[VbXmlSamples#50](../vs140/codesnippet/VisualBasic/how-to--enable-xml-intellisense-in-visual-basic_2.vb)]  
...
2.  On the **References** tab, in the **Imported namespaces** box, type the target XML namespace in the form of a full XML namespace declaration (for example, <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>). If the target XML namespace does not specify a namespace prefix, the namespace will be the default XML namespace for the project.  
