...
-   **Regular expression error**  
    ...
    -   Mismatched parentheses (...) or unescaped parentheses \\(...\\). If you want to match a parenthesis in the search string, you must escape it. For example, to match a function use: <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
  
    -   Asterisk or plus at the start of an expression. To match any string of characters, use a dot followed by an asterisk: <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
...
### Custom .runsettings file with incorrect exclusions  
 ...
 Resolution  
 Remove all the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> nodes from the .runsettings file, and then remove all the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> nodes. If that fixes the problem, put them back in stages.  
