...
## Example  
 The following example demonstrates how a BSTR can be passed from an unmanaged to an unmanaged function. The receiving managed function can either use the string in as a BSTR or use [PtrToStringBSTR](assetId:///M:System.Runtime.InteropServices.Marshal.PtrToStringBSTR(System.IntPtr)?qualifyHint=False&autoUpgrade=True) to convert it to a <xref:System.String*> for use with other managed functions. Because the memory representing the BSTR is allocated on the unmanaged heap, no pinning is necessary, because there is no garbage collection on the unmanaged heap.  
