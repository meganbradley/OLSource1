...
-   **WSAEFAULT** The <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> argument is incorrect.  
...
-   <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> No buffer space is available. The socket cannot be connected.  
...
> [!NOTE]
>  In [CSocket](../vs140/csocket-class.md), the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> notification function is never called. For connections, you simply call **Connect**, which will return when the connection is completed (either successfully or in error). How connection notifications are handled is an MFC implementation detail.  
