...
2.  Make sure that stdafx.h is on the include path. For more information about include paths in Visual C++ projects, see [#include](../vs140/sharpinclude-directive--c-c---.md) and [/I](../vs140/-i--additional-include-directories-.md).  
...
-   IntelliSense is not fully supported when you reference a header file multiple times by using the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> directive, and the meaning of that header file changes because of various macro states that are defined through the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> directive. In other words, when you include a header file several times and the header usage changes under different macro states, IntelliSense does not always work.  
