...
|Name|Description|  
|----------|-----------------|  
|[CDaoDatabase::CDaoDatabase](#cdaodatabase__cdaodatabase)|Constructs a <CodeContentPlaceHolder>25\</CodeContentPlaceHolder> object. Call **Open** to connect the object to a database.|  
...
|[CDaoDatabase::CanUpdate](#cdaodatabase__canupdate)|Returns nonzero if the <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> object is updatable (not read-only).|  
|[CDaoDatabase::Close](#cdaodatabase__close)|Closes the database connection.|  
|[CDaoDatabase::Create](#cdaodatabase__create)|Creates the underlying DAO database object and initializes the <CodeContentPlaceHolder>27\</CodeContentPlaceHolder> object.|  
...
|[CDaoDatabase::GetConnect](#cdaodatabase__getconnect)|Returns the connection string used to connect the <CodeContentPlaceHolder>28\</CodeContentPlaceHolder> object to a database. Used for ODBC.|  
...
|[CDaoDatabase::IsOpen](#cdaodatabase__isopen)|Returns nonzero if the <CodeContentPlaceHolder>29\</CodeContentPlaceHolder> object is currently connected to a database.|  
...
## Remarks  
 For information about the database formats supported, see the [GetName](../vs140/cdaoworkspace-class.md#cdaoworkspace__getname) member function. You can have one or more <CodeContentPlaceHolder>30\</CodeContentPlaceHolder> objects active at a time in a given "workspace," represented by a [CDaoWorkspace](../vs140/cdaoworkspace-class.md) object. The workspace maintains a collection of open database objects, called the Databases collection.  
  
> [!NOTE]
>  The MFC DAO database classes are distinct from the MFC database classes based on ODBC. All DAO database class names have the "CDao" prefix. Class <CodeContentPlaceHolder>31\</CodeContentPlaceHolder> supplies an interface similar to that of the ODBC class [CDatabase](../vs140/cdatabase-class.md). The main difference is that <CodeContentPlaceHolder>32\</CodeContentPlaceHolder> accesses the DBMS through Open Database Connectivity (ODBC) and an ODBC driver for that DBMS. <CodeContentPlaceHolder>33\</CodeContentPlaceHolder> accesses data through a Data Access Object (DAO) based on the Microsoft Jet database engine. In general, the MFC classes based on DAO are more capable than the MFC classes based on ODBC; the DAO-based classes can access data, including through ODBC drivers, via their own database engine. The DAO-based classes also support Data Definition Language (DDL) operations, such as adding tables via the classes, without having to call DAO directly.  
  
## Usage  
 You can create database objects implicitly, when you create recordset objects. But you can also create database objects explicitly. To use an existing database explicitly with <CodeContentPlaceHolder>34\</CodeContentPlaceHolder>, do either of the following:  
  
-   Construct a <CodeContentPlaceHolder>35\</CodeContentPlaceHolder> object, passing a pointer to an open [CDaoWorkspace](../vs140/cdaoworkspace-class.md) object.  
  
-   Or construct a <CodeContentPlaceHolder>36\</CodeContentPlaceHolder> object without specifying the workspace (MFC creates a temporary workspace object).  
  
 To create a new Microsoft Jet (.MDB) database, construct a <CodeContentPlaceHolder>37\</CodeContentPlaceHolder> object and call its [Create](#cdaodatabase__create) member function. Do                         *not* call **Open** after **Create**.  
  
 To open an existing database, construct a <CodeContentPlaceHolder>38\</CodeContentPlaceHolder> object and call its [Open](#cdaodatabase__open) member function.  
  
 Any of these techniques appends the DAO database object to the workspace's Databases collection and opens a connection to the data. When you then construct [CDaoRecordset](../vs140/cdaorecordset-class.md), [CDaoTableDef](../vs140/cdaotabledef-class.md), or [CDaoQueryDef](../vs140/cdaoquerydef-class.md) objects for operating on the connected database, pass the constructors for these objects a pointer to your <CodeContentPlaceHolder>39\</CodeContentPlaceHolder> object. When you finish using the connection, call the [Close](#cdaodatabase__close) member function and destroy the <CodeContentPlaceHolder>40\</CodeContentPlaceHolder> object. **Close** closes any recordsets you have not closed previously.  
  
## Transactions  
 Database transaction processing is supplied at the workspace level — see the [BeginTrans](../vs140/cdaoworkspace-class.md#cdaoworkspace__begintrans), [CommitTrans](../vs140/cdaoworkspace-class.md#cdaoworkspace__committrans), and [Rollback](../vs140/cdaoworkspace-class.md#cdaoworkspace__rollback) member functions of class <CodeContentPlaceHolder>41\</CodeContentPlaceHolder>.  
  
## ODBC Connections  
 The recommended way to work with ODBC data sources is to attach external tables to a Microsoft Jet (.MDB) database.  
  
## Collections  
 Each database maintains its own collections of tabledef, querydef, recordset, and relation objects. Class <CodeContentPlaceHolder>42\</CodeContentPlaceHolder> supplies member functions for manipulating these objects.  
...
##  \<a name="cdaodatabase__canupdate">\</a>  CDaoDatabase::CanUpdate  
 Call this member function to determine whether the <CodeContentPlaceHolder>44\</CodeContentPlaceHolder> object allows updates.  
  
<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>  
### Return Value  
 Nonzero if the <CodeContentPlaceHolder>45\</CodeContentPlaceHolder> object allows updates; otherwise 0, indicating either that you passed **TRUE** in <CodeContentPlaceHolder>46\</CodeContentPlaceHolder> when you opened the <CodeContentPlaceHolder>47\</CodeContentPlaceHolder> object or that the database itself is read-only. See the [Open](#cdaodatabase__open) member function.  
...
##  \<a name="cdaodatabase__cdaodatabase">\</a>  CDaoDatabase::CDaoDatabase  
 Constructs a <CodeContentPlaceHolder>48\</CodeContentPlaceHolder> object.  
  
<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>  
### Parameters  
 *pWorkspace*  
 A pointer to the <CodeContentPlaceHolder>49\</CodeContentPlaceHolder> object that will contain the new database object. If you accept the default value of **NULL**, the constructor creates a temporary <CodeContentPlaceHolder>50\</CodeContentPlaceHolder> object that uses the default DAO workspace. You can get a pointer to the workspace object via the [m_pWorkspace](#cdaodatabase__m_pworkspace) data member.  
...
### Remarks  
 ...
 When you finish with the object, you should call its [Close](#cdaodatabase__close) member function and then destroy the <CodeContentPlaceHolder>51\</CodeContentPlaceHolder> object.  
  
 You might find it convenient to embed the <CodeContentPlaceHolder>52\</CodeContentPlaceHolder> object in your document class.  
  
> [!NOTE]
>  A <CodeContentPlaceHolder>53\</CodeContentPlaceHolder> object is also created implicitly if you open a [CDaoRecordset](../vs140/cdaorecordset-class.md) object without passing a pointer to an existing <CodeContentPlaceHolder>54\</CodeContentPlaceHolder> object. This database object is closed when you close the recordset object.  
...
### Remarks  
 It is good practice to close these objects yourself before you call this member function. Closing a <CodeContentPlaceHolder>55\</CodeContentPlaceHolder> object removes it from the Databases collection in the associated [workspace](../vs140/cdaoworkspace-class.md). Because **Close** does not destroy the <CodeContentPlaceHolder>56\</CodeContentPlaceHolder> object, you can reuse the object by opening the same database or a different database.  
  
> [!CAUTION]
>  Call the [Update](../vs140/cdaorecordset-class.md#cdaorecordset__update) member function (if there are pending edits) and the **Close** member function on all open recordset objects before you close a database. If you exit a function that declares [CDaoRecordset](../vs140/cdaorecordset-class.md) or <CodeContentPlaceHolder>57\</CodeContentPlaceHolder> objects on the stack, the database is closed, any unsaved changes are lost, all pending transactions are rolled back, and any pending edits to your data are lost.  
...
##  \<a name="cdaodatabase__create">\</a>  CDaoDatabase::Create  
 To create a new Microsoft Jet (.MDB) database, call this member function after you construct a <CodeContentPlaceHolder>58\</CodeContentPlaceHolder> object.  
...
### Parameters  
 ...
 The name of the foreign table in the relation. If the table does not exist, MFC throws an exception of type <CodeContentPlaceHolder>65\</CodeContentPlaceHolder>.  
 ...
 *lpszField*  
 A pointer to a null-terminated string containing the name of a field in the primary table (named by <CodeContentPlaceHolder>67\</CodeContentPlaceHolder>).  
  
 *lpszForeignField*  
 A pointer to a null-terminated string containing the name of a field in the foreign table (named by <CodeContentPlaceHolder>68\</CodeContentPlaceHolder>).  
...
### Remarks  
 ...
 This action creates an underlying DAO relation object, but this is an MFC implementation detail since MFC's encapsulation of relation objects is contained within class <CodeContentPlaceHolder>69\</CodeContentPlaceHolder>. MFC does not supply a class for relations.  
...
##  \<a name="cdaodatabase__deletequerydef">\</a>  CDaoDatabase::DeleteQueryDef  
 Call this member function to delete the specified querydef — saved query — from the <CodeContentPlaceHolder>70\</CodeContentPlaceHolder> object's QueryDefs collection.  
...
### Remarks  
 ...
 For information about creating querydef objects, see class [CDaoQueryDef](../vs140/cdaoquerydef-class.md). A querydef object becomes associated with a particular <CodeContentPlaceHolder>72\</CodeContentPlaceHolder> object when you construct the <CodeContentPlaceHolder>73\</CodeContentPlaceHolder> object, passing it a pointer to the database object.  
...
##  \<a name="cdaodatabase__deletetabledef">\</a>  CDaoDatabase::DeleteTableDef  
 Call this member function to delete the specified table and all of its data from the <CodeContentPlaceHolder>75\</CodeContentPlaceHolder> object's TableDefs collection.  
...
>  Be very careful not to delete system tables.  
 ...
 For information about creating tabledef objects, see class [CDaoTableDef](../vs140/cdaotabledef-class.md). A tabledef object becomes associated with a particular <CodeContentPlaceHolder>77\</CodeContentPlaceHolder> object when you construct the <CodeContentPlaceHolder>78\</CodeContentPlaceHolder> object, passing it a pointer to the database object.  
...
> [!TIP]
>  Given a syntactically correct SQL statement and proper permissions, the **Execute** member function will not fail even if not a single row can be modified or deleted. Therefore, always use the **dbFailOnError** option when using the **Execute** member function to run an update or delete query. This option causes MFC to throw an exception of type [CDaoException](../vs140/cdaoexception-class.md) and rolls back all successful changes if any of the records affected are locked and cannot be updated or deleted. Note that you can always call <CodeContentPlaceHolder>81\</CodeContentPlaceHolder> to see how many records were affected.  
  
 Call the [GetRecordsAffected](#cdaodatabase__getrecordsaffected) member function of the database object to determine the number of records affected by the most recent **Execute** call. For example, <CodeContentPlaceHolder>82\</CodeContentPlaceHolder> returns information about the number of records deleted, updated, or inserted when executing an action query. The count returned will not reflect changes in related tables when cascade updates or deletes are in effect.  
  
 **Execute** does not return a recordset. Using **Execute** on a query that selects records causes MFC to throw an exception of type <CodeContentPlaceHolder>83\</CodeContentPlaceHolder>. (There is no <CodeContentPlaceHolder>84\</CodeContentPlaceHolder> member function analogous to <CodeContentPlaceHolder>85\</CodeContentPlaceHolder>.)  
  
##  \<a name="cdaodatabase__getconnect">\</a>  CDaoDatabase::GetConnect  
 Call this member function to retrieve the connection string used to connect the <CodeContentPlaceHolder>86\</CodeContentPlaceHolder> object to an ODBC or ISAM database.  
...
### Remarks  
 <CodeContentPlaceHolder>87\</CodeContentPlaceHolder> is useful if you need to loop through all querydefs in the QueryDefs collection. To obtain information about a given query in the collection, see [GetQueryDefInfo](#cdaodatabase__getquerydefinfo).  
...
-   <CodeContentPlaceHolder>90\</CodeContentPlaceHolder> (Default) Name, Type  
  
-   <CodeContentPlaceHolder>91\</CodeContentPlaceHolder> Primary information plus: Date Created, Date of Last Update, Returns Records, Updatable  
  
-   <CodeContentPlaceHolder>92\</CodeContentPlaceHolder> Primary and secondary information plus: SQL, Connect, ODBCTimeout  
...
### Remarks  
 ...
 For a description of the information returned in                         *querydefinfo*, see the [CDaoQueryDefInfo](../vs140/cdaoquerydefinfo-structure.md) structure. This structure has members that correspond to the items of information listed above in the description of <CodeContentPlaceHolder>94\</CodeContentPlaceHolder>. If you request one level of information, you get any prior levels of information as well.  
...
### Remarks  
 An operation might time out due to network access problems, excessive query processing time, and so on. While the setting is in effect, it affects all open, add new, update, and delete operations on any recordsets associated with this <CodeContentPlaceHolder>95\</CodeContentPlaceHolder> object. You can change the current timeout setting by calling [SetQueryTimeout](#cdaodatabase__setquerytimeout). Changing the query timeout value for a recordset after opening does not change the value for the recordset. For example, subsequent [Move](../vs140/cdaorecordset-class.md#cdaorecordset__move) operations do not use the new value. The default value is initially set when the database engine is initialized.  
...
-   <CodeContentPlaceHolder>98\</CodeContentPlaceHolder> (Default) Name, Table, Foreign Table  
  
-   <CodeContentPlaceHolder>99\</CodeContentPlaceHolder> Attributes, Field Information  
...
### Remarks  
 Two versions of this function provide access either by index or by name. For a description of the information returned in                         *relinfo*, see the [CDaoRelationInfo](../vs140/cdaorelationinfo-structure.md) structure. This structure has members that correspond to the items of information listed above in the description of <CodeContentPlaceHolder>101\</CodeContentPlaceHolder>. If you request information at one level, you also get information at any prior levels as well.  
...
### Remarks  
 <CodeContentPlaceHolder>102\</CodeContentPlaceHolder> is useful if you need to loop through all tabledefs in the database's TableDefs collection. To obtain information about a given table in the collection, see [GetTableDefInfo](#cdaodatabase__gettabledefinfo).  
...
-   <CodeContentPlaceHolder>106\</CodeContentPlaceHolder> (Default) Name, Updatable, Attributes  
  
-   <CodeContentPlaceHolder>107\</CodeContentPlaceHolder> Primary information plus: Date Created, Date Last Updated, Source Table Name, Connect  
  
-   <CodeContentPlaceHolder>108\</CodeContentPlaceHolder> Primary and secondary information plus: Validation Rule, Validation Text, Record Count  
...
### Remarks  
 ...
 For a description of the information returned in <CodeContentPlaceHolder>110\</CodeContentPlaceHolder>, see the [CDaoTableDefInfo](../vs140/cdaotabledefinfo-structure.md) structure. This structure has members that correspond to the items of information listed above in the description of <CodeContentPlaceHolder>111\</CodeContentPlaceHolder>. If you request information at one level, you get information for any prior levels as well.  
  
> [!NOTE]
>  The <CodeContentPlaceHolder>112\</CodeContentPlaceHolder> option provides information that can be slow to obtain. In this case, counting the records in the table could be very time consuming if there are many records.  
...
##  \<a name="cdaodatabase__isopen">\</a>  CDaoDatabase::IsOpen  
 Call this member function to determine whether the <CodeContentPlaceHolder>113\</CodeContentPlaceHolder> object is currently open on a database.  
  
<CodeContentPlaceHolder>22\</CodeContentPlaceHolder>  
### Return Value  
 Nonzero if the <CodeContentPlaceHolder>114\</CodeContentPlaceHolder> object is currently open; otherwise 0.  
  
### Remarks  
  
##  \<a name="cdaodatabase__m_pdaodatabase">\</a>  CDaoDatabase::m_pDAODatabase  
 Contains a pointer to the OLE interface for the DAO database object underlying the <CodeContentPlaceHolder>115\</CodeContentPlaceHolder> object.  
...
##  \<a name="cdaodatabase__open">\</a>  CDaoDatabase::Open  
 You must call this member function to initialize a newly constructed <CodeContentPlaceHolder>116\</CodeContentPlaceHolder> object that represents an existing database.  
...
### Parameters  
 ...
 Some considerations apply when using <CodeContentPlaceHolder>118\</CodeContentPlaceHolder>. If it:  
...
-   Otherwise does not refer to an existing database or valid ODBC data source name, MFC throws an exception of type <CodeContentPlaceHolder>119\</CodeContentPlaceHolder>.  
...
-   If you are opening a Microsoft Jet (.MDB) database, use the <CodeContentPlaceHolder>123\</CodeContentPlaceHolder> parameter and pass an empty string for the <CodeContentPlaceHolder>124\</CodeContentPlaceHolder> parameter or pass a password string of the form ";PWD=password" if the database is password-protected (.MDB databases only).  
  
-   If you are opening an ODBC data source, pass a valid ODBC connection string in <CodeContentPlaceHolder>125\</CodeContentPlaceHolder> and an empty string in <CodeContentPlaceHolder>126\</CodeContentPlaceHolder>.  
...
### Remarks  
 An operation might time out because of network access problems, excessive query processing time, and so on. Call <CodeContentPlaceHolder>131\</CodeContentPlaceHolder> before opening your recordset or before calling the recordset's [AddNew](../vs140/cdaorecordset-class.md#cdaorecordset__addnew), [Update](../vs140/cdaorecordset-class.md#cdaorecordset__update), or [Delete](../vs140/cdaorecordset-class.md#cdaorecordset__delete) member functions if you want to change the query timeout value. The setting affects all subsequent [Open](../vs140/cdaorecordset-class.md#cdaorecordset__open), <CodeContentPlaceHolder>132\</CodeContentPlaceHolder>, **Update**, and **Delete** calls to any recordsets associated with this <CodeContentPlaceHolder>133\</CodeContentPlaceHolder> object. Changing the query timeout value for a recordset after opening does not change the value for the recordset. For example, subsequent [Move](../vs140/cdaorecordset-class.md#cdaorecordset__move) operations do not use the new value.  
