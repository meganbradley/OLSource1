...
These classes handle common OLE tasks by implementing a number of standard OLE dialog boxes. They also provide a consistent user interface for OLE functionality.  
 ...
 Used by the framework to contain common implementations for all OLE dialog boxes. All dialog box classes in the user-interface category are derived from this base class. <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> cannot be used directly.  
 ...
 Displays the Server Busy and Server Not Responding dialog boxes, the standard user interface for handling calls to busy applications. Usually displayed automatically by the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> implementation.  
