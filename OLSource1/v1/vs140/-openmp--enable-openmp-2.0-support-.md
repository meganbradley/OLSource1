...
Causes the compiler to process <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>[omp](../vs140/omp.md).  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Remarks  
 <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is used to specify [Directives](../vs140/openmp-directives.md) and [Clauses](../vs140/openmp-clauses.md). If **/openmp** is not specified in a compilation, the compiler ignores OpenMP clauses and directives. [OpenMP Function](../vs140/openmp-functions.md) calls are processed by the compiler even if **/openmp** is not specified.  
  
 An application compiled with **/openmp** and using the [OpenMP Libraries](../vs140/openmp-libraries.md) can only be run on Windows 2000 or later operating systems.  
  
 Applications compiled with **/openmp** and **/clr** can only be run in a single application domain process; multiple application domains are not supported. That is, when the module constructor (.cctor) is run, it will detect the process is compiled with **/openmp** and if the application is being loaded into a non-default runtime. For more information, see [appdomain](../vs140/appdomain.md), [/clr (Common Language Runtime Compilation)](../vs140/-clr--common-language-runtime-compilation-.md), and [Initialization of Mixed Assemblies](../vs140/initialization-of-mixed-assemblies.md).  
...
-   If you pass your **/clr** application to a utility, such as regasm.exe ([Assembly Registration Tool (Regasm.exe)](assetId:///e190e342-36ef-4651-a0b4-0e8c2c0281cb)), which loads its target assemblies into a non-default application domain.  
