...
### Remarks  
 ...
 When you declare your derived recordset class with ClassWizard, the wizard writes an override of <CodeContentPlaceHolder>136\</CodeContentPlaceHolder> for you, which resembles the following example:  
...
>  When you call <CodeContentPlaceHolder>175\</CodeContentPlaceHolder>, the Microsoft Jet database engine fully populates your recordset before beginning the search, if this has not already been done. The first search may take longer than subsequent searches.  
 ...
 Using one of the Find operations is not the same as calling **MoveFirst** or <CodeContentPlaceHolder>176\</CodeContentPlaceHolder>, however, which simply makes the first or next record current without specifying a condition. You can follow a Find operation with a Move operation.  
...
>  If you edit the current record, be sure you save the changes by calling the **Update** member function before you move to another record. If you move to another record without updating, your changes are lost without warning.  
 ...
 Using one of the Find operations is not the same as calling **MoveFirst** or <CodeContentPlaceHolder>183\</CodeContentPlaceHolder>, however, which simply makes the first or next record current without specifying a condition. You can follow a Find operation with a Move operation.  
...
>  If you edit the current record, be sure you save the changes by calling the **Update** member function before you move to another record. If you move to another record without updating, your changes are lost without warning.  
 ...
 Using one of the Find operations is not the same as calling **MoveFirst** or <CodeContentPlaceHolder>190\</CodeContentPlaceHolder>, however, which simply makes the first or next record current without specifying a condition. You can follow a Find operation with a Move operation.  
...
>  If you edit the current record, be sure you save the changes by calling the **Update** member function before you move to another record. If you move to another record without updating, your changes are lost without warning.  
 ...
 Using one of the Find operations is not the same as calling **MoveFirst** or <CodeContentPlaceHolder>197\</CodeContentPlaceHolder>, however, which simply makes the first or next record current without specifying a condition. You can follow a Find operation with a Move operation.  
...
### Remarks  
 ...
 The string returned by <CodeContentPlaceHolder>262\</CodeContentPlaceHolder> is typically different from any string you may have passed to the recordset in the <CodeContentPlaceHolder>263\</CodeContentPlaceHolder> parameter to the [Open](#cdaorecordset__open) member function. This is because the recordset constructs a full SQL statement based on what you passed to **Open**, what you specified with ClassWizard, and what you may have specified in the [m_strFilter](#cdaorecordset__m_strfilter) and [m_strSort](#cdaorecordset__m_strsort) data members.  
...
-   Any **Delete** call, even if it removes the only remaining record from a recordset, will not change the value of <CodeContentPlaceHolder>286\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>287\</CodeContentPlaceHolder>.  
...
>  You don't need to check the deleted status for records in a snapshot or table-type recordset. Because records cannot be deleted from a snapshot, there is no need to call <CodeContentPlaceHolder>302\</CodeContentPlaceHolder>. For table-type recordsets, deleted records are actually removed from the recordset. Once a record has been deleted, either by you, another user, or in another recordset, you cannot scroll back to that record. Therefore, there is no need to call <CodeContentPlaceHolder>303\</CodeContentPlaceHolder>.  
 ...
 When you delete a record from a dynaset, it is removed from the recordset and you cannot scroll back to that record. However, if a record in a dynaset is deleted either by another user or in another recordset based on the same table, <CodeContentPlaceHolder>304\</CodeContentPlaceHolder> will return **TRUE** when you later scroll to that record.  
...
-   Any **Delete** call, even if it removes the only remaining record from a recordset, will not change the value of <CodeContentPlaceHolder>323\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>324\</CodeContentPlaceHolder>.  
 ...
 This table shows which Move operations are allowed with different combinations of <CodeContentPlaceHolder>325\</CodeContentPlaceHolder>/ <CodeContentPlaceHolder>326\</CodeContentPlaceHolder>.  
...
### Remarks  
 ...
 <CodeContentPlaceHolder>341\</CodeContentPlaceHolder> is implemented through <CodeContentPlaceHolder>342\</CodeContentPlaceHolder>.  
...
>  If you call any of the **Move** functions while the current record is being updated or added, the updates are lost without warning.  
 ...
 When you call **Move** on a forward-only scrolling snapshot, the <CodeContentPlaceHolder>385\</CodeContentPlaceHolder> parameter must be a positive integer and bookmarks are not allowed, so you can move forward only.  
 ...
 *pTableDef*  
 A pointer to a [CDaoTableDef](../vs140/cdaotabledef-class.md) object. This version is valid only for table-type recordsets. When using this option, the <CodeContentPlaceHolder>424\</CodeContentPlaceHolder> pointer used to construct the <CodeContentPlaceHolder>425\</CodeContentPlaceHolder> is not used; rather, the database in which the tabledef resides is used.  
  
 *pQueryDef*  
 A pointer to a [CDaoQueryDef](../vs140/cdaoquerydef-class.md) object. This version is valid only for dynaset-type and snapshot-type recordsets. When using this option, the <CodeContentPlaceHolder>426\</CodeContentPlaceHolder> pointer used to construct the <CodeContentPlaceHolder>427\</CodeContentPlaceHolder> is not used; rather, the database in which the querydef resides is used.  
  
### Remarks  
 Before calling **Open**, you must construct the recordset object. There are several ways to do this:  
  
-   When you construct the recordset object, pass a pointer to a <CodeContentPlaceHolder>428\</CodeContentPlaceHolder> object that is already open.  
  
-   When you construct the recordset object, pass a pointer to a <CodeContentPlaceHolder>429\</CodeContentPlaceHolder> object that is not open. The recordset opens a <CodeContentPlaceHolder>430\</CodeContentPlaceHolder> object, but will not close it when the recordset object closes.  
  
-   When you construct the recordset object, pass a **NULL** pointer. The recordset object calls <CodeContentPlaceHolder>431\</CodeContentPlaceHolder> to get the name of the Microsoft Access .MDB file to open. The recordset then opens a <CodeContentPlaceHolder>432\</CodeContentPlaceHolder> object and keeps it open as long as the recordset is open. When you call **Close** on the recordset, the <CodeContentPlaceHolder>433\</CodeContentPlaceHolder> object is also closed.  
  
    > [!NOTE]
    >  When the recordset opens the <CodeContentPlaceHolder>434\</CodeContentPlaceHolder> object, it opens the data source with nonexclusive access.  
  
 For the version of **Open** that uses the <CodeContentPlaceHolder>435\</CodeContentPlaceHolder> parameter, once the recordset is open you can retrieve records in one of several ways. The first option is to have DFX functions in your <CodeContentPlaceHolder>436\</CodeContentPlaceHolder>. The second option is to use dynamic binding by calling the <CodeContentPlaceHolder>437\</CodeContentPlaceHolder> member function. These options can be implemented separately or in combination. If they are combined, you will have to pass in the SQL statement yourself on the call to **Open**.  
  
 When you use the second version of **Open** where you pass in a <CodeContentPlaceHolder>438\</CodeContentPlaceHolder> object, the resulting columns will be available for you to bind via <CodeContentPlaceHolder>439\</CodeContentPlaceHolder> and the DFX mechanism, and/or bind dynamically via <CodeContentPlaceHolder>440\</CodeContentPlaceHolder>.  
  
> [!NOTE]
>  You can only call **Open** using a <CodeContentPlaceHolder>441\</CodeContentPlaceHolder> object for table-type recordsets.  
  
 When you use the third version of **Open** where you pass in a <CodeContentPlaceHolder>442\</CodeContentPlaceHolder> object, that query will be executed, and the resulting columns will be available for you to bind via <CodeContentPlaceHolder>443\</CodeContentPlaceHolder> and the DFX mechanism, and/or bind dynamically via <CodeContentPlaceHolder>444\</CodeContentPlaceHolder>.  
  
> [!NOTE]
>  You can only call **Open** using a <CodeContentPlaceHolder>445\</CodeContentPlaceHolder> object for dynaset-type and snapshot-type recordsets.  
  
 For the first version of **Open** that uses the <CodeContentPlaceHolder>446\</CodeContentPlaceHolder> parameter, records are selected based on criteria shown in the following table.  
  
|Value of the <CodeContentPlaceHolder>447\</CodeContentPlaceHolder> parameter|Records selected are determined by|Example|  
|--------------------------------------|----------------------------------------|-------------|  
|**NULL**|The string returned by <CodeContentPlaceHolder>448\</CodeContentPlaceHolder>.||  
|A comma-separated list of one or more tabledefs and/or querydef names.|All columns represented in the <CodeContentPlaceHolder>449\</CodeContentPlaceHolder>.|<CodeContentPlaceHolder>450\</CodeContentPlaceHolder>|  
|**SELECT** column-list **FROM** table-list|The specified columns from the specified tabledef(s) and/or querydef(s).|<CodeContentPlaceHolder>451\</CodeContentPlaceHolder>\<br />\<br /> <CodeContentPlaceHolder>452\</CodeContentPlaceHolder>|  
  
 The usual procedure is to pass **NULL** to **Open**; in that case, **Open** calls <CodeContentPlaceHolder>453\</CodeContentPlaceHolder>, an overridable member function that ClassWizard generates when creating a <CodeContentPlaceHolder>454\</CodeContentPlaceHolder>-derived class. This value gives the tabledef(s) and/or querydef name(s) you specified in ClassWizard. You can instead specify other information in the <CodeContentPlaceHolder>455\</CodeContentPlaceHolder> parameter.  
  
 Whatever you pass, **Open** constructs a final SQL string for the query (the string may have SQL **WHERE** and **ORDERBY** clauses appended to the <CodeContentPlaceHolder>456\</CodeContentPlaceHolder> string you passed) and then executes the query. You can examine the constructed string by calling <CodeContentPlaceHolder>457\</CodeContentPlaceHolder> after calling **Open**.  
 ...
 If you want to set options for the recordset, such as a filter or sort, set <CodeContentPlaceHolder>458\</CodeContentPlaceHolder> or **m_strFilter** after you construct the recordset object but before you call **Open**. If you want to refresh the records in the recordset after the recordset is already open, call **Requery**.  
...
### Remarks  
 ...
 If the attempt to rebuild the recordset fails, the recordset is closed. Before you call **Requery**, you can determine whether the recordset can be requeried by calling the [CanRestart](#cdaorecordset__canrestart) member function. <CodeContentPlaceHolder>462\</CodeContentPlaceHolder> does not guarantee that **Requery** will succeed.  
...
>  Calling [Requery](#cdaorecordset__requery) changes DAO bookmarks.  
 ...
 You can't call **Requery** on a dynaset-type or snapshot-type recordset if calling <CodeContentPlaceHolder>463\</CodeContentPlaceHolder> returns 0, nor can you use it on a table-type recordset.  
  
 If both <CodeContentPlaceHolder>464\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>465\</CodeContentPlaceHolder> return nonzero after you call **Requery**, the query didn't return any records and the recordset will contain no data.  
 ...
 *pKey2*  
 A pointer to a <CodeContentPlaceHolder>468\</CodeContentPlaceHolder> whose value corresponds to the second field in the index, if any. Defaults to **NULL**.  
  
 *pKey3*  
 A pointer to a <CodeContentPlaceHolder>469\</CodeContentPlaceHolder> whose value corresponds to the third field in the index, if any. Defaults to **NULL**.  
...
> [!NOTE]
>  Do not specify wildcards in the keys. Wildcards will cause <CodeContentPlaceHolder>470\</CodeContentPlaceHolder> to return no matching records.  
  
### Return Value  
 Nonzero if matching records are found, otherwise 0.  
  
### Remarks  
 Use the second (array) version of <CodeContentPlaceHolder>471\</CodeContentPlaceHolder> to handle indexes of four fields or more.  
  
 <CodeContentPlaceHolder>472\</CodeContentPlaceHolder> enables high-performance index searching on table-type recordsets. You must set the current index by calling <CodeContentPlaceHolder>473\</CodeContentPlaceHolder> before calling <CodeContentPlaceHolder>474\</CodeContentPlaceHolder>. If the index identifies a nonunique key field or fields, <CodeContentPlaceHolder>475\</CodeContentPlaceHolder> locates the first record that satisfies the criteria. If you do not set an index, an exception is thrown.  
  
 Note that if you are not creating a UNICODE recordset, the <CodeContentPlaceHolder>476\</CodeContentPlaceHolder> objects must be explicitly declared ANSI. This can be done by using the [COleVariant::COleVariant](../vs140/colevariant-class.md#colevariant__colevariant)**(** <CodeContentPlaceHolder>477\</CodeContentPlaceHolder>**,** <CodeContentPlaceHolder>478\</CodeContentPlaceHolder> **)** form of constructor with <CodeContentPlaceHolder>479\</CodeContentPlaceHolder> set to <CodeContentPlaceHolder>480\</CodeContentPlaceHolder> (ANSI) or by using the **COleVariant** function [SetString](../vs140/colevariant-class.md#colevariant__setstring)**(** <CodeContentPlaceHolder>481\</CodeContentPlaceHolder>**,** <CodeContentPlaceHolder>482\</CodeContentPlaceHolder> **)** with <CodeContentPlaceHolder>483\</CodeContentPlaceHolder> set to <CodeContentPlaceHolder>484\</CodeContentPlaceHolder>.  
  
 When you call <CodeContentPlaceHolder>485\</CodeContentPlaceHolder>, you pass one or more key values and a comparison operator ("<", "\<=", "=", ">=", or ">"). <CodeContentPlaceHolder>486\</CodeContentPlaceHolder> searches through the specified key fields and locates the first record that satisfies the criteria specified by <CodeContentPlaceHolder>487\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>488\</CodeContentPlaceHolder>. Once found, <CodeContentPlaceHolder>489\</CodeContentPlaceHolder> returns nonzero, and makes that record current. If <CodeContentPlaceHolder>490\</CodeContentPlaceHolder> fails to locate a match, <CodeContentPlaceHolder>491\</CodeContentPlaceHolder> returns zero, and the current record is undefined. When using DAO directly, you must explicitly check the NoMatch property.  
  
 If <CodeContentPlaceHolder>492\</CodeContentPlaceHolder> is "=", ">=", or ">", <CodeContentPlaceHolder>493\</CodeContentPlaceHolder> starts at the beginning of the index. If <CodeContentPlaceHolder>494\</CodeContentPlaceHolder> is "<" or "<=", <CodeContentPlaceHolder>495\</CodeContentPlaceHolder> starts at the end of the index and searches backward unless there are duplicate index entries at the end. In this case, <CodeContentPlaceHolder>496\</CodeContentPlaceHolder> starts at an arbitrary entry among the duplicate index entries at the end of the index.  
  
 There does not have to be a current record when you use <CodeContentPlaceHolder>497\</CodeContentPlaceHolder>.  
 ...
 You cannot call <CodeContentPlaceHolder>498\</CodeContentPlaceHolder> on an attached table of any type because attached tables must be opened as dynaset-type or snapshot-type recordsets. However, if you call <CodeContentPlaceHolder>499\</CodeContentPlaceHolder> to directly open an installable ISAM database, you can call <CodeContentPlaceHolder>500\</CodeContentPlaceHolder> on tables in that database, although the performance may be slow.  
...
### Remarks  
 Calling <CodeContentPlaceHolder>501\</CodeContentPlaceHolder> enables you to position the current record pointer to a specific record based on its ordinal position in a dynaset-type or snapshot-type recordset. You can also determine the current record number by calling [GetAbsolutePosition](#cdaorecordset__getabsoluteposition).  
  
> [!NOTE]
>  This member function is valid only for dynaset-type and snapshot-type recordsets.  
  
 The AbsolutePosition property value of the underlying DAO object is zero-based; a setting of 0 refers to the first record in the recordset. Setting a value greater than the number of populated records causes MFC to throw an exception. You can determine the number of populated records in the recordset by calling the <CodeContentPlaceHolder>502\</CodeContentPlaceHolder> member function.  
...
### Remarks  
 When a recordset object is created or opened, each of its records already has a unique bookmark. You can retrieve the bookmark for the current record by calling <CodeContentPlaceHolder>504\</CodeContentPlaceHolder> and saving the value to a <CodeContentPlaceHolder>505\</CodeContentPlaceHolder> object. You can later return to that record by calling <CodeContentPlaceHolder>506\</CodeContentPlaceHolder> using the saved bookmark value.  
  
> [!NOTE]
>  Calling [Requery](#cdaorecordset__requery) changes DAO bookmarks.  
  
 Note that if you are not creating a UNICODE recordset, the <CodeContentPlaceHolder>507\</CodeContentPlaceHolder> object must be explicitly declared ANSI. This can be done by using the [COleVariant::COleVariant](../vs140/colevariant-class.md#colevariant__colevariant)**(** <CodeContentPlaceHolder>508\</CodeContentPlaceHolder>**,** <CodeContentPlaceHolder>509\</CodeContentPlaceHolder> **)** form of constructor with <CodeContentPlaceHolder>510\</CodeContentPlaceHolder> set to <CodeContentPlaceHolder>511\</CodeContentPlaceHolder> (ANSI) or by using the **COleVariant** function [SetString](../vs140/colevariant-class.md#colevariant__setstring)**(** <CodeContentPlaceHolder>512\</CodeContentPlaceHolder>**,** <CodeContentPlaceHolder>513\</CodeContentPlaceHolder> **)** with <CodeContentPlaceHolder>514\</CodeContentPlaceHolder> set to <CodeContentPlaceHolder>515\</CodeContentPlaceHolder>.  
...
### Remarks  
 ...
 Any ODBC data source, such as an attached table, can have a local cache. To create the cache, open a recordset object from the remote data source, call the <CodeContentPlaceHolder>517\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>518\</CodeContentPlaceHolder> member functions, and then call the <CodeContentPlaceHolder>519\</CodeContentPlaceHolder> member function or step through the records by using one of the Move operations. The <CodeContentPlaceHolder>520\</CodeContentPlaceHolder> parameter of the <CodeContentPlaceHolder>521\</CodeContentPlaceHolder> member function can be based on the number of records your application can work with at one time. For example, if you are using a recordset as the source of the data to be displayed on screen, you could pass the <CodeContentPlaceHolder>522\</CodeContentPlaceHolder> parameter as 20 to display 20 records at one time.  
...
### Remarks  
 You can use the bookmark value of any record for the <CodeContentPlaceHolder>524\</CodeContentPlaceHolder> parameter of the <CodeContentPlaceHolder>525\</CodeContentPlaceHolder> member function. Make the record you want to start the cache with the current record, establish a bookmark for that record using [SetBookmark](#cdaorecordset__setbookmark), and pass the bookmark value as the parameter for the <CodeContentPlaceHolder>526\</CodeContentPlaceHolder> member function.  
  
 The Microsoft Jet database engine requests records within the cache range from the cache, and it requests records outside the cache range from the server.  
  
 Records retrieved from the cache do not reflect changes made concurrently to the source data by other users.  
  
 To force an update of all the cached data, pass the <CodeContentPlaceHolder>527\</CodeContentPlaceHolder> parameter of <CodeContentPlaceHolder>528\</CodeContentPlaceHolder> as 0, call <CodeContentPlaceHolder>529\</CodeContentPlaceHolder> again with the size of the cache you originally requested, and then call the <CodeContentPlaceHolder>530\</CodeContentPlaceHolder> member function.  
  
 Note that if you are not creating a UNICODE recordset, the <CodeContentPlaceHolder>531\</CodeContentPlaceHolder> object must be explicitly declared ANSI. This can be done by using the [COleVariant::COleVariant](../vs140/colevariant-class.md#colevariant__colevariant)**(** <CodeContentPlaceHolder>532\</CodeContentPlaceHolder>**,** <CodeContentPlaceHolder>533\</CodeContentPlaceHolder> **)** form of constructor with <CodeContentPlaceHolder>534\</CodeContentPlaceHolder> set to <CodeContentPlaceHolder>535\</CodeContentPlaceHolder> (ANSI) or by using the **COleVariant** function [SetString](../vs140/colevariant-class.md#colevariant__setstring)**(** <CodeContentPlaceHolder>536\</CodeContentPlaceHolder>**,** <CodeContentPlaceHolder>537\</CodeContentPlaceHolder> **)** with <CodeContentPlaceHolder>538\</CodeContentPlaceHolder> set to <CodeContentPlaceHolder>539\</CodeContentPlaceHolder>.  
...
>  Call this member function only after you have called [Edit](#cdaorecordset__edit) or [AddNew](#cdaorecordset__addnew).  
 ...
 Using **NULL** for the first argument of the function will apply the function to all **outputColumn** fields, not **param** fields in <CodeContentPlaceHolder>544\</CodeContentPlaceHolder>. For instance, the call  
  
 [!code[NVC_MFCDatabase#6](../vs140/codesnippet/CPP/cdaorecordset-class_6.cpp)]  
 ...
 [!code[NVC_MFCDatabase#7](../vs140/codesnippet/CPP/cdaorecordset-class_7.cpp)]  
  
 This means you cannot set all **param** fields to **NULL**, as you can with **outputColumn** fields.  
  
 <CodeContentPlaceHolder>545\</CodeContentPlaceHolder> is implemented through <CodeContentPlaceHolder>546\</CodeContentPlaceHolder>.  
...
### Remarks  
 <CodeContentPlaceHolder>549\</CodeContentPlaceHolder> is used for fields bound in the <CodeContentPlaceHolder>550\</CodeContentPlaceHolder> mechanism.  
 ...
 If you are using the double-buffering mechanism, for example, if you specifically wish to designate a field of the current record as not having a value, call <CodeContentPlaceHolder>551\</CodeContentPlaceHolder> with <CodeContentPlaceHolder>552\</CodeContentPlaceHolder> set to **TRUE** to flag it as Null. If a field was previously marked Null and you now want to give it a value, simply set its new value. You do not have to remove the Null flag with <CodeContentPlaceHolder>553\</CodeContentPlaceHolder>. To determine whether the field is allowed to be Null, call [IsFieldNullable](#cdaorecordset__isfieldnullable).  
...
>  Call this member function only after you have called [Edit](#cdaorecordset__edit) or [AddNew](#cdaorecordset__addnew).  
 ...
 Using **NULL** for the first argument of the function will apply the function only to **outputColumn** fields, not **param** fields in <CodeContentPlaceHolder>554\</CodeContentPlaceHolder>. For instance, the call  
  
 [!code[NVC_MFCDatabase#8](../vs140/codesnippet/CPP/cdaorecordset-class_8.cpp)]  
...
### Remarks  
 Use <CodeContentPlaceHolder>559\</CodeContentPlaceHolder> and [GetFieldValue](#cdaorecordset__getfieldvalue) to dynamically bind fields at run time rather than statically binding columns using the [DoFieldExchange](#cdaorecordset__dofieldexchange) mechanism.  
  
 Note that if you are not creating a UNICODE recordset, you must either use a form of <CodeContentPlaceHolder>560\</CodeContentPlaceHolder> that does not contain a <CodeContentPlaceHolder>561\</CodeContentPlaceHolder> parameter, or the <CodeContentPlaceHolder>562\</CodeContentPlaceHolder> object must be explicitly declared ANSI. This can be done by using the [COleVariant::COleVariant](../vs140/colevariant-class.md#colevariant__colevariant)**(** <CodeContentPlaceHolder>563\</CodeContentPlaceHolder>**,** <CodeContentPlaceHolder>564\</CodeContentPlaceHolder> **)** form of constructor with <CodeContentPlaceHolder>565\</CodeContentPlaceHolder> set to <CodeContentPlaceHolder>566\</CodeContentPlaceHolder> (ANSI) or by using the **COleVariant** function [SetString](../vs140/colevariant-class.md#colevariant__setstring)**(** <CodeContentPlaceHolder>567\</CodeContentPlaceHolder>**,** <CodeContentPlaceHolder>568\</CodeContentPlaceHolder> **)** with <CodeContentPlaceHolder>569\</CodeContentPlaceHolder> set to <CodeContentPlaceHolder>570\</CodeContentPlaceHolder>.  
...
### Remarks  
 ...
 If a page is locked, no other user can edit records on the same page. If you call <CodeContentPlaceHolder>573\</CodeContentPlaceHolder> and pass a nonzero value and another user already has the page locked, an exception is thrown when you call **Edit**. Other users can read data from locked pages.  
  
 If you call <CodeContentPlaceHolder>574\</CodeContentPlaceHolder> with a zero value and later call **Update** while the page is locked by another user, an exception occurs. To see the changes made to your record by another user (and lose your changes), call the <CodeContentPlaceHolder>575\</CodeContentPlaceHolder> member function with the bookmark value of the current record.  
...
### Remarks  
 ...
 Specify the value to set as a <CodeContentPlaceHolder>579\</CodeContentPlaceHolder> object. For information about setting the desired value and type in your <CodeContentPlaceHolder>580\</CodeContentPlaceHolder> object, see class [COleVariant](../vs140/colevariant-class.md). Note that if you are not creating a UNICODE recordset, the <CodeContentPlaceHolder>581\</CodeContentPlaceHolder> object must be explicitly declared ANSI. This can be done by using the [COleVariant::COleVariant](../vs140/colevariant-class.md#colevariant__colevariant)**(** <CodeContentPlaceHolder>582\</CodeContentPlaceHolder>**,** <CodeContentPlaceHolder>583\</CodeContentPlaceHolder> **)** form of constructor with <CodeContentPlaceHolder>584\</CodeContentPlaceHolder> set to <CodeContentPlaceHolder>585\</CodeContentPlaceHolder> (ANSI) or by using the **COleVariant** function [SetString](../vs140/colevariant-class.md#colevariant__setstring)**(** <CodeContentPlaceHolder>586\</CodeContentPlaceHolder>**,** <CodeContentPlaceHolder>587\</CodeContentPlaceHolder> **)** with <CodeContentPlaceHolder>588\</CodeContentPlaceHolder> set to <CodeContentPlaceHolder>589\</CodeContentPlaceHolder>.  
...
### Parameters  
 <CodeContentPlaceHolder>590\</CodeContentPlaceHolder>  
 The index of the field in the recordset, for lookup by zero-based index.  
...
### Remarks  
 When working with a dynaset-type or snapshot-type recordset, first populate the recordset by moving to the last record before you call <CodeContentPlaceHolder>592\</CodeContentPlaceHolder>. If you call <CodeContentPlaceHolder>593\</CodeContentPlaceHolder> before fully populating the recordset, the amount of movement is relative to the number of records accessed as indicated by the value of [GetRecordCount](#cdaorecordset__getrecordcount). You can move to the last record by calling <CodeContentPlaceHolder>594\</CodeContentPlaceHolder>.  
  
 Once you call <CodeContentPlaceHolder>595\</CodeContentPlaceHolder>, the record at the approximate position corresponding to that value becomes current.  
  
> [!NOTE]
>  Calling <CodeContentPlaceHolder>596\</CodeContentPlaceHolder> to move the current record to a specific record in a recordset is not recommended. Call the [SetBookmark](#cdaorecordset__setbookmark) member function instead.  
  
 For related information, see the topic "PercentPosition Property" in DAO Help.  
  
##  \<a name="cdaorecordset__update">\</a>  CDaoRecordset::Update  
 Call this member function after a call to the <CodeContentPlaceHolder>597\</CodeContentPlaceHolder> or **Edit** member function.  
  
<CodeContentPlaceHolder>73\</CodeContentPlaceHolder>  
### Remarks  
 This call is required to complete the <CodeContentPlaceHolder>598\</CodeContentPlaceHolder> or **Edit** operation.  
  
 Both <CodeContentPlaceHolder>599\</CodeContentPlaceHolder> and **Edit** prepare an edit buffer in which the added or edited data is placed for saving to the data source. **Update** saves the data. Only those fields marked or detected as changed are updated.  
  
 If the data source supports transactions, you can make the **Update** call (and its corresponding <CodeContentPlaceHolder>600\</CodeContentPlaceHolder> or **Edit** call) part of a transaction.  
  
> [!CAUTION]
>  If you call **Update** without first calling either <CodeContentPlaceHolder>601\</CodeContentPlaceHolder> or **Edit**, **Update** throws a <CodeContentPlaceHolder>602\</CodeContentPlaceHolder>. If you call <CodeContentPlaceHolder>603\</CodeContentPlaceHolder> or **Edit**, you must call **Update** before you call [MoveNext](#cdaorecordset__movenext) or close either the recordset or the data source connection. Otherwise, your changes are lost without notification.  
  
 When the recordset object is pessimistically locked in a multiuser environment, the record remains locked from the time **Edit** is used until the updating is complete. If the recordset is optimistically locked, the record is locked and compared with the pre-edited record just before it is updated in the database. If the record has changed since you called **Edit**, the **Update** operation fails and MFC throws an exception. You can change the locking mode with <CodeContentPlaceHolder>604\</CodeContentPlaceHolder>.  
