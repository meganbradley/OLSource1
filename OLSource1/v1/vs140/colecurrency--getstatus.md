...
## Remarks  
 The return value is defined by the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> enumerated type that is defined within the **COleCurrency** class.  
...
-   If its value is set from a **VARIANT** or <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> value that could not be converted to a currency value.  
  
-   If this object has experienced an overflow or underflow during an arithmetic assignment operation, for example <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> or **\*=**.  
