---
title: "CMFCToolBar::IsAutoGrayInactiveImages"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "reference"
f1_keywords: 
  - "CMFCToolBar.IsAutoGrayInactiveImages"
  - "IsAutoGrayInactiveImages"
  - "CMFCToolBar::IsAutoGrayInactiveImages"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "IsAutoGrayInactiveImages method"
ms.assetid: 3033a845-30de-4be6-a079-c751ccf4c61f
caps.latest.revision: 15
robots: noindex,nofollow
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# CMFCToolBar::IsAutoGrayInactiveImages
Specifies whether the automatic generation of inactive (non-highlighted) button images is enabled.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Return Value  
 <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> if the option to automatically dim inactive images is enabled; otherwise <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
  
## Remarks  
 You can enable or disable automatic dimming of inactive images by calling [CMFCToolBar::AutoGrayInactiveImages](../vs140/cmfctoolbar--autograyinactiveimages.md).  
  
## Requirements  
 **Header:** afxtoolbar.h  
  
## See Also  
 [CMFCToolBar Class](../vs140/cmfctoolbar-class.md)   
 [Hierarchy Chart](../vs140/hierarchy-chart.md)