...
---
# inline_depth
Specifies the inline heuristic search depth, such that, no function will be inlined if it is at a depth (in the call graph) greater than <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
...
## Remarks  
 ...
 <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> can be a value between 0 and 255, where 255 means unlimited depth in the call graph, and zero inhibits inline expansion.  When <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is not specified, the default (254) is used.  
 ...
 The **inline_depth** pragma has no effect on functions marked with <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>.  
