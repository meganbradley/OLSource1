...
5.  In the **Property Name** box, type <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>.  
...
9. Add a parameter, called <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> (type <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>), using the **Parameter Name** and **Parameter Type** controls.  
  
10. Add a second parameter called <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> (type <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>).  
...
### Changes Made by the Add Property Wizard  
 ...
 This code declares two functions called <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> that allow the user to request a specific row and column when accessing the property.  
 ...
 Finally, the implementations of the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> functions are added to the end of the .CPP file. In most cases, you will modify the Get function to return the value of the property. The Set function will usually contain code that should execute, either before or after the property changes.  
...
##  \<a name="_core_handling_errors_in_your_activex_control">\</a> Handling Errors in Your ActiveX Control  
 ...
 To indicate the kind of error that has occurred, the control must pass an error code to <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> or <CodeContentPlaceHolder>10\</CodeContentPlaceHolder>. An error code is an OLE status code, which has a 32-bit value. When possible, choose an error code from the standard set of codes defined in the OLECTL.H header file. The following table summarizes these codes.  
...
|**CTL_E_REPLACEMENTSTOOLONG**|Replacements too long|  
 ...
 [!code[NVC_MFC_AxUI#37](../vs140/codesnippet/CPP/mfc-activex-controls--advanced-topics_4.cpp)]  
...
##  \<a name="_core_handling_special_keys_in_your_control">\</a> Handling Special Keys in the Control  
 ...
 If the base class of your ActiveX control is <CodeContentPlaceHolder>11\</CodeContentPlaceHolder>, you can override [CWnd::PreTranslateMessage](../vs140/cwnd--pretranslatemessage.md) to handle messages before the container processes them. When using this technique, always return **TRUE** if you handle the message in your override of <CodeContentPlaceHolder>12\</CodeContentPlaceHolder>.  
...
-   Declare a local variable and subclass as the dialog item. Insert code that resembles the following (<CodeContentPlaceHolder>13\</CodeContentPlaceHolder> is the wrapper class, <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> is the control's ID):  
