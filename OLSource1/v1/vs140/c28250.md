...
warning C28250: Inconsistent annotation for function: the prior instance has an error.  
 ...
 If a declaration is made using a <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, the line where the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> appears is more useful than the line of the declaration.  
  
 This warning refers to an error in the annotation and reflects the requirement that the annotations on a function declaration must match those on the definition, except if a function <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> is involved. In that case, the function <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> is taken as definitive for both the declaration and the definition.  
