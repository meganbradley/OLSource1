...
Resolving among overloaded versions  
 ...
 [!code[VbVbcnProcedures#62](../vs140/codesnippet/VisualBasic/overload-resolution--visual-basic-_1.vb)]  
  
 [!code[VbVbcnProcedures#63](../vs140/codesnippet/VisualBasic/overload-resolution--visual-basic-_2.vb)]  
  
 In the first call, the compiler eliminates the first overload because the type of the first argument (<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>) narrows to the type of the corresponding parameter (<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>). It then eliminates the third overload because each argument type in the second overload (<CodeContentPlaceHolder>4\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>) widens to the corresponding type in the third overload (<CodeContentPlaceHolder>6\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>). The second overload requires less widening, so the compiler uses it for the call.  
  
 In the second call, the compiler cannot eliminate any of the overloads on the basis of narrowing. It eliminates the third overload for the same reason as in the first call, because it can call the second overload with less widening of the argument types. However, the compiler cannot resolve between the first and second overloads. Each has one defined parameter type that widens to the corresponding type in the other (<CodeContentPlaceHolder>8\</CodeContentPlaceHolder> to <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>, but <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> to <CodeContentPlaceHolder>11\</CodeContentPlaceHolder>). The compiler therefore generates an overload resolution error.  
...
|No value (argument omitted)|<CodeContentPlaceHolder>12\</CodeContentPlaceHolder>|  
|A single value|<CodeContentPlaceHolder>13\</CodeContentPlaceHolder>|  
|Two or more values in a comma-separated list|<CodeContentPlaceHolder>14\</CodeContentPlaceHolder>|  
|An array of any length (including an empty array)|<CodeContentPlaceHolder>15\</CodeContentPlaceHolder>|  
