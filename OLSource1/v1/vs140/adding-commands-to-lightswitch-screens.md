...
4.  In the **Add Button** dialog box, choose the **Write my own method** option button, name the button <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, and then choose the **OK** button.  
...
7.  In the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> method, add the following code.  
...
5.  In the **Properties** window, choose the **Display Name** property, and then enter <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>.  
...
## Closer Look  
 This lesson showed how to add a command to the **CurrentProductList** screen and add code that runs every time a user chooses the command button. The code executes the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> method, which displays a message box with the count of products in the selected category.  
  
 You also added a command that executes the existing <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> method to display a popup, and you changed the display name and icon for the button.  
