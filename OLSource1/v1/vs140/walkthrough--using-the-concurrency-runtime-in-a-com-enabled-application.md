...
-   A thread can call <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> multiple times as long as it provides the same arguments to every call.  
  
-   For each call to <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, a thread must also call [CoUninitialize](http://msdn.microsoft.com/library/windows/desktop/ms688715). In other words, calls to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> must be balanced.  
  
-   To switch from one thread apartment to another, a thread must completely free the COM library before it calls <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> with the new threading specification.  
...
### Using COM with the Parallel Patterns Library  
 When you use COM with a component in the Parallel Patterns Library (PPL), for example, a task group or parallel algorithm, call <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> before you use the COM library during each task or iteration, and call <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> before each task or iteration finishes. The following example shows how to manage the lifetime of the COM library with a [concurrency::structured_task_group](../vs140/structured_task_group-class.md) object.  
 ...
 You must make sure that the COM library is correctly freed when a task or parallel algorithm is canceled or when the task body throws an exception. To guarantee that the task calls <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> before it exits, use a <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> block or the *Resource Acquisition Is Initialization* (RAII) pattern. The following example uses a <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> block to free the COM library when the task completes or is canceled, or when an exception is thrown.  
 ...
 The following example uses the RAII pattern to define the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> class, which manages the lifetime of the COM library in a given scope.  
 ...
 You can use the <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> class to automatically free the COM library when the task exits, as follows.  
...
### Using COM with Asynchronous Agents  
 When you use COM with asynchronous agents, call <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> before you use the COM library in the [concurrency::agent::run](../vs140/agent--run-method.md) method for your agent. Then call <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> before the <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> method returns. Do not use COM management routines in the constructor or destructor of your agent, and do not override the [concurrency::agent::start](../vs140/agent--start-method.md) or [concurrency::agent::done](../vs140/agent--done-method.md) methods because these methods are called from a different thread than the <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> method.  
  
 The following example shows a basic agent class, named <CodeContentPlaceHolder>16\</CodeContentPlaceHolder>, which manages the COM library in the <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> method.  
...
## An Example of a COM-Enabled Application  
 This section shows a complete COM-enabled application that uses the <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> interface to execute a script that computes the n<sup>th\</sup> Fibonacci number. This example first calls the script from the main thread, and then uses the PPL and agents to call the script concurrently.  
  
 Consider the following helper function, <CodeContentPlaceHolder>20\</CodeContentPlaceHolder>, which calls a procedure in an <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> object.  
 ...
 The <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> function creates an <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> object, adds script code to it that computes the n<sup>th\</sup> Fibonacci number, and then calls the <CodeContentPlaceHolder>24\</CodeContentPlaceHolder> function to run that script.  
...
### Calling the Script from the PPL  
 The following function, <CodeContentPlaceHolder>25\</CodeContentPlaceHolder>, uses the [concurrency::parallel_for](../vs140/parallel_for-function.md) algorithm to call the script in parallel. This function uses the <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> class to manage the lifetime of the COM library during every iteration of the task.  
 ...
 To use the <CodeContentPlaceHolder>27\</CodeContentPlaceHolder> function with the example, add the following code before the <CodeContentPlaceHolder>28\</CodeContentPlaceHolder> function returns.  
...
### Calling the Script from an Agent  
 The following example shows the <CodeContentPlaceHolder>29\</CodeContentPlaceHolder> class, which calls a script procedure to compute the n<sup>th\</sup> Fibonacci number. The <CodeContentPlaceHolder>30\</CodeContentPlaceHolder> class uses message passing to receive, from the main program, input values to the script function. The <CodeContentPlaceHolder>31\</CodeContentPlaceHolder> method manages the lifetime of the COM library throughout the task.  
 ...
 The following function, <CodeContentPlaceHolder>32\</CodeContentPlaceHolder>, creates several <CodeContentPlaceHolder>33\</CodeContentPlaceHolder> objects and uses message passing to send several input values to those objects.  
 ...
 To use the <CodeContentPlaceHolder>34\</CodeContentPlaceHolder> function with the example, add the following code before the <CodeContentPlaceHolder>35\</CodeContentPlaceHolder> function returns.  
...
**fib(12) = 144**   
## Compiling the Code  
 Copy the example code and paste it in a Visual Studio project, or paste it in a file that is named <CodeContentPlaceHolder>36\</CodeContentPlaceHolder> and then run the following command in a Visual Studio Command Prompt window.  
