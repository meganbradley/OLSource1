...
2.  Add the following method to that `BankAccountTests` class:  
 ...
 The method is rather simple. We set up a new `BankAccount` object with a beginning balance and then withdraw a valid amount. We use the Microsoft unit test framework for managed code [AreEqual](assetId:///M:Microsoft.VisualStudio.TestTools.UnitTesting.Assert.AreEqual(System.Double,System.Double,System.Double,System.String)?qualifyHint=False&autoUpgrade=True) method to verify that the ending balance is what we expect.  
...
-   Assert that the message (the third parameter of the constructor) includes the `DebitAmountExceedsBalanceMessage` defined in the `BankAccount` class.  
 ...
 The [StringAssert.Contains](assetId:///M:Microsoft.VisualStudio.TestTools.UnitTesting.StringAssert.Contains(System.String,System.String)?qualifyHint=True&autoUpgrade=True) method in the Microsoft unit test framework enables us to verify the second option without the calculations that are required of the first option.  
...
3.  If the `debitAmount` value is valid (i.e., less than the balance but greater than zero, no exception is caught, so the assert is never caught. The test method passes. This is not good, because we want the test method to fail if no exception is thrown.  
 ...
 The third fact is a bug in our test method. To attempt to resolve the issue, we add a [Fail](assetId:///M:Microsoft.VisualStudio.TestTools.UnitTesting.Assert.Fail(System.String)?qualifyHint=False&autoUpgrade=True) assert at the end of the test method to handle the case where no exception is thrown.  
