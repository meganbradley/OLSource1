...
-   A bitmask type can be implemented as either an integer type or an enumeration. In either case, you can perform bitwise operations (such as <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>) on values of the same bitmask type. The elements <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> of a bitmask type are nonzero values such that <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> & <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> is zero.  
...
-   Functions supplied by the library, including the default versions of replaceable functions, can throw *at most* those exceptions listed in any exception specification. No destructors supplied by the library throw exceptions. Functions in the Standard C Library may propagate an exception, as when <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> calls a comparison function that throws an exception, but they do not otherwise throw exceptions.  
