...
## Remarks  
 When a recordset object is created or opened, each of its records already has a unique bookmark if it supports them. Call <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> to determine whether a recordset supports bookmarks.  
  
 You can save the bookmark for the current record by assigning the value of the bookmark to a <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> object. To quickly return to that record at any time after moving to a different record, call <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> with a parameter corresponding to the value of that <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> object.  
