...
1.  Start the profiler by typing the following command:  
    ...
    |[/automark](../vs140/automark.md) **:** <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>|Use with **/wincounter** only. Specifies the number of milliseconds between Windows performance counter collection events. The default value is 500.|  
...
3.  Attach the profiler to the ASP.NET worker process by typing the following command:**VSPerfCmd /attach:**<CodeContentPlaceHolder>9\</CodeContentPlaceHolder> [**/targetclr:**<CodeContentPlaceHolder>10\</CodeContentPlaceHolder>]  
  
    -   <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> specifies the ID or name of the ASP.NET worker process. You can view the process IDs of all running processes in Windows Task Manager.  
  
    -   [/targetclr](../vs140/targetclr.md) **:** <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> specifies the version of the common language runtime (CLR) to profile when more than one version of the runtime is loaded in an application. This parameter is optional.  
...
-   The pairs of VSPerfCmd options in the following table start and stop data collection. Specify each option on a separate command line. You can turn data collection on and off multiple times.  
    ...
    |[/processon](../vs140/processon-and-processoff.md) **:** <CodeContentPlaceHolder>13\</CodeContentPlaceHolder>  [processoff](../vs140/processon-and-processoff.md) **:** <CodeContentPlaceHolder>14\</CodeContentPlaceHolder>|Starts (**/processon**) or stops (**/processoff**) data collection for the process that the process ID (<CodeContentPlaceHolder>15\</CodeContentPlaceHolder>) specifies.|  
    |[/attach](../vs140/attach.md) **:**{<CodeContentPlaceHolder>16\</CodeContentPlaceHolder>&#124;<CodeContentPlaceHolder>17\</CodeContentPlaceHolder>} [/detach](../vs140/detach.md)[**:**{<CodeContentPlaceHolder>18\</CodeContentPlaceHolder>&#124;<CodeContentPlaceHolder>19\</CodeContentPlaceHolder>}]|**/attach** starts to collect data for the process that the process ID (<CodeContentPlaceHolder>20\</CodeContentPlaceHolder>) or process name (*ProcName*) specifies. **/detach** stops data collection for the specified process or for all processes if no process is specified.|  
