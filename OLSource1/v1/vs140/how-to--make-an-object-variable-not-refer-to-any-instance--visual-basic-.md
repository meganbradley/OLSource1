...
-   Set the variable to <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> in an assignment statement.  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Robust Programming  
 If your code tries to access a member of an object variable that has been set to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, a \<xref:System.NullReferenceException*> occurs. If you set an object variable to <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> frequently, or if it is possible the variable is not initialized, it is a good idea to enclose member accesses in a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> block.  
  
## .NET Framework Security  
 If you use an object variable for objects that contain confidential or sensitive data, you can set the variable to <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> when you are not actively dealing with one of those objects. This reduces the chance of malicious code gaining access to the data.  
