...
-   **Widening.** The `Short` data type widens to `Integer`, `Long`, `Decimal`, `Single`, or `Double`. This means you can convert `Short` to any one of these types without encountering a <xref:System.OverflowException?qualifyHint=True> error.  
...
-   **Framework Type.** The corresponding type in the .NET Framework is the <xref:System.Int16?qualifyHint=True> structure.  
