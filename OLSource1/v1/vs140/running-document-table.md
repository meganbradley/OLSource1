...
|Document data object|At the minimum, this is an <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>\<br />\<br /> object. The IDE does not require any particular interface beyond the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> interface for a custom editor's document data object. However, for a standard editor, the editor's implementation of the \<xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistDocData2*> interface is required to handle file persistence calls from the project. For more information, see [Saving a Standard Document](../vs140/saving-a-standard-document.md).|  
