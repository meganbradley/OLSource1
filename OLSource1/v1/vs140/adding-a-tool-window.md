...
1.  In FirstToolWindowPackage.cs, find the <xref:Microsoft.VisualStudio.Shell.ProvideToolWindowAttribute?qualifyHint=False> attribute on the `FirstToolWindowPackage` class, which passes the FirstToolWindow type to the constructor. To specify a default position, you must add more parameters to the constructor following example.  
    ...
    > [!NOTE]
    >  For more information about the types of windows in the IDE, see <xref:EnvDTE.vsWindowType?qualifyHint=False>.  
