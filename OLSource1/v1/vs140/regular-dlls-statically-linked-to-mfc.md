...
-   This type of DLL must instantiate a class derived from <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
  
-   This type of DLL uses the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> provided by MFC. Place all DLL-specific initialization code in the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> member function and termination code in <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> as in a normal MFC application.  
...
-   Even though the term USRDLL is obsolete, you must still define "**_USRDLL**" on the compiler command line. This definition determines which declarations is pulled in from the MFC header files.  
 ...
 Regular DLLs must have a <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>-derived class and a single object of that application class, as does an MFC application. However, the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> object of the DLL does not have a main message pump, as does the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> object of an application.  
  
 Note that the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> mechanism does not apply to a DLL, because the application owns the main message pump. If the DLL opens modeless dialogs or has a main frame window of its own, the application's main message pump must call a routine exported by the DLL that in turn calls the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> member function of the DLL's application object.  
