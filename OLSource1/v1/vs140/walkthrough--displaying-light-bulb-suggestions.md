...
2.  Inside the source provider class, import the <xref:Microsoft.VisualStudio.Text.Operations.ITextStructureNavigatorSelectorService?qualifyHint=False> and add it as a property.  
...
3.  Implement the <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedActionsSourceProvider.CreateSuggestedActionsSource?qualifyHint=False> method to return an <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedActionsSource?qualifyHint=False> object. We will discuss the source in the next section.  
...
1.  Add a class **TestSuggestedActionsSource** that implements <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedActionsSource?qualifyHint=False>.  
...
5.  Implement the <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedActionsSource.HasSuggestedActionsAsync?qualifyHint=False> method. The editor calls this method to find out whether to display the light bulb. This call is made quite often, for example whenever the cursor moves from one line to another, or when the mouse hovers over an error squiggle. It is asynchronous in order to allow other UI operations to carry on while this method is working. In most cases this method needs to perform some parsing and analysis of the current line, so the processing may take some time.  
  
     In our implementation it asynchronously gets the <xref:Microsoft.VisualStudio.Text.Operations.TextExtent?qualifyHint=False> and determines whether the extent is significant, i.e., whether it has some text other than whitespace.  
...
6.  Implement the <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedActionsSource.GetSuggestedActions?qualifyHint=False> method, which returns an array of <xref:Microsoft.VisualStudio.Language.Intellisense.SuggestedActionSet?qualifyHint=False> objects that contain the different <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedAction?qualifyHint=False> objects. This method is called when the light bulb is expanded.  
...
2.  Create two classes, the first named `UpperCaseSuggestedAction` and the second named `LowerCaseSuggestedAction`. Both classes implement <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedAction?qualifyHint=False>.  
...
6.  Implement the <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedAction.GetPreviewAsync?qualifyHint=False> method so that it displays the action preview.  
...
7.  Implement the <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedAction.GetActionSetsAsync?qualifyHint=False> method so that it returns an empty <xref:Microsoft.VisualStudio.Language.Intellisense.SuggestedActionSet?qualifyHint=False> enumeration.  
...
9. Implement the <xref:Microsoft.VisualStudio.Language.Intellisense.ISuggestedAction.Invoke?qualifyHint=False> method by replacing the text in the span with its uppercase equivalent.  
...
11. Don’t forget to do the same thing for `LowerCaseSuggestedAction` changing the display text to “Convert ‘{0}’ to lower case” and the call <xref:System.String.ToUpper?qualifyHint=False> to <xref:System.String.ToLower?qualifyHint=False>.  
