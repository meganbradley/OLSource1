...
1.  Insert this code in the <xref:Microsoft.VisualStudio.Shell.Package.Initialize?qualifyHint=False> method or in any other method except the VSPackage constructor:  
    ...
    ```  
     ...
     This code gets the <xref:Microsoft.VisualStudio.Shell.Interop.SVsActivityLog?qualifyHint=False> service and casts it to an <xref:Microsoft.VisualStudio.Shell.Interop.IVsActivityLog?qualifyHint=False> interface. <xref:Microsoft.VisualStudio.Shell.Interop.IVsActivityLog.LogEntry?qualifyHint=False> writes an informational entry into the activity log using the current cultural context.  
...
## See Also  
 <xref:Microsoft.VisualStudio.Shell.Interop.IVsActivityLog?qualifyHint=False>   
 <xref:Microsoft.VisualStudio.Shell.Interop.__ACTIVITYLOG_ENTRYTYPE?qualifyHint=False>   
 [How to: Troubleshoot VSPackages](../vs140/troubleshooting-vspackages.md)   
