...
>  Many of the toolbars available to Visual Studio are not defined by Visual Studio, and their GUID and ID values are not public. This topic lists only toolbars that are defined in Visual Studio SDK .vsct files.  
 ...
 The default toolbars provided by the Visual Studio IDE use the GUID <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, except where otherwise specified by using GUID:ID syntax.  
...
> [!NOTE]
>  Toolbars and groups use the GUID <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, except where otherwise specified by using GUID:ID syntax. Where a GUID is specified for a toolbar, it also applies to the groups that descend from that toolbar.  
