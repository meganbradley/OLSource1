...
The procedure declaration determines the passing mechanism. If a parameter is declared [ByRef](../vs140/byref--visual-basic-.md), [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] expects to pass the corresponding argument by reference. This allows the procedure to change the value of the programming element underlying the argument in the calling code. If you wish to protect the underlying element against such change, you can override the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> passing mechanism in the procedure call by enclosing the argument name in parentheses. These parentheses are in addition to the parentheses enclosing the argument list in the call.  
  
 The calling code cannot override a [ByVal](../vs140/byval--visual-basic-.md) mechanism.  
  
### To force an argument to be passed by value  
  
-   If the corresponding parameter is declared <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> in the procedure, you do not need to take any additional steps. [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] already expects to pass the argument by value.  
  
-   If the corresponding parameter is declared <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> in the procedure, enclose the argument in parentheses in the procedure call.  
  
## Example  
 The following example overrides a <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> parameter declaration. In the call that forces <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>, note the two levels of parentheses.  
 ...
 When <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> is enclosed in extra parentheses within the argument list, the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> procedure cannot change its value in the calling code, and <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> displays "Cannot be replaced if passed ByVal". When <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> is not enclosed in extra parentheses, the procedure can change it, and <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> displays "This is a new value for the inString argument."  
  
## Compiling the Code  
 When you pass a variable by reference, you must use the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> keyword to specify this mechanism.  
