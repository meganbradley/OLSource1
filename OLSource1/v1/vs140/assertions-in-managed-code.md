...
##  <a name="BKMK_The_Debug_Assert_method"></a> The Debug.Assert method  
 Use the [Debug.Assert](assetId:///M:System.Diagnostics.Debug.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True) method freely to test conditions that should hold true if your code is correct. For example, suppose you have written an integer divide function. By the rules of mathematics, the divisor can never be zero. You might test this using an assertion:  
...
```  
 ...
 Note that calls to the assetId:///M:System.Diagnostics.Debug.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True method disappear when you create a Release version of your code. That means that the call that checks the balance disappears in the Release version. To solve this problem, you should replace assetId:///M:System.Diagnostics.Debug.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True with [Trace.Assert](assetId:///M:System.Diagnostics.Trace.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True), which does not disappear in the Release version:  
  
 Calls to assetId:///M:System.Diagnostics.Trace.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True add overhead to your Release version, unlike calls to assetId:///M:System.Diagnostics.Debug.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True.  
  
 [In this topic](#BKMK_In_this_topic)  
  
##  <a name="BKMK_Side_effects_of_Debug_Assert"></a> Side effects of Debug.Assert  
 When you use assetId:///M:System.Diagnostics.Debug.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True, make sure that any code inside `Assert` does not change the results of the program if `Assert` is removed. Otherwise, you might accidentally introduce a bug that only shows up in the Release version of your program. Be especially careful about asserts that contain function or procedure calls, such as the following example:  
...
```  
 ...
 This use of assetId:///M:System.Diagnostics.Debug.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True might appear safe at first glance, but suppose the function meas updates a counter each time it is called. When you build the Release version, this call to meas is eliminated, so the counter does not get updated. This is an example of a function with a side effect. Eliminating a call to a function that has side effects could result in a bug that only appears in the Release version. To avoid such problems, do not place function calls in a assetId:///M:System.Diagnostics.Debug.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True statement. Use a temporary variable instead:  
...
```  
 ...
 Even when you use assetId:///M:System.Diagnostics.Trace.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True, you might still want to avoid placing function calls inside an `Assert` statement. Such calls should be safe, because assetId:///M:System.Diagnostics.Trace.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True statements are not eliminated in a Release build. However, if you avoid such constructs as a matter of habit, you are less likely to make a mistake when you use assetId:///M:System.Diagnostics.Debug.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True.  
  
 [In this topic](#BKMK_In_this_topic)  
  
##  <a name="BKMK_Trace_and_Debug_Requirements"></a> Trace and Debug Requirements  
 If you create your project using the [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] wizards, the TRACE symbol is defined by default in both Release and Debug configurations. The DEBUG symbol is defined by default only in the Debug build.  
...
```  
 ...
 The following example shows [Assert](assetId:///M:System.Diagnostics.Debug.Assert(System.Boolean,System.String,System.String)?qualifyHint=False&autoUpgrade=True) and [Assert](assetId:///M:System.Diagnostics.Trace.Assert(System.Boolean,System.String,System.String)?qualifyHint=False&autoUpgrade=True):  
...
##  <a name="BKMK_Customizing_Assert_behavior"></a> Customizing Assert behavior  
 ...
 You can customize the output behavior by adding a <xref:System.Diagnostics.TraceListener*> object to the `Listeners` collection, by removing a <xref:System.Diagnostics.TraceListener*> from the `Listeners` collection, or by overriding the [TraceListener.Fail](assetId:///M:System.Diagnostics.TraceListener.Fail(System.String)?qualifyHint=True&autoUpgrade=True) method of an existing `TraceListener` to make it behave differently.  
  
 For example, you could override the assetId:///M:System.Diagnostics.TraceListener.Fail(System.String)?qualifyHint=True&autoUpgrade=True method to write to an event log instead of displaying the **Assertion Failed** dialog box.  
  
 To customize the output in this way, your program must contain a listener, and you must inherit from <xref:System.Diagnostics.TraceListener*> and override its assetId:///M:System.Diagnostics.TraceListener.Fail(System.String)?qualifyHint=True&autoUpgrade=True method.  
  
 For more Information, see [Trace Listeners](assetId:///444b0d33-67ea-4c36-9e94-79c50f839025).  
  
 [In this topic](#BKMK_In_this_topic)  
  
##  <a name="BKMK_Setting_assertions_in_configuration_files"></a> Setting assertions in configuration files  
 You can set assertions in your program configuration file as well as in your code. For more information, see assetId:///M:System.Diagnostics.Trace.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True or assetId:///M:System.Diagnostics.Debug.Assert(System.Boolean)?qualifyHint=True&autoUpgrade=True.  
...
## See Also  
 ...
 [Conditional Compiling with Trace and Debug](assetId:///56d051c3-012c-42c1-9a58-7270edc624aa)   
