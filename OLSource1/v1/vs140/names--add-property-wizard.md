 ...
 **Return type**  
 ATL interfaces only. Sets the return type for the property. For dual interfaces, <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> is always the return type, and this box is unavailable. For custom interfaces, you can select a return type from the list. <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is still recommended, because it provides a standard way to return errors.  
...
|**Get/Set methods**|Specifies the property is added as Get*PropertyName* and Set*PropertyName* functions, by default. These names appear under **Get function** and **Set function**.\<br />\<br /> You can change the default **Property type**, which passes a value for the Get function. You can specify parameters for the **Get** and <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> functions.|  
  
 **Get function**  
 For ATL interfaces. Sets the property as readable; that is, it creates the **Get** method for retrieving this property from the object. You must select **Get**, <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, or both.  
  
 **Put function**  
 ATL interfaces only. Sets the property writable; that is, it creates the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> method for setting, or "putting," this property of the object. You must select **Get**, <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>, or both. If you select this option, you can choose from the following two ways to implement the method:  
