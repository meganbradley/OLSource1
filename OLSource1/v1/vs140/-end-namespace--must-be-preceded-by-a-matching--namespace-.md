---
title: "&#39;End Namespace&#39; must be preceded by a matching &#39;Namespace&#39;"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "vbc30623"
  - "bc30623"
helpviewer_keywords: 
  - "BC30623"
ms.assetid: b280e206-7cf0-444a-971d-c719d05eb2b8
caps.latest.revision: 12
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# &#39;End Namespace&#39; must be preceded by a matching &#39;Namespace&#39;
Namespace blocks begin with a <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> statement and end with an <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> construct.  
  
 **Error ID:** BC30623  
  
### To correct this error  
  
-   Make sure that the namespace block begins with a <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> statement.  
  
## See Also  
 [Namespace Statement](../vs140/namespace-statement.md)   
 [Namespaces in Visual Basic](../vs140/namespaces-in-visual-basic.md)