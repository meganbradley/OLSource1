...
-   **UI Context** UI contexts are identified by the values of the <xref:Microsoft.VisualStudio.VSConstants.UICONTEXT?qualifyHint=False> class, for example, <xref:Microsoft.VisualStudio.VSConstants.UICONTEXT_SolutionBuilding?qualifyHint=False> when the solution is being built, or <xref:Microsoft.VisualStudio.VSConstants.UICONTEXT_Debugging?qualifyHint=False> when the debugger is active. Multiple UI contexts can be active at the same time.  
...
1.  Register context GUIDs by calling the <xref:Microsoft.VisualStudio.Shell.Interop.IVsMonitorSelection.GetCmdUIContextCookie?qualifyHint=False> method.  
  
2.  Get the state of a context GUID by calling the <xref:Microsoft.VisualStudio.Shell.Interop.IVsMonitorSelection.IsCmdUIContextActive?qualifyHint=False> method.  
  
3.  Turn context GUIDs on and off by calling the <xref:Microsoft.VisualStudio.Shell.Interop.IVsMonitorSelection.SetCmdUIContext?qualifyHint=False> method.  
