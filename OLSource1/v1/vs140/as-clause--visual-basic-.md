---
title: "As Clause (Visual Basic)"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-visual-basic"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "vb.as"
dev_langs: 
  - "VB"
helpviewer_keywords: 
  - "constraints, Visual Basic generic types"
  - "As keyword, statement syntax"
  - "As keyword"
ms.assetid: b4281ec8-2be5-49f7-aae8-ae0a96265b0d
caps.latest.revision: 23
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# As Clause (Visual Basic)
Introduces an <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> clause, which identifies a data type in a declaration statement or a constraint list on a generic type parameter.  
  
## Remarks  
 The <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> keyword can be used in these contexts:  
  
 [Aggregate Clause](../vs140/aggregate-clause--visual-basic-.md)  
  
 [Class Statement](../vs140/class-statement--visual-basic-.md)  
  
 [Const Statement](../vs140/const-statement--visual-basic-.md)  
  
 [Declare Statement](../vs140/declare-statement.md)  
  
 [Delegate Statement](../vs140/delegate-statement.md)  
  
 [Dim Statement](../vs140/dim-statement--visual-basic-.md)  
  
 [Enum Statement](../vs140/enum-statement--visual-basic-.md)  
  
 [Event Statement](../vs140/event-statement.md)  
  
 [For...Next Statements](../vs140/for...next-statement--visual-basic-.md)  
  
 [For Each...Next Statements](../vs140/for-each...next-statement--visual-basic-.md)  
  
 [From Clause](../vs140/from-clause--visual-basic-.md)  
  
 [Function Statement](../vs140/function-statement--visual-basic-.md)  
  
 [Group Join Clause](../vs140/group-join-clause--visual-basic-.md)  
  
 [Interface Statement](../vs140/interface-statement--visual-basic-.md)  
  
 [Operator Statement](../vs140/operator-statement.md)  
  
 [Property Statement](../vs140/property-statement.md)  
  
 [Structure Statement](../vs140/structure-statement.md)  
  
 [Sub Statement](../vs140/sub-statement--visual-basic-.md)  
  
 [Try...Catch...Finally Statements](../vs140/try...catch...finally-statement--visual-basic-.md)  
  
## See Also  
 [How to: Create a New Variable](../vs140/how-to--create-a-new-variable--visual-basic-.md)   
 [Data Types in Visual Basic](../vs140/data-types-in-visual-basic.md)   
 [Variable Declaration in Visual Basic](../vs140/variable-declaration-in-visual-basic.md)   
 [Type List](../vs140/type-list--visual-basic-.md)   
 [Generic Types in Visual Basic](../vs140/generic-types-in-visual-basic--visual-basic-.md)   
 [Keywords (Visual Basic)](../vs140/keywords--visual-basic-.md)