...
An object is assigned to a variable declared to be of the [Object Data Type](../vs140/object-data-type.md).  
 ...
 When you declare a variable as <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, the compiler must perform *late binding*, which causes extra operations at run time. It also exposes your application to potential run-time errors. For example, if you assign a \<xref:System.Windows.Forms.Form*> to the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> variable and then try to access the \<xref:System.Xml.XmlDocument.NameTable*?displayProperty=fullName> property, the runtime throws a \<xref:System.MemberAccessException*> because the \<xref:System.Windows.Forms.Form*> class does not expose a <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> property.  
