...
An object is assigned to a variable declared to be of the [Object Data Type](../vs140/object-data-type.md).  
 ...
 When you declare a variable as `Object`, the compiler must perform *late binding*, which causes extra operations at run time. It also exposes your application to potential run-time errors. For example, if you assign a <xref:System.Windows.Forms.Form?qualifyHint=False> to the `Object` variable and then try to access the <xref:System.Xml.XmlDocument.NameTable?qualifyHint=True> property, the runtime throws a <xref:System.MemberAccessException?qualifyHint=False> because the <xref:System.Windows.Forms.Form?qualifyHint=False> class does not expose a `NameTable` property.  
