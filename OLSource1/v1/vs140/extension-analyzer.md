...
-   Exceptions. When a VSPackage is loaded, [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] calls its [SetSite](assetId:///M:Microsoft.VisualStudio.Shell.Interop.IVsPackage.SetSite(Microsoft.VisualStudio.OLE.Interop.IServiceProvider)?qualifyHint=False&autoUpgrade=True) method. If this method throws an exception, then the VSPackage load fails. The best way to isolate this issue is to step through the SetSite code.  
