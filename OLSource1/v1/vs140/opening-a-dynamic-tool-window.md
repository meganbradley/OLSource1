...
>  For a list of predefined UI contexts, see <xref:Microsoft.VisualStudio.VSConstants.UICONTEXT*>. For the  
 ...
 If you want to open a dynamic tool window at startup, and it is possible for the creation to fail, you must implement the <xref:Microsoft.VisualStudio.Shell.Interop.IVsPackageDynamicToolOwnerEx*> interface and test the failure conditions in the [QueryShowTool](assetId:///M:Microsoft.VisualStudio.Shell.Interop.IVsPackageDynamicToolOwnerEx.QueryShowTool(System.Guid@,System.UInt32,System.Int32@)?qualifyHint=False&autoUpgrade=True) method. In order for the shell to know that you have a dynamic tool window that should be opened at startup, you must add the `SupportsDynamicToolOwner` value (set to 1) to your package registration. This value is not part of the standard <xref:Microsoft.VisualStudio.Shell.PackageRegistrationAttribute*>, so you must create a custom attribute to add it. For more information about custom attributes, see [Using a Custom Registration Attribute to Register an Extension](../vs140/using-a-custom-registration-attribute-to-register-an-extension.md).  
  
 Use [FindToolWindow](assetId:///M:Microsoft.VisualStudio.Shell.Package.FindToolWindow(System.Type,System.Int32,System.Boolean)?qualifyHint=False&autoUpgrade=True) to open a tool window. The tool window is created as needed.  
