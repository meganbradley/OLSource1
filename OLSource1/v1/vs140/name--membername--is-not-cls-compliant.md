...
An assembly is marked as <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> but exposes a member with a name that begins with an underscore (<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>).  
...
-   If you require that the member name remain unchanged, remove the \<xref:System.CLSCompliantAttribute*> from its definition or mark it as <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>. You can still mark the assembly as <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>.  
