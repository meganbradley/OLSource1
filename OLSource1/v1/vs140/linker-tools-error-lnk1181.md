...
cannot open input file 'filename'  
 ...
 The linker could not find <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> because it does not exist or the path was not found.  
  
 Some common causes for error LNK1181 include:  
  
-   <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is referenced as an additional dependency on the linker line, but the file does not exist.  
  
-   A **/LIBPATH** statement that specifies the directory containing <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> is missing.  
