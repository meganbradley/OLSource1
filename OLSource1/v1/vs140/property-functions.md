...
|int BitwiseOr(int first, int second)|Perform a bitwise <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> on the first and second (first &#124; second).|  
|int BitwiseAnd(int first, int second)|Perform a bitwise <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> on the first and second (first & second).|  
|int BitwiseXor(int first, int second)|Perform a bitwise <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> on the first and second (first ^ second).|  
|int BitwiseNot(int first)|Perform a bitwise <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> (~first).|  
...
##  \<a name="BKMK_Nested">\</a> Nested Property Functions  
 ...
 This example returns the value of the \<xref:System.IO.FileAttributes*><CodeContentPlaceHolder>21\</CodeContentPlaceHolder> bit (32 or 0) of the file given by the path <CodeContentPlaceHolder>22\</CodeContentPlaceHolder>. Notice that enumerated data values cannot appear by name within property functions. The numeric value (32) must be used instead.  
  
 Metadata may also appear in nested property functions. For more information, see [Batching](../vs140/msbuild-batching.md).  
  
##  \<a name="BKMK_DoesTaskHostExist">\</a> MSBuild DoesTaskHostExist  
 The <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> property function in MSBuild returns whether a task host is currently installed for the specified runtime and architecture values.  
...
##  \<a name="BKMK_GetDirectoryNameOfFileAbove">\</a> MSBuild GetDirectoryNameOfFileAbove  
 The MSBuild <CodeContentPlaceHolder>24\</CodeContentPlaceHolder> property function looks for a file in the directories above the current directory in the path.  
...
##  \<a name="BKMK_GetRegistryValue">\</a> MSBuild GetRegistryValue  
 The MSBuild <CodeContentPlaceHolder>25\</CodeContentPlaceHolder> property function returns the value of a registry key. This function takes two arguments, the key name and the value name, and returns the value from the registry. If you don't specify a value name, the default value is returned.  
...
##  \<a name="BKMK_GetRegistryValueFromView">\</a> MSBuild GetRegistryValueFromView  
 The MSBuild <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> property function gets system registry data given the registry key, value, and one or more ordered registry views. The key and value are searched in each registry view in order until they are found.  
...
##  \<a name="BKMK_MakeRelative">\</a> MSBuild MakeRelative  
 The MSBuild <CodeContentPlaceHolder>28\</CodeContentPlaceHolder> property function returns the relative path of the second path relative to first path. Each path can be a file or folder.  
...
##  \<a name="BKMK_ValueOrDefault">\</a> MSBuild ValueOrDefault  
 The MSBuild <CodeContentPlaceHolder>29\</CodeContentPlaceHolder> property function returns the first argument, unless it's null or empty. If the first argument is null or empty, the function returns the second argument.  
