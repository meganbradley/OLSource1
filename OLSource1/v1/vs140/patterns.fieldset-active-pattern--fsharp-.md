...
## Return Value  
 The formal return type is <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>. The option determines whether the input results in a match. In a pattern matching expression, the input is decomposed into a tuple of three elements. The first is an option that can contain an expression that represents the instance. This element is <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> if the field is static. The second element is a \<xref:System.Reflection.FieldInfo*> object, and the third element is an expression that represents the value to set the field to.  
