...
## Remarks  
 User-defined types can overload the `>` operator (see [operator](../vs140/operator--csharp-reference-2.md)). If `>` is overloaded, [<](../vs140/--operator--csharp-reference-.md) must also be overloaded. When a binary operator is overloaded, the corresponding assignment operator, if any, is also implicitly overloaded.  
