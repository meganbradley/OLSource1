...
You can update a Visual Studio extension on your system by using **Extensions and Updates** to install the updated version. If you create an updated version of an extension, you can signify it as updated by incrementing the version number in the VSIX manifest.  
 ...
 Updates are installed when the VSIX manifest of the incoming extension has the same <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> as the installed one and a higher <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> number. If the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> number is the same or lower, the package cannot be installed. If the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> values do not match, the package that is not yet installed is recognized as a separate extension.  
