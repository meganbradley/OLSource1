...
## Remarks  
 Call it in a loop as you sample mouse position, in order to give visual feedback. When you call <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>, the previous rectangle is erased and a new one is drawn. For example, as the user drags a rectangle across the screen, <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> will erase the original rectangle and redraw a new one in its new position. By default, <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> draws the rectangle by using a halftone brush to eliminate flicker and to create the appearance of a smoothly moving rectangle.  
  
 The first time you call <CodeContentPlaceHolder>8\</CodeContentPlaceHolder>, the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> parameter should be **NULL**.  
