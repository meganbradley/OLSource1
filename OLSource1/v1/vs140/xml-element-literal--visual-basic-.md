...
-   `attributeList`  
    ...
    -   Attribute assignment, of the form [`aPrefix``:`]`aName``=``aValue`, where:  
        ...
        |`aName`|Required. Name of the attribute. The format is one of the following:<br /><br /> -   Literal text. See [Declared XML Element and Attribute Names](../vs140/names-of-declared-xml-elements-and-attributes--visual-basic-.md).<br />-   Embedded expression of the form `<%=` `aNameExp` `%>`. The type of `aNameExp` must be `String` or a type that is implicitly convertible to <xref:System.Xml.Linq.XName*>.|  
...
>  An XML literal can span multiple lines without using line continuation characters. This feature enables you to copy content from an XML document and paste it directly into a [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] program.  
 ...
 The [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] compiler converts the XML element literal into calls to the [XElement](assetId:///M:System.Xml.Linq.XElement.#ctor(System.Xml.Linq.XName)?qualifyHint=False&autoUpgrade=True) constructor and, if it is required, the [XAttribute](assetId:///M:System.Xml.Linq.XAttribute.#ctor(System.Xml.Linq.XName,System.Object)?qualifyHint=False&autoUpgrade=True) constructor.  
