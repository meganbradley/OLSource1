...
4.  In the **Name** box, specify a name such as <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>, and then choose **OK**.  
...
8.  Choose **Zoom in**.  
     ...
     The events in the Timeline details confirm visible trends in the CPU utilization graph: there are a lot of events taking place over short periods of time. The Timeline details view shows that these events are <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>, and <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> events.  
  
9. Use the context menu (or right-click) one of the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> events in the lower pane, and choose **Filter to event**. The following illustration shows an example of details typical for one of the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> events in this test app.  
    ...
    -   Each <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> event, color-coded to identify it as a scripting event, includes a call to <CodeContentPlaceHolder>12\</CodeContentPlaceHolder>, followed by a style calculation and a call to <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>14\</CodeContentPlaceHolder>.  
  
    -   In the short time span selected (approximately one to two seconds), there are a great number of <CodeContentPlaceHolder>15\</CodeContentPlaceHolder>, <CodeContentPlaceHolder>16\</CodeContentPlaceHolder>, and <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> events taking place. The <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> events occur far more frequently than the one update per second that is visibly apparent after you run the app and choose the **Waiting for values** button.  
  
10. To investigate, choose the link to the anonymous function for one of the <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> events in the lower left pane. The following function opens in default.js:  
  
<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>  
     This recursive function sets up a loop that calls the <CodeContentPlaceHolder>20\</CodeContentPlaceHolder> function, which updates the button in the UI. By examining the different timer events in the profiler, you discover that most or all of the timer events result from this code, which is running too frequently, so it appears likely that the problem originates here.  
  
### Fixing the performance issue  
  
1.  Replace the <CodeContentPlaceHolder>21\</CodeContentPlaceHolder> function with the following code:  
  
<CodeContentPlaceHolder>4\</CodeContentPlaceHolder>  
     This fixed version of the code includes a 1000 millisecond delay, which was omitted from the previous version of the code, resulting in the use of a default delay value. From profiler data, it appears that the default value is zero milliseconds, which caused the <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> function to run too frequently.  
