...
---
# CWnd::get_accParent
Called by the framework to retrieve the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> interface of the object's parent.  
...
#### Parameters  
 *ppdispParent*  
 Receives the address of the parent object's <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> interface. The variable is set to NULL if no parent exists, or if the child cannot access its parent.  
