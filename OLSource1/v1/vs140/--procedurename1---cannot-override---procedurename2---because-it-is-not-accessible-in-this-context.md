...
A procedure or property overrides a procedure or property with an access level that prevents access by the overriding procedure or property.  
 ...
 For example, if a procedure is declared as <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> in one assembly, it cannot be accessed outside that assembly. If a procedure in another assembly in the same project attempts to override the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> procedure, it cannot access it to override it.  
...
-   Remove the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> keyword.  
