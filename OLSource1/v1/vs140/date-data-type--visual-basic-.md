...
-   **Interop Considerations.** If you are interfacing with components not written for the .NET Framework, for example Automation or COM objects, keep in mind that date/time types in other environments are not compatible with the Visual Basic `Date` type. If you are passing a date/time argument to such a component, declare it as `Double` instead of `Date` in your new Visual Basic code, and use the conversion methods [DateTime.FromOADate](assetId:///M:System.DateTime.FromOADate(System.Double)?qualifyHint=True&autoUpgrade=True) and <xref:System.DateTime.ToOADate*?displayProperty=fullName>.  
