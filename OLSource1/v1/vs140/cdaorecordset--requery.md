...
## Remarks  
 ...
 If the attempt to rebuild the recordset fails, the recordset is closed. Before you call **Requery**, you can determine whether the recordset can be requeried by calling the [CanRestart](../vs140/cdaorecordset--canrestart.md) member function. <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> does not guarantee that **Requery** will succeed.  
...
>  Calling [Requery](#_mfc_cdaorecordset.3a3a.requery) changes DAO bookmarks.  
 ...
 You can't call **Requery** on a dynaset-type or snapshot-type recordset if calling <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> returns 0, nor can you use it on a table-type recordset.  
  
 If both <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> return nonzero after you call **Requery**, the query didn't return any records and the recordset will contain no data.  
