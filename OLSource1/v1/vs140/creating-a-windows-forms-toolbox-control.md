...
3.  This adds a user control, a `ProvideToolboxControlAttribute`<xref:Microsoft.VisualStudio.Shell.RegistrationAttribute?qualifyHint=False> to place the control in the **Toolbox**, and a **Microsoft.VisualStudio.ToolboxControl** Asset entry in the VSIX manifest for deployment.  
  
### Building a User Interface for the Control  
 The `Counter` control requires two child controls: a <xref:System.Windows.Forms.Label?qualifyHint=False> to display the current count, and a <xref:System.Windows.Forms.Button?qualifyHint=False> to reset the count to 0. No other child controls are required because callers will increment the counter programmatically.  
...
4.  Put the following code in the `Load` event for the control.  
    ...
    ```  
     ...
     Setting the **Label** text in the <xref:System.Windows.Forms.UserControl.Load?qualifyHint=False> event enables the target properties to load before their values are applied. Setting the **Label** text in the constructor would result in an empty **Label**.  
...
5.  Drag a `Counter` control to your form, and then select it. The `Value`, `Message`, and `ShowReset` properties will be displayed in the **Properties** window, together with the properties that are inherited from <xref:System.Windows.Forms.UserControl?qualifyHint=False>.  
  
6.  Set the `Message` property to `Count:`.  
  
7.  Drag a <xref:System.Windows.Forms.Button?qualifyHint=False> control to the form, and then set the name and text properties of the button to `Test`.  
