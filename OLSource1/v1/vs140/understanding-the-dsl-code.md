...
-   SerializationHelper class, which provides functions that are used in common by the serialization classes.  
 ...
 A class for every shape class in the DSL Definition. Shapes are derived from <xref:Microsoft.VisualStudio.Modeling.Diagrams.NodeShape?qualifyHint=False>. For more information, see [Customizing Files](../vs140/customizing-file-storage-and-xml-serialization.md).  
...
```  
 ...
 Instantiates `DocData` and `DocView`. It fulfills a standard interface that [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] uses to open an editor when your DSL package starts. It is referenced in the `ProvideEditorFactory` attribute in Package.cs  
