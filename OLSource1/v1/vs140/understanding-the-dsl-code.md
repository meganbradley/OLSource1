...
A Domain-Specific Language (DSL) solution generates an API that you can use to read and update instances of the DSL in [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)]. This API is defined in the code that is generated from the DSL definition. This topic describes the generated API.  
...
>  The Component Diagrams DSL template is not related to the UML component diagrams that you can create by using the Architecture menu in Visual Studio. In the **New Project** dialog box, expand **Other Project Types\Extensibility** and then click **Domain-Specific Language Designer**.  
 ...
 ![Components and interconnected ports](../vs140/media/componentsample.png "ComponentSample")  
  
## The Structure of the DSL Solution  
 The **Dsl** project defines the API for your DSL. The **DslPackage** project defines how it integrates with [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)]. You can also add your own projects, which can also contain code generated from the model.  
...
### The code directories  
 ...
 The \*.tt files contain very little generating code. Instead, they use <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> directives to include shared template files. The shared files can be found in **\Program Files\Microsoft Visual Studio 10.0\Common7\IDE\Extensions\Microsoft\DSL SDK\DSL Designer\11.0\TextTemplates**  
...
-   Write your own text templates to generate code specific to your application. You can use include files to share parts of the templates that are common to many projects, and you can create [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] project templates to set up projects that are initialized with your own file structure.  
  
## Generated Files in Dsl  
 The following generated files appear in the **Dsl** project.  
  
 *YourDsl* <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>  
  
 The schema for files that contains instances of your DSL. This file is copied to the compilation (**bin**) directory. When you install your DSL, you can copy this file to **\Program Files\Microsoft Visual Studio 11.0\Xml\Schemas** so that model files can be validated. For more information, see [Deployment](../vs140/deploying-domain-specific-language-solutions.md).  
  
 If you customize serialization by setting options in DSL Explorer, the schema will change accordingly. However, if you write your own serialization code, this file might no longer represent the actual schema. For more information, see [Customization](../vs140/customizing-file-storage-and-xml-serialization.md).  
 ...
 The relationship is created in the  *Relationship*<CodeContentPlaceHolder>5\</CodeContentPlaceHolder> method. The default version verifies that the source and target model elements are acceptable, and then instantiates the relationship. For example:  
  
 <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>  
  
 Each builder class is generated from a node in the **Connection Builders** section in DSL Explorer. One <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> method can create relationships between one or more pairs of domain classes. Each pair is defined by a Link Connect Directive, which you can find in DSL Explorer under the builder node.  
 ...
 In addition, this file contains the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> rule, which responds when a new element is added to the model. The rule adds a new shape and links the shape to the model element.  
  
 <CodeContentPlaceHolder>11\</CodeContentPlaceHolder>  
  
 This directive processor helps your users to write text templates that read an instance of your DSL. The directive processor loads the assemblies (DLLs) for your DSL, and effectively inserts <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> statements for your namespace. This allows the code in the text templates to use the classes and relationships that you have defined in your DSL.  
  
 For more information, see [Generating Code from a DSL](../vs140/generating-code-from-a-domain-specific-language.md) and [Creating Custom Directive Processors](../vs140/creating-custom-t4-text-template-directive-processors.md).  
...
-   A property definition and a nested handler class for each domain property. You can override OnValueChanging() and OnValueChanged(). For more information, see [Domain Property Change Handlers](../vs140/domain-property-value-change-handlers.md).  
  
     In the example DSL, the <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> class contains a property <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> and a handler class <CodeContentPlaceHolder>16\</CodeContentPlaceHolder>.  
...
-   Accessor properties for the relationships in which this domain class participates. (There is no nested class for role properties.)  
     ...
     In the example DSL, the <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> class has accessors that access its parent model through the embedding relationship <CodeContentPlaceHolder>18\</CodeContentPlaceHolder>.  
...
>  This is not the same as the root class of the model.  
 ...
 Copy and Delete Closures define what other elements should be included when an element is copied or deleted. You can control this behavior by setting the **Propagates Copy** and **Propagates Delete** properties of the roles at each side of every relationship. If you want the values to be determined dynamically, you can write code to override the methods of the Closure classes. For more information see [Customizing Copy and Delete](../vs140/how-to--program-copy-and-paste-behavior---redirect.md).  
 ...
 For these constraints to be executed, you must have set one of the **Uses...** options in the **Editor\Validation** node in DSL Explorer. For more information, see [Validation](../vs140/validation-in-a-domain-specific-language.md).  
  
 <CodeContentPlaceHolder>24\</CodeContentPlaceHolder>  
  
 This file contains code only if you have attached a Custom Type Descriptor to a domain property. For more information, see [Customizing the Properties Window](../vs140/customizing-the-properties-window.md).  
  
 <CodeContentPlaceHolder>25\</CodeContentPlaceHolder>  
  
-   A validation method to ensure that no two elements are referenced by the same moniker. For more information, see [Customizing File Storage](../vs140/customizing-file-storage-and-xml-serialization.md).  
...
-   SerializationHelper class, which provides functions that are used in common by the serialization classes.  
 ...
 A class for every shape class in the DSL Definition. Shapes are derived from \<xref:Microsoft.VisualStudio.Modeling.Diagrams.NodeShape*>. For more information, see [Customizing Files](../vs140/customizing-file-storage-and-xml-serialization.md).  
 ...
 You could override <CodeContentPlaceHolder>29\</CodeContentPlaceHolder> to define a toolbox item that creates a group of several objects. For example, you could define an item to represent objects that have sub-components. After changing the code, reset the experimental instance of [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] to clear the toolbox cache.  
  
## Generated files in the DslPackage project  
 DslPackage couples the DSL model to the [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] shell, managing the window, toolbox, and menu commands. Most of the classes are double derived, so that you can override any of their methods.  
  
 <CodeContentPlaceHolder>30\</CodeContentPlaceHolder>  
  
 The context menu commands that are visible on the diagram. You can adapt or add to this set. This file contains the code for the commands. The location of the commands on menus is determined by the Commands.vsct file. For more information, see [Commands](../vs140/writing-user-commands-and-actions.md).  
 ...
 *YourDsl* <CodeContentPlaceHolder>33\</CodeContentPlaceHolder> manages loading and saving a model to file, and creates the Store instance.  
  
 For example, if you want to save your DSL in a database instead of a file, you could override <CodeContentPlaceHolder>34\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>35\</CodeContentPlaceHolder> methods.  
  
 <CodeContentPlaceHolder>36\</CodeContentPlaceHolder>  
  
 *YourDsl* <CodeContentPlaceHolder>37\</CodeContentPlaceHolder> manages the window in which the diagram appears. For example, you could embed the diagram inside a windows Form:  
...
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 ...
 Instantiates <CodeContentPlaceHolder>39\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>40\</CodeContentPlaceHolder>. It fulfills a standard interface that [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] uses to open an editor when your DSL package starts. It is referenced in the <CodeContentPlaceHolder>41\</CodeContentPlaceHolder> attribute in Package.cs  
  
 <CodeContentPlaceHolder>42\</CodeContentPlaceHolder>  
  
 Locates the standard menu commands on menus, such as the diagram context menu, the **Edit** menu, and so on. The code for the commands is in CommandSet.cs. You can relocate or modify the standard commands, and you can add your own commands. For more information, see [Commands](../vs140/writing-user-commands-and-actions.md).  
 ...
 For example, you could override <CodeContentPlaceHolder>44\</CodeContentPlaceHolder> to change the order in which elements appear in the Model Explorer.  
...
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 ...
 This file defines how the DSL integrates into [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)]. Attributes on the package class register the DSL as the handler for files that have your file extension, define its toolbox, and define how to open a new window. The Initialize() method is called one time when the first DSL is loaded into a [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] instance.  
  
 <CodeContentPlaceHolder>47\</CodeContentPlaceHolder>  
  
 To customize this file, edit the <CodeContentPlaceHolder>48\</CodeContentPlaceHolder> file.  
  
> [!WARNING]
>  If you edit the .tt file to include resources such as icons or images, make sure that the resource is included in the VSIX build. In Solution Explorer, select the file and make sure that the **Include in VSIX** property is <CodeContentPlaceHolder>49\</CodeContentPlaceHolder>.  
  
 This file controls how the DSL is packaged into a Visual Studio Integration Extension (VSIX). For more information, see [Deployment](../vs140/deploying-domain-specific-language-solutions.md).  
  
## See Also  
 [How to Define a Domain-Specific Language](../vs140/how-to-define-a-domain-specific-language.md)   
 [Understanding Models, Relationships, and Classes](../vs140/understanding-models--classes-and-relationships.md)   
 [Customizing and Extending a DSL](../vs140/customizing-and-extending-a-domain-specific-language.md)   
 [Writing Code to Customize a DSL](../vs140/writing-code-to-customise-a-domain-specific-language.md)