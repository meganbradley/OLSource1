...
### SpanID  
 Use the cvSpanId field, an int, to match pairs of events. The value for each pair of start/stop events that represent a span must be unique. Typically for concurrent code, this requires the use of synchronization primitives such as [Exchange](assetId:///M:System.Threading.Interlocked.Exchange(System.Object@,System.Object)?qualifyHint=False&autoUpgrade=True) to ensure that the key (the value that's used for CvSpanID) is correct.  
