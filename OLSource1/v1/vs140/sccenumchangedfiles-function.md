...
#### Parameters  
 ...
 cFiles  
 [in] Number of file names specified in the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> array. Also specifies size of <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> array.  
  
 lpFileNames  
 [in] Array of local file names to check.  
  
 plIsFileDifferent  
 [in, out] Array of values indicating the difference status of each file (array must have at least <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> entries). Nonzero means that the file is different.  
