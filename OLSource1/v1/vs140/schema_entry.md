...
## Remarks  
 [IDBSchemaRowsetImpl](../vs140/idbschemarowsetimpl-class.md) can then query the map for a list of GUIDs, or it can create a rowset if it is given a GUID. The schema rowset <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> creates is similar to a standard <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>-derived class, except it must provide an **Execute** method that has the following signature:  
