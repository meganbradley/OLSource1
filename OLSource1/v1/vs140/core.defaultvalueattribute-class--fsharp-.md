...
Adding this attribute to a field declaration means that the field is not initialized. During type checking a constraint is asserted that the field type supports `null`. If the [Check](../vs140/defaultvalueattribute.check-property--fsharp-.md) value is `false` then the constraint is not asserted.  
...
## Remarks  
 ...
 The .NET Framework namespace <xref:System.ComponentModel?qualifyHint=False> defines an attribute that has the same name:  <xref:System.ComponentModel.DefaultValueAttribute?qualifyHint=False>. Therefore, you must fully qualify the F# attribute if you open the <xref:System.ComponentModel?qualifyHint=False> namespace.  
