...
## Example  
 The following application uses the library that was defined previously. Note that the type that is declared outside a namespace is created when the name <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> is not qualified by a namespace. Note also that to access the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> type in <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, the namespace name is required.  
