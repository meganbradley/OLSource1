...
Any program element — such as a variable, class, or member — can have the same name as a restricted keyword. For example, you can create a variable named <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>. However, to refer to your version of it — which has the same name as the restricted <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> keyword — you must either precede it with a full qualification string or enclose it in square brackets (<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>), as the following example shows.  
  
 [!code[VbVbcnConventions#8](../vs140/codesnippet/VisualBasic/keywords-as-element-names-in-code--visual-basic-_1.vb)]  
  
 If you do not do either of these, then Visual Basic assumes use of the intrinsic <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> keyword and produces an error, as in the following example:  
