---
title: "CMFCTasksPane::IsAnimationEnabled"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "reference"
f1_keywords: 
  - "CMFCTasksPane.IsAnimationEnabled"
  - "CMFCTasksPane::IsAnimationEnabled"
  - "IsAnimationEnabled"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "IsAnimationEnabled method"
ms.assetid: 842dfb4f-2aa8-4aa9-8ffb-94c72ad1a7d2
caps.latest.revision: 14
robots: noindex,nofollow
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# CMFCTasksPane::IsAnimationEnabled
Indicates whether animation is enabled.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Return Value  
 <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> if the animation that occurs when a user expands or collapses a group is enabled; otherwise, <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>.  
  
## Remarks  
 Call [CMFCTasksPane::EnableAnimation](../vs140/cmfctaskspane--enableanimation.md) to enable or disable animation.  
  
## Requirements  
 **Header:** afxTasksPane.h  
  
## See Also  
 [CMFCTasksPane Class](../vs140/cmfctaskspane-class.md)   
 [MFC Hierarchy Chart](../vs140/hierarchy-chart.md)