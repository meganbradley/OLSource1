...
A class using a <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> attribute block is declared inside a class that is not <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>. If a class is to be exposed as a COM object, its entire containment hierarchy must be declared with <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> access.  
  
 **Error ID:** BC32504  
  
### To correct this error  
  
-   Declare all containing classes <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, or remove the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> attribute block.  
