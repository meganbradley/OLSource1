...
## Property pages  
 ...
 The following illustration shows the property pages for a Visual C++ project. In the left pane, the                  **VC++ Directories***rule* is selected, and the right pane lists the properties that are associated with that rule. The                  <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> values are unfortunately called                  *macros*. These are                  *not* C/C++ macros but simply compile-time constants. Macros are discussed in the                  [Property Page Macros](#bkmkPropertiesVersusMacros) section later in this article.)  
...
### Predefined macros  
 global macros  
 Applies to all items in a project configuration. Has the syntax                                  <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>. An example of a global macro is                                  <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, which stores the root directory of your Visual Studio installation. A global macro corresponds to a                                  <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> in MSBuild.  
  
 item macros  
 Has the syntax                                  <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>. For a file, an item macro applies only to that file—for example, you can use                                  <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> to specify include directories that apply only to a particular file. This kind of item macro corresponds to an                                  <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> metadata in MSBuild. When it's used in the context of a project configuration, an item macro applies to all files of a certain type. For example, the C/C++                                  **Preprocessor Definitions** configuration property can take a                                  <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> item macro that applies to all .cpp files in the project. This kind of item macro corresponds to an                                  <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> metadata in MSBuild. For more information, see                                  [Item Definitions](../vs140/item-definitions.md).  
...
> [!TIP]
>  On a property page, a property in                      <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> is defined in the current context. A property in normal font is inherited.  
