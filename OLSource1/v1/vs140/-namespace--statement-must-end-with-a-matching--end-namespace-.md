---
title: "&#39;Namespace&#39; statement must end with a matching &#39;End Namespace&#39;"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "vbc30626"
  - "bc30626"
helpviewer_keywords: 
  - "BC30626"
ms.assetid: d3c74252-0e71-4621-ad83-4e2183b7b442
caps.latest.revision: 12
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# &#39;Namespace&#39; statement must end with a matching &#39;End Namespace&#39;
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder> blocks must end with an <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> statement.  
  
 **Error ID:** BC30626  
  
### To correct this error  
  
-   Add an <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> statement to the end of the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> block.  
  
## See Also  
 [Namespace Statement](../vs140/namespace-statement.md)   
 [Namespaces in Visual Basic](../vs140/namespaces-in-visual-basic.md)