---
title: "Command requires one argument."
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "vs.message.VS_E_INCORRECTPARAMCOUNT1"
  - "vs.message.0x800A00C3"
ms.assetid: b4d98e6d-6970-42fb-b1de-43f2e952fb9d
caps.latest.revision: 11
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# Command requires one argument.
This error generally occurs when not enough information has been entered for the command or because an incorrect combination of arguments was used.  
  
 For example, if <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> has been entered for the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> command, this error will appear because alias <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> can only take one alias name, not two. Alias names do not contain spaces, so the **Find/Command** box and **Command** window interpret <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> as two different alias names.  
  
### To correct this error  
  
1.  Check the documentation for the correct syntax for the command and try again.  
  
## See Also  
 [Visual Studio Commands](../vs140/visual-studio-commands.md)