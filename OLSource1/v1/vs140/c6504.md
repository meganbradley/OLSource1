---
title: "C6504"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "vs-devops-test"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "C6504"
helpviewer_keywords: 
  - "C6504"
ms.assetid: 6baeed46-e73d-4974-af16-7487c55b3473
caps.latest.revision: 18
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# C6504
warning C6504: invalid annotation: property may only be used on values of pointer, pointer-to-member, or array type  
  
 This warning indicates the use of a property on an incompatible data type. For more information about data types supported by properties, see [Annotation Properties](assetId:///f77b4370-6bda-4294-bd2a-e7d0df182a3d).  
  
## Example  
 The following code generates this warning because the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> property cannot be used on the reference data type.  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 To correct this warning, use the following code:  
  
<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>  
 The defective code shown earlier also generates warning [C6516](../vs140/c6516.md) because property conflicts resulted in an invalid annotation.