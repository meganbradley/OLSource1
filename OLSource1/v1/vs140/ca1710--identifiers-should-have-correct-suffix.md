...
|<xref:System.Collections.Generic.ICollection<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>2*?displayProperty=fullName>|Dictionary|  
...
|An event-handler delegate.|EventHandler|  
 ...
 Types that implement \<xref:System.Collections.ICollection*> and are a collection of specific items have names that end with the word 'Collection'. For example, a collection of \<xref:System.Collections.Queue*> objects would have the name 'QueueCollection'. The 'Collection' suffix signifies that the members of the collection can be enumerated by using the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> (<CodeContentPlaceHolder>4\</CodeContentPlaceHolder> in [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)]) statement.  
