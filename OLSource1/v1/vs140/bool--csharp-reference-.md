...
---
# bool (C# Reference)
The `bool` keyword is an alias of <xref:System.Boolean?qualifyHint=True>. It is used to declare variables to store the Boolean values, [true](../vs140/true--csharp-reference-.md) and [false](../vs140/false--csharp-reference-.md).  
...
## Example  
 In this example, you enter a character from the keyboard and the program checks if the input character is a letter. If it is a letter, it checks if it is lowercase or uppercase. These checks are performed with the <xref:System.Char.IsLetter?qualifyHint=False>, and <xref:System.Char.IsLower?qualifyHint=False>, both of which return the `bool` type:  
