...
```  
 ...
 In the previous code example, the query expression is in curly braces. The meaning of the code in the expression is, return every customer in the Customers table in the database in the query results. Query expressions return a type that implements <xref:System.Linq.IQueryable`1*> and <xref:System.Collections.Generic.IEnumerable`1*>, and so they can be iterated using the [Seq module](../vs140/collections.seq-module--fsharp-.md) as the example shows.  
