...
---
# CRichEditCtrl::ReplaceSel
Replaces the current selection in this <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> object with the specified text.  
...
## Remarks  
 To replace all the text in this <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> object, use [CWnd::SetWindowText](../vs140/cwnd--setwindowtext.md).  
 ...
 This function will format the inserted text with the existing character formatting. When replacing the entire range of text (by calling <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>(0,-1) before calling <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>), there is an end of paragraph character that retains the previous paragraph's formatting, which in inherited by the newly inserted text.  
