...
##  \<a name="BKMK_Trigger_Process_Lifecycle_Management_events">\</a> Trigger Process Lifetime Management events  
 Windows can suspend your app when the user switches away from it or when Windows enters a low power state. You can respond to the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> event to save relevant app and user data to persistent storage and to release resources. When an app is resumed from the **Suspended** state, it enters the **Running** state and continues from where it was when it was suspended. You can respond to the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> event to restore or refresh application state and reclaim resources.  
