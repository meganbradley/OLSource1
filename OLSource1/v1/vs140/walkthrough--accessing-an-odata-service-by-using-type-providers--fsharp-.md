...
4.  Specify a substring condition to the query by using the [Contains](assetId:///M:System.String.Contains(System.String)?qualifyHint=False&autoUpgrade=True) method. The following query returns all products that have "Chef" in their names. Also notice the use of [GetValueOrDefault](assetId:///M:System.Nullable`1.GetValueOrDefault(`0)?qualifyHint=False&autoUpgrade=True). The `UnitPrice` is a nullable value, so you must either get the value by using the `Value` property, or you must call <xref:System.Nullable`1.GetValueOrDefault*>.  
...
5.  Use the [EndsWith](assetId:///M:System.String.EndsWith(System.String)?qualifyHint=False&autoUpgrade=True) method to specify that a string ends with a certain substring.  
