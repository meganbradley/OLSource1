...
## Remarks  
 ...
 An application typically uses <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> to give the user feedback about each key pressed. For example, an application can display the accent in the current character position without moving the caret.  
  
 Since there is not necessarily a one-to-one correspondence between keys pressed and <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> calls, the information in <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> is generally not useful to applications. The information in <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> applies only to the most recent call to the [OnKeyUp](../vs140/cwnd--onkeyup.md) member function or the [OnKeyDown](../vs140/cwnd--onkeydown.md) member function that precedes the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> call.  
