...
This document describes the pages (or properties collections) that are associated with the **Environment** category, <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, of the **Options** dialog box. The title of each subsection is the call that is used to access the Properties collection, and the table in each subsection lists the properties in the collection.  
...
|Property Item Name|Value|Description|  
|------------------------|-----------|-----------------|  
|ReuseSavedActiveDocWindow|Get/Set (Boolean)|Determines whether opening a new file reuses the current document window if the current document is saved. <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> means always open a new document window for each document opened.|  
...
|AutoloadExternalChanges|Get/Set (Boolean)|Determines whether detected external modifications to open documents automatically reload the modified file if the open document is not modified. If the open document is modified and this property is <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>, then the IDE prompts as if this property were <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>.|  
...
|CommentTokens|SafeArray|Returns a SafeArray of comment token values. Each has the fields, <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> (string) and <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> (\<xref:EnvDTE.vsTaskPriority*>, High, Medium, or Low).|  
