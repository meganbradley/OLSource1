---
title: "File not found (Visual Basic Run-Time Error)"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-visual-basic"
ms.tgt_pltfrm: na
ms.topic: "article"
f1_keywords: 
  - "vbrID53"
dev_langs: 
  - "VB"
ms.assetid: 57addb16-6f9a-444d-8af8-dda52431daca
caps.latest.revision: 12
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# File not found (Visual Basic Run-Time Error)
The file was not found where specified. The error has the following possible causes:  
  
-   A statement refers to a file that does not exist.  
  
-   An attempt was made to call a procedure in a dynamic-link library (DLL), but the library specified in the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> clause of the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> statement cannot be found.  
  
-   You attempted to open a project or load a text file that does not exist.  
  
### To correct this error  
  
1.  Check the spelling of the file name and the path specification.  
  
## See Also  
 [Declare Statement](../vs140/declare-statement.md)