...
A declaration or assignment statement includes an [Of](../vs140/of-clause--visual-basic-.md) clause when invoking a non-generic type.  
 ...
 Type parameters are defined with an <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> clause inside parentheses, and type arguments are supplied using an <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> clause inside parentheses. The <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> clause is used only when dealing with generic types.  
...
2.  If you are invoking a non-generic type, remove the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> clause and its parentheses, if any. Do not remove parentheses surrounding a standard argument list for a procedure, delegate, or class constructor.  
