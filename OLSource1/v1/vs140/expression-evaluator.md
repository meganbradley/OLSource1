...
2.  The debug package gets an <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> object from an [IDebugStackFrame2](../vs140/idebugstackframe2.md) interface and then calls the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> method on it to get an [IDebugExpression2](../vs140/idebugexpression2.md) object.  
  
3.  The debug package calls the [EvaluateSync](../vs140/idebugexpression2--evaluatesync.md) method or the [EvaluateAsync](../vs140/idebugexpression2--evaluateasync.md) method to get the value of the expression. <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> is called from the Command/Immediate window. All other UI components call <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>.  
...
4.  The result of expression evaluation is an [IDebugProperty2](../vs140/idebugproperty2.md) object, which contains the name, type, and value of the result of the expression evaluation.  
 ...
 When asynchronous expression evaluation is complete, an asynchronous event is sent by the DE through the session debug manager (SDM) to notify the IDE that expression evaluation is complete. When synchronous expression evaluation is complete, the result of the evaluation is returned from the call to the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> method.  
