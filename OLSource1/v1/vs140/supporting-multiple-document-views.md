...
Multiple views  
 ...
 By default, each view (document view object) is contained in its own window frame (\<xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowFrame*>). As already noted, however, document data can be displayed in multiple views. To enable this, Visual Studio checks the RDT to determine whether the document in question is already open in an editor. When the IDE calls \<xref:Microsoft.VisualStudio.Shell.Interop.IVsEditorFactory.CreateEditorInstance*> to create the editor, a non-NULL value returned in the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> parameter indicates that the document is already open in another editor. For more information about how the RDT functions, see [Running Document Table](../vs140/running-document-table.md).  
  
 In your \<xref:Microsoft.VisualStudio.Shell.Interop.IVsEditorFactory*> implementation, examine the document data object returned in <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> to determine whether the document data is appropriate for your editor. (For example, only HTML data should be displayed by an HTML editor.) If it is appropriate, then your editor factory should provide a second view for the data. If the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> parameter is not <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, it is possible either that the document data object is open in another editor, or, more likely, that the document data is already open in a different view with same the editor. If the document data is open in a different editor that your editor factory does not support, then Visual Studio fails to open your editor factory. For more information, see [How to: Attach Views to Document Data](../vs140/how-to--attach-views-to-document-data.md).