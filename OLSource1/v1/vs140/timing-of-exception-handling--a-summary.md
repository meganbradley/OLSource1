...
A termination handler is executed no matter how the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> statement block is terminated. Causes include jumping out of the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> block, a <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> statement that transfers control out of the block, and unwinding the stack due to exception handling.  
  
> [!NOTE]
>  Visual C++ supports two forms of the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> statements. The fast version bypasses termination handling but is more efficient. To use this version, include the file SETJMP.H. The other version supports termination handling as described in the previous paragraph. To use this version, include the file SETJMPEX.H. The increase in performance of the fast version depends on hardware configuration.  
