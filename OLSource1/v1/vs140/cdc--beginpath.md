...
## Remarks  
 After a path bracket is open, an application can begin calling GDI drawing functions to define the points that lie in the path. An application can close an open path bracket by calling the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> member function. When an application calls <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, any previous paths are discarded.  
