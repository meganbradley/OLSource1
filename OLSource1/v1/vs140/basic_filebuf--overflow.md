...
-   It can convert any pending output in the output buffer, followed by **ch**, by using the file conversion facet **fac** to call **fac.out** as needed. Each element <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> of type *char* thus produced is written to the associated stream designated by the file pointer **fp** as if by successive calls of the form <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>(**ch**, **fp**). If any conversion or write fails, the function does not succeed.  
