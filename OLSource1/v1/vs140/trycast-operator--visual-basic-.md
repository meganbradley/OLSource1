...
## Remarks  
 If an attempted conversion fails, `CType` and `DirectCast` both throw an <xref:System.InvalidCastException*> error. This can adversely affect the performance of your application. `TryCast` returns [Nothing (Visual Basic)](../vs140/nothing--visual-basic-.md), so that instead of having to handle a possible exception, you need only test the returned result against `Nothing`.  
