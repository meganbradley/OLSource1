...
4.  When the ATL Property Page Wizard appears, enter <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> as the **Short** name.  
...
|File|Description|  
|----------|-----------------|  
|PolyProp.h|Contains the C++ class <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, which implements the property page.|  
...
-   The <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> class is added to the Polygon.idl file.  
...
3.  Select that label and change it to read <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> by altering the **Caption** text in the **Properties** window.  
...
6.  Finally, change the **ID** of the Edit control to <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> using the Properties window.  
...
## Adding Code to Create and Manage the Property Page  
 ...
 First, enable the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> class to set the number of sides in your object when the **Apply** button is pressed.  
  
#### To modify the Apply function to set the number of sides  
  
1.  Replace the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> function in PolyProp.h with the following code:  
  
     [!code[NVC_ATL_Windowing#58](../vs140/codesnippet/CPP/adding-a-property-page--atl-tutorial--part-6-_1.h)]  
  
 A property page can have more than one client attached to it at a time, so the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> function loops around and calls <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> on each client with the value retrieved from the edit box. You are using the [CComQIPtr](../vs140/ccomqiptr-class.md) class, which performs the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> on each object to obtain the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> interface from the **IUnknown** interface (stored in the <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> array).  
  
 The code now checks that setting the <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> property actually worked. If it fails, the code displays a message box displaying error details from the **IErrorInfo** interface. Typically, a container asks an object for the **ISupportErrorInfo** interface and calls <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> first, to determine whether the object supports setting error information. You can skip this task.  
  
 [CComPtr](../vs140/ccomptr-class.md) helps you by automatically handling the reference counting, so you do not need to call <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> on the interface. <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> helps you with <CodeContentPlaceHolder>16\</CodeContentPlaceHolder> processing, so you do not have to perform the final <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> call. You also use one of the various string conversion classes, so you can convert the <CodeContentPlaceHolder>18\</CodeContentPlaceHolder> if necessary (this is why the <CodeContentPlaceHolder>19\</CodeContentPlaceHolder> macro is at the start of the function).  
...
3.  Expand the <CodeContentPlaceHolder>20\</CodeContentPlaceHolder> node in the event list.  
  
4.  Select <CodeContentPlaceHolder>21\</CodeContentPlaceHolder>, and from the drop-down menu to the right, click **\<Add> OnEnChangeSides**. The <CodeContentPlaceHolder>22\</CodeContentPlaceHolder> handler declaration will be added to Polyprop.h, and the handler implementation to Polyprop.cpp.  
  
 Next, you will modify the handler.  
  
#### To modify the OnEnChangeSides method  
  
1.  Add the following code in Polyprop.cpp to the <CodeContentPlaceHolder>23\</CodeContentPlaceHolder> method (deleting any code that the wizard put there):  
  
     [!code[NVC_ATL_Windowing#59](../vs140/codesnippet/CPP/adding-a-property-page--atl-tutorial--part-6-_2.cpp)]  
  
 <CodeContentPlaceHolder>24\</CodeContentPlaceHolder> will be called when a **WM_COMMAND** message is sent with the **EN_CHANGE** notification for the <CodeContentPlaceHolder>25\</CodeContentPlaceHolder> control. <CodeContentPlaceHolder>26\</CodeContentPlaceHolder> then calls <CodeContentPlaceHolder>27\</CodeContentPlaceHolder> and passes <CodeContentPlaceHolder>28\</CodeContentPlaceHolder> to indicate the property page is now dirty and the **Apply** button should be enabled.  
...
1.  Open PolyCtl.h and add this line to the property map:  
 ...
 You could have added a <CodeContentPlaceHolder>29\</CodeContentPlaceHolder> macro with the CLSID of your property page, but if you use the <CodeContentPlaceHolder>30\</CodeContentPlaceHolder> macro as shown, the <CodeContentPlaceHolder>31\</CodeContentPlaceHolder> property value is also saved when the control is saved.  
...
## Building and Testing the Control  
 ...
 The **Apply** button is initially disabled. Start typing a value in the **Sides** box and the **Apply** button will become enabled. After you have finished entering the value, click the **Apply** button. The control display changes, and the **Apply** button is again disabled. Try entering an invalid value. You will see a message box containing the error description that you set from the <CodeContentPlaceHolder>32\</CodeContentPlaceHolder> function.  
