...
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistDocData*>|Data|Enables persistence for the document. For read-only files, call [SetDocDataReadOnly](assetId:///M:Microsoft.VisualStudio.Shell.Interop.IVsPersistDocData2.SetDocDataReadOnly(System.Int32)?qualifyHint=False&autoUpgrade=True) to provide the "lock" icon that indicates read-only files.|  
