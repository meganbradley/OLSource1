...
## Remarks  
 ...
 An item is selected only if its initial characters (from the starting point) match the characters in the string specified by <CodeContentPlaceHolder>5\</CodeContentPlaceHolder>.  
  
 Use the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> member function to find a string without selecting the item.  
