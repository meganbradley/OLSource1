...
-   If your implementation of either method recognizes both the GUID and the command, then the method should set the command-flags field of every command (in the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> parameter) by using the following flags:  
  
    -   \<xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF.OLECMDF_SUPPORTED*> if the command is supported.  
  
    -   \<xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF.OLECMDF_INVISIBLE*> if the command should not be visible.  
  
    -   \<xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF.OLECMDF_LATCHED*> if the command is toggled on and appears to have been checked.  
  
    -   \<xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF.OLECMDF_ENABLED*> if the command is enabled.  
  
    -   \<xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF.OLECMDF_DEFHIDEONCTXTMENU*> if the command should be hidden if it appears on a shortcut menu.  
  
    -   \<xref:Microsoft.VisualStudio.OLE.Interop.OLECMDF.OLECMDF_NINCHED*> if the command is a menu controller and is not enabled, but its drop-down menu list is not empty and is still available. (This flag is rarely used.)  
  
-   If the command was defined in the .vsct file with the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> flag, set the following parameters:  
  
    -   Set the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> element of the <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> parameter to the new text of the command.  
  
    -   Set the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> element of the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> parameter to the size of the command string.  
