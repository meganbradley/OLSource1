...
##  \<a name="_core_support_for_the_input_method_editor_.28.ime.29">\</a> Support for the Input Method Editor (IME)  
 ...
 Japanese keyboards do not directly support Kanji characters. The IME converts a phonetic string, entered in one of the other Japanese alphabets (Romaji, Katakana, or Hiragana) into its possible Kanji representations. If there is ambiguity, you can select from several alternatives. When you have selected the intended Kanji character, the IME passes two <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> messages to the controlling application.  
  
 The IME, activated by the ALT+\<CodeContentPlaceHolder>2\</CodeContentPlaceHolder>WM_MOVE<CodeContentPlaceHolder>3\</CodeContentPlaceHolder>WM_SIZE` messages by repositioning the conversion window to conform to the new location or size of the target window.  
