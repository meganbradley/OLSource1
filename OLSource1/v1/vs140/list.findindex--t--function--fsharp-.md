...
Returns the index of the first element in the list that satisfies the given predicate. Raises <xref:System.Collections.Generic.KeyNotFoundException?qualifyHint=False> if no such element exists.  
...
|Exception|Condition|  
|---------------|---------------|  
|<xref:System.ArgumentException?qualifyHint=False>|Thrown if the predicate evaluates to false for all the elements of the list.|  
