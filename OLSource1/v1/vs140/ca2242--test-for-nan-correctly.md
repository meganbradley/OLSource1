...
## Rule Description  
 \<xref:System.Double.NaN*?displayProperty=fullName>, which represents not-a-number, results when an arithmetic operation is undefined. Any expression that tests equality between a value and \<xref:System.Double.NaN*?displayProperty=fullName> always returns <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>. Any expression that tests inequality between a value and \<xref:System.Double.NaN*?displayProperty=fullName> always returns <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
