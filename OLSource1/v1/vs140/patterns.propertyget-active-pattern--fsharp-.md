...
apitype: "Assembly"
f1_keywords: 
  - "Microsoft.FSharp.Quotations.PatternsModule.PropertyGetPattern"
dev_langs: 
  - "FSharp"
helpviewer_keywords: 
  - "PropertyGet active pattern [F#]"
...
## Return Value  
 The formal return type is <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>. The option indicates whether the input results in a match. In a pattern matching expression, the input is decomposed, upon a successful match, into a tuple that has three elements. The first element is an optional expression that represents the instance. For a static property, this option is <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>. The second element is a \<xref:System.Reflection.PropertyInfo*> object that represents the property. The third element is a list that contains the arguments of the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> accessor, which is used for indexed properties.  
