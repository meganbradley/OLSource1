...
The <xref:Microsoft.Win32.RegistryKey.DeleteSubKey(System.String)?qualifyHint=False> and <xref:Microsoft.Win32.RegistryKey.DeleteSubKey(System.String,System.Boolean)?qualifyHint=False> methods can be used to delete registry keys.  
...
-   The user does not have permissions to delete registry keys (<xref:System.Security.SecurityException?qualifyHint=False>).  
...
## .NET Framework Security  
 Registry calls fail if either sufficient run-time permissions are not granted (<xref:System.Security.Permissions.RegistryPermission?qualifyHint=False>) or if the user does not have the correct access (as determined by the ACLs) for creating or writing to settings. For example, a local application that has the code access security permission might not have operating system permission.  
