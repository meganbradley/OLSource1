...
'type': Operators cannot be used on the published surface of a WinRT type  
 ...
 To fix this issue, remove the operator member function from the public interface, or change it to a named member function. For example, instead of <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>, name the member function <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.