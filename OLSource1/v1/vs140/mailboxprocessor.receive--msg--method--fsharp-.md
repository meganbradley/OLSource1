...
## Remarks  
 This method is for use within the body of the agent. For each agent, at most one concurrent reader may be active, so no more than one concurrent call to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>, [TryReceive](../vs140/mailboxprocessor.tryreceive--msg--method--fsharp-.md), [Scan](../vs140/mailboxprocessor.scan--msg--t--method--fsharp-.md) or [TryScan](../vs140/mailboxprocessor.tryscan--msg--t--method--fsharp-.md) may be active.  
  
## Example  
 The following example shows how to use the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> method. In this case, a timeout of 10 seconds is specified. In general, the message processing function runs on a different thread from the [Post](../vs140/mailboxprocessor.post--msg--method--fsharp-.md) function, so you must catch the timeout exception in the message processor function. In this example, the timeout exception just causes the loop to continue, and increases the message number by 1.  
