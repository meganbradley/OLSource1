...
1.  To convert the synchronous solution to an asynchronous solution, the best place to start is in `GetURLContents` because the calls to the <xref:System.Net.HttpWebRequest*> method <xref:System.Net.HttpWebRequest.GetResponse*> and to the <xref:System.IO.Stream*> method [CopyTo](assetId:///M:System.IO.Stream.CopyTo(System.IO.Stream)?qualifyHint=False&autoUpgrade=True) are where the application accesses the web. The .NET Framework makes the conversion easy by supplying asynchronous versions of both methods.  
...
2.  `GetResponseAsync` returns a <xref:System.Threading.Tasks.Task`1*>. In this case, the *task return variable*, `TResult`, has type <xref:System.Net.WebResponse*>. The task is a promise to produce an actual `WebResponse` object after the requested data has been downloaded and the task has run to completion.  
     ...
     To retrieve the `WebResponse` value from the task, apply an [Await](../vs140/await-operator--visual-basic-.md) operator to the call to `GetResponseAsync`, as the following code shows.  
...
3.  Because you added the `Await` operator in the previous step, a compiler error occurs. The operator can be used only in methods that are marked with the [Async](../vs140/async--visual-basic-.md) modifier. Ignore the error while you repeat the conversion steps to replace the call to `CopyTo` with a call to `CopyToAsync`.  
  
    -   Change the name of the method that’s called to [CopyToAsync](assetId:///M:System.IO.Stream.CopyToAsync(System.IO.Stream)?qualifyHint=False&autoUpgrade=True).  
...
4.  All that remains to be done in `GetURLContents` is to adjust the method signature. You can use the `Await` operator only in methods that are marked with the [Async](../vs140/async--visual-basic-.md) modifier. Add the modifier to mark the method as an *async method*, as the following code shows.  
