...
Attributes in an XML document literal are specified in an invalid order. Valid attribute order for an XML document literal is based on the XML 1.0 specification. For example, the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> attribute of an XML document literal must immediately follow the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> attribute.  
