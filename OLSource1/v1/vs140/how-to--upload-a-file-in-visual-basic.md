...
The <xref:Microsoft.VisualBasic.Devices.Network.UploadFile?qualifyHint=False> method can be used to upload a file and store it to a remote location. If the `ShowUI` parameter is set to `True`, a dialog box is displayed that shows the progress of the download and allows users to cancel the operation.  
...
-   The local file path is not valid (<xref:System.ArgumentException?qualifyHint=False>).  
  
-   Authentication failed (<xref:System.Security.SecurityException?qualifyHint=False>).  
  
-   The connection timed out (<xref:System.TimeoutException?qualifyHint=False>).  
  
## See Also  
 <xref:Microsoft.VisualBasic.Devices.Network?qualifyHint=True>   
