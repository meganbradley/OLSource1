...
### Group Join Example  
 The following example performs a group join of objects of type <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> based on the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> matching the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> property. Unlike a non-group join, which would produce a pair of elements for each match, the group join produces only one resulting object for each element of the first collection, which in this example is a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> object. The corresponding elements from the second collection, which in this example are <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> objects, are grouped into a collection. Finally, the result selector function creates an anonymous type for each match that consists of <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> and a collection of <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> objects.  
...
-   Copy and paste the code from the example into the program.cs file, below the <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> method. Add a line of code to the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> method to call the method you pasted in.  
