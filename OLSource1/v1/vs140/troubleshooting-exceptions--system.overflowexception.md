...
## Remarks  
 In languages that detect overflow, \<xref:System.OverflowException*> is the exception that is thrown when overflow occurs. For example, in C#, the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> keyword is used to detect overflow conditions. An \<xref:System.OverflowException*> exception occurs only in a checked context.  
