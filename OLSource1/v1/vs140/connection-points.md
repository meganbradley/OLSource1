...
---
# Connection Points
This article explains how to implement connection points (formerly known as OLE connection points) using the MFC classes <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>.  
...
An Implemented Connection Point  
 ...
 MFC implements this model in the [CConnectionPoint](../vs140/cconnectionpoint-class.md) and [CCmdTarget](../vs140/ccmdtarget-class.md) classes. Classes derived from **CConnectionPoint** implement the **IConnectionPoint** interface, used to expose connection points to other objects. Classes derived from <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> implement the **IConnectionPointContainer** interface, which can enumerate all of an object's available connection points or find a specific connection point.  
 ...
 The following examples demonstrate a simple connection map and one connection point. The first example declares the connection map and point; the second example implements the map and point. Note that <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> must be a <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>-derived class. In the first example, code is inserted in the class declaration, under the **protected** section:  
  
 [!code[NVC_MFCConnectionPoints#1](../vs140/codesnippet/CPP/connection-points_1.h)]  
  
 The <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> and **END_CONNECTION_PART** macros declare an embedded class, <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> (derived from <CodeContentPlaceHolder>7\</CodeContentPlaceHolder>), that implements this particular connection point. If you want to override any <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> member functions or add member functions of your own, declare them between these two macros. For example, the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> macro overrides the <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> member function when placed between these two macros.  
  
 In the second example, code is inserted in the control's implementation file (.cpp file). This code implements the connection map, which includes the connection point, <CodeContentPlaceHolder>11\</CodeContentPlaceHolder>:  
  
 [!code[NVC_MFCConnectionPoints#2](../vs140/codesnippet/CPP/connection-points_2.cpp)]  
  
 If your class has more than one connection point, insert additional <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> macros between the <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> macros.  
  
 Finally, add a call to <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> in the class's constructor. For example:  
  
 [!code[NVC_MFCConnectionPoints#3](../vs140/codesnippet/CPP/connection-points_3.cpp)]  
  
 Once this code has been inserted, your <CodeContentPlaceHolder>16\</CodeContentPlaceHolder>-derived class exposes a connection point for the **ISampleSink** interface. The following figure illustrates this example.  
...
A Connection Point Implemented with MFC  
 ...
 This example retrieves the current set of connections on the <CodeContentPlaceHolder>17\</CodeContentPlaceHolder> connection point with a call to <CodeContentPlaceHolder>18\</CodeContentPlaceHolder>. It then iterates through the connections and calls **ISampleSink::SinkFunc** on every active connection.  
