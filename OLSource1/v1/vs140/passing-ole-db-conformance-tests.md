...
>  You need to add several validation functions for your provider to pass the OLE DB conformance tests.  
 ...
 This provider requires two validation routines. The first routine, <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, is part of your rowset class. It is called during the creation of the rowset by the provider templates. The sample uses this routine to tell consumers that it does not support indexes. The first call is to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> (note that the provider uses the **_RowsetBaseClass** typedef added in the interface map for <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> in [Provider Support for Bookmarks](../vs140/provider-support-for-bookmarks.md), so you do not have to type that long line of template arguments). Next, return **DB_E_NOINDEX** if the index parameter is not **NULL** (this indicates the consumer wants to use an index on us). For more information about command IDs, see the OLE DB specification and look for **IOpenRowset::OpenRowset**.  
  
 The following code is the **ValidateCommandID** validation routine:  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 The provider templates call the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> method whenever someone changes a property on the **DBPROPSET_ROWSET** group. If you want to handle properties for other groups, you add them to the appropriate object (that is, **DBPROPSET_SESSION** checks go in the <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> class).  
 ...
 You might also want to add the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> routine to your code. The templates call <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> when attempting to set a property. You would override this method if you require additional processing when setting a property value. You can have one of these methods for each property set.  
...
-   In your provider project, search for all instances of <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> and replace it with <CodeContentPlaceHolder>9\</CodeContentPlaceHolder>, which should be in your data source, session, and rowset headers.  
