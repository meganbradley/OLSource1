...
|**Multiplicity**|1|Input Pin, Output Pin|-   Can be a single value, or a range <CodeContentPlaceHolder>0\</CodeContentPlaceHolder>.\<br />-   Lower bound <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> - the action cannot start (for an input pin) or stop (for an output pin) until there are <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> objects waiting on the pin.\<br />-   Upper bound <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> - The action cannot consume or produce more than <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> objects in one execution. * means that there is no limit.|  
