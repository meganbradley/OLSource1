...
|**–**[<CodeContentPlaceHolder>0\</CodeContentPlaceHolder> ]*command*|Turns off error checking for *command*. By default, NMAKE halts when a command returns a nonzero exit code. If –<CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is used, NMAKE stops if the exit code exceeds <CodeContentPlaceHolder>2\</CodeContentPlaceHolder>. Spaces or tabs cannot appear between the dash and *number.* At least one space or tab must appear between <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> and *command*. Use /I to turn off error checking for the entire makefile; use **.IGNORE** to turn off error checking for part of the makefile.|  
