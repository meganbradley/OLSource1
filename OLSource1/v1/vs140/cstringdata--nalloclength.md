---
title: "CStringData::nAllocLength"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "reference"
f1_keywords: 
  - "CStringData.nAllocLength"
  - "ATL::CStringData::nAllocLength"
  - "CStringData::nAllocLength"
  - "ATL.CStringData.nAllocLength"
  - "nAllocLength"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "nAllocLength data member"
ms.assetid: 3a553ccb-1fcb-4bdd-9fdc-d1e33fe4f32e
caps.latest.revision: 13
robots: noindex,nofollow
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# CStringData::nAllocLength
Length of the allocated character buffer.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Remarks  
 Stores the length of the allocated data buffer in <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>s (not including terminating null).  
  
## Requirements  
 **Header:** atlsimpstr.h  
  
## See Also  
 [CStringData Class](../vs140/cstringdata-class.md)   
 [CStringData::nDataLength](../vs140/cstringdata--ndatalength.md)