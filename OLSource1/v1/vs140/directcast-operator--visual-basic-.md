...
|`DirectCast`|Any data types|One type must inherit from or implement the other type|Throws <xref:System.InvalidCastException?qualifyHint=False>|  
...
## Example  
 ...
 In the preceding example, the run-time type of `q` is `Double`. `CType` succeeds because `Double` can be converted to `Integer`. However, the first `DirectCast` fails at run time because the run-time type of `Double` has no inheritance relationship with `Integer`, even though a conversion exists. The second `DirectCast` succeeds because it converts from type <xref:System.Windows.Forms.Form?qualifyHint=False> to type <xref:System.Windows.Forms.Control?qualifyHint=False>, from which <xref:System.Windows.Forms.Form?qualifyHint=False> inherits.  
  
## See Also  
 <xref:System.Convert.ChangeType?qualifyHint=True>   
