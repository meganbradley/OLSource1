...
-   The <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> keyword is not allowed on an explicit interface declaration. In this case, remove the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> keyword from the explicit interface declaration.  
...
-   The [abstract](../vs140/abstract--csharp-reference-.md) keyword is not allowed on an explicit interface declaration because an explicit interface implementation can never be overridden.  
 ...
 In prior releases of Visual Studio, the <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> modifier was not permitted on a class, but <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> classes are allowed starting with [!INCLUDE[vsprvsext](../vs140/includes/vsprvsext_md.md)].  
