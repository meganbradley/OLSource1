...
## Example  
 In this example, the class uses one instance of the \<xref:System.ComponentModel.EventHandlerList*> class, stored in the <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> field, to store information about the events in use. The \<xref:System.ComponentModel.EventHandlerList*> class is an optimized list class designed to hold delegates.  
  
 All events in the class use the <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> field to keep track of what methods are handling each event.  
