...
|Param|These parts are optional. You can add as many as needed.|  
 ...
 The parameters enable the .vsz file to pass additional custom parameters to the wizard. Each value is passed as a string element in an array of variants to the wizard. For more information, see [Custom Parameters](../vs140/custom-parameters.md). For information about how to use a .vsz file in the development of custom wizards, see [.Vsz File (Project Control)](../vs140/.vsz-file--project-control-.md)  
  
 To add a default locale ID to your .vsz file, specify <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>=xxxx, where xxxx is the locale ID, for example, 1033 for English. When <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> parameter is defined, the wizard uses the supplied fallback locale ID if the current ID is not found.  
