...
The service control manager (SCM) calls <CodeContentPlaceHolder>0\</CodeContentPlaceHolder> when you open the Services Control Panel application, select the service, and click **Start**.  
 ...
 <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> then calls <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, which calls the Win32 API function [CoInitializeEx](http://msdn.microsoft.com/library/windows/desktop/ms695279). By default, <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> passes the **COINIT_MULTITHREADED** flag to the function. This flag indicates that the program is to be a free-threaded server.  
  
 Now, <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> is called to perform the main work of the service. **Run** continues to execute until the service is stopped.  
