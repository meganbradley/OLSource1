---
title: "CMFCShellListCtrl::Refresh"
ms.custom: na
ms.date: "09/22/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "reference"
f1_keywords: 
  - "Refresh"
  - "CMFCShellListCtrl::Refresh"
  - "CMFCShellListCtrl.Refresh"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "Refresh method"
ms.assetid: 43ab3b02-4c4d-4435-822b-9efacc8329e6
caps.latest.revision: 14
robots: noindex,nofollow
translation.priority.ht: 
  - "de-de"
  - "ja-jp"
---
# CMFCShellListCtrl::Refresh
Refreshes and repaints the [CMFCShellListCtrl](../vs140/cmfcshelllistctrl-class.md) object.  
  
## Syntax  
  
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
## Return Value  
 <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> if successful; otherwise an error value.  
  
## Remarks  
 Call this method to refresh the list of items displayed by the <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> object.  
  
## Requirements  
 **Header:** afxshelllistctrl.h  
  
## See Also  
 [CMFCShellListCtrl Class](../vs140/cmfcshelllistctrl-class.md)   
 [Hierarchy Chart (MFC Feature Pack)](../vs140/hierarchy-chart.md)