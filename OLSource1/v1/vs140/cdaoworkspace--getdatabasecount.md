...
## Remarks  
 <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> is useful if you need to loop through all defined databases in the workspace's Databases collection. To obtain information about a given database in the collection, see [GetDatabaseInfo](../vs140/cdaoworkspace--getdatabaseinfo.md). Typical usage is to call <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> for the number of open databases, then use that number as a loop index for repeated calls to <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>.  
