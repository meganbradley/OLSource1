...
## Return Value  
 If an undo action is in the control's undo queue, <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> returns the [UNDONAMEID](http://msdn.microsoft.com/library/windows/desktop/bb774365) enumeration type indicating the type of the next action in the queue. If the undo queue is empty, or if the undo action in the queue is of an unknown type, <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> returns 0.  
