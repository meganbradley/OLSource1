...
> [!IMPORTANT]
>  In many cases you can simply transfer your add-in code to a VSIX project with a VSPackage project item. You can get the DTE automation object by calling <xref:Microsoft.VisualStudio.Shell.Package.GetService?qualifyHint=False> in the <xref:Microsoft.VisualStudio.Shell.Package.Initialize?qualifyHint=False> method.  
>   
...
4.  Find the `MenuItemCallback` method. Add a call to <xref:Microsoft.VisualStudio.Shell.Package.GetService?qualifyHint=False> to get the <xref:EnvDTE80.DTE2?qualifyHint=False> object:  
...
5.  In the experimental instance, open a project. The VSPackage should load, and your breakpoint should be hit.  
 ...
 You can specify other contexts in which to load your VSPackage by using the fields of <xref:Microsoft.VisualStudio.Shell.Interop.UIContextGuids80?qualifyHint=False>. For more information, see [How to: Autoload a VSPackage](../vs140/loading-vspackages.md).  
...
1.  In a VSIX project with a Visual Studio Package item template, look for the *<project name\>*Package.cs file. This is the class that derives from <xref:Microsoft.VisualStudio.Shell.Package?qualifyHint=False>; it can help you interact with Visual Studio. In this case, you use its <xref:Microsoft.VisualStudio.Shell.Package.GetService?qualifyHint=False> to get the <xref:EnvDTE80.DTE2?qualifyHint=False> object.  
...
3.  Find the `Initialize` method. This method handles the command you specified in the package wizard. Add a call to <xref:Microsoft.VisualStudio.Shell.Package.GetService?qualifyHint=False> to get the DTE object:  
  
    ```c#  
    DTE dte = (DTE)GetService(typeof(DTE));  
    ```  
  
 After you have the <xref:EnvDTE.DTE?qualifyHint=False> automation object, you can add the rest of your add-in code to the project. If you need the <xref:EnvDTE80.DTE2?qualifyHint=False> object, you can do the same thing.  
