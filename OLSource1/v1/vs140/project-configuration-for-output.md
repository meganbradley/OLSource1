...
Output groups  
 ...
 Note that if a project has an `IVsOutputGroup` that it does not want to package or deploy, it is sufficient to not put that output in a group. The output can still be enumerated normally by implementing the [EnumOutputs](assetId:///M:Microsoft.VisualStudio.Shell.Interop.IVsProjectCfg.EnumOutputs(Microsoft.VisualStudio.Shell.Interop.IVsEnumOutputs@)?qualifyHint=False&autoUpgrade=True) method that returns all of a configuration's outputs regardless of grouping.  
