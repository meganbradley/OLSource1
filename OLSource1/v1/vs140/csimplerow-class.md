...
## Remarks  
 A row handle is logically a unique tag for a result row. <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> creates a new <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> for every row requested in [IRowsetImpl::GetNextRows](../vs140/irowsetimpl--getnextrows.md). <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> can also be replaced with your own implementation of the row handle, as it is a default template argument to <CodeContentPlaceHolder>4\</CodeContentPlaceHolder>. The only requirement to replacing this class is to have the replacement class provide a constructor that accepts a single parameter of type **LONG**.  
