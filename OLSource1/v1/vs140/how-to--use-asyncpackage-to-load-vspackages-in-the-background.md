...
Loading and initializing a VS package can result in disk I/O. If such I/O happens on the UI thread, it can lead to responsiveness issues. To address this, Visual Studio 2015 introduced the  <xref:Microsoft.VisualStudio.Shell.AsyncPackage?qualifyHint=False> class that enables package loading on a background thread.  
...
1.  Derive the package from <xref:Microsoft.VisualStudio.Shell.AsyncPackage?qualifyHint=False>.  
  
2.  If you are providing services whose querying may cause your package to load:  
  
     To indicate to Visual Studio that your package is safe for background loading and to opt into this behavior, your <xref:Microsoft.VisualStudio.Shell.PackageRegistrationAttribute?qualifyHint=False> should set **AllowsBackgroundLoading** property to true in the attribute constructor.  
    ...
    ```  
     ...
     To indicate to Visual Studio that it is safe to instantiate your service on a background thread, you should set the <xref:Microsoft.VisualStudio.Shell.ProvideServiceAttributeBase.IsAsyncQueryable?qualifyHint=False> property to true in the <xref:Microsoft.VisualStudio.Shell.ProvideServiceAttribute?qualifyHint=False> constructor.  
...
3.  If you are loading via UI contexts, then you should specify **PackageAutoLoadFlags.BackgroundLoad** for the <xref:Microsoft.VisualStudio.Shell.ProvideAutoLoadAttribute?qualifyHint=False> OR the value (0x2) into the flags written as the value of your package’s auto-load entry.  
...
4.  If you have asynchronous initialization work to do, you should override <xref:Microsoft.VisualStudio.Shell.AsyncPackage.InitializeAsync?qualifyHint=False>. Remove the **Initialize()** method provided by the VSIX template. (The **Initialize()** method in **AsyncPackage** is sealed). You can use any of the <xref:Microsoft.VisualStudio.Shell.AsyncPackage.AddService?qualifyHint=False> methods to add asynchronous services to your package.  
