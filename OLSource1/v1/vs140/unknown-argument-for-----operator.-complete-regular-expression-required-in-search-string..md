...
This error generally occurs during search or replace when regular expressions or wildcards are used in a search string. The operator colon (<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>) was entered but the argument that followed was not a valid argument.  
  
> [!NOTE]
>  Arguments for the colon (<CodeContentPlaceHolder>1\</CodeContentPlaceHolder>) operator are case-sensitive.  
