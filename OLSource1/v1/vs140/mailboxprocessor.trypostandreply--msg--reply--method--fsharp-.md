...
Like [MailboxProcessor.PostAndReply](../vs140/mailboxprocessor.postandreply--msg--reply--method--fsharp-.md), but returns <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> if there is no reply within the timeout period.  
...
## Return Value  
 The reply from the agent or <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> if the timeout expires.  
  
## Remarks  
  
## Example  
 The following example shows how to use <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>. In this example, the agent has a delay that eventually results in a timeout.  
