...
##  \<a name="vccongenerating_an_object">\</a> Generating a New Simple Object  
 Create a new project, ensuring that the Attributes box under Application Settings is cleared. Use the ATL Add Class dialog box and Add Simple Object Wizard to generate a Simple Object called <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>. Make sure that a dual interface called <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> is generated. Objects of the generated class will be used to represent a collection of words (that is, strings).  
  
##  \<a name="vcconedit_the_idl">\</a> Editing the IDL File  
 Now, open the IDL file and add the three properties necessary to turn <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> into a read-only collection interface, as shown below:  
...
1.  Collection interfaces are usually dual because Automation clients accesses the <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> property via **IDispatch::Invoke**. However, Automation clients can access the remaining methods via the vtable, so dual interfaces are preferable to dispinterfaces.  
...
##  \<a name="vcconstorage_and_exposure_typedefs">\</a> Creating Typedefs for Storage and Exposure  
 ...
 Since Visual Basic support is vital to the success of this interface, the enumerator returned by the <CodeContentPlaceHolder>6\</CodeContentPlaceHolder> property must support the **IEnumVARIANT** interface. This is the only enumerator interface understood by Visual Basic.  
...
##  \<a name="vcconcopy_classes">\</a> Creating Typedefs for Copy Policy Classes  
 ...
 In this example, you can use the custom <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> class defined in VCUE_Copy.h and VCUE_CopyString.h from the [ATLCollections](../vs140/visual-c---samples.md) sample. You can use this class in other code, but you may need to define further specializations of <CodeContentPlaceHolder>8\</CodeContentPlaceHolder> to support data types used in your own collections. For more information, see [ATL Copy Policy Classes](../vs140/atl-copy-policy-classes.md).  
  
##  \<a name="vcconenumeration_and_collection">\</a> Creating Typedefs for Enumeration and Collection  
 Now all the template parameters necessary to specialize the <CodeContentPlaceHolder>9\</CodeContentPlaceHolder> and <CodeContentPlaceHolder>10\</CodeContentPlaceHolder> classes for this situation have been provided in the form of typedefs. To simplify the use of the specializations, create two more typedefs as shown below:  
  
 [!code[NVC_ATL_COM#27](../vs140/codesnippet/CPP/implementing-an-stl-based-collection_4.h)]  
  
 Now <CodeContentPlaceHolder>11\</CodeContentPlaceHolder> is a synonym for a specialization of <CodeContentPlaceHolder>12\</CodeContentPlaceHolder> that implements the <CodeContentPlaceHolder>13\</CodeContentPlaceHolder> interface defined earlier and provides an enumerator that supports **IEnumVARIANT**.  
  
##  \<a name="vcconedit_the_generated_code">\</a> Editing the Wizard-Generated Code  
 Now you must derive <CodeContentPlaceHolder>14\</CodeContentPlaceHolder> from the interface implementation represented by the <CodeContentPlaceHolder>15\</CodeContentPlaceHolder> typedef rather than <CodeContentPlaceHolder>16\</CodeContentPlaceHolder>, as shown below:  
