...
<CodeContentPlaceHolder>0\</CodeContentPlaceHolder>  
 This function uses a second recordset, <CodeContentPlaceHolder>1\</CodeContentPlaceHolder>, which contains a record for each course offered, and a <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> control, <CodeContentPlaceHolder>3\</CodeContentPlaceHolder>, which is stored in the record view class.  
  
 The function gets <CodeContentPlaceHolder>4\</CodeContentPlaceHolder> from the document and opens it. Then it empties <CodeContentPlaceHolder>5\</CodeContentPlaceHolder> and scrolls through <CodeContentPlaceHolder>6\</CodeContentPlaceHolder>. For each record, the function calls the combo box's <CodeContentPlaceHolder>7\</CodeContentPlaceHolder> member function to add the course ID value from the record. Finally, the code sets the combo box's selection.  
