...
## How to Fix Violations  
 To fix a violation of this rule, add a call to \<xref:System.GC.KeepAlive*> to the method, passing the current instance (<CodeContentPlaceHolder>0\</CodeContentPlaceHolder> in C# and C++) as the argument. Position the call after the last line of code where the object must be protected from garbage collection. Immediately after the call to \<xref:System.GC.KeepAlive*>, the object is again considered ready for garbage collection assuming that there are no managed references to it.  
...
## Example  
 In the following example, <CodeContentPlaceHolder>1\</CodeContentPlaceHolder> does not include a call to <CodeContentPlaceHolder>2\</CodeContentPlaceHolder> and therefore violates the rule. <CodeContentPlaceHolder>3\</CodeContentPlaceHolder> contains the corrected code.  
